 ...
 * @author vladimir
 */
class RM_Admin_Menu_Hashes {
    
    const TASK_ID = 'rm_admin_menu_access_data_conversion';
    ...
    public static function calc_menu_item_id_old($menu_kind, $menu_item) {
        ...
        } elseif (strpos($menu_item, 'designs.php?page=custom-header')!==false) {
            $menu_item = 'custom-header';
        } elseif (strpos($menu_item, 'designs.php?page=custom-background')!==false) {
    ...
    private static function update_hash($prefix, $menu_item, &$menu_access_data) {
        ...
        foreach($menu_access_data as $role=>$access_data_for_role) {
            ...
            for($i=0; $i<count($data); $i++) {
                ...
                if ($data[$i]==$old_hash) {
                    $menu_access_data[$role]['data'][$i] = RM_Admin_Menu::calc_menu_item_id($prefix, $menu_item[3]);
                    if($old_hash===$customize_hash) {  // if top menu item 'Customize' blocked, mark as blocked all other related menu items
                        ...
                        $menu_access_data[$role]['data'] = self::add_value($menu_access_data[$role]['data'], 'e07dad6171f87801da401de77966ff87'); // Header - designs.php?page=custom-header
                        $menu_access_data[$role]['data'] = self::add_value($menu_access_data[$role]['data'], '5e52942b3efe461d923740a27c075872'); // Backgroudn - designs.php?page=custom-background
                    }
                }                
            }                        
    ...
    private static function convert_site() {
        ...
        RM_Admin_Menu_Copy::update();
        
        $menu_access_data = get_option(RM_Admin_Menu::ADMIN_MENU_ACCESS_DATA_KEY);
        if (empty($menu_access_data)) { // nothing to update
            return;
        }
        
        $current_menu = get_option(RM_Admin_Menu::ADMIN_MENU_COPY_KEY);                
        foreach($current_menu as $menu_item) {
            self::update_hash('menu', $menu_item, $menu_access_data);
        } 
        $current_submenu = get_option(RM_Admin_Menu::ADMIN_SUBMENU_COPY_KEY);
        foreach($current_submenu as $submenu) {
            foreach($submenu as $menu_item) {
                self::update_hash('submenu', $menu_item, $menu_access_data);
            }
        }
        update_option(RM_Admin_Menu::ADMIN_MENU_ACCESS_DATA_KEY, $menu_access_data);        
    }
    // end of convert_site()
    ...
    public static function convert() {
        ...
        $data_version = get_option(RM_Admin_Menu::DATA_VERSION_KEY, 0);
        if ($data_version>=RM_Admin_Menu::DATA_VERSION) {
            self::remove_task();
            return;
        }        
        ...
        update_option(RM_Admin_Menu::DATA_VERSION_KEY, RM_Admin_Menu::DATA_VERSION, true);
        self::remove_task();
    ...
    /**
     * Add coversion data task to the RM's tasks queue for the later execution by 'admin_menu' action
     * 
     * @param RM_Task_Queue $task_queue
     */
    private static function require_site_data_conversion($task_queue) {
        
        $args = array('action'=>'admin_menu', 'routine'=>'RM_Admin_Menu_Hashes::convert', 'priority'=>99);
    ...
    /**
     * Convert admin menu data if we update from an older RM Pro version     
     */
...
// end of RM_Admin_Menu_Hashes class
