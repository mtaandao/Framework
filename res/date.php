 ...
 *
 * MN_Date_Query is a helper that allows primary query classes, such as MN_Query, to filter
...
class MN_Date_Query {
	 ...
	 *
	 * See MN_Date_Query::__construct() for information on date query arguments.
	 *
	 * @since 3.7.0
	 * @access public
	 * @var array
	...
	public function validate_date_values( $date_query = array() ) {
		...
		if ( $day_exists && $month_exists && $year_exists ) {
			...
			if ( ! mn_checkdate( $date_query['month'], $date_query['day'], $date_query['year'], sprintf( '%s-%s-%s', $date_query['year'], $date_query['month'], $date_query['day'] ) ) ) {
			 ...
			 */
			if ( ! mn_checkdate( $date_query['month'], $date_query['day'], 2012, sprintf( '2012-%s-%s', $date_query['month'], $date_query['day'] ) ) ) {
	 ...
	 * known tables, and then, if found, have a table prefix (such as 'mn_posts.') prepended.
	 * Prefixed column names (such as 'mn_posts.post_date') bypass this whitelist check,
	...
	public function get_sql() {
		 ...
		 * @param MN_Date_Query $this  The MN_Date_Query instance.
		 */
	 ...
	 *
	 * Called by the public MN_Date_Query::get_sql(), this method is abstracted
	...
	protected function get_sql_for_clause( $query, $parent_query ) {
		...
		if ( isset( $query['week'] ) && false !== ( $value = $this->build_value( $compare, $query['week'] ) ) ) {
			$where_parts[] = _mn_mysql_week( $column ) . " $compare $value";
	...
	public function build_time_query( $column, $compare, $hour = null, $minute = null, $second = null ) {
		...
		return $mndb->prepare( "DATE_FORMAT( $column, %s ) $compare %f", $format, $time );
	}
}
