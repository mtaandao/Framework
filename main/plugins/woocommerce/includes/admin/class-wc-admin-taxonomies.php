...
class WC_Admin_Taxonomies {
	...
	public function __construct() {
		...
		add_filter( 'mn_terms_checklist_args', array( $this, 'disable_checked_ontop' ) );
	}
	...
	public function delete_term( $term_id ) {
		...
		if ( $term_id && get_option( 'db_version' ) < 34370 ) {
			$mndb->delete( $mndb->woocommerce_termmeta, array( 'woocommerce_term_id' => $term_id ), array( '%d' ) );
		}
	}
		...
		<div class="form-field term-thumbnail-wrap">
			...
			<script type="text/javascript">
				...
				jQuery( document ).on( 'click', '.upload_image_button', function( event ) {
					...
					file_frame = mn.media.frames.downloadable_file = mn.media({
				...
				jQuery( document ).ajaxComplete( function( event, request, options ) {
					if ( request && 4 === request.readyState && 200 === request.status
						...
						var res = mnAjax.parseAjaxResponse( request.responseXML, 'ajax-response' );
						if ( ! res || res.errors ) {
							return;
						}
	...
	public function edit_category_fields( $term ) {
		...
		if ( $thumbnail_id ) {
			$image = mn_get_attachment_thumb_url( $thumbnail_id );
		} else {
	...
	public function product_cat_description() {
		echo mnautop( __( 'Product categories for your store can be managed here. To change the order of categories on the front-end you can drag and drop to sort them. To see more categories listed click the "screen options" link at the top of the page.', 'woocommerce' ) );
	}
	...
	public function product_attribute_description() {
		echo mnautop( __( 'Attribute terms can be assigned to products and variations.<br/><br/><b>Note</b>: Deleting a term will remove it from all products and variations to which it has been assigned. Recreating a term will not automatically assign it back to products.', 'woocommerce' ) );
	}
	...
	public function product_cat_column( $columns, $column, $id ) {
		...
		if ( 'thumb' == $column ) {
			...
			$columns .= '<img src="' . esc_url( $image ) . '" alt="' . esc_attr__( 'Thumbnail', 'woocommerce' ) . '" class="mn-post-image" height="48" width="48" />';
