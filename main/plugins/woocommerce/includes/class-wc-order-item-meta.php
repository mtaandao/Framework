	...
	public function display( $flat = false, $return = false, $hideprefix = '_', $delimiter = ", \n" ) {
		...
		if ( ! empty( $formatted_meta ) ) {
			...
			foreach ( $formatted_meta as $meta ) {
				if ( $flat ) {
					$meta_list[] = mn_kses_post( $meta['label'] . ': ' . $meta['value'] );
				} else {
					$meta_list[] = '
						<dt class="variation-' . sanitize_html_class( sanitize_text_field( $meta['key'] ) ) . '">' . mn_kses_post( $meta['label'] ) . ':</dt>
						<dd class="variation-' . sanitize_html_class( sanitize_text_field( $meta['key'] ) ) . '">' . mn_kses_post( mnautop( make_clickable( $meta['value'] ) ) ) . '</dd>
					';
				}
			}
		...
		if ( ! empty( $this->item['item_meta_array'] ) ) {
			foreach ( $this->item['item_meta_array'] as $meta_id => $meta ) {
				...
				if ( taxonomy_exists( $attribute_key ) ) {
					...
					if ( ! is_mn_error( $term ) && is_object( $term ) && $term->name ) {
