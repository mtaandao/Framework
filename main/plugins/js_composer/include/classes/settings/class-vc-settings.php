 ...
 * Settings are build with MN settings API and organized as tabs.
 *
 * List of tabs
 ...
 * 3. Custom CSS - add custom css to your MN pages.
...
class Vc_Settings {
	...
	public function initAdmin() {
		...
		if ( 'restore_color' === vc_post_param( 'vc_action' ) && vc_user_access()
				->check( 'mn_verify_nonce', vc_post_param( '_mnnonce' ), vc_settings()->getOptionGroup() . '_color' . '-options' )// see settings_fields() function
				->validateDie()->mnAny( 'manage_options' )->validateDie()->part( 'settings' )->can( 'vc-color-tab' )->validateDie()->get()
		) {
			$this->restoreColor();
		}
	...
	function adminLoad() {
		mn_register_script( 'wpb_js_composer_settings', vc_asset_url( 'js/dist/settings.min.js' ), array(), WPB_VC_VERSION, true );
		mn_enqueue_style( 'js_composer_settings', vc_asset_url( 'css/js_composer_settings.min.css' ), false, WPB_VC_VERSION, false );
		mn_enqueue_script( 'backbone' );
		mn_enqueue_script( 'shortcode' );
		mn_enqueue_script( 'underscore' );
		mn_enqueue_script( 'jquery-ui-accordion' );
		mn_enqueue_script( 'jquery-ui-sortable' );
		mn_enqueue_script( 'wpb_js_composer_settings' );
		$this->locale = array(
		...
		mn_localize_script( 'wpb_js_composer_settings', 'vcData', apply_filters( 'vc_global_js_data', array(
			'version' => WPB_VC_VERSION,
			'debug' => wpb_debug(),
		) ) );
		mn_localize_script( 'wpb_js_composer_settings', 'i18nLocaleSettings', $this->locale );
	}
	...
	public function groups_access_rules_callback() {
		...
		global $mn_roles;
		$groups = is_object( $mn_roles ) ? $mn_roles->roles : array();
	...
	public static function buildCustomColorCss() {
		 ...
		 * Filesystem API init.
		 * */
		$url = mn_nonce_url( 'admin.php?page=vc-color&build_css=1', 'wpb_js_settings_save_action' );
		 ...
		 *
		 */
		if ( false === ( $js_composer_upload_dir = self::checkCreateUploadDir( $mn_filesystem, 'use_custom', 'js_composer_front_custom.css' ) ) ) {
			return;
		}
		...
		if ( ! $use_custom ) {
			$mn_filesystem->put_contents( $filename, '', FS_CHMOD_FILE );
		...
		if ( strlen( trim( $css_string ) ) > 0 ) {
			...
			// HERE goes the magic
			if ( ! $mn_filesystem->put_contents( $filename, $css_string, FS_CHMOD_FILE ) ) {
				if ( is_mn_error( $mn_filesystem->errors ) && $mn_filesystem->errors->get_error_code() ) {
					add_settings_error( self::$field_prefix . 'main_color', $mn_filesystem->errors->get_error_code(), __( 'Something went wrong: js_composer_front_custom.css could not be created.', 'js_composer' ) . ' ' . $mn_filesystem->errors->get_error_message(), 'error' );
				} elseif ( ! $mn_filesystem->connect() ) {
					add_settings_error( self::$field_prefix . 'main_color', $mn_filesystem->errors->get_error_code(), __( 'js_composer_front_custom.css could not be created. Connection error.', 'js_composer' ), 'error' );
				} elseif ( ! $mn_filesystem->is_writable( $filename ) ) {
					add_settings_error( self::$field_prefix . 'main_color', $mn_filesystem->errors->get_error_code(), sprintf( __( 'js_composer_front_custom.css could not be created. Cannot write custom css to "%s".', 'js_composer' ), $filename ), 'error' );
				} else {
					add_settings_error( self::$field_prefix . 'main_color', $mn_filesystem->errors->get_error_code(), __( 'js_composer_front_custom.css could not be created. Problem with access.', 'js_composer' ), 'error' );
				}
				delete_option( self::$field_prefix . 'use_custom' );
				delete_option( self::$field_prefix . 'less_version' );
			}
	...
	public static function buildCustomCss() {
		 ...
		 * Building css file.
		 */
		if ( false === ( $js_composer_upload_dir = self::checkCreateUploadDir( $mn_filesystem, 'custom_css', 'custom.css' ) ) ) {
			return true;
		}
		...
		if ( ! $mn_filesystem->put_contents( $filename, $css_string, FS_CHMOD_FILE ) ) {
			if ( is_mn_error( $mn_filesystem->errors ) && $mn_filesystem->errors->get_error_code() ) {
				add_settings_error( self::$field_prefix . 'custom_css', $mn_filesystem->errors->get_error_code(), __( 'Something went wrong: custom.css could not be created.', 'js_composer' ) . $mn_filesystem->errors->get_error_message(), 'error' );
			} elseif ( ! $mn_filesystem->connect() ) {
				add_settings_error( self::$field_prefix . 'custom_css', $mn_filesystem->errors->get_error_code(), __( 'custom.css could not be created. Connection error.', 'js_composer' ), 'error' );
			} elseif ( ! $mn_filesystem->is_writable( $filename ) ) {
				add_settings_error( self::$field_prefix . 'custom_css', $mn_filesystem->errors->get_error_code(), __( 'custom.css could not be created. Cannot write custom css to "' . $filename . '".', 'js_composer' ), 'error' );
			} else {
				add_settings_error( self::$field_prefix . 'custom_css', $mn_filesystem->errors->get_error_code(), __( 'custom.css could not be created. Problem with access.', 'js_composer' ), 'error' );
			}
	 ...
	 * @return bool|string
	 */
	public static function checkCreateUploadDir( $mn_filesystem, $option, $filename ) {
		$js_composer_upload_dir = self::uploadDir();
		if ( ! $mn_filesystem->is_dir( $js_composer_upload_dir ) ) {
			if ( ! $mn_filesystem->mkdir( $js_composer_upload_dir, 0777 ) ) {
				add_settings_error( self::$field_prefix . $option, $mn_filesystem->errors->get_error_code(), __( sprintf( '%s could not be created. Not available to create js_composer directory in uploads directory (' . $js_composer_upload_dir . ').', $filename ), 'js_composer' ), 'error' );
	...
	public static function uploadDir() {
		...
		return $mn_filesystem->find_folder( $upload_dir['basedir'] ) . vc_upload_dir();
	}
	...
	protected static function getFileSystem( $url = '' ) {
		if ( empty( $url ) ) {
			$url = mn_nonce_url( 'admin.php?page=vc-general', 'wpb_js_settings_save_action' );
		}
		...
		$upload_dir = mn_upload_dir();
		if ( ! MN_Filesystem( $creds, $upload_dir['basedir'] ) ) {
