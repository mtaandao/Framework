<?php
/**
 * Customize API: MN_Customize_Nav_Menu_Setting class
 *
 * @package Mtaandao
 ...
 * @since 4.3.0
 *
 * @see mn_get_nav_menu_object()
 * @see MN_Customize_Setting
 */
class MN_Customize_Nav_Menu_Setting extends MN_Customize_Setting {
	 ...
	 * @var int
	 *
	 * @see MN_Customize_Nav_Menu_Setting::update()
	 * @see MN_Customize_Nav_Menu_Setting::amend_customize_save_response()
	 */
	...
	/**
	 * Any error object returned by mn_update_nav_menu_object() when setting is updated.
	 *
	 * @since 4.3.0
	 * @access public
	 * @var MN_Error
	...
	public function value() {
		if ( $this->is_previewed && $this->_previewed_blog_id === get_current_blog_id() ) {
			...
			if ( $this->term_id > 0 ) {
				$term = mn_get_nav_menu_object( $this->term_id );

				if ( $term ) {
					$value = mn_array_slice_assoc( (array) $term, array_keys( $this->default ) );
	...
	public function preview() {
		...
		add_filter( 'mn_get_nav_menus', array( $this, 'filter_mn_get_nav_menus' ), 10, 2 );
		add_filter( 'mn_get_nav_menu_object', array( $this, 'filter_mn_get_nav_menu_object' ), 10, 2 );
	...
	/**
	 * Filters the mn_get_nav_menus() result to ensure the inserted menu object is included, and the deleted one is removed.
	 *
	 * @since 4.3.0
	 * @access public
	 *
	 * @see mn_get_nav_menus()
	 *
	 * @param array $menus An array of menu objects.
	 * @param array $args  An array of arguments used to retrieve menu objects.
	 * @return array
	 */
	public function filter_mn_get_nav_menus( $menus, $args ) {
	 ...
	 * @var string
	 *
	 * @see MN_Customize_Nav_Menu_Setting::filter_mn_get_nav_menus()
	 * @see MN_Customize_Nav_Menu_Setting::_sort_menus_by_orderby()
	 */
	...
	/**
	 * Filters the mn_get_nav_menu_object() result to supply the previewed menu object.
	 *
	 ...
	 * @see mn_get_nav_menu_object()
	 *
	 * @param object|null $menu_obj Object returned by mn_get_nav_menu_object().
	 * @param string      $menu_id  ID of the nav_menu term. Requests by slug or name will be ignored.
	 * @return object|null
	 */
	public function filter_mn_get_nav_menu_object( $menu_obj, $menu_id ) {
		$ok = (
	...
	public function sanitize( $value ) {
		...
		$value = mn_array_slice_assoc( $value, array_keys( $default ) );
	 ...
	 * @access protected
	 *
	 * @see mn_update_nav_menu_object()
	 *
	 * @param array|false $value {
	 *     The value to update. Note that slug cannot be updated via mn_update_nav_menu_object().
	...
	protected function update( $value ) {
		...
		if ( $is_delete ) {
			...
			if ( $is_placeholder ) {
				...
				$r = mn_delete_nav_menu( $this->term_id );
			...
			$menu_data = mn_array_slice_assoc( $value, array( 'description', 'parent' ) );
			$menu_data['menu-name'] = $value['name'];

			$menu_id = $is_placeholder ? 0 : $this->term_id;
			$r = mn_update_nav_menu_object( $menu_id, mn_slash( $menu_data ) );
			$original_name = $menu_data['menu-name'];
			$name_conflict_suffix = 1;
			while ( is_mn_error( $r ) && 'menu_exists' === $r->get_error_code() ) {
				$name_conflict_suffix += 1;
		...
		if ( 'inserted' === $this->update_status ) {
			...
			foreach ( array_keys( $this->manager->unsanitized_post_values() ) as $setting_id ) {
				...
				$widget_instance = $nav_menu_widget_setting->post_value(); // Note that this calls MN_Customize_Widgets::sanitize_widget_instance().
	 ...
	 * @access protected
	 *
	 * @see MN_Customize_Nav_Menu_Setting::filter_nav_menu_options()
	 * @see MN_Customize_Nav_Menu_Setting::update()
	 *
