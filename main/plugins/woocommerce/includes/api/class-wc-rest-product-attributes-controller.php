...
class WC_REST_Product_Attributes_Controller extends WC_REST_Controller {
	...
	public function create_item_permissions_check( $request ) {
		if ( ! wc_rest_check_manager_permissions( 'attributes', 'create' ) ) {
			return new MN_Error( 'woocommerce_rest_cannot_create', __( 'Sorry, you cannot create new resource.', 'woocommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function get_item_permissions_check( $request ) {
		if ( ! $this->get_taxonomy( $request ) ) {
			return new MN_Error( "woocommerce_rest_taxonomy_invalid", __( "Resource doesn't exist.", 'woocommerce' ), array( 'status' => 404 ) );
		}
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( ! wc_rest_check_manager_permissions( 'attributes', 'edit' ) ) {
			return new MN_Error( 'woocommerce_rest_cannot_update', __( 'Sorry, you cannot update resource.', 'woocommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( ! wc_rest_check_manager_permissions( 'attributes', 'delete' ) ) {
			return new MN_Error( 'woocommerce_rest_cannot_delete', __( 'Sorry, you cannot delete resource.', 'woocommerce' ), array( 'status' => rest_authorization_required_code() ) );
		}
	 ...
	 *
	 * @param MN_REST_Request $request Full details about the request.
	 * @return MN_REST_Request|MN_Error
	 */
	public function create_item( $request ) {
		...
		if ( is_mn_error( $insert ) ) {
			return new MN_Error( 'woocommerce_rest_cannot_create', $insert->get_error_message(), array( 'status' => 400 ) );
		}

		$attribute = $this->get_attribute( $mndb->insert_id );
	...
	public function update_item( $request ) {
		...
		// Checks for errors.
		if ( false === $update ) {
			return new MN_Error( 'woocommerce_rest_cannot_edit', __( 'Could not edit the attribute', 'woocommerce' ), array( 'status' => 400 ) );
		}
	...
	public function delete_item( $request ) {
		...
		if ( ! $force ) {
			return new MN_Error( 'woocommerce_rest_trash_not_supported', __( 'Resource does not support trashing.', 'woocommerce' ), array( 'status' => 501 ) );
		}
	 ...
	 * @param MN_REST_Request $request
	 * @return MN_REST_Response $response
	 */
	...
	protected function get_attribute( $id ) {
		...
		if ( is_mn_error( $attribute ) || is_null( $attribute ) ) {
			return new MN_Error( 'woocommerce_rest_attribute_invalid', __( "Resource doesn't exist.", 'woocommerce' ), array( 'status' => 404 ) );
		}
	...
	protected function validate_attribute_slug( $slug, $new_data = true ) {
		if ( strlen( $slug ) >= 28 ) {
			return new MN_Error( 'woocommerce_rest_invalid_product_attribute_slug_too_long', sprintf( __( 'Slug "%s" is too long (28 characters max).', 'woocommerce' ), $slug ), array( 'status' => 400 ) );
		} elseif ( wc_check_if_attribute_name_is_reserved( $slug ) ) {
			return new MN_Error( 'woocommerce_rest_invalid_product_attribute_slug_reserved_name', sprintf( __( 'Slug "%s" is not allowed because it is a reserved term.', 'woocommerce' ), $slug ), array( 'status' => 400 ) );
		} elseif ( $new_data && taxonomy_exists( wc_attribute_taxonomy_name( $slug ) ) ) {
			return new MN_Error( 'woocommerce_rest_invalid_product_attribute_slug_already_exists', sprintf( __( 'Slug "%s" is already in use.', 'woocommerce' ), $slug ), array( 'status' => 400 ) );
		}
