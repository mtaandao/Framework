 ...
 * @see MN_Upgrader
 */
class Language_Pack_Upgrader extends MN_Upgrader {
	 ...
	 * @since 3.7.0
	 * @access public
	 * @var array|MN_Error $result
	 * @see MN_Upgrader::$result
	 */
	public $result;
	 ...
	 * @static
	 *
	 * @param false|MN_Upgrader $upgrader Optional. MN_Upgrader instance or false. If `$upgrader` is
	 ...
	 * @return array|bool|MN_Error The result of the upgrade, or a MN_Error object instead.
	 */
	 ...
	 *                                    Default true.
	 * }
	 * @return array|bool|MN_Error Will return an array of results, or true if there are no updates,
	 *                                   false or MN_Error for initial errors.
	 */
	public function bulk_upgrade( $language_updates = array(), $args = array() ) {
		...
		// Remove any existing upgrade filters from the plugin/theme upgraders #MN29425 & #MN29230
		...
		/*
		 * The filesystem's mkdir() is not recursive. Make sure MN_LANG_DIR exists,
		 * as we then may need to create a /plugins or /themes directory inside of it.
		 */
		$remote_destination = $mn_filesystem->find_folder( MN_LANG_DIR );
		if ( ! $mn_filesystem->exists( $remote_destination ) )
			if ( ! $mn_filesystem->mkdir( $remote_destination, FS_CHMOD_DIR ) )
				return new MN_Error( 'mkdir_failed_lang_dir', $this->strings['mkdir_failed'], $remote_destination );
	...
	public function check_package( $source, $remote_source ) {
		...
		$files = $mn_filesystem->dirlist( $remote_source );
		...
		if ( ! $mo || ! $po ) {
			return new MN_Error( 'incompatible_archive_pomo', $this->strings['incompatible_archive'],
	...
	public function get_name_for_update( $update ) {
		switch ( $update->type ) {
			case 'core':
				return 'Mtaandao'; // Not translated

			case 'theme':
				$theme = mn_get_theme( $update->slug );
				if ( $theme->exists() )
