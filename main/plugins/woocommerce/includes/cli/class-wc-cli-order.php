...
class WC_CLI_Order extends WC_CLI_Command {
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc order create --customer_id=1 --status=pending ...
	 *
	 * @since  2.5.0
	 */
	public function create( $__, $assoc_args ) {
		...
		try {
			...
			if ( $porcelain ) {
				MN_CLI::line( $order->id );
			} else {
				MN_CLI::success( "Created order {$order->id}." );
			}
	...
	public function delete( $args, $assoc_args ) {
		...
		foreach ( $args as $id ) {
			...
			if ( ! $order ) {
				MN_CLI::warning( "Invalid order ID $id" );
				continue;
			}
			...
			if ( $r ) {
				MN_CLI::success( "Deleted order $id." );
			} else {
				$exit_code += 1;
				MN_CLI::warning( "Failed deleting order $id." );
			}
		}
		exit( $exit_code ? 1 : 0 );
	}
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc order get 123 --fields=id,title,sku
	 *
	 * @since 2.5.0
	 */
	 ...
	 * ## AVAILABLE FIELDS
	 *
	 * For available fields, see: mn wc order create --help
	 *
	 * ## EXAMPLES
	 *
	 *    mn wc order update 123 --status=completed
	 *
	 * @todo  gedex
	 * @since 2.5.0
	 */
	public function update( $args, $assoc_args ) {
		try {
			...
			MN_CLI::success( "Updated order {$order->id}." );
