 ...
 * @since 4.0.0
 */
abstract class MN_Session_Tokens {
	 ...
	 * the session manager for a subclass of MN_Session_Tokens.
	 *
	 * @since 4.0.0
	 * @access public
	 * @static
	...
	final public static function get_instance( $user_id ) {
		 ...
		 *                        Default 'MN_User_Meta_Session_Tokens'.
		 */
		$manager = apply_filters( 'session_token_manager', 'MN_User_Meta_Session_Tokens' );
	...
	final public function create( $expiration ) {
		...
		if ( ! empty( $_SERVER['HTTP_USER_AGENT'] ) ) {
			$session['ua'] = mn_unslash( $_SERVER['HTTP_USER_AGENT'] );
		}

		// Timestamp
		$session['login'] = time();

		$token = mn_generate_password( 43, false, false );
 ...
 * @since 4.0.0
 */
class MN_User_Meta_Session_Tokens extends MN_Session_Tokens {
