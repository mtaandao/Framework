...
class WC_Admin {
	...
	public function admin_redirects() {
		...
		if ( ! empty( $_GET['wc-install-plugin-redirect'] ) ) {
			...
			if ( current_user_can( 'install_plugins' ) && in_array( $plugin_slug, array( 'woocommerce-gateway-stripe' ) ) ) {
				$nonce = mn_create_nonce( 'install-plugin_' . $plugin_slug );
				$url   = self_admin_url( 'update.php?action=install-plugin&plugin=' . $plugin_slug . '&_mnnonce=' . $nonce );
			} else {
		...
		if ( get_transient( '_wc_activation_redirect' ) ) {
			...
			if ( WC_Admin_Notices::has_notice( 'install' ) ) {
				mn_safe_redirect( admin_url( 'index.php?page=wc-setup' ) );
				exit;
			}
		}
	}
	...
	public function preview_emails() {
		...
		if ( isset( $_GET['preview_woocommerce_mail'] ) ) {
			if ( ! mn_verify_nonce( $_REQUEST['_mnnonce'], 'preview-mail') ) {
				die( 'Security check' );
			}
