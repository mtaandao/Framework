 ...
 *
 * This class and related code (JSON response handler, resource classes) are based on MN-API v0.6 (https://github.com/MN-API/MN-API)
...
class WC_API_Server {
	...
	/**
	 * Requested path (relative to the API root, mn-json.php)
	 *
	 * @var string
	 */
	public $path = '';
	 ...
	 *
	 * @since 2.1
	 * @return MN_User|MN_Error MN_User object indicates successful login, MN_Error indicates unsuccessful login
	 */
	public function check_authentication() {
		...
		if ( is_a( $user, 'MN_User' ) ) {
			mn_set_current_user( $user->ID );
		}

		// MN_Errors are handled in serve_request()
		elseif ( ! is_mn_error( $user ) ) {
			$user = new MN_Error( 'woocommerce_api_authentication_error', __( 'Invalid authentication method', 'woocommerce' ), array( 'code' => 500 ) );
		}
	 ...
	 *
	 * @since 2.1
	 * @return mixed The value returned by the callback, or a MN_Error instance
	 */
	public function dispatch() {
		...
		switch ( $this->method ) {
			...
			default :
				return new MN_Error( 'woocommerce_api_unsupported_method', __( 'Unsupported request method', 'woocommerce' ), array( 'status' => 400 ) );
		}
		...
		foreach ( $this->get_routes() as $route => $handlers ) {
			foreach ( $handlers as $handler ) {
				...
				if ( ! is_callable( $callback ) ) {
					return new MN_Error( 'woocommerce_api_invalid_handler', __( 'The handler for the route is invalid', 'woocommerce' ), array( 'status' => 500 ) );
				}
		...
		return new MN_Error( 'woocommerce_api_no_route', __( 'No route was found matching the URL and request method', 'woocommerce' ), array( 'status' => 404 ) );
	}
	...
	protected function sort_callback_params( $callback, $provided ) {
		...
		foreach ( $wanted as $param ) {
			if ( isset( $provided[ $param->getName() ] ) ) {
				...
				return new MN_Error( 'woocommerce_api_missing_callback_param', sprintf( __( 'Missing parameter %s', 'woocommerce' ), $param->getName() ), array( 'status' => 400 ) );
			}
		}
	 ...
	 *
	 * @since 2.1
	 * @param MN_Query|MN_User_Query|stdClass $query
	 */
	public function add_pagination_headers( $query ) {

		// MN_User_Query
		if ( is_a( $query, 'MN_User_Query' ) ) {
