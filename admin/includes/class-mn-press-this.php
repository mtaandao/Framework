...
class MN_Press_This {
	...
	public function side_load_images( $post_id, $content = '' ) {
		...
		if ( preg_match_all( '/<img [^>]+>/', $content, $matches ) && current_user_can( 'upload_files' ) ) {
			foreach ( (array) $matches[0] as $image ) {
				...
				// Don't try to sideload a file without a file extension, leads to MN upload error.
	...
	public function save_post() {
		if ( empty( $_POST['post_ID'] ) || ! $post_id = (int) $_POST['post_ID'] ) {
			mn_send_json_error( array( 'errorMessage' => __( 'Missing post ID.' ) ) );
		}

		if ( empty( $_POST['_mnnonce'] ) || ! mn_verify_nonce( $_POST['_mnnonce'], 'update-post_' . $post_id ) ||
			! current_user_can( 'edit_post', $post_id ) ) {

			mn_send_json_error( array( 'errorMessage' => __( 'Invalid post.' ) ) );
		}
		...
		$updated = mn_update_post( $post_data, true );

		if ( is_mn_error( $updated ) ) {
			mn_send_json_error( array( 'errorMessage' => $updated->get_error_message() ) );
		} else {
			...
			if ( $redirect ) {
				mn_send_json_success( array( 'redirect' => $redirect, 'force' => $forceRedirect ) );
			} else {
				mn_send_json_success( array( 'postSaved' => true ) );
			}
		}
	}
	...
	public function add_category() {
		if ( false === mn_verify_nonce( $_POST['new_cat_nonce'], 'add-category' ) ) {
			mn_send_json_error();
		}
		...
		foreach ( $names as $cat_name ) {
			...
			if ( term_exists( $cat_name, $taxonomy->name, $parent ) ) {
				if ( count( $names ) === 1 ) {
					mn_send_json_error( array( 'errorMessage' => __( 'This category already exists.' ) ) );
				} else {
					continue;
				}
			}
		...
		if ( empty( $added ) ) {
			mn_send_json_error( array( 'errorMessage' => __( 'This category cannot be added. Please change the name and try again.' ) ) );
		}

		foreach ( $added as $new_cat_id ) {
			$new_cat = get_category( $new_cat_id );

			if ( is_mn_error( $new_cat ) ) {
				mn_send_json_error( array( 'errorMessage' => __( 'Error while adding the category. Please try again later.' ) ) );
			}
	...
	public function fetch_source_html( $url ) {
		...
		$source_content = mn_remote_retrieve_body( $remote_url );
		$source_content = mn_kses( $source_content, $allowed_elements );
	 ...
	 *
	 * Excluded URLs include share-this type buttons, loaders, spinners, spacers, Mtaandao interface images,
	 * tiny buttons or thumbs, mathtag.com or quantserve.com images, or the Mtaandao.com stats gif.
	 *
	 * @ignore
	 * @since 4.2.0
	 *
	...
	private function _limit_img( $src ) {
		...
		if ( preg_match( '!/ad[sx]?/!i', $src ) ) {
			...
			return '';
		} else if ( false !== stripos( $src, '/res/' ) ) {
			// Classic Mtaandao interface images
			return '';
	...
	public function merge_or_fetch_data() {
		...
		foreach ( array( 'u', 's', 't', 'v' ) as $key ) {
			if ( ! empty( $_POST[ $key ] ) ) {
				$value = mn_unslash( $_POST[ $key ] );
			} else if ( ! empty( $_GET[ $key ] ) ) {
				$value = mn_unslash( $_GET[ $key ] );
			} else {
				continue;
			}
		...
		if ( apply_filters( 'enable_press_this_media_discovery', true ) ) {
			...
			if ( empty( $_POST ) && ! empty( $data['u'] ) ) {
				...
				foreach ( array( '_images', '_embeds' ) as $type ) {
					...
					foreach ( $items as $key => $value ) {
						if ( $type === '_images' ) {
							$value = $this->_limit_img( mn_unslash( $value ) );
						} else {
							$value = $this->_limit_embed( mn_unslash( $value ) );
						}
				...
				foreach ( array( '_meta', '_links' ) as $type ) {
					...
					foreach ( $items as $key => $value ) {
						...
						if ( $type === '_meta' ) {
							$value = $this->_limit_string( mn_unslash( $value ) );
							...
							if ( in_array( $key, array( 'canonical', 'shortlink', 'icon' ), true ) ) {
								$data[ $type ][ $key ] = $this->_limit_url( mn_unslash( $value ) );
							}
						}
					}
				}
			...
			if ( ! empty( $_REQUEST['i'] ) && ( $img_src = $this->_limit_img( mn_unslash( $_REQUEST['i'] ) ) ) ) {
				if ( empty( $data['_images'] ) ) {
	...
	public function categories_html( $post ) {
		...
		<div aria-label="<?php esc_attr_e( 'Categories' ); ?>">
			<ul class="categories-select">
				<?php mn_terms_checklist( $post->ID, array( 'taxonomy' => 'category', 'list_only' => true ) ); ?>
			</ul>
		</div>
		<?php
	}
	...
	public function tags_html( $post ) {
		...
		if ( ! $esc_tags || is_mn_error( $esc_tags ) ) {
			$esc_tags = '';
		}
...
<head>
	...
	<script>
		window.mnPressThisData   = <?php echo mn_json_encode( $site_data ); ?>;
		window.mnPressThisConfig = <?php echo mn_json_encode( $site_settings ); ?>;
	</script>
	...
	<?php
		...
		mn_enqueue_media( array( 'post' => $post_ID ) );
		mn_enqueue_style( 'press-this' );
		mn_enqueue_script( 'press-this' );
		mn_enqueue_script( 'json2' );
...
<body class="mn-admin mn-core-ui <?php echo $admin_body_classes . ' ' . $admin_body_class; ?>">
	...
	<form id="pressthis-form" method="post" action="post.php" autocomplete="off">
		...
		mn_nonce_field( 'update-post_' . $post_ID, '_mnnonce', false );
		mn_nonce_field( 'add-category', '_ajax_nonce-add-category', false );
	...
	<div class="wrapper">
		<div class="editor-wrapper">
			...
			<div id="app-container" class="editor">
				...
				<?php
				mn_editor( $post_content, 'pressthis', array(
