 ...
 * Core Metadata API
 *
 * Functions for retrieving and manipulating metadata of various Mtaandao object types. Metadata
...
function add_metadata($meta_type, $object_id, $meta_key, $meta_value, $unique = false) {
	...
	if ( $unique && $mndb->get_var( $mndb->prepare(
	...
	$result = $mndb->insert( $table, array(
		$column => $object_id,
		'meta_key' => $meta_key,
	...
	$mid = (int) $mndb->insert_id;

	mn_cache_delete($object_id, $meta_type . '_meta');
...
function update_metadata($meta_type, $object_id, $meta_key, $meta_value, $prev_value = '') {
	...
	$meta_ids = $mndb->get_col( $mndb->prepare( "SELECT $id_column FROM $table WHERE meta_key = %s AND $column = %d", $meta_key, $object_id ) );
	if ( empty( $meta_ids ) ) {
	...
	$result = $mndb->update( $table, $data, $where );
	if ( ! $result )
		return false;
...
function delete_metadata($meta_type, $object_id, $meta_key, $meta_value = '', $delete_all = false) {
	...
	$query = $mndb->prepare( "SELECT $id_column FROM $table WHERE meta_key = %s", $meta_key );

	if ( !$delete_all )
		$query .= $mndb->prepare(" AND $type_column = %d", $object_id );

	if ( '' !== $meta_value && null !== $meta_value && false !== $meta_value )
		$query .= $mndb->prepare(" AND meta_value = %s", $meta_value );

	$meta_ids = $mndb->get_col( $query );
	if ( !count( $meta_ids ) )
		return false;
	...
	if ( $delete_all ) {
		$value_clause = '';
		if ( '' !== $meta_value && null !== $meta_value && false !== $meta_value ) {
			$value_clause = $mndb->prepare( " AND meta_value = %s", $meta_value );
		}

		$object_ids = $mndb->get_col( $mndb->prepare( "SELECT $type_column FROM $table WHERE meta_key = %s $value_clause", $meta_key ) );
	}
	...
	if ( $delete_all ) {
		foreach ( (array) $object_ids as $o_id ) {
			mn_cache_delete($o_id, $meta_type . '_meta');
		}
	} else {
		mn_cache_delete($object_id, $meta_type . '_meta');
	}
...
function get_metadata($meta_type, $object_id, $meta_key = '', $single = false) {
	...
	$meta_cache = mn_cache_get($object_id, $meta_type . '_meta');
...
function metadata_exists( $meta_type, $object_id, $meta_key ) {
	...
	/** This filter is documented in res/meta.php */
...
function get_metadata_by_mid( $meta_type, $meta_id ) {
	...
	$meta = $mndb->get_row( $mndb->prepare( "SELECT * FROM $table WHERE $id_column = %d", $meta_id ) );
...
function update_metadata_by_mid( $meta_type, $meta_id, $meta_value, $meta_key = false ) {
	...
	if ( $meta = get_metadata_by_mid( $meta_type, $meta_id ) ) {
		...
		/** This action is documented in res/meta.php */
		do_action( "update_{$meta_type}_meta", $meta_id, $object_id, $meta_key, $_meta_value );
		...
		$result = $mndb->update( $table, $data, $where, '%s', '%d' );
		if ( ! $result )
			return false;
...
function delete_metadata_by_mid( $meta_type, $meta_id ) {
	...
	if ( $meta = get_metadata_by_mid( $meta_type, $meta_id ) ) {
		...
		$result = (bool) $mndb->delete( $table, array( $id_column => $meta_id ) );
...
function update_meta_cache($meta_type, $object_ids) {
	...
	foreach ( $object_ids as $id ) {
		$cached_object = mn_cache_get( $id, $cache_key );
	...
	$id_list = join( ',', $ids );
	$id_column = 'user' == $meta_type ? 'umeta_id' : 'meta_id';
	$meta_list = $mndb->get_results( "SELECT $column, meta_key, meta_value FROM $table WHERE $column IN ($id_list) ORDER BY $id_column ASC", ARRAY_A );
	...
	foreach ( $ids as $id ) {
		if ( ! isset($cache[$id]) )
			$cache[$id] = array();
		mn_cache_add( $id, $cache[$id], $cache_key );
	}
 ...
 * @since 4.5.0
 *
 * @return MN_Metadata_Lazyloader $lazyloader Metadata lazyloader queue.
 */
function mn_metadata_lazyloader() {
	static $mn_metadata_lazyloader;

	if ( null === $mn_metadata_lazyloader ) {
		$mn_metadata_lazyloader = new MN_Metadata_Lazyloader();
	}

	return $mn_metadata_lazyloader;
}
...
function get_meta_sql( $meta_query, $type, $primary_table, $primary_id_column, $context = null ) {
	$meta_query_obj = new MN_Meta_Query( $meta_query );
...
function _get_meta_table($type) {
	...
	if ( empty($mndb->$table_name) )
		return false;

	return $mndb->$table_name;
}
...
function registered_meta_key_exists( $object_type, $meta_key ) {
	...
	if ( ! isset( $mn_meta_keys[ $object_type ] ) ) {
		return false;
	}

	if ( isset( $mn_meta_keys[ $object_type ][ $meta_key ] ) ) {
		return true;
	}
...
function unregister_meta_key( $object_type, $meta_key ) {
	...
	$args = $mn_meta_keys[ $object_type ][ $meta_key ];
	...
	unset( $mn_meta_keys[ $object_type ][ $meta_key ] );

	// Do some clean up
	if ( empty( $mn_meta_keys[ $object_type ] ) ) {
		unset( $mn_meta_keys[ $object_type ] );
	}
...
function get_registered_meta_keys( $object_type ) {
	...
	if ( ! is_array( $mn_meta_keys ) || ! isset( $mn_meta_keys[ $object_type ] ) ) {
		return array();
	}

	return $mn_meta_keys[ $object_type ];
}
 ...
 */
function _mn_register_meta_args_whitelist( $args, $default_args ) {
