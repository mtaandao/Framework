	...
	switch ( $_GET['action'] ) {
		case 'deleteuser':
			...
			if ( $id != '0' && $id != '1' ) {
				...
				mn_redirect( network_admin_url( 'users.php' ) );
			}
			exit();
		...
		case 'allusers':
			...
			if ( ( isset( $_POST['action']) || isset($_POST['action2'] ) ) && isset( $_POST['allusers'] ) ) {
						...
						switch ( $doaction ) {
							...
							case 'spam':
								...
								if ( is_super_admin( $user->ID ) )
									mn_die( sprintf( __( 'Warning! User cannot be modified. The user %s is a network administrator.' ), esc_html( $user->user_login ) ) );
				...
				mn_safe_redirect( add_query_arg( array( 'updated' => 'true', 'action' => $userfunction ), mn_get_referer() ) );
			} else {
		...
		case 'dodelete':
			...
			mn_redirect( add_query_arg( array( 'updated' => 'true', 'action' => $deletefunction ), network_admin_url( 'users.php' ) ) );
			exit();
	}
}

$mn_list_table = _get_list_table('MN_MS_Users_List_Table');
...
<div class="wrap">
	...
	<form method="get" class="search-form">
		<?php $mn_list_table->search_box( __( 'Search Users' ), 'all-user' ); ?>
	</form>
