 ...
 * Class RM_Admin_Menu - support stuff for manipulations with MN admin dashboard menu
 * Author: Mtaandao
...
class RM_Admin_Menu {
    ...
    const DATA_VERSION_KEY = 'rm_admin_menu_access_data_version';    
    const ADMIN_MENU_COPY_KEY = 'rm_admin_menu_copy';
    const ADMIN_SUBMENU_COPY_KEY = 'rm_admin_sub_menu_copy';
    const ADMIN_MENU_ACCESS_DATA_KEY = 'rm_admin_menu_access_data';
    // full list of hashes for the all admin menu/submenu items
    const ADMIN_MENU_HASHES = 'rm_admin_menu_hashes';

    
    public static function load_menu_access_data_for_role($role_id) {
        
        $rm_menu_access_data = get_option(self::ADMIN_MENU_ACCESS_DATA_KEY);
        if (is_array($rm_menu_access_data) && array_key_exists($role_id, $rm_menu_access_data)) {
            $result =  $rm_menu_access_data[$role_id];
    ...
    public static function load_menu_access_data_for_user($user) {
        ...
        $rm_menu_access_data = get_option(self::ADMIN_MENU_ACCESS_DATA_KEY);
        if (empty($rm_menu_access_data)) {
            $rm_menu_access_data = array();
        }
        
        if (!empty($user->roles)) {
            foreach ($user->roles as $role) {
                if (isset($rm_menu_access_data[$role])) {
                    if (!isset($rm_menu_access_data[$role]['access_model'])) { // for backward compatibility
                        $access_model = 1;   // Use default (block selected) access model
                        $data = $rm_menu_access_data[$role];
                    } else {
                        $access_model = $rm_menu_access_data[$role]['access_model'];
                        if (isset($rm_menu_access_data[$role]['data'])) {
                            $data = $rm_menu_access_data[$role]['data'];
                        } else {
                            $data = array();
                        }
                    }
    ...
    private static function get_menu_access_post_data() {
        ...
        $keys_to_skip = array(
            ...
            'rm_object_name', 
            'user_role', 
            'rm_admin_menu_access_model');
        
        $access_model = $_POST['rm_admin_menu_access_model'];
    ...
    public static function update_data() {
        ...
        if (!isset($_POST['action']) || $_POST['action']!=='rm_update_admin_menu_access') {            
            return;
        }
        ...
        if (!current_user_can('rm_admin_menu_access')) {
            $lib->notification = esc_html__('RM: Insufficient permissions to use this add-on','role-manager');
            return;
        }
        
        $rm_object_type = filter_input(INPUT_POST, 'rm_object_type', FILTER_SANITIZE_STRING);
        if ($rm_object_type!=='role' && $rm_object_type!=='user') {
            $lib->notification = esc_html__('RM: administrator menu access: Wrong object type. Data was not updated.', 'role-manager');
            return;
        }
        
        $rm_object_name = filter_input(INPUT_POST, 'rm_object_name', FILTER_SANITIZE_STRING);
        if (empty($rm_object_name)) {
            $lib->notification = esc_html__('RM: administrator menu access: Empty object name. Data was not updated', 'role-manager');
            return;
        }
                        
        if ($rm_object_type=='role') {
            RM_Admin_Menu::save_menu_access_data_for_role($rm_object_name);
        } else {
            RM_Admin_Menu::save_menu_access_data_for_user($rm_object_name);
        }
        
        $lib->notification = esc_html__('Administrator menu access data was updated successfully', 'role-manager');
    }
    // end of update_data()
    ...
    private static function has_cap($cap_required, $allowed_roles, $allowed_caps) {
        ...
        if ( in_array($cap_required, array('switch_designs', 'customize')) && 
            (in_array('edit_design_options', $allowed_roles) || array_key_exists('edit_design_options', $allowed_caps)) ) {
