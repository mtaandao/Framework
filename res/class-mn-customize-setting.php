<?php
/**
 * Mtaandao Customize Setting classes
 *
 * @package Mtaandao
...
class MN_Customize_Setting {
	 ...
	 * @since 3.4.0
	 *
	 * @param MN_Customize_Manager $manager
	 ...
	 *
	 * @see MN_Customize_Setting::preview()
	 * @since 4.1.1
	 * @var mixed
	 */
	...
	public function preview() {
		...
		switch ( $this->type ) {
			...
			default :
				...
				/**
				 * Fires when the MN_Customize_Setting::preview() method is called for settings
				 ...
				 * @since 3.4.0
				 *
				 * @param MN_Customize_Setting $this MN_Customize_Setting instance.
				 */
	 ...
	 * that `MN_Customize_Setting::_multidimensional_preview_filter()`
	 * is called for this setting.
	 *
	 * @since 4.4.0
	 * @access private
	 * @see MN_Customize_Manager::set_post_value()
	 * @see MN_Customize_Setting::_multidimensional_preview_filter()
	 */
	 ...
	 * @access private
	 *
	 * @see MN_Customize_Setting::$aggregated_multidimensionals
	...
	final public function save() {
		...
		/**
		 * Fires when the MN_Customize_Setting::save() method is called.
		 *
	 ...
	 * @return string|array|null|MN_Error Sanitized value, or `null`/`MN_Error` if invalid.
	 */
	 ...
	 * @return true|MN_Error True if the input was validated, otherwise MN_Error.
	 */
	public function validate( $value ) {
		...
		if ( is_null( $value ) ) {
			return new MN_Error( 'invalid_value', __( 'Invalid value.' ) );
		}

		$validity = new MN_Error();

		/**
		 * Validates a Customize setting value.
		 *
		 * Plugins should amend the `$validity` object via its `MN_Error::add()` method.
		 *
		 * The dynamic portion of the hook name, `$this->ID`, refers to the setting ID.
		 *
		 * @since 16.10.0
		...
		if ( is_mn_error( $validity ) && empty( $validity->errors ) ) {
			$validity = true;
		}
		return $validity;
	}
	...
	protected function get_root_value( $default = null ) {
		...
		if ( 'option' === $this->type ) {
			...
			/*
			 * Any MN_Customize_Setting subclass implementing aggregate multidimensional
	...
	protected function update( $value ) {
		...
		if ( 'option' === $this->type || 'theme_mod' === $this->type ) {
			...
			/**
			 * Fires when the MN_Customize_Setting::update() method is called for settings
			 * not handled as theme_mods or options.
			 *
