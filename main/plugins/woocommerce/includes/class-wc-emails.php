...
class WC_Emails {
	...
	public function get_from_name() {
		return mn_specialchars_decode( get_option( 'woocommerce_email_from_name' ), ENT_QUOTES );
	}
	...
	public function wrap_message( $email_heading, $message, $plain_text = false ) {
		...
		echo mnautop( mntexturize( $message ) );
	...
	public function order_schema_markup( $order, $sent_to_admin = false, $plain_text = false ) {
		...
		foreach ( $order->get_items() as $item ) {
			...
			if ( $product_exists ) {
				...
				if ( $image_id = $product->get_image_id() ) {
					$item_offered['image'] = mn_get_attachment_image_url( $image_id, 'thumbnail' );
				}
			}
		...
		echo '<div style="display:none;"><script type="application/ld+json">' . mn_json_encode( (object) $markup ) . '</script></div>';
	}
	...
	public function order_meta( $order, $sent_to_admin = false, $plain_text = false ) {
		...
		if ( $_fields ) {
			foreach ( $_fields as $key => $field ) {
				...
				$fields[ $key ] = array(
					'label' => mntexturize( $key ),
					'value' => mntexturize( get_post_meta( $order->id, $field, true ) )
				);
			}
		}
	...
	public function customer_details( $order, $sent_to_admin = false, $plain_text = false ) {
		...
		if ( $order->customer_note ) {
			$fields['customer_note'] = array(
				...
				'value' => mntexturize( $order->customer_note )
			);
		}
		...
		if ( $order->billing_email ) {
			$fields['billing_email'] = array(
				...
				'value' => mntexturize( $order->billing_email )
			);
	    }
	    ...
	    if ( $order->billing_phone ) {
			$fields['billing_phone'] = array(
				...
				'value' => mntexturize( $order->billing_phone )
			);
	    }
