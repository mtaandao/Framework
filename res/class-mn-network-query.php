<?php
/**
 * Network API: MN_Network_Query class
 *
 * @package Mtaandao
...
class MN_Network_Query {
	 ...
	 * @access public
	 *
	 * @param string|array $query MN_Network_Query arguments. See MN_Network_Query::__construct()
	 */
	...
	public function get_networks() {
		...
		if ( false === $cache_value ) {
			...
			mn_cache_add( $cache_key, $cache_value, 'networks' );
		} else {
		...
		// Convert to MN_Network instances
	...
	protected function get_network_ids() {
		...
		if ( ! empty( $this->query_vars['network__in'] ) ) {
			$this->sql_clauses['where']['network__in'] = "$mndb->site.id IN ( " . implode( ',', mn_parse_id_list( $this->query_vars['network__in'] ) ) . ' )';
		}

		// Parse network IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['network__not_in'] ) ) {
			$this->sql_clauses['where']['network__not_in'] = "$mndb->site.id NOT IN ( " . implode( ',', mn_parse_id_list( $this->query_vars['network__not_in'] ) ) . ' )';
		}

		if ( ! empty( $this->query_vars['domain'] ) ) {
			$this->sql_clauses['where']['domain'] = $mndb->prepare( "$mndb->site.domain = %s", $this->query_vars['domain'] );
		}

		// Parse network domain for an IN clause.
		if ( is_array( $this->query_vars['domain__in'] ) ) {
			$this->sql_clauses['where']['domain__in'] = "$mndb->site.domain IN ( '" . implode( "', '", $mndb->_escape( $this->query_vars['domain__in'] ) ) . "' )";
		}

		// Parse network domain for a NOT IN clause.
		if ( is_array( $this->query_vars['domain__not_in'] ) ) {
			$this->sql_clauses['where']['domain__not_in'] = "$mndb->site.domain NOT IN ( '" . implode( "', '", $mndb->_escape( $this->query_vars['domain__not_in'] ) ) . "' )";
		}

		if ( ! empty( $this->query_vars['path'] ) ) {
			$this->sql_clauses['where']['path'] = $mndb->prepare( "$mndb->site.path = %s", $this->query_vars['path'] );
		}

		// Parse network path for an IN clause.
		if ( is_array( $this->query_vars['path__in'] ) ) {
			$this->sql_clauses['where']['path__in'] = "$mndb->site.path IN ( '" . implode( "', '", $mndb->_escape( $this->query_vars['path__in'] ) ) . "' )";
		}

		// Parse network path for a NOT IN clause.
		if ( is_array( $this->query_vars['path__not_in'] ) ) {
			$this->sql_clauses['where']['path__not_in'] = "$mndb->site.path NOT IN ( '" . implode( "', '", $mndb->_escape( $this->query_vars['path__not_in'] ) ) . "' )";
		}
		...
		if ( strlen( $this->query_vars['search'] ) ) {
			$this->sql_clauses['where']['search'] = $this->get_search_sql(
				...
				array( "$mndb->site.domain", "$mndb->site.path" )
			);
		}
		...
		$this->sql_clauses['from']    = "FROM $mndb->site $join";
		...
		$network_ids = $mndb->get_col( $this->request );
	...
	private function set_found_networks() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			...
			$this->found_networks = (int) $mndb->get_var( $found_networks_query );
		}
	}
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( $orderby == 'network__in' ) {
			...
			$parsed = "FIELD( {$mndb->site}.id, $network__in )";
		} elseif ( $orderby == 'domain_length' || $orderby == 'path_length' ) {
			$field = substr( $orderby, 0, -7 );
			$parsed = "CHAR_LENGTH($mndb->site.$field)";
		} elseif ( in_array( $orderby, $allowed_keys ) ) {
			$parsed = "$mndb->site.$orderby";
		}
