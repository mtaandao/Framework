 ...
 *
 * If the locale is not set already, then the MNLANG constant is used if it is
 ...
 */
function get_locale() {
	global $locale, $mn_local_package;

	if ( isset( $locale ) ) {
		/**
		 * Filters Mtaandao install's locale ID.
		 *
		 * @since 1.5.0
		 *
	...
	if ( isset( $mn_local_package ) ) {
		$locale = $mn_local_package;
	}

	// MNLANG was defined in mn-config.
	if ( defined( 'MNLANG' ) ) {
		$locale = MNLANG;
	}
	...
	if ( is_multisite() ) {
		...
		if ( mn_installing() || ( false === $ms_locale = get_option( 'MNLANG' ) ) ) {
			$ms_locale = get_site_option( 'MNLANG' );
		}
	...
	/** This filter is documented in res/l10n.php */
	return apply_filters( 'locale', $locale );
}
 ...
 *
 * Loads the .mo file in MN_LANG_DIR constant path from Mtaandao root.
...
function load_default_textdomain( $locale = null ) {
	...
	$return = load_textdomain( 'default', MN_LANG_DIR . "/$locale.mo" );

	if ( ( is_multisite() || ( defined( 'MN_INSTALLING_NETWORK' ) && MN_INSTALLING_NETWORK ) ) && ! file_exists(  MN_LANG_DIR . "/admin-$locale.mo" ) ) {
		load_textdomain( 'default', MN_LANG_DIR . "/ms-$locale.mo" );
		return $return;
	}

	if ( is_admin() || mn_installing() || ( defined( 'MN_REPAIRING' ) && MN_REPAIRING ) ) {
		load_textdomain( 'default', MN_LANG_DIR . "/admin-$locale.mo" );
	}

	if ( is_network_admin() || ( defined( 'MN_INSTALLING_NETWORK' ) && MN_INSTALLING_NETWORK ) )
		load_textdomain( 'default', MN_LANG_DIR . "/admin-network-$locale.mo" );
 ...
 *
 * @since 1.5.0
 * @since 16.10.0 The function now tries to load the .mo file from the languages directory first.
 *
 * @param string $domain          Unique identifier for retrieving translated strings
 * @param string $deprecated      Optional. Use the $plugin_rel_path parameter instead. Defaukt false.
 * @param string $plugin_rel_path Optional. Relative path to MN_PLUGIN_DIR where the .mo file resides.
 *                                Default false.
...
function load_plugin_textdomain( $domain, $deprecated = false, $plugin_rel_path = false ) {
	...
	if ( load_textdomain( $domain, MN_LANG_DIR . '/plugins/' . $mofile ) ) {
		return true;
	}

	if ( false !== $plugin_rel_path ) {
		$path = MN_PLUGIN_DIR . '/' . trim( $plugin_rel_path, '/' );
 ...
 *
 * @param string $domain             Text domain. Unique identifier for retrieving translated strings.
 * @param string $mu_plugin_rel_path Optional. Relative to `MNMU_PLUGIN_DIR` directory in which the .mo
...
function load_muplugin_textdomain( $domain, $mu_plugin_rel_path = '' ) {
	...
	$path = trailingslashit( MNMU_PLUGIN_DIR . '/' . ltrim( $mu_plugin_rel_path, '/' ) );
...
function load_theme_textdomain( $domain, $path = false ) {
	...
	// Try to load from the languages directory first.
	if ( load_textdomain( $domain, MN_LANG_DIR . '/themes/' . $mofile ) ) {
		return true;
	}
...
function _load_textdomain_just_in_time( $domain ) {
	...
	if ( in_array( MN_LANG_DIR . '/plugins/' . $mofile, $cached_mofiles ) ) {
		return load_textdomain( $domain, MN_LANG_DIR . '/plugins/' . $mofile );
	}

	if ( in_array( MN_LANG_DIR . '/themes/' . $mofile, $cached_mofiles ) ) {
		return load_textdomain( $domain, MN_LANG_DIR . '/themes/' . $mofile );
	}
 ...
 *
 * The default directory is MN_LANG_DIR.
 *
 * @since 3.0.0
 *
...
function get_available_languages( $dir = null ) {
	...
	$lang_files = glob( ( is_null( $dir) ? MN_LANG_DIR : $dir ) . '/*.mo' );
	if ( $lang_files ) {
 ...
 */
function mn_get_installed_translations( $type ) {
	if ( $type !== 'themes' && $type !== 'plugins' && $type !== 'core' )
		return array();

	$dir = 'core' === $type ? '' : "/$type";

	if ( ! is_dir( MN_LANG_DIR ) )
		return array();

	if ( $dir && ! is_dir( MN_LANG_DIR . $dir ) )
		return array();

	$files = scandir( MN_LANG_DIR . $dir );
	if ( ! $files )
		return array();
	...
	foreach ( $files as $file ) {
		if ( '.' === $file[0] || is_dir( MN_LANG_DIR . "$dir/$file" ) ) {
			continue;
		}
		if ( substr( $file, -3 ) !== '.po' ) {
			continue;
		...
		$language_data[ $textdomain ][ $language ] = mn_get_pomo_file_data( MN_LANG_DIR . "$dir/$file" );
	}
 ...
 */
function mn_get_pomo_file_data( $po_file ) {
 ...
 */
function mn_dropdown_languages( $args = array() ) {
