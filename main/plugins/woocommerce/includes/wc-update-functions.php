...
function wc_update_200_file_paths() {
	...
	$existing_file_paths = $mndb->get_results( "SELECT * FROM {$mndb->postmeta} WHERE meta_key = '_file_path' AND meta_value != '';" );
	...
	if ( $existing_file_paths ) {
		...
		foreach( $existing_file_paths as $existing_file_path ) {
			...
			if ( ! empty( $old_file_path ) ) {
				...
				$mndb->query( $mndb->prepare( "UPDATE {$mndb->postmeta} SET meta_key = '_file_paths', meta_value = %s WHERE meta_id = %d", $file_paths, $existing_file_path->meta_id ) );

				$mndb->query( $mndb->prepare( "UPDATE {$mndb->prefix}woocommerce_downloadable_product_permissions SET download_id = %s WHERE product_id = %d", md5( $old_file_path ), $existing_file_path->post_id ) );
...
function wc_update_200_taxrates() {
		...
		foreach ( $tax_rates as $tax_rate ) {
			...
			foreach ( $tax_rate['countries'] as $country => $states ) {
				...
				foreach ( $states as $state ) {
					...
					$mndb->insert(
						$mndb->prefix . "woocommerce_tax_rates",
						array(
	...
	if ( $local_tax_rates )
		foreach ( $local_tax_rates as $tax_rate ) {
			...
			$tax_rate_id = $mndb->insert_id;

			if ( $tax_rate['locations'] ) {
				foreach ( $tax_rate['locations'] as $location ) {

					$mndb->insert(
						$mndb->prefix . "woocommerce_tax_rate_locations",
						array(
...
function wc_update_200_line_items() {
	...
	// Reverse with UPDATE `mnwc_postmeta` SET meta_key = '_order_items' WHERE meta_key = '_order_items_old'
	$order_item_rows = $mndb->get_results( "
		SELECT * FROM {$mndb->postmeta}
		 	...
		 	if ( $item_id ) {
				...
				if ( sizeof( $meta_rows ) > 0 ) {
					$mndb->query( $mndb->prepare( "
						INSERT INTO {$mndb->prefix}woocommerce_order_itemmeta ( order_item_id, meta_key, meta_value )
	...
	// Reverse with UPDATE `mnwc_postmeta` SET meta_key = '_order_taxes' WHERE meta_key = '_order_taxes_old'
	$order_tax_rows = $mndb->get_results( "
		SELECT * FROM {$mndb->postmeta}
...
function wc_update_210_remove_pages() {
	...
	mn_trash_post( get_option('woocommerce_pay_page_id') );
	mn_trash_post( get_option('woocommerce_thanks_page_id') );
	...
	mn_trash_post( get_option('woocommerce_lost_password_page_id') );
}
...
function wc_update_210_file_paths() {
	...
	$existing_file_paths = $mndb->get_results( "SELECT * FROM {$mndb->postmeta} WHERE meta_key = '_file_paths' AND meta_value != '';" );
		...
		foreach( $existing_file_paths as $existing_file_path ) {
			...
			if ( $value ) {
				...
				if ( $needs_update ) {
					...
					$mndb->query( $mndb->prepare( "UPDATE {$mndb->postmeta} SET meta_key = %s, meta_value = %s WHERE meta_id = %d", '_downloadable_files', $new_value, $existing_file_path->meta_id ) );
				}
			}
		}
	}
...
function wc_update_220_order_status() {
	global $mndb;
	$mndb->query( "
		UPDATE {$mndb->posts} as posts
		LEFT JOIN {$mndb->term_relationships} AS rel ON posts.ID = rel.object_id
		LEFT JOIN {$mndb->term_taxonomy} AS tax USING( term_taxonomy_id )
		LEFT JOIN {$mndb->terms} AS term USING( term_id )
...
function wc_update_220_variations() {
	...
	$update_variations = $mndb->get_results( "
		SELECT DISTINCT posts.ID AS variation_id, posts.post_parent AS variation_parent FROM {$mndb->posts} as posts
		LEFT OUTER JOIN {$mndb->postmeta} AS postmeta ON posts.ID = postmeta.post_id AND postmeta.meta_key = '_stock'
		LEFT OUTER JOIN {$mndb->postmeta} as postmeta2 ON posts.ID = postmeta2.post_id AND postmeta2.meta_key = '_manage_stock'
...
function wc_update_220_attributes() {
	...
	$attribute_taxonomies = $mndb->get_results( "SELECT * FROM " . $mndb->prefix . "woocommerce_attribute_taxonomies" );
	...
	foreach ( $attribute_taxonomies as $attribute_taxonomy ) {
		...
		if ( $sanitized_attribute_name !== $attribute_taxonomy->attribute_name ) {
			if ( ! $mndb->get_var( $mndb->prepare( "SELECT 1=1 FROM {$mndb->prefix}woocommerce_attribute_taxonomies WHERE attribute_name = %s;", $sanitized_attribute_name ) ) ) {
				// Update attribute
				$mndb->update(
...
function wc_update_240_api_keys() {
	 ...
	 */
	$api_users = $mndb->get_results( "SELECT user_id FROM $mndb->usermeta WHERE meta_key = 'woocommerce_api_consumer_key'" );
	$apps_keys = array();
...
function wc_update_240_refunds() {
	...
	foreach ( $refunded_orders as $refunded_order ) {
		...
		$refunded_total = $mndb->get_var( $mndb->prepare( "
			SELECT SUM( postmeta.meta_value )
			FROM $mndb->postmeta AS postmeta
			INNER JOIN $mndb->posts AS posts ON ( posts.post_type = 'shop_order_refund' AND posts.post_parent = %d )
...
function wc_update_241_variations() {
	...
	$update_variations = $mndb->get_results( "
		...
		FROM {$mndb->posts} as posts
		LEFT OUTER JOIN {$mndb->postmeta} AS postmeta ON posts.ID = postmeta.post_id AND postmeta.meta_key = '_stock_status'
		WHERE posts.post_type = 'product_variation'
...
function wc_update_260_termmeta() {
	global $mndb;
	/**
	 * Migrate term meta to Mtaandao tables
	 */
	if ( get_option( 'db_version' ) >= 34370 && $mndb->get_var( "SHOW TABLES LIKE '{$mndb->prefix}woocommerce_termmeta';" ) ) {
		if ( $mndb->query( "INSERT INTO {$mndb->termmeta} ( term_id, meta_key, meta_value ) SELECT woocommerce_term_id, meta_key, meta_value FROM {$mndb->prefix}woocommerce_termmeta;" ) ) {
...
function wc_update_260_zones() {
	 ...
	 */
	if ( $mndb->get_var( "SHOW COLUMNS FROM `{$mndb->prefix}woocommerce_shipping_zones` LIKE 'zone_enabled';" ) ) {
		$mndb->query( "ALTER TABLE {$mndb->prefix}woocommerce_shipping_zones CHANGE `zone_type` `zone_type` VARCHAR(40) NOT NULL DEFAULT '';" );
		$mndb->query( "ALTER TABLE {$mndb->prefix}woocommerce_shipping_zones CHANGE `zone_enabled` `zone_enabled` INT(1) NOT NULL DEFAULT 1;" );
	}
}
...
function wc_update_260_zone_methods() {
	 ...
	 */
	if ( $mndb->get_var( "SHOW TABLES LIKE '{$mndb->prefix}woocommerce_shipping_zone_shipping_methods';" ) ) {
		$old_methods = $mndb->get_results( "SELECT * FROM {$mndb->prefix}woocommerce_shipping_zone_shipping_methods;" );

		if ( $old_methods ) {
			$max_new_id = $mndb->get_var( "SELECT MAX(instance_id) FROM {$mndb->prefix}woocommerce_shipping_zone_methods" );
			$max_old_id = $mndb->get_var( "SELECT MAX(shipping_method_id) FROM {$mndb->prefix}woocommerce_shipping_zone_shipping_methods" );

			// Avoid ID conflicts
			$mndb->query( $mndb->prepare( "ALTER TABLE {$mndb->prefix}woocommerce_shipping_zone_methods AUTO_INCREMENT = %d;", max( $max_new_id, $max_old_id ) + 1 ) );
			...
			foreach ( $old_methods as $old_method ) {
				...
				if ( 'table_rate' === $old_method->shipping_method_type ) {
					...
					// Move rates
					$mndb->update(
						$mndb->prefix . 'woocommerce_shipping_table_rates',
						array(
					...
					$mndb->update(
						$mndb->prefix . 'woocommerce_shipping_flat_rate_boxes',
						array(
							'shipping_method_id' => $new_instance_id
						),
						array(
	...
	$mndb->query( "UPDATE {$mndb->prefix}woocommerce_shipping_zone_locations SET location_code = REPLACE( location_code, '-', '...' );" );
}
...
function wc_update_260_refunds() {
	 ...
	 */
	$mndb->query( "
	UPDATE {$mndb->prefix}woocommerce_order_itemmeta as item_meta
	LEFT JOIN {$mndb->prefix}woocommerce_order_items as items ON item_meta.order_item_id = items.order_item_id
	LEFT JOIN {$mndb->posts} as posts ON items.order_id = posts.ID
