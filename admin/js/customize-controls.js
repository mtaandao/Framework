/* global _mnCustomizeHeader, _mnCustomizeBackground, _mnMediaViewsL10n, MediaElementPlayer */
(function( exports, $ ){
	var Container, focus, api = mn.customize;

	/**
	 * A Customizer Setting.
	 *
	 * A setting is Mtaandao data (theme mod, option, menu, etc.) that the user can
	 * draft changes to in the Customizer.
	 *
	 * @see PHP class MN_Customize_Setting.
	 *
	 * @class
	 * @augments mn.customize.Value
	 * @augments mn.customize.Class
	 *
	...
	api.Setting = api.Value.extend({
		 ...
		 *
		 * @since 16.10.0
		 * @returns {mn.customize.Control[]} Controls associated with setting.
		 */
	 ...
	 * @since 4.1.0
	 *
	 * @param {mn.customize.Class} instance
	 ...
	 * @since 4.1.0
	 *
	 * @param {(mn.customize.Panel|mn.customize.Section|mn.customize.Control)} a
	 * @param {(mn.customize.Panel|mn.customize.Section|mn.customize.Control)} b
	 * @returns {Number}
	 */
	...
	Container = api.Class.extend({
		 ...
		 * @param {string=default} [options.params.type] - The type of the panel. See mn.customize.panelConstructor.
		...
		getContainer: function () {
			...
			if ( 0 !== $( '#tmpl-' + container.templateSelector ).length ) {
				template = mn.template( container.templateSelector );
			} else {
				template = mn.template( 'customize-' + container.containerType + '-default' );
			}
	...
	api.Section = Container.extend({
		 ...
		 * @param {string=default} [options.params.type] - The type of the section. See mn.customize.sectionConstructor.
		...
		onChangeExpanded: function ( expanded, args ) {
			var section = this,
				container = section.container.closest( '.mn-full-overlay-sidebar-content' ),
				content = section.container.find( '.accordion-section-content' ),
				overlay = section.container.closest( '.mn-full-overlay' ),
	 ...
	 * @augments mn.customize.Container
	 */
	api.ThemesSection = api.Section.extend({
		 ...
		 */
		initialize: function () {
			this.$customizeSidebar = $( '.mn-full-overlay-sidebar-content:first' );
		...
		ready: function () {
			...
			section.template = mn.template( 'customize-themes-details-view' );
		...
		onChangeExpanded: function ( expanded, args ) {
			...
			var position, scroll,
				...
				overlay = section.closest( '.mn-full-overlay' ),
				container = section.closest( '.mn-full-overlay-sidebar-content' ),
	...
	api.Panel = Container.extend({
		...
		onChangeExpanded: function ( expanded, args ) {
			...
			var position, scroll,
				...
				overlay = accordionSection.closest( '.mn-full-overlay' ),
				container = accordionSection.closest( '.mn-full-overlay-sidebar-content' ),
				siblings = container.find( '.open' ),
		...
		renderContent: function () {
			...
			if ( 0 !== $( '#tmpl-' + panel.templateSelector + '-content' ).length ) {
				template = mn.template( panel.templateSelector + '-content' );
			} else {
				template = mn.template( 'customize-panel-default-content' );
			}
	 ...
	 *
	 * @see PHP class MN_Customize_Control.
	 *
	 * @class
	 * @augments mn.customize.Class
	 *
	...
	api.Control = api.Class.extend({
		...
		initialize: function( id, options ) {
			...
			control.deferred.embedded.done( function () {
				...
				control.notifications.bind( 'add', function( notification ) {
					mn.a11y.speak( notification.message, 'assertive' );
		...
		renderNotifications: function() {
			...
			if ( ! control.notificationsTemplate ) {
				control.notificationsTemplate = mn.template( 'customize-control-notifications' );
			}
		...
		renderContent: function () {
			...
			if ( 0 !== $( '#tmpl-' + control.templateSelector ).length ) {
				template = mn.template( control.templateSelector );
	...
	api.ColorControl = api.Control.extend({
		ready: function() {
			...
			picker.val( control.setting() ).mnColorPicker({
				change: function() {
					control.setting.set( picker.mnColorPicker('color') );
				},
				clear: function() {
					control.setting.set( '' );
				}
			...
			this.setting.bind( function ( value ) {
				picker.val( value );
				picker.mnColorPicker( 'color', value );
			});
		}
	});
	...
	api.MediaControl = api.Control.extend({
		...
		ready: function() {
			...
			function setAttachmentDataAndRenderContent( value ) {
				...
				if ( 'pending' === hasAttachmentData.state() ) {
					mn.media.attachment( value ).fetch().done( function() {
						...
						mn.customize.previewer.send( control.setting.id + '-attachment-data', this.attributes );
					} );
				}
		...
		cleanupPlayer: function () {
			this.player && mn.media.mixin.removePlayer( this.player );
		},
			...
			this.frame = mn.media({
				...
				states: [
					new mn.media.controller.Library({
						...
						library:   mn.media.query({ type: this.params.mime_type }),
						multiple:  false,
						date:      false
					})
				]
		...
		select: function() {
			...
			var node,
				...
				mejsSettings = window._mnmejsSettings || {};
	 ...
	 *
	 * @class
	 * @augments mn.customize.MediaControl
	 * @augments mn.customize.Control
	 * @augments mn.customize.Class
	 */
	 ...
	 *
	 * @class
	 * @augments mn.customize.UploadControl
	 * @augments mn.customize.MediaControl
	 * @augments mn.customize.Control
	 * @augments mn.customize.Class
	 */
	...
	api.BackgroundControl = api.UploadControl.extend({
		...
		select: function() {
			...
			mn.ajax.post( 'custom-background-add', {
				nonce: _mnCustomizeBackground.nonces.add,
				mn_customize: 'on',
	...
	api.CroppedImageControl = api.MediaControl.extend({
		...
		initFrame: function() {
			...
			this.frame = mn.media({
				...
				states: [
					...
					new mn.media.controller.CustomizeImageCropper({
		 ...
		 * @param {mn.media.controller.Cropper} controller
	 ...
	 *
	 * @class
	 * @augments mn.customize.CroppedImageControl
	 * @augments mn.customize.MediaControl
	 * @augments mn.customize.Control
	 * @augments mn.customize.Class
	 */
		...
		initFrame: function() {
			...
			this.frame = mn.media({
				...
				states: [
					...
					new mn.media.controller.SiteIconCropper({
						imgSelectOptions: this.calculateImageSelectOptions,
						control: this
					})
				]
	...
	api.HeaderControl = api.Control.extend({
		ready: function() {
			...
			mn.media.controller.Cropper.prototype.defaults.doCropArgs.mn_customize = 'on';
			mn.media.controller.Cropper.prototype.defaults.doCropArgs.theme = api.settings.theme.stylesheet;
		},
		...
		getInitialHeaderImage: function() {
			...
			var currentHeaderObject = _.find( _mnCustomizeHeader.uploads, function( imageObj ) {
		 ...
		 */
		calculateImageSelectOptions: function(attachment, controller) {
			var xInit = parseInt(_mnCustomizeHeader.data.width, 10),
				yInit = parseInt(_mnCustomizeHeader.data.height, 10),
				flexWidth = !! parseInt(_mnCustomizeHeader.data['flex-width'], 10),
				flexHeight = !! parseInt(_mnCustomizeHeader.data['flex-height'], 10),
			...
			this.frame = mn.media({
				...
				states: [
					new mn.media.controller.Library({
						...
						date:      false,
						priority:  20,
						suggestedWidth: _mnCustomizeHeader.data.width,
						suggestedHeight: _mnCustomizeHeader.data.height
					}),
					new mn.media.controller.Cropper({
						imgSelectOptions: this.calculateImageSelectOptions
					})
				]
			});
		...
		/**
		 * Creates a new mn.customize.HeaderTool.ImageModel from provided
	...
	api.ThemeControl = api.Control.extend({
		...
		ready: function() {
			...
			control.container.on( 'click keydown touchend', '.theme', function( event ) {
				...
				$( '.mn-full-overlay' ).addClass( 'customize-loading' );
	 ...
	 *
	 * @class
	 * @augments mn.customize.Messenger
	 * @augments mn.customize.Class
	 * @mixes mn.customize.Events
	 */
	...
	api.Previewer = api.Messenger.extend({
		...
		initialize: function( params, options ) {
			...
			this.add( 'previewUrl', params.previewUrl ).setter( function( to ) {
				...
				// Check for URLs that include "/admin/" or end in "/admin".
		...
		refreshNonces: function() {
			...
			request = mn.ajax.post( 'customize_refresh_nonces', {
				mn_customize: 'on',
				theme: api.settings.theme.stylesheet
			});
	 ...
	 * @returns {object<string, mn.customize.Control>} Mapping setting ids to arrays of controls.
	 */
	...
	$( function() {
		api.settings = window._mnCustomizeSettings;
		api.l10n = window._mnCustomizeControlsL10n;
		...
		var parent, topFocus,
			body = $( document.body ),
			overlay = body.children( '.mn-full-overlay' ),
		...
		api.previewer = new api.Previewer({
			...
			signature:   'MN_CUSTOMIZER_SIGNATURE'
		}, {
			...
			save: function() {
				...
				submit = function () {
					...
					request = mn.ajax.post( 'customize_save', query );
