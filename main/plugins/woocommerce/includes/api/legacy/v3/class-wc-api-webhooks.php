...
class WC_API_Webhooks extends WC_API_Resource {
	...
	public function create_webhook( $data ) {
		...
		try {
			...
			$webhook_id = mn_insert_post( $webhook_data );

			if ( is_mn_error( $webhook_id ) || ! $webhook_id ) {
				throw new WC_API_Exception( 'woocommerce_api_cannot_create_webhook', sprintf( __( 'Cannot create webhook: %s', 'woocommerce' ), is_mn_error( $webhook_id ) ? implode( ', ', $webhook_id->get_error_messages() ) : '0' ), 500 );
			}
