/* global tinymce */
tinymce.PluginManager.add( 'mneditimage', function( editor ) {
	...
	editor.addButton( 'mn_img_remove', {
		tooltip: 'Remove',
	...
	each( {
		...
		editor.addButton( 'mn_img_' + name, {
			tooltip: tooltip,
			icon: 'dashicon dashicons-align-' + direction,
			cmd: 'alignnone' === name ? 'mnAlignNone' : 'Justify' + direction.slice( 0, 1 ).toUpperCase() + direction.slice( 1 ),
			onPostRender: function() {
				...
				editor.on( 'NodeChange', function( event ) {
					...
					node = editor.dom.getParent( event.element, '.mn-caption' ) || event.element;
	...
	function parseShortcode( content ) {
		return content.replace( /(?:<p>)?\[(?:mn_)?caption([^\]]+)\]([\s\S]+?)\[\/(?:mn_)?caption\](?:<\/p>)?/g, function( a, b, c ) {
			...
			if ( ! editor.getParam( 'mneditimage_html5_captions' ) ) {
				width += 10;
			}

			return '<div class="mceTemp"><dl id="' + id + '" class="mn-caption ' + align + classes + '" style="width: ' + width + 'px">' +
				'<dt class="mn-caption-dt">'+ img +'</dt><dd class="mn-caption-dd">'+ caption +'</dd></dl></div>';
		});
	}

	function getShortcode( content ) {
		return content.replace( /(?:<div [^>]+mceTemp[^>]+>)?\s*(<dl [^>]+mn-caption[^>]+>[\s\S]+?<\/dl>)\s*(?:<\/div>)?/g, function( all, dl ) {
			var out = '';
			...
			out = dl.replace( /\s*<dl ([^>]+)>\s*<dt [^>]+>([\s\S]+?)<\/dt>\s*<dd [^>]+>([\s\S]*?)<\/dd>\s*<\/dl>\s*/gi, function( a, b, c, caption ) {
				...
				classes = classes.replace( /mn-caption ?|align[a-z]+ ?/gi, '' );
	...
	function extractImageData( imageNode ) {
		...
		tinymce.each( classes, function( name ) {
			...
			if ( /^mn-image/.test( name ) ) {
				metadata.attachment_id = parseInt( name.replace( 'mn-image-', '' ), 10 );
		...
		// Extract caption
		captionBlock = dom.getParents( imageNode, '.mn-caption' );
		...
		if ( captionBlock.length ) {
			...
			caption = dom.select( 'dd.mn-caption-dd', captionBlock );
			if ( caption.length ) {
				caption = caption[0];
	...
	function updateImage( imageNode, imageData ) {
		...
		if ( imageData.attachment_id ) {
			classes.push( 'mn-image-' + imageData.attachment_id );
		...
		if ( imageData.caption ) {
			...
			if ( captionNode ) {
				dl = dom.select( 'dl.mn-caption', captionNode );
				...
				dd = dom.select( '.mn-caption-dd', captionNode );
				...
				html =  '<dl ' + id + 'class="' + className +'" style="width: '+ width +'px">' +
					'<dt class="mn-caption-dt"></dt><dd class="mn-caption-dd">'+ imageData.caption +'</dd></dl>';
				...
				editor.$( wrap ).find( 'dt.mn-caption-dt' ).append( node );
		...
		if ( mn.media.events ) {
			mn.media.events.trigger( 'editor:image-update', {
				editor: editor,
				metadata: imageData,
				image: imageNode
			} );
	...
	function editImage( img ) {
		...
		mn.media.events.trigger( 'editor:image-edit', {
			editor: editor,
			metadata: metadata,
			image: img
		} );
		...
		mn.media.events.trigger( 'editor:frame-create', { frame: frame } );
	...
	editor.on( 'init', function() {
		var dom = editor.dom,
			captionClass = editor.getParam( 'mneditimage_html5_captions' ) ? 'html5-captions' : 'html4-captions';

		dom.addClass( editor.getBody(), captionClass );

		// Add caption field to the default image dialog
		editor.on( 'mnLoadImageForm', function( event ) {
			if ( editor.getParam( 'mneditimage_disable_captions' ) ) {
				return;
			}
		...
		editor.on( 'mnNewImageRefresh', function( event ) {
			var parent, captionWidth;

			if ( parent = dom.getParent( event.node, 'dl.mn-caption' ) ) {
		...
		editor.on( 'mnImageFormSubmit', function( event ) {
			var data = event.imgData.data,
				imgNode = event.imgData.node,
				caption = event.imgData.mncaption,
				captionId = '',
				captionAlign = '',
				captionWidth = '',
			...
			if ( ! imgNode ) {
				...
				if ( caption ) {
					...
					html = '<dl class="mn-caption alignnone"' + captionWidth + '>' +
						'<dt class="mn-caption-dt">'+ html +'</dt><dd class="mn-caption-dd">'+ caption +'</dd></dl>';
				...
				wrap = dom.getParent( imgNode, 'dl.mn-caption' );

				if ( caption ) {
					if ( wrap ) {
						if ( parent = dom.select( 'dd.mn-caption-dd', wrap )[0] ) {
							parent.innerHTML = caption;
						}
					} else {
						if ( imgNode.className ) {
							captionId = imgNode.className.match( /mn-image-([0-9]+)/ );
						...
						captionAlign = ' class="mn-caption ' + captionAlign + '"';
						...
						html = '<dl ' + captionId + captionAlign + captionWidth + '>' +
							'<dt class="mn-caption-dt"></dt><dd class="mn-caption-dd">'+ caption +'</dd></dl>';
			...
			imgNode = dom.get('__mn-temp-img-id');
			dom.setAttrib( imgNode, 'id', imgId );
			event.imgData.node = imgNode;
		});

		editor.on( 'mnLoadImageData', function( event ) {
			var parent,
				data = event.imgData.data,
				imgNode = event.imgData.node;

			if ( parent = dom.getParent( imgNode, 'dl.mn-caption' ) ) {
				parent = dom.select( 'dd.mn-caption-dd', parent )[0];

				if ( parent ) {
					data.mncaption = editor.serializer.serialize( parent )
						.replace( /<br[^>]*>/g, '$&\n' ).replace( /^<p>/, '' ).replace( /<\/p>$/, '' );
				}
			}
		});

		// Prevent IE11 from making dl.mn-caption resizable
		if ( tinymce.Env.ie && tinymce.Env.ie > 10 ) {
			// The 'mscontrolselect' event is supported only in IE11+
			dom.bind( editor.getBody(), 'mscontrolselect', function( event ) {
				if ( event.target.nodeName === 'IMG' && dom.getParent( event.target, '.mn-caption' ) ) {
					// Hide the thick border with resize handles around dl.mn-caption
					editor.getBody().focus(); // :(
				} else if ( event.target.nodeName === 'DL' && dom.hasClass( event.target, 'mn-caption' ) ) {
	...
	editor.on( 'ObjectResized', function( event ) {
		...
		if ( node.nodeName === 'IMG' ) {
			editor.undoManager.transact( function() {
				...
				if ( parent = dom.getParent( node, '.mn-caption' ) ) {
	...
	editor.on( 'BeforeExecCommand', function( event ) {
		...
		} else if ( cmd === 'JustifyLeft' || cmd === 'JustifyRight' || cmd === 'JustifyCenter' || cmd === 'mnAlignNone' ) {
			node = editor.selection.getNode();
			align = 'align' + cmd.slice( 7 ).toLowerCase();
			DL = editor.dom.getParent( node, '.mn-caption' );
	...
	editor.mnSetImgCaption = function( content ) {
		return parseShortcode( content );
	};

	editor.mnGetImgCaption = function( content ) {
		return getShortcode( content );
	};

	editor.on( 'BeforeSetContent', function( event ) {
		if ( event.format !== 'raw' ) {
			event.content = editor.mnSetImgCaption( event.content );
		}
	});

	editor.on( 'PostProcess', function( event ) {
		if ( event.get ) {
			event.content = editor.mnGetImgCaption( event.content );
		}
	});
	...
	// Add to editor.mn
	editor.mn = editor.mn || {};
	editor.mn.isPlaceholder = isPlaceholder;
