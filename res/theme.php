...
/**
 * Returns an array of MN_Theme objects based on the arguments.
 *
 * Despite advances over get_themes(), this function is quite expensive, and grows
 * linearly with additional themes. Stick to mn_get_theme() if possible.
 *
 * @since 3.4.0
 *
 ...
 * @return array Array of MN_Theme objects.
 */
function mn_get_themes( $args = array() ) {
	...
	if ( count( $mn_theme_directories ) > 1 ) {
	...
	if ( is_multisite() && null !== $args['allowed'] ) {
		...
		if ( 'network' === $allowed )
			$theme_directories = array_intersect_key( $theme_directories, MN_Theme::get_allowed_on_network() );
		elseif ( 'site' === $allowed )
			$theme_directories = array_intersect_key( $theme_directories, MN_Theme::get_allowed_on_site( $args['blog_id'] ) );
		elseif ( $allowed )
			$theme_directories = array_intersect_key( $theme_directories, MN_Theme::get_allowed( $args['blog_id'] ) );
		else
			$theme_directories = array_diff_key( $theme_directories, MN_Theme::get_allowed( $args['blog_id'] ) );
	}
	...
	foreach ( $theme_directories as $theme => $theme_root ) {
		if ( isset( $_themes[ $theme_root['theme_root'] . '/' . $theme ] ) )
			...
			$themes[ $theme ] = $_themes[ $theme_root['theme_root'] . '/' . $theme ] = new MN_Theme( $theme, $theme_root['theme_root'] );
	}

	if ( null !== $args['errors'] ) {
		foreach ( $themes as $theme => $mn_theme ) {
			if ( $mn_theme->errors() != $args['errors'] )
				unset( $themes[ $theme ] );
		}
	}
...
/**
 * Gets a MN_Theme object for a theme.
 *
 * @since 3.4.0
 *
 * @global array $mn_theme_directories
 ...
 * @return MN_Theme Theme object. Be sure to check the object's exists() method if you need to confirm the theme's existence.
 */
function mn_get_theme( $stylesheet = null, $theme_root = null ) {
	global $mn_theme_directories;
...
/**
 * Clears the cache held by get_theme_roots() and MN_Theme.
 *
 * @since 3.5.0
 * @param bool $clear_update_cache Whether to clear the Theme updates cache
 */
function mn_clean_themes_cache( $clear_update_cache = true ) {
	if ( $clear_update_cache )
		delete_site_transient( 'update_themes' );
	search_theme_directories( true );
	foreach ( mn_get_themes( array( 'errors' => null ) ) as $theme )
		$theme->cache_delete();
}
 ...
 * Mtaandao workflow, then change the former. If you just have the locale in a
...
function get_locale_stylesheet_uri() {
	...
	elseif ( !empty($mn_locale->text_direction) && file_exists("$dir/{$mn_locale->text_direction}.css") )
		$stylesheet_uri = "$stylesheet_dir_uri/{$mn_locale->text_direction}.css";
	else
		$stylesheet_uri = '';
	/**
...
function register_theme_directory( $directory ) {
	...
	if ( ! is_array( $mn_theme_directories ) ) {
		$mn_theme_directories = array();
	}

	$untrailed = untrailingslashit( $directory );
	if ( ! empty( $untrailed ) && ! in_array( $untrailed, $mn_theme_directories ) ) {
		$mn_theme_directories[] = $untrailed;
	}
...
function search_theme_directories( $force = false ) {
	...
	$mn_theme_directories = (array) $mn_theme_directories;
	 ...
	 */
	if ( $cache_expiration = apply_filters( 'mn_cache_themes_persistently', false, 'search_theme_directories' ) ) {
	...
	foreach ( $mn_theme_directories as $theme_root ) {
		...
		foreach ( $dirs as $dir ) {
			...
			if ( file_exists( $theme_root . '/' . $dir . '/style.css' ) ) {
				...
				// Return it; MN_Theme will catch the error.
				if ( ! $found_theme )
					$found_themes[ $dir ] = array(
						'theme_file' => $dir . '/style.css',
						'theme_root' => $theme_root,
...
function get_theme_root_uri( $stylesheet_or_template = false, $theme_root = false ) {
	 ...
	 * @param string $siteurl                Mtaandao web address which is set in General Options.
...
function switch_theme( $stylesheet ) {
	global $mn_theme_directories, $mn_customize, $sidebars_widgets;

	$_sidebars_widgets = null;
	if ( 'mn_ajax_customize_save' === current_action() ) {
		$_sidebars_widgets = $mn_customize->post_value( $mn_customize->get_setting( 'old_sidebars_widgets_data' ) );
	...
	$old_theme = mn_get_theme();
	$new_theme = mn_get_theme( $stylesheet );
	 ...
	 * @param MN_Theme $new_theme MN_Theme instance of the new theme.
	 * @param MN_Theme $old_theme MN_Theme instance of the old theme.
	 */
...
function validate_current_theme() {
	 ...
	 */
	if ( mn_installing() || ! apply_filters( 'validate_current_theme', true ) )
		return true;
...
function get_theme_mods() {
	...
	if ( false === $mods ) {
		...
		if ( false === $theme_name )
			$theme_name = mn_get_theme()->get('Name');
...
function get_theme_mod( $name, $default = false ) {
	...
	/** This filter is documented in res/theme.php */
...
function get_header_image_tag( $attr = array() ) {
	...
	if ( empty( $attr['srcset'] ) && ! empty( $header->attachment_id ) ) {
		$image_meta = get_post_meta( $header->attachment_id, '_mn_attachment_metadata', true );
		$size_array = array( $width, $height );

		if ( is_array( $image_meta ) ) {
			$srcset = mn_calculate_image_srcset( $size_array, $header->url, $image_meta, $header->attachment_id );
			$sizes = ! empty( $attr['sizes'] ) ? $attr['sizes'] : mn_calculate_image_sizes( $size_array, $header->url, $image_meta, $header->attachment_id );
 ...
 * @access private
 *
 * @global array  $_mn_default_headers
 * @staticvar object $_mn_random_header
 *
 * @return object
 */
function _get_random_header_data() {
	static $_mn_random_header = null;

	if ( empty( $_mn_random_header ) ) {
		global $_mn_default_headers;
		$header_image_mod = get_theme_mod( 'header_image', '' );
		$headers = array();

		if ( 'random-uploaded-image' == $header_image_mod )
			$headers = get_uploaded_header_images();
		elseif ( ! empty( $_mn_default_headers ) ) {
			if ( 'random-default-image' == $header_image_mod ) {
				$headers = $_mn_default_headers;
			} else {
		...
		$_mn_random_header = (object) $headers[ array_rand( $headers ) ];

		$_mn_random_header->url =  sprintf( $_mn_random_header->url, get_template_directory_uri(), get_stylesheet_directory_uri() );
		$_mn_random_header->thumbnail_url =  sprintf( $_mn_random_header->thumbnail_url, get_template_directory_uri(), get_stylesheet_directory_uri() );
	}
	return $_mn_random_header;
}
...
function get_uploaded_header_images() {
	...
	// @todo caching
	$headers = get_posts( array( 'post_type' => 'attachment', 'meta_key' => '_mn_attachment_is_custom_header', 'meta_value' => get_option('stylesheet'), 'orderby' => 'none', 'nopaging' => true ) );

	if ( empty( $headers ) )
		return array();

	foreach ( (array) $headers as $header ) {
		$url = esc_url_raw( mn_get_attachment_url( $header->ID ) );
		$header_data = mn_get_attachment_metadata( $header->ID );
		...
		$header_images[$header_index]['alt_text'] = get_post_meta( $header->ID, '_mn_attachment_image_alt', true );
...
function get_custom_header() {
	...
	if ( is_random_header_image() ) {
		...
		if ( ! $data && current_theme_supports( 'custom-header', 'default-image' ) ) {
			...
			if ( ! empty( $_mn_default_headers ) ) {
				foreach ( (array) $_mn_default_headers as $default_header ) {
...
function register_default_headers( $headers ) {
	...
	$_mn_default_headers = array_merge( (array) $_mn_default_headers, (array) $headers );
}
...
function unregister_default_headers( $header ) {
	...
	} elseif ( isset( $_mn_default_headers[ $header ] ) ) {
		unset( $_mn_default_headers[ $header ] );
		return true;
	} else {
		return false;
	}
...
function add_theme_support( $feature ) {
	...
	switch ( $feature ) {
		case 'post-thumbnails':
			 ...
			 * for post thumbnails.
			 */
			if ( is_array( $args[0] ) && isset( $_mn_theme_features['post-thumbnails'] ) ) {
				$args[0] = array_unique( array_merge( $_mn_theme_features['post-thumbnails'][0], $args[0] ) );
			}
		...
		case 'html5' :
			...
			if ( isset( $_mn_theme_features['html5'] ) )
				$args[0] = array_merge( $_mn_theme_features['html5'][0], $args[0] );
			break;
		...
		case 'custom-logo':
			...
			$args[0] = mn_parse_args( array_intersect_key( $args[0], $defaults ), $defaults );
		...
		case 'custom-header' :
			...
			if ( isset( $_mn_theme_features['custom-header'] ) )
				$args[0] = mn_parse_args( $_mn_theme_features['custom-header'][0], $args[0] );

			// Load in the defaults at the end, as we need to insure first one wins.
			// This will cause all constants to be defined, as each arg will then be set to the default.
			if ( $jit )
				$args[0] = mn_parse_args( $args[0], $defaults );

			// If a constant was defined, use that value. Otherwise, define the constant to ensure
			// the constant is always accurate (and is not defined later,  overriding our value).
			// As stated above, the first value wins.
			// Once we get to mn_loaded (just-in-time), define any constants we haven't already.
		...
		case 'custom-background' :
			...
			if ( isset( $_mn_theme_features['custom-background'] ) )
				$args[0] = mn_parse_args( $_mn_theme_features['custom-background'][0], $args[0] );
		...
		case 'title-tag' :
			// Can be called in functions.php but must happen before mn_loaded, i.e. not in header.php.
			if ( did_action( 'mn_loaded' ) ) {
				...
				_doing_it_wrong( "add_theme_support( 'title-tag' )", sprintf( __( 'Theme support for %1$s should be registered before the %2$s hook.' ),
					'<code>title-tag</code>', '<code>mn_loaded</code>' ), '4.1.0' );

				return false;
			}
	}

	$_mn_theme_features[ $feature ] = $args;
}
...
function _custom_header_background_just_in_time() {
	...
	if ( current_theme_supports( 'custom-header' ) ) {
		...
		if ( $args[0]['mn-head-callback'] )
			add_action( 'mn_head', $args[0]['mn-head-callback'] );
...
function get_theme_support( $feature ) {
	global $_mn_theme_features;
	if ( ! isset( $_mn_theme_features[ $feature ] ) )
		return false;

	if ( func_num_args() <= 1 )
		return $_mn_theme_features[ $feature ];
	...
	switch ( $feature ) {
		...
		case 'custom-header' :
		case 'custom-background' :
			if ( isset( $_mn_theme_features[ $feature ][0][ $args[0] ] ) )
				return $_mn_theme_features[ $feature ][0][ $args[0] ];
			return false;
...
function _remove_theme_support( $feature ) {
	...
	unset( $_mn_theme_features[ $feature ] );
	return true;
}
...
function current_theme_supports( $feature ) {
	...
	switch ( $feature ) {
		case 'post-thumbnails':
			...
			if ( true === $_mn_theme_features[$feature] )  // Registered for all types
				return true;
			$content_type = $args[0];
			return in_array( $content_type, $_mn_theme_features[$feature][0] );
		...
		case 'post-formats':
			...
			$type = $args[0];
			return in_array( $type, $_mn_theme_features[$feature][0] );

		case 'custom-logo':
		case 'custom-header':
		case 'custom-background':
			// Specific capabilities can be registered by passing an array to add_theme_support().
			return ( isset( $_mn_theme_features[ $feature ][0][ $args[0] ] ) && $_mn_theme_features[ $feature ][0][ $args[0] ] );
	}
	 ...
	 */
	return apply_filters( "current_theme_supports-{$feature}", true, $args, $_mn_theme_features[$feature] );
}
...
function _delete_attachment_theme_mod( $id ) {
	$attachment_image = mn_get_attachment_url( $id );
...
/**
 * Checks if a theme has been changed and runs 'after_switch_theme' hook on the next MN load.
 *
...
function check_theme_switched() {
	if ( $stylesheet = get_option( 'theme_switched' ) ) {
		$old_theme = mn_get_theme( $stylesheet );

		// Prevent retrieve_widgets() from running since Customizer already called it up front
		if ( get_option( 'theme_switched_via_customizer' ) ) {
			remove_action( 'after_switch_theme', '_mn_sidebars_changed' );
			update_option( 'theme_switched_via_customizer', false );
		}

		if ( $old_theme->exists() ) {
			/**
			 * Fires on the first MN load after a theme switch if the old theme still exists.
			 *
			...
			/** This action is documented in res/theme.php */
 ...
 * @since 3.4.0
 */
function _mn_customize_loader_settings() {
	...
	$browser = array(
		'mobile' => mn_is_mobile(),
		'ios'    => mn_is_mobile() && preg_match( '/iPad|iPod|iPhone/', $_SERVER['HTTP_USER_AGENT'] ),
	);
	...
	$script = 'var _mnCustomizeLoaderSettings = ' . mn_json_encode( $settings ) . ';';

	$mn_scripts = mn_scripts();
	$data = $mn_scripts->get_data( 'customize-loader', 'data' );
	if ( $data )
		$script = "$data\n$script";

	$mn_scripts->add_data( 'customize-loader', 'data', $script );
}
 ...
 * @return string
 */
function mn_customize_url( $stylesheet = null ) {
 ...
 * @since 4.0.0
 *
 * @global MN_Customize_Manager $mn_customize Customizer instance.
 *
...
function is_customize_preview() {
	...
	return ( $mn_customize instanceof MN_Customize_Manager ) && $mn_customize->is_preview();
}
