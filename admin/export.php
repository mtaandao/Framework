<?php
/**
 * Mtaandao Export Administration Screen
 *
 * @package Mtaandao
...
if ( !current_user_can('export') )
	mn_die(__('Sorry, you are not allowed to export the content of this site.'));

/** Load Mtaandao export API */
...
get_current_screen()->add_help_tab( array(
	...
	'content' => '<p>' . __('You can export a file of your site&#8217;s content in order to import it into another installation or platform. The export file will be an XML file format called WXR. Posts, pages, comments, custom fields, categories, and tags can be included. You can choose for the WXR file to include only certain posts or pages by setting the dropdown filters to limit the export by category, author, date range by month, or publishing status.') . '</p>' .
		'<p>' . __('Once generated, your WXR file can be imported by another Mtaandao site or by another blogging platform able to access this format.') . '</p>',
) );
 ...
 * @global MN_Locale $mn_locale Date and Time Locale object.
 *
 * @since 3.1.0
 *
...
function export_date_options( $post_type = 'post' ) {
	...
	foreach ( $months as $date ) {
		...
		echo '<option value="' . $date->year . '-' . $month . '">' . $mn_locale->get_month( $month ) . ' ' . $date->year . '</option>';
	}
}
?>
...
<p><?php _e('Once you&#8217;ve saved the download file, you can use the Import function in another Mtaandao installation to import the content from this site.'); ?></p>
...
<ul id="post-filters" class="export-filters">
	<li>
		...
		<?php mn_dropdown_categories( array( 'show_option_all' => __('All') ) ); ?>
		</label>
	</li>
	<li>
		<label><span class="label-responsive"><?php _e( 'Authors:' ); ?></span>
		<?php
		$authors = $mndb->get_col( "SELECT DISTINCT post_author FROM {$mndb->posts} WHERE post_type = 'post'" );
...
<ul id="page-filters" class="export-filters">
	<li>
		<label><span class="label-responsive"><?php _e( 'Authors:' ); ?></span>
		<?php
		$authors = $mndb->get_col( "SELECT DISTINCT post_author FROM {$mndb->posts} WHERE post_type = 'page'" );
		mn_dropdown_users( array(
			'include' => $authors,
			'name' => 'page_author',
			'multi' => true,
