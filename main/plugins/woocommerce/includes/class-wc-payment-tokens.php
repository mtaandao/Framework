...
class WC_Payment_Tokens {
	...
	public static function get_tokens( $args ) {
		...
		$sql   = "SELECT * FROM {$mndb->prefix}woocommerce_payment_tokens";
		$where = array( '1=1' );
		...
		$token_results = $mndb->get_results( $sql . ' WHERE ' . implode( ' AND ', $where ) );
		$tokens        = array();
	...
	public static function get_customer_default_token( $customer_id ) {
		...
		$token = $mndb->get_row( $mndb->prepare(
			"SELECT * FROM {$mndb->prefix}woocommerce_payment_tokens WHERE user_id = %d AND is_default = 1",
			$customer_id
		) );
	...
	public static function get( $token_id, $token_result = null ) {
		...
		if ( is_null( $token_result ) ) {
			$token_result = $mndb->get_row( $mndb->prepare(
				"SELECT * FROM {$mndb->prefix}woocommerce_payment_tokens WHERE token_id = %d",
				$token_id
			) );
	...
	public static function set_users_default( $user_id, $token_id ) {
		global $mndb; // DB queries so we avoid an  infinite loop (update & create use this function)
		$users_tokens = self::get_customer_tokens( $user_id );
		foreach ( $users_tokens as $token ) {
			if ( $token_id === $token->get_id() ) {
				$token->set_default( true );
				$mndb->update(
					$mndb->prefix . 'woocommerce_payment_tokens',
					array( 'is_default' => 1 ),
	...
	public static function get_token_type_by_id( $token_id ) {
		global $mndb;
		$type = $mndb->get_var( $mndb->prepare(
			"SELECT type FROM {$mndb->prefix}woocommerce_payment_tokens WHERE token_id = %d",
			$token_id
		) );
		return $type;
	}
