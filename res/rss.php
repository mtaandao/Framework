...
define('RSS', 'RSS');
define('ATOM', 'Atom');
define('MAGPIE_USER_AGENT', 'Mtaandao/' . $GLOBALS['mn_version']);
...
/**
 * Retrieve URL headers and content using MN HTTP Request API.
 *
 * @since 1.5.0
 * @package External
 * @subpackage MagpieRSS
...
function _fetch_remote_file($url, $headers = "" ) {
	$resp = mn_safe_remote_request( $url, array( 'headers' => $headers, 'timeout' => MAGPIE_FETCH_TIME_OUT ) );
	if ( is_mn_error($resp) ) {
	...
	// Also note, MN_HTTP lowercases all keys, Snoopy did not.
	$return_headers = array();
	foreach ( mn_remote_retrieve_headers( $resp ) as $key => $value ) {
		if ( !is_array($value) ) {
	...
	$response->status = mn_remote_retrieve_response_code( $resp );
	$response->response_code = mn_remote_retrieve_response_code( $resp );
	$response->headers = $return_headers;
	$response->results = mn_remote_retrieve_body( $resp );
...
function init () {
	...
	if ( !defined('MAGPIE_USER_AGENT') ) {
		$ua = 'Mtaandao/' . $GLOBALS['mn_version'];
 ...
 */
function mn_rss( $url, $num_items = -1 ) {
 ...
 * to display. You can't display all of them like you can with mn_rss()
 * function.
 *
 * @since 1.5.0
 * @package External
