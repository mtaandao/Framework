...
/**
 * Taxonomy API: MN_Term_Query class.
 *
 * @package Mtaandao
 * @subpackage Taxonomy
 * @since 16.10.0
 ...
 * @since 16.10.0
 *
 * @see MN_Term_Query::__construct() for accepted arguments.
 */
class MN_Term_Query {
	 ...
	 *
	 * @since 16.10.0
	 * @since 16.10.0 Introduced 'term_taxonomy_id' parameter.
	 * @access public
	 *
	 ...
	 * @access public
	 *
	 * @param string|array $query MN_Term_Query arguments. See MN_Term_Query::__construct()
	 */
	public function parse_query( $query = '' ) {
		...
		$query = mn_parse_args( $query, $this->query_var_defaults );
		 ...
		 * @since 16.10.0
		 *
		 * @param MN_Term_Query $this Current instance of MN_Term_Query.
		 */
	...
	public function get_terms() {
		...
		if ( ! empty( $include ) ) {
			$exclude = '';
			$exclude_tree = '';
			$inclusions = implode( ',', mn_parse_id_list( $include ) );
		}
		...
		if ( ! empty( $exclude_tree ) ) {
			$exclude_tree = mn_parse_id_list( $exclude_tree );
		...
		if ( ! empty( $exclude ) ) {
			$exclusions = array_merge( mn_parse_id_list( $exclude ), $exclusions );
		}
		...
		if ( ! empty( $args['term_taxonomy_id'] ) ) {
			if ( is_array( $args['term_taxonomy_id'] ) ) {
				...
				$this->sql_clauses['where']['term_taxonomy_id'] = $mndb->prepare( "tt.term_taxonomy_id = %d", $args['term_taxonomy_id'] );
			}
		}

		if ( ! empty( $args['name__like'] ) ) {
			$this->sql_clauses['where']['name__like'] = $mndb->prepare( "t.name LIKE %s", '%' . $mndb->esc_like( $args['name__like'] ) . '%' );
		}

		if ( ! empty( $args['description__like'] ) ) {
			$this->sql_clauses['where']['description__like'] = $mndb->prepare( "tt.description LIKE %s", '%' . $mndb->esc_like( $args['description__like'] ) . '%' );
		}
		...
		$join .= " INNER JOIN $mndb->term_taxonomy AS tt ON t.term_id = tt.term_id";
		...
		$this->sql_clauses['from']    = "FROM $mndb->terms AS t $join";
		...
		$key = md5( serialize( mn_array_slice_assoc( $args, array_keys( $this->query_var_defaults ) ) ) . serialize( $taxonomies ) . $this->request );
		$last_changed = mn_cache_get( 'last_changed', 'terms' );
		if ( ! $last_changed ) {
			$last_changed = microtime();
			mn_cache_set( 'last_changed', $last_changed, 'terms' );
		}
		$cache_key = "get_terms:$key:$last_changed";
		$cache = mn_cache_get( $cache_key, 'terms' );
		if ( false !== $cache ) {
			if ( 'all' === $_fields ) {
		...
		if ( 'count' == $_fields ) {
			return $mndb->get_var( $this->request );
		}

		$terms = $mndb->get_results( $this->request );
		if ( 'all' == $_fields ) {
		...
		if ( empty( $terms ) ) {
			mn_cache_add( $cache_key, array(), 'terms', DAY_IN_SECONDS );
			return array();
		}
		...
		mn_cache_add( $cache_key, $terms, 'terms', DAY_IN_SECONDS );
	 ...
	 * @access public
	 *
	 * @param string $orderby_raw Raw 'orderby' value passed to MN_Term_Query.
	 * @return string
	 */
	...
	protected function get_search_sql( $string ) {
		...
		return $mndb->prepare( '((t.name LIKE %s) OR (t.slug LIKE %s))', $like, $like );
	}
}
