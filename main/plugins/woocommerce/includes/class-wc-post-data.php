...
class WC_Post_Data {
	...
	public static function init() {
		...
		add_filter( 'mn_insert_post_data', array( __CLASS__, 'mn_insert_post_data' ) );
	...
	public static function edited_term( $term_id, $tt_id, $taxonomy ) {
		if ( ! is_null( self::$editing_term ) && strpos( $taxonomy, 'pa_' ) === 0 ) {
			...
			if ( $edited_term->slug !== self::$editing_term->slug ) {
				...
				$mndb->query( $mndb->prepare( "UPDATE {$mndb->postmeta} SET meta_value = %s WHERE meta_key = %s AND meta_value = %s;", $edited_term->slug, 'attribute_' . sanitize_title( $taxonomy ), self::$editing_term->slug ) );
			}
		} else {
			self::$editing_term = null;
		}
	 ...
	 * @return array
	 */
	public static function mn_insert_post_data( $data ) {
