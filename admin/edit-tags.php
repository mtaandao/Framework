...
if ( ! in_array( $tax->name, get_taxonomies( array( 'show_ui' => true ) ) ) ) {
   mn_die( __( 'Sorry, you are not allowed to manage these items.' ) );
}
...
/**
 * $post_type is set when the MN_Terms_List_Table instance is created
 *
 * @global string $post_type
 */
global $post_type;

$mn_list_table = _get_list_table('MN_Terms_List_Table');
...
switch ( $mn_list_table->current_action() ) {

case 'add-tag':

	check_admin_referer( 'add-tag', '_mnnonce_add-tag' );
	...
	$ret = mn_insert_term( $_POST['tag-name'], $taxonomy, $_POST );
...
case 'delete':
	...
	mn_delete_term( $tag_ID, $taxonomy );
...
case 'edit':
	...
	mn_redirect( esc_url_raw( get_edit_term_link( $term_id, $taxonomy, $post_type ) ) );
	exit;
...
case 'editedtag':
	...
	$ret = mn_update_term( $tag_ID, $taxonomy, $_POST );
...
if ( ! $location && ! empty( $_REQUEST['_mn_http_referer'] ) ) {
	$location = remove_query_arg( array('_mn_http_referer', '_mnnonce'), mn_unslash($_SERVER['REQUEST_URI']) );
}
...
if ( $location ) {
	 ...
	 */
	mn_redirect( apply_filters( 'redirect_term_location', $location, $tax ) );
	exit;
}
...
mn_enqueue_script('admin-tags');
if ( current_user_can($tax->cap->edit_terms) )
	mn_enqueue_script('inline-edit-tax');
...
if ( 'category' == $taxonomy || 'link_category' == $taxonomy || 'post_tag' == $taxonomy  ) {
	...
	if ( 'category' == $taxonomy )
		$help .= '<p>' . __( '<a href="https://mtaandao.co.ke/docs/Posts_Categories_Screen" target="_blank">Documentation on Categories</a>' ) . '</p>';
	elseif ( 'link_category' == $taxonomy )
		$help .= '<p>' . __( '<a href="https://mtaandao.co.ke/docs/Links_Link_Categories_Screen" target="_blank">Documentation on Link Categories</a>' ) . '</p>';
	else
		$help .= '<p>' . __( '<a href="https://mtaandao.co.ke/docs/Posts_Tags_Screen" target="_blank">Documentation on Tags</a>' ) . '</p>';

	$help .= '<p>' . __('<a href="https://mtaandao.co.ke/support/" target="_blank">Support Forums</a>') . '</p>';
...
if ( is_plugin_active( 'mncat2tag-importer/mncat2tag-importer.php' ) ) {
	$import_link = admin_url( 'admin.php?import=mncat2tag' );
} else {
...
if ( isset( $_REQUEST['s'] ) && strlen( $_REQUEST['s'] ) ) {
	...
	printf( '<span class="subtitle">' . __( 'Search results for &#8220;%s&#8221;' ) . '</span>', esc_html( mn_unslash( $_REQUEST['s'] ) ) );
}
?>
</h1>
...
<form class="search-form mn-clearfix" method="get">
<input type="hidden" name="taxonomy" value="<?php echo esc_attr($taxonomy); ?>" />
<input type="hidden" name="post_type" value="<?php echo esc_attr($post_type); ?>" />

<?php $mn_list_table->search_box( $tax->labels->search_items, 'tag' ); ?>

</form>

<div id="col-container" class="mn-clearfix">
...
<input type="hidden" name="taxonomy" value="<?php echo esc_attr($taxonomy); ?>" />
<input type="hidden" name="post_type" value="<?php echo esc_attr($post_type); ?>" />
<?php mn_nonce_field('add-tag', '_mnnonce_add-tag'); ?>
...
<div class="form-field term-parent-wrap">
	...
	mn_dropdown_categories( $dropdown_args );
	?>
...
if ( ! is_null( $tax->labels->popular_items ) ) {
	if ( current_user_can( $tax->cap->edit_terms ) ) {
		$tag_cloud = mn_tag_cloud( array( 'taxonomy' => $taxonomy, 'post_type' => $post_type, 'echo' => false, 'link' => 'edit' ) );
	} else {
		$tag_cloud = mn_tag_cloud( array( 'taxonomy' => $taxonomy, 'echo' => false ) );
	}
