 ...
 */
 abstract class WC_Payment_Token extends WC_Data {
	...
	public function read( $token_id ) {
		global $mndb;
		if ( $token = $mndb->get_row( $mndb->prepare( "SELECT * FROM {$mndb->prefix}woocommerce_payment_tokens WHERE token_id = %d LIMIT 1;", $token_id ) ) ) {
	...
	public function create() {
		...
		$mndb->insert( $mndb->prefix . 'woocommerce_payment_tokens', $payment_token_data );
		$this->_data['id'] = $token_id = $mndb->insert_id;
		$this->save_meta_data();
	...
	public function delete() {
		...
		$mndb->delete( $mndb->prefix . 'woocommerce_payment_tokens', array( 'token_id' => $this->get_id() ), array( '%d' ) );
		$mndb->delete( $mndb->prefix . 'woocommerce_payment_tokenmeta', array( 'payment_token_id' => $this->get_id() ), array( '%d' ) );
