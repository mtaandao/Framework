...
abstract class WC_REST_Terms_Controller extends WC_REST_Controller {
	...
	protected function check_permissions( $request, $context = 'read' ) {
		...
		if ( ! $taxonomy ) {
			return new MN_Error( 'woocommerce_rest_taxonomy_invalid', __( "Taxonomy doesn't exist.", 'woocommerce' ), array( 'status' => 404 ) );
		}
		...
		if ( $id = intval( $request['id'] ) ) {
			...
			if ( ! $term || $term->taxonomy !== $taxonomy ) {
				return new MN_Error( 'woocommerce_rest_term_invalid', __( "Resource doesn't exist.", 'woocommerce' ), array( 'status' => 404 ) );
			}
	...
	public function get_items( $request ) {
		...
		if ( ! empty( $prepared_args['product'] )  ) {
			...
			$total_terms = mn_count_terms( $taxonomy, $count_args );
			...
			// mn_count_terms can return a falsy value when the term has no children.
			if ( ! $total_terms ) {
				$total_terms = 0;
			}
		}
		...
		$response->header( 'X-MN-Total', (int) $total_terms );
	...
	public function create_item( $request ) {
		...
		if ( isset( $request['parent'] ) ) {
			if ( ! is_taxonomy_hierarchical( $taxonomy ) ) {
				return new MN_Error( 'woocommerce_rest_taxonomy_not_hierarchical', __( 'Can not set resource parent, taxonomy is not hierarchical.', 'woocommerce' ), array( 'status' => 400 ) );
			}

			$parent = get_term( (int) $request['parent'], $taxonomy );

			if ( ! $parent ) {
				return new MN_Error( 'woocommerce_rest_term_invalid', __( "Parent resource doesn't exist.", 'woocommerce' ), array( 'status' => 404 ) );
			}

			$args['parent'] = $parent->term_id;
		}

		$term = mn_insert_term( $name, $taxonomy, $args );
		if ( is_mn_error( $term ) ) {
		 ...
		 *
		 * @param MN_Term         $term      Inserted Term object.
	...
	public function update_item( $request ) {
		...
		if ( isset( $request['parent'] ) ) {
			...
			if ( ! $parent ) {
				return new MN_Error( 'woocommerce_rest_term_invalid', __( "Parent resource doesn't exist.", 'woocommerce' ), array( 'status' => 400 ) );
			}
		...
		if ( ! empty( $prepared_args ) ) {
			$update = mn_update_term( $term->term_id, $term->taxonomy, $prepared_args );
	...
	public function delete_item( $request ) {
		...
		$retval = mn_delete_term( $term->term_id, $term->taxonomy );
		if ( ! $retval ) {
		 ...
		 *
		 * @param MN_Term          $term     The deleted term.
