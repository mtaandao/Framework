...
class Vc_Vendor_Qtranslate implements Vc_Vendor_Interface {
	...
	public function qtransPostInit() {
		...
		switchEditors.go = function(id, lang) {
			...
			var dom = tinymce.DOM;
			var wrap_id = 'mn-'+id+'-wrap';
			var wrap_id2 = 'mn-qtrans_textarea_content-wrap';
			...
			if (lang !== 'tmce' && lang !== 'html' && document.getElementById('qtrans_select_'+lang).className === 'mn-switch-editor switch-tmce switch-html') {
				return;
			}

			if (lang !== 'tmce' && lang !== 'html') {
				document.getElementById('qtrans_select_'+qtrans_get_active_language()).className='mn-switch-editor';
				document.getElementById('qtrans_select_'+lang).className='mn-switch-editor switch-tmce switch-html';
			}
			...
			if (lang === 'html') {
				...
				if ( tinyMCEPreInit.mceInit['qtrans_textarea_'+id] && tinyMCEPreInit.mceInit['qtrans_textarea_'+id].mnautop )
					vta.value = this.mnautop(qtrans_use(qtrans_get_active_language(),ta.value));
				if (inst) {
					inst.show();
				} else {
	...
	public function enqueueJsBackend() {
		...
		if ( $this->isValidPostType() || apply_filters( 'vc_vendor_qtranslate_enqueue_js_backend', false ) ) {
			...
			mn_enqueue_script( 'vc_vendor_qtranslate_backend', vc_asset_url( 'js/vendors/qtranslate_backend.js' ), array( 'vc-backend-min-js' ), '1.0', true );
		}
	}
	...
	public function enqueueJsFrontend() {
		if ( $this->isValidPostType() ) {
			...
			mn_enqueue_script( 'vc_vendor_qtranslate_frontend', vc_asset_url( 'js/vendors/qtranslate_frontend.js' ), array( 'vc-frontend-editor-min-js' ), '1.0', true );
			global $q_config;
			...
			$q_config['js']['qtrans_mnActiveEditorOverload'] = '';
			$q_config['js']['qtrans_updateTinyMCE'] = '';
			$q_config['js']['qtrans_mnOnload'] = '';
