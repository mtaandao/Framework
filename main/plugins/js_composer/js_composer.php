...
Description: Drag and drop page builder for Mtaandao. Take full control over your Mtaandao site, build any layout you can imagine – no programming knowledge required.
Version: 5.0
*/
...
/**
 * Vc starts here. Manager sets mode, adds required mn hooks and loads required object of structure
 *
...
class Vc_Manager {
	 ...
	 *
	 * It depends on what functionality is required from vc to work with current page/part of MN.
	 *
	 * Possible values:
	 *  none - current status is unknown, default mode;
	 *  page - simple mn page;
	 *  admin_page - mn dashboard;
	...
	/**
	 * Constructor loads API functions, defines paths and adds required mn actions
	 *
	 * @since  4.2
	 */
	private function __construct() {
		 ...
		 * MN_ROOT         - MN application root directory.
	...
	/**
	 * Callback function MN plugin_loaded action hook. Loads locale
	 *
	 * @since  4.2
	 * @access public
	 */
	...
	/**
	 * Callback function for MN init action hook. Sets Vc mode and loads required objects.
	 *
	 * @since  4.2
	 * @access public
	 *
	...
	public function init() {
		...
		vc_automapper()->map();
		if ( vc_user_access()->mnAny( 'manage_options' )->part( 'settings' )->can( 'vc-updater-tab' )->get() ) {
	...
	protected function setMode() {
		...
		if ( is_admin() ) {
			...
			} elseif ( ( vc_user_access()->mnAny( 'edit_posts', 'edit_pages' )
					->get() ) && ( 'vc_upgrade' === vc_action() || ( 'update-selected' === vc_get_param( 'action' ) && $this->pluginName() === vc_get_param( 'plugins' ) ) )
			) {
				$this->mode = 'admin_updater';
			} elseif ( vc_user_access()->mnAny( 'manage_options' )->get() && isset( $_GET['page'] ) && array_key_exists( $_GET['page'], vc_settings()->getTabs() ) ) {
			...
			if ( isset( $_GET['vc_editable'] ) && 'true' === $_GET['vc_editable'] ) {
				vc_user_access()->checkAdminNonce()->validateDie()->mnAny( array(
					'edit_post',
	...
	/**
	 * Setter for as network plugin for MultiMN.
	 *
	 * @since  4.2
	 * @access public
	 *
