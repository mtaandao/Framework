<?php
/**
 * Mtaandao Administration Revisions API
 *
 * @package Mtaandao
 ...
 *                    Or, false on failure.
 */
function mn_get_revision_ui_diff( $post, $compare_from, $compare_to ) {
	...
	foreach ( _mn_post_revision_fields( $post ) as $field => $name ) {
		/**
		 ...
		 * @param MN_Post $compare_from         The revision post object to compare to or from.
		 * @param string  null                  The context of whether the current revision is the old
		 *                                      or the new one. Values are 'to' or 'from'.
		 */
		$content_from = $compare_from ? apply_filters( "_mn_post_revision_field_$field", $compare_from->$field, $field, $compare_from, 'from' ) : '';

		/** This filter is documented in admin/includes/revision.php */
		$content_to = apply_filters( "_mn_post_revision_field_$field", $compare_to->$field, $field, $compare_to, 'to' );
		 ...
		 *
		 * Filters the options passed to mn_text_diff() when viewing a post revision.
		 *
		 * @since 4.1.0
		 *
		 * @param array   $args {
		 *     Associative array of options to pass to mn_text_diff().
		 *
		 *     @type bool $show_split_view True for split view (two columns), false for
		 *                                 un-split view (single column). Default true.
		 * }
		 * @param string  $field        The current revision field.
		 * @param MN_Post $compare_from The revision post to compare from.
		 * @param MN_Post $compare_to   The revision post to compare to.
		 */
		$args = apply_filters( 'revision_text_diff_options', $args, $field, $compare_from, $compare_to );

		$diff = mn_text_diff( $content_from, $content_to, $args );
	 ...
	 * @param MN_Post $compare_to   The revision post to compare to.
	 */
	return apply_filters( 'mn_get_revision_ui_diff', $return, $compare_from, $compare_to );
 ...
 */
function mn_prepare_revisions_for_js( $post, $selected_revision_id, $from = null ) {
	$post = get_post( $post );
	$authors = array();
	$now_gmt = time();

	$revisions = mn_get_post_revisions( $post->ID, array( 'order' => 'ASC', 'check_enabled' => false ) );
	...
	cache_users( mn_list_pluck( $revisions, 'post_author' ) );
	...
	foreach ( $revisions as $revision ) {
		...
		if ( $can_restore ) {
			$restore_link = str_replace( '&amp;', '&', mn_nonce_url(
				add_query_arg(
		...
		$autosave = (bool) mn_is_post_autosave( $revision );
		 ...
		 *     @type string     $title      Title for the revision's parent MN_Post object.
		 ...
		 * }
		 * @param MN_Post $revision       The revision's MN_Post object.
		 * @param MN_Post $post           The revision's parent MN_Post object.
		 */
		$revisions[ $revision->ID ] = apply_filters( 'mn_prepare_revision_for_js', $revisions_data, $revision, $post );
	}
	...
	$diffs = array( array(
		...
		'fields' => mn_get_revision_ui_diff( $post->ID, $from, $selected_revision_id ),
	));

	return array(
		'postId'           => $post->ID,
		'nonce'            => mn_create_nonce( 'revisions-ajax-nonce' ),
 ...
 * @since 4.1.0
 *
 * @global MN_Post $post The global `$post` object.
 */
function mn_print_revision_templates() {
	global $post;
		...
		<# if ( ! _.isUndefined( data.attributes ) ) { #>
			<div class="diff-title">
				...
				<# if ( 'to' === data.type && data.attributes.restoreUrl ) { #>
					<input  <?php if ( mn_check_post_lock( $post->ID ) ) { ?>
						disabled="disabled"
					<?php } else { ?>
