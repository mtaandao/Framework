...
function wc_get_orders( $args ) {
	...
	// Handle some BW compatibility arg names where mn_query args differ in naming.
	$map_legacy = array(
		'numberposts'    => 'limit',
		'post_type'      => 'type',
		'post_status'    => 'status',
	...
	/**
	 * Generate MN_Query args. This logic will change if orders are moved to
	...
	if ( ! is_null( $args['parent'] ) ) {
		$mn_query_args['post_parent'] = absint( $args['parent'] );
	}

	if ( ! is_null( $args['offset'] ) ) {
		$mn_query_args['offset'] = absint( $args['offset'] );
	} else {
		$mn_query_args['paged'] = absint( $args['page'] );
	}

	if ( ! empty( $args['customer'] ) ) {
		$values = is_array( $args['customer'] ) ? $args['customer'] : array( $args['customer'] );
		$mn_query_args['meta_query'][] = _wc_get_orders_generate_customer_meta_query( $values );
	}

	if ( ! empty( $args['exclude'] ) ) {
		$mn_query_args['post__not_in'] = array_map( 'absint', $args['exclude'] );
	}

	if ( ! $args['paginate' ] ) {
		$mn_query_args['no_found_rows'] = true;
	}

	// Get results.
	$orders = new MN_Query( $mn_query_args );
...
function wc_get_order_id_by_order_key( $order_key ) {
	...
	$order_id = $mndb->get_var( $mndb->prepare( "SELECT post_id FROM {$mndb->prefix}postmeta WHERE meta_key = '_order_key' AND meta_value = %s", $order_key ) );
...
function wc_register_order_type( $type, $args = array() ) {
	...
	if ( is_mn_error( register_post_type( $type, $args ) ) ) {
		return false;
	}
	...
	$args                    = mn_parse_args( $args, $order_type_args );
...
function wc_downloadable_file_permission( $download_id, $product_id, $order, $qty = 1 ) {
	...
	$result = $mndb->insert( $mndb->prefix . 'woocommerce_downloadable_product_permissions',
		$data,
		$format
	);
	...
	return $result ? $mndb->insert_id : false;
}
...
function wc_add_order_item( $order_id, $item ) {
	...
	$item = mn_parse_args( $item, $defaults );

	$mndb->insert(
		$mndb->prefix . "woocommerce_order_items",
		array(
...
function wc_update_order_item( $item_id, $args ) {
	...
	$update = $mndb->update( $mndb->prefix . 'woocommerce_order_items', $args, array( 'order_item_id' => $item_id ) );
...
function wc_delete_order_item( $item_id ) {
	...
	$mndb->query( $mndb->prepare( "DELETE FROM {$mndb->prefix}woocommerce_order_items WHERE order_item_id = %d", $item_id ) );
	$mndb->query( $mndb->prepare( "DELETE FROM {$mndb->prefix}woocommerce_order_itemmeta WHERE order_item_id = %d", $item_id ) );
...
function wc_update_order_item_meta( $item_id, $meta_key, $meta_value, $prev_value = '' ) {
	if ( update_metadata( 'order_item', $item_id, $meta_key, $meta_value, $prev_value ) ) {
		...
		mn_cache_delete( $cache_key, 'orders' );
		return true;
	}
	return false;
}
...
function wc_cancel_unpaid_orders() {
	...
	$unpaid_orders = $mndb->get_col( $mndb->prepare( "
		SELECT posts.ID
...
function wc_orders_count( $status ) {
	...
	$cached_count = mn_cache_get( $cache_key, 'counts' );
	...
	foreach ( wc_get_order_types( 'order-count' ) as $type ) {
		$query = "SELECT COUNT( * ) FROM {$mndb->posts} WHERE post_type = %s AND post_status = %s";
		$count += $mndb->get_var( $mndb->prepare( $query, $type, $status ) );
	}

	mn_cache_set( $cache_key, $count, 'counts' );
 ...
 * @since 2.2
 * @param array $args
 * @return WC_Order_Refund|MN_Error
 */
function wc_create_refund( $args = array() ) {
	...
	if ( $updating ) {
		$refund_id = mn_update_post( $refund_data );
	} else {
		$refund_id = mn_insert_post( apply_filters( 'woocommerce_new_refund_data', $refund_data ), true );
	}

	if ( is_mn_error( $refund_id ) ) {
		return $refund_id;
	}
...
function wc_get_tax_class_by_tax_id( $tax_id ) {
	...
	$tax_class = $mndb->get_var( $mndb->prepare( "SELECT tax_rate_class FROM {$mndb->prefix}woocommerce_tax_rates WHERE tax_rate_id = %d", $tax_id ) );
...
function wc_order_search( $term ) {
	...
	if ( is_numeric( $term ) ) {
		$post_ids = array_unique( array_merge(
			$mndb->get_col(
				$mndb->prepare( "SELECT DISTINCT p1.post_id FROM {$mndb->postmeta} p1 WHERE p1.meta_key IN ('" . implode( "','", array_map( 'esc_sql', $search_fields ) ) . "') AND p1.meta_value LIKE '%%%s%%';", wc_clean( $term ) )
			),
			array( absint( $term ) )
		) );
		...
		$post_ids = array_unique( array_merge(
			$mndb->get_col(
				$mndb->prepare( "
					...
					FROM {$mndb->postmeta} p1
					INNER JOIN {$mndb->postmeta} p2 ON p1.post_id = p2.post_id
					WHERE
			...
			$mndb->get_col(
				$mndb->prepare( "
					SELECT order_id
					FROM {$mndb->prefix}woocommerce_order_items as order_items
