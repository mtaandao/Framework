...
class Vc_License {
	 ...
	 * @var string
	 */
	static protected $support_host = 'http://support.mtaandao.co.ke/plugins/page-builder';
	...
	public function init() {
		...
		add_action( 'mn_ajax_vc_get_activation_url', array(
			$this,
			'startActivationResponse',
		) );
		add_action( 'mn_ajax_vc_get_deactivation_url', array(
			$this,
			'startDeactivationResponse',
		) );

		// @deprecated 4.8 Remove after 2015-12-01
		add_action( 'mn_ajax_wpb_activate_license', array(
			$this,
			'activate',
		) );
		add_action( 'mn_ajax_wpb_deactivate_license', array(
			$this,
			'deactivate',
		) );

		add_action( 'mn_ajax_nopriv_vc_check_license_key', array(
			vc_license(),
	...
	function finishActivationDeactivation( $activation, $user_token ) {
		...
		$response = mn_remote_post( $url, $params );
	...
	public function startActivationResponse() {
		vc_user_access()->checkAdminNonce()->validateDie()->mnAny( 'manage_options' )->validateDie()->part( 'settings' )->can( 'vc-updater-tab' )->validateDie();
	...
	public function startDeactivationResponse() {
		vc_user_access()->checkAdminNonce()->validateDie( 'Failed nonce check' )->mnAny( 'manage_options' )->validateDie( 'Failed access check' )->part( 'settings' )->can( 'vc-updater-tab' )
	...
	public function activate() {
		...
		$response = mn_remote_get( $request_url, array( 'timeout' => 300 ) );
		if ( is_mn_error( $response ) ) {
	 ...
	 * - ip address
	 * - tld is local, dev, mn, test, example, localhost or invalid
