...
?>
<a class="preview button" href="<?php echo $preview_link; ?>" target="mn-preview-<?php echo (int) $post->ID; ?>" id="post-preview"><?php echo $preview_button; ?></a>
<input type="hidden" name="mn-preview" id="mn-preview" value="" />
</div>
 ...
 * @since 4.4.0
 *
 * @param MN_Post $post MN_Post object for the current post.
 */
	...
	<div id="taxonomy-<?php echo $tax_name; ?>" class="categorydiv">
		...
		<div id="<?php echo $tax_name; ?>-pop" class="tabs-panel" style="display: none;">
			<ul id="<?php echo $tax_name; ?>checklist-pop" class="categorychecklist form-no-clear" >
				<?php $popular_ids = mn_popular_terms_checklist( $tax_name ); ?>
			</ul>
		</div>
		...
		<div id="<?php echo $tax_name; ?>-all" class="tabs-panel">
			...
			?>
			<ul id="<?php echo $tax_name; ?>checklist" data-mn-lists="list:<?php echo $tax_name; ?>" class="categorychecklist form-no-clear">
				<?php mn_terms_checklist( $post->ID, array( 'taxonomy' => $tax_name, 'popular_cats' => $popular_ids ) ); ?>
			</ul>
		</div>
	<?php if ( current_user_can( $taxonomy->cap->edit_terms ) ) : ?>
			<div id="<?php echo $tax_name; ?>-adder" class="mn-hidden-children">
				...
				<p id="<?php echo $tax_name; ?>-add" class="category-add mn-hidden-child">
					...
					mn_dropdown_categories( $parent_dropdown_args );
					?>
					<input type="button" id="<?php echo $tax_name; ?>-add-submit" data-mn-lists="add:<?php echo $tax_name; ?>checklist:<?php echo $tax_name; ?>-add" class="button category-add-submit" value="<?php echo esc_attr( $taxonomy->labels->add_new_item ); ?>" />
					<?php mn_nonce_field( 'add-' . $tax_name, '_ajax_nonce-add-' . $tax_name, false ); ?>
...
<p><?php
	printf(
		...
		__( 'https://mtaandao.co.ke/docs/Excerpt' )
	);
?></p>
<?php
}
...
<p><?php
	printf(
		/* translators: %s: Codex URL */
		__( 'Trackbacks are a way to notify legacy blog systems that you&#8217;ve linked to them. If you link other Mtaandao sites, they&#8217;ll be notified automatically using <a href="%s">pingbacks</a>, no other action necessary.' ),
		__( 'https://mtaandao.co.ke/docs/Introduction_to_Blogging#Managing_Comments' )
	);
?></p>
<?php
if ( ! empty($pings) )
...
<p><?php
	printf(
		...
		__( 'https://mtaandao.co.ke/docs/Using_Custom_Fields' )
	);
?></p>
<?php
}
...
<p class="meta-options">
	 ...
	 * @since 3.1.0
	 *
	 * @param MN_Post $post MN_Post object of the current post.
	 */
...
function post_comment_meta_box( $post ) {
	mn_nonce_field( 'get-comments', 'add_comment_nonce', false );
	?>
...
function page_attributes_meta_box($post) {
	...
	if ( $post_type_object->hierarchical ) {
		 ...
		 * @since 3.3.0
		 *
		 * @see mn_dropdown_pages()
		 *
		 * @param array   $dropdown_args Array of arguments used to generate the pages drop-down.
		 * @param MN_Post $post          The current MN_Post object.
		 */
		$dropdown_args = apply_filters( 'page_attributes_dropdown_pages_args', $dropdown_args, $post );
		$pages = mn_dropdown_pages( $dropdown_args );
		if ( ! empty($pages) ) {
?>
...
<p><strong><?php _e('Template') ?></strong><?php
	 ...
	 * @param MN_Post $post     The current post.
	 */
...
if ( !empty($_GET['action']) && 'edit' == $_GET['action'] && current_user_can('manage_links') ) { ?>
	<a class="submitdelete deletion" href="<?php echo mn_nonce_url("link.php?action=delete&amp;link_id=$link->link_id", 'delete-bookmark_' . $link->link_id); ?>" onclick="if ( confirm('<?php echo esc_js(sprintf(__("You are about to delete this link '%s'\n  'Cancel' to stop, 'OK' to delete."), $link->link_name )); ?>') ) {return true;}return false;"><?php _e('Delete'); ?></a>
<?php } ?>
</div>
...
<div id="taxonomy-linkcategory" class="categorydiv">
	...
	<div id="categories-all" class="tabs-panel">
		<ul id="categorychecklist" data-mn-lists="list:category" class="categorychecklist form-no-clear">
			<?php
			if ( isset($link->link_id) )
				mn_link_category_checklist($link->link_id);
			else
				mn_link_category_checklist();
			?>
		</ul>
	</div>
	...
	<div id="categories-pop" class="tabs-panel" style="display: none;">
		<ul id="categorychecklist-pop" class="categorychecklist form-no-clear">
			<?php mn_popular_terms_checklist('link_category'); ?>
		</ul>
	</div>

	<div id="category-adder" class="mn-hidden-children">
		<a id="category-add-toggle" href="#category-add" class="taxonomy-add-new"><?php _e( '+ Add New Category' ); ?></a>
		<p id="link-category-add" class="mn-hidden-child">
			<label class="screen-reader-text" for="newcat"><?php _e( '+ Add New Category' ); ?></label>
			<input type="text" name="newcat" id="newcat" class="form-required form-input-tip" value="<?php esc_attr_e( 'New category name' ); ?>" aria-required="true" />
			<input type="button" id="link-category-add-submit" data-mn-lists="add:categorychecklist:link-category-add" class="button" value="<?php esc_attr_e( 'Add' ); ?>" />
			<?php mn_nonce_field( 'add-link-category', '_ajax_nonce', false ); ?>
...
function post_thumbnail_meta_box( $post ) {
	...
	echo _mn_post_thumbnail_html( $thumbnail_id, $post->ID );
}
