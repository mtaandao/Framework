 ...
 * 
 */
class RM_Content_View_Restrictions_Posts_List {
    ...
    private function do_not_restrict_editors($post) {
        ...
        $restrict_even_if_can_edit = apply_filters('rm_restrict_content_view_for_authors_and_editors', false);
    ...
    private function do_not_hide_at_post_level($post) {
        ...
        $rm_content_for_roles = get_post_meta($post->ID, RM_Content_View_Restrictions::content_for_roles, true);
        if (empty($rm_content_for_roles)) {
            return true;
        }

        $roles = $this->extract_roles_from_string($rm_content_for_roles);
        if (count($roles) == 0) {
            return true;
        }
        ...
        $rm_prohibit_allow_flag = get_post_meta($post->ID, RM_Content_View_Restrictions::prohibit_allow_flag, true);
        $post_access_error_action = get_post_meta($post->ID, RM_Content_View_Restrictions::post_access_error_action, true);
        ...
        if (!is_user_logged_in()) {
            ...
            return $result;
        } elseif ($rm_prohibit_allow_flag == 1) {
            $result0 = true;
    ...
    private function do_not_hide_at_role_level($post) {
        ...
        if (RM_Content_View_Restrictions::is_object_restricted_for_role($post->ID, $blocked, 'posts')) {
            $result = $blocked['access_error_action'] == 2 ? true : false;
            return $result;
        }

        if (RM_Content_View_Restrictions::is_term_restricted_for_role($post->ID, $blocked)) {
            $result = $blocked['access_error_action'] == 2 ? true : false;
            return $result;
        }
    ...
    private function get_post_level_data_from_db() {
        ...
        $query .= " where {$mndb->postmeta}.meta_key='rm_content_for_roles' OR {$mndb->postmeta}.meta_key='rm_prohibit_allow_flag' OR".
                  " {$mndb->postmeta}.meta_key='rm_post_access_error_action'".
    ...
    private function get_post_level_restrictions() {
        ...
        foreach($data as $post_id => $restriction) {
            if (empty($restriction[RM_Content_View_Restrictions::content_for_roles])) { 
                continue;
            }
            if (isset($restriction[RM_Content_View_Restrictions::post_access_error_action]) && 
                $restriction[RM_Content_View_Restrictions::post_access_error_action]==2) { // show access error message
                continue;
            }            
            ...
            $roles = $this->extract_roles_from_string($restriction[RM_Content_View_Restrictions::content_for_roles]);
            if ($restriction[RM_Content_View_Restrictions::prohibit_allow_flag]==1) {   // Prohibited
    ...
    private function get_current_user_prohibited_posts() {
        ...
        $this->prohibited_posts = get_transient('rm_posts_view_access_prohibited_posts');
        if (!is_array($this->prohibited_posts) || !isset($this->prohibited_posts[$current_user->ID])) {            
            ...
            set_transient('rm_posts_view_access_prohibited_posts', $this->prohibited_posts, 30);
        }
...
// end of RM_Content_View_Restrictions_Posts_List