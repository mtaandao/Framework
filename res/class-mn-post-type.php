...
final class MN_Post_Type {
	 ...
	 * to use a font icon, e.g. 'dashicons-chart-pie'. Pass 'none' to leave div.mn-menu-image empty
	...
	public function add_rewrite_rules() {
		global $mn_rewrite, $mn;

		if ( false !== $this->query_var && $mn && is_post_type_viewable( $this ) ) {
			$mn->add_query_var( $this->query_var );
		}
		...
		if ( false !== $this->rewrite && ( is_admin() || '' != get_option( 'permalink_structure' ) ) ) {
			...
			if ( $this->has_archive ) {
				...
				if ( $this->rewrite['with_front'] ) {
					$archive_slug = substr( $mn_rewrite->front, 1 ) . $archive_slug;
				} else {
					$archive_slug = $mn_rewrite->root . $archive_slug;
				}

				add_rewrite_rule( "{$archive_slug}/?$", "index.php?post_type=$this->name", 'top' );
				if ( $this->rewrite['feeds'] && $mn_rewrite->feeds ) {
					$feeds = '(' . trim( implode( '|', $mn_rewrite->feeds ) ) . ')';
				...
				if ( $this->rewrite['pages'] ) {
					add_rewrite_rule( "{$archive_slug}/{$mn_rewrite->pagination_base}/([0-9]{1,})/?$", "index.php?post_type=$this->name" . '&paged=$matches[1]', 'top' );
				}
			}
	...
	public function remove_supports() {
		...
		unset( $_mn_post_type_features[ $this->name ] );
	}
	...
	public function remove_rewrite_rules() {
		global $mn, $mn_rewrite, $post_type_meta_caps;

		// Remove query var.
		if ( false !== $this->query_var ) {
			$mn->remove_query_var( $this->query_var );
		}
		...
		if ( false !== $this->rewrite ) {
			...
			foreach ( $mn_rewrite->extra_rules_top as $regex => $query ) {
				if ( false !== strpos( $query, "index.php?post_type=$this->name" ) ) {
					unset( $mn_rewrite->extra_rules_top[ $regex ] );
				}
			}
		}
