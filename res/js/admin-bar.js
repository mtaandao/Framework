...
if ( typeof(jQuery) != 'undefined' ) {
	if ( typeof(jQuery.fn.hoverIntent) == 'undefined' ) {
		/* jshint ignore:start */
		// hoverIntent v1.8.1 - Copy of res/js/hoverIntent.min.js
	...
	jQuery(document).ready(function($){
		var adminbar = $('#mnadminbar'), refresh, touchOpen, touchClose, disableHoverIntent = false;
		...
		touchOpen = function(unbind) {
			adminbar.find('li.menupop').on('click.mn-mobile-hover', function(e) {
				var el = $(this);

				if ( el.parent().is('#mn-admin-bar-root-default') && !el.hasClass('hover') ) {
					e.preventDefault();
				...
				if ( unbind ) {
					$('li.menupop').off('click.mn-mobile-hover');
		...
		touchClose = function() {
			...
			$(document.body).on( mobileEvent+'.mn-mobile-hover', function(e) {
				if ( !$(e.target).closest('#mnadminbar').length )
					adminbar.find('li.menupop.hover').removeClass('hover');
			});
		};
		...
		$('#mn-admin-bar-get-shortlink').click(function(e){
			e.preventDefault();
			$(this).addClass('selected').children('.shortlink-input').blur(function(){
				$(this).parents('#mn-admin-bar-get-shortlink').removeClass('selected');
			}).focus().select();
		});

		$('#mnadminbar li.menupop > .ab-item').bind('keydown.adminbar', function(e){
			if ( e.which != 13 )
				return;
		...
		$('#mnadminbar .ab-item').bind('keydown.adminbar', function(e){
			if ( e.which != 27 )
				return;
		...
		adminbar.click( function(e) {
			if ( e.target.id != 'mnadminbar' && e.target.id != 'mn-admin-bar-top-secondary' ) {
				return;
			}
		...
		if ( 'sessionStorage' in window ) {
			$('#mn-admin-bar-logout a').click( function() {
				try {
					for ( var key in sessionStorage ) {
						if ( key.indexOf('mn-autosave-') != -1 )
		...
		if ( navigator.userAgent && document.body.className.indexOf( 'no-font-face' ) === -1 &&
			/Android (1.0|1.1|1.5|1.6|2.0|2.1)|Nokia|Opera Mini|w(eb)?OSBrowser|webOS|UCWEB|Windows Phone OS 7|XBLMN7|ZuneMN7|MSIE 7/.test( navigator.userAgent ) ) {
	...
	(function(d, w) {
		...
		clickShortlink = function(e) {
			...
			while ( true ) {
				...
				if ( t.id && t.id == 'mn-admin-bar-get-shortlink' )
					break;
				t = t.parentNode;
			}
		...
		scrollToTop = function(t) {
			...
			// Ensure that the #mnadminbar was the target of the click.
			if ( t.id != 'mnadminbar' && t.id != 'mn-admin-bar-top-secondary' )
				return;
		...
		addEvent(w, 'load', function() {
			aB = d.getElementById('mnadminbar');
			...
			if ( d.body && aB ) {
				...
				addEvent( document.getElementById('mn-admin-bar-logout'), 'click', function() {
					if ( 'sessionStorage' in window ) {
						try {
							for ( var key in sessionStorage ) {
								if ( key.indexOf('mn-autosave-') != -1 )
