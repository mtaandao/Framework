			...
			<div class="wc-used-coupons">
				<ul class="wc_coupon_list"><?php
					...
					foreach ( $coupons as $item_id => $item ) {
						$post_id = $mndb->get_var( $mndb->prepare( "SELECT ID FROM {$mndb->posts} WHERE post_title = %s AND post_type = 'shop_coupon' AND post_status = 'publish' LIMIT 1;", $item['name'] ) );
			...
			<section class="wc-backbone-modal-main" role="main">
				...
				<article>
					<form action="" method="post">
						...
						<?php
							$rates = $mndb->get_results( "SELECT * FROM {$mndb->prefix}woocommerce_tax_rates ORDER BY tax_rate_name LIMIT 100" );
						...
						<?php if ( absint( $mndb->get_var( "SELECT COUNT(tax_rate_id) FROM {$mndb->prefix}woocommerce_tax_rates;" ) ) > 100 ) : ?>
							<p>
