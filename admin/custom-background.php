...
class Custom_Background {
	...
	public function __construct($admin_header_callback = '', $admin_image_div_callback = '') {
		...
		add_action( 'mn_ajax_custom-background-add', array( $this, 'ajax_background_add' ) );

		// Unused since 3.5.0.
		add_action( 'mn_ajax_set-background-image', array( $this, 'mn_set_background_image' ) );
	}
	...
	public function admin_load() {
		...
		get_current_screen()->set_help_sidebar(
			...
			'<p>' . __( '<a href="https://mtaandao.co.ke/docs/Appearance_Background_Screen" target="_blank">Documentation on Custom Background</a>' ) . '</p>' .
			'<p>' . __( '<a href="https://mtaandao.co.ke/support/" target="_blank">Support Forums</a>' ) . '</p>'
		);

		mn_enqueue_media();
		mn_enqueue_script('custom-background');
	...
	public function take_action() {
		...
		if ( isset($_POST['reset-background']) ) {
			check_admin_referer('custom-background-reset', '_mnnonce-custom-background-reset');
		...
		if ( isset($_POST['remove-background']) ) {
			...
			check_admin_referer('custom-background-remove', '_mnnonce-custom-background-remove');
			set_theme_mod('background_image', '');
			set_theme_mod('background_image_thumb', '');
			$this->updated = true;
			mn_safe_redirect( $_POST['_mn_http_referer'] );
			return;
		}
...
<td>
<form method="post">
<?php mn_nonce_field('custom-background-remove', '_mnnonce-custom-background-remove'); ?>
...
<td>
<form method="post">
<?php mn_nonce_field('custom-background-reset', '_mnnonce-custom-background-reset'); ?>
...
<td><form enctype="multipart/form-data" id="upload-form" class="mn-upload-form" method="post">
	<p>
		<label for="upload"><?php _e( 'Choose an image from your computer:' ); ?></label><br />
		<input type="file" id="upload" name="import" />
		<input type="hidden" name="action" value="save" />
		<?php mn_nonce_field( 'custom-background-upload', '_mnnonce-custom-background-upload' ); ?>
...
<?php mn_nonce_field('custom-background'); ?>
...
<?php
	...
	public function handle_upload() {
		...
		check_admin_referer('custom-background-upload', '_mnnonce-custom-background-upload');
		...
		// Save the data
		$id = mn_insert_attachment($object, $file);

		// Add the meta-data
		mn_update_attachment_metadata( $id, mn_generate_attachment_metadata( $id, $file ) );
		update_post_meta( $id, '_mn_attachment_is_custom_background', get_option('stylesheet' ) );

		set_theme_mod('background_image', esc_url_raw($url));

		$thumbnail = mn_get_attachment_image_src( $id, 'thumbnail' );
		set_theme_mod('background_image_thumb', esc_url_raw( $thumbnail[0] ) );

		/** This action is documented in admin/custom-header.php */
		do_action( 'mn_create_file_in_uploads', $file, $id ); // For replication
		$this->updated = true;
	}
	...
	public function ajax_background_add() {
		...
		update_post_meta( $attachment_id, '_mn_attachment_is_custom_background', get_stylesheet() );
	 ...
	 * @deprecated 3.5.0
	 */
	public function mn_set_background_image() {
		...
		update_post_meta( $attachment_id, '_mn_attachment_is_custom_background', get_option('stylesheet' ) );
		$url = mn_get_attachment_image_src( $attachment_id, $size );
		$thumbnail = mn_get_attachment_image_src( $attachment_id, 'thumbnail' );
