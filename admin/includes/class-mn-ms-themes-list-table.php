<?php
/**
 * List Table API: MN_MS_Themes_List_Table class
 *
 * @package Mtaandao
 ...
 * @see MN_List_Table
 */
class MN_MS_Themes_List_Table extends MN_List_Table {
	...
	public function prepare_items() {
		...
		mn_reset_vars( array( 'orderby', 'order', 's' ) );

		$themes = array(
			/**
			 * Filters the full array of MN_Theme objects to list in the Multisite
			 * themes list table.
			 *
			 * @since 3.1.0
			 *
			 * @param array $all An array of MN_Theme objects to display in the list table.
			 */
			'all' => apply_filters( 'all_themes', mn_get_themes() ),
			'search' => array(),
			'enabled' => array(),
			'disabled' => array(),
			'upgrade' => array(),
			'broken' => $this->is_site_themes ? array() : mn_get_themes( array( 'errors' => true ) ),
		);
		...
		MN_Theme::sort_by_name( $this->items );
	...
	public function column_name( $theme ) {
		...
		if ( ! $allowed ) {
			if ( ! $theme->errors() ) {
				...
				$actions['enable'] = sprintf( '<a href="%s" class="edit" aria-label="%s">%s</a>',
					esc_url( mn_nonce_url( $url, 'enable-theme_' . $stylesheet ) ),
					esc_attr( $aria_label ),
			...
			$actions['disable'] = sprintf( '<a href="%s" aria-label="%s">%s</a>',
				esc_url( mn_nonce_url( $url, 'disable-theme_' . $stylesheet ) ),
				esc_attr( $aria_label ),
		...
		if ( ! $allowed && current_user_can( 'delete_themes' ) && ! $this->is_site_themes && $stylesheet != get_option( 'stylesheet' ) && $stylesheet != get_option( 'template' ) ) {
			...
			$actions['delete'] = sprintf( '<a href="%s" class="delete" aria-label="%s">%s</a>',
				esc_url( mn_nonce_url( $url, 'bulk-themes' ) ),
				esc_attr( $aria_label ),
				__( 'Delete' )
			);
		}
	 ...
	 * @access public
	 *
	 * @param MN_Theme $item The current MN_Theme object.
	 */
	...
	public function single_row( $theme ) {
		...
		if ( $this->is_site_themes )
			remove_action( "after_theme_row_$stylesheet", 'mn_theme_update_row' );
