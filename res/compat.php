<?php
/**
 * Mtaandao implementation for PHP functions either missing from older PHP versions or not included by default.
 *
 * @package PHP
 * @access private
 */
 ...
 */
function _mn_can_use_pcre_u( $set = null ) {
...
if ( !function_exists('json_encode') ) {
	function json_encode( $string ) {
		...
		if ( ! ( $mn_json instanceof Services_JSON ) ) {
			require_once( ABSPATH . RES . '/class-json.php' );
			$mn_json = new Services_JSON();
		}

		return $mn_json->encodeUnsafe( $string );
	}
}

if ( !function_exists('json_decode') ) {
	/**
	 * @global Services_JSON $mn_json
	 * @param string $string
...
// Defined here to prevent a notice when using it with mn_json_encode()
...
if ( ! interface_exists( 'JsonSerializable' ) ) {
	define( 'MN_JSON_SERIALIZE_COMPATIBLE', true );
	/**
...
if ( ! function_exists( 'array_replace_recursive' ) ) :
	 ...
	 *
	 * The array_replace_recursive() function is a PHP 5.3 function. Mtaandao
	 ...
	 *
	 * Subject to removal once Mtaandao makes PHP 5.3.0 the minimum requirement.
	 *
	 * @since 4.5.3
	 *
...
if ( ! function_exists( 'spl_autoload_register' ) ):
	...
	function __autoload( $classname ) {
		global $_mn_spl_autoloaders;
		foreach ( $_mn_spl_autoloaders as $autoloader ) {
	...
	function spl_autoload_register( $autoload_function, $throw = true, $prepend = false ) {
		...
		if ( in_array( $autoload_function, $_mn_spl_autoloaders ) ) {
			return;
		}

		if ( $prepend ) {
			array_unshift( $_mn_spl_autoloaders, $autoload_function );
		} else {
			$_mn_spl_autoloaders[] = $autoload_function;
		}
	}
	...
	function spl_autoload_functions() {
		return $GLOBALS['_mn_spl_autoloaders'];
	}
endif;
