...
class WC_Query {
	...
	/** @public array Query vars to add to mn */
	...
	public function __construct() {
		...
		if ( ! is_admin() ) {
			add_action( 'mn_loaded', array( $this, 'get_errors' ), 20 );
			add_filter( 'query_vars', array( $this, 'add_query_vars'), 0 );
			add_action( 'parse_request', array( $this, 'parse_request'), 0 );
			add_action( 'pre_get_posts', array( $this, 'pre_get_posts' ) );
			add_action( 'mn', array( $this, 'remove_product_query' ) );
			add_action( 'mn', array( $this, 'remove_ordering_args' ) );
		}
	...
	public function get_endpoint_title( $endpoint ) {
		...
		switch ( $endpoint ) {
			...
			break;
			case 'orders' :
				if ( ! empty( $mn->query_vars['orders'] ) ) {
					$title = sprintf( __( 'Orders (page %d)', 'woocommerce' ), intval( $mn->query_vars['orders'] ) );
				} else {
					$title = __( 'Orders', 'woocommerce' );
				}
			break;
			case 'view-order' :
				$order = wc_get_order( $mn->query_vars['view-order'] );
	...
	public function parse_request() {
		...
		foreach ( $this->query_vars as $key => $var ) {
			if ( isset( $_GET[ $var ] ) ) {
				$mn->query_vars[ $key ] = $_GET[ $var ];
			}

			elseif ( isset( $mn->query_vars[ $var ] ) ) {
				$mn->query_vars[ $key ] = $mn->query_vars[ $var ];
			}
		}
	}
	...
	public function pre_get_posts( $q ) {
		...
		if ( $GLOBALS['mn_rewrite']->use_verbose_page_rules && isset( $q->queried_object->ID ) && $q->queried_object->ID === wc_get_page_id( 'shop' ) ) {
		...
		// When orderby is set, Mtaandao shows posts. Get around that here.
		...
		if ( $q->is_page() && 'page' === get_option( 'show_on_front' ) && absint( $q->get( 'page_id' ) ) === wc_get_page_id( 'shop' ) ) {
			...
			// Get the actual MN page to avoid errors and let us use is_front_page()
			// This is hacky but works. Awaiting https://core.trac.wordpress.org/ticket/21096
			global $mn_post_types;

			$shop_page 	= get_post( wc_get_page_id( 'shop' ) );

			$mn_post_types['product']->ID 			= $shop_page->ID;
			$mn_post_types['product']->post_title 	= $shop_page->post_title;
			$mn_post_types['product']->post_name 	= $shop_page->post_name;
			$mn_post_types['product']->post_type    = $shop_page->post_type;
			$mn_post_types['product']->ancestors    = get_ancestors( $shop_page->ID, $shop_page->post_type );
			...
			// Fix MN SEO
			if ( class_exists( 'MNSEO_Meta' ) ) {
				add_filter( 'mnseo_metadesc', array( $this, 'mnseo_metadesc' ) );
				add_filter( 'mnseo_metakey', array( $this, 'mnseo_metakey' ) );
			}
		...
		if ( is_search() ) {
			...
			add_filter( 'mn', array( $this, 'remove_posts_where' ) );
		}
	...
	public function search_post_excerpt( $where = '' ) {
		...
		if ( empty( $mn_the_query->query_vars['wc_query'] ) || empty( $mn_the_query->query_vars['s'] ) )
			return $where;
	...
	/**
	 * MN SEO meta description.
	 *
	 * Hooked into mnseo_ hook already, so no need for function_exist.
	 *
	 * @access public
	 * @return string
	 */
	public function mnseo_metadesc() {
		return MNSEO_Meta::get_value( 'metadesc', wc_get_page_id( 'shop' ) );
	}
	 ...
	 * @return string
	 */
	public function mnseo_metakey() {
		return MNSEO_Meta::get_value( 'metakey', wc_get_page_id( 'shop' ) );
	}
	...
	/**
	 * MN Core doens't let us change the sort direction for invidual orderby params - https://core.trac.wordpress.org/ticket/17065.
	 *
	...
	public function order_by_popularity_post_clauses( $args ) {
		global $mndb;
		$args['orderby'] = "$mndb->postmeta.meta_value+0 DESC, $mndb->posts.post_date DESC";
		return $args;
	}
	...
	public function order_by_rating_post_clauses( $args ) {
		...
		$args['fields'] .= ", AVG( $mndb->commentmeta.meta_value ) as average_rating ";
		$args['where']  .= " AND ( $mndb->commentmeta.meta_key = 'rating' OR $mndb->commentmeta.meta_key IS null ) ";
		$args['join']   .= "
			LEFT OUTER JOIN $mndb->comments ON($mndb->posts.ID = $mndb->comments.comment_post_ID)
			LEFT JOIN $mndb->commentmeta ON($mndb->comments.comment_ID = $mndb->commentmeta.comment_id)
		";
		$args['orderby'] = "average_rating DESC, $mndb->posts.post_date DESC";
		$args['groupby'] = "$mndb->posts.ID";
	...
	/**
	 * Based on MN_Query::parse_search
	 */
	public static function get_main_search_query_sql() {
		global $mn_the_query, $mndb;
		...
		foreach ( $search_terms as $term ) {
			...
			$sql[] = $mndb->prepare( "(($mndb->posts.post_title $like_op %s) $andor_op ($mndb->posts.post_excerpt $like_op %s) $andor_op ($mndb->posts.post_content $like_op %s))", $like, $like, $like );
		}

		if ( ! empty( $sql ) && ! is_user_logged_in() ) {
			$sql[] = "($mndb->posts.post_password = '')";
		}
