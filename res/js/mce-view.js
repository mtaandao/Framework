...
( function( window, mn, shortcode, $ ) {
	...
	mn.mce.views = {
		 ...
		 * @param {Object} extend An object to extend mn.mce.View.prototype with.
		 */
		register: function( type, extend ) {
			views[ type ] = mn.mce.View.extend( _.extend( extend, { type: type } ) );
		},
		...
		setMarkers: function( content ) {
			...
			_.each( views, function( view, type ) {
				...
				_.each( current, function( piece ) {
					...
					while ( remaining && ( result = view.prototype.match( remaining ) ) ) {
						...
						pieces.push( {
							content: instance.ignore ? text : '<p data-mnview-marker="' + instance.encodedText + '">' + text + '</p>',
							processed: true
						} );
			...
			return content.replace( /<p>\s*<p data-mnview-marker=/g, '<p data-mnview-marker=' ).replace( /<\/p>\s*<\/p>/g, '</p>' );
		},
		 ...
		 *
		 * @return {mn.mce.View} The view instance.
		 */
		 ...
		 *
		 * @return {mn.mce.View} The view instance or undefined.
		 */
		getInstance: function( object ) {
			...
			return instances[ $( object ).attr( 'data-mnview-text' ) ];
		},
		...
		getText: function( node ) {
			return decodeURIComponent( $( node ).attr( 'data-mnview-text' ) || '' );
		},
	 ...
	 * @param {Object} options Options.
	 */
	mn.mce.View = function( options ) {
		_.extend( this, options );
		this.initialize();
	};

	mn.mce.View.extend = Backbone.View.extend;

	_.extend( mn.mce.View.prototype, {
		...
		getNodes: function( callback, rendered ) {
			this.getEditors( function( editor ) {
				...
				$( editor.getBody() )
					.find( '[data-mnview-text="' + self.encodedText + '"]' )
					.filter( function() {
						var data;
		...
		getMarkers: function( callback ) {
			this.getEditors( function( editor ) {
				...
				$( editor.getBody() )
					.find( '[data-mnview-marker="' + this.encodedText + '"]' )
					.each( function() {
		...
		replaceMarkers: function() {
			this.getMarkers( function( editor, node ) {
				...
				if ( ! this.loader && $( node ).text() !== this.text ) {
					editor.dom.setAttrib( node, 'data-mnview-marker', null );
					return;
				}

				$viewNode = editor.$(
					'<div class="mnview mnview-wrap" data-mnview-text="' + this.encodedText + '" data-mnview-type="' + this.type + '" contenteditable="false"></div>'
				);
			...
			if ( _.isObject( content ) && content.body.indexOf( '<script' ) !== -1 ) {
				...
				this.getNodes( function( editor, node ) {
					...
					editor.undoManager.transact( function() {
						...
						editor.dom.add( node, 'span', { 'class': 'mnview-end' } );
					} );
			...
			this.getNodes( function( editor, node ) {
				...
				tinymce.each( dom.$( 'link[rel="stylesheet"]', editorHead ), function( link ) {
					if ( link.href && link.href.indexOf( 'skins/lightgray/content.min.css' ) === -1 &&
						link.href.indexOf( 'skins/mtaandao/main.css' ) === -1 ) {
				...
				setTimeout( function() {
					...
					editor.undoManager.transact( function() {
						...
						dom.add( node, 'span', { 'class': 'mnview-end' } );
					} );
							...
							'<head>' +
								...
								'<style>' +
									'html {' +
										...
										'margin: 0;' +
									'}' +
									'body#mnview-iframe-sandbox {' +
										'background: transparent;' +
										'padding: 1px 0 !important;' +
										'margin: -1px 0 0 !important;' +
									'}' +
									'body#mnview-iframe-sandbox:before,' +
									'body#mnview-iframe-sandbox:after {' +
										'display: none;' +
										'content: "";' +
									'}' +
								'</style>' +
							'</head>' +
							'<body id="mnview-iframe-sandbox" class="' + bodyClasses + '">' +
								body +
							'</body>' +
						'</html>'
					);
		...
		setLoader: function() {
			this.setContent(
				'<div class="loading-placeholder">' +
					...
					'<div class="mnview-loading"><ins></ins></div>' +
				'</div>'
			);
		},
		 ...
		 * @param {String} dashicon A dashicon ID. Optional. {@link https://developer.mtaandao.co.ke/resource/dashicons/}
		 */
		...
		update: function( text, editor, node, force ) {
			_.find( views, function( view, type ) {
				...
				if ( match ) {
					...
					editor.dom.setAttrib( node, 'data-mnview-text', encodeURIComponent( text ) );
					mn.mce.views.createInstance( type, text, match.options, force ).render();
					editor.focus();
...
} )( window, window.mn, window.mn.shortcode, window.jQuery );

/*
 * The Mtaandao core TinyMCE views.
...
( function( window, views, media, $ ) {
	...
	av = _.extend( {}, base, {
		...
		initialize: function() {
			...
			this.getEditors( function( editor ) {
				editor.on( 'mnview-selected', function() {
					self.pausePlayers();
				} );
			} );
		},
		...
		pausePlayers: function() {
			this.getNodes( function( editor, node, content ) {
				var win = $( 'iframe.mnview-sandbox', content ).get( 0 );
...
} )( window, window.mn.mce.views, window.mn.media, window.jQuery );
