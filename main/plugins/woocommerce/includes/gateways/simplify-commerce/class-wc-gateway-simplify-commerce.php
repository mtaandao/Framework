...
class WC_Gateway_Simplify_Commerce extends WC_Payment_Gateway_CC {
	...
	public function __construct() {
		...
		// Hooks
		add_action( 'mn_enqueue_scripts', array( $this, 'payment_scripts' ) );
	...
	public function payment_fields() {
		...
		if ( $description ) {
			echo mnautop( mntexturize( trim( $description ) ) );
		}
	...
	public function payment_scripts() {
		...
		mn_enqueue_script( 'simplify-commerce', 'https://www.simplify.com/commerce/v1/simplify.js', array( 'jquery' ), WC_VERSION, true );
		mn_enqueue_script( 'wc-simplify-commerce', WC()->plugin_url() . '/includes/gateways/simplify-commerce/assets/js/simplify-commerce.js', array( 'simplify-commerce', 'wc-credit-card-form' ), WC_VERSION, true );
		mn_localize_script( 'wc-simplify-commerce', 'Simplify_commerce_params', array(
			'key'           => $this->public_key,
	...
	protected function process_standard_payments( $order, $cart_token = '', $customer_token = '' ) {
		try {
			...
			if ( is_mn_error( $payment_response ) ) {
	...
	public function do_payment( $order, $amount = 0, $token = array() ) {
		if ( $amount * 100 < 50 ) {
			return new MN_Error( 'simplify_error', __( 'Sorry, the minimum allowed order total is 0.50 to use this payment method.', 'woocommerce' ) );
		}
		...
		try {
			...
			return new MN_Error( 'simplify_payment_declined', $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
		...
		if ( 'APPROVED' == $payment->paymentStatus ) {
			...
			return new MN_Error( 'simplify_payment_declined', __( 'Payment was declined - please try another card.', 'woocommerce' ) );
		}
	}
	...
	public function return_handler() {
		...
		if ( isset( $_REQUEST['reference'] ) && isset( $_REQUEST['paymentId'] ) && isset( $_REQUEST['signature'] ) ) {
			...
			if ( $signature === $_REQUEST['signature'] ) {
				...
				mn_redirect( $this->get_return_url( $order ) );
				exit();
			}
		}
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		try {
			...
			if ( $e instanceof Simplify_BadRequestException && $e->hasFieldErrors() && $e->getFieldErrors() ) {
				foreach ( $e->getFieldErrors() as $error ) {
					return new MN_Error( 'simplify_refund_error', $error->getFieldName() . ': "' . $error->getMessage() . '" (' . $error->getErrorCode() . ')' );
				}
			} else {
				return new MN_Error( 'simplify_refund_error', $e->getMessage() );
			}
		}
