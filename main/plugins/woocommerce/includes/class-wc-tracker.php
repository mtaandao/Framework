...
class WC_Tracker {
	...
	public static function send_tracking_data( $override = false ) {
		...
		mn_safe_remote_post( self::$api_url, array(
				'method'      => 'POST',
				'timeout'     => 45,
				'redirection' => 5,
				'httpversion' => '1.0',
	...
	private static function get_tracking_data() {
		...
		// Mtaandao Info
		$data['mn']                 = self::get_wordpress_info();
		...
		$data['connect_active']     = ( class_exists( 'WC_Connect_Loader' ) && mn_next_scheduled( 'wc_connect_fetch_service_schemas' ) ) ? 'yes' : 'no';
	...
	private static function get_wordpress_info() {
		...
		$mn_data['memory_limit'] = size_format( $memory );
		$mn_data['debug_mode']   = ( defined('MN_DEBUG') && MN_DEBUG ) ? 'Yes' : 'No';
		$mn_data['locale']       = get_locale();
		$mn_data['version']      = get_bloginfo( 'version' );
		$mn_data['multisite']    = is_multisite() ? 'Yes' : 'No';
	...
	private static function get_server_info() {
		...
		global $mndb;
		$server_data['mysql_version'] = $mndb->db_version();

		$server_data['php_max_upload_size'] = size_format( mn_max_upload_size() );
	...
	private static function get_product_counts() {
		$product_count          = array();
		$product_count_data     = mn_count_posts( 'product' );
	...
	private static function get_order_counts() {
		$order_count      = array();
		$order_count_data = mn_count_posts( 'shop_order' );
