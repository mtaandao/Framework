...
<body class="mn-core-ui">
	<p id="logo"><a href="https://mtaandao.co.ke/">Mtaandao</a></p>
	<h1>Error: PHP is not running</h1>
	<p>Mtaandao requires that your web server is running PHP. Your server does not have PHP installed, or PHP is turned off.</p>
</body>
</html>
<?php
}
...
function display_setup_form( $error = null ) {
	...
	$sql = $mndb->prepare( "SHOW TABLES LIKE %s", $mndb->esc_like( $mndb->users ) );
	$user_table = ( $mndb->get_var( $sql ) != null );
	...
	$admin_email  = isset( $_POST['admin_email']  ) ? trim( mn_unslash( $_POST['admin_email'] ) ) : '';
...
<form id="setup" method="post" action="install.php?step=2" novalidate="novalidate">
	...
	<p class="step"><?php submit_button( __( 'Install Mtaandao' ), 'large', 'Submit', false, array( 'id' => 'submit' ) ); ?></p>
	<input type="hidden" name="language" value="<?php echo isset( $_REQUEST['language'] ) ? esc_attr( $_REQUEST['language'] ) : ''; ?>" />
</form>
<?php
} // end display_setup_form()

// Let's check to make sure MN isn't already installed.
if ( is_blog_installed() ) {
	display_header();
	die(
		'<h1>' . __( 'Already Installed' ) . '</h1>' .
		'<p>' . __( 'You appear to have already installed Mtaandao. To reinstall please clear your old database tables first.' ) . '</p>' .
		'<p class="step"><a href="' . esc_url( mn_login_url() ) . '" class="button button-large">' . __( 'Log In' ) . '</a></p>' .
		'</body></html>'
	);
}
...
if ( ! is_string( $mndb->base_prefix ) || '' === $mndb->base_prefix ) {
	display_header();
	die(
...
if ( defined( 'DO_NOT_UPGRADE_GLOBAL_TABLES' ) ) {
	...
	die(
		...
		'<p>' . sprintf(
			...
			__( 'The constant %s cannot be defined when installing Mtaandao.' ),
...
?>
<h1><?php _ex( 'Welcome', 'Howdy' ); ?></h1>
<p><?php _e( 'Welcome to the famous five-minute Mtaandao installation process! Just fill in the information below and you&#8217;ll be on your way to using the most extendable and powerful personal publishing platform in the world.' ); ?></p>
...
<?php
	...
	case 2:
		...
		if ( ! empty( $mndb->error ) )
			mn_die( $mndb->error->get_error_message() );
		...
		$weblog_title = isset( $_POST['weblog_title'] ) ? trim( mn_unslash( $_POST['weblog_title'] ) ) : '';
		$user_name = isset($_POST['user_name']) ? trim( mn_unslash( $_POST['user_name'] ) ) : '';
		$admin_password = isset($_POST['admin_password']) ? mn_unslash( $_POST['admin_password'] ) : '';
		$admin_password_check = isset($_POST['admin_password2']) ? mn_unslash( $_POST['admin_password2'] ) : '';
		$admin_email  = isset( $_POST['admin_email'] ) ?trim( mn_unslash( $_POST['admin_email'] ) ) : '';
...
<p><?php _e( 'Mtaandao has been installed. Thank you, and enjoy!' ); ?></p>
...
<p class="step"><a href="<?php echo esc_url( mn_login_url() ); ?>" class="button button-large"><?php _e( 'Log In' ); ?></a></p>
...
mn_print_scripts( $scripts_to_print );
?>
