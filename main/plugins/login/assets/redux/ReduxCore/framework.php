<?php
    ...
    if ( ! class_exists( 'ReduxFramework' ) ) {
        ...
        class ReduxFramework {
            ...
            public static $_version = '3.3.8.4';
            public static $_dir;
            public static $_url;
            public static $_upload_dir;
            public static $_upload_url;
            public static $mn_content_url;
            public static $base_mn_content_url;
            ...
            public static function init() {
                ...
                self::$mn_content_url = trailingslashit( Redux_Helpers::cleanFilePath( ( is_ssl() ? str_replace( 'http://', 'https://', MAIN_URL ) : MAIN_URL ) ) );

                // See if Redux is a plugin or not
                if ( strpos( Redux_Helpers::cleanFilePath( __FILE__ ), Redux_Helpers::cleanFilePath( get_stylesheet_directory() ) ) !== false || strpos( Redux_Helpers::cleanFilePath( __FILE__ ), Redux_Helpers::cleanFilePath( get_template_directory_uri() ) ) !== false || strpos( Redux_Helpers::cleanFilePath( __FILE__ ), Redux_Helpers::cleanFilePath( MAIN_DIR . '/themes/' ) ) !== false ) {
                    ...
                    if ( strpos( self::$_dir, ABSPATH ) === false ) {
                        ...
                        foreach ( get_plugins() as $key => $value ) {
                            if ( strpos( $key, 'redux-framework.php' ) !== false ) {
                                self::$_dir = trailingslashit( Redux_Helpers::cleanFilePath( MAIN_DIR . '/plugins/' . plugin_dir_path( $key ) . 'ReduxCore/' ) );
                                $is_plugin  = true;
                            }
                        }
                        if ( ! $is_plugin ) {
                ...
                if ( self::$_is_plugin == true || self::$_as_plugin == true ) {
                    ...
                    if ( strpos( Redux_Helpers::cleanFilePath( __FILE__ ), Redux_Helpers::cleanFilePath( get_template_directory() ) ) !== false ) {
                        ...
                        $mn_content_dir = trailingslashit( Redux_Helpers::cleanFilePath( MAIN_DIR ) );
                        $mn_content_dir = trailingslashit( str_replace( '//', '/', $mn_content_dir ) );
                        $relative_url   = str_replace( $mn_content_dir, '', self::$_dir );
                        self::$_url     = trailingslashit( self::$mn_content_url . $relative_url );
                    }
                }
            }
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                // Disregard mn AJAX 'heartbeat'call.  Why waste resources?
                ...
                $this->args = mn_parse_args( $args, $this->args );
...
//                                if (empty($saveVer)) {
//                                    //logconsole('redir');
//                                    mn_safe_redirect ( admin_url ( 'index.php?page=redux-getting-started' ) );
//                                    exit;
//                                } else if (version_compare($curVer, $saveVer, '>')) {
//                                    mn_safe_redirect ( admin_url ( 'index.php?page=redux-about' ) );
//                                    exit;
//                                }
//                            }
//                        }
//                    }
                    ...
                    if( empty( $this->args['output_location'] ) || in_array( 'frontend', $this->args['output_location'] ) ) {
                        add_action( 'mn_head', array( &$this, '_output_css' ), 150 );
                        add_action( 'mn_enqueue_scripts', array( &$this, '_enqueue_output' ), 150 );
                    }
                    ...
                    add_action( 'mn_print_scripts', array( $this, 'vc_fixes' ), 100 );
            ...
            public function vc_fixes() {
                if ( redux_helpers::isFieldInUse( $this, 'ace_editor' ) ) {
                    mn_dequeue_script( 'wpb_ace' );
                    mn_deregister_script( 'wpb_ace' );
                }
            }

            public function network_admin_bar( $mn_admin_bar ) {
            ...
            public function save_network_page() {
                ...
                mn_redirect( add_query_arg( array(
            ...
            public function get_wordpress_data( $type = false, $args = array() ) {
                ...
                if ( empty( $data ) && isset( $this->mn_data[ $type . $argsKey ] ) ) {
                    $data = $this->mn_data[ $type . $argsKey ];
                }
                ...
                if ( empty( $data ) && ! empty( $type ) ) {
                    ...
                    if ( ! empty( $type ) && empty( $data ) ) {
                        ...
                        if ( $type == "categories" || $type == "category" ) {
                            ...
                            foreach ( $post_types as $name => $title ) {
                                if ( isset( $mn_post_types[ $name ]->labels->menu_name ) ) {
                                    $data[ $name ] = $mn_post_types[ $name ]->labels->menu_name;
                                } else {
                            ...
                            global $_mn_registered_nav_menus;

                            foreach ( $_mn_registered_nav_menus as $k => $v ) {
                                $data[ $k ] = $v;
                            }
                        } else if ( $type == "image_size" || $type == "image_sizes" ) {
                            global $_mn_additional_image_sizes;

                            foreach ( $_mn_additional_image_sizes as $size_name => $size_attrs ) {
                    ...
                    $this->mn_data[ $type . $argsKey ] = $data;
                }
             ...
             * @since       3.1.5.16
             * @access      public
             * @global      $menu , $submenu, $mn_admin_bar
             * @return      void
             */
                ...
                if ( $menu ) {
                    foreach ( $menu as $menu_item ) {
                        if ( isset( $menu_item[2] ) && $menu_item[2] === $this->args["page_slug"] ) {
                            ...
                            $mn_admin_bar->add_node( $nodeargs );
                    ...
                    if ( isset( $submenu[ $this->args["page_slug"] ] ) && is_array( $submenu[ $this->args["page_slug"] ] ) ) {
                        foreach ( $submenu[ $this->args["page_slug"] ] as $index => $redux_options_submenu ) {
                            ...
                            $mn_admin_bar->add_node( $subnodeargs );
                        }
                    }
                    ...
                    if ( isset( $this->args['admin_bar_links'] ) ) {
                        ...
                        foreach ( $this->args['admin_bar_links'] as $link ) {
                            ...
                            $mn_admin_bar->add_node( $externalnodeargs );
                        }
                    }
                } else {
                    // Fetch the title
            ...
            public function _enqueue_output() {
                ...
                if ( ! empty( $this->typography ) && ! empty( $this->typography ) && filter_var( $this->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
                    ...
                    <?php
                        ...
                        mn_register_style( 'redux-google-fonts', $protocol . $typography->makeGoogleWebfontLink( $this->typography ), '', $version );
                        mn_enqueue_style( 'redux-google-fonts' );
                    }
                }
            ...
            public function _enqueue() {
                ...
                $mn_styles->add_data( 'redux-elusive-icon-ie7', 'conditional', 'lte IE 7' );
                ...
                mn_enqueue_style( 'jquery-ui-css' );
                mn_enqueue_style( 'redux-lte-ie8' );
                mn_enqueue_style( 'qtip-css' );
                mn_enqueue_style( 'redux-elusive-icon' );
                mn_enqueue_style( 'redux-elusive-icon-ie7' );
                ...
                if ( is_rtl() ) {
                    ...
                    mn_enqueue_style( 'redux-rtl-css' );
                }

                mn_enqueue_script( 'jquery' );
                mn_enqueue_script( 'jquery-ui-core' );
                ...
                if ( Redux_Helpers::isFieldInUseByType( $this->fields, array(
                    ...
                    mn_enqueue_style( 'jquery-ui-sortable' );
                }
                ...
                if ( Redux_Helpers::isFieldInUseByType( $this->fields, array( 'slides', 'group' ) ) ) {
                    mn_enqueue_script( 'jquery-ui-accordion' );
                }

                // Load mn-color-picker for color, color_gradient, link_color, border, background and typography
                if ( Redux_Helpers::isFieldInUseByType( $this->fields, array(
                    'background',
                    'color',
                    'color_gradient',
                            ...
                            if ( isset( $field['type'] ) && $field['type'] != 'callback' ) {
                                ...
                                if ( $class_file ) {
                                    ...
                                    if ( ( method_exists( $field_class, 'enqueue' ) ) || method_exists( $field_class, 'localize' ) ) {
                                        ...
                                        if ( ! mn_script_is( 'redux-field-' . $field['type'] . '-js', 'enqueued' ) && class_exists( $field_class ) && method_exists( $field_class, 'enqueue' ) ) {
                ...
                mn_enqueue_script( 'redux-js' ); // Enque the JS now
            ...
            public function _register_settings() {
                ...
                if ( ! function_exists( 'mn_get_current_user' ) ) {
                    include( ABSPATH . "res/pluggable.php" );
                }
...
//                            }
                            ...
                            if ( $field['type'] == "media" ) {
                                if ( isset( $this->options[ $field['id'] ]['id'] ) && isset( $this->options[ $field['id'] ]['url'] ) && ! empty( $this->options[ $field['id'] ]['url'] ) && strpos( $this->options[ $field['id'] ]['url'], str_replace( 'http://', '', MAIN_URL ) ) === false ) {
                                    $data = mn_get_attachment_url( $this->options[ $field['id'] ]['id'] );

                                    if ( isset( $data ) && ! empty( $data ) ) {
                                        $this->options[ $field['id'] ]['url']       = $data;
                                        $data                                       = mn_get_attachment_image_src( $this->options[ $field['id'] ]['id'], array(
                                            150,
                                            150
                                        ) );
                            ...
                            if ( $field['type'] == "background" ) {
                                if ( isset( $this->options[ $field['id'] ]['media']['id'] ) && isset( $this->options[ $field['id'] ]['background-image'] ) && ! empty( $this->options[ $field['id'] ]['background-image'] ) && strpos( $this->options[ $field['id'] ]['background-image'], str_replace( 'http://', '', MAIN_URL ) ) === false ) {
                                    $data = mn_get_attachment_url( $this->options[ $field['id'] ]['media']['id'] );

                                    if ( isset( $data ) && ! empty( $data ) ) {
                                        $this->options[ $field['id'] ]['background-image']   = $data;
                                        $data                                                = mn_get_attachment_image_src( $this->options[ $field['id'] ]['media']['id'], array(
                                            150,
                                            150
                                        ) );
                            ...
                            if ( $field['type'] == "slides" ) {
                                if ( isset( $this->options[ $field['id'] ][0]['attachment_id'] ) && isset( $this->options[ $field['id'] ][0]['image'] ) && ! empty( $this->options[ $field['id'] ][0]['image'] ) && strpos( $this->options[ $field['id'] ][0]['image'], str_replace( 'http://', '', MAIN_URL ) ) === false ) {
                                    foreach ( $this->options[ $field['id'] ] as $k => $v ) {
                                        $data = mn_get_attachment_url( $v['attachment_id'] );

                                        if ( isset( $data ) && ! empty( $data ) ) {
                                            $this->options[ $field['id'] ][ $k ]['image'] = $data;
                                            $data                                         = mn_get_attachment_image_src( $v['attachment_id'], array(
                                                150,
                                                150
                                            ) );
            ...
            public function _validate_options( $plugin_options ) {
                ...
                if ( ! empty( $plugin_options['import'] ) ) {
                    ...
                    if ( $plugin_options['import_code'] != '' ) {
                        ...
                        $import = mn_remote_retrieve_body( mn_remote_get( $plugin_options['import_link'] ) );
                    }
                    ...
                    if ( ! empty( $imported_options ) && is_array( $imported_options ) && isset( $imported_options['redux-backup'] ) && $imported_options['redux-backup'] == '1' ) {
                        ...
                        $plugin_options = mn_parse_args( $imported_options, $plugin_options );
                ...
                if ( defined( 'mn_CACHE' ) && mn_CACHE && class_exists( 'W3_ObjectCache' ) ) {
                    //echo "here";
            ...
            public function _options_page_html() {
                ...
                // Security is vital!
                echo '<input type="hidden" id="ajaxsecurity" name="security" value="' . mn_create_nonce( 'redux_ajax_nonce' ) . '" />';
