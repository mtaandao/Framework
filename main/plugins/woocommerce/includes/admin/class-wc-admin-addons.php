...
class WC_Admin_Addons {
	...
	public static function get_sections() {
		if ( false === ( $sections = get_transient( 'wc_addons_sections' ) ) ) {
			$raw_sections = mn_safe_remote_get( 'https://d3t0oesq8995hv.cloudfront.net/addon-sections.json', array( 'user-agent' => 'WooCommerce Addons Page' ) );
			if ( ! is_mn_error( $raw_sections ) ) {
				$sections = json_decode( mn_remote_retrieve_body( $raw_sections ) );
	...
	public static function get_section_data( $section_id ) {
		...
		if ( ! empty( $section->endpoint ) ) {
			if ( false === ( $section_data = get_transient( 'wc_addons_section_' . $section_id ) ) ) {
				$raw_section = mn_safe_remote_get( esc_url_raw( $section->endpoint ), array( 'user-agent' => 'WooCommerce Addons Page' ) );

				if ( ! is_mn_error( $raw_section ) ) {
					$section_data = json_decode( mn_remote_retrieve_body( $raw_section ) );
