/* global mn, woocommerce_admin_meta_boxes_variations, woocommerce_admin, accounting */
jQuery( function( $ ) {
	...
	var wc_meta_boxes_product_variations_media = {
		 ...
		 * @type {Int}
		 */
		mn_media_post_id: mn.media.model.settings.post.id,
		...
		init: function() {
			...
			$( 'a.add_media' ).on( 'click', this.restore_mn_media_post_id );
		},
		...
		add_image: function( event ) {
			...
			if ( $button.is( '.remove' ) ) {
				...
				if ( wc_meta_boxes_product_variations_media.variable_image_frame ) {
					...
					mn.media.model.settings.post.id = wc_meta_boxes_product_variations_media.setting_variation_image_id;
				}

				// Create the media frame.
				wc_meta_boxes_product_variations_media.variable_image_frame = mn.media.frames.variable_image = mn.media({
				...
				wc_meta_boxes_product_variations_media.variable_image_frame.on( 'select', function () {
					...
					mn.media.model.settings.post.id = wc_meta_boxes_product_variations_media.mn_media_post_id;
				});
		...
		/**
		 * Restore mn.media post ID.
		 */
		restore_mn_media_post_id: function() {
			mn.media.model.settings.post.id = wc_meta_boxes_product_variations_media.mn_media_post_id;
		}
	};
