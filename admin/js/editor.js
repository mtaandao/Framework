...
( function( $ ) {
	function SwitchEditors() {
		...
		function init() {
			if ( ! tinymce && window.tinymce ) {
				...
				$$( document ).on( 'click', function( event ) {
					...
					if ( target.hasClass( 'mn-switch-editor' ) ) {
						id = target.attr( 'data-mn-editor-id' );
		...
		function removep( html ) {
			...
			if ( html.indexOf( '[caption' ) !== -1 ) {
				...
				html = html.replace( /\[caption[\s\S]+?\[\/caption\]/g, function( a ) {
					return a.replace( /<br([^>]*)>/g, '<mn-temp-br$1>' ).replace( /[\r\n\t]+/, '' );
				});
			}
			...
			if ( preserve_linebreaks ) {
				html = html.replace( /<mn-line-break>/g, '\n' );
			}

			// and the <br> tags in captions
			if ( preserve_br ) {
				html = html.replace( /<mn-temp-br([^>]*)>/g, '<br$1>' );
			}

			// Put back preserved tags.
			if ( preserve.length ) {
				html = html.replace( /<mn-preserve>/g, function() {
		...
		// Similar to `mnautop()` in formatting.php
		function autop( text ) {
			...
			// put back the line breaks in pre|script
			if ( preserve_linebreaks ) {
				text = text.replace( /<mn-line-break>/g, '\n' );
			}

			if ( preserve_br ) {
				text = text.replace( /<mn-temp-br([^>]*)>/g, '<br$1>' );
			}
		...
		window.mn.editor.autop = mnautop;
		window.mn.editor.removep = pre_mnautop;

		exports = {
			go: switchEditor,
			mnautop: mnautop,
			pre_mnautop: pre_mnautop,
			_mn_Autop: autop,
			_mn_Nop: removep
		};
