...
class custom_avatar {
	...
	public function load_textdomain() {
		...
		load_textdomain( $domain, MN_LANG_DIR . '/' . $domain . '/' . $domain . '-' . $locale . '.mo' );
	...
	public function admin_init() {
		...
		register_setting( 'discussion', 'custom_avatar_caps', array( $this, 'sanitize_options' ) );
	...
	public function avatar_settings_field( $args ) {
		$options = get_option( 'custom_avatar_caps' );
		?>
		<label for="custom_avatar_caps">
			<input type="checkbox" name="custom_avatar_caps" id="custom_avatar_caps" value="1" <?php checked( $options['custom_avatar_caps'], 1 ); ?>/>
	...
	public function sanitize_options( $input ) {
		$new_input['custom_avatar_caps'] = empty( $input['custom_avatar_caps'] ) ? 0 : 1;
		return $new_input;
	}

	/**
	 * Filter the avatar Mtaandao returns
	 *
	 * @since 1.0.0
	 * @param string $avatar 
	...
	public function get_avatar( $avatar = '', $id_or_email, $size = 96, $default = '', $alt = false ) {
		...
		if ( empty( $local_avatars[$size] ) ) {
			...
			$upload_path      = mn_upload_dir();
			$avatar_full_path = str_replace( $upload_path['baseurl'], $upload_path['basedir'], $local_avatars['full'] );
			$image            = mn_get_image_editor( $avatar_full_path );

			if ( ! is_mn_error( $image ) ) {
				$image->resize( $size, $size, true );
				$image_sized = $image->save();
			}

			// Deal with original being >= to original image (or lack of sizing ability)
			$local_avatars[$size] = is_mn_error( $image_sized ) ? $local_avatars[$size] = $local_avatars['full'] : str_replace( $upload_path['basedir'], $upload_path['baseurl'], $image_sized['path'] );
	...
	public function edit_user_profile( $profileuser ) {
		...
		<h3><?php _e( 'Custom Avatar', 'basic-user-avatars' ); ?></h3>
		<table class="form-table">
			<tr>
				...
				<?php
				$options = get_option( 'custom_avatar_caps' );
				if ( empty( $options['custom_avatar_caps'] ) || current_user_can( 'upload_files' ) ) {
					// Nonce security ftw
					mn_nonce_field( 'basic_user_avatar_nonce', '_basic_user_avatar_nonce', false );
	...
	public function edit_user_profile_update( $user_id ) {
		...
		if ( ! isset( $_POST['_basic_user_avatar_nonce'] ) || ! mn_verify_nonce( $_POST['_basic_user_avatar_nonce'], 'basic_user_avatar_nonce' ) )
			return;
		...
		if ( ! empty( $_FILES['basic-user-avatar']['name'] ) ) {
			...
			if ( ! function_exists( 'mn_handle_upload' ) )
			...
			if ( strstr( $_FILES['basic-user-avatar']['name'], '.php' ) )
				mn_die( 'For security reasons, the extension ".php" cannot be in your file name.' );

			// Make user_id known to unique_filename_callback function
			$this->user_id_being_edited = $user_id; 
			$avatar = mn_handle_upload( $_FILES['basic-user-avatar'], array( 'mimes' => $mimes, 'test_form' => false, 'unique_filename_callback' => array( $this, 'unique_filename_callback' ) ) );
...
$custom_avatar = new custom_avatar;