...
class WC_Admin_Dashboard {
	...
	public function __construct() {
		...
		if ( current_user_can( 'view_woocommerce_reports' ) || current_user_can( 'manage_woocommerce' ) || current_user_can( 'publish_shop_orders' ) ) {
			add_action( 'mn_dashboard_setup', array( $this, 'init' ) );
		}
	}
	...
	public function init() {
		if ( current_user_can( 'publish_shop_orders' ) ) {
			mn_add_dashboard_widget( 'woocommerce_dashboard_recent_reviews', __( 'WooCommerce Recent Reviews', 'woocommerce' ), array( $this, 'recent_reviews' ) );
		}

		mn_add_dashboard_widget( 'woocommerce_dashboard_status', __( 'WooCommerce Status', 'woocommerce' ), array( $this, 'status_widget' ) );
	}
	...
	public function status_widget() {
		...
		$query['join']    = "INNER JOIN {$mndb->prefix}woocommerce_order_items AS order_items ON posts.ID = order_id ";
		$query['join']   .= "INNER JOIN {$mndb->prefix}woocommerce_order_itemmeta AS order_item_meta ON order_items.order_item_id = order_item_meta.order_item_id ";
		$query['join']   .= "INNER JOIN {$mndb->prefix}woocommerce_order_itemmeta AS order_item_meta_2 ON order_items.order_item_id = order_item_meta_2.order_item_id ";
		...
		$top_seller = $mndb->get_row( implode( ' ', apply_filters( 'woocommerce_dashboard_status_widget_top_seller_query', $query ) ) );
		...
		foreach ( wc_get_order_types( 'order-count' ) as $type ) {
			$counts           = (array) mn_count_posts( $type );
		...
		if ( false === ( $lowinstock_count = get_transient( $transient_name ) ) ) {
			$query_from = apply_filters( 'woocommerce_report_low_in_stock_query_from', "FROM {$mndb->posts} as posts
				INNER JOIN {$mndb->postmeta} AS postmeta ON posts.ID = postmeta.post_id
				INNER JOIN {$mndb->postmeta} AS postmeta2 ON posts.ID = postmeta2.post_id
				WHERE 1=1
			...
			" );
			$lowinstock_count = absint( $mndb->get_var( "SELECT COUNT( DISTINCT posts.ID ) {$query_from};" ) );
		...
		if ( false === ( $outofstock_count = get_transient( $transient_name ) ) ) {
			$query_from = apply_filters( 'woocommerce_report_out_of_stock_query_from', "FROM {$mndb->posts} as posts
				INNER JOIN {$mndb->postmeta} AS postmeta ON posts.ID = postmeta.post_id
				INNER JOIN {$mndb->postmeta} AS postmeta2 ON posts.ID = postmeta2.post_id
				WHERE 1=1
				AND posts.post_type IN ( 'product', 'product_variation' )
				...
				AND postmeta.meta_key = '_stock' AND CAST(postmeta.meta_value AS SIGNED) <= '{$nostock}'
			" );
			$outofstock_count = absint( $mndb->get_var( "SELECT COUNT( DISTINCT posts.ID ) {$query_from};" ) );
	...
	public function recent_reviews() {
		global $mndb;
		$comments = $mndb->get_results( "SELECT *, SUBSTRING(comment_content,1,100) AS comment_excerpt
		FROM $mndb->comments
		LEFT JOIN $mndb->posts ON ($mndb->comments.comment_post_ID = $mndb->posts.ID)
		...
		if ( $comments ) {
			...
			foreach ( $comments as $comment ) {
				...
				echo '<blockquote>' . mn_kses_data( $comment->comment_excerpt ) . ' [...]</blockquote></li>';
