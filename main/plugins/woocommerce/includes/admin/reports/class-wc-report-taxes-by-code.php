			...
			<?php if ( ! empty( $tax_rows ) ) : ?>
				<tbody>
					...
					foreach ( $tax_rows as $rate_id => $tax_row ) {
						$rate = $mndb->get_var( $mndb->prepare( "SELECT tax_rate FROM {$mndb->prefix}woocommerce_tax_rates WHERE tax_rate_id = %d;", $rate_id ) );
						?>
						<tr>
				...
				<tfoot>
					<tr>
						...
						<th class="total_row"><?php echo wc_price( wc_round_tax_total( array_sum( mn_list_pluck( (array) $tax_rows, 'tax_amount' ) ) ) ); ?></th>
						<th class="total_row"><?php echo wc_price( wc_round_tax_total( array_sum( mn_list_pluck( (array) $tax_rows, 'shipping_tax_amount' ) ) ) ); ?></th>
						<th class="total_row"><strong><?php echo wc_price( wc_round_tax_total( array_sum( mn_list_pluck( (array) $tax_rows, 'tax_amount' ) ) + array_sum( mn_list_pluck( (array) $tax_rows, 'shipping_tax_amount' ) ) ) ); ?></strong></th>
					</tr>
				</tfoot>
			<?php else : ?>
				<tbody>
