...
final class Menu_Icons_Picker {
	...
	public static function init() {
		...
		add_filter( 'mn_edit_nav_menu_walker', array( __CLASS__, '_filter_mn_edit_nav_menu_walker' ), 99 );
		add_filter( 'mn_nav_menu_item_custom_fields', array( __CLASS__, '_fields' ), 10, 4 );
		add_filter( 'manage_nav-menus_columns', array( __CLASS__, '_columns' ), 99 );
		add_action( 'mn_update_nav_menu_item', array( __CLASS__, '_save' ), 10, 3 );
	 ...
	 *
	 * @since   0.9.0
	 * @mn_hook action load-nav-menus.php
	 */
	 ...
	 * @since   0.3.0
	 * @access  protected
	 * @mn_hook filter    mn_edit_nav_menu_walker
	 */
	public static function _filter_mn_edit_nav_menu_walker( $walker ) {
	 ...
	 * @mn_hook action       menu_item_custom_fields
	 *
	 ...
	 * @since   0.1.0
	 * @access  private
	 * @mn_hook action  manage_nav-menus_columns
	 * @link    http://codex.mtaandao.co.ke/Plugin_API/Filter_Reference/manage_posts_columns
	 *
	 ...
	 * @since   0.1.0
	 * @access  protected
	 * @mn_hook action    mn_update_nav_menu_item
	 * @link    http://codex.mtaandao.co.ke/Plugin_API/Action_Reference/mn_update_nav_menu_item
	 *
	...
	public static function _save( $menu_id, $menu_item_db_id, $menu_item_args ) {
		...
		if ( ! $screen instanceof MN_Screen || 'nav-menus' !== $screen->id ) {
			return;
		}
		...
		if ( ! empty( $_POST['menu-icons'][ $menu_item_db_id ] ) ) {
			$value = array_map(
				'sanitize_text_field',
				mn_unslash( (array) $_POST['menu-icons'][ $menu_item_db_id ] )
			);
		} else {
			$value = array();
		}
	 ...
	 * @mn_hook action print_media_templates
	 */
	 ...
	 *
	 * @since   0.9.0
	 * @mn_hook action icon_picker_type_props
	 *
