...
if ( ! current_user_can( 'manage_sites' ) )
	mn_die( __( 'Sorry, you are not allowed to add sites to this network.' ) );
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="https://mtaandao.co.ke/docs/Network_Admin_Sites_Screen" target="_blank">Documentation on Site Management</a>') . '</p>' .
	'<p>' . __('<a href="https://mtaandao.co.ke/support/forum/multisite/" target="_blank">Support Forums</a>') . '</p>'
);

if ( isset($_REQUEST['action']) && 'add-site' == $_REQUEST['action'] ) {
	check_admin_referer( 'add-blog', '_mnnonce_add-blog' );

	if ( ! is_array( $_POST['blog'] ) )
		mn_die( __( 'Can&#8217;t create an empty site.' ) );
	...
	if ( ! is_subdomain_install() ) {
		...
		if ( in_array( $domain, $subdirectory_reserved_names ) ) {
			mn_die(
				...
				sprintf( __( 'The following words are reserved for use by Mtaandao functions and cannot be used as blog names: %s' ),
	...
	if ( ! empty( $_POST['MNLANG'] ) && mn_can_install_language_pack() ) {
		$language = mn_download_language_pack( mn_unslash( $_POST['MNLANG'] ) );
		if ( $language ) {
			$meta['MNLANG'] = $language;
		}
	}

	if ( empty( $domain ) )
		mn_die( __( 'Missing or invalid site address.' ) );

	if ( isset( $blog['email'] ) && '' === trim( $blog['email'] ) ) {
		mn_die( __( 'Missing email address.' ) );
	}

	$email = sanitize_email( $blog['email'] );
	if ( ! is_email( $email ) ) {
		mn_die( __( 'Invalid email address.' ) );
	}
	...
	if ( !$user_id ) { // Create a new user with a random password
		...
		if ( $user_id ) {
			mn_die( __( 'The domain or path entered conflicts with an existing username.' ) );
		}
		$password = mn_generate_password( 12, false );
		$user_id = mnmu_create_user( $domain, $password, $email );
		if ( false === $user_id ) {
			mn_die( __( 'There was an error creating the user.' ) );
		}
	...
	$mndb->hide_errors();
	$id = mnmu_create_blog( $newdomain, $path, $title, $user_id, $meta, $current_site->id );
	$mndb->show_errors();
	if ( ! is_mn_error( $id ) ) {
...
Name: %3$s' ),
		...
		mnmu_welcome_notification( $id, $user_id, $password, $title, array( 'public' => 1 ) );
		mn_redirect( add_query_arg( array( 'update' => 'added', 'id' => $id ), 'site-new.php' ) );
		exit;
	} else {
		mn_die( $id->get_error_message() );
	}
}
...
<?php mn_nonce_field( 'add-blog', '_mnnonce_add-blog' ) ?>
	<table class="form-table">
			...
			<tr class="form-field form-required">
				...
				<td>
					...
					mn_dropdown_languages( array(
						...
						'show_available_translations' => mn_can_install_language_pack(),
					) );
					?>
				</td>
			</tr>
		...
		<tr class="form-field form-required">
			...
			<td><input name="blog[email]" type="email" class="regular-text mn-suggest-user" id="admin-email" data-autocomplete-type="search" data-autocomplete-field="user_email" /></td>
		</tr>
		<tr class="form-field">
