...
class WC_Download_Handler {
	...
	private static function get_download_data( $args = array() ) {
		...
		$query = "SELECT * FROM " . $mndb->prefix . "woocommerce_downloadable_product_permissions ";
		...
		return $mndb->get_row( $mndb->prepare( $query, array( $args['email'], $args['order_key'], $args['product_id'], $args['download_id'] ) ) );
	}
		...
		if ( $download_data->user_id && 'yes' === get_option( 'woocommerce_downloads_require_login' ) ) {
			if ( ! is_user_logged_in() ) {
				if ( wc_get_page_id( 'myaccount' ) ) {
					mn_safe_redirect( add_query_arg( 'wc_error', urlencode( __( 'You must be logged in to download files.', 'woocommerce' ) ), wc_get_page_permalink( 'myaccount' ) ) );
					exit;
				} else {
					self::download_error( __( 'You must be logged in to download files.', 'woocommerce' ) . ' <a href="' . esc_url( mn_login_url( wc_get_page_permalink( 'myaccount' ) ) ) . '" class="wc-forward">' . __( 'Login', 'woocommerce' ) . '</a>', __( 'Log in to Download Files', 'woocommerce' ), 403 );
				}
	...
	public static function count_download( $download_data ) {
		...
		$mndb->update(
			$mndb->prefix . 'woocommerce_downloadable_product_permissions',
			array(
	...
	public static function parse_file_path( $file_path ) {
		$mn_uploads     = mn_upload_dir();
		$mn_uploads_dir = $mn_uploads['basedir'];
		$mn_uploads_url = $mn_uploads['baseurl'];

		// Replace uploads dir, site url etc with absolute counterparts if we can
		$replacements = array(
			$mn_uploads_url                  => $mn_uploads_dir,
	...
	private static function download_error( $message, $title = '', $status = 404 ) {
		...
		mn_die( $message, $title, array( 'response' => $status ) );
	}
}
