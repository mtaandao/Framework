...
function check_upload_size( $file ) {
	...
	if ( $file['error'] != '0' && ! isset( $_POST['html-upload'] ) && ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) ) {
		mn_die( $file['error'] . ' <a href="javascript:history.go(-1)">' . __( 'Back' ) . '</a>' );
	}
 ...
 */
function mnmu_delete_blog( $blog_id, $drop = false ) {
	global $mndb;
	...
	// If ms_files_rewriting is enabled and upload_path is empty, mn_upload_dir is not reliable.
	...
	if ( $drop ) {
		 ...
		 */
		$drop_tables = apply_filters( 'mnmu_drop_tables', $tables, $blog_id );

		foreach ( (array) $drop_tables as $table ) {
			$mndb->query( "DROP TABLE IF EXISTS `$table`" );
		}

		$mndb->delete( $mndb->blogs, array( 'blog_id' => $blog_id ) );
		 ...
		 * @since MU
		 *
		 * @param string $uploads['basedir'] Uploads path without subdirectory. @see mn_upload_dir()
		 * @param int    $blog_id            The site ID.
		 */
		$dir = apply_filters( 'mnmu_delete_blog_upload_dir', $uploads['basedir'], $blog_id );
 ...
 * @since 3.0.0
 *
 * @todo Merge with mn_delete_user() ?
 *
 * @global mndb $mndb Mtaandao database abstraction object.
 *
...
function mnmu_delete_user( $id ) {
	...
	if ( ! empty( $blogs ) ) {
		foreach ( $blogs as $blog ) {
			...
			$post_ids = $mndb->get_col( $mndb->prepare( "SELECT ID FROM $mndb->posts WHERE post_author = %d", $id ) );
			foreach ( (array) $post_ids as $post_id ) {
				mn_delete_post( $post_id );
			}

			// Clean links
			$link_ids = $mndb->get_col( $mndb->prepare( "SELECT link_id FROM $mndb->links WHERE link_owner = %d", $id ) );
	...
	$meta = $mndb->get_col( $mndb->prepare( "SELECT umeta_id FROM $mndb->usermeta WHERE user_id = %d", $id ) );
	foreach ( $meta as $mid )
		delete_metadata_by_mid( 'user', $mid );

	$mndb->delete( $mndb->users, array( 'ID' => $id ) );
...
###SITEURL###' );
	...
	mn_mail( $value, sprintf( __( '[%s] New Admin Email Address' ), mn_specialchars_decode( get_option( 'blogname' ) ) ), $content );
}
 ...
 * @since 3.0.0
 *
 * @global MN_Error $errors MN_Error object.
 * @global mndb     $mndb   Mtaandao database object.
 */
function send_confirmation_on_profile_email() {
	...
	if ( $current_user->user_email != $_POST['email'] ) {
		...
		if ( $mndb->get_var( $mndb->prepare( "SELECT user_email FROM {$mndb->users} WHERE user_email=%s", $_POST['email'] ) ) ) {
...
###SITEURL###' );
		...
		mn_mail( $_POST['email'], sprintf( __( '[%s] New Email Address' ), mn_specialchars_decode( get_option( 'blogname' ) ) ), $content );
 ...
 *
 * @param int    $id         The user ID.
 * @param string $pref       The column in the mn_users table to update the user's status
...
function update_user_status( $id, $pref, $value, $deprecated = null ) {
	...
	$mndb->update( $mndb->users, array( sanitize_key( $pref ) => $value ), array( 'ID' => $id ) );
...
function _access_denied_splash() {
	...
	if ( mn_list_filter( $blogs, array( 'userblog_id' => get_current_blog_id() ) ) )
		return;

	$blog_name = get_bloginfo( 'name' );

	if ( empty( $blogs ) )
		mn_die( sprintf( __( 'You attempted to access the "%1$s" dashboard, but you do not currently have privileges on this site. If you believe you should be able to access the "%1$s" dashboard, please contact your network administrator.' ), $blog_name ), 403 );
...
function mu_dropdown_languages( $lang_files = array(), $current = '' ) {
	...
	if ( $flag === false ) // Mtaandao english
 ...
 * @since 3.0.0
 *
 * @global int    $mn_db_version The version number of the database.
 * @global string $pagenow
 *
 * @return false False if the current user is not a super admin.
 */
function site_admin_notice() {
	global $mn_db_version, $pagenow;
	...
	if ( get_site_option( 'mnmu_upgrade_site' ) != $mn_db_version ) {
...
function confirm_delete_users( $users ) {
	...
	<?php
	mn_nonce_field( 'ms-users-delete' );
	...
	<?php foreach ( ( $allusers = (array) $_POST['allusers'] ) as $user_id ) {
		if ( $user_id != '' && $user_id != '0' ) {
			...
			if ( ! current_user_can( 'delete_user', $delete_user->ID ) ) {
				mn_die( sprintf( __( 'Warning! User %s cannot be deleted.' ), $delete_user->user_login ) );
			}

			if ( in_array( $delete_user->user_login, $site_admins ) ) {
				mn_die( sprintf( __( 'Warning! User cannot be deleted. The user %s is a network administrator.' ), '<em>' . $delete_user->user_login . '</em>' ) );
			}
			?>
			<tr>
...
function network_edit_site_nav( $args = array() ) {
	...
	// All done!
	echo '<h2 class="nav-tab-wrapper mn-clearfix">';
