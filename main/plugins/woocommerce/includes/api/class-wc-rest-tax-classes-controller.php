...
class WC_REST_Tax_Classes_Controller extends WC_REST_Controller {
	...
	public function create_item( $request ) {
		...
		if ( $exists ) {
			return new MN_Error( 'woocommerce_rest_tax_class_exists', __( 'Cannot create existing resource.', 'woocommerce' ), array( 'status' => 400 ) );
		}
	...
	public function delete_item( $request ) {
		...
		$mndb->query( $mndb->prepare( "
			DELETE locations.*
			FROM {$mndb->prefix}woocommerce_tax_rate_locations AS locations
			INNER JOIN
				{$mndb->prefix}woocommerce_tax_rates AS rates
				ON rates.tax_rate_id = locations.tax_rate_id
			WHERE rates.tax_rate_class = '%s'
		", $tax_class['slug'] ) );

		// Delete tax rates in the selected class.
		$mndb->delete( $mndb->prefix . 'woocommerce_tax_rates', array( 'tax_rate_class' => $tax_class['slug'] ), array( '%s' ) );
