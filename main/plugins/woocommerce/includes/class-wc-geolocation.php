...
class WC_Geolocation {
	...
	public static function get_external_ip_address() {
		...
		if ( false === $external_ip_address ) {
			...
			foreach ( $ip_lookup_services_keys as $service_name ) {
				...
				$response         = mn_safe_remote_get( $service_endpoint, array( 'timeout' => 2 ) );

				if ( ! is_mn_error( $response ) && $response['body'] ) {
	...
	/**
	 * Update geoip database. Adapted from https://mtaandao.co.ke/plugins/geoip-detect/.
	 */
	public static function update_database() {
		...
		foreach ( $tmp_databases as $tmp_database_version => $tmp_database_path ) {
			if ( ! is_mn_error( $tmp_database_path ) ) {
	...
	private static function geolocate_via_api( $ip_address ) {
		...
		if ( false === $country_code ) {
			...
			foreach ( $geoip_services_keys as $service_name ) {
				...
				$response         = mn_safe_remote_get( sprintf( $service_endpoint, $ip_address ), array( 'timeout' => 2 ) );
