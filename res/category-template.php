 ...
 * @return string|MN_Error A list of category parents on success, MN_Error on failure.
 */
 ...
 * @return array Array of MN_Term objects, one for each category assigned to the post.
 */
function get_the_category( $id = false ) {
	$categories = get_the_terms( $id, 'category' );
	if ( ! $categories || is_mn_error( $categories ) )
		$categories = array();
 ...
 * @return string|MN_Error Category name on success, MN_Error on failure.
 */
...
function get_the_category_list( $separator = '', $parents='', $post_id = false ) {
	...
	$rel = ( is_object( $mn_rewrite ) && $mn_rewrite->using_permalinks() ) ? 'rel="category tag"' : 'rel="category"';
 ...
 *
 * Prior to v2.5 of Mtaandao, category names were not supported.
 * Prior to v2.7, category slugs were not supported.
 * Prior to v2.7, only one category could be compared: in_category( $single_category ).
 * Prior to v2.7, this function could only be used in the Mtaandao Loop.
 ...
 * @since 16.10.0 Introduced the `required` argument.
 *
 ...
 */
function mn_dropdown_categories( $args = '' ) {
	$defaults = array(
		'show_option_all'   => '',
		'show_option_none'  => '',
		'orderby'           => 'id',
	 ...
	 */
	$output = apply_filters( 'mn_dropdown_cats', $output, $r );
 ...
 */
function mn_list_categories( $args = '' ) {
	$defaults = array(
		'child_of'            => 0,
		'current_category'    => 0,
		'depth'               => 0,
	...
	if ( true == $r['hierarchical'] ) {
		...
		if ( $r['exclude_tree'] ) {
			$exclude_tree = array_merge( $exclude_tree, mn_parse_id_list( $r['exclude_tree'] ) );
		}

		if ( $r['exclude'] ) {
			$exclude_tree = array_merge( $exclude_tree, mn_parse_id_list( $r['exclude'] ) );
		}
	 ...
	 */
	$html = apply_filters( 'mn_list_categories', $output, $args );
...
function mn_tag_cloud( $args = '' ) {
	...
	if ( empty( $tags ) || is_mn_error( $tags ) )
		return;
	...
	$return = mn_generate_tag_cloud( $tags, $args ); // Here's where those top tags get sorted according to $args
	 ...
	 */
	$return = apply_filters( 'mn_tag_cloud', $return, $args );
 ...
 *                                                via {@see 'mn_generate_tag_cloud'}. Default 1|true.
 ...
 */
function mn_generate_tag_cloud( $tags, $args = '' ) {
	$defaults = array(
	...
	if ( $tags_sorted !== $tags ) {
		...
		if ( 'RAND' === $args['order'] ) {
			...
			if ( 'name' === $args['orderby'] ) {
				uasort( $tags, '_mn_object_name_sort_cb' );
			} else {
				uasort( $tags, '_mn_object_count_sort_cb' );
			}
	 ...
	 */
	$tags_data = apply_filters( 'mn_generate_tag_cloud_data', $tags_data );
	...
	if ( $args['filter'] ) {
		 ...
		 * to the $filter argument in mn_generate_tag_cloud().
		 *
		 * @since 2.3.0
		 *
		 * @see mn_generate_tag_cloud()
		 *
		 * @param array|string $return String containing the generated HTML tag cloud output
		 *                             or an array of tag links if the 'format' argument
		 *                             equals 'array'.
		 * @param array        $tags   An array of terms used in the tag cloud.
		 * @param array        $args   An array of mn_generate_tag_cloud() arguments.
		 */
		return apply_filters( 'mn_generate_tag_cloud', $return, $tags, $args );
	}
 ...
 */
function _mn_object_name_sort_cb( $a, $b ) {
 ...
 */
function _mn_object_count_sort_cb( $a, $b ) {
 ...
 * @return array|false|MN_Error Array of tag objects on success, false on failure.
 */
 ...
 * @return string|false|MN_Error A list of tags on success, false if there are no terms, MN_Error on failure.
 */
...
function term_description( $term = 0, $taxonomy = 'post_tag' ) {
	...
	return is_mn_error( $description ) ? '' : $description;
}
 ...
 * @return array|false|MN_Error Array of MN_Term objects on success, false if there are no terms
 *                              or the post does not exist, MN_Error on failure.
 */
function get_the_terms( $post, $taxonomy ) {
	...
	if ( false === $terms ) {
		$terms = mn_get_object_terms( $post->ID, $taxonomy );
		if ( ! is_mn_error( $terms ) ) {
			$term_ids = mn_list_pluck( $terms, 'term_id' );
			mn_cache_add( $post->ID, $term_ids, $taxonomy . '_relationships' );
		}
	}
	 ...
	 * @since 3.1.0
	 *
	 * @param array|MN_Error $terms    List of attached terms, or MN_Error on failure.
 ...
 * @param string $sep Optional. Separate items using this.
 * @param string $after Optional. After list.
 * @return string|false|MN_Error A list of terms on success, false if there are no terms, MN_Error on failure.
 */
 ...
 * @param string $sep Optional. Separate items using this.
 * @param string $after Optional. After list.
 * @return false|void False on Mtaandao error.
 */
function the_terms( $id, $taxonomy, $before = '', $sep = ', ', $after = '' ) {
	$term_list = get_the_term_list( $id, $taxonomy, $before, $sep, $after );

	if ( is_mn_error( $term_list ) )
		return false;
 ...
 *
 * Prior to v2.7 of Mtaandao, tags given as integers would also be checked against the post's tags' names and slugs (in addition to term_ids)
 * Prior to v2.7, this function could only be used in the Mtaandao Loop.
 * As of 2.7, the function can be used anywhere if it is provided a post ID or post object.
 *
 * @since 2.6.0
