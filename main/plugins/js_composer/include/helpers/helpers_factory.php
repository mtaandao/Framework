...
function vc_file_get_contents( $filename, $partial = false ) {
	...
	if ( empty( $mn_filesystem ) ) {
		require_once( ABSPATH . '/admin/includes/file.php' );
		MN_Filesystem();
	}
	/** @var $mn_filesystem MN_Filesystem_Base */
	if ( ! is_object( $mn_filesystem ) || ! $output = $mn_filesystem->get_contents( $filename ) ) {
...
function vc_generate_nonce( $data ) {
	return mn_create_nonce( is_array( $data ) ? ( 'vc-nonce-' . implode( '|', $data ) ) : ( 'vc-nonce-' . $data ) );
}
...
function vc_verify_nonce( $nonce, $data ) {
	return (bool) mn_verify_nonce( $nonce, ( is_array( $data ) ? ( 'vc-nonce-' . implode( '|', $data ) ) : ( 'vc-nonce-' . $data ) ) );
}
