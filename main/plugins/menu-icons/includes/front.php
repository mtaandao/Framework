...
final class Menu_Icons_Front_End {
	...
	public static function init() {
		...
		add_action( 'mn_enqueue_scripts', array( __CLASS__, '_enqueue_styles' ), 7 );
		add_filter( 'mn_nav_menu_args', array( __CLASS__, '_add_menu_item_title_filter' ) );
		add_filter( 'mn_nav_menu', array( __CLASS__, '_remove_menu_item_title_filter' ) );
	}


	/**
	 * Get nav menu ID based on arguments passed to mn_nav_menu()
	 *
	 * @since  0.3.0
	 * @param  array $args mn_nav_menu() Arguments
	...
	public static function get_nav_menu_id( $args ) {
		...
		if ( is_object( $menu ) && ! is_mn_error( $menu ) ) {
			return $menu->term_id;
		} else {
			return false;
		}
	 ...
	 *
	 * @since   0.1.0
	 * @mn_hook action mn_enqueue_scripts
	 * @link    http://codex.mtaandao.co.ke/Plugin_API/Action_Reference/mn_enqueue_scripts
	 */
	public static function _enqueue_styles() {
		foreach ( self::$icon_types as $type ) {
			if ( mn_style_is( $type->stylesheet_id, 'registered' ) ) {
				mn_enqueue_style( $type->stylesheet_id );
			}
		}
	 ...
	 * Thus, we're adding the filter when `mn_nav_menu()` is called.
	 *
	 * @since   0.1.0
	 * @mn_hook filter mn_nav_menu_args
	 ...
	 * filter when `mn_nav_menu()` exits.
	 *
	 * @since   0.1.0
	 * @mn_hook filter mn_nav_menu
	...
	public static function get_image_icon( $meta ) {
		...
		return mn_get_attachment_image( $meta['icon'], $meta['image_size'], false, $args );
	}
	...
	public static function get_svg_icon( $meta ) {
		...
		return sprintf(
			...
			esc_url( mn_get_attachment_url( $meta['icon'] ) ),
			esc_attr( $classes ),
			$style
		);
	}
