<?php
/**
 * Comment API: MN_Comment_Query class
 *
 * @package Mtaandao
 * @subpackage Comments
 * @since 4.4.0
 ...
 * @since 3.1.0
 *
 * @see MN_Comment_Query::__construct() for accepted arguments.
 */
class MN_Comment_Query {
	 ...
	 * @since 16.10.0 Introduced the `$cache_domain` argument.
	 * @access public
	 *
	 ...
	 *     @type array        $date_query                Date query clauses to limit comments by. See MN_Date_Query.
	 *                                                   Default null.
	 ...
	 *                                                   stored in a `children` property on the `MN_Comment`
	...
	public function __construct( $query = '' ) {
		$this->query_var_defaults = array(
			...
			'meta_value' => '',
			'meta_query' => '',
			'date_query' => null, // See MN_Date_Query
			'hierarchical' => false,
			'cache_domain' => 'core',
	 ...
	 *
	 * @since 4.2.0 Extracted from MN_Comment_Query::query().
	 *
	 * @access public
	 *
	 * @param string|array $query MN_Comment_Query arguments. See MN_Comment_Query::__construct()
	 */
	public function parse_query( $query = '' ) {
		...
		$this->query_vars = mn_parse_args( $query, $this->query_var_defaults );
		 ...
		 * @since 4.2.0
		 *
		 * @param MN_Comment_Query &$this The MN_Comment_Query instance (passed by reference).
		 */
		do_action_ref_array( 'parse_comment_query', array( &$this ) );
	}

	/**
	 * Sets up the Mtaandao query for retrieving comments.
	 *
	 * @since 3.1.0
	 ...
	 * @since 4.2.0 Moved parsing to MN_Comment_Query::parse_query().
	 * @access public
	 *
	...
	public function query( $query ) {
		$this->query_vars = mn_parse_args( $query );
	...
	public function get_comments() {
		 ...
		 * @since 3.1.0
		 *
		 * @param MN_Comment_Query &$this Current instance of MN_Comment_Query, passed by reference.
		 */
		...
		if ( ! empty( $this->meta_query->queries ) ) {
			$this->meta_query_clauses = $this->meta_query->get_sql( 'comment', $mndb->comments, 'comment_ID', $this );
		}

		// $args can include anything. Only use the args defined in the query_var_defaults to compute the key.
		$key = md5( serialize( mn_array_slice_assoc( $this->query_vars, array_keys( $this->query_var_defaults ) ) ) );
		$last_changed = mn_cache_get( 'last_changed', 'comment' );
		if ( ! $last_changed ) {
			$last_changed = microtime();
			mn_cache_set( 'last_changed', $last_changed, 'comment' );
		}

		$cache_key   = "get_comments:$key:$last_changed";
		$cache_value = mn_cache_get( $cache_key, 'comment' );
		if ( false === $cache_value ) {
			...
			mn_cache_add( $cache_key, $cache_value, 'comment' );
		} else {
		...
		// Convert to MN_Comment instances
	...
	protected function get_comment_ids() {
		...
		if ( ! in_array( 'any', $statuses ) ) {
			foreach ( $statuses as $status ) {
				switch ( $status ) {
					...
					default :
						$status_clauses[] = $mndb->prepare( "comment_approved = %s", $status );
						break;
				}
			}
		...
		if ( ! empty( $this->query_vars['include_unapproved'] ) ) {
			...
			foreach ( $include_unapproved as $unapproved_identifier ) {
				...
				if ( is_numeric( $unapproved_identifier ) ) {
					$approved_clauses[] = $mndb->prepare( "( user_id = %d AND comment_approved = '0' )", $unapproved_identifier );

				// Otherwise we match against email addresses.
				} else {
					$approved_clauses[] = $mndb->prepare( "( comment_author_email = %s AND comment_approved = '0' )", $unapproved_identifier );
				}
			}
		}
		...
		if ( in_array( $this->query_vars['orderby'], array( 'none', array(), false ), true ) ) {
			...
			if ( empty( $orderby_array ) ) {
				$orderby_array[] = "$mndb->comments.comment_date_gmt $order";
			}
			...
			if ( ! $found_orderby_comment_ID ) {
				...
				$orderby_array[] = "$mndb->comments.comment_ID $comment_ID_order";
			}

			$orderby = implode( ', ', $orderby_array );
		} else {
			$orderby = "$mndb->comments.comment_date_gmt $order";
		}
		...
		if ( $this->query_vars['count'] ) {
			...
			$fields = "$mndb->comments.comment_ID";
		}

		$post_id = absint( $this->query_vars['post_id'] );
		if ( ! empty( $post_id ) ) {
			$this->sql_clauses['where']['post_id'] = $mndb->prepare( 'comment_post_ID = %d', $post_id );
		}

		// Parse comment IDs for an IN clause.
		if ( ! empty( $this->query_vars['comment__in'] ) ) {
			$this->sql_clauses['where']['comment__in'] = "$mndb->comments.comment_ID IN ( " . implode( ',', mn_parse_id_list( $this->query_vars['comment__in'] ) ) . ' )';
		}

		// Parse comment IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['comment__not_in'] ) ) {
			$this->sql_clauses['where']['comment__not_in'] = "$mndb->comments.comment_ID NOT IN ( " . implode( ',', mn_parse_id_list( $this->query_vars['comment__not_in'] ) ) . ' )';
		}

		// Parse comment parent IDs for an IN clause.
		if ( ! empty( $this->query_vars['parent__in'] ) ) {
			$this->sql_clauses['where']['parent__in'] = 'comment_parent IN ( ' . implode( ',', mn_parse_id_list( $this->query_vars['parent__in'] ) ) . ' )';
		}

		// Parse comment parent IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['parent__not_in'] ) ) {
			$this->sql_clauses['where']['parent__not_in'] = 'comment_parent NOT IN ( ' . implode( ',', mn_parse_id_list( $this->query_vars['parent__not_in'] ) ) . ' )';
		}

		// Parse comment post IDs for an IN clause.
		if ( ! empty( $this->query_vars['post__in'] ) ) {
			$this->sql_clauses['where']['post__in'] = 'comment_post_ID IN ( ' . implode( ',', mn_parse_id_list( $this->query_vars['post__in'] ) ) . ' )';
		}

		// Parse comment post IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['post__not_in'] ) ) {
			$this->sql_clauses['where']['post__not_in'] = 'comment_post_ID NOT IN ( ' . implode( ',', mn_parse_id_list( $this->query_vars['post__not_in'] ) ) . ' )';
		}

		if ( '' !== $this->query_vars['author_email'] ) {
			$this->sql_clauses['where']['author_email'] = $mndb->prepare( 'comment_author_email = %s', $this->query_vars['author_email'] );
		}

		if ( '' !== $this->query_vars['author_url'] ) {
			$this->sql_clauses['where']['author_url'] = $mndb->prepare( 'comment_author_url = %s', $this->query_vars['author_url'] );
		}

		if ( '' !== $this->query_vars['karma'] ) {
			$this->sql_clauses['where']['karma'] = $mndb->prepare( 'comment_karma = %d', $this->query_vars['karma'] );
		}
		...
		foreach ( $raw_types as $operator => $_raw_types ) {
			...
			foreach ( $_raw_types as $type ) {
				switch ( $type ) {
					...
					default:
						$comment_types[ $operator ][] = $mndb->prepare( '%s', $type );
						break;
				}
			}
		...
		if ( '' !== $parent ) {
			$this->sql_clauses['where']['parent'] = $mndb->prepare( 'comment_parent = %d', $parent );
		}

		if ( is_array( $this->query_vars['user_id'] ) ) {
			$this->sql_clauses['where']['user_id'] = 'user_id IN (' . implode( ',', array_map( 'absint', $this->query_vars['user_id'] ) ) . ')';
		} elseif ( '' !== $this->query_vars['user_id'] ) {
			$this->sql_clauses['where']['user_id'] = $mndb->prepare( 'user_id = %d', $this->query_vars['user_id'] );
		}
		...
		$plucked = mn_array_slice_assoc( $this->query_vars, array( 'post_author', 'post_name', 'post_parent' ) );
		...
		if ( ! empty( $post_fields ) ) {
			...
			foreach ( $post_fields as $field_name => $field_value ) {
				...
				$this->sql_clauses['where'][ $field_name ] = $mndb->prepare( " {$mndb->posts}.{$field_name} IN (" . implode( ',', $esses ) . ')', $field_value );
			}
		}
		...
		foreach ( array( 'post_status', 'post_type' ) as $field_name ) {
			...
			if ( ! empty( $this->query_vars[ $field_name ] ) ) {
				...
				$this->sql_clauses['where'][ $field_name ] = $mndb->prepare( " {$mndb->posts}.{$field_name} IN (" . implode( ',', $esses ) . ")", $q_values );
			}
		}

		// Comment author IDs for an IN clause.
		if ( ! empty( $this->query_vars['author__in'] ) ) {
			$this->sql_clauses['where']['author__in'] = 'user_id IN ( ' . implode( ',', mn_parse_id_list( $this->query_vars['author__in'] ) ) . ' )';
		}

		// Comment author IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['author__not_in'] ) ) {
			$this->sql_clauses['where']['author__not_in'] = 'user_id NOT IN ( ' . implode( ',', mn_parse_id_list( $this->query_vars['author__not_in'] ) ) . ' )';
		}

		// Post author IDs for an IN clause.
		if ( ! empty( $this->query_vars['post_author__in'] ) ) {
			$join_posts_table = true;
			$this->sql_clauses['where']['post_author__in'] = 'post_author IN ( ' . implode( ',', mn_parse_id_list( $this->query_vars['post_author__in'] ) ) . ' )';
		}

		// Post author IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['post_author__not_in'] ) ) {
			$join_posts_table = true;
			$this->sql_clauses['where']['post_author__not_in'] = 'post_author NOT IN ( ' . implode( ',', mn_parse_id_list( $this->query_vars['post_author__not_in'] ) ) . ' )';
		}

		$join = '';

		if ( $join_posts_table ) {
			$join .= "JOIN $mndb->posts ON $mndb->posts.ID = $mndb->comments.comment_post_ID";
		}
		...
		if ( ! empty( $this->meta_query_clauses ) ) {
			...
			if ( ! $this->query_vars['count'] ) {
				$groupby = "{$mndb->comments}.comment_ID";
			}
		}

		if ( ! empty( $this->query_vars['date_query'] ) && is_array( $this->query_vars['date_query'] ) ) {
			$this->date_query = new MN_Date_Query( $this->query_vars['date_query'], 'comment_date' );
		...
		$this->sql_clauses['from']    = "FROM $mndb->comments $join";
		...
		if ( $this->query_vars['count'] ) {
			return intval( $mndb->get_var( $this->request ) );
		} else {
			$comment_ids = $mndb->get_col( $this->request );
	...
	private function set_found_comments() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			 ...
			 * @param MN_Comment_Query $comment_query        The `MN_Comment_Query` instance.
			 */
			$found_comments_query = apply_filters( 'found_comments_query', 'SELECT FOUND_ROWS()', $this );

			$this->found_comments = (int) $mndb->get_var( $found_comments_query );
		}
	}
	...
	protected function fill_descendants( $comments ) {
		...
		$levels = array(
			0 => mn_list_pluck( $comments, 'comment_ID' ),
		);
		...
		do {
			...
			foreach ( $_parent_ids as $parent_id ) {
				...
				$parent_child_ids = mn_cache_get( $cache_key, 'comment' );
			...
			if ( $uncached_parent_ids ) {
				...
				$level_comments = $mndb->get_results( "SELECT $mndb->comments.comment_ID, $mndb->comments.comment_parent {$this->sql_clauses['from']} {$where} {$this->sql_clauses['groupby']} ORDER BY comment_date_gmt ASC, comment_ID ASC" );
				...
				foreach ( $parent_map as $parent_id => $children ) {
					$cache_key = "get_comment_child_ids:$parent_id:$key:$last_changed";
					mn_cache_set( $cache_key, $children, 'comment' );
				}
			}
	...
	protected function get_search_sql( $string, $cols ) {
		...
		$like = '%' . $mndb->esc_like( $string ) . '%';

		$searches = array();
		foreach ( $cols as $col ) {
			$searches[] = $mndb->prepare( "$col LIKE %s", $like );
		}
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( $orderby == $this->query_vars['meta_key'] || $orderby == 'meta_value' ) {
			$parsed = "$mndb->commentmeta.meta_value";
		} elseif ( $orderby == 'meta_value_num' ) {
			$parsed = "$mndb->commentmeta.meta_value+0";
		} elseif ( $orderby == 'comment__in' ) {
			$comment__in = implode( ',', array_map( 'absint', $this->query_vars['comment__in'] ) );
			$parsed = "FIELD( {$mndb->comments}.comment_ID, $comment__in )";
