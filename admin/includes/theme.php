 ...
 * @return void|bool|MN_Error When void, echoes content.
 */
function delete_theme($stylesheet, $redirect = '') {
	...
	if ( empty( $redirect ) ) {
		$redirect = mn_nonce_url('themes.php?action=delete&stylesheet=' . urlencode( $stylesheet ), 'delete-theme_' . $stylesheet);
	}
	...
	$themes_dir = $mn_filesystem->mn_themes_dir();
	if ( empty( $themes_dir ) ) {
		return new MN_Error( 'fs_no_themes_dir', __( 'Unable to locate Mtaandao theme directory.' ) );
	}

	$themes_dir = trailingslashit( $themes_dir );
	$theme_dir = trailingslashit( $themes_dir . $stylesheet );
	$deleted = $mn_filesystem->delete( $theme_dir, true );

	if ( ! $deleted ) {
		return new MN_Error( 'could_not_remove_theme', sprintf( __( 'Could not fully remove the theme %s.' ), $stylesheet ) );
	}

	$theme_translations = mn_get_installed_translations( 'themes' );
	...
	if ( ! empty( $theme_translations[ $stylesheet ] ) ) {
		...
		foreach ( $translations as $translation => $data ) {
			$mn_filesystem->delete( MN_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '.po' );
			$mn_filesystem->delete( MN_LANG_DIR . '/themes/' . $stylesheet . '-' . $translation . '.mo' );
		}
	}

	// Remove the theme from allowed themes on the network.
	if ( is_multisite() ) {
		MN_Theme::network_disable_theme( $stylesheet );
	}
 ...
 *
 * @param MN_Theme $theme Theme data object.
 */
 ...
 *
 * @param MN_Theme $theme MN_Theme object.
...
function get_theme_update_available( $theme ) {
	...
	if ( isset($themes_update->response[ $stylesheet ]) ) {
		...
		$details_url = add_query_arg(array('TB_iframe' => 'true', 'width' => 1024, 'height' => 800), $update['url']); //Theme browser inside MN? replace this, Also, theme preview JS will override this on the available list.
		$update_url = mn_nonce_url( admin_url( 'update.php?action=upgrade-theme&amp;theme=' . urlencode( $stylesheet ) ), 'upgrade-theme_' . $stylesheet );
...
/**
 * Retrieve list of Mtaandao theme features (aka theme tags)
 *
 * @since 3.1.0
 *
...
function get_theme_feature_list( $api = true ) {
	...
	if ( !$feature_list = get_site_transient( 'mnorg_theme_feature_list' ) )
		set_site_transient( 'mnorg_theme_feature_list', array(), 3 * HOUR_IN_SECONDS );

	if ( !$feature_list ) {
		$feature_list = themes_api( 'feature_list', array() );
		if ( is_mn_error( $feature_list ) )
			return $features;
	}
	...
	set_site_transient( 'mnorg_theme_feature_list', $feature_list, 3 * HOUR_IN_SECONDS );
	...
	// Loop over the mnorg canonical list and apply translations
	$mnorg_features = array();
	foreach ( (array) $feature_list as $feature_category => $feature_items ) {
		if ( isset($category_translations[$feature_category]) )
			$feature_category = $category_translations[$feature_category];
		$mnorg_features[$feature_category] = array();

		foreach ( $feature_items as $feature ) {
			if ( isset($features[$feature_category][$feature]) )
				$mnorg_features[$feature_category][$feature] = $features[$feature_category][$feature];
			else
				$mnorg_features[$feature_category][$feature] = $feature;
		}
	}

	return $mnorg_features;
}
 ...
 *     }
 * }
 * @return object|array|MN_Error Response object or array on success, MN_Error on failure. See the
 *         {@link https://developer.mtaandao.co.ke/reference/functions/themes_api/ function reference article}
 ...
 * @since 3.8.0
 *
 * @param array $themes Optional. Array of MN_Theme objects to prepare.
 *                      Defaults to all allowed themes.
 *
 * @return array An associative array of theme data, sorted by name.
 */
function mn_prepare_themes_for_js( $themes = null ) {
	$current_theme = get_stylesheet();

	/**
	 * Filters theme data before it is prepared for JavaScript.
	 *
	 * Passing a non-empty array will result in mn_prepare_themes_for_js() returning
	 * early with that value instead.
	 *
	 * @since 4.2.0
	 *
	 * @param array      $prepared_themes An associative array of theme data. Default empty array.
	 * @param null|array $themes          An array of MN_Theme objects to prepare, if any.
	...
	if ( null === $themes ) {
		...
		if ( ! isset( $themes[ $current_theme ] ) ) {
			$themes[ $current_theme ] = mn_get_theme();
		}
	}
	...
	foreach ( $themes as $theme ) {
		...
		if ( current_user_can( 'edit_theme_options' ) && current_user_can( 'customize' ) ) {
			$customize_action = esc_url( add_query_arg(
				array(
					'return' => urlencode( esc_url_raw( mn_unslash( $_SERVER['REQUEST_URI'] ) ) ),
				),
		...
		$prepared_themes[ $slug ] = array(
			...
			'actions'      => array(
				...
				'delete'   => current_user_can( 'delete_themes' ) ? mn_nonce_url( admin_url( 'themes.php?action=delete&amp;stylesheet=' . $encoded_slug ), 'delete-theme_' . $slug ) : null,
			),
		);
	}
	 ...
	 */
	$prepared_themes = apply_filters( 'mn_prepare_themes_for_js', $prepared_themes );
...
function customize_themes_print_templates() {
	...
	<script type="text/html" id="tmpl-customize-themes-details-view">
		...
		<div class="theme-wrap mn-clearfix">
			...
			<div class="theme-about mn-clearfix">
