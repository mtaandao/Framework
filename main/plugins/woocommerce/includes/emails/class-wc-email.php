...
class WC_Email extends WC_Settings_API {
	 ...
	 *  https://raw.github.com/ushahidi/mn-silcc/master/class.html2text.inc
	...
	/**
	 * Get Mtaandao blog name.
	 *
	 * @return string
	 */
	public function get_blogname() {
		return mn_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES );
	}
	...
	public function get_from_name() {
		...
		return mn_specialchars_decode( esc_html( $from_name ), ENT_QUOTES );
	}
	...
	public function send( $to, $subject, $message, $headers, $attachments ) {
		add_filter( 'mn_mail_from', array( $this, 'get_from_address' ) );
		add_filter( 'mn_mail_from_name', array( $this, 'get_from_name' ) );
		add_filter( 'mn_mail_content_type', array( $this, 'get_content_type' ) );

		$message = apply_filters( 'woocommerce_mail_content', $this->style_inline( $message ) );
		$return  = mn_mail( $to, $subject, $message, $headers, $attachments );

		remove_filter( 'mn_mail_from', array( $this, 'get_from_address' ) );
		remove_filter( 'mn_mail_from_name', array( $this, 'get_from_name' ) );
		remove_filter( 'mn_mail_content_type', array( $this, 'get_content_type' ) );
	...
	protected function move_template_action( $template_type ) {
		if ( $template = $this->get_template( $template_type ) ) {
			if ( ! empty( $template ) ) {
				...
				if ( mn_mkdir_p( dirname( $theme_file ) ) && ! file_exists( $theme_file ) ) {
	...
	protected function admin_actions() {
		...
		if (
			...
			if ( empty( $_GET['_wc_email_nonce'] ) || ! mn_verify_nonce( $_GET['_wc_email_nonce'], 'woocommerce_email_template_nonce' ) ) {
				mn_die( __( 'Action failed. Please refresh the page and retry.', 'woocommerce' ) );
			}

			if ( ! current_user_can( 'edit_themes' ) ) {
				mn_die( __( 'Cheatin&#8217; huh?', 'woocommerce' ) );
			}
	...
	public function admin_options() {
		...
		<?php echo mnautop( mn_kses_post( $this->get_description() ) ); ?>
			...
			<?php
				...
				foreach ( $templates as $template_type => $title ) :
					...
					<div class="template <?php echo $template_type; ?>">
						...
						<h4><?php echo mn_kses_post( $title ); ?></h4>
						...
						<?php if ( file_exists( $local_file ) ) { ?>
							...
							<p>
								...
								<?php if ( is_writable( $local_file ) ) : ?>
									<a href="<?php echo esc_url( mn_nonce_url( remove_query_arg( array( 'move_template', 'saved' ), add_query_arg( 'delete_template', $template_type ) ), 'woocommerce_email_template_nonce', '_wc_email_nonce' ) ); ?>" class="delete_template button"><?php _e( 'Delete template file', 'woocommerce' ); ?></a>
								<?php endif; ?>
						...
						<?php } elseif ( file_exists( $template_file ) ) { ?>
							...
							<p>
								...
								<?php if ( ( is_dir( get_stylesheet_directory() . '/' . $template_dir . '/emails/' ) && is_writable( get_stylesheet_directory() . '/' . $template_dir . '/emails/' ) ) || is_writable( get_stylesheet_directory() ) ) { ?>
									<a href="<?php echo esc_url( mn_nonce_url( remove_query_arg( array( 'delete_template', 'saved' ), add_query_arg( 'move_template', $template_type ) ), 'woocommerce_email_template_nonce', '_wc_email_nonce' ) ); ?>" class="button"><?php _e( 'Copy file to theme', 'woocommerce' ); ?></a>
								<?php } ?>
