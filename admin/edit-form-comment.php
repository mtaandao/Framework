...
<?php mn_nonce_field('update-comment_' . $comment->comment_ID) ?>
<div class="wrap">
...
<?php
if ( 'approved' === mn_get_comment_status( $comment ) && $comment->comment_post_ID > 0 ) :
...
<?php
	...
	mn_editor( $comment->comment_content, 'content', array( 'media_buttons' => false, 'tinymce' => false, 'quicktags' => $quicktags_settings ) );
...
<?php echo "<a class='submitdelete deletion' href='" . mn_nonce_url("comment.php?action=" . ( !EMPTY_TRASH_DAYS ? 'deletecomment' : 'trashcomment' ) . "&amp;c=$comment->comment_ID&amp;_mn_original_http_referer=" . urlencode(mn_get_referer()), 'delete-comment_' . $comment->comment_ID) . "'>" . ( !EMPTY_TRASH_DAYS ? __('Delete Permanently') : __('Move to Trash') ) . "</a>\n"; ?>
</div>
