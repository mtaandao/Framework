<?php
/**
 * Widget API: MN_Widget base class
 *
 * @package Mtaandao
 * @subpackage Widgets
 * @since 4.4.0
...
class MN_Widget {
	...
	/**
	 * Option array passed to mn_register_widget_control().
	 *
	 * @since 2.8.0
	 * @access public
	 * @var array
	...
	public function widget( $args, $instance ) {
		die('function MN_Widget::widget() must be over-ridden in a sub-class.');
	}
	 ...
	 * @param array  $widget_options  Optional. Widget options. See mn_register_sidebar_widget() for information
	 *                                on accepted arguments. Default empty array.
	 * @param array  $control_options Optional. Widget control options. See mn_register_widget_control() for
	 *                                information on accepted arguments. Default empty array.
	 */
	public function __construct( $id_base, $name, $widget_options = array(), $control_options = array() ) {
		$this->id_base = empty($id_base) ? preg_replace( '/(mn_)?widget_/', '', strtolower(get_class($this)) ) : strtolower($id_base);
		$this->name = $name;
		$this->option_name = 'widget_' . $this->id_base;
		$this->widget_options = mn_parse_args( $widget_options, array( 'classname' => $this->option_name, 'customize_selective_refresh' => false ) );
		$this->control_options = mn_parse_args( $control_options, array( 'id_base' => $this->id_base ) );
	}
	 ...
	 *                                information on accepted arguments. Default empty array.
	 */
	public function MN_Widget( $id_base, $name, $widget_options = array(), $control_options = array() ) {
		_deprecated_constructor( 'MN_Widget', '4.3.0', get_class( $this ) );
		MN_Widget::__construct( $id_base, $name, $widget_options, $control_options );
	}
	...
	/**
	 * Constructs id attributes for use in MN_Widget::form() fields.
	 *
	 * This function should be used in form() methods to create id attributes
	 * for fields to be saved by MN_Widget::update().
	 *
	 * @since 2.8.0
	...
	public function is_preview() {
		global $mn_customize;
		return ( isset( $mn_customize ) && $mn_customize->is_preview() ) ;
	}

	/**
	 * Generates the actual widget content (Do NOT override).
	 *
	 * Finds the instance and calls MN_Widget::widget().
	 *
	 * @since 2.8.0
	 * @access public
	 *
	 * @param array     $args        Display arguments. See MN_Widget::widget() for information
	...
	public function display_callback( $args, $widget_args = 1 ) {
		...
		$widget_args = mn_parse_args( $widget_args, array( 'number' => -1 ) );
		...
		if ( array_key_exists( $this->number, $instances ) ) {
			 ...
			 * @param MN_Widget $this     The current widget instance.
			...
			$was_cache_addition_suspended = mn_suspend_cache_addition();
			if ( $this->is_preview() && ! $was_cache_addition_suspended ) {
				mn_suspend_cache_addition( true );
			}

			$this->widget( $args, $instance );

			if ( $this->is_preview() ) {
				mn_suspend_cache_addition( $was_cache_addition_suspended );
			}
		}
	}
	...
	public function update_callback( $deprecated = 1 ) {
		...
		if ( isset($_POST['delete_widget']) && $_POST['delete_widget'] ) {
			...
			if ( isset($mn_registered_widgets[$del_id]['params'][0]['number']) ) {
				$number = $mn_registered_widgets[$del_id]['params'][0]['number'];
	...
	public function form_callback( $widget_args = 1 ) {
		...
		if ( false !== $instance ) {
			 ...
			 * @since 2.8.0
			 *
			 * @param MN_Widget $this     The widget instance, passed by reference.
	...
	public function _register_one( $number = -1 ) {
		mn_register_sidebar_widget(	$this->id, $this->name,	$this->_get_display_callback(), $this->widget_options, array( 'number' => $number ) );
	...
	public function get_settings() {
		...
		if ( ! empty( $settings ) && ! isset( $settings['_multiwidget'] ) ) {
			...
			$settings = mn_convert_widget_settings( $this->id_base, $this->option_name, $settings );
		}
