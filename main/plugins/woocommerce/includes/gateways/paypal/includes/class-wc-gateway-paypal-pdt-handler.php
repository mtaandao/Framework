...
class WC_Gateway_Paypal_PDT_Handler extends WC_Gateway_Paypal_Response {
	...
	protected function validate_transaction( $transaction ) {
		...
		$response = mn_safe_remote_post( $this->sandbox ? 'https://www.sandbox.paypal.com/cgi-bin/webscr' : 'https://www.paypal.com/cgi-bin/webscr', $pdt );

		if ( is_mn_error( $response ) || ! strpos( $response['body'], "SUCCESS" ) === 0 ) {
			return false;
		}
