...
class WC_Session_Handler extends WC_Session {
	...
	public function __construct() {
		...
		$this->_cookie = 'mn_woocommerce_session_' . COOKIEHASH;
		$this->_table  = $mndb->prefix . 'woocommerce_sessions';
		...
		add_action( 'mn_logout', array( $this, 'destroy_session' ) );
	 ...
	 *
	 * Since the cookie name (as of 2.1) is prepended with mn, cache systems like batcache will not cache pages when set.
	 *
	...
	public function set_customer_session_cookie( $set ) {
		if ( $set ) {
			...
			$cookie_hash       = hash_hmac( 'md5', $to_hash, mn_hash( $to_hash ) );
	...
	public function get_session_cookie() {
		...
		$hash    = hash_hmac( 'md5', $to_hash, mn_hash( $to_hash ) );
	...
	public function save_data() {
		...
		if ( $this->_dirty && $this->has_session() ) {
			...
			// Set cache
			mn_cache_set( $this->get_cache_prefix() . $this->_customer_id, $this->_data, WC_SESSION_CACHE_GROUP, $this->_session_expiration - time() );
	...
	public function cleanup_sessions() {
		...
		if ( ! defined( 'MN_SETUP_CONFIG' ) && ! defined( 'MN_INSTALLING' ) ) {
			...
			$mndb->query( $mndb->prepare( "DELETE FROM $this->_table WHERE session_expiry < %d", time() ) );
	...
	public function get_session( $customer_id, $default = false ) {
		...
		$value = mn_cache_get( $this->get_cache_prefix() . $customer_id, WC_SESSION_CACHE_GROUP );

		if ( false === $value ) {
			$value = $mndb->get_var( $mndb->prepare( "SELECT session_value FROM $this->_table WHERE session_key = %s", $customer_id ) );

			if ( is_null( $value ) ) {
				$value = $default;
			}

			mn_cache_add( $this->get_cache_prefix() . $customer_id, $value, WC_SESSION_CACHE_GROUP, $this->_session_expiration - time() );
		}
	...
	public function delete_session( $customer_id ) {
		...
		mn_cache_delete( $this->get_cache_prefix() . $customer_id, WC_SESSION_CACHE_GROUP );
