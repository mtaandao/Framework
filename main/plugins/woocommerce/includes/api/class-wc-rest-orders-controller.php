...
class WC_REST_Orders_Controller extends WC_REST_Posts_Controller {
	...
	public function query_args( $args, $request ) {
		...
		if ( ! empty( $request['product'] ) ) {
			...
			// Force MN_Query return empty if don't found any order.
	 ...
	 *
	 * @param MN_REST_Request $request Request object.
	 * @return MN_Error|stdClass $data Object.
	 */
	...
	protected function set_item( $order, $item_type, $item, $action ) {
		...
		if ( 'update' === $action ) {
			$result = $mndb->get_row(
				$mndb->prepare( "SELECT * FROM {$mndb->prefix}woocommerce_order_items WHERE order_item_id = %d AND order_id = %d",
				absint( $item['id'] ),
				absint( $order->id )
			) );
