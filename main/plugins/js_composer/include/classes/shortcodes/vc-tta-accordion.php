...
class WPBakeryShortCode_VC_Tta_Accordion extends WPBakeryShortCodesContainer {
	...
	public function getParamContent( $atts, $content ) {
		$panelsContent = wpb_js_remove_mnautop( $content );
	...
	public function enqueueTtaStyles() {
		mn_register_style( 'vc_tta_style', vc_asset_url( 'css/js_composer_tta.min.css' ), false, WPB_VC_VERSION );
		mn_enqueue_style( 'vc_tta_style' );
	}
	...
	public function enqueueTtaScript() {
		...
		mn_register_script( 'vc_tta_autoplay_script', vc_asset_url( 'lib/vc-tta-autoplay/vc-tta-autoplay.min.js' ), array( 'vc_accordion_script' ), WPB_VC_VERSION, true );

		mn_enqueue_script( 'vc_accordion_script' );
		if ( ! vc_is_page_editable() ) {
			mn_enqueue_script( 'vc_tta_autoplay_script' );
		}
	}
