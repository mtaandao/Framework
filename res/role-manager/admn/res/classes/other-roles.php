 ...
 * Class RM_Other_Roles - support stuff for "Other Roles Access" add-on
 * Author: Mtaandao
...
class RM_Other_Roles {

    private $lib = null;
    const ACCESS_DATA_KEY = 'rm_other_roles_access_data';
    ...
    protected function get_access_data_from_post() {
        ...
        $keys_to_skip = array(
            ...
            'user_role', 
            'rm_access_model');
        $access_model = $_POST['rm_access_model'];
    ...
    public function get_html($user=null) {        
        ...
        ob_start();
?>
<form name="rm_other_roles_access_form" id="rm_other_roles_access_form" method="POST"
      action="<?php echo RM_MN_ADMIN_URL . RM_PARENT.'?page=users-'.RM_PLUGIN_FILE;?>" >
    <span style="font-weight: bold;"><?php echo esc_html_e('Block roles:', 'role-manager');?></span>&nbsp;&nbsp;
    ...
    <hr/>
<table id="rm_other_roles_access_table">
    <th><input type="checkbox" id="rm_other_roles_select_all"></th>
    <th><?php esc_html_e('Role Name','role-manager');?></th>
    <th><?php esc_html_e('Role ID', 'role-manager');?></th>    
<?php
...
?>
    ...
    <input type="hidden" name="action" id="action" value="rm_update_other_roles_access" />
