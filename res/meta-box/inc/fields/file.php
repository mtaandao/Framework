...
class RWMB_File_Field extends RWMB_Field {
	...
	public static function admin_enqueue_scripts() {
		mn_enqueue_style( 'rwmb-file', RWMB_CSS_URL . 'file.css', array(), RWMB_VER );
		mn_enqueue_script( 'rwmb-file', RWMB_JS_URL . 'file.js', array( 'jquery' ), RWMB_VER, true );
	...
	public static function add_actions() {
		...
		add_action( 'mn_ajax_rwmb_delete_file', array( __CLASS__, 'mn_ajax_delete_file' ) );
		add_action( 'mn_ajax_rwmb_reorder_files', array( __CLASS__, 'mn_ajax_reorder_files' ) );
	}
	 ...
	 */
	public static function mn_ajax_reorder_files() {
	 ...
	 */
	public static function mn_ajax_delete_file() {
		$post_id       = (int) filter_input( INPUT_POST, 'post_id', FILTER_SANITIZE_NUMBER_INT );
		$field_id      = (string) filter_input( INPUT_POST, 'field_id' );
		...
		$success = $force_delete ? mn_delete_attachment( $attachment_id ) : true;

		if ( $success ) {
			mn_send_json_success();
		}
		mn_send_json_error( __( 'Error: Cannot delete file', 'meta-box' ) );
	}
	...
	protected static function get_uploaded_files( $files, $field ) {
		$reorder_nonce = mn_create_nonce( "rwmb-reorder-files_{$field['id']}" );
		$delete_nonce  = mn_create_nonce( "rwmb-delete-file_{$field['id']}" );
	...
	protected static function file_html( $file ) {
		...
		return sprintf(
			...
			$file,
			mn_get_attachment_image( $file, array( 60, 60 ), true ),
			mn_get_attachment_url( $file ),
			get_the_title( $file ),
			$mime_type,
	...
	protected static function upload( $file, $post ) {
		$file = mn_handle_upload( $file, array( 'test_form' => false ) );
		if ( ! isset( $file['file'] ) ) {
			return false;
		}

		$attachment = mn_insert_attachment( array(
		...
		if ( is_mn_error( $attachment ) || ! $attachment ) {
			return false;
		}
		mn_update_attachment_metadata( $attachment, mn_generate_attachment_metadata( $attachment, $file['file'] ) );
		return $attachment;
	}
	...
	public static function file_info( $file, $args = array() ) {
		...
		return mn_parse_args( array(
			'ID'    => $file,
			'name'  => basename( $path ),
			'path'  => $path,
			'url'   => mn_get_attachment_url( $file ),
			'title' => get_the_title( $file ),
		), mn_get_attachment_metadata( $file ) );
	}
