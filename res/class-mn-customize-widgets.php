<?php
/**
 * Mtaandao Customize Widgets classes
 *
 * @package Mtaandao
 ...
 */
final class MN_Customize_Widgets {
	 ...
	 * @access public
	 *
	 * @param MN_Customize_Manager $manager Customize manager bootstrap instance.
	 */
	public function __construct( $manager ) {
		...
		add_action( 'mn_loaded',                               array( $this, 'override_sidebars_widgets_for_theme_switch' ) );
	...
	public function get_selective_refreshable_widgets() {
		...
		if ( ! isset( $this->selective_refreshable_widgets ) ) {
			...
			foreach ( $mn_widget_factory->widgets as $mn_widget ) {
				$this->selective_refreshable_widgets[ $mn_widget->id_base ] = ! empty( $mn_widget->widget_options['customize_selective_refresh'] );
			}
		}
	...
	public function register_settings() {
		...
		if ( $this->manager->doing_ajax( 'update-widget' ) && isset( $_REQUEST['widget-id'] ) ) {
			$widget_setting_ids[] = $this->get_setting_id( mn_unslash( $_REQUEST['widget-id'] ) );
		}
		 ...
		 * But don't do this if a customize_save because this will cause MN to think there is nothing
	 ...
	 * @access public
	 *
	 * @param false|array $args       The arguments to the MN_Customize_Setting constructor.
	...
	public function override_sidebars_widgets_for_theme_switch() {
		...
		$this->old_sidebars_widgets = mn_get_sidebars_widgets();
		...
		// reset global cache var used by mn_get_sidebars_widgets()
		unset( $GLOBALS['_mn_sidebars_widgets'] );
	}
	 ...
	 * @access public
	 *
	 * @see MN_Customize_Widgets::handle_theme_switch()
	 *
	 ...
	 *
	 * Note: these actions are also fired in mn_ajax_update_widget().
	 *
	 * @since 3.9.0
	 * @access public
	 */
	 ...
	 *
	 * When in preview, hook to {@see 'customize_register'} for settings after Mtaandao is loaded
	...
	public function schedule_customize_register() {
		if ( is_admin() ) {
			...
			add_action( 'mn', array( $this, 'customize_register' ) );
		}
	}
	...
	public function customize_register() {
		global $mn_registered_widgets, $mn_registered_widget_controls, $mn_registered_sidebars;

		add_filter( 'sidebars_widgets', array( $this, 'preview_sidebars_widgets' ), 1 );

		$sidebars_widgets = array_merge(
			array( 'mn_inactive_widgets' => array() ),
			array_fill_keys( array_keys( $mn_registered_sidebars ), array() ),
			mn_get_sidebars_widgets()
		);
		 ...
		 */
		foreach ( array_keys( $mn_registered_widgets ) as $widget_id ) {
		...
		foreach ( $sidebars_widgets as $sidebar_id => $sidebar_widget_ids ) {
			...
			$is_inactive_widgets   = ( 'mn_inactive_widgets' === $sidebar_id );
			...
			if ( $is_registered_sidebar || $is_inactive_widgets ) {
				...
				if ( $is_active_sidebar ) {
					...
					$section_args = array(
						'title' => $mn_registered_sidebars[ $sidebar_id ]['name'],
						'description' => $mn_registered_sidebars[ $sidebar_id ]['description'],
						'priority' => array_search( $sidebar_id, array_keys( $mn_registered_sidebars ) ),
						'panel' => 'widgets',
					...
					$section = new MN_Customize_Sidebar_Section( $this->manager, $section_id, $section_args );
					$this->manager->add_section( $section );

					$control = new MN_Widget_Area_Customize_Control( $this->manager, $setting_id, array(
						'section'    => $section_id,
						'sidebar_id' => $sidebar_id,
			...
			foreach ( $sidebar_widget_ids as $i => $widget_id ) {
				...
				if ( ! $is_active_sidebar || ! isset( $mn_registered_widgets[$widget_id] ) ) {
					continue;
				}

				$registered_widget = $mn_registered_widgets[$widget_id];
				$setting_id        = $this->get_setting_id( $widget_id );
				$id_base           = $mn_registered_widget_controls[$widget_id]['id_base'];

				$control = new MN_Widget_Form_Customize_Control( $this->manager, $setting_id, array(
					...
					'priority'       => $i,
					'width'          => $mn_registered_widget_controls[$widget_id]['width'],
					'height'         => $mn_registered_widget_controls[$widget_id]['height'],
	 ...
	 * @access public
	 *
	 * @see MN_Customize_Panel::$active_callback
	 *
	 * @global array $mn_registered_sidebars
	 * @return bool Active.
	 */
	public function is_panel_active() {
		...
		return ! empty( $mn_registered_sidebars );
	}
	 ...
	 * @return MN_Error|array Array containing a widget's id_base and number components,
	 *                        or a MN_Error object.
	 */
	public function parse_widget_setting_id( $setting_id ) {
		if ( ! preg_match( '/^(widget_(.+?))(?:\[(\d+)\])?$/', $setting_id, $matches ) ) {
			return new MN_Error( 'widget_setting_invalid_id' );
		}
	...
	public function enqueue_scripts() {
		global $mn_scripts, $mn_registered_sidebars, $mn_registered_widgets;

		mn_enqueue_style( 'customize-widgets' );
		mn_enqueue_script( 'customize-widgets' );
		...
		$settings = array(
			'registeredSidebars'   => array_values( $mn_registered_sidebars ),
			'registeredWidgets'    => $mn_registered_widgets,
		...
		$mn_scripts->add_data(
			'customize-widgets',
			'data',
			sprintf( 'var _mnCustomizeWidgetsSettings = %s;', mn_json_encode( $settings ) )
		);
	}
	...
	public function get_available_widgets() {
		...
		foreach ( $sort as $widget ) {
			...
			$is_disabled     = false;
			$is_multi_widget = ( isset( $mn_registered_widget_controls[$widget['id']]['id_base'] ) && isset( $widget['params'][0]['number'] ) );
			if ( $is_multi_widget ) {
				$id_base            = $mn_registered_widget_controls[$widget['id']]['id_base'];
				...
				if ( $sidebar && 'mn_inactive_widgets' !== $sidebar ) {
					$is_disabled = true;
				}
				$id_base = $widget['id'];
			}

			$list_widget_controls_args = mn_list_widget_controls_dynamic_sidebar( array( 0 => $args, 1 => $widget['params'][0] ) );
			...
			$available_widget = array_merge( $available_widget, array(
				...
				'width'        => $mn_registered_widget_controls[$widget['id']]['width'],
				'height'       => $mn_registered_widget_controls[$widget['id']]['height'],
	...
	public function get_widget_control( $args ) {
		...
		ob_start();
		call_user_func_array( 'mn_widget_control', $args );
	...
	public function customize_preview_init() {
		add_action( 'mn_enqueue_scripts', array( $this, 'customize_preview_enqueue' ) );
		add_action( 'mn_print_styles',    array( $this, 'print_preview_css' ), 1 );
		add_action( 'mn_footer',          array( $this, 'export_preview_data' ), 20 );
	}
	...
	public function refresh_nonces( $nonces ) {
		$nonces['update-widget'] = mn_create_nonce( 'update-widget' );
		return $nonces;
	}
	 ...
	 *
	 * Because mn_get_sidebars_widgets() gets called early at {@see 'init' } (via
	 * mn_convert_widget_settings()) and can set global variable `$_mn_sidebars_widgets`
	...
	public function customize_preview_enqueue() {
		mn_enqueue_script( 'customize-preview-widgets' );
	 ...
	 * and at the very end of the mn_footer,
	 *
	 * @since 3.9.0
	 * @access public
     *
	...
	public function export_preview_data() {
		...
		<script type="text/javascript">
			var _mnWidgetCustomizerPreviewSettings = <?php echo mn_json_encode( $settings ); ?>;
		</script>
		<?php
	}
	...
	protected function get_instance_hash_key( $serialized_instance ) {
		return mn_hash( $serialized_instance );
	}
	...
	public function sanitize_sidebar_widgets_js_instance( $widget_ids ) {
		...
		$widget_ids = array_values( array_intersect( $widget_ids, array_keys( $mn_registered_widgets ) ) );
		return $widget_ids;
	}
	 ...
	 *
	 * @param  string $widget_id Widget ID.
	 * @return MN_Error|array Array containing the updated widget information.
	 *                        A MN_Error object, otherwise.
	 */
	public function call_widget_update( $widget_id ) {
		global $mn_registered_widget_updates, $mn_registered_widget_controls;
		...
		if ( ! empty( $_POST['sanitized_widget_setting'] ) ) {
			...
			if ( false === $sanitized_widget_setting ) {
				...
				return new MN_Error( 'widget_setting_malformed' );
			}

			$instance = $this->sanitize_widget_instance( $sanitized_widget_setting );
			if ( is_null( $instance ) ) {
				$this->stop_capturing_option_updates();
				return new MN_Error( 'widget_setting_unsanitized' );
			}
			...
			if ( ! is_null( $parsed_id['number'] ) ) {
				...
				$_REQUEST[$key] = $_POST[$key] = mn_slash( $value );
		...
		if ( 0 !== $this->count_captured_options() ) {
			if ( $this->count_captured_options() > 1 ) {
				$this->stop_capturing_option_updates();
				return new MN_Error( 'widget_setting_too_many_options' );
			}

			$updated_option_name = key( $this->get_captured_options() );
			if ( $updated_option_name !== $option_name ) {
				$this->stop_capturing_option_updates();
				return new MN_Error( 'widget_setting_unexpected_option' );
			}
		}
		 ...
		 * in place from MN_Customize_Setting::preview() will use this value
		...
		ob_start();
		$form = $mn_registered_widget_controls[ $widget_id ];
		if ( $form ) {
	 ...
	 *
	 * Most code here copied from mn_ajax_save_widget().
	 *
	 * @since 3.9.0
	 * @access public
	 *
	 * @see mn_ajax_save_widget()
	 */
	public function mn_ajax_update_widget() {
		...
		if ( empty( $_POST['widget-id'] ) ) {
			mn_send_json_error( 'missing_widget-id' );
		}
		...
		if ( $is_updating_widget_template ) {
			mn_send_json_error( 'template_widget_not_updatable' );
		}

		$updated_widget = $this->call_widget_update( $widget_id ); // => {instance,form}
		if ( is_mn_error( $updated_widget ) ) {
			mn_send_json_error( $updated_widget->get_error_code() );
		}

		$form = $updated_widget['form'];
		$instance = $this->sanitize_widget_js_instance( $updated_widget['instance'] );

		mn_send_json_success( compact( 'form', 'instance' ) );
	}
	...
	public function selective_refresh_init() {
		...
		add_filter( 'mn_kses_allowed_html', array( $this, 'filter_mn_kses_allowed_data_attributes' ) );
	 ...
	 * }
	 * @see MN_Customize_Nav_Menus_Partial_Refresh::filter_mn_nav_menu_args()
	 *
	 * @return array Params.
	 */
	public function filter_dynamic_sidebar_params( $params ) {
		...
		$attributes .= sprintf( ' data-customize-partial-placement-context="%s"', esc_attr( mn_json_encode( $context ) ) );
	 ...
	 *
	 * This is used in the {@see 'filter_mn_kses_allowed_html'} filter to ensure that the
	 ...
	 *
	 * This is needed in case the `$before_widget` is run through mn_kses() when printed.
	 *
	 * @since 4.5.0
	 * @access public
	 *
	 ...
	 */
	public function filter_mn_kses_allowed_data_attributes( $allowed_html ) {
	...
	public function capture_filter_pre_get_option( $value ) {
		...
		if ( isset( $this->_captured_options[ $option_name ] ) ) {
			...
			/** This filter is documented in res/option.php */
