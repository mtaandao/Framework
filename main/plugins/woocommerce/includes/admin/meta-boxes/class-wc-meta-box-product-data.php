...
class WC_Meta_Box_Product_Data {
	...
	public static function output( $post ) {
		...
		mn_nonce_field( 'woocommerce_save_data', 'woocommerce_meta_nonce' );

		$thepostid = $post->ID;

		if ( $terms = mn_get_object_terms( $post->ID, 'product_type' ) ) {
		...
		<div class="panel-wrap product_data">
			...
			<div id="general_product_data" class="panel woocommerce_options_panel"><?php
				...
				echo '<div class="options_group show_if_external">';
					...
					// External URL
					woocommerce_mn_text_input( array( 'id' => '_product_url', 'label' => __( 'Product URL', 'woocommerce' ), 'placeholder' => 'http://', 'description' => __( 'Enter the external URL to the product.', 'woocommerce' ) ) );

					// Button text
					woocommerce_mn_text_input( array( 'id' => '_button_text', 'label' => __( 'Button text', 'woocommerce' ), 'placeholder' => _x('Buy product', 'placeholder', 'woocommerce'), 'description' => __( 'This text will be shown on the button linking to the external product.', 'woocommerce' ) ) );
				...
				echo '<div class="options_group pricing show_if_simple show_if_external hidden">';
					...
					// Price
					woocommerce_mn_text_input( array( 'id' => '_regular_price', 'label' => __( 'Regular price', 'woocommerce' ) . ' (' . get_woocommerce_currency_symbol() . ')', 'data_type' => 'price' ) );

					// Special Price
					woocommerce_mn_text_input( array( 'id' => '_sale_price', 'data_type' => 'price', 'label' => __( 'Sale price', 'woocommerce' ) . ' ('.get_woocommerce_currency_symbol().')', 'description' => '<a href="#" class="sale_schedule">' . __( 'Schedule', 'woocommerce' ) . '</a>' ) );
				...
				echo '<div class="options_group show_if_downloadable hidden">';
					...
					// Download Limit
					woocommerce_mn_text_input( array( 'id' => '_download_limit', 'label' => __( 'Download limit', 'woocommerce' ), 'placeholder' => __( 'Unlimited', 'woocommerce' ), 'description' => __( 'Leave blank for unlimited re-downloads.', 'woocommerce' ), 'type' => 'number', 'custom_attributes' => array(
						'step' 	=> '1',
						'min'	=> '0'
					) ) );

					// Expirey
					woocommerce_mn_text_input( array( 'id' => '_download_expiry', 'label' => __( 'Download expiry', 'woocommerce' ), 'placeholder' => __( 'Never', 'woocommerce' ), 'description' => __( 'Enter the number of days before a download link expires, or leave blank.', 'woocommerce' ), 'type' => 'number', 'custom_attributes' => array(
						'step' 	=> '1',
						'min'	=> '0'
					) ) );

					 // Download Type
					woocommerce_mn_select( array( 'id' => '_download_type', 'label' => __( 'Download type', 'woocommerce' ), 'description' => sprintf( __( 'Choose a download type - this controls the <a href="%s">schema</a>.', 'woocommerce' ), 'http://schema.org/' ), 'options' => array(
			...
			<div id="inventory_product_data" class="panel woocommerce_options_panel hidden">
				...
				if ( wc_product_sku_enabled() ) {
					woocommerce_mn_text_input( array( 'id' => '_sku', 'label' => '<abbr title="'. __( 'Stock Keeping Unit', 'woocommerce' ) .'">' . __( 'SKU', 'woocommerce' ) . '</abbr>', 'desc_tip' => 'true', 'description' => __( 'SKU refers to a Stock-keeping unit, a unique identifier for each distinct product and service that can be purchased.', 'woocommerce' ) ) );
				} else {
				...
				if ( 'yes' === get_option( 'woocommerce_manage_stock' ) ) {
					...
					// manage stock
					woocommerce_mn_checkbox( array( 'id' => '_manage_stock', 'wrapper_class' => 'show_if_simple show_if_variable', 'label' => __( 'Manage stock?', 'woocommerce' ), 'description' => __( 'Enable stock management at product level', 'woocommerce' ) ) );
					...
					// Stock
					woocommerce_mn_text_input( array(
						'id'                => '_stock',
					...
					// Backorders?
					woocommerce_mn_select( array( 'id' => '_backorders', 'label' => __( 'Allow backorders?', 'woocommerce' ), 'options' => array(
				...
				// Stock status
				woocommerce_mn_select( array( 'id' => '_stock_status', 'wrapper_class' => 'hide_if_variable hide_if_external', 'label' => __( 'Stock status', 'woocommerce' ), 'options' => array(
				...
				// Individual product
				woocommerce_mn_checkbox( array( 'id' => '_sold_individually', 'wrapper_class' => 'show_if_simple show_if_variable', 'label' => __( 'Sold individually', 'woocommerce' ), 'description' => __( 'Enable this to only allow one of this item to be bought in a single order', 'woocommerce' ) ) );
			...
			<div id="shipping_product_data" class="panel woocommerce_options_panel hidden">
				...
				echo '<div class="options_group">';
					...
					if ( wc_product_weight_enabled() ) {
						woocommerce_mn_text_input( array( 'id' => '_weight', 'label' => __( 'Weight', 'woocommerce' ) . ' (' . get_option( 'woocommerce_weight_unit' ) . ')', 'placeholder' => wc_format_localized_decimal( 0 ), 'desc_tip' => 'true', 'description' => __( 'Weight in decimal form', 'woocommerce' ), 'type' => 'text', 'data_type' => 'decimal' ) );
					}
				...
				echo '<div class="options_group">';
					...
					if ( $classes && ! is_mn_error( $classes ) ) {
					...
					?><p class="form-field dimensions_field"><label for="product_shipping_class"><?php _e( 'Shipping class', 'woocommerce' ); ?></label> <?php mn_dropdown_categories( $args ); ?> <?php echo wc_help_tip( __( 'Shipping classes are used by certain shipping methods to group similar products.', 'woocommerce' ) ); ?></p><?php
			...
			<div id="linked_product_data" class="panel woocommerce_options_panel hidden">
						...
						<input type="hidden" class="wc-product-search" style="width: 50%;" id="upsell_ids" name="upsell_ids" data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'woocommerce' ); ?>" data-action="woocommerce_json_search_products" data-multiple="true" data-exclude="<?php echo intval( $post->ID ); ?>" data-selected="<?php
							...
							foreach ( $product_ids as $product_id ) {
								...
								if ( is_object( $product ) ) {
									$json_ids[ $product_id ] = mn_kses_post( html_entity_decode( $product->get_formatted_name(), ENT_QUOTES, get_bloginfo( 'charset' ) ) );
								}
							}
				...
				<div class="options_group grouping show_if_simple show_if_external">
					...
					<p class="form-field">
						...
						<input type="hidden" class="wc-product-search" style="width: 50%;" id="parent_id" name="parent_id" data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'woocommerce' ); ?>" data-action="woocommerce_json_search_grouped_products" data-allow_clear="true" data-multiple="false" data-exclude="<?php echo intval( $post->ID ); ?>" data-selected="<?php
							...
							if ( $parent_id ) {
								...
								if ( is_object( $parent ) ) {
									$parent_title = mn_kses_post( html_entity_decode( $parent->get_formatted_name(), ENT_QUOTES, get_bloginfo( 'charset' ) ) );
								}
					...
					<?php
						woocommerce_mn_hidden_input( array( 'id' => 'previous_parent_id', 'value' => absint( $post->post_parent ) ) );
			...
			<div id="advanced_product_data" class="panel woocommerce_options_panel hidden">
				...
				<div class="options_group hide_if_external">
					<?php
						// Purchase note
						woocommerce_mn_textarea_input(  array( 'id' => '_purchase_note', 'label' => __( 'Purchase note', 'woocommerce' ), 'desc_tip' => 'true', 'description' => __( 'Enter an optional note to send the customer after purchase.', 'woocommerce' ) ) );
					?>
				</div>
				...
				<div class="options_group">
					<?php
						// menu_order
						woocommerce_mn_text_input(  array( 'id' => 'menu_order', 'label' => __( 'Menu order', 'woocommerce' ), 'desc_tip' => 'true', 'description' => __( 'Custom ordering position.', 'woocommerce' ), 'value' => intval( $post->menu_order ), 'type' => 'number', 'custom_attributes' => array(
							'step' 	=> '1'
						)  ) );
					?>
				</div>
				...
				<div class="options_group reviews">
					<?php
						woocommerce_mn_checkbox( array( 'id' => 'comment_status', 'label' => __( 'Enable reviews', 'woocommerce' ), 'cbvalue' => 'open', 'value' => esc_attr( $post->comment_status ) ) );
	...
	public static function output_variations() {
		...
		$variations_count       = absint( $mndb->get_var( $mndb->prepare( "SELECT COUNT(ID) FROM $mndb->posts WHERE post_parent = %d AND post_type = 'product_variation' AND post_status IN ('publish', 'private')", $post->ID ) ) );
					...
					<div class="variations-defaults">
						...
						<?php
							...
							foreach ( $attributes as $attribute ) {
								...
								if ( $attribute['is_taxonomy'] ) {
									$post_terms = mn_get_post_terms( $post->ID, $attribute['name'] );
	...
	public static function save( $post_id, $post ) {
		...
		mn_set_object_terms( $post_id, $product_type, 'product_type' );
		...
		if ( isset( $_POST['_purchase_note'] ) ) {
			update_post_meta( $post_id, '_purchase_note', mn_kses_post( stripslashes( $_POST['_purchase_note'] ) ) );
		}
		...
		mn_set_object_terms( $post_id, $product_shipping_class, 'product_shipping_class');
					...
					if ( taxonomy_exists( $attribute_names[ $i ] ) ) {
						...
						foreach( $values as $key => $value ) {
							...
							if ( $term ) {
								...
								$term = mn_insert_term( trim( $value ), $attribute_names[ $i ] );
						...
						mn_set_object_terms( $post_id, $values, $attribute_names[ $i ] );
					}
		...
		if ( ! empty( $old_attributes ) ) {
			foreach ( $old_attributes as $key => $value ) {
				if ( empty( $attributes[ $key ] ) && ! empty( $value['is_taxonomy'] ) && taxonomy_exists( $key ) ) {
					mn_set_object_terms( $post_id, array(), $key );
				}
			}
		}
		...
		if ( 'yes' == $is_downloadable ) {
			...
			if ( isset( $_POST['_wc_file_urls'] ) ) {
				...
				$file_urls          = isset( $_POST['_wc_file_urls'] )  ? mn_unslash( array_map( 'trim', $_POST['_wc_file_urls'] ) ) : array();
				...
				for ( $i = 0; $i < $file_url_size; $i ++ ) {
					if ( ! empty( $file_urls[ $i ] ) ) {
						...
						if ( in_array( $file_is, array( 'absolute', 'relative' ) ) ) {
							$file_type  = mn_check_filetype( strtok( $file_url, '?' ), $allowed_file_types );
		...
		if ( isset( $_POST['variable_sku'] ) ) {
			...
			for ( $i = 0; $i <= $max_loop; $i ++ ) {
				...
				if ( ! $variation_id ) {
					...
					$variation_id = mn_insert_post( $variation );
				...
				update_post_meta( $variation_id, '_variation_description', mn_kses_post( $variable_description[ $i ] ) );

				// Save shipping class
				$variable_shipping_class[ $i ] = ! empty( $variable_shipping_class[ $i ] ) ? (int) $variable_shipping_class[ $i ] : '';
				mn_set_object_terms( $variation_id, $variable_shipping_class[ $i ], 'product_shipping_class');
				...
				$delete_attribute_keys = $mndb->get_col( $mndb->prepare( "SELECT meta_key FROM {$mndb->postmeta} WHERE meta_key LIKE 'attribute_%%' AND meta_key NOT IN ( '" . implode( "','", $updated_attribute_keys ) . "' ) AND post_id = %d;", $variation_id ) );
