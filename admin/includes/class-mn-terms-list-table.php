<?php
/**
 * List Table API: MN_Terms_List_Table class
 *
 * @package Mtaandao
 ...
 * @see MN_List_Table
 */
class MN_Terms_List_Table extends MN_List_Table {
	...
	public function prepare_items() {
		...
		$search = !empty( $_REQUEST['s'] ) ? trim( mn_unslash( $_REQUEST['s'] ) ) : '';
		...
		if ( !empty( $_REQUEST['orderby'] ) )
			$args['orderby'] = trim( mn_unslash( $_REQUEST['orderby'] ) );

		if ( !empty( $_REQUEST['order'] ) )
			$args['order'] = trim( mn_unslash( $_REQUEST['order'] ) );

		$this->callback_args = $args;

		$this->set_pagination_args( array(
			'total_items' => mn_count_terms( $this->screen->taxonomy, compact( 'search' ) ),
	...
	public function display_rows_or_placeholder() {
		...
		$args = mn_parse_args( $this->callback_args, array(
			'page' => 1,
			'number' => 20,
			'search' => '',
			'hide_empty' => 0
	...
	public function column_name( $tag ) {
		 ...
		 * @since 2.5.0
		 *
		 * @see MN_Terms_List_Table::column_name()
		 *
		...
		$uri = ( defined( 'DOING_AJAX' ) && DOING_AJAX ) ? mn_get_referer() : $_SERVER['REQUEST_URI'];

		$edit_link = add_query_arg(
			'mn_http_referer',
			urlencode( mn_unslash( $uri ) ),
	...
	protected function handle_row_actions( $tag, $column_name, $primary ) {
		...
		if ( current_user_can( $tax->cap->delete_terms ) && $tag->term_id != $default_term ) {
			$actions['delete'] = sprintf(
				...
				mn_nonce_url( "edit-tags.php?action=delete&amp;taxonomy=$taxonomy&amp;tag_ID=$tag->term_id", 'delete-tag_' . $tag->term_id ),
				/* translators: %s: taxonomy term name */
...
?>
	...
	?>
		...
		<p class="inline-edit-save submit">
			...
			<?php mn_nonce_field( 'taxinlineeditnonce', '_inline_edit', false ); ?>
