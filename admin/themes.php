...
if ( current_user_can( 'switch_themes' ) && isset($_GET['action'] ) ) {
	if ( 'activate' == $_GET['action'] ) {
		...
		$theme = mn_get_theme( $_GET['stylesheet'] );
		...
		mn_redirect( admin_url('themes.php?activated=true') );
		exit;
		...
		if ( $active->get( 'Template' ) == $_GET['stylesheet'] ) {
			mn_redirect( admin_url( 'themes.php?delete-active-child=true' ) );
		} else {
			delete_theme( $_GET['stylesheet'] );
			mn_redirect( admin_url( 'themes.php?deleted=true' ) );
		}
		exit;
	}
}
...
if ( current_user_can( 'switch_themes' ) ) {
	$help_overview  = '<p>' . __( 'This screen is used for managing your installed themes. Aside from the default theme(s) included with your Mtaandao installation, themes are designed and developed by third parties.' ) . '</p>' .
...
if ( current_user_can( 'install_themes' ) ) {
	if ( is_multisite() ) {
		...
		$help_install = '<p>' . sprintf( __('If you would like to see more themes to choose from, click on the &#8220;Add New&#8221; button and you will be able to browse or search for additional themes from the <a href="%s" target="_blank">Mtaandao Theme Directory</a>. Themes in the Mtaandao Theme Directory are designed and developed by third parties, and are compatible with the license Mtaandao uses. Oh, and they&#8217;re free!'), 'https://mtaandao.co.ke/themes/' ) . '</p>';
	}
...
if ( current_user_can( 'switch_themes' ) ) {
	...
	$themes = mn_prepare_themes_for_js( array( mn_get_theme() ) );
}
mn_reset_vars( array( 'theme', 'search' ) );
...
mn_enqueue_script( 'customize-loader' );
	...
	if ( is_array( $submenu ) && isset( $submenu['themes.php'] ) ) {
		foreach ( (array) $submenu['themes.php'] as $item) {
			...
			if ( !empty($submenu[$item[2]]) ) {
				...
				if ( file_exists(MN_PLUGIN_DIR . "/{$submenu[$item[2]][0][2]}") || !empty($menu_hook))
...
if ( ! is_multisite() && current_user_can('edit_themes') && $broken_themes = mn_get_themes( array( 'errors' => true ) ) ) {
?>
	...
	<?php foreach ( $broken_themes as $broken_theme ) : ?>
		<tr>
			...
			if ( $can_delete ) {
				...
				$delete_url = mn_nonce_url( $delete_url, 'delete-theme_' . $stylesheet );
				?>
			...
			if ( $can_install && 'theme_no_parent' === $broken_theme->errors()->get_error_code() ) {
				...
				if ( ! is_mn_error( $parent_theme ) ) {
					$install_url = add_query_arg( array(
						'action' => 'install-theme',
						'theme'  => urlencode( $parent_theme_name ),
					), admin_url( 'update.php' ) );
					$install_url = mn_nonce_url( $install_url, 'install-theme_' . $parent_theme_name );
					?>
