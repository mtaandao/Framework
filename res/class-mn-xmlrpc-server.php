<?php
/**
 * XML-RPC protocol support for Mtaandao
 *
 * @package Mtaandao
...
/**
 * Mtaandao XMLRPC server implementation.
 *
 * Implements compatibility for Blogger API, MetaWeblog API, MovableType, and
 * pingback. Additional Mtaandao API for managing comments, pages, posts,
 * options, etc.
 *
 * As of Mtaandao 3.5.0, XML-RPC is enabled by default. It can be disabled
 * via the {@see 'xmlrpc_enabled'} filter found in mn_xmlrpc_server::login().
 *
 * @package Mtaandao
 * @subpackage Publishing
 * @since 1.5.0
 */
class mn_xmlrpc_server extends IXR_Server {
	/**
	 * Methods.
	 *
	 * @access public
	...
	/**
	 * Flags that the user authentication has failed in this instance of mn_xmlrpc_server.
	 *
	 * @access protected
	 * @var bool
	 */
	...
	public function __construct() {
		$this->methods = array(
			// Mtaandao API
			'mn.getUsersBlogs'		=> 'this:mn_getUsersBlogs',
			'mn.newPost'			=> 'this:mn_newPost',
			'mn.editPost'			=> 'this:mn_editPost',
			'mn.deletePost'			=> 'this:mn_deletePost',
			'mn.getPost'			=> 'this:mn_getPost',
			'mn.getPosts'			=> 'this:mn_getPosts',
			'mn.newTerm'			=> 'this:mn_newTerm',
			'mn.editTerm'			=> 'this:mn_editTerm',
			'mn.deleteTerm'			=> 'this:mn_deleteTerm',
			'mn.getTerm'			=> 'this:mn_getTerm',
			'mn.getTerms'			=> 'this:mn_getTerms',
			'mn.getTaxonomy'		=> 'this:mn_getTaxonomy',
			'mn.getTaxonomies'		=> 'this:mn_getTaxonomies',
			'mn.getUser'			=> 'this:mn_getUser',
			'mn.getUsers'			=> 'this:mn_getUsers',
			'mn.getProfile'			=> 'this:mn_getProfile',
			'mn.editProfile'		=> 'this:mn_editProfile',
			'mn.getPage'			=> 'this:mn_getPage',
			'mn.getPages'			=> 'this:mn_getPages',
			'mn.newPage'			=> 'this:mn_newPage',
			'mn.deletePage'			=> 'this:mn_deletePage',
			'mn.editPage'			=> 'this:mn_editPage',
			'mn.getPageList'		=> 'this:mn_getPageList',
			'mn.getAuthors'			=> 'this:mn_getAuthors',
			'mn.getCategories'		=> 'this:mw_getCategories',		// Alias
			'mn.getTags'			=> 'this:mn_getTags',
			'mn.newCategory'		=> 'this:mn_newCategory',
			'mn.deleteCategory'		=> 'this:mn_deleteCategory',
			'mn.suggestCategories'	=> 'this:mn_suggestCategories',
			'mn.uploadFile'			=> 'this:mw_newMediaObject',	// Alias
			'mn.deleteFile'			=> 'this:mn_deletePost',		// Alias
			'mn.getCommentCount'	=> 'this:mn_getCommentCount',
			'mn.getPostStatusList'	=> 'this:mn_getPostStatusList',
			'mn.getPageStatusList'	=> 'this:mn_getPageStatusList',
			'mn.getPageTemplates'	=> 'this:mn_getPageTemplates',
			'mn.getOptions'			=> 'this:mn_getOptions',
			'mn.setOptions'			=> 'this:mn_setOptions',
			'mn.getComment'			=> 'this:mn_getComment',
			'mn.getComments'		=> 'this:mn_getComments',
			'mn.deleteComment'		=> 'this:mn_deleteComment',
			'mn.editComment'		=> 'this:mn_editComment',
			'mn.newComment'			=> 'this:mn_newComment',
			'mn.getCommentStatusList' => 'this:mn_getCommentStatusList',
			'mn.getMediaItem'		=> 'this:mn_getMediaItem',
			'mn.getMediaLibrary'	=> 'this:mn_getMediaLibrary',
			'mn.getPostFormats'     => 'this:mn_getPostFormats',
			'mn.getPostType'		=> 'this:mn_getPostType',
			'mn.getPostTypes'		=> 'this:mn_getPostTypes',
			'mn.getRevisions'		=> 'this:mn_getRevisions',
			'mn.restoreRevision'	=> 'this:mn_restoreRevision',
	 ...
	 * @return MN_User|bool MN_User object if authentication passed, false otherwise
	 */
	public function login( $username, $password ) {
		...
		if ( $this->auth_failed ) {
			$user = new MN_Error( 'login_prevented' );
		} else {
		...
		if ( is_mn_error( $user ) ) {
			...
			// Flag that authentication has failed once on this mn_xmlrpc_server instance
			 ...
			 * @param MN_User $user  MN_User object.
			 */
			$this->error = apply_filters( 'xmlrpc_login_error', $this->error, $user );
			return false;
		}

		mn_set_current_user( $user->ID );
		return $user;
	}
	 ...
	 *
	 * @since 1.5.0
	 * @deprecated 2.8.0 Use mn_xmlrpc_server::login()
	 * @see mn_xmlrpc_server::login()
	 *
	 * @param string $username User's username.
	 * @param string $password User's password.
	...
	public function set_custom_fields($post_id, $fields) {
		...
		foreach ( (array) $fields as $meta ) {
			if ( isset($meta['id']) ) {
				...
				if ( isset($meta['key']) ) {
					$meta['key'] = mn_unslash( $meta['key'] );
					if ( $meta['key'] !== $pmeta->meta_key )
						continue;
					$meta['value'] = mn_unslash( $meta['value'] );
					if ( current_user_can( 'edit_post_meta', $post_id, $meta['key'] ) )
						update_metadata_by_mid( 'post', $meta['id'], $meta['value'] );
				} elseif ( current_user_can( 'delete_post_meta', $post_id, $pmeta->meta_key ) ) {
					delete_metadata_by_mid( 'post', $meta['id'] );
				}
			} elseif ( current_user_can( 'add_post_meta', $post_id, mn_unslash( $meta['key'] ) ) ) {
	 ...
	 */
	public function mn_getUsersBlogs( $args ) {
		// If this isn't on MNMU then just use blogger_getUsersBlogs
		if ( !is_multisite() ) {
		 ...
		 * equal to the method's name, e.g., mn.getUsersBlogs, mn.newPost, etc.
		 *
		 * @since 2.5.0
		 *
		 * @param string $name The method name.
		 */
		do_action( 'xmlrpc_call', 'mn.getUsersBlogs' );
	...
	/**
	 * Convert a Mtaandao date string to an IXR_Date object.
	 *
	 * @access protected
	 *
	...
	/**
	 * Convert a Mtaandao GMT date string to an IXR_Date object.
	 *
	 * @access protected
	 *
	 * @param string $date_gmt Mtaandao GMT date string.
	...
	protected function _prepare_post( $post, $fields ) {
		...
		if ( $all_taxonomy_fields || in_array( 'terms', $fields ) ) {
			...
			$terms = mn_get_object_terms( $post['ID'], $post_type_taxonomies );
			$_post['terms'] = array();
	 ...
	 * @access protected
	 *
	 * @param MN_Post_Type $post_type Post type object.
	...
	protected function _prepare_media_item( $media_item, $thumbnail_size = 'thumbnail' ) {
		$_media_item = array(
			...
			'link'             => mn_get_attachment_url( $media_item->ID ),
			'title'            => $media_item->post_title,
			'caption'          => $media_item->post_excerpt,
			'description'      => $media_item->post_content,
			'metadata'         => mn_get_attachment_metadata( $media_item->ID ),
	...
	protected function _prepare_page( $page ) {
		...
		if ( is_object_in_taxonomy( 'page', 'category' ) ) {
			foreach ( mn_get_post_categories( $page->ID ) as $cat_id ) {
		...
		$_page = array(
			...
			'mn_slug'                => $page->post_name,
			'mn_password'            => $page->post_password,
			'mn_author'              => $author->display_name,
			'mn_page_parent_id'      => $page->post_parent,
			'mn_page_parent_title'   => $parent_title,
			'mn_page_order'          => $page->menu_order,
			'mn_author_id'           => (string) $author->ID,
			'mn_author_display_name' => $author->display_name,
			'date_created_gmt'       => $page_date_gmt,
			'custom_fields'          => $this->get_custom_fields( $page->ID ),
			'mn_page_template'       => $page_template
		);
	 ...
	 * @access protected
	 *
	 * @param MN_User $user   The unprepared user object.
	...
	protected function _prepare_user( $user, $fields ) {
		 ...
		 * @param MN_User $user   User object.
	 ...
	 *         Content struct for adding a new post. See mn_insert_post() for information on
	 ...
	 */
	public function mn_newPost( $args ) {
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.newPost' );
	...
	private function _toggle_sticky( $post_data, $update = false ) {
		...
		if ( 'private' === $post_data['post_status'] || ! empty( $post_data['post_password'] ) ) {
			...
			$sticky = mn_validate_boolean( $post_data['sticky'] );
			if ( $sticky ) {
	...
	/**
	 * Helper method for mn_newPost() and mn_editPost(), containing shared logic.
	 *
	 * @since 3.4.0
	 * @access protected
	 *
	 * @see mn_insert_post()
	 *
	 * @param MN_User         $user           The post author if post_author isn't set in $content_struct.
	...
	protected function _insert_post( $user, $content_struct ) {
		...
		$post_data = mn_parse_args( $content_struct, $defaults );
				...
				foreach ( $taxonomies as $taxonomy ) {
					...
					foreach ( $term_names as $term_name ) {
						...
						if ( ! $term ) {
							...
							// Create the new term.
							$term_info = mn_insert_term( $term_name, $taxonomy );
		 ...
		 */
		$post_data = apply_filters( 'xmlrpc_mn_insert_post_data', $post_data, $content_struct );

		$post_ID = $update ? mn_update_post( $post_data, true ) : mn_insert_post( $post_data, true );
	 ...
	 */
	public function mn_editPost( $args ) {
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.editPost' );
	 ...
	 */
	public function mn_deletePost( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.deletePost' );
	 ...
	 *  - 'enclosure'
	 */
	public function mn_getPost( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 */
			$fields = apply_filters( 'xmlrpc_default_post_fields', array( 'post', 'terms', 'custom_fields' ), 'mn.getPost' );
		}
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getPost' );
	 ...
	 * @since 3.4.0
	 *
	 * @see mn_get_recent_posts()
	 * @see mn_getPost() for more on `$fields`
	 ...
	 */
	public function mn_getPosts( $args ) {
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in res/class-mn-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_post_fields', array( 'post', 'terms', 'custom_fields' ), 'mn.getPosts' );
		}
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getPosts' );
		...
		$posts_list = mn_get_recent_posts( $query );
	 ...
	 */
	public function mn_newTerm( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.newTerm' );
		...
		$term = mn_insert_term( $term_data['name'] , $taxonomy['name'] , $term_data );
	 ...
	 * @return true|IXR_Error True on success, IXR_Error instance on failure.
	 */
	public function mn_editTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) )
			return $this->error;
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.editTerm' );
		...
		$term = mn_update_term( $term_id , $taxonomy['name'] , $term_data );
	 ...
	 */
	public function mn_deleteTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) )
			return $this->error;
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.deleteTerm' );
		...
		$result = mn_delete_term( $term_id, $taxonomy->name );
	 ...
	 *  - 'count'
	 */
	public function mn_getTerm( $args ) {
		if ( ! $this->minimum_args( $args, 5 ) )
			return $this->error;
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getTerm' );
	 ...
	 */
	public function mn_getTerms( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getTerms' );
	 ...
	 */
	public function mn_getTaxonomy( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 */
			$fields = apply_filters( 'xmlrpc_default_taxonomy_fields', array( 'labels', 'cap', 'object_type' ), 'mn.getTaxonomy' );
		}
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getTaxonomy' );
	 ...
	 */
	public function mn_getTaxonomies( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in res/class-mn-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_taxonomy_fields', array( 'labels', 'cap', 'object_type' ), 'mn.getTaxonomies' );
		}
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getTaxonomies' );
	 ...
	 *  - 'roles'
	 */
	public function mn_getUser( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 * @param string $method The method name.
			 */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'mn.getUser' );
		}
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getUser' );
	 ...
	 *
	 * @uses get_users()
	 * @see mn_getUser() for more on $fields and return values
	 *
	 * @param array  $args {
	 *     Method arguments. Note: arguments must be ordered as documented.
	 *
	 ...
	 */
	public function mn_getUsers( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in res/class-mn-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'mn.getUsers' );
		}
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getUsers' );
	 ...
	 *     @type array  $fields (optional)
	 * }
	 * @return array|IXR_Error (@see mn_getUser)
	 */
	public function mn_getProfile( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		if ( isset( $args[3] ) ) {
			...
			/** This action is documented in res/class-mn-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_user_fields', array( 'all' ), 'mn.getProfile' );
		}
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getProfile' );
	 ...
	 */
	public function mn_editProfile( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.editProfile' );
		...
		$result = mn_update_user( $user_data );
	 ...
	 */
	public function mn_getPage( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getPage' );
	 ...
	 * @return array|IXR_Error
	 */
	public function mn_getPages( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getPages' );
	 ...
	 * @since 2.2.0
	 *
	 * @see mn_xmlrpc_server::mw_newPost()
	 *
	 * @param array  $args {
	 *     Method arguments. Note: arguments must be ordered as documented.
	 *
	 ...
	 */
	public function mn_newPage( $args ) {
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.newPage' );
	 ...
	 */
	public function mn_deletePage( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.deletePage' );
		 ...
		 */
		do_action( 'xmlrpc_call_success_mn_deletePage', $page_id, $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function mn_editPage( $args ) {
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.editPage' );
	 ...
	 * @return array|IXR_Error
	 */
	public function mn_getPageList( $args ) {
		global $mndb;
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getPageList' );

		// Get list of pages ids and titles
		$page_list = $mndb->get_results("
			SELECT ID page_id,
	 ...
	 * @return array|IXR_Error
	 */
	public function mn_getAuthors( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getAuthors' );
	 ...
	 * @return array|IXR_Error
	 */
	public function mn_getTags( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getKeywords' );
	 ...
	 */
	public function mn_newCategory( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.newCategory' );
		...
		$cat_id = mn_insert_category($new_category, true);
		 ...
		 */
		do_action( 'xmlrpc_call_success_mn_newCategory', $cat_id, $args );
	 ...
	 * }
	 * @return bool|IXR_Error See mn_delete_term() for return info.
	 */
	public function mn_deleteCategory( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.deleteCategory' );

		if ( !current_user_can('manage_categories') )
			return new IXR_Error( 401, __( 'Sorry, you are not allowed to delete a category.' ) );

		$status = mn_delete_term( $category_id, 'category' );
		...
		if ( true == $status ) {
			 ...
			 */
			do_action( 'xmlrpc_call_success_mn_deleteCategory', $category_id, $args );
		}
	 ...
	 * @return array|IXR_Error
	 */
	public function mn_suggestCategories( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.suggestCategories' );
	 ...
	 * @return array|IXR_Error
	 */
	public function mn_getComment($args) {
		$this->escape($args);
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getComment' );
	 ...
	 * - 'offset' - Default is 0. See MN_Query::query() for more.
	 *
	 * @since 2.7.0
	 *
	 * @param array  $args {
	 ...
	 *     @type array  $struct
	 * }
	 * @return array|IXR_Error Contains a collection of comments. See mn_xmlrpc_server::mn_getComment() for a description of each item contents
	 */
	public function mn_getComments( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getComments' );
	 ...
	 * See mn_delete_comment() for more information on this behavior.
	 *
	 * @since 2.7.0
	 *
	 * @param array  $args {
	 ...
	 *     @type int    $comment_ID
	 * }
	 * @return bool|IXR_Error See mn_delete_comment().
	 */
	public function mn_deleteComment( $args ) {
		$this->escape($args);
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.deleteComment' );

		$status = mn_delete_comment( $comment_ID );
		...
		if ( $status ) {
			 ...
			 */
			do_action( 'xmlrpc_call_success_mn_deleteComment', $comment_ID, $args );
		}
	 ...
	 * @return true|IXR_Error True, on success.
	 */
	public function mn_editComment( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.editComment' );
		...
		$result = mn_update_comment($comment);
		if ( is_mn_error( $result ) )
			return new IXR_Error(500, $result->get_error_message());
		 ...
		 */
		do_action( 'xmlrpc_call_success_mn_editComment', $comment_ID, $args );
	 ...
	 *     @type array      $content_struct
	 * }
	 * @return int|IXR_Error See mn_new_comment().
	 */
	public function mn_newComment($args) {
		$this->escape($args);
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.newComment' );

		$comment_ID = mn_new_comment( $comment );
		 ...
		 */
		do_action( 'xmlrpc_call_success_mn_newComment', $comment_ID, $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function mn_getCommentStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getCommentStatusList' );
	 ...
	 * @return array|IXR_Error
	 */
	public function mn_getCommentCount( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getCommentCount' );

		$count = mn_count_comments( $post_id );
	 ...
	 * @return array|IXR_Error
	 */
	public function mn_getPostStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getPostStatusList' );
	 ...
	 * @return array|IXR_Error
	 */
	public function mn_getPageStatusList( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getPageStatusList' );
	 ...
	 * @return array|IXR_Error
	 */
	public function mn_getPageTemplates( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function mn_getOptions( $args ) {
		$this->escape( $args );
	 ...
	 * @return array|IXR_Error
	 */
	public function mn_setOptions( $args ) {
		$this->escape( $args );
		...
		foreach ( $options as $o_name => $o_value ) {
			...
			update_option( $this->blog_options[$o_name]['option'], mn_unslash( $o_value ) );
		}
	 ...
	 *  - 'metadata'
	 */
	public function mn_getMediaItem( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getMediaItem' );
	 ...
	 *     @type array  $struct
	 * }
	 * @return array|IXR_Error Contains a collection of media items. See mn_xmlrpc_server::mn_getMediaItem() for a description of each item contents
	 */
	public function mn_getMediaLibrary($args) {
		$this->escape($args);
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getMediaLibrary' );
	 ...
	 */
	public function mn_getPostFormats( $args ) {
		$this->escape( $args );
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getPostFormats' );
	 ...
	 *  - 'supports'
	 */
	public function mn_getPostType( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 * @param string $method The method name.
			 */
			$fields = apply_filters( 'xmlrpc_default_posttype_fields', array( 'labels', 'cap', 'taxonomies' ), 'mn.getPostType' );
		}
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getPostType' );
	 ...
	 * @return array|IXR_Error
	 */
	public function mn_getPostTypes( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			...
			/** This action is documented in res/class-mn-xmlrpc-server.php */
			$fields = apply_filters( 'xmlrpc_default_posttype_fields', array( 'labels', 'cap', 'taxonomies' ), 'mn.getPostTypes' );
		}
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getPostTypes' );
	 ...
	 */
	public function mn_getRevisions( $args ) {
		if ( ! $this->minimum_args( $args, 4 ) )
			return $this->error;
		...
		if ( isset( $args[4] ) ) {
			 ...
			 * @param string $method The method name.
			 */
			$fields = apply_filters( 'xmlrpc_default_revision_fields', array( 'post_date', 'post_date_gmt' ), 'mn.getRevisions' );
		}
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.getRevisions' );
		...
		if ( ! mn_revisions_enabled( $post ) )
			return new IXR_Error( 401, __( 'Sorry, revisions are disabled.' ) );

		$revisions = mn_get_post_revisions( $post_id );
		...
		foreach ( $revisions as $revision ) {
			...
			// Skip autosaves
			if ( mn_is_post_autosave( $revision ) )
				continue;
	 ...
	 */
	public function mn_restoreRevision( $args ) {
		if ( ! $this->minimum_args( $args, 3 ) )
			return $this->error;
		...
		/** This action is documented in res/class-mn-xmlrpc-server.php */
		do_action( 'xmlrpc_call', 'mn.restoreRevision' );

		if ( ! $revision = mn_get_post_revision( $revision_id ) )
			return new IXR_Error( 404, __( 'Invalid post ID.' ) );
		...
		$post = mn_restore_post_revision( $revision_id );
	...
	protected function _multisite_getUsersBlogs( $args ) {
		...
		$rpc->query('mn.getUsersBlogs', $args[1], $args[2]);
	...
	public function blogger_getPost( $args ) {
		...
		$categories = implode(',', mn_get_post_categories($post_ID));

		$content  = '<title>'.mn_unslash($post_data['post_title']).'</title>';
		$content .= '<category>'.$categories.'</category>';
		$content .= mn_unslash($post_data['post_content']);
	...
	public function blogger_getRecentPosts( $args ) {
		...
		foreach ($posts_list as $entry) {
			...
			$categories = implode(',', mn_get_post_categories($entry['ID']));

			$content  = '<title>'.mn_unslash($entry['post_title']).'</title>';
			$content .= '<category>'.$categories.'</category>';
			$content .= mn_unslash($entry['post_content']);
	...
	public function blogger_newPost( $args ) {
		...
		$post_ID = mn_insert_post($post_data);
		if ( is_mn_error( $post_ID ) )
			return new IXR_Error(500, $post_ID->get_error_message());
	...
	public function blogger_editPost( $args ) {
		...
		$result = mn_update_post( $postdata );
	...
	public function mw_newPost($args) {
		...
		if ( !empty( $content_struct['post_type'] ) ) {
			if ( $content_struct['post_type'] == 'page' ) {
				...
				$post_type = 'page';
				if ( !empty( $content_struct['mn_page_template'] ) )
					$page_template = $content_struct['mn_page_template'];
		...
		if ( isset( $content_struct['mn_post_format'] ) ) {
			$content_struct['mn_post_format'] = sanitize_key( $content_struct['mn_post_format'] );
			if ( !array_key_exists( $content_struct['mn_post_format'], get_post_format_strings() ) ) {
				return new IXR_Error( 404, __( 'Invalid post format' ) );
			}
		}

		// Let Mtaandao generate the post_name (slug) unless
		// one has been provided.
		$post_name = "";
		if ( isset($content_struct['mn_slug']) )
			$post_name = $content_struct['mn_slug'];

		// Only use a password if one was given.
		if ( isset($content_struct['mn_password']) )
			$post_password = $content_struct['mn_password'];

		// Only set a post parent if one was provided.
		if ( isset($content_struct['mn_page_parent_id']) )
			$post_parent = $content_struct['mn_page_parent_id'];

		// Only set the menu_order if it was provided.
		if ( isset($content_struct['mn_page_order']) )
			$menu_order = $content_struct['mn_page_order'];

		$post_author = $user->ID;

		// If an author id was provided then use it instead.
		if ( isset( $content_struct['mn_author_id'] ) && ( $user->ID != $content_struct['mn_author_id'] ) ) {
			switch ( $post_type ) {
				case "post":
			...
			$author = get_userdata( $content_struct['mn_author_id'] );
			if ( ! $author )
				return new IXR_Error( 404, __( 'Invalid author ID.' ) );
			$post_author = $content_struct['mn_author_id'];
		}
		...
		if ( isset ( $content_struct['mn_post_thumbnail'] ) ) {
			if ( set_post_thumbnail( $post_ID, $content_struct['mn_post_thumbnail'] ) === false )
				return new IXR_Error( 404, __( 'Invalid attachment ID.' ) );

			unset( $content_struct['mn_post_thumbnail'] );
		}
		...
		// in this function
		if ( isset( $content_struct['mn_post_format'] ) )
			set_post_format( $post_ID, $content_struct['mn_post_format'] );

		$post_ID = mn_insert_post( $postdata, true );
		if ( is_mn_error( $post_ID ) )
			return new IXR_Error(500, $post_ID->get_error_message());
	...
	public function attach_uploads( $post_ID, $post_content ) {
		...
		$attachments = $mndb->get_results( "SELECT ID, guid FROM {$mndb->posts} WHERE post_parent = '0' AND post_type = 'attachment'" );
		if ( is_array( $attachments ) ) {
			foreach ( $attachments as $file ) {
				if ( ! empty( $file->guid ) && strpos( $post_content, $file->guid ) !== false )
					$mndb->update($mndb->posts, array('post_parent' => $post_ID), array('ID' => $file->ID) );
			}
		}
	}
	...
	public function mw_editPost( $args ) {
		...
		// Use mn.editPost to edit post types other than post and page.
		...
		// Let Mtaandao manage slug if none was provided.
		...
		$page_template = null;
		if ( ! empty( $content_struct['mn_page_template'] ) && 'page' == $post_type )
			$page_template = $content_struct['mn_page_template'];

		$post_author = $postdata['post_author'];

		// Only set the post_author if one is set.
		if ( isset( $content_struct['mn_author_id'] ) ) {
			// Check permissions if attempting to switch author to or from another user.
			if ( $user->ID != $content_struct['mn_author_id'] || $user->ID != $post_author ) {
				switch ( $post_type ) {
					case 'post':
						if ( ! current_user_can( 'edit_others_posts' ) ) {
		...
		// We've got all the data -- post it.
		$nemnost = compact('ID', 'post_content', 'post_title', 'post_category', 'post_status', 'post_excerpt', 'comment_status', 'ping_status', 'edit_date', 'post_date', 'post_date_gmt', 'to_ping', 'post_name', 'post_password', 'post_parent', 'menu_order', 'post_author', 'tags_input', 'page_template');

		$result = mn_update_post($nemnost, true);
		if ( is_mn_error( $result ) )
			return new IXR_Error(500, $result->get_error_message());
		...
		if ( isset ( $content_struct['mn_post_thumbnail'] ) ) {
			...
			// Empty value deletes, non-empty value adds/updates.
			if ( empty( $content_struct['mn_post_thumbnail'] ) ) {
				delete_post_thumbnail( $post_ID );
			} else {
				if ( set_post_thumbnail( $post_ID, $content_struct['mn_post_thumbnail'] ) === false )
					return new IXR_Error( 404, __( 'Invalid attachment ID.' ) );
	...
	public function mw_getPost( $args ) {
		...
		if ($postdata['post_date'] != '') {
			...
			$categories = array();
			$catids = mn_get_post_categories($post_ID);
			foreach ($catids as $catid)
				$categories[] = get_cat_name($catid);

			$tagnames = array();
			$tags = mn_get_post_tags( $post_ID );
			if ( !empty( $tags ) ) {
				foreach ( $tags as $tag )
					$tagnames[] = $tag->name;
			...
			$resp = array(
				...
				'mn_more_text' => $post['more_text'],
				'mt_allow_comments' => $allow_comments,
				'mt_allow_pings' => $allow_pings,
				'mt_keywords' => $tagnames,
				'mn_slug' => $postdata['post_name'],
				'mn_password' => $postdata['post_password'],
				'mn_author_id' => (string) $author->ID,
				'mn_author_display_name' => $author->display_name,
				'date_created_gmt' => $post_date_gmt,
				'post_status' => $postdata['post_status'],
				'custom_fields' => $this->get_custom_fields($post_ID),
				'mn_post_format' => $post_format,
				'sticky' => $sticky,
			...
			$resp['mn_post_thumbnail'] = get_post_thumbnail_id( $postdata['ID'] );
	...
	public function mw_getRecentPosts( $args ) {
		...
		foreach ($posts_list as $entry) {
			...
			$categories = array();
			$catids = mn_get_post_categories($entry['ID']);
			foreach ( $catids as $catid )
				$categories[] = get_cat_name($catid);

			$tagnames = array();
			$tags = mn_get_post_tags( $entry['ID'] );
			if ( !empty( $tags ) ) {
				foreach ( $tags as $tag ) {
					$tagnames[] = $tag->name;
				}
			...
			$recent_posts[] = array(
				...
				'mt_allow_pings' => $allow_pings,
				'mt_keywords' => $tagnames,
				'mn_slug' => $entry['post_name'],
				'mn_password' => $entry['post_password'],
				'mn_author_id' => (string) $author->ID,
				'mn_author_display_name' => $author->display_name,
				'date_created_gmt' => $post_date_gmt,
				...
				'mn_post_thumbnail' => get_post_thumbnail_id( $entry['ID'] )
			);
		}
	 ...
	 *
	 * @link http://mycvs.org/archives/2004/06/30/file-upload-to-mtaandao-in-ecto/
	 *
	 * @since 1.5.0
	 *
	 * @global mndb $mndb Mtaandao database abstraction object.
	...
	public function mw_newMediaObject( $args ) {
		...
		$upload = mn_upload_bits($name, null, $bits);
		...
		// Save the data
		$id = mn_insert_attachment( $attachment, $upload[ 'file' ], $post_id );
		mn_update_attachment_metadata( $id, mn_generate_attachment_metadata( $id, $upload['file'] ) );
	...
	public function mt_getPostCategories( $args ) {
		...
		$categories = array();
		$catids = mn_get_post_categories(intval($post_ID));
	...
	public function mt_setPostCategories( $args ) {
		...
		mn_set_post_categories($post_ID, $catids);
	...
	public function mt_getTrackbackPings( $post_ID ) {
		...
		$comments = $mndb->get_results( $mndb->prepare("SELECT comment_author_url, comment_content, comment_author_IP, comment_type FROM $mndb->comments WHERE comment_post_ID = %d", $post_ID) );
	...
	public function mt_publishPost( $args ) {
		...
		// retain old cats
		$cats = mn_get_post_categories($post_ID);
		$postdata['post_category'] = $cats;
		$this->escape($postdata);

		return mn_update_post( $postdata );
	}
	...
	public function pingback_ping( $args ) {
		...
		if ( $post_ID = url_to_postid($pagelinkedto) ) {
			...
			if ( intval($urltest['fragment']) ) {
				...
				$sql = $mndb->prepare("SELECT ID FROM $mndb->posts WHERE post_title RLIKE %s", $title );
				if (! ($post_ID = $mndb->get_var($sql)) ) {
		...
		if ( $mndb->get_results( $mndb->prepare("SELECT * FROM $mndb->comments WHERE comment_post_ID = %d AND comment_author_url = %s", $post_ID, $pagelinkedfrom) ) )
		...
		/** This filter is documented in res/class-http.php */
		$user_agent = apply_filters( 'http_headers_useragent', 'Mtaandao/' . $mn_version . '; ' . get_bloginfo( 'url' ) );
		...
		$request = mn_safe_remote_get( $pagelinkedfrom, $http_api_args );
		$remote_source = $remote_source_original = mn_remote_retrieve_body( $request );
		...
		foreach ( $p as $para ) {
			if ( strpos($para, $pagelinkedto) !== false ) { // it exists, but is it a link?
				...
				$excerpt = preg_replace('|\</?mncontext\>|', '', $para);

				// prevent really long link text
				if ( strlen($context[1]) > 100 )
					$context[1] = substr($context[1], 0, 100) . '&#8230;';

				$marker = '<mncontext>'.$context[1].'</mncontext>';    // set up our marker
				$excerpt= str_replace($context[0], $marker, $excerpt); // swap out the link for our marker
				$excerpt = strip_tags($excerpt, '<mncontext>');        // strip all tags but our context marker
