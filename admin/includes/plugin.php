 ...
 *     Version: Must be set in the plugin for Mtaandao 2.3+
...
function _get_plugin_data_markup_translate( $plugin_file, $plugin_data, $markup = true, $translate = true ) {
	...
	$plugin_data['Name']        = mn_kses( $plugin_data['Name'],        $allowed_tags_in_links );
	$plugin_data['Author']      = mn_kses( $plugin_data['Author'],      $allowed_tags );

	$plugin_data['Description'] = mn_kses( $plugin_data['Description'], $allowed_tags );
	$plugin_data['Version']     = mn_kses( $plugin_data['Version'],     $allowed_tags );
	...
	if ( $markup ) {
		...
		$plugin_data['Description'] = mntexturize( $plugin_data['Description'] );
 ...
 *
 * Mtaandao only supports plugin files in the base plugins directory
...
function get_plugins($plugin_folder = '') {
	...
	if ( ! $cache_plugins = mn_cache_get('plugins', 'plugins') )
		$cache_plugins = array();
	...
	foreach ( $plugin_files as $plugin_file ) {
		...
		$mn_plugins[plugin_basename( $plugin_file )] = $plugin_data;
	}

	uasort( $mn_plugins, '_sort_uname_callback' );

	$cache_plugins[ $plugin_folder ] = $mn_plugins;
	mn_cache_set('plugins', $cache_plugins, 'plugins');
 ...
 *
 * Mtaandao only includes mu-plugin files in the base mu-plugins directory (main/mu-plugins).
 *
 * @since 3.0.0
...
function get_mu_plugins() {
	...
	if ( ! is_dir( MNMU_PLUGIN_DIR ) )
		return $mn_plugins;
	if ( $plugins_dir = @ opendir( MNMU_PLUGIN_DIR ) ) {
		while ( ( $file = readdir( $plugins_dir ) ) !== false ) {
			if ( substr( $file, -4 ) == '.php' )
				$plugin_files[] = $file;
		}
	...
	foreach ( $plugin_files as $plugin_file ) {
		if ( !is_readable( MNMU_PLUGIN_DIR . "/$plugin_file" ) )
			continue;

		$plugin_data = get_plugin_data( MNMU_PLUGIN_DIR . "/$plugin_file", false, false ); //Do not apply markup/translate as it'll be cached.

		if ( empty ( $plugin_data['Name'] ) )
			$plugin_data['Name'] = $plugin_file;

		$mn_plugins[ $plugin_file ] = $plugin_data;
	}

	if ( isset( $mn_plugins['index.php'] ) && filesize( MNMU_PLUGIN_DIR . '/index.php') <= 30 ) // silence is golden
		unset( $mn_plugins['index.php'] );
...
/**
 * Returns drop-ins that Mtaandao uses.
 *
...
function _get_dropins() {
	$dropins = array(
		'advanced-cache.php' => array( __( 'Advanced caching plugin.'       ), 'MN_CACHE' ), // MN_CACHE
 ...
 * @return MN_Error|null MN_Error on invalid file or null on success.
 */
function activate_plugin( $plugin, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( ( $network_wide && ! isset( $current[ $plugin ] ) ) || ( ! $network_wide && ! in_array( $plugin, $current ) ) ) {
		...
		if ( ob_get_length() > 0 ) {
			$output = ob_get_clean();
			return new MN_Error('unexpected_output', __('The plugin generated unexpected output.'), $output);
		}
		ob_end_clean();
	}
 ...
 *
 * When MN_Error is returned, it does not mean that one of the plugins had
 ...
 * @return bool|MN_Error True when finished or MN_Error if there were errors during a plugin activation.
 */
function activate_plugins( $plugins, $redirect = '', $network_wide = false, $silent = false ) {
	...
	if ( !empty($errors) )
		return new MN_Error('plugins_invalid', __('One of the plugins is invalid.'), $errors);
 ...
 * @return bool|null|MN_Error True on success, false is $plugins is empty, MN_Error on failure.
...
function delete_plugins( $plugins, $deprecated = '' ) {
	...
	$url = mn_nonce_url('plugins.php?action=delete-selected&verify-delete=1&' . implode('&', $checked), 'bulk-plugins');
	...
	if ( is_mn_error($mn_filesystem->errors) && $mn_filesystem->errors->get_error_code() )
		return new MN_Error('fs_error', __('Filesystem error.'), $mn_filesystem->errors);

	// Get the base plugin folder.
	$plugins_dir = $mn_filesystem->mn_plugins_dir();
	if ( empty( $plugins_dir ) ) {
		return new MN_Error( 'fs_no_plugins_dir', __( 'Unable to locate Mtaandao plugin directory.' ) );
	}

	$plugins_dir = trailingslashit( $plugins_dir );

	$plugin_translations = mn_get_installed_translations( 'plugins' );
	...
	foreach ( $plugins as $plugin_file ) {
		...
		if ( strpos( $plugin_file, '/' ) && $this_plugin_dir != $plugins_dir ) { //base check on if plugin includes directory separator AND that it's not the root plugin folder
			$deleted = $mn_filesystem->delete( $this_plugin_dir, true );
		} else {
			$deleted = $mn_filesystem->delete( $plugins_dir . $plugin_file );
		}
		...
		if ( '.' !== $plugin_slug && ! empty( $plugin_translations[ $plugin_slug ] ) ) {
			...
			foreach ( $translations as $translation => $data ) {
				$mn_filesystem->delete( MN_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '.po' );
				$mn_filesystem->delete( MN_LANG_DIR . '/plugins/' . $plugin_slug . '-' . $translation . '.mo' );
			}
		}
	}
	...
	if ( ! empty($errors) )
		return new MN_Error('could_not_remove_plugin', sprintf(__('Could not fully remove the plugin(s) %s.'), implode(', ', $errors)) );
 ...
 * @return MN_Error|int 0 on success, MN_Error on failure.
 */
function validate_plugin($plugin) {
	...
	if ( ! isset($installed_plugins[$plugin]) )
		return new MN_Error('no_plugin_header', __('The plugin does not have a valid header.'));
	return 0;
}
 ...
 *                             * Pass 'none' to leave div.mn-menu-image empty so an icon can be added via CSS.
 ...
 * @global array $menu
 * @global array $_mn_real_parent_file
 * @global bool  $_mn_submenu_nopriv
 * @global array $_registered_pages
 * @global array $_parent_pages
 *
 * @param string   $parent_slug The slug name for the parent menu (or the file name of a standard Mtaandao admin page).
 * @param string   $page_title  The text to be displayed in the title tags of the page when the menu is selected.
 * @param string   $menu_title  The text to be used for the menu.
 * @param string   $capability  The capability required for this menu to be displayed to the user.
 * @param string   $menu_slug   The slug name to refer to this menu by (should be unique for this menu).
...
function add_submenu_page( $parent_slug, $page_title, $menu_title, $capability, $menu_slug, $function = '' ) {
	global $submenu, $menu, $_mn_real_parent_file, $_mn_submenu_nopriv,
		$_registered_pages, $_parent_pages;

	$menu_slug = plugin_basename( $menu_slug );
	$parent_slug = plugin_basename( $parent_slug);

	if ( isset( $_mn_real_parent_file[$parent_slug] ) )
		$parent_slug = $_mn_real_parent_file[$parent_slug];

	if ( !current_user_can( $capability ) ) {
		$_mn_submenu_nopriv[$parent_slug][$menu_slug] = true;
		return false;
	}
 ...
 * @global array $_mn_real_parent_file
 * @global array $_mn_menu_nopriv
 * @global array $_mn_submenu_nopriv
 */
function get_admin_page_parent( $parent = '' ) {
	global $parent_file, $menu, $submenu, $pagenow, $typenow,
		$plugin_page, $_mn_real_parent_file, $_mn_menu_nopriv, $_mn_submenu_nopriv;

	if ( !empty ( $parent ) && 'admin.php' != $parent ) {
		if ( isset( $_mn_real_parent_file[$parent] ) )
			$parent = $_mn_real_parent_file[$parent];
		return $parent;
	}
	...
	if ( $pagenow == 'admin.php' && isset( $plugin_page ) ) {
		foreach ( (array)$menu as $parent_menu ) {
			if ( $parent_menu[2] == $plugin_page ) {
				...
				if ( isset( $_mn_real_parent_file[$parent_file] ) )
					$parent_file = $_mn_real_parent_file[$parent_file];
				return $parent_file;
			}
		}
		if ( isset( $_mn_menu_nopriv[$plugin_page] ) ) {
			$parent_file = $plugin_page;
			if ( isset( $_mn_real_parent_file[$parent_file] ) )
					$parent_file = $_mn_real_parent_file[$parent_file];
			return $parent_file;
	...
	if ( isset( $plugin_page ) && isset( $_mn_submenu_nopriv[$pagenow][$plugin_page] ) ) {
		$parent_file = $pagenow;
		if ( isset( $_mn_real_parent_file[$parent_file] ) )
			$parent_file = $_mn_real_parent_file[$parent_file];
		return $parent_file;
...
function user_can_access_admin_page() {
	global $pagenow, $menu, $submenu, $_mn_menu_nopriv, $_mn_submenu_nopriv,
		$plugin_page, $_registered_pages;

	$parent = get_admin_page_parent();

	if ( !isset( $plugin_page ) && isset( $_mn_submenu_nopriv[$parent][$pagenow] ) )
		return false;

	if ( isset( $plugin_page ) ) {
		if ( isset( $_mn_submenu_nopriv[$parent][$plugin_page] ) )
			return false;
	...
	if ( empty( $parent) ) {
		if ( isset( $_mn_menu_nopriv[$pagenow] ) )
			return false;
		if ( isset( $_mn_submenu_nopriv[$pagenow][$pagenow] ) )
			return false;
		if ( isset( $plugin_page ) && isset( $_mn_submenu_nopriv[$pagenow][$plugin_page] ) )
			return false;
		if ( isset( $plugin_page ) && isset( $_mn_menu_nopriv[$plugin_page] ) )
			return false;
		foreach (array_keys( $_mn_submenu_nopriv ) as $key ) {
			if ( isset( $_mn_submenu_nopriv[$key][$pagenow] ) )
				return false;
			if ( isset( $plugin_page ) && isset( $_mn_submenu_nopriv[$key][$plugin_page] ) )
			return false;
		}
		return true;
	}

	if ( isset( $plugin_page ) && ( $plugin_page == $parent ) && isset( $_mn_menu_nopriv[$plugin_page] ) )
		return false;
...
function settings_fields($option_group) {
	...
	mn_nonce_field("$option_group-options");
}
 ...
 */
function mn_clean_plugins_cache( $clear_update_cache = true ) {
	if ( $clear_update_cache )
		delete_site_transient( 'update_plugins' );
	mn_cache_delete( 'plugins', 'plugins' );
}
