...
class WC_REST_Customers_Controller extends WC_REST_Controller {
	...
	public function register_routes() {
		register_rest_route( $this->namespace, '/' . $this->rest_base, array(
			...
			array(
				...
				'args'                => array_merge( $this->get_endpoint_args_for_item_schema( MN_REST_Server::CREATABLE ), array(
					'email' => array(
						'required' => true,
					),
					'username' => array(
	...
	public function get_items( $request ) {
		...
		/**
		 * Filter arguments, before passing to MN_User_Query, when querying users via the REST API.
		 *
		 * @see https://developer.wordpress.org/reference/classes/mn_user_query/
		 *
		 * @param array           $prepared_args Array of arguments for MN_User_Query.
		...
		$query = new MN_User_Query( $prepared_args );
		...
		if ( $total_users < 1 ) {
			...
			$count_query = new MN_User_Query( $prepared_args );
			$total_users = $count_query->get_total();
		}
		$response->header( 'X-MN-Total', (int) $total_users );
	...
	public function create_item( $request ) {
		if ( ! empty( $request['id'] ) ) {
			return new MN_Error( 'woocommerce_rest_customer_exists', __( 'Cannot create existing resource.', 'woocommerce' ), array( 'status' => 400 ) );
		}
		 ...
		 *
		 * @param MN_User         $customer  Data used to create the customer.
	...
	public function get_item( $request ) {
		...
		if ( empty( $id ) || empty( $customer->ID ) ) {
			return new MN_Error( 'woocommerce_rest_invalid_id', __( 'Invalid resource id.', 'woocommerce' ), array( 'status' => 404 ) );
		}
	...
	public function update_item( $request ) {
		...
		if ( ! $customer ) {
			return new MN_Error( 'woocommerce_rest_invalid_id', __( 'Invalid resource id.', 'woocommerce' ), array( 'status' => 400 ) );
		}

		if ( ! empty( $request['email'] ) && email_exists( $request['email'] ) && $request['email'] !== $customer->user_email ) {
			return new MN_Error( 'woocommerce_rest_customer_invalid_email', __( 'Email address is invalid.', 'woocommerce' ), array( 'status' => 400 ) );
		}

		if ( ! empty( $request['username'] ) && $request['username'] !== $customer->user_login ) {
			return new MN_Error( 'woocommerce_rest_customer_invalid_argument', __( "Username isn't editable.", 'woocommerce' ), array( 'status' => 400 ) );
		}

		// Customer email.
		if ( isset( $request['email'] ) ) {
			mn_update_user( array( 'ID' => $customer->ID, 'user_email' => sanitize_email( $request['email'] ) ) );
		}

		// Customer password.
		if ( isset( $request['password'] ) ) {
			mn_update_user( array( 'ID' => $customer->ID, 'user_pass' => wc_clean( $request['password'] ) ) );
		}
	...
	public function delete_item( $request ) {
		...
		if ( ! $force ) {
			return new MN_Error( 'woocommerce_rest_trash_not_supported', __( 'Customers do not support trashing.', 'woocommerce' ), array( 'status' => 501 ) );
		}
		...
		if ( ! empty( $reassign ) ) {
			if ( $reassign === $id || ! get_userdata( $reassign ) ) {
				return new MN_Error( 'woocommerce_rest_customer_invalid_reassign', __( 'Invalid resource id for reassignment.', 'woocommerce' ), array( 'status' => 400 ) );
			}
		}
		...
		/** Include admin customer functions to get access to mn_delete_user() */
		require_once ABSPATH . 'admin/includes/user.php';

		$result = mn_delete_user( $id, $reassign );

		if ( ! $result ) {
			return new MN_Error( 'woocommerce_rest_cannot_delete', __( 'The resource cannot be deleted.', 'woocommerce' ), array( 'status' => 500 ) );
		}

		/**
		 * Fires after a customer is deleted via the REST API.
		 *
		 * @param MN_User          $customer The customer data.
	 ...
	 *
	 * @param MN_User $customer Customer object.
	 * @param MN_REST_Request $request Request object.
	 * @return MN_REST_Response $response Response data.
	 */
	public function prepare_item_for_response( $customer, $request ) {
		 ...
		 * @param MN_User          $customer  User object used to create response.
		 * @param MN_REST_Request  $request   Request object.
		 */
	...
	protected function get_role_names() {
		...
		return array_keys( $mn_roles->role_names );
	}
