...
class RM_Posts_Edit_Access_User {
    
    const CONTENT_EDIT_ACCESS = 'rm_content_edit_access';
    const TRANSIENT_EXPIRE = 15;
    private $lib = null;    // RM_Lib_Mn class instance
    private $pea = null;    // RM_Posts_Edit_Access class instance
    private $user_meta = null;    // user meta keys
    private $attachments_list = null;       
    private $roles_data = null;
    
    
    public function __construct(RM_Posts_Edit_Access $pea) {
        
        $this->lib = RM_Lib_Mn::get_instance();  
        $this->pea = $pea;
        $this->user_meta = new RM_Posts_Edit_Access_User_Meta();
                
        add_action('edit_user_profile', array($this, 'show_profile'), 10, 2);
        add_action('profile_update', array($this, 'save_user_restrictions'), 10);
        
        new RM_Posts_Edit_Access_Role();
        new RM_Posts_Edit_Access_Bulk_Action();
    ...
    public function is_restriction_applicable() {
        ...
        $show_full_list = apply_filters('rm_posts_show_full_list', false);
    ...
    public function show_profile($user) {
        ...
        if (!current_user_can('rm_edit_posts_access')) {
            return;
        }
    ...
    private function update_posts_list($user_id) {
        ...
        if (!empty($_POST['rm_posts_list'])) {
            $posts_list = explode(',', trim($_POST['rm_posts_list']));
            if (count($posts_list)>0) {
                $posts_list_str = RM_Utils::filter_int_array_to_str($posts_list);
    ...
    private function update_categories_list($user_id) {
        ...
        if (!empty($_POST['rm_categories_list'])) {
            $categories_list = explode(',', trim($_POST['rm_categories_list']));
            if (count($categories_list)>0) {
                $categories_list_str = RM_Utils::filter_int_array_to_str($categories_list);
    ...
    private function update_authors_list($user_id) {
        ...
        if (!empty($_POST['rm_post_authors_list'])) {
            $authors_list = explode(',', trim($_POST['rm_post_authors_list']));
            if (count($authors_list)>0) {
                $post_authors_list_str = RM_Utils::filter_int_array_to_str($authors_list);
    ...
    public function save_user_restrictions($user_id) {
        ...
        if (!isset($_POST['rm_posts_list'])) {
            // 'profile_update' action was fired not from the 'user profile' page, so there is no data to update
            return;
        }
        
        if (!current_user_can('edit_users', $user_id) || !current_user_can('rm_edit_posts_access')) {
            // not enough permissions for this action
            return;
        }
        
        $restriction_type = $_POST['rm_posts_restriction_type'];
        ...
        $own_data_only = $this->lib->get_request_var('rm_own_data_only', 'post', 'int');
        ...
        do_action('rm_save_user_edit_content_restrictions', $user_id);
    ...
    private function get_restricted_items_from_roles($item_id) {
        ...
        foreach($items as $item) {              
            $data0 = RM_Utils::filter_int_array($item);
    ...
    public function get_post_categories_list() {
        ...
        $value = RM_Utils::concat_with_comma($value, $value1);   
        $value = RM_Utils::validate_int_values_unique($value);
    ...
    public function get_post_authors_list() {
        ...
        $value = RM_Utils::concat_with_comma($value, $value2);
        $value = RM_Utils::validate_int_values_unique($value);
    ...
    private function add_related_wc_orders($posts) {
        ...
        //$shop_order_post_types = wc_get_order_types();
        $orders = array();
        foreach($posts as $post) {
            if ($post->post_type==='product') {
                $query = "SELECT order_id FROM mn_woocommerce_order_items 
                            WHERE order_item_id in (
                                SELECT order_item_id FROM mn_woocommerce_order_itemmeta 
    ...
    private function get_posts_list_by_authors() {
        ...
        if (RM_Plugin_Presence::is_active('woocommerce')) {
            // Add WooCommerce orders linked to the products with these authors/owners
            $add_orders = apply_filters('rm_edit_posts_access_add_orders_by_product_owner', true);
            if ($add_orders) {
    ...
    private function add_child_pages($posts_list) {
        ...
        $auto_access = apply_filters('rm_auto_access_child_pages', true);
    ...
    public function get_posts_list() {
        ...
        $posts_list1 = RM_Utils::filter_int_array_from_str($posts_list_str);        
        ...
        $posts_list = RM_Utils::filter_int_array($posts_list);
        for ($i=0; $i<count($posts_list); $i++) {
        ...
        if ($restriction_type==1) { // for 'Allow' only, do not exclude anything if it's a blocked/prohibited list
            $show_posts_which_can_edit_only = apply_filters('rm_show_posts_which_can_edit_only_backend',  true);
...
// end of RM_Posts_Edit_Access_User class