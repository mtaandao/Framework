 ...
 * @param  array $args
 *
 * @return WC_Order|MN_Error WC_Order on success, MN_Error on failure.
 */
function wc_create_order( $args = array() ) {
	...
	if ( $args['status'] ) {
		if ( ! in_array( 'wc-' . $args['status'], array_keys( wc_get_order_statuses() ) ) ) {
			return new MN_Error( 'woocommerce_invalid_order_status', __( 'Invalid order status', 'woocommerce' ) );
		}
	...
	if ( $updating ) {
		$order_id = mn_update_post( $order_data );
	} else {
		$order_id = mn_insert_post( apply_filters( 'woocommerce_new_order_data', $order_data ), true );
	}

	if ( is_mn_error( $order_id ) ) {
		return $order_id;
	}
...
function wc_update_order( $args ) {
	if ( ! $args['order_id'] ) {
		return new MN_Error( __( 'Invalid order ID', 'woocommerce' ) );
	}
	...
	return array_unique(
		apply_filters( 'woocommerce_currencies',
			array(
				...
				'BMN' => __( 'Botswana pula', 'woocommerce' ),
...
function wc_print_js() {
	...
	if ( ! empty( $wc_queued_js ) ) {
		// Sanitize.
		$wc_queued_js = mn_check_invalid_utf8( $wc_queued_js );
...
/**
 * Set a cookie - wrapper for setcookie using MN constants.
 *
...
function wc_setcookie( $name, $value, $expire = 0, $secure = false ) {
	...
	} elseif ( defined( 'MN_DEBUG' ) && MN_DEBUG ) {
...
function wc_get_log_file_path( $handle ) {
	return trailingslashit( WC_LOG_DIR ) . $handle . '-' . sanitize_file_name( mn_hash( $handle ) ) . '.log';
}
...
function wc_fix_rewrite_rules_init() {
	...
	if ( ! empty( $permalinks['use_verbose_page_rules'] ) ) {
		$GLOBALS['mn_rewrite']->use_verbose_page_rules = true;
	}
}
...
function wc_fix_rewrite_rules( $rules ) {
	...
	if ( ! empty( $permalinks['use_verbose_page_rules'] ) ) {
		$page_rewrite_rules = $mn_rewrite->page_rewrite_rules();
...
// This function can be removed when MN 3.9.2 or greater is required.
...
function wc_get_shipping_method_count( $include_legacy = false ) {
	...
	if ( false === $method_count ) {
		$method_count = absint( $mndb->get_var( "SELECT COUNT(*) FROM {$mndb->prefix}woocommerce_shipping_zone_methods" ) );
