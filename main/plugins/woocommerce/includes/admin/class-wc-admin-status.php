...
class WC_Admin_Status {
	...
	public static function status_tools() {
		...
		if ( ! empty( $_GET['action'] ) && ! empty( $_REQUEST['_mnnonce'] ) && mn_verify_nonce( $_REQUEST['_mnnonce'], 'debug_action' ) ) {
			...
			switch ( $_GET['action'] ) {
				...
				case 'clear_expired_transients' :
					...
					$rows = $mndb->query( $mndb->prepare( $sql, $mndb->esc_like( '_transient_' ) . '%', $mndb->esc_like( '_transient_timeout_' ) . '%', time() ) );
					...
					$sql = "DELETE a, b FROM $mndb->options a, $mndb->options b
						...
						AND b.option_value < %d";
					$rows2 = $mndb->query( $mndb->prepare( $sql, $mndb->esc_like( '_site_transient_' ) . '%', $mndb->esc_like( '_site_transient_timeout_' ) . '%', time() ) );
				...
				case 'clear_sessions' :
					...
					$mndb->query( "TRUNCATE {$mndb->prefix}woocommerce_sessions" );
				...
				case 'delete_taxes' :
					...
					$mndb->query( "TRUNCATE TABLE {$mndb->prefix}woocommerce_tax_rates;" );
					$mndb->query( "TRUNCATE TABLE {$mndb->prefix}woocommerce_tax_rate_locations;" );
	...
	public static function get_tools() {
		$tools = array(
			...
			'clear_expired_transients' => array(
				...
				'desc'    => __( 'This tool will clear ALL expired transients from Mtaandao.', 'woocommerce' ),
			),
			'recount_terms' => array(
	...
	/**
	 * Retrieve metadata from a file. Based on MN Core's get_file_data function.
	 * @since  2.1.1
	 ...
	 * @param  object $theme MN_Theme object.
	...
	public static function get_latest_theme_version( $theme ) {
		...
		if ( is_object( $api ) && ! is_mn_error( $api ) ) {
		...
		// Check WooThemes Theme Version.
			...
			if ( false === ( $theme_version_data = get_transient( $theme_dir . '_version_data' ) ) ) {
				$theme_changelog = mn_safe_remote_get( 'http://dzv365zjfbd8v.cloudfront.net/changelogs/' . $theme_dir . '/changelog.txt' );
				$cl_lines  = explode( "\n", mn_remote_retrieve_body( $theme_changelog ) );
				if ( ! empty( $cl_lines ) ) {
