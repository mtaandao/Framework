...
add_action( 'mn_ajax_vc_media_editor_add_image', 'vc_media_editor_add_image' );
add_action( 'mn_ajax_vc_media_editor_preview_image', 'vc_media_editor_preview_image' );
...
function vc_media_editor_add_image() {
	...
	// default action is mn_handle_upload, which forces mn to check upload with is_uploaded_file()
	...
	foreach ( $ids as $key => $attachment_id ) {
		...
		if ( ! $new_attachment_id || is_mn_error( $new_attachment_id ) ) {
			continue;
		}
...
function vc_media_editor_preview_image() {
	...
	$attachment_details = mn_prepare_attachment_for_js( $attachment_id );

	if ( ! isset( $attachment_details['sizes'][ $preferred_size ] ) ) {
		$preferred_size = 'thumbnail';
	}

	$attachment_url = mn_get_attachment_image_src( $attachment_id, $preferred_size );
