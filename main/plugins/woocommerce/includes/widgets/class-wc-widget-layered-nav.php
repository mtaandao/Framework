...
class WC_Widget_Layered_Nav extends WC_Widget {
	...
	protected function layered_nav_dropdown( $terms, $taxonomy, $query_type ) {
		...
		if ( $taxonomy !== $this->get_current_taxonomy() ) {
			$term_counts          = $this->get_filtered_term_product_counts( mn_list_pluck( $terms, 'term_id' ), $taxonomy, $query_type );
			$_chosen_attributes   = WC_Query::get_layered_nav_chosen_attributes();
	...
	/**
	 * Count products within certain terms, taking the main MN query into consideration.
	...
	protected function get_filtered_term_product_counts( $term_ids, $taxonomy, $query_type ) {
		...
		// Generate query
		$query           = array();
		$query['select'] = "SELECT COUNT( DISTINCT {$mndb->posts}.ID ) as term_count, terms.term_id as term_count_id";
		$query['from']   = "FROM {$mndb->posts}";
		$query['join']   = "
			INNER JOIN {$mndb->term_relationships} AS term_relationships ON {$mndb->posts}.ID = term_relationships.object_id
			INNER JOIN {$mndb->term_taxonomy} AS term_taxonomy USING( term_taxonomy_id )
			INNER JOIN {$mndb->terms} AS terms USING( term_id )
			" . $tax_query_sql['join'] . $meta_query_sql['join'];

		$query['where']   = "
			WHERE {$mndb->posts}.post_type IN ( 'product' )
		...
		return mn_list_pluck( $results, 'term_count', 'term_count_id' );
	}
