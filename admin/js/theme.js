/* global _mnThemeSettings, confirm */
window.mn = window.mn || {};
...
// Setup app structure
_.extend( themes, { model: {}, view: {}, routes: {}, router: {}, template: mn.template });
...
themes.view.Appearance = mn.Backbone.View.extend({

	el: '#mnbody-content .wrap .theme-browser',
	...
	searchContainer: $( '#mnbody h1:first' ),
	...
	search: function() {
		...
		this.searchContainer
			.append( $.parseHTML( '<label class="screen-reader-text" for="mn-filter-search-input">' + l10n.search + '</label>' ) )
			.append( view.el );
	},
...
themes.Collection = Backbone.Collection.extend({
	...
	// Send request to api.mtaandao.co.ke/themes
	apiCall: function( request, paginated ) {
		return mn.ajax.send( 'query-themes', {
			data: {
			// Request data
				request: _.extend({
					per_page: 100,
...
themes.view.Theme = mn.Backbone.View.extend({
	...
	installTheme: function( event ) {
		...
		$( document ).on( 'mn-theme-install-success', function( event, response ) {
			if ( _this.model.get( 'id' ) === response.slug ) {
				_this.model.set( { 'installed': true } );
			}
		} );

		mn.updates.installTheme( {
	...
	updateTheme: function( event ) {
		...
		$( document ).on( 'mn-theme-update-success', function( event, response ) {
...
themes.view.Details = mn.Backbone.View.extend({
	...
	containFocus: function( $el ) {
		...
		$el.on( 'keydown.mn-themes', function( event ) {
	...
	deleteTheme: function( event ) {
		...
		if ( ! window.confirm( mn.themes.data.settings.confirmDelete ) ) {
			return;
		}

		mn.updates.maybeRequestFilesystemCredentials( event );

		$( document ).one( 'mn-theme-delete-success', function( event, response ) {
...
themes.view.Preview = themes.view.Details.extend({
	...
	className: 'mn-full-overlay expanded',
	...
	tooglePreviewDeviceButtons: function( newDevice ) {
		var $devices = $( '.mn-full-overlay-footer .devices' );
	...
	installTheme: function( event ) {
		...
		$( document ).on( 'mn-theme-install-success', function() {
			_this.model.set( { 'installed': true } );
		} );
...
themes.view.Themes = mn.Backbone.View.extend({

	className: 'themes mn-clearfix',
		...
		if ( this.model.get( 'hasUpdate' ) ) {
			...
			if ( $card.find( '.updating-message' ).length ) {
				...
				$modal.find( '.notice-warning' )
					...
					.find( 'p' ).text( mn.updates.l10n.updating );
	...
	announceSearchResults: function( count ) {
		if ( 0 === count ) {
			mn.a11y.speak( l10n.noThemesFound );
		} else {
			mn.a11y.speak( l10n.themesFound.replace( '%d', count ) );
		}
	}
});

// Search input view controller.
themes.view.Search = mn.Backbone.View.extend({

	tagName: 'input',
	className: 'mn-filter-search',
	id: 'mn-filter-search-input',
	searching: false,
...
themes.Run = {
	...
	routes: function() {
		...
		themes.router.on( 'route:search', function() {
			$( '.mn-filter-search' ).trigger( 'keyup' );
		});
...
themes.view.InstallerSearch =  themes.view.Search.extend({
	...
	doSearch: _.debounce( function( value ) {
		...
		// Get the themes by sending Ajax POST request to api.mtaandao.co.ke/themes
...
themes.view.Installer = themes.view.Appearance.extend({
	...
	events: {
		...
		'keyup #mnorg-username-input': 'saveUsername'
	},
	...
	// Save the user's mtaandao.co.ke username and get his favorite themes.
	saveUsername: function ( event ) {
		var username = $( '#mnorg-username-input' ).val(),
			nonce = $( '#mnorg-username-nonce' ).val(),
		...
		return mn.ajax.send( 'save-mnorg-username', {
			data: {
				_mnnonce: nonce,
				username: username
			},
	...
	// in new location
	searchContainer: $( '.mn-filter .search-form' ),
...
themes.RunInstaller = {
	...
	routes: function() {
		...
		// Bind to our global `mn.themes` object
		...
		themes.router.on( 'route:search', function() {
			$( '.mn-filter-search' ).focus().trigger( 'keyup' );
		});
...
$( document ).ready(function() {
	...
	$( '.broken-themes .delete-theme' ).on( 'click', function() {
		return confirm( _mnThemeSettings.settings.confirmDelete );
	});
});
