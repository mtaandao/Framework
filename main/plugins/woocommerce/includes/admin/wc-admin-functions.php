...
function wc_create_page( $slug, $option = '', $page_title = '', $page_content = '', $post_parent = 0 ) {
	...
	if ( strlen( $page_content ) > 0 ) {
		...
		$valid_page_found = $mndb->get_var( $mndb->prepare( "SELECT ID FROM $mndb->posts WHERE post_type='page' AND post_status NOT IN ( 'pending', 'trash', 'future', 'auto-draft' ) AND post_content LIKE %s LIMIT 1;", "%{$page_content}%" ) );
	} else {
		// Search for an existing page with the specified page slug
		$valid_page_found = $mndb->get_var( $mndb->prepare( "SELECT ID FROM $mndb->posts WHERE post_type='page' AND post_status NOT IN ( 'pending', 'trash', 'future', 'auto-draft' )  AND post_name = %s LIMIT 1;", $slug ) );
	}
	...
	if ( strlen( $page_content ) > 0 ) {
		// Search for an existing page with the specified page content (typically a shortcode)
		$trashed_page_found = $mndb->get_var( $mndb->prepare( "SELECT ID FROM $mndb->posts WHERE post_type='page' AND post_status = 'trash' AND post_content LIKE %s LIMIT 1;", "%{$page_content}%" ) );
	} else {
		// Search for an existing page with the specified page slug
		$trashed_page_found = $mndb->get_var( $mndb->prepare( "SELECT ID FROM $mndb->posts WHERE post_type='page' AND post_status = 'trash' AND post_name = %s LIMIT 1;", $slug ) );
	}
	...
	if ( $trashed_page_found ) {
		...
		$page_id = mn_insert_post( $page_data );
	}
...
function wc_save_order_items( $order_id, $items ) {
		...
		foreach ( $items['order_item_id'] as $item_id ) {
			...
			if ( isset( $items['order_item_name'][ $item_id ] ) ) {
				$mndb->update(
					...
					array( 'order_item_name' => wc_clean( mn_unslash( $items['order_item_name'][ $item_id ] ) ) ),
			...
			// Clear meta cache
			mn_cache_delete( $item_id, 'order_item_meta' );
		}
	}
	...
	foreach ( $meta_keys as $id => $meta_key ) {
		...
		if ( $meta_key === '' && $meta_value === '' ) {
			$mndb->query( $mndb->prepare( "DELETE FROM {$mndb->prefix}woocommerce_order_itemmeta WHERE meta_id = %d", $id ) );
		} else {

			$mndb->update(
				$mndb->prefix . 'woocommerce_order_itemmeta',
				array(
	...
	if ( isset( $items['shipping_method_id'] ) ) {
		...
		foreach ( $items['shipping_method_id'] as $item_id ) {
			...
			$method_title = isset( $items['shipping_method_title'][ $item_id ] ) ? wc_clean( mn_unslash( $items['shipping_method_title'][ $item_id ] ) ) : '';
