 ...
 *
 * Do not use for redirects, use {@see mn_get_referer()} instead.
 *
 * @since 2.6.1
 * @return string|false Referer URL on success, false on failure.
 */
function wc_get_raw_referer() {
	if ( function_exists( 'mn_get_raw_referer' ) ) {
		return mn_get_raw_referer();
	}
...
function wc_add_to_cart_message( $products, $show_qty = false ) {
	...
	if ( 'yes' === get_option( 'woocommerce_cart_redirect_after_add' ) ) {
		$return_to = apply_filters( 'woocommerce_continue_shopping_redirect', wc_get_raw_referer() ? mn_validate_redirect( wc_get_raw_referer(), false ) : wc_get_page_permalink( 'shop' ) );
...
function wc_clear_cart_after_payment() {
	...
	if ( ! empty( $mn->query_vars['order-received'] ) ) {

		$order_id  = absint( $mn->query_vars['order-received'] );
