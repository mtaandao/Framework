...
class Vc_Grid_Item_Editor extends Vc_Backend_Editor {
	...
	public function replaceTemplatesPanelEditorJsAction() {
		mn_dequeue_script( 'vc-template-preview-script' );
	...
	public function registerBackendJavascript() {
		...
		mn_register_script( 'vc_grid_item_editor', vc_asset_url( 'js/dist/grid-builder.min.js' ), array( 'vc-backend-min-js' ), WPB_VC_VERSION, true );
		mn_localize_script( 'vc_grid_item_editor', 'i18nLocaleGItem', array(
	...
	public function enqueueJs() {
		parent::enqueueJs();
		mn_enqueue_script( 'vc_grid_item_editor' );
	}
	...
	public function renderTemplatePreview() {
		vc_user_access()->checkAdminNonce()->validateDie()->mnAny( 'edit_posts', 'edit_pages' )->validateDie()->part( 'grid_builder' )->can()->validateDie();
