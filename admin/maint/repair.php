...
if ( ! defined( 'MN_ALLOW_REPAIR' ) ) {
	...
	$query = $mndb->prepare( "SHOW TABLES LIKE %s", $mndb->esc_like( $mndb->sitecategories ) );
	if ( is_multisite() && ! $mndb->get_var( $query ) ) {
	...
	foreach ( $tables as $table ) {
		$check = $mndb->get_row( "CHECK TABLE $table" );
		...
		if ( 'OK' == $check->Msg_text ) {
			...
			printf( __( 'The %1$s table is not okay. It is reporting the following error: %2$s. Mtaandao will attempt to repair this table&hellip;' ) , "<code>$table</code>", "<code>$check->Msg_text</code>" );

			$repair = $mndb->get_row( "REPAIR TABLE $table" );
		...
		if ( $okay && $optimize ) {
			$check = $mndb->get_row( "ANALYZE TABLE $table" );
			...
			if ( 'Table is already up to date' == $check->Msg_text )  {
				...
				$check = $mndb->get_row( "OPTIMIZE TABLE $table" );
	...
	if ( $problems ) {
		printf( '<p>' . __('Some database problems could not be repaired. Please copy-and-paste the following list of errors to the <a href="%s">Mtaandao support forums</a> to get additional assistance.') . '</p>', __( 'https://mtaandao.co.ke/support/forum/how-to-and-troubleshooting' ) );
		$problem_output = '';
	...
	echo '<h1 class="screen-reader-text">' . __( 'Mtaandao database repair' ) . '</h1>';

	if ( isset( $_GET['referrer'] ) && 'is_blog_installed' == $_GET['referrer'] )
		echo '<p>' . __( 'One or more database tables are unavailable. To allow Mtaandao to attempt to repair these tables, press the &#8220;Repair Database&#8221; button. Repairing can take a while, so please be patient.' ) . '</p>';
	else
		echo '<p>' . __( 'Mtaandao can automatically look for some common database problems and repair them. Repairing can take a while, so please be patient.' ) . '</p>';
?>
	<p class="step"><a class="button button-large" href="repair.php?repair=1"><?php _e( 'Repair Database' ); ?></a></p>
	<p><?php _e( 'Mtaandao can also attempt to optimize the database. This improves performance in some situations. Repairing and optimizing the database can take a long time and the database will be locked while optimizing.' ); ?></p>
