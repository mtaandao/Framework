...
if ( is_multisite() && ! is_network_admin() ) {
	mn_redirect( network_admin_url( 'theme-editor.php' ) );
	exit();
}

if ( !current_user_can('edit_themes') )
	mn_die('<p>'.__('Sorry, you are not allowed to edit templates for this site.').'</p>');
...
if ( ! $theme->exists() ) {
	mn_die( __( 'The requested theme does not exist.' ) );
}

if ( $theme->errors() && 'theme_no_stylesheet' == $theme->errors()->get_error_code() ) {
	mn_die( __( 'The requested theme does not exist.' ) . ' ' . $theme->errors()->get_error_message() );
}
 ...
 * @param MN_Theme $theme         The current Theme object.
 */
$file_types = apply_filters( 'mn_theme_editor_filetypes', $default_types, $theme );
...
<?php
foreach ( mn_get_themes( array( 'errors' => null ) ) as $a_stylesheet => $a_theme ) {
...
<?php if ( $error ) :
	...
	<?php mn_nonce_field( 'edit-theme_' . $file . $stylesheet ); ?>
