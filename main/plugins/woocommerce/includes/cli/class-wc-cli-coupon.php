...
class WC_CLI_Coupon extends WC_CLI_Command {
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc coupon create --code=new-coupon --type=percent
	 *
	 */
	public function create( $__, $assoc_args ) {
		...
		try {
			...
			$coupon_data = mn_parse_args( $assoc_args, $defaults );
			...
			$id = mn_insert_post( $new_coupon, $mn_error = false );
			...
			if ( $porcelain ) {
				...
				MN_CLI::success( "Created coupon $id." );
			}
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc coupon delete 123
	 *
	 *     mn wc coupon delete $(mn wc coupon list --format=ids)
	 *
	 */
	public function delete( $args, $assoc_args ) {
		...
		foreach ( $this->get_many_coupons_from_ids_or_codes( $args, true ) as $coupon ) {
			...
			$r = mn_delete_post( $coupon->id, true );

			if ( $r ) {
				MN_CLI::success( "Deleted coupon {$coupon->id}." );
			} else {
				$exit_code += 1;
				MN_CLI::warning( "Failed deleting coupon {$coupon->id}." );
			}
		}
		exit( $exit_code ? 1 : 0 );
	}
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc coupon get 123 --field=discount_type
	 *
	 *     mn wc coupon get disc50 --format=json > disc50.json
	 *
	 * @since 2.5.0
	 */
	 ...
	 *     mn wc coupon list
	 *
	 *     mn wc coupon list --field=id
	 *
	 *     mn wc coupon list --fields=id,code,type --format=json
	 *
	 * @since      2.5.0
	 * @subcommand list
	 */
	public function list_( $__, $assoc_args ) {
		$query_args = $this->merge_mn_query_args( $this->get_list_query_args(), $assoc_args );
		$formatter  = $this->get_formatter( $assoc_args );
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc coupon update 123 --amount=5
	 *
	 *     mn wc coupon update coupon-code --code=new-coupon-code
	 *
	 * @since 2.5.0
	 */
	public function update( $args, $assoc_args ) {
		try {
			...
			$id = mn_update_post( array( 'ID' => intval( $id ), 'post_title' => $coupon_code, 'post_excerpt' => isset( $data['description'] ) ? $data['description'] : '' ) );
			if ( 0 === $id ) {
			...
			MN_CLI::success( "Updated coupon $id." );
		} catch ( WC_CLI_Exception $e ) {
			MN_CLI::error( $e->getMessage() );
		}
	}
	...
	protected function get_coupon_from_id_or_code( $coupon_id_or_code, $display_warning = false ) {
		...
		$code = $mndb->get_var( $mndb->prepare( "SELECT post_title FROM $mndb->posts WHERE (id = %s OR post_title = %s) AND post_type = 'shop_coupon' AND post_status = 'publish' LIMIT 1", $coupon_id_or_code, $coupon_id_or_code ) );
		if ( ! $code ) {
			if ( $display_warning ) {
				MN_CLI::warning( "Invalid coupon ID or code $coupon_id_or_code" );
			}
			return null;
		}
