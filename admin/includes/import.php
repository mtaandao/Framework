<?php
/**
 * Mtaandao Administration Importer API.
 *
 * @package Mtaandao
...
function get_importers() {
	global $mn_importers;
	if ( is_array( $mn_importers ) ) {
		uasort( $mn_importers, '_usort_by_first_member' );
	}
	return $mn_importers;
}
...
/**
 * Register importer for Mtaandao.
 *
 * @since 2.0.0
 *
 * @global array $mn_importers
 ...
 * @return MN_Error Returns MN_Error when $callback is MN_Error.
 */
function register_importer( $id, $name, $description, $callback ) {
	global $mn_importers;
	if ( is_mn_error( $callback ) )
		return $callback;
	$mn_importers[$id] = array ( $name, $description, $callback );
}
 ...
 */
function mn_import_cleanup( $id ) {
	mn_delete_attachment( $id );
}
 ...
 */
function mn_import_handle_upload() {
	...
	$upload = mn_handle_upload( $_FILES['import'], $overrides );
	...
	// Save the data
	$id = mn_insert_attachment( $object, $upload['file'] );

	/*
	 * Schedule a cleanup for one day from now in case of failed
	 * import or missing mn_import_cleanup() call.
	 */
	mn_schedule_single_event( time() + DAY_IN_SECONDS, 'importer_scheduled_cleanup', array( $id ) );
...
function mn_get_popular_importers() {
	...
	if ( ! $popular_importers ) {
		...
		$popular_importers = json_decode( mn_remote_retrieve_body( $response ), true );
	...
	return array(
		...
		'mncat2tag' => array(
			...
			'plugin-slug' => 'mncat2tag-importer',
			'importer-id' => 'mn-cat2tag',
		),
		'livejournal' => array(
			'name' => __( 'LiveJournal' ),
