<?php
/**
 * Mtaandao Theme Install Administration API
 *
 * @package Mtaandao
...
function install_themes_feature_list() {
	...
	if ( !$cache = get_transient( 'mnorg_theme_feature_list' ) )
		set_transient( 'mnorg_theme_feature_list', array(), 3 * HOUR_IN_SECONDS );
	...
	set_transient( 'mnorg_theme_feature_list', $feature_list, 3 * HOUR_IN_SECONDS );
...
<form method="post" enctype="multipart/form-data" class="mn-upload-form" action="<?php echo self_admin_url('update.php?action=upload-theme'); ?>">
	<?php mn_nonce_field( 'theme-upload' ); ?>
 ...
 * @deprecated 3.4.0
 *
 * @global MN_Theme_Install_List_Table $mn_list_table
 *
 * @param object $theme
 */
function display_theme( $theme ) {
	...
	if ( ! isset( $mn_list_table ) ) {
		$mn_list_table = _get_list_table('MN_Theme_Install_List_Table');
	}
	$mn_list_table->prepare_items();
	$mn_list_table->single_row( $theme );
}
...
function install_theme_information() {
	...
	$theme = themes_api( 'theme_information', array( 'slug' => mn_unslash( $_REQUEST['theme'] ) ) );

	if ( is_mn_error( $theme ) )
		mn_die( $theme );
	...
	$mn_list_table->theme_installer_single( $theme );
	iframe_footer();
	exit;
}
