...
class Vc_Updating_Manager {
	...
	/**
	 * Initialize a new instance of the Mtaandao Auto-Update class
	 *
	...
	public function getRemote_version() {
		$request = mn_remote_get( $this->update_path );
		if ( ! is_mn_error( $request ) || mn_remote_retrieve_response_code( $request ) === 200 ) {
			return $request['body'];
		}
	...
	public function getRemote_information() {
		$request = mn_remote_get( $this->update_path.'information.json' );
		if ( ! is_mn_error( $request ) || mn_remote_retrieve_response_code( $request ) === 200 ) {
	...
	public function addUpgradeMessageLink() {
		...
		if ( ! $is_activated ) {
			...
			echo sprintf( ' ' . __( 'To receive automatic updates license activation is required. Please visit %s to activate your Visual Composer.', 'js_composer' ), $redirect ) . sprintf( ' <a href="http://go.mtaandao.co.ke/plugins/page-builder/faq-update-in-theme" target="_blank">%s</a>', __( 'Got Visual Composer in theme?', 'js_composer' ) );
		}
	}
}
