...
class WC_Legacy_API {
	...
	public function handle_rest_api_requests() {
		...
		if ( ! empty( $_GET['wc-api-version'] ) ) {
			$mn->query_vars['wc-api-version'] = $_GET['wc-api-version'];
		}

		if ( ! empty( $_GET['wc-api-route'] ) ) {
			$mn->query_vars['wc-api-route'] = $_GET['wc-api-route'];
		}

		// REST API request.
		if ( ! empty( $mn->query_vars['wc-api-version'] ) && ! empty( $mn->query_vars['wc-api-route'] ) ) {

			define( 'WC_API_REQUEST', true );
			define( 'WC_API_REQUEST_VERSION', absint( $mn->query_vars['wc-api-version'] ) );
			...
			if ( 1 === WC_API_REQUEST_VERSION ) {
				...
				$this->server = new WC_API_Server( $mn->query_vars['wc-api-route'] );
	...
	private function handle_v1_rest_api_request() {
		...
		$this->server = new WC_API_Server( $GLOBALS['mn']->query_vars['wc-api-route'] );
