<?php
/**
 * REST API: MN_REST_Request class
 *
 * @package Mtaandao
 * @subpackage REST_API
 * @since 4.4.0
 ...
 *
 * Note: When using features provided by ArrayAccess, be aware that Mtaandao deliberately
 ...
 * MN_REST_Request::get_body_params(), MN_REST_Request::get_url_params(), etc.
 *
 * @since 4.4.0
 *
 * @see ArrayAccess
 */
class MN_REST_Request implements ArrayAccess {
	...
	protected function get_parameter_order() {
		 ...
		 * }
		 * @param MN_REST_Request $this The request object.
		 */
	...
	protected function parse_json_params() {
		 ...
		 *
		 * Note that due to MN's JSON compatibility functions, json_last_error
		 * might not be defined: https://core.trac.mtaandao.co.ke/ticket/27799
		 */
	...
	protected function parse_body_params() {
		 ...
		 *
		 * NOTE: Do not refactor to use `mn_unslash`.
		 */
	 ...
	 * @access public
	 *
	 * @return bool|MN_Error True if there are no parameters to validate or if all pass validation,
	 *                       MN_Error if required parameters are missing.
	 */
	public function has_valid_params() {
		...
		if ( ! empty( $required ) ) {
			return new MN_Error( 'rest_missing_callback_param', sprintf( __( 'Missing parameter(s): %s' ), implode( ', ', $required ) ), array( 'status' => 400, 'params' => $required ) );
		}
		...
		if ( $invalid_params ) {
			return new MN_Error( 'rest_invalid_param', sprintf( __( 'Invalid parameter(s): %s' ), implode( ', ', array_keys( $invalid_params ) ) ), array( 'status' => 400, 'params' => $invalid_params ) );
		}
	...
	/**
	 * Retrieves a MN_REST_Request object from a full URL.
	 *
	 * @static
	 * @since 4.5.0
	 * @access public
	 ...
	 * @return MN_REST_Request|false MN_REST_Request object on success, false on failure.
	 */
	public static function from_url( $url ) {
		...
		if ( ! empty( $bits['query'] ) ) {
			mn_parse_str( $bits['query'], $query_params );
		}
		...
		$request = false;
		if ( ! empty( $route ) ) {
			$request = new MN_REST_Request( 'GET', $route );
		 ...
		 * @since 4.5.0
		 *
		 * @param MN_REST_Request|false $request Generated request object, or false if URL
		 *                                       could not be parsed.
