...
class WC_API_Orders extends WC_API_Resource {
	...
	public function get_order( $id, $fields = null, $filter = array() ) {
		...
		foreach ( $order->get_items( 'coupon' ) as $coupon_item_id => $coupon_item ) {
			...
			if ( in_array( 'coupons', $expand ) ) {
				...
				if ( ! is_mn_error( $_coupon_data ) && isset( $_coupon_data['coupon'] ) ) {
	 ...
	 * @return MN_Error|array error or created note response data
	 */
	 ...
	 * @return MN_Error|array error or edited note response data
	 */
	 ...
	 * @param string $order_id order ID
	 * @param string $id note ID
	 * @return MN_Error|array error or deleted message
	 */
	 ...
	 * @return MN_Error|array error or created refund response data
	 */
	 ...
	 * @param array $data parsed request data
	 * @return MN_Error|array error or edited refund response data
	 */
	public function edit_order_refund( $order_id, $id, $data ) {
		try {
			...
			if ( isset( $data['reason'] ) ) {
				$updated_refund = mn_update_post( array( 'ID' => $refund->ID, 'post_excerpt' => $data['reason'] ) );

				if ( is_mn_error( $updated_refund ) ) {
