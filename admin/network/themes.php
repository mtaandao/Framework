...
if ( !current_user_can('manage_network_themes') )
	mn_die( __( 'Sorry, you are not allowed to manage network themes.' ) );
...
$referer = remove_query_arg( $temp_args, mn_get_referer() );

if ( $action ) {
	switch ( $action ) {
		case 'enable':
			check_admin_referer('enable-theme_' . $_GET['theme']);
			MN_Theme::network_enable_theme( $_GET['theme'] );
			if ( false === strpos( $referer, '/network/themes.php' ) )
				mn_redirect( network_admin_url( 'themes.php?enabled=1' ) );
			else
				mn_safe_redirect( add_query_arg( 'enabled', 1, $referer ) );
			exit;
		case 'disable':
			check_admin_referer('disable-theme_' . $_GET['theme']);
			MN_Theme::network_disable_theme( $_GET['theme'] );
			mn_safe_redirect( add_query_arg( 'disabled', '1', $referer ) );
			exit;
		case 'enable-selected':
			check_admin_referer('bulk-themes');
			$themes = isset( $_POST['checked'] ) ? (array) $_POST['checked'] : array();
			if ( empty($themes) ) {
				mn_safe_redirect( add_query_arg( 'error', 'none', $referer ) );
				exit;
			}
			MN_Theme::network_enable_theme( (array) $themes );
			mn_safe_redirect( add_query_arg( 'enabled', count( $themes ), $referer ) );
			exit;
		case 'disable-selected':
			...
			MN_Theme::network_disable_theme( (array) $themes );
			mn_safe_redirect( add_query_arg( 'disabled', count( $themes ), $referer ) );
			exit;
		...
		case 'delete-selected':
			if ( ! current_user_can( 'delete_themes' ) ) {
				mn_die( __('Sorry, you are not allowed to delete themes for this site.') );
			}
			...
			if ( empty( $themes ) ) {
				mn_safe_redirect( add_query_arg( 'error', 'main', $referer ) );
				exit;
			}

			$theme_info = array();
			foreach ( $themes as $key => $theme ) {
				$theme_info[ $theme ] = mn_get_theme( $theme );
			}
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="https://mtaandao.co.ke/docs/Network_Admin_Themes_Screen" target="_blank">Documentation on Network Themes</a>') . '</p>' .
...
mn_enqueue_script( 'theme-preview' );
...
<form method="get">
<?php $mn_list_table->search_box( __( 'Search Installed Themes' ), 'theme' ); ?>
</form>
