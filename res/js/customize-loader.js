/* global _mnCustomizeLoaderSettings, confirm */
/*
...
(function( exports, $ ){
	 ...
	 *
	 *     e.g. <a class="load-customize" href="<?php echo mn_customize_url(); ?>">Open Customizer</a>
	 *
	 * @augments mn.customize.Events
	 */
	Loader = $.extend( {}, api.Events, {
		...
		initialize: function() {
			...
			// the Customizer.
			$('#mnbody').on( 'click', '.load-customize', function( event ) {
		...
		hashchange: function() {
			...
			if ( hash && 0 === hash.indexOf( 'mn_customize=on' ) ) {
		...
		pushState: function ( src ) {
			...
			if ( $.support.history && window.location.href !== src ) {
				...
				window.location.hash = 'mn_customize=on&' + hash;
			}
	...
	$( function() {
		Loader.settings = _mnCustomizeLoaderSettings;
		Loader.initialize();
	});

	// Expose the API publicly on window.mn.customize.Loader
	api.Loader = Loader;
})( mn, jQuery );
