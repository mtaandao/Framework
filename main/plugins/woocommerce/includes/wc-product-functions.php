...
function wc_delete_product_transients( $post_id = 0 ) {
	...
	if ( $post_id > 0 ) {
		...
		if ( $parent_id = mn_get_post_parent_id( $post_id ) ) {
...
function wc_get_product_ids_on_sale() {
	...
	$on_sale_posts = $mndb->get_results( "
		SELECT post.ID, post.post_parent FROM `$mndb->posts` AS post
		LEFT JOIN `$mndb->postmeta` AS meta ON post.ID = meta.post_id
		LEFT JOIN `$mndb->postmeta` AS meta2 ON post.ID = meta2.post_id
	...
	$product_ids_on_sale = array_unique( array_map( 'absint', array_merge( mn_list_pluck( $on_sale_posts, 'ID' ), array_diff( mn_list_pluck( $on_sale_posts, 'post_parent' ), array( 0 ) ) ) ) );
...
function wc_placeholder_img( $size = 'shop_thumbnail' ) {
	...
	return apply_filters('woocommerce_placeholder_img', '<img src="' . wc_placeholder_img_src() . '" alt="' . esc_attr__( 'Placeholder', 'woocommerce' ) . '" width="' . esc_attr( $dimensions['width'] ) . '" class="woocommerce-placeholder mn-post-image" height="' . esc_attr( $dimensions['height'] ) . '" />', $size, $dimensions );
}
	...
	if ( is_array( $variation ) ) {
		...
		foreach ( $variation as $name => $value ) {
			...
			if ( taxonomy_exists( esc_attr( str_replace( 'attribute_', '', $name ) ) ) ) {
				...
				if ( ! is_mn_error( $term ) && ! empty( $term->name ) ) {
					$value = $term->name;
				}
			} else {
...
function wc_scheduled_sales() {
	...
	$product_ids = $mndb->get_col( $mndb->prepare( "
		SELECT postmeta.post_id FROM {$mndb->postmeta} as postmeta
		LEFT JOIN {$mndb->postmeta} as postmeta_2 ON postmeta.post_id = postmeta_2.post_id
		LEFT JOIN {$mndb->postmeta} as postmeta_3 ON postmeta.post_id = postmeta_3.post_id
	...
	if ( $product_ids ) {
		foreach ( $product_ids as $product_id ) {
			...
			$parent = mn_get_post_parent_id( $product_id );
...
add_filter( 'mn_get_attachment_image_attributes', 'wc_get_attachment_image_attributes' );
...
add_filter( 'mn_prepare_attachment_for_js', 'wc_prepare_attachment_for_js' );
...
function wc_product_has_unique_sku( $product_id, $sku ) {
	...
	$sku_found = $mndb->get_var( $mndb->prepare( "
		SELECT $mndb->posts.ID
		FROM $mndb->posts
		LEFT JOIN $mndb->postmeta ON ( $mndb->posts.ID = $mndb->postmeta.post_id )
		WHERE $mndb->posts.post_type IN ( 'product', 'product_variation' )
		AND $mndb->posts.post_status = 'publish'
		AND $mndb->postmeta.meta_key = '_sku' AND $mndb->postmeta.meta_value = '%s'
		AND $mndb->postmeta.post_id <> %d LIMIT 1
	 ", mn_slash( $sku ), $product_id ) );
...
function wc_get_product_id_by_sku( $sku ) {
	...
	$product_id = $mndb->get_var( $mndb->prepare( "
		SELECT posts.ID
...
function wc_get_product_variation_attributes( $variation_id ) {
	...
	$parent_id               = mn_get_post_parent_id( $variation_id );
...
function wc_get_product_cat_ids( $product_id ) {
	$product_cats = mn_get_post_terms( $product_id, 'product_cat', array( "fields" => "ids" ) );
...
function wc_get_product_attachment_props( $attachment_id, $product = false ) {
	...
	if ( $attachment_id ) {
		...
		$props['url']     = mn_get_attachment_url( $attachment_id );
		$props['alt']     = trim( strip_tags( get_post_meta( $attachment_id, '_mn_attachment_image_alt', true ) ) );
