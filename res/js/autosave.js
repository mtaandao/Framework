/* global tinymce, mnCookies, autosaveL10n, switchEditors */
// Back-compat
	...
	function autosave() {
		...
		function autosaveLocal() {
			...
			function checkStorage() {
				...
				try {
					window.sessionStorage.setItem( 'mn-test', test );
					result = window.sessionStorage.getItem( 'mn-test' ) === test;
					window.sessionStorage.removeItem( 'mn-test' );
				} catch(e) {}
			...
			function getStorage() {
				...
				if ( hasStorage && blog_id ) {
					stored_obj = sessionStorage.getItem( 'mn-autosave-' + blog_id );
			...
			function run() {
				...
				$( 'form#post' ).on( 'submit.autosave-local', function() {
					...
					mnCookies.set( 'mn-saving-post', post_id + '-check', 24 * 60 * 60, false, false, secure );
				});
			}
			...
			function restorePost( postData ) {
				...
				if ( postData ) {
					...
					if ( editor && ! editor.isHidden() && typeof switchEditors !== 'undefined' ) {
						if ( editor.settings.mnautop && postData.content ) {
							postData.content = switchEditors.mnautop( postData.content );
						}
		...
		function autosaveServer() {
			...
			function save() {
				...
				$document.trigger( 'mncountwords', [ postData.content ] )
					.trigger( 'before-autosave', [ postData ] );

				postData._mnnonce = $( '#_mnnonce' ).val() || '';
			...
			$document.on( 'heartbeat-send.autosave', function( event, data ) {
				...
				if ( autosaveData ) {
					data.mn_autosave = autosaveData;
				}
			}).on( 'heartbeat-tick.autosave', function( event, data ) {
				if ( data.mn_autosave ) {
					response( data.mn_autosave );
				}
				...
				if ( 'timeout' === error || 603 === status ) {
					...
					if ( ! mn.autosave.local.hasStorage ) {
