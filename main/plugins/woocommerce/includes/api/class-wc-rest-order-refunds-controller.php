...
class WC_REST_Order_Refunds_Controller extends WC_REST_Posts_Controller {
	...
	public function prepare_item_for_response( $post, $request ) {
		...
		if ( ! $order ) {
			return new MN_Error( 'woocommerce_rest_invalid_order_id', __( 'Invalid order ID.', 'woocommerce' ), 404 );
		}

		$refund = wc_get_order( $post );

		if ( ! $refund || intval( $refund->post->post_parent ) !== intval( $order->id ) ) {
			return new MN_Error( 'woocommerce_rest_invalid_order_refund_id', __( 'Invalid order refund ID.', 'woocommerce' ), 404 );
		}
	...
	public function create_item( $request ) {
		...
		if ( empty( $order_data ) ) {
			return new MN_Error( 'woocommerce_rest_invalid_order', __( 'Order is invalid', 'woocommerce' ), 400 );
		}

		if ( 0 > $request['amount'] ) {
			return new MN_Error( 'woocommerce_rest_invalid_order_refund', __( 'Refund amount must be greater than zero.', 'woocommerce' ), 400 );
		}
		...
		if ( ! $refund ) {
			return new MN_Error( 'woocommerce_rest_cannot_create_order_refund', __( 'Cannot create order refund, please try again.', 'woocommerce' ), 500 );
		}
		...
		if ( $api_refund ) {
			...
			if ( isset( $payment_gateways[ $order->payment_method ] ) && $payment_gateways[ $order->payment_method ]->supports( 'refunds' ) ) {
				...
				if ( is_mn_error( $result ) ) {
					...
					return new MN_Error( 'woocommerce_rest_create_order_refund_api_failed', __( 'An error occurred while attempting to create the refund using the payment gateway API.', 'woocommerce' ), 500 );
				}
			}
		}
