...
mn_reset_vars( array( 'revision', 'action', 'from', 'to' ) );
...
case 'restore' :
	...
	if ( ! mn_revisions_enabled( $post ) && ! mn_is_post_autosave( $revision ) ) {
	...
	mn_restore_post_revision( $revision->ID );
...
mn_enqueue_script( 'revisions' );
mn_localize_script( 'revisions', '_mnRevisionsSettings', mn_prepare_revisions_for_js( $post, $revision_id, $from ) );
...
$revisions_sidebar .= '<p>' . __( '<a href="https://mtaandao.co.ke/docs/Revision_Management" target="_blank">Revisions Management</a>' ) . '</p>';
$revisions_sidebar .= '<p>' . __( '<a href="https://mtaandao.co.ke/support/" target="_blank">Support Forums</a>' ) . '</p>';
