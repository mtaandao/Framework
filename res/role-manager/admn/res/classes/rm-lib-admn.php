<?php
/*
 * Stuff specific for Mtaandao Role Manager Mtaandao plugin
 * Author: Mtaandao
...
class RM_Lib_Mn extends RM_Lib {

    public static function get_instance($options_id = '') {
        
        if (self::$instance === null) {
            if (empty($options_id)) {
                throw new Exception('RM_Lib_Mn::get_inctance() - Error: plugin options ID string is required');
            }
            // new static() will work too
            self::$instance = new RM_Lib_Mn($options_id);
        }
    ...
    protected function get_rm_caps() {
        $rm_caps = parent::get_rm_caps();
        
        $rm_caps['rm_export_roles'] = 1;
        $rm_caps['rm_import_roles'] = 1;
        $rm_caps['rm_admin_menu_access'] = 1;
        $rm_caps['rm_widgets_access'] = 1;
        $rm_caps['rm_meta_boxes_access'] = 1;
        $rm_caps['rm_other_roles_access'] = 1;
        $rm_caps['rm_edit_posts_access'] = 1;
        $rm_caps['rm_plugins_activation_access'] = 1;   
        $rm_caps['rm_view_posts_access'] = 1;
        if ($this->multisite) {
            $rm_caps['rm_designs_access'] = 1;
        }
    ...
    public function get_key_capability() {
        $rm_key_capability = $this->get_option('rm_key_capability');
        if (!$this->multisite) {
            $key_capability = empty($rm_key_capability) ? RM_KEY_CAPABILITY : $rm_key_capability;
        } else {
    ...
    /**
     * return addition to the array of built-in MN capabilities (MN 3.1 admin/res/schema.php) 
     * 
     * @return array 
     */
    public function get_built_in_mn_caps() {
        
        $caps = parent::get_built_in_mn_caps();
    ...
    public function get_edit_custom_post_type_caps() {
        $caps = get_transient('rm_edit_custom_post_type_caps');
        if (empty($caps)) {
            ...
            set_transient('rm_edit_custom_post_type_caps', $caps, 15);
        }
    ...
    protected function network_replicate_addons_data() {
        global $mndb;
        ...
        $replicate_widgets_access_restrictions = filter_input(INPUT_POST, 'rm_replicate_widgets_access_restrictions', FILTER_SANITIZE_NUMBER_INT);
        if (empty($replicate_widgets_access_restrictions)) {
            return true;
        }
    
        $widgets_access_restrictions = get_option(RM_WIDGETS::ACCESS_DATA_KEY);
        ...
        foreach ($this->blog_ids as $blog_id) {
            ...
            $query1 = "select option_id from $options_table_name where option_name='". RM_WIDGETS::ACCESS_DATA_KEY ."' limit 0,1";
            ...
            if ($option_id>0) {
                ...
                $query = "insert into $options_table_name
                            set option_name='". RM_WIDGETS::ACCESS_DATA_KEY ."',
    ...
    public function direct_network_roles_update() {
        ...
        $result = $this->network_replicate_addons_data();
    ...
    public function get_assign_role() {
        $assign_role = new RM_Assign_Role_Mn($this);
    ...
    public function get_posts_by_terms($terms_list) {
        ...
        if (is_array($terms_list)) {
            $terms_list_str = RM_Utils::filter_int_array_to_str($terms_list);
        } else {
    ...
    public function can_edit($post) {        
        ...
        $checked_posts = get_transient('rm_checked_posts');
        ...
        if (!isset($checked_posts[$post->ID]) || !isset($checked_posts[$post->ID][$current_user->ID])) {
            ...
            set_transient('rm_checked_posts', $checked_posts, 30);
        }
...
?>       
        ...
        <table>
            ...
            <tr>
                ...
                <td>
                    <a href="https://www.mtaandao.co.ke">www.mtaandao.co.ke</a>
                </td>
            </tr>
            <tr>
                <td>
                ...
                <td>
                    <a href="https://www.mtaandao.co.ke/download-plugin">www.mtaandao.co.ke/download-plugin</a>
                </td>
            </tr>
            <tr>
                <td>
                ...
                <td>
                    <a href="https://www.mtaandao.co.ke/about">Mtaandao</a>
                </td>
            </tr>
            <tr>
                <td colspan="2">
                    <a href="mailto:dev@mtaandao.co.ke" target="_top">Send support question</a>
                </td>
            </tr>
        </table>        
<?php        
