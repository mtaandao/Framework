...
class WC_API_Coupons extends WC_API_Resource {
	...
	public function get_coupon( $id, $fields = null ) {
		...
		try {
			...
			// get the coupon code
			$code = $mndb->get_var( $mndb->prepare( "SELECT post_title FROM $mndb->posts WHERE id = %s AND post_type = 'shop_coupon' AND post_status = 'publish'", $id ) );
	...
	public function get_coupon_by_code( $code, $fields = null ) {
		...
		try {
			$id = $mndb->get_var( $mndb->prepare( "SELECT id FROM $mndb->posts WHERE post_title = %s AND post_type = 'shop_coupon' AND post_status = 'publish' ORDER BY post_date DESC LIMIT 1;", $code ) );
	...
	public function create_coupon( $data ) {
		...
		try {
			...
			$coupon_data = mn_parse_args( $data, $defaults );
			...
			$id = mn_insert_post( $new_coupon, true );
	...
	public function edit_coupon( $id, $data ) {
		...
		try {
			...
			if ( isset( $data['code'] ) ) {
				...
				$updated = mn_update_post( array( 'ID' => intval( $id ), 'post_title' => $coupon_code ) );
			...
			if ( isset( $data['description'] ) ) {
				$updated = mn_update_post( array( 'ID' => intval( $id ), 'post_excerpt' => $data['description'] ) );
