...
abstract class WC_Data {
	 ...
	 * MN defines 'post', 'user', 'comment', and 'term'.
	 */
	...
	public function get_meta( $key = '', $single = true ) {
		$array_keys = array_keys( mn_list_pluck( $this->_meta_data, 'key' ), $key );
		$value    = '';
	...
	public function update_meta_data( $key, $value, $meta_id = '' ) {
		$array_key = '';
		if ( $meta_id ) {
			$array_key = array_keys( mn_list_pluck( $this->_meta_data, 'meta_id' ), $meta_id );
		}
		if ( $array_key ) {
	...
	protected function read_meta_data() {
		...
		if ( ! empty ( $this->_cache_group ) ) {
			...
			$cached_meta = mn_cache_get( $cache_key, $this->_cache_group );
		...
		if ( ! $cache_loaded ) {
			...
			$raw_meta_data = $mndb->get_results( $mndb->prepare( "
			...
			if ( ! empty ( $this->_cache_group ) ) {
				mn_cache_set( $cache_key, $this->_meta_data, $this->_cache_group );
			}
		}
	}
	...
	protected function save_meta_data() {
		global $mndb;
		$db_info = $this->_get_db_info();
		$all_meta_ids = array_map( 'absint', $mndb->get_col( $mndb->prepare( "
