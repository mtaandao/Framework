...
class WC_Shipping_Zone extends WC_Data {
	...
	public function create() {
		global $mndb;
		$mndb->insert( $mndb->prefix . 'woocommerce_shipping_zones', array(
			'zone_name'  => $this->get_zone_name(),
			'zone_order' => $this->get_zone_order(),
		) );
		$this->set_zone_id( $mndb->insert_id );
	}
	...
	public function read( $id ) {
		...
		if ( $zone_data = $mndb->get_row( $mndb->prepare( "SELECT * FROM {$mndb->prefix}woocommerce_shipping_zones WHERE zone_id = %d LIMIT 1;", $id ) ) ) {
	...
	public function update() {
		global $mndb;
		$mndb->update( $mndb->prefix . 'woocommerce_shipping_zones', array(
			'zone_name'  => $this->get_zone_name(),
			'zone_order' => $this->get_zone_order(),
	...
	public function delete() {
		if ( $this->get_id() ) {
			global $mndb;
			$mndb->delete( $mndb->prefix . 'woocommerce_shipping_zone_methods', array( 'zone_id' => $this->get_id() ) );
			$mndb->delete( $mndb->prefix . 'woocommerce_shipping_zone_locations', array( 'zone_id' => $this->get_id() ) );
			$mndb->delete( $mndb->prefix . 'woocommerce_shipping_zones', array( 'zone_id' => $this->get_id() ) );
	...
	public function get_shipping_methods( $enabled_only = false ) {
		...
		$raw_methods_sql = $enabled_only ? "SELECT method_id, method_order, instance_id, is_enabled FROM {$mndb->prefix}woocommerce_shipping_zone_methods WHERE zone_id = %d AND is_enabled = 1 order by method_order ASC;" : "SELECT method_id, method_order, instance_id, is_enabled FROM {$mndb->prefix}woocommerce_shipping_zone_methods WHERE zone_id = %d order by method_order ASC;";
		$raw_methods     = $mndb->get_results( $mndb->prepare( $raw_methods_sql, $this->get_zone_id() ) );
	...
	private function read_zone_locations( $zone_id ) {
		...
		if ( $locations = $mndb->get_results( $mndb->prepare( "SELECT * FROM {$mndb->prefix}woocommerce_shipping_zone_locations WHERE zone_id = %d;", $zone_id ) ) ) {
			foreach ( $locations as $location ) {
	...
	private function save_locations() {
		...
		global $mndb;
		$mndb->delete( $mndb->prefix . 'woocommerce_shipping_zone_locations', array( 'zone_id' => $this->get_zone_id() ) );

		foreach ( $this->get_zone_locations() as $location ) {
			$mndb->insert( $mndb->prefix . 'woocommerce_shipping_zone_locations', array(
	...
	public function add_shipping_method( $type ) {
		...
		$wc_shipping     = WC_Shipping::instance();
		$allowed_classes = $wc_shipping->get_shipping_method_class_names();
		$count           = $mndb->get_var( $mndb->prepare( "SELECT COUNT(*) FROM {$mndb->prefix}woocommerce_shipping_zone_methods WHERE zone_id = %d", $this->get_zone_id() ) );

		if ( in_array( $type, array_keys( $allowed_classes ) ) ) {
			$mndb->insert(
				$mndb->prefix . 'woocommerce_shipping_zone_methods',
				array(
					'method_id'    => $type,
					'zone_id'      => $this->get_zone_id(),
