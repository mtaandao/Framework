...
class WC_Auth {
	...
	protected function post_consumer_data( $consumer_data, $url ) {
		...
		$response = mn_safe_remote_post( esc_url_raw( $url ), $params );
	...
	public function handle_auth_requests() {
		...
		if ( ! empty( $_GET['wc-auth-version'] ) ) {
			$mn->query_vars['wc-auth-version'] = $_GET['wc-auth-version'];
		}

		if ( ! empty( $_GET['wc-auth-route'] ) ) {
			$mn->query_vars['wc-auth-route'] = $_GET['wc-auth-route'];
		}

		// wc-auth endpoint requests
		if ( ! empty( $mn->query_vars['wc-auth-version'] ) && ! empty( $mn->query_vars['wc-auth-route'] ) ) {
			$this->auth_endpoint( $mn->query_vars['wc-auth-route'] );
		}
	}
		...
		try {
			...
			// Authorize endpoint
				...
				wc_get_template( 'auth/form-grant-access.php', array(
					...
					'granted_url' => mn_nonce_url( $this->build_url( $_REQUEST, 'access_granted' ), 'wc_auth_grant_access', 'wc_auth_nonce' ),
					'logout_url'  => mn_logout_url( $this->build_url( $_REQUEST, 'login' ) ),
					'user'        => mn_get_current_user()
				) );
				exit;
			...
			// Granted access endpoint
				...
				if ( ! isset( $_GET['wc_auth_nonce'] ) || ! mn_verify_nonce( $_GET['wc_auth_nonce'], 'wc_auth_grant_access' ) ) {
				...
				if ( $response ) {
					mn_redirect( esc_url_raw( add_query_arg( array( 'success' => 1, 'user_id' => wc_clean( $_REQUEST['user_id'] ) ), $this->get_formatted_url( $_REQUEST['return_url'] ) ) ) );
					exit;
				}
			} else {
			...
			mn_die( sprintf( __( 'Error: %s', 'woocommerce' ), $e->getMessage() ), __( 'Access Denied', 'woocommerce' ), array( 'response' => 401 ) );
		}
	}
	...
	private function maybe_delete_key( $key ) {
		...
		if ( isset( $key['key_id'] ) ) {
			$mndb->delete( $mndb->prefix . 'woocommerce_api_keys', array( 'key_id' => $key['key_id'] ), array( '%d' ) );
		}
	}
}
