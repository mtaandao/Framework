...
class RWMB_Loader {
	 ...
	 *
	 * @link http://www.deluxeblogtips.com/2013/07/get-url-of-php-file-in-mtaandao.html
	 * @param string $path Base folder path
	 * @return array Path and URL.
	 */
	public static function get_path( $path = '' ) {
		// Plugin base path
		$path       = mn_normalize_path( untrailingslashit( $path ) );
		$themes_dir = mn_normalize_path( untrailingslashit( dirname( realpath( get_stylesheet_directory() ) ) ) );
		...
		if (
			0 !== strpos( $path, mn_normalize_path( MN_PLUGIN_DIR ) )
			&& 0 !== strpos( $path, mn_normalize_path( MNMU_PLUGIN_DIR ) )
