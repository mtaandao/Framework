...
class Vc_Frontend_Editor implements Vc_Editor_Interface {
	 ...
	 * @var string
	 */
	protected static $brand_url = 'http://vc.mtaandao.co.ke/plugins/page-builder/?utm_campaign=VCplugin&utm_source=vc_user&utm_medium=frontend_editor';
	...
	public function buildEditablePage() {
		...
		add_filter( 'mn_enqueue_scripts', array(
			&$this,
			'loadIFrameJsCss',
		) );

		add_action( 'mn_footer', array(
			&$this,
			'printPostShortcodes',
		) );
	}
	...
	public function parseEditableContent( $post ) {
		...
		if ( $post_id > 0 && $post->ID === $post_id && ! defined( 'VC_LOADING_EDITABLE_CONTENT' ) ) {
			...
			remove_filter( 'the_content', 'mnautop' );
	...
	/**
	 * Used for mn filter 'mn_insert_post_empty_content' to allow empty post insertion.
	 *
	 * @param $allow_empty
	 *
	 * @return bool
	...
	function renderEditor() {
		...
		if ( ! self::inlineEnabled() || ! vc_user_access()->mnAny( array(
				'edit_post',
				$this->post_id,
			) )->get()
		) {
		...
		if ( $this->post && 'auto-draft' === $this->post->post_status ) {
			...
			add_filter( 'mn_insert_post_empty_content', array(
				$this,
				'allowInsertEmptyPost',
			) );
			mn_update_post( $post_data, true );
		...
		mn_enqueue_media( array( 'post' => $this->post_id ) );
	 ...
	 */
	function adminBarEditLink( $mn_admin_bar ) {
		if ( ! is_object( $mn_admin_bar ) ) {
			global $mn_admin_bar;
		}
		if ( is_singular() ) {
			if ( $this->showButton( get_the_ID() ) ) {
	...
	function loadTinyMceSettings() {
		if ( ! class_exists( '_MN_Editors' ) ) {
			require( ABSPATH . RES . '/class-mn-editor.php' );
		}
		$set = _MN_Editors::parse_settings( self::$content_editor_id, self::$content_editor_settings );
		_MN_Editors::editor_settings( self::$content_editor_id, $set );
	}
	...
	function loadIFrameJsCss() {
		mn_enqueue_script( 'jquery-ui-tabs' );
		mn_enqueue_script( 'jquery-ui-sortable' );
		mn_enqueue_script( 'jquery-ui-droppable' );
		mn_enqueue_script( 'jquery-ui-draggable' );
		mn_enqueue_script( 'jquery-ui-accordion' );
		mn_enqueue_script( 'jquery-ui-autocomplete' );
		mn_enqueue_script( 'wpb_composer_front_js' );
		mn_enqueue_style( 'js_composer_front' );
		mn_enqueue_style( 'vc_inline_css', vc_asset_url( 'css/js_composer_frontend_editor_iframe.min.css' ), array(), WPB_VC_VERSION );
		mn_enqueue_script( 'waypoints' );
		mn_enqueue_script( 'wpb_scrollTo_js', vc_asset_url( 'lib/bower/scrollTo/jquery.scrollTo.min.js' ), array( 'jquery' ), WPB_VC_VERSION, true );
		mn_enqueue_style( 'js_composer_custom_css' );

		mn_enqueue_script( 'wpb_php_js', vc_asset_url( 'lib/php.default/php.default.min.js' ), array( 'jquery' ), WPB_VC_VERSION, true );
		mn_enqueue_script( 'vc_inline_iframe_js', vc_asset_url( 'js/dist/page_editable.min.js' ), array(
			'jquery',
			'underscore',
		), WPB_VC_VERSION, true );
	...
	public function registerJs() {
		...
		mn_register_script( 'vc_bootstrap_js', vc_asset_url( 'lib/bower/bootstrap3/dist/js/bootstrap.min.js' ), array( 'jquery' ), WPB_VC_VERSION, true );
		mn_register_script( 'vc_accordion_script', vc_asset_url( 'lib/vc_accordion/vc-accordion.min.js' ), array( 'jquery' ), WPB_VC_VERSION, true );
		mn_register_script( 'wpb_php_js', vc_asset_url( 'lib/php.default/php.default.min.js' ), array( 'jquery' ), WPB_VC_VERSION, true );
		// used as polyfill for JSON.stringify and etc
		mn_register_script( 'wpb_json-js', vc_asset_url( 'lib/bower/json-js/json2.min.js' ), array(), WPB_VC_VERSION, true );
		// used in post settings editor
		mn_register_script( 'ace-editor', vc_asset_url( 'lib/bower/ace-builds/src-min-noconflict/ace.js' ), array( 'jquery' ), WPB_VC_VERSION, true );
		mn_register_script( 'webfont', '//ajax.googleapis.com/ajax/libs/webfont/1.4.7/webfont.js' ); // Google Web Font CDN
		mn_register_script( 'wpb_scrollTo_js', vc_asset_url( 'lib/bower/scrollTo/jquery.scrollTo.min.js' ), array( 'jquery' ), WPB_VC_VERSION, true );
		mn_register_script( 'vc_accordion_script', vc_asset_url( 'lib/vc_accordion/vc-accordion.min.js' ), array( 'jquery' ), WPB_VC_VERSION, true );
		mn_register_script( 'vc-frontend-editor-min-js', vc_asset_url( 'js/dist/frontend-editor.min.js' ), array(), WPB_VC_VERSION, true );
		mn_localize_script( 'vc-frontend-editor-min-js', 'i18nLocale', visual_composer()->getEditorsLocale() );
	}
	...
	public function enqueueJs() {
		...
		foreach ( $mn_dependencies as $dependency ) {
			mn_enqueue_script( $dependency );
		}
	...
	public function registerCss() {
		mn_register_style( 'ui-custom-theme', vc_asset_url( 'css/ui-custom-theme/jquery-ui-less.custom.min.css' ), false, WPB_VC_VERSION, false );
		mn_register_style( 'animate-css', vc_asset_url( 'lib/bower/animate-css/animate.min.css' ), false, WPB_VC_VERSION, 'screen' );
		mn_register_style( 'font-awesome', vc_asset_url( 'lib/bower/font-awesome/css/font-awesome.min.css' ), false, WPB_VC_VERSION, 'screen' );

		mn_register_style( 'vc_inline_css', vc_asset_url( 'css/js_composer_frontend_editor.min.css' ), array(), WPB_VC_VERSION );
	...
	public function enqueueMappedShortcodeJs( $value ) {
		mn_enqueue_script( 'front_enqueue_js_' . md5( $value ), $value, array( 'vc-frontend-editor-min-js' ), WPB_VC_VERSION, true );
	}

	public function enqueueMappedShortcodeCss( $value ) {
		mn_enqueue_style( 'front_enqueue_css_' . md5( $value ), $value, array( 'vc_inline_css' ), WPB_VC_VERSION );
	}
	...
	function getPageShortcodesByContent( $content ) {
		...
		foreach ( $not_shortcodes as $string ) {
			$temp = str_replace( array(
				'<p>',
				'</p>',
			), '', $string ); // just to avoid autop @todo maybe do it better like vc_mnnop in js.
