...
class WC_Webhook {
	...
	public function process( $arg ) {
		...
		if ( apply_filters( 'woocommerce_webhook_deliver_async', true, $this, $arg ) ) {
			...
			mn_schedule_single_event( time(), 'woocommerce_deliver_webhook_async', array( $this->id, $arg ) );
	 ...
	 * (like `mn_trash_post`) will fire for every post type, not just ours.
	 *
	 * @since 2.2
	...
	/**
	 * Deliver the webhook payload using mn_safe_remote_request().
	 *
	 * @since 2.2
	 * @param mixed $arg First hook argument.
	 */
	public function deliver( $arg ) {
		...
		$http_args = array(
			...
			'httpversion' => '1.0',
			'blocking'    => true,
			'user-agent'  => sprintf( 'WooCommerce/%s Hookshot (Mtaandao/%s)', WC_VERSION, $GLOBALS['mn_version'] ),
		...
		// Webhook away!
		$response = mn_safe_remote_request( $this->get_delivery_url(), $http_args );
	...
	private function build_payload( $resource_id ) {
		...
		mn_set_current_user( $this->get_user_id() );
		...
		mn_set_current_user( $current_user );
	...
	public function get_new_delivery_id() {
		...
		$comment_id = mn_insert_comment( $comment_data );
	 ...
	 * @param array|MN_Error $response response data
	...
	public function log_delivery( $delivery_id, $request, $response, $duration ) {
		...
		if ( is_mn_error( $response ) ) {
			...
			$response_code    = mn_remote_retrieve_response_code( $response );
			$response_message = mn_remote_retrieve_response_message( $response );
			$response_headers = mn_remote_retrieve_headers( $response );
			$response_body    = mn_remote_retrieve_body( $response );
		}
		...
		$log = mn_count_comments( $this->id );
		if ( $log->total_comments > apply_filters( 'woocommerce_max_webhook_delivery_logs', 25 ) ) {
			global $mndb;

			$comment_id = $mndb->get_var( $mndb->prepare( "SELECT comment_ID FROM {$mndb->comments} WHERE comment_post_ID = %d ORDER BY comment_date_gmt ASC LIMIT 1", $this->id ) );
	...
	public function deliver_ping() {
		...
		$test          = mn_safe_remote_post( $this->get_delivery_url(), $args );
		$response_code = mn_remote_retrieve_response_code( $test );

		if ( is_mn_error( $test ) ) {
			return new MN_Error( 'error', sprintf( __( 'Error: Delivery URL cannot be reached: %s', 'woocommerce' ), $test->get_error_message() ) );
		}

		if ( 200 !== $response_code ) {
			return new MN_Error( 'error', sprintf( __( 'Error: Delivery URL returned response code: %s', 'woocommerce' ), absint( $response_code ) ) );
		}
	...
	public function update_status( $status ) {
		...
		$mndb->update( $mndb->posts, array( 'post_status' => $post_status ), array( 'ID' => $this->id ) );
