...
function sm_addon_active( $addons ) {
	...
	$paths = array(
		'maker' => 'shortcodes-maker/shortcodes-maker.php',
		'skins' => 'shortcodes-skins/shortcodes-skins.php',
		'extra' => 'shortcodes-extra/shortcodes-extra.php',
	);
...
function sm_skins_link() {
	if ( sm_addon_active( 'skins' ) ) return sprintf( '<br><strong>%s</strong><br><strong>%s</strong>', __( 'Additional skins successfully installed', 'shortcodes' ), __( 'Open dropdown to choose one of new styles', 'shortcodes' ) );
	else return sprintf( '<br><a href="http://mtaandao.co.ke/sus" target="_blank">%s &rarr;</a>', __( 'Get more styles', 'shortcodes' ) );
}

/**
 *  Resizes an image and returns an array containing the resized URL, width, height and file type. Uses native Mtaandao functionality.
 *
...
function sm_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
	...
	if ( isset( $mn_version ) && version_compare( $mn_version, '3.5' ) >= 0 ) {
		...
		if ( is_multisite() ) {
			...
			$file_path = str_replace( $blog_details->path . 'files/', '/mn-content/blogs.dir/' . $blog_id . '/files/', $file_path );
		}
...
/**
 *  Deletes the resized images when the original image is deleted from the Mtaandao Media Library.
 *
...
function sm_delete_resized_images( $post_id ) {
	...
	// Get Mtaandao uploads directory (and bail if it doesn't exist)
...
class Sm_Tools {
	function __construct() {
		add_action( 'mn_ajax_sm_example_preview', array( __CLASS__, 'example' ) );
	...
	public static function get_slides( $args ) {
		...
		if ( is_array( $query->posts ) ) foreach ( $query->posts as $post ) {
				...
				elseif ( $args['link'] === 'post' ) $slide['link'] = get_prettylink( $post->ID );
	...
	public static function example() {
		...
		self::access();
		// Check nonce
		if ( !isset( $_REQUEST['nonce'] ) || !mn_verify_nonce( $_REQUEST['nonce'], 'sm_examples_nonce' ) ) return;
		// Check incoming data
		...
		// Cache not found
			...
			// Check for code
			if ( !$code ) die( '<p class="sm-examples-error">' . __( 'Example code does not found, please check it later', 'shortcodes' ) . '</p>' );
			// Clean-up the code
			...
			foreach ( $chunks as $chunk ) {
				...
				echo '<div class="sm-examples-code"><span class="sm-examples-get-code button"><i class="fa fa-code"></i>&nbsp;&nbsp;' . __( 'Get the code', 'shortcodes' ) . '</span><textarea rows="' . $rows . '">' . esc_textarea( $chunk ) . '</textarea></div>';
			}
	...
	public static function slide_link_input( $form_fields, $post ) {
		$form_fields['sm_slide_link'] = array(
			'label' => __( 'Slide link', 'shortcodes' ),
			'input' => 'text',
			'value' => get_post_meta( $post->ID, 'sm_slide_link', true ),
			'helps' => sprintf( '<strong>%s</strong><br>%s', __( 'Mtaandao Shortcodes', 'shortcodes' ), __( 'Use this field to add custom links to slides used with Slider, Carousel and Custom Gallery shortcodes', 'shortcodes' ) )
		);
		return $form_fields;
	}
