...
class WC_Admin_Duplicate_Product {
	...
	public function dupe_link( $actions, $post ) {
		...
		$actions['duplicate'] = '<a href="' . mn_nonce_url( admin_url( 'edit.php?post_type=product&action=duplicate_product&amp;post=' . $post->ID ), 'woocommerce-duplicate-product_' . $post->ID ) . '" title="' . esc_attr__( 'Make a duplicate from this product', 'woocommerce' )
	...
	public function dupe_button() {
		...
		if ( isset( $_GET['post'] ) ) {
			$notify_url = mn_nonce_url( admin_url( "edit.php?post_type=product&action=duplicate_product&post=" . absint( $_GET['post'] ) ), 'woocommerce-duplicate-product_' . $_GET['post'] );
			?>
	...
	public function duplicate_product_action() {
		...
		if ( empty( $_REQUEST['post'] ) ) {
			mn_die( __( 'No product to duplicate has been supplied!', 'woocommerce' ) );
		}
		...
		if ( ! empty( $post ) ) {
			...
			// If you have written a plugin which uses non-MN database tables to save
			...
			mn_redirect( admin_url( 'post.php?action=edit&post=' . $new_id ) );
			exit;
		} else {
			mn_die( __( 'Product creation failed, could not find original product:', 'woocommerce' ) . ' ' . $id );
		}
	}
	...
	public function duplicate_product( $post, $parent = 0, $post_status = '' ) {
		...
		$new_post_author    = mn_get_current_user();
		...
		if ( ! in_array( $post_status, array( 'draft', 'pending', 'auto-draft' ) ) ) {
	        $mndb->update(
				$mndb->posts,
				array(
					'post_name' => mn_unique_post_slug( sanitize_title( $post_title, $new_post_id ), $new_post_id, $post_status, $post->post_type, $post_parent ),
	...
	private function get_product_to_duplicate( $id ) {
		...
		$post = $mndb->get_results( "SELECT * FROM $mndb->posts WHERE ID=$id" );
	...
	private function duplicate_post_taxonomies( $id, $new_id, $post_type ) {
		...
		foreach ( $taxonomies as $taxonomy ) {
			$post_terms       = mn_get_object_terms( $id, $taxonomy );
			$post_terms_count = sizeof( $post_terms );

			for ( $i = 0; $i < $post_terms_count; $i++ ) {
				mn_set_object_terms( $new_id, $post_terms[ $i ]->slug, $taxonomy, true );
			}
		}
	}
	...
	private function duplicate_post_meta( $id, $new_id ) {
		...
		$sql     = $mndb->prepare( "SELECT meta_key, meta_value FROM $mndb->postmeta WHERE post_id = %d", absint( $id ) );
		...
		$post_meta = $mndb->get_results( $sql );

		if ( sizeof( $post_meta ) ) {
			$sql_query_sel = array();
			$sql_query     = "INSERT INTO $mndb->postmeta (post_id, meta_key, meta_value) ";

			foreach ( $post_meta as $post_meta_row ) {
				$sql_query_sel[] = $mndb->prepare( "SELECT %d, %s, %s", $new_id, $post_meta_row->meta_key, $post_meta_row->meta_value );
			}

			$sql_query .= implode( " UNION ALL ", $sql_query_sel );
			$mndb->query( $sql_query );
		}
	}
}
