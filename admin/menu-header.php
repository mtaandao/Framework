 ...
 */
function _mn_menu_output( $menu, $submenu, $submenu_as_parent = true ) {
	...
	foreach ( $menu as $key => $item ) {
		...
		if ( ! empty( $submenu[$item[2]] ) ) {
			$class[] = 'mn-has-submenu';
			$submenu_items = $submenu[$item[2]];
		}

		if ( ( $parent_file && $item[2] == $parent_file ) || ( empty($typenow) && $self == $item[2] ) ) {
			$class[] = ! empty( $submenu_items ) ? 'mn-has-current-submenu mn-menu-open' : 'current';
		} else {
			$class[] = 'mn-not-current-submenu';
		...
		if ( false !== strpos( $class, 'mn-menu-separator' ) ) {
			$is_separator = true;
		}

		/*
		 * If the string 'none' (previously 'div') is passed instead of a URL, don't output
		 * the default menu image so an icon can be added to div.mn-menu-image as background
		...
		$arrow = '<div class="mn-menu-arrow"><div></div></div>';

		$title = mntexturize( $item[0] );
		...
		if ( $is_separator ) {
			...
			if ( ! empty( $menu_hook ) || ( ( 'index.php' != $submenu_items[0][2] ) && file_exists( MN_PLUGIN_DIR . "/$menu_file" ) && ! file_exists( ABSPATH . "/admin/$menu_file" ) ) ) {
				...
				echo "\n\t<a href='{$submenu_items[0][2]}'$class $aria_attributes>$arrow<div class='mn-menu-image$img_class'$img_style>$img</div><div class='mn-menu-name'>$title</div></a>";
			}
			...
			if ( ! empty( $menu_hook ) || ( ( 'index.php' != $item[2] ) && file_exists( MN_PLUGIN_DIR . "/$menu_file" ) && ! file_exists( ABSPATH . "/admin/$menu_file" ) ) ) {
				...
				echo "\n\t<a href='{$item[2]}'$class $aria_attributes>$arrow<div class='mn-menu-image$img_class'$img_style>$img</div><div class='mn-menu-name'>{$item[0]}</div></a>";
			}
		}

		if ( ! empty( $submenu_items ) ) {
			echo "\n\t<ul class='mn-submenu mn-submenu-wrap'>";
			echo "<li class='mn-submenu-head' aria-hidden='true'>{$item[0]}</li>";
...
<a href="#mnbody-content" class="screen-reader-shortcut"><?php _e( 'Skip to main content' ); ?></a>
<a href="#mn-toolbar" class="screen-reader-shortcut"><?php _e( 'Skip to toolbar' ); ?></a>
