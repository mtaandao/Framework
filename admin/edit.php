...
if ( 'attachment' === $typenow ) {
	if ( mn_redirect( admin_url( 'upload.php' ) ) ) {
		exit;
	}
}
...
foreach ( array( 'p', 'attachment_id', 'page_id' ) as $_redirect ) {
	if ( ! empty( $_REQUEST[ $_redirect ] ) ) {
		mn_redirect( admin_url( 'edit-comments.php?p=' . absint( $_REQUEST[ $_redirect ] ) ) );
		exit;
	}
}
unset( $_redirect );
...
if ( $doaction ) {
	...
	$sendback = remove_query_arg( array('trashed', 'untrashed', 'deleted', 'locked', 'ids'), mn_get_referer() );
	if ( ! $sendback )
	...
	if ( 'delete_all' == $doaction ) {
		...
		if ( get_post_status_object( $post_status ) ) {
			$post_ids = $mndb->get_col( $mndb->prepare( "SELECT ID FROM $mndb->posts WHERE post_type=%s AND post_status = %s", $post_type, $post_status ) );
		}
		$doaction = 'delete';
		...
		case 'delete':
			...
			foreach ( (array) $post_ids as $post_id ) {
				...
				if ( $post_del->post_type == 'attachment' ) {
					if ( ! mn_delete_attachment($post_id) )
						mn_die( __('Error in deleting.') );
				} else {
					if ( !mn_delete_post($post_id) )
						mn_die( __('Error in deleting.') );
				}
				$deleted++;
			}
	...
	mn_redirect($sendback);
	exit();
} elseif ( ! empty($_REQUEST['_mn_http_referer']) ) {
...
mn_enqueue_script('inline-edit-post');
mn_enqueue_script('heartbeat');
...
if ( 'post' == $post_type ) {
	...
	'<p>' . __('<a href="https://mtaandao.co.ke/docs/Posts_Screen" target="_blank">Documentation on Managing Posts</a>') . '</p>' .
	...
	get_current_screen()->set_help_sidebar(
	'<p><strong>' . __('For more information:') . '</strong></p>' .
	'<p>' . __('<a href="https://mtaandao.co.ke/docs/Pages_Screen" target="_blank">Documentation on Managing Pages</a>') . '</p>' .
	'<p>' . __('<a href="https://mtaandao.co.ke/support/" target="_blank">Support Forums</a>') . '</p>'
	);
...
foreach ( $bulk_counts as $message => $count ) {
	...
	if ( $message == 'trashed' && isset( $_REQUEST['ids'] ) ) {
		...
		$messages[] = '<a href="' . esc_url( mn_nonce_url( "edit.php?post_type=$post_type&doaction=undo&action=untrash&ids=$ids", "bulk-posts" ) ) . '">' . __('Undo') . '</a>';
	}
}
...
<?php $mn_list_table->search_box( $post_type_object->labels->search_items, 'post' ); ?>
