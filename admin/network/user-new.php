...
if ( ! current_user_can('create_users') )
	mn_die(__('Sorry, you are not allowed to add users to this network.'));
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="https://mtaandao.co.ke/docs/Network_Admin_Users_Screen" target="_blank">Documentation on Network Users</a>') . '</p>' .
...
if ( isset($_REQUEST['action']) && 'add-user' == $_REQUEST['action'] ) {
	...
	if ( ! is_array( $_POST['user'] ) )
		mn_die( __( 'Cannot create an empty user.' ) );

	$user = mn_unslash( $_POST['user'] );

	$user_details = mnmu_validate_user_signup( $user['username'], $user['email'] );
	if ( is_mn_error( $user_details[ 'errors' ] ) && ! empty( $user_details[ 'errors' ]->errors ) ) {
		$add_user_errors = $user_details[ 'errors' ];
	} else {
		$password = mn_generate_password( 12, false);
		$user_id = mnmu_create_user( esc_html( strtolower( $user['username'] ) ), $password, sanitize_email( $user['email'] ) );

		if ( ! $user_id ) {
	 		$add_user_errors = new MN_Error( 'add_user_fail', __( 'Cannot add user.' ) );
		} else {
			/**
			...
			mn_redirect( add_query_arg( array('update' => 'added', 'user_id' => $user_id ), 'user-new.php' ) );
			exit;
		}
	}
}
...
if ( isset( $add_user_errors ) && is_mn_error( $add_user_errors ) ) { ?>
	<div class="error">
		<?php
...
<?php } ?>
	...
	mn_nonce_field( 'add-user', '_mnnonce_add-user' );
