...
<form action="<?php echo admin_url( 'admin-ajax.php' ); ?>" method="post"
	...
	data-vc-roles="form">
	<div class="tab_intro">
		<p><?php _e( 'Control user group role access to the features and options of Visual Composer - manage Mtaandao default and custom roles.', 'js_composer' ) ?></p>
	</div>
	<!-- Settings template start -->
	<div class="vc_mn-settings">
		<div class="vc_mn-accordion" data-vc-action="collapseAll">
			<?php foreach ( $editable_roles as $role => $details ) :
				...
				if ( count( $valid_roles ) > 0 ) :
					?>
					<div
						class="vc_mn-accordion-panel vc_ui-settings-roles-role<?php echo ! isset( $next ) ? ' vc_active' : '' ?>"
						data-vc-unique-id="<?php echo esc_attr( $unique_id ) ?>"
						data-vc-content=".vc_mn-accordion-panel-body"
						data-vc-role="<?php echo esc_attr( $role ) ?>">
						<div class="widget" data-vc-accordion=""
							data-vc-container=".vc_mn-accordion"
						...
						<div class="vc_mn-accordion-panel-body">
	...
	<span class="vc_settings-spinner vc_ui-mn-spinner" style="display: none;" id="vc_mn-spinner"></span>
	<!-- Settings template end -->
	<?php
	mn_nonce_field( 'vc_settings-' . $tab . '-action', 'vc_nonce_field' );
