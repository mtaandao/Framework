...
if ( isset($_GET['action']) ) {
	...
	if ( 'update-selected' == $action ) {
		if ( ! current_user_can( 'update_plugins' ) )
			mn_die( __( 'Sorry, you are not allowed to update plugins for this site.' ) );
		...
		if ( ! isset($_GET['failure']) && ! isset($_GET['success']) ) {
			mn_redirect( admin_url('update.php?action=activate-plugin&failure=true&plugin=' . urlencode( $plugin ) . '&_mnnonce=' . $_GET['_mnnonce']) );
			activate_plugin( $plugin, '', ! empty( $_GET['networkwide'] ), true );
			mn_redirect( admin_url('update.php?action=activate-plugin&success=true&plugin=' . urlencode( $plugin ) . '&_mnnonce=' . $_GET['_mnnonce']) );
			die();
		}
		...
		if ( ! current_user_can('update_themes') )
			mn_die(__('Sorry, you are not allowed to update themes for this site.'));
		...
		include_once( ABSPATH . 'admin/includes/class-mn-upgrader.php' ); //for themes_api..
