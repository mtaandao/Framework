...
class WC_Install {
	...
	public static function init() {
		...
		add_filter( 'mnmu_drop_tables', array( __CLASS__, 'mnmu_drop_tables' ) );
	...
	public static function install_actions() {
		...
		if ( ! empty( $_GET['force_update_woocommerce'] ) ) {
			do_action( 'mn_wc_updater_cron' );
			mn_safe_redirect( admin_url( 'admin.php?page=wc-settings' ) );
		}
	}
	...
	private static function create_cron_jobs() {
		...
		mn_schedule_event( strtotime( '00:00 tomorrow ' . $ve . get_option( 'gmt_offset' ) . ' HOURS' ), 'daily', 'woocommerce_scheduled_sales' );

		$held_duration = get_option( 'woocommerce_hold_stock_minutes', '60' );

		if ( $held_duration != '' ) {
			mn_schedule_single_event( time() + ( absint( $held_duration ) * 60 ), 'woocommerce_cancel_unpaid_orders' );
		}

		mn_schedule_event( time(), 'twicedaily', 'woocommerce_cleanup_sessions' );
		mn_schedule_event( strtotime( 'first tuesday of next month' ), 'monthly', 'woocommerce_geoip_updater' );
		mn_schedule_event( time(), apply_filters( 'woocommerce_tracker_event_recurrence', 'daily' ), 'woocommerce_tracker_send_event' );
	}
	 ...
	 *		woocommerce_termmeta - Term meta table - sadly Mtaandao does not have termmeta so we need our own
	...
	private static function create_tables() {
		 ...
		 */
		if ( $mndb->get_var( "SHOW TABLES LIKE '{$mndb->prefix}woocommerce_downloadable_product_permissions';" ) ) {
			if ( ! $mndb->get_var( "SHOW COLUMNS FROM `{$mndb->prefix}woocommerce_downloadable_product_permissions` LIKE 'permission_id';" ) ) {
				$mndb->query( "ALTER TABLE {$mndb->prefix}woocommerce_downloadable_product_permissions DROP PRIMARY KEY, ADD `permission_id` bigint(20) NOT NULL PRIMARY KEY AUTO_INCREMENT;" );
			}
		}
	...
	private static function get_schema() {
		...
		if ( $mndb->has_cap( 'collation' ) ) {
			$collate = $mndb->get_charset_collate();
		}

		/*
		 * Indexes have a maximum size of 767 bytes. Historically, we haven't need to be concerned about that.
		 * As of Mtaandao 4.2, however, we moved to utf8mb4, which uses 4 bytes per character. This means that an index which
		...
		$tables = "
CREATE TABLE {$mndb->prefix}woocommerce_sessions (
...
CREATE TABLE {$mndb->prefix}woocommerce_api_keys (
...
CREATE TABLE {$mndb->prefix}woocommerce_attribute_taxonomies (
...
CREATE TABLE {$mndb->prefix}woocommerce_downloadable_product_permissions (
...
CREATE TABLE {$mndb->prefix}woocommerce_order_items (
...
CREATE TABLE {$mndb->prefix}woocommerce_order_itemmeta (
...
CREATE TABLE {$mndb->prefix}woocommerce_tax_rates (
...
CREATE TABLE {$mndb->prefix}woocommerce_tax_rate_locations (
...
CREATE TABLE {$mndb->prefix}woocommerce_shipping_zones (
  zone_id bigint(20) NOT NULL auto_increment,
  zone_name varchar(255) NOT NULL,
  zone_order bigint(20) NOT NULL,
  PRIMARY KEY  (zone_id)
) $collate;
CREATE TABLE {$mndb->prefix}woocommerce_shipping_zone_locations (
  location_id bigint(20) NOT NULL auto_increment,
...
CREATE TABLE {$mndb->prefix}woocommerce_shipping_zone_methods (
  zone_id bigint(20) NOT NULL,
...
CREATE TABLE {$mndb->prefix}woocommerce_payment_tokens (
...
CREATE TABLE {$mndb->prefix}woocommerce_payment_tokenmeta (
  meta_id bigint(20) NOT NULL auto_increment,
  ...
  KEY meta_key (meta_key($max_index_length))
		...
		if ( ! function_exists( 'get_term_meta' ) ) {
			$tables .= "
CREATE TABLE {$mndb->prefix}woocommerce_termmeta (
  meta_id bigint(20) NOT NULL auto_increment,
  ...
  KEY meta_key (meta_key($max_index_length))
	...
	public static function create_roles() {
		...
		foreach ( $capabilities as $cap_group ) {
			foreach ( $cap_group as $cap ) {
				$mn_roles->add_cap( 'shop_manager', $cap );
				$mn_roles->add_cap( 'administrator', $cap );
			}
		}
	}
	...
	public static function remove_roles() {
		...
		foreach ( $capabilities as $cap_group ) {
			foreach ( $cap_group as $cap ) {
				$mn_roles->remove_cap( 'shop_manager', $cap );
				$mn_roles->remove_cap( 'administrator', $cap );
			}
		}
	...
	private static function create_files() {
		...
		foreach ( $files as $file ) {
			if ( mn_mkdir_p( $file['base'] ) && ! file_exists( trailingslashit( $file['base'] ) . $file['file'] ) ) {
	...
	public static function in_plugin_update_message( $args ) {
		...
		if ( false === ( $upgrade_notice = get_transient( $transient_name ) ) ) {
			$response = mn_safe_remote_get( 'https://plugins.svn.wordpress.org/woocommerce/trunk/readme.txt' );

			if ( ! is_mn_error( $response ) && ! empty( $response['body'] ) ) {
				$upgrade_notice = self::parse_update_notice( $response['body'], $args['new_version'] );
				set_transient( $transient_name, $upgrade_notice, DAY_IN_SECONDS );
			}
		}

		echo mn_kses_post( $upgrade_notice );
	}
	...
	private static function parse_update_notice( $content, $new_version ) {
		...
		if ( preg_match( $regexp, $content, $matches ) ) {
			...
			if ( $version === $new_version && version_compare( WC_VERSION, $version, '<' ) ) {
				...
				foreach ( $notices as $index => $line ) {
					$upgrade_notice .= mn_kses_post( preg_replace( '~\[([^\]]*)\]\(([^\)]*)\)~', '<a href="${2}">${1}</a>', $line ) );
				}
		...
		return mn_kses_post( $upgrade_notice );
	}
	 ...
	 * @return string[]
	 */
	public static function mnmu_drop_tables( $tables ) {
		global $mndb;

		$tables[] = $mndb->prefix . 'woocommerce_sessions';
		$tables[] = $mndb->prefix . 'woocommerce_api_keys';
		$tables[] = $mndb->prefix . 'woocommerce_attribute_taxonomies';
		$tables[] = $mndb->prefix . 'woocommerce_downloadable_product_permissions';
		$tables[] = $mndb->prefix . 'woocommerce_termmeta';
		$tables[] = $mndb->prefix . 'woocommerce_tax_rates';
		$tables[] = $mndb->prefix . 'woocommerce_tax_rate_locations';
		$tables[] = $mndb->prefix . 'woocommerce_order_items';
		$tables[] = $mndb->prefix . 'woocommerce_order_itemmeta';
		...
		if ( ! empty( $plugin_to_install['repo-slug'] ) ) {
			...
			if ( ! $installed ) {
				...
				try {
					...
					if ( is_mn_error( $plugin_information ) ) {
