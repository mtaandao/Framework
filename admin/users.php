...
get_current_screen()->set_help_sidebar(
    ...
    '<p>' . __('<a href="https://mtaandao.co.ke/docs/Users_Screen" target="_blank">Documentation on Managing Users</a>') . '</p>' .
    '<p>' . __('<a href="https://mtaandao.co.ke/docs/Roles_and_Capabilities" target="_blank">Descriptions of Roles and Capabilities</a>') . '</p>' .
...
if ( empty($_REQUEST) ) {
	$referer = '<input type="hidden" name="mn_http_referer" value="'. esc_attr( mn_unslash( $_SERVER['REQUEST_URI'] ) ) . '" />';
} elseif ( isset($_REQUEST['mn_http_referer']) ) {
	$redirect = remove_query_arg(array('mn_http_referer', 'updated', 'delete_count'), mn_unslash( $_REQUEST['mn_http_referer'] ) );
	$referer = '<input type="hidden" name="mn_http_referer" value="' . esc_attr($redirect) . '" />';
} else {
	$redirect = 'users.php';
	$referer = '';
}
...
case 'promote':
	...
	if ( ! current_user_can( 'promote_users' ) )
		mn_die( __( 'You can&#8217;t edit that user.' ) );
	...
	foreach ( $userids as $id ) {
		...
		if ( $id == $current_user->ID && ! $mn_roles->role_objects[ $role ]->has_cap('promote_users')
	...
	mn_redirect(add_query_arg('update', $update, $redirect));
	exit();

case 'dodelete':
	if ( is_multisite() )
		mn_die( __('User deletion is not allowed from this screen.') );
	...
	if ( empty( $_REQUEST['delete_option'] ) ) {
		...
		$url = str_replace( '&amp;', '&', mn_nonce_url( $url, 'bulk-users' ) );
		mn_redirect( $url );
		exit;
	}

	if ( ! current_user_can( 'delete_users' ) )
		mn_die(__('You can&#8217;t delete users.'));
	...
	foreach ( $userids as $id ) {
		if ( ! current_user_can( 'delete_user', $id ) )
			mn_die(__( 'You can&#8217;t delete that user.' ) );
		...
		case 'delete':
			...
			break;
		case 'reassign':
			mn_delete_user( $id, $_REQUEST['reassign_user'] );
			break;
		}
		++$delete_count;
	}
...
case 'delete':
	...
	if ( ! current_user_can( 'delete_users' ) )
		$errors = new MN_Error( 'edit_users', __( 'You can&#8217;t delete users.' ) );
	...
	if ( $mndb->get_var( "SELECT ID FROM {$mndb->posts} WHERE post_author IN( " . implode( ',', $userids ) . " ) LIMIT 1" ) ) {
		$users_have_content = true;
	} elseif ( $mndb->get_var( "SELECT link_id FROM {$mndb->links} WHERE link_owner IN( " . implode( ',', $userids ) . " ) LIMIT 1" ) ) {
		$users_have_content = true;
	}
...
<?php mn_nonce_field('delete-users') ?>
<?php echo $referer; ?>
...
<?php if ( $go_delete ) :
	 ...
	 *
	 * @param MN_User $current_user MN_User object for the current user.
...
case 'doremove':
	...
	if ( ! is_multisite() )
		mn_die( __( 'You can&#8217;t remove users.' ) );
...
case 'remove':
	...
	if ( !current_user_can('remove_users') )
		$error = new MN_Error('edit_users', __('You can&#8217;t remove users.'));
...
?>
<form method="post" name="updateusers" id="updateusers">
<?php mn_nonce_field('remove-users') ?>
<?php echo $referer; ?>
...
default:
	...
	if ( !empty($_GET['_mn_http_referer']) ) {
		...
		case 'add':
			if ( isset( $_GET['id'] ) && ( $user_id = $_GET['id'] ) && current_user_can( 'edit_user', $user_id ) ) {
				...
				$messages[] = '<div id="message" class="updated notice is-dismissible"><p>' . sprintf( __( 'New user created. <a href="%s">Edit user</a>' ),
					esc_url( add_query_arg( 'mn_http_referer', urlencode( mn_unslash( $_SERVER['REQUEST_URI'] ) ),
...
<?php $mn_list_table->search_box( __( 'Search Users' ), 'user' ); ?>
