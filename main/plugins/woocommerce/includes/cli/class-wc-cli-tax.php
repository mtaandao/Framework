...
class WC_CLI_Tax extends WC_CLI_Command {
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc tax create --country=US --rate=5 --class=standard --type=percent
	 *
	 * @since 2.5.0
	 */
	public function create( $__, $assoc_args ) {
		...
		if ( $porcelain ) {
			...
			MN_CLI::success( "Created tax rate $id." );
		}
	}
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc tax create_class --name="Reduced Rate"
	 *
	 * @since 2.5.0
	 */
	public function create_class( $__, $assoc_args ) {
		try {
			...
			if ( $porcelain ) {
				...
				MN_CLI::success( "Created tax class $slug." );
			}
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc tax delete 123
	 *
	 *     mn wc tax delete $(mn wc tax list --format=ids)
	 *
	 * @since 2.5.0
	 */
	public function delete( $args, $assoc_args ) {
		...
		foreach ( $args as $tax_id ) {
			...
			if ( is_null( $tax ) ) {
				$exit_code += 1;
				MN_CLI::warning( "Failed deleting tax rate {$tax_id}." );
				continue;
			}
			...
			MN_CLI::success( "Deleted tax rate {$tax_id}." );
		}
		exit( $exit_code ? 1 : 0 );
	}
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc tax delete_class reduced-rate
	 *
	 *     mn wc tax delete_class $(mn wc tax list_class --format=ids)
	 *
	 * @since 2.5.0
	 */
	public function delete_class( $args, $assoc_args ) {
		...
		foreach ( $args as $slug ) {
			...
			if ( $deleted ) {
				MN_CLI::success( "Deleted tax class {$slug}." );
			} else {
				$exit_code += 1;
				MN_CLI::warning( "Failed deleting tax class {$slug}." );
			}
		}
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc tax get 123 --field=rate
	 *
	 *     mn wc tax get 321 --format=json > rate321.json
	 *
	 * @since 2.5.0
	 */
	 ...
	 *     mn wc tax list
	 *
	 *     mn wc tax list --field=id
	 *
	 *     mn wc tax list --fields=id,rate,class --format=json
	 *
	 * @since      2.5.0
	 * @subcommand list
	 */
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc tax list_class
	 *
	 *     mn wc tax list_class --field=slug
	 *
	 *     mn wc tax list_class --format=json
	 *
	 * @since      2.5.0
	...
	public function update( $args, $assoc_args ) {
		try {
			...
			MN_CLI::success( "Updated tax rate $id." );
		} catch ( WC_CLI_Exception $e ) {
			MN_CLI::error( $e->getMessage() );
		}
	}
	...
	/**
	 * Add common cli arguments to argument list before $mndb->get_results() is run.
	 *
	 * @since  2.5.0
	...
	protected function query_tax_rates( $args ) {
		...
		return $mndb->get_results( $query . $order_by . $pagination );
	}
