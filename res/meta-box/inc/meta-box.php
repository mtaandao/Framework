...
class RW_Meta_Box {
	...
	/**
	 * @var bool Used to prevent duplicated calls like revisions, manual hook to mn_insert_post, etc.
	 */
	public $saved = false;
	...
	public function __construct( $meta_box ) {
		...
		foreach ( $this->meta_box['post_types'] as $post_type ) {
			if ( 'attachment' === $post_type ) {
				...
				// @see mn_update_post(), mn_insert_attachment()
	...
	public function enqueue() {
		...
		mn_enqueue_style( 'rwmb', RWMB_CSS_URL . 'style.css', array(), RWMB_VER );
		if ( is_rtl() ) {
			mn_enqueue_style( 'rwmb-rtl', RWMB_CSS_URL . 'style-rtl.css', array(), RWMB_VER );
		}

		// Load clone script conditionally
		foreach ( $this->fields as $field ) {
			if ( $field['clone'] ) {
				mn_enqueue_script( 'rwmb-clone', RWMB_JS_URL . 'clone.js', array( 'jquery-ui-sortable' ), RWMB_VER, true );
				break;
			}
		}
		...
		if ( $this->meta_box['autosave'] ) {
			mn_enqueue_script( 'rwmb-autosave', RWMB_JS_URL . 'autosave.js', array( 'jquery' ), RWMB_VER, true );
		}
	...
	public function show() {
		...
		mn_nonce_field( "rwmb-save-{$this->meta_box['id']}", "nonce_{$this->meta_box['id']}" );
	 ...
	 * - If this function is called to prevent duplicated calls like revisions, manual hook to mn_insert_post, etc.
	 * - Autosave
	...
	protected function validate() {
		...
		return
			...
			&& mn_verify_nonce( $nonce, "rwmb-save-{$this->meta_box['id']}" );
	}
	...
	public static function normalize( $meta_box ) {
		...
		$meta_box = mn_parse_args( $meta_box, array(
	 ...
	 *
	 * @param MN_Screen $screen Screen object. Optional. Use current screen object by default.
	 * @return bool
	 */
