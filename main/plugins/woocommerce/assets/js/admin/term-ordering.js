...
jQuery( function( $ ) {
	...
	$( 'table.widefat.mn-list-table tbody th, table.widefat tbody td' ).css( 'cursor', 'move' );

	$( 'table.widefat.mn-list-table' ).sortable({
		...
		update: function( event, ui ) {
			...
			if ( ( prevtermid === undefined && nexttermid === undefined ) || ( nexttermid === undefined && nexttermparent === prevtermid ) || ( nexttermid !== undefined && prevtermparent === termid ) ) {
				$( 'table.widefat.mn-list-table' ).sortable( 'cancel' );
				return;
			}

			// Show Spinner
			ui.item.find( '.check-column input' ).hide().after( '<img alt="processing" src="images/mnspin_light.gif" class="waiting" style="margin-left: 6px;" />' );
