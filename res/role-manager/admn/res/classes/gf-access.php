...
class RM_GF_Access {
    ...
    public function __construct(RM_Lib $lib) {
        ...
        $this->lib = $lib;
        $this->user_meta_key = $mndb->prefix . 'rm_allow_gravity_forms';
...
?>        
        <h3><?php _e('Gravity Forms Restrictions', 'role-manager'); ?></h3>
<table class="form-table">
        		<tr>
        			<th scope="row"><?php esc_html_e('Allow access to forms with ID (comma separated) ', 'role-manager'); ?></th>
        			<td>
               <input type="text" name="rm_allow_gravity_forms" id="rm_allow_gravity_forms" value="<?php echo $allow_gravity_forms; ?>" size="40" />
        			</td>
        		</tr>
        </table>		        
...
<?php        
    ...
    public function save_user_allowed_forms_list($user_id) {
        ...
        if (isset($_POST['rm_allow_gravity_forms'])) {
            $gf_list = explode(',', trim($_POST['rm_allow_gravity_forms']));
            if (count($gf_list)>0) {
                $gf_id_arr = array();
    ...
    private function get_allowed_forms() {
        ...
        $allowed_forms_list = apply_filters('rm_get_allowed_gf_forms', $this->allowed_forms_list);
