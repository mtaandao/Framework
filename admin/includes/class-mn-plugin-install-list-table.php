<?php
/**
 * List Table API: MN_Plugin_Install_List_Table class
 *
 * @package Mtaandao
 ...
 * @see MN_List_Table
 */
class MN_Plugin_Install_List_Table extends MN_List_Table {
	...
	public function prepare_items() {
		...
		if ( $tab === 'beta' || false !== strpos( $GLOBALS['mn_version'], '-' ) ) {
	...
	public function views() {
		...
		/** This filter is documented in admin/inclues/class-mn-list-table.php */
...
<?php
	...
	public function display() {
		...
		if ( $singular ) {
			$data_attr = " data-mn-lists='list:$singular'";
		}

		$this->display_tablenav( 'top' );

?>
<div class="mn-list-table <?php echo implode( ' ', $this->get_table_classes() ); ?>">
<?php
...
<?php
	...
	public function display_rows() {
		...
		foreach ( (array) $this->items as $plugin ) {
			...
			$title = mn_kses( $plugin['name'], $plugins_allowedtags );

			// Remove any HTML from the description.
			$description = strip_tags( $plugin['short_description'] );
			$version = mn_kses( $plugin['version'], $plugins_allowedtags );

			$name = strip_tags( $title . ' ' . $version );

			$author = mn_kses( $plugin['author'], $plugins_allowedtags );
			if ( ! empty( $author ) ) {
					...
					case 'newer_installed':
						if ( is_plugin_active( $status['file'] ) ) {
							...
							$activate_url = add_query_arg( array(
								'_mnnonce'    => mn_create_nonce( 'activate-plugin_' . $status['file'] ),
								'action'      => 'activate',
								'plugin'      => $status['file'],
		...
		<div class="plugin-card plugin-card-<?php echo sanitize_html_class( $plugin['slug'] ); ?>">
			...
			<div class="plugin-card-bottom">
				<div class="vers column-rating">
					<?php mn_star_rating( array( 'rating' => $plugin['rating'], 'type' => 'percent', 'number' => $plugin['num_ratings'] ) ); ?>
				...
				<div class="column-compatibility">
					<?php
					if ( ! empty( $plugin['tested'] ) && version_compare( substr( $GLOBALS['mn_version'], 0, strlen( $plugin['tested'] ) ), $plugin['tested'], '>' ) ) {
						echo '<span class="compatibility-untested">' . __( 'Untested with your version of Mtaandao' ) . '</span>';
					} elseif ( ! empty( $plugin['requires'] ) && version_compare( substr( $GLOBALS['mn_version'], 0, strlen( $plugin['requires'] ) ), $plugin['requires'], '<' ) ) {
						echo '<span class="compatibility-incompatible">' . __( '<strong>Incompatible</strong> with your version of Mtaandao' ) . '</span>';
					} else {
						echo '<span class="compatibility-compatible">' . __( '<strong>Compatible</strong> with your version of Mtaandao' ) . '</span>';
					}
					?>
				</div>
			</div>
