...
class WC_Gateway_BACS extends WC_Payment_Gateway {
						...
						if ( $this->account_details ) {
							foreach ( $this->account_details as $account ) {
								...
								echo '<tr class="account">
									<td class="sort"></td>
									<td><input type="text" value="' . esc_attr( mn_unslash( $account['account_name'] ) ) . '" name="bacs_account_name[' . $i . ']" /></td>
									<td><input type="text" value="' . esc_attr( $account['account_number'] ) . '" name="bacs_account_number[' . $i . ']" /></td>
									<td><input type="text" value="' . esc_attr( mn_unslash( $account['bank_name'] ) ) . '" name="bacs_bank_name[' . $i . ']" /></td>
	...
	public function thankyou_page( $order_id ) {
		...
		if ( $this->instructions ) {
			echo mnautop( mntexturize( mn_kses_post( $this->instructions ) ) );
		}
	...
	public function email_instructions( $order, $sent_to_admin, $plain_text = false ) {
		...
		if ( ! $sent_to_admin && 'bacs' === $order->payment_method && $order->has_status( 'on-hold' ) ) {
			if ( $this->instructions ) {
				echo mnautop( mntexturize( $this->instructions ) ) . PHP_EOL;
			}
		...
		if ( ! empty( $bacs_accounts ) ) {
			...
			foreach ( $bacs_accounts as $bacs_account ) {
				...
				if ( $bacs_account->account_name || $bacs_account->bank_name ) {
					echo '<h3>' . mn_unslash( implode( ' - ', array_filter( array( $bacs_account->account_name, $bacs_account->bank_name ) ) ) ) . '</h3>' . PHP_EOL;
				}
				...
				foreach ( $account_fields as $field_key => $field ) {
					if ( ! empty( $field['value'] ) ) {
						echo '<li class="' . esc_attr( $field_key ) . '">' . esc_attr( $field['label'] ) . ': <strong>' . mntexturize( $field['value'] ) . '</strong></li>' . PHP_EOL;
					}
				}
