...
/** Mtaandao Administration Widgets API */
...
if ( 'on' == $widgets_access ) {
	add_filter( 'admin_body_class', 'mn_widgets_access_body_class' );
} else {
	mn_enqueue_script('admin-widgets');
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="https://mtaandao.co.ke/docs/Appearance_Widgets_Screen" target="_blank">Documentation on Widgets</a>') . '</p>' .
	'<p>' . __('<a href="https://mtaandao.co.ke/support/" target="_blank">Support Forums</a>') . '</p>'
);

if ( ! current_theme_supports( 'widgets' ) ) {
	mn_die( __( 'The theme you are currently using isn&#8217;t widget-aware, meaning that it has no sidebars that you are able to change. For information on making your theme widget-aware, please <a href="https://mtaandao.co.ke/docs/Widgetizing_Themes">follow these instructions</a>.' ) );
}
...
if ( empty( $sidebars_widgets ) )
	$sidebars_widgets = mn_get_widget_defaults();
...
if ( isset($_POST['savewidget']) || isset($_POST['removewidget']) ) {
	...
	if ( isset($_POST['removewidget']) && $_POST['removewidget'] ) {
		...
		if ( !in_array($widget_id, $sidebar, true) ) {
			mn_redirect( admin_url('widgets.php?error=0') );
			exit;
		}
	...
	mn_redirect( admin_url('widgets.php?message=0') );
	exit;
}

// Remove inactive widgets without js
if ( isset( $_POST['removeinactivewidgets'] ) ) {
	check_admin_referer( 'remove-inactive-widgets', '_mnnonce_remove_inactive_widgets' );
...
if ( isset($_GET['editwidget']) && $_GET['editwidget'] ) {
	...
	if ( isset($_GET['addnew']) ) {
		...
		$keys = array_keys( $mn_registered_sidebars );
		$sidebar = reset( $keys );

		if ( isset($_GET['base']) && isset($_GET['num']) ) { // multi-widget
			// Copy minimal info from an existing instance of this widget to a new instance
			foreach ( $mn_registered_widget_controls as $control ) {
				if ( $_GET['base'] === $control['id_base'] ) {
					$control_callback = $control['callback'];
					$multi_number = (int) $_GET['num'];
					$control['params'][0]['number'] = -1;
					$widget_id = $control['id'] = $control['id_base'] . '-' . $multi_number;
					$mn_registered_widget_controls[$control['id']] = $control;
					break;
				}
			}
		}
	...
	if ( isset($mn_registered_widget_controls[$widget_id]) && !isset($control) ) {
		$control = $mn_registered_widget_controls[$widget_id];
		$control_callback = $control['callback'];
	} elseif ( !isset($mn_registered_widget_controls[$widget_id]) && isset($mn_registered_widgets[$widget_id]) ) {
		$name = esc_html( strip_tags($mn_registered_widgets[$widget_id]['name']) );
	}
	...
	if ( !isset($sidebar) )
		$sidebar = isset($_GET['sidebar']) ? $_GET['sidebar'] : 'mn_inactive_widgets';
...
<?php
	foreach ( $mn_registered_sidebars as $sbname => $sbvalue ) {
		echo "\t\t<tr><td><label><input type='radio' name='sidebar' value='" . esc_attr($sbname) . "'" . checked( $sbname, $sidebar, false ) . " /> $sbvalue[name]</label></td><td>";
		if ( 'mn_inactive_widgets' == $sbname || 'orphaned_widgets' == substr( $sbname, 0, 16 ) ) {
			echo '&nbsp;';
		} else {
...
<?php	mn_nonce_field("save-delete-widget-$widget_id"); ?>
	<br class="clear" />
	</div>
	</form>
	</div>
...
foreach ( $mn_registered_sidebars as $sidebar => $registered_sidebar ) {
	if ( false !== strpos( $registered_sidebar['class'], 'inactive-sidebar' ) || 'orphaned_widgets' == substr( $sidebar, 0, 16 ) ) {
		$wrap_class = 'widgets-holder-wrap';
		if ( !empty( $registered_sidebar['class'] ) )
			$wrap_class .= ' ' . $registered_sidebar['class'];

		$is_inactive_widgets = 'mn_inactive_widgets' == $registered_sidebar['id'];
		?>
		<div class="<?php echo esc_attr( $wrap_class ); ?>">
			<div class="widget-holder inactive">
				<?php mn_list_widget_controls( $registered_sidebar['id'], $registered_sidebar['name'] ); ?>
				...
				<div class="remove-inactive-widgets">
					<form action="" method="post">
						<p>
							...
							if ( empty($sidebars_widgets['mn_inactive_widgets']) ) {
								$attributes['disabled'] = '';
							}
						...
						<?php mn_nonce_field( 'remove-inactive-widgets', '_mnnonce_remove_inactive_widgets' ); ?>
					</form>
				</div>
				<?php } ?>
			</div>
...
<div id="widgets-right" class="mn-clearfix<?php echo $single_sidebar_class; ?>">
...
foreach ( $theme_sidebars as $sidebar => $registered_sidebar ) {
	...
	?>
	<div class="<?php echo esc_attr( $wrap_class ); ?>">
		<?php mn_list_widget_controls( $sidebar, $registered_sidebar['name'] ); // Show the control forms for each of the widgets in this sidebar ?>
	</div>
	<?php
...
<form method="post">
<?php mn_nonce_field( 'save-sidebar-widgets', '_mnnonce_widgets', false ); ?>
</form>
<br class="clear" />
</div>
