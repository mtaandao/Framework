...
abstract class WC_Abstract_Order {
	...
	public function remove_order_items( $type = null ) {
		...
		if ( ! empty( $type ) ) {
			$mndb->query( $mndb->prepare( "DELETE FROM itemmeta USING {$mndb->prefix}woocommerce_order_itemmeta itemmeta INNER JOIN {$mndb->prefix}woocommerce_order_items items WHERE itemmeta.order_item_id = items.order_item_id AND items.order_id = %d AND items.order_item_type = %s", $this->id, $type ) );
			$mndb->query( $mndb->prepare( "DELETE FROM {$mndb->prefix}woocommerce_order_items WHERE order_id = %d AND order_item_type = %s", $this->id, $type ) );
		} else {
			$mndb->query( $mndb->prepare( "DELETE FROM itemmeta USING {$mndb->prefix}woocommerce_order_itemmeta itemmeta INNER JOIN {$mndb->prefix}woocommerce_order_items items WHERE itemmeta.order_item_id = items.order_item_id and items.order_id = %d", $this->id ) );
			$mndb->query( $mndb->prepare( "DELETE FROM {$mndb->prefix}woocommerce_order_items WHERE order_id = %d", $this->id ) );
		}
	}
	...
	public function get_items( $type = '' ) {
		...
		$items          = array();
		$get_items_sql  = $mndb->prepare( "SELECT order_item_id, order_item_name, order_item_type FROM {$mndb->prefix}woocommerce_order_items WHERE order_id = %d ", $this->id );
		$get_items_sql .= "AND order_item_type IN ( '" . implode( "','", array_map( 'esc_sql', $type ) ) . "' ) ORDER BY order_item_id;";
		$line_items     = $mndb->get_results( $get_items_sql );
	...
	public function get_tax_totals() {
		...
		if ( apply_filters( 'woocommerce_order_hide_zero_taxes', true ) ) {
			$amounts    = array_filter( mn_list_pluck( $tax_totals, 'amount' ) );
	...
	public function has_meta( $order_item_id ) {
		...
		return $mndb->get_results( $mndb->prepare( "SELECT meta_key, meta_value, meta_id, order_item_id
			FROM {$mndb->prefix}woocommerce_order_itemmeta WHERE order_item_id = %d
	...
	public function get_item_meta_array( $order_item_id ) {
		...
		$item_meta_array = mn_cache_get( $cache_key, 'orders' );

		if ( false === $item_meta_array ) {
			$item_meta_array = array();
			$metadata        = $mndb->get_results( $mndb->prepare( "SELECT meta_key, meta_value, meta_id FROM {$mndb->prefix}woocommerce_order_itemmeta WHERE order_item_id = %d ORDER BY meta_id", absint( $order_item_id ) ) );
			...
			mn_cache_set( $cache_key, $item_meta_array, 'orders' );
		}
	...
	public function get_item_downloads( $item ) {
		...
		$download_ids = $mndb->get_col( $mndb->prepare("
			SELECT download_id
			FROM {$mndb->prefix}woocommerce_downloadable_product_permissions
			WHERE user_email = %s
			AND order_key = %s
			AND product_id = %s
	...
	public function update_status( $new_status, $note = '', $manual = false ) {
		...
		if ( ! mn_update_post( $update_post_data ) ) {
