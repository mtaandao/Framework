...
class WC_REST_Authentication {
	...
	public function authenticate( $user_id ) {
		// Do not authenticate twice and check if is a request to our endpoint in the MN REST API.
	 ...
	 *
	 * @param MN_Error|null|bool $error
	 * @return MN_Error|null|bool
	 */
	...
	private function perform_basic_authentication() {
		...
		if ( ! hash_equals( $user->consumer_secret, $consumer_secret ) ) {
			$wc_rest_authentication_error = new MN_Error( 'woocommerce_rest_authentication_error', __( 'Consumer Secret is invalid.', 'woocommerce' ), array( 'status' => 401 ) );
	...
	private function perform_oauth_authentication() {
		...
		// Fetch MN user by consumer key
		$user = $this->get_user_data_by_consumer_key( $_GET['oauth_consumer_key'] );

		if ( empty( $user ) ) {
			$wc_rest_authentication_error = new MN_Error( 'woocommerce_rest_authentication_error', __( 'Consumer Key is invalid.', 'woocommerce' ), array( 'status' => 401 ) );
		...
		if ( is_mn_error( $wc_rest_authentication_error ) ) {
			return false;
		}
	...
	private function check_oauth_signature( $user, $params ) {
		...
		$mn_base      = get_home_url( null, '/', 'relative' );
		if ( substr( $request_path, 0, strlen( $mn_base ) ) === $mn_base ) {
			$request_path = substr( $request_path, strlen( $mn_base ) );
		}
		...
		if ( ! uksort( $params, 'strcmp' ) ) {
			return new MN_Error( 'woocommerce_rest_authentication_error', __( 'Invalid Signature - failed to sort parameters.', 'woocommerce' ), array( 'status' => 401 ) );
		}
		...
		if ( $params['oauth_signature_method'] !== 'HMAC-SHA1' && $params['oauth_signature_method'] !== 'HMAC-SHA256' ) {
			return new MN_Error( 'woocommerce_rest_authentication_error', __( 'Invalid Signature - signature method is invalid.', 'woocommerce' ), array( 'status' => 401 ) );
		}
		...
		if ( ! hash_equals( $signature, $consumer_signature ) ) {
			return new MN_Error( 'woocommerce_rest_authentication_error', __( 'Invalid Signature - provided signature does not match.', 'woocommerce' ), array( 'status' => 401 ) );
		}
	...
	private function check_oauth_timestamp_and_nonce( $user, $timestamp, $nonce ) {
		...
		if ( ( $timestamp < time() - $valid_window ) || ( $timestamp > time() + $valid_window ) ) {
			return new MN_Error( 'woocommerce_rest_authentication_error', __( 'Invalid timestamp.', 'woocommerce' ), array( 'status' => 401 ) );
		}
		...
		if ( in_array( $nonce, $used_nonces ) ) {
			return new MN_Error( 'woocommerce_rest_authentication_error', __( 'Invalid nonce - nonce has already been used.', 'woocommerce' ), array( 'status' => 401 ) );
		}
	...
	private function get_user_data_by_consumer_key( $consumer_key ) {
		...
		$user         = $mndb->get_row( $mndb->prepare( "
			SELECT key_id, user_id, permissions, consumer_key, consumer_secret, nonces
			FROM {$mndb->prefix}woocommerce_api_keys
		...
		switch ( $_SERVER['REQUEST_METHOD'] ) {
			...
			case 'GET' :
				if ( 'read' !== $permissions && 'read_write' !== $permissions ) {
					$wc_rest_authentication_error = new MN_Error( 'woocommerce_rest_authentication_error', __( 'The API key provided does not have read permissions.', 'woocommerce' ), array( 'status' => 401 ) );
					$valid = false;
				}
				break;
			...
			case 'DELETE' :
				if ( 'write' !== $permissions && 'read_write' !== $permissions ) {
					$wc_rest_authentication_error = new MN_Error( 'woocommerce_rest_authentication_error', __( 'The API key provided does not have write permissions.', 'woocommerce' ), array( 'status' => 401 ) );
					$valid = false;
				}
				break;
		}
	 ...
	 * @return MN_REST_Response
	 */
	public function send_unauthorized_headers( $response ) {
		global $wc_rest_authentication_error;

		if ( is_mn_error( $wc_rest_authentication_error ) && is_ssl() ) {
