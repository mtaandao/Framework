...
function wpb_getImageBySize( $params = array() ) {
	...
	if ( is_string( $thumb_size ) && ( ( ! empty( $_mn_additional_image_sizes[ $thumb_size ] ) && is_array( $_mn_additional_image_sizes[ $thumb_size ] ) ) || in_array( $thumb_size, array(
				'thumbnail',
				'thumb',
				'medium',
				'large',
		...
		$thumbnail = mn_get_attachment_image( $attach_id, $thumb_size, false, $attributes );
	} elseif ( $attach_id ) {
		...
		if ( is_array( $thumb_size ) ) {
			...
			$alt = trim( strip_tags( get_post_meta( $attach_id, '_mn_attachment_image_alt', true ) ) );
	...
	$p_img_large = mn_get_attachment_image_src( $attach_id, 'large' );
 ...
 * @return string
 */
function wpb_js_remove_mnautop( $content, $autop = false ) {

	if ( $autop ) { // Possible to use !preg_match('('.WPBMap::getTagsRegexp().')', $content)
		$content = mnautop( preg_replace( '/<\/?p\>/', "\n", $content ) . "\n" );
	}
...
if ( ! function_exists( 'shortcode_exists' ) ) {
	/**
	 * Check if a shortcode is registered in Mtaandao.
	 *
...
if ( ! function_exists( 'vc_siteAttachedImages' ) ) {
	...
	function vc_siteAttachedImages( $att_ids = array() ) {
		...
		foreach ( $media_images as $image_post ) {
			$thumb_src = mn_get_attachment_image_src( $image_post->ID, 'thumbnail' );
...
function fieldAttachedImages( $images = array() ) {
	...
	foreach ( $images as $image ) {
		if ( is_numeric( $image ) ) {
			$thumb_src = mn_get_attachment_image_src( $image, 'thumbnail' );
...
/*
* Resize images dynamically using mn built in functions
* Victor Teixeira
*
* php 5.2+
*
...
if ( ! function_exists( 'wpb_resize' ) ) {
	...
	function wpb_resize( $attach_id = null, $img_url = null, $width, $height, $crop = false ) {
		...
		$image_src = array();
		if ( $attach_id ) {
			$image_src = mn_get_attachment_image_src( $attach_id, 'full' );
		...
		if ( ! empty( $actual_file_path ) ) {
			...
			if ( $image_src[1] > $width || $image_src[2] > $height ) {
				...
				if ( false == $crop ) {
					...
					$proportional_size = mn_constrain_dimensions( $image_src[1], $image_src[2], $width, $height );
				...
				$img_editor = mn_get_image_editor( $actual_file_path );

				if ( is_mn_error( $img_editor ) || is_mn_error( $img_editor->resize( $width, $height, $crop ) ) ) {
					return array(
						'url' => '',
						'width' => '',
						'height' => '',
				...
				if ( is_mn_error( $img_editor->save( $new_img_path ) ) ) {
					return array(
						'url' => '',
						'width' => '',
						'height' => '',
...
function wpb_js_composer_check_version_schedule_deactivation() {
	...
	mn_clear_scheduled_hook( 'wpb_check_for_update' );
