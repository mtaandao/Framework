...
$help  = '<p>' . __( 'The left-hand navigation menu provides links to all of the Mtaandao administration screens, with submenu items displayed on hover. You can minimize this menu to a narrow icon strip by clicking on the Collapse Menu arrow at the bottom.' ) . '</p>';
$help .= '<p>' . __( 'Links in the Toolbar at the top of the screen connect your dashboard and the front end of your site, and provide access to your profile and helpful Mtaandao information.' ) . '</p>';
...
if ( current_user_can( 'edit_posts' ) )
	$help .= '<p>' . __( '<strong>At A Glance</strong> &mdash; Displays a summary of the content on your site and identifies which theme and version of Mtaandao you are using.' ) . '</p>';
...
<?php if ( has_action( 'welcome_panel' ) && current_user_can( 'edit_theme_options' ) ) :
	...
	$hide = 0 == $option || ( 2 == $option && mn_get_current_user()->user_email != get_option( 'admin_email' ) );
	if ( $hide )
		$classes .= ' hidden'; ?>

	<div id="welcome-panel" class="<?php echo esc_attr( $classes ); ?>">
		<?php mn_nonce_field( 'welcome-panel-nonce', 'welcomepanelnonce', false ); ?>
