...
mn.customize.selectiveRefresh = ( function( $, api ) {
	'use strict';
	 ...
	 *
	 * @see PHP class MN_Customize_Partial.
	 *
	 * @class
	...
	Partial = self.Partial = api.Class.extend({
		 ...
		 * @since 4.5.0
		 *
		 * @param {mn.customize.Value|string} setting  ID or object for setting.
		 ...
		 * @since 4.5.0
		 *
		 * @this {mn.customize.selectiveRefresh.Partial}
		...
		renderContent: function( placement ) {
			...
			try {
				...
				if ( mn.emoji && mn.emoji.parse && ! $.contains( document.head, placement.container[0] ) ) {
					content = mn.emoji.parse( content );
				}
	...
	self.Placement = Placement = api.Class.extend({
		 ...
		 *
		 * @param {mn.customize.selectiveRefresh.Partial}
		 */
		partial: null,
	 ...
	 * @since 4.5.0
	 *
	 * @type {Object.<string, mn.customize.selectiveRefresh.Partial>}
	 */
	 ...
	 *
	 * @since 4.5.0
	 * @see mn.customize.previewer.query()
	 *
	 * @return {object}
	 */
	 ...
	 *
	 * @since 4.5.0
	 * @type {Object<string, { deferred: jQuery.Promise, partial: mn.customize.selectiveRefresh.Partial }>}
	 */
	 ...
	 * @since 4.5.0
	 *
	 * @param {mn.customize.selectiveRefresh.Partial} partial
	 * @return {jQuery.Promise}
	 */
	self.requestPartial = function( partial ) {
		...
		self._debouncedTimeoutId = setTimeout(
			function() {
				...
				request = self._currentRequest = mn.ajax.send( null, {
					data: data,
	...
	api.bind( 'preview-ready', function() {
		 ...
		 *
		 * Note this is largely needed because adding a 'change' event handler to mn.customize
		 * will only include the changed setting object as an argument, not including the
		 * new value or the old value.
		 *
		 * @since 4.5.0
		 * @this {mn.customize.Setting}
		 *
		 ...
		 *
		 * @since 4.5.0
		 * @this {mn.customize.Values}
		 *
		 * @param {mn.customize.Setting} setting
		 */
