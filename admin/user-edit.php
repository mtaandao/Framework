...
require_once( dirname( __FILE__ ) . '/custom-avatar.php' );

mn_reset_vars( array( 'action', 'user_id', 'mn_http_referer' ) );
...
$profile_help = '<p>' . __('Your profile contains information about you (your &#8220;account&#8221;) as well as some personal options related to using Mtaandao.') . '</p>' .
	'<p>' . __('You can change your password, turn on keyboard shortcuts, change the color scheme of your Mtaandao administration screens, and turn off the WYSIWYG (Visual) editor, among other things. You can hide the Toolbar (formerly called the Admin Bar) from the front end of your site, however it cannot be disabled on the admin screens.') . '</p>' .
...
get_current_screen()->set_help_sidebar(
    ...
    '<p>' . __('<a href="https://mtaandao.co.ke/docs/Users_Your_Profile_Screen" target="_blank">Documentation on User Profiles</a>') . '</p>' .
    '<p>' . __('<a href="https://mtaandao.co.ke/support/" target="_blank">Support Forums</a>') . '</p>'
);

$mn_http_referer = remove_query_arg( array( 'update', 'delete_count', 'user_id' ), $mn_http_referer );
...
if ( is_multisite()
	...
	mn_die( __( 'Sorry, you are not allowed to edit this user.' ) );
}
...
if ( is_multisite() && IS_PROFILE_PAGE && isset( $_GET[ 'newuseremail' ] ) && $current_user->ID ) {
	...
	if ( $new_email && hash_equals( $new_email[ 'hash' ], $_GET[ 'newuseremail' ] ) ) {
		...
		if ( $mndb->get_var( $mndb->prepare( "SELECT user_login FROM {$mndb->signups} WHERE user_login = %s", $current_user->user_login ) ) ) {
			$mndb->query( $mndb->prepare( "UPDATE {$mndb->signups} SET user_email = %s WHERE user_login = %s", $user->user_email, $current_user->user_login ) );
		}
		mn_update_user( $user );
		delete_user_meta( $current_user->ID, '_new_email' );
		mn_redirect( add_query_arg( array( 'updated' => 'true' ), self_admin_url( 'profile.php' ) ) );
		die();
	} else {
		mn_redirect( add_query_arg( array( 'error' => 'new-email' ), self_admin_url( 'profile.php' ) ) );
	}
...
if ( is_multisite() ) {
	...
	if ( $user->user_login && isset( $_POST[ 'email' ] ) && is_email( $_POST[ 'email' ] ) && $mndb->get_var( $mndb->prepare( "SELECT user_login FROM {$mndb->signups} WHERE user_login = %s", $user->user_login ) ) ) {
		$mndb->query( $mndb->prepare( "UPDATE {$mndb->signups} SET user_email = %s WHERE user_login = %s", $_POST[ 'email' ], $user_login ) );
	}
}
...
if ( !is_mn_error( $errors ) ) {
	...
	if ( $mn_http_referer )
		$redirect = add_query_arg('mn_http_referer', urlencode($mn_http_referer), $redirect);
	mn_redirect($redirect);
	exit;
}
...
$sessions = MN_Session_Tokens::get_instance( $profileuser->ID );
...
<div id="message" class="updated notice is-dismissible">
	...
	<?php endif; ?>
	<?php if ( $mn_http_referer && false === strpos( $mn_http_referer, 'user-new.php' ) && ! IS_PROFILE_PAGE ) : ?>
	<p><a href="<?php echo esc_url( $mn_http_referer ); ?>"><?php _e('&larr; Back to Users'); ?></a></p>
	<?php endif; ?>
</div>
<?php endif; ?>
...
<?php endif; ?>
<?php if ( isset( $errors ) && is_mn_error( $errors ) ) : ?>
...
?>>
<?php mn_nonce_field('update-user_' . $user_id) ?>
<?php if ( $mn_http_referer ) : ?>
	<input type="hidden" name="mn_http_referer" value="<?php echo esc_url($mn_http_referer); ?>" />
<?php endif; ?>
<p>
...
<?php endif; ?>
<?php if ( count($_mn_admin_css_colors) > 1 && has_action('admin_color_scheme_picker') ) : ?>
...
<?php
endif; // $_mn_admin_css_colors
if ( !( IS_PROFILE_PAGE && !$user_can_edit ) ) : ?>
<tr class="user-comment-shortcuts-wrap">
<th scope="row"><?php _e( 'Keyboard Shortcuts' ); ?></th>
<td><label for="comment_shortcuts"><input type="checkbox" name="comment_shortcuts" id="comment_shortcuts" value="true" <?php if ( ! empty( $profileuser->comment_shortcuts ) ) checked( 'true', $profileuser->comment_shortcuts ); ?> /> <?php _e('Enable keyboard shortcuts for comment moderation.'); ?></label> <?php _e('<a href="https://mtaandao.co.ke/docs/Keyboard_Shortcuts" target="_blank">More information</a>'); ?></td>
</tr>
<?php endif; ?>
 ...
 * @since 2.7.0
 *
 * @param MN_User $profileuser The current MN_User object.
 */
...
<tr class="user-email-wrap">
	...
	<p><?php
		...
		printf(
			' <a href="%1$s">%2$s</a>',
			esc_url( mn_nonce_url( self_admin_url( 'profile.php?dismiss=' . $current_user->ID . '_new_email' ), 'dismiss-' . $current_user->ID . '_new_email' ) ),
			__( 'Cancel' )
		);
	?></p>
	</div>
...
<?php
	foreach ( mn_get_user_contact_methods( $profileuser ) as $name => $desc ) {
?>
	...
	<td>
		...
		<p class="description"><?php
			if ( IS_PROFILE_PAGE ) {
				...
				$description = sprintf( __( 'You can change your profile picture on <a href="%s">Gravatar</a> or upload a new one below..' ),
 ...
 * @param MN_User $profileuser User object for the current user to edit.
 */
...
<tr id="password" class="user-pass1-wrap">
	...
	<td>
		...
		<button type="button" class="button button-secondary mn-generate-pw hide-if-no-js"><?php _e( 'Generate Password' ); ?></button>
		<div class="mn-pwd hide-if-js">
			<span class="password-input-wrapper">
				<input type="password" name="pass1" id="pass1" class="regular-text" value="" autocomplete="off" data-pw="<?php echo esc_attr( mn_generate_password( 24 ) ); ?>" aria-describedby="pass-strength-result" />
			</span>
			<button type="button" class="button button-secondary mn-hide-pw hide-if-no-js" data-toggle="0" aria-label="<?php esc_attr_e( 'Hide password' ); ?>">
				<span class="dashicons dashicons-hidden"></span>
				<span class="text"><?php _e( 'Hide' ); ?></span>
			</button>
			<button type="button" class="button button-secondary mn-cancel-pw hide-if-no-js" data-toggle="0" aria-label="<?php esc_attr_e( 'Cancel password change' ); ?>">
