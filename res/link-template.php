<?php
/**
 * Mtaandao Link Template Functions
 *
 * @package Mtaandao
 * @subpackage Template
 */
 ...
 *
 * @param int|MN_Post $post Optional. Post ID or post object. Default is the global `$post`.
 */
function the_permalink( $post = 0 ) {
	 ...
	 * @param int|MN_Post $post      Post ID, MN_Post object, or 0. Default 0.
	 */
...
function user_trailingslashit($string, $type_of_url = '') {
	global $mn_rewrite;
	if ( $mn_rewrite->use_trailing_slashes )
	...
	if ( '' != $permalink && !in_array( $post->post_status, array( 'draft', 'pending', 'auto-draft', 'future' ) ) ) {
		...
		if ( strpos($permalink, '%category%') !== false ) {
			...
			if ( empty($category) ) {
				...
				if ( $default_category && ! is_mn_error( $default_category ) ) {
 ...
 * @return string|MN_Error The post permalink.
 */
function get_post_permalink( $id = 0, $leavename = false, $sample = false ) {
	...
	$post_link = $mn_rewrite->get_extra_permastruct($post->post_type);
 ...
 * @since 1.5.0
 *
 * @param int|MN_Post $post      Optional. Post ID or object. Default uses the global `$post`.
...
function _get_page_link( $post = false, $leavename = false, $sample = false ) {
	...
	$link = $mn_rewrite->get_page_permastruct();
 ...
 *
 * This can be used in the Mtaandao Loop or outside of it.
 *
 * @since 2.0.0
 *
 * @global MN_Rewrite $mn_rewrite
...
function get_attachment_link( $post = null, $leavename = false ) {
	...
	if ( $mn_rewrite->using_permalinks() && $parent ) {
	...
	} elseif ( $mn_rewrite->using_permalinks() && ! $leavename ) {
...
function get_year_link( $year ) {
	...
	$yearlink = $mn_rewrite->get_year_permastruct();
	if ( !empty($yearlink) ) {
...
function get_month_link($year, $month) {
	...
	$monthlink = $mn_rewrite->get_month_permastruct();
	if ( !empty($monthlink) ) {
...
function get_day_link($year, $month, $day) {
	...
	$daylink = $mn_rewrite->get_day_permastruct();
	if ( !empty($daylink) ) {
...
function get_feed_link( $feed = '' ) {
	...
	$permalink = $mn_rewrite->get_feed_permastruct();
	if ( '' != $permalink ) {
		if ( false !== strpos($feed, 'comments_') ) {
			$feed = str_replace('comments_', '', $feed);
			$permalink = $mn_rewrite->get_comment_feed_permastruct();
		}
 ...
 * @param MN_Term $tag    Optional. Term object. If null, the queried object will be inspected.
 *                        Default null.
 */
...
function get_search_link( $query = '' ) {
	...
	$permastruct = $mn_rewrite->get_search_permastruct();
...
function get_post_type_archive_link( $post_type ) {
	...
	if ( get_option( 'permalink_structure' ) && is_array( $post_type_obj->rewrite ) ) {
		...
		if ( $post_type_obj->rewrite['with_front'] )
			$struct = $mn_rewrite->front . $struct;
		else
			$struct = $mn_rewrite->root . $struct;
 ...
 * @since 4.4.0
 *
 * @param int|MN_Post $post         Optional. Post ID or `MN_Post` object. Defaults to global `$post`.
 ...
 *
 * Can be used within the Mtaandao loop or outside of it. Can be used with
 ...
 *
 * Can be used within the Mtaandao loop or outside of it, with any post type.
 *
 * @since 2.9.0
 *
 * @param int    $id           Optional. Post ID. Default is the ID of the global `$post`.
...
function get_delete_post_link( $id = 0, $deprecated = '', $force_delete = false ) {
	 ...
	 */
	return apply_filters( 'get_delete_post_link', mn_nonce_url( $delete_link, "$action-post_{$post->ID}" ), $post->ID, $force_delete );
}
 ...
 * @since 2.3.0
 *
 * @param int|MN_Comment $comment_id Optional. Comment ID or MN_Comment object.
 ...
 * @return null|string|MN_Post Post object if successful. Null if global $post is not set. Empty string if no
...
function get_adjacent_post( $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	...
	if ( $in_same_term || ! empty( $excluded_terms ) ) {
		...
		if ( $in_same_term ) {
			$join .= " INNER JOIN $mndb->term_relationships AS tr ON p.ID = tr.object_id INNER JOIN $mndb->term_taxonomy tt ON tr.term_taxonomy_id = tt.term_taxonomy_id";
			$where .= $mndb->prepare( "AND tt.taxonomy = %s", $taxonomy );

			if ( ! is_object_in_taxonomy( $post->post_type, $taxonomy ) )
				return '';
			$term_array = mn_get_object_terms( $post->ID, $taxonomy, array( 'fields' => 'ids' ) );

			// Remove any exclusions from the term array to include.
			$term_array = array_diff( $term_array, (array) $excluded_terms );
			$term_array = array_map( 'intval', $term_array );

			if ( ! $term_array || is_mn_error( $term_array ) )
				return '';
		...
		if ( ! empty( $excluded_terms ) ) {
			$where .= " AND p.ID NOT IN ( SELECT tr.object_id FROM $mndb->term_relationships tr LEFT JOIN $mndb->term_taxonomy tt ON (tr.term_taxonomy_id = tt.term_taxonomy_id) WHERE tt.term_id IN (" . implode( ',', array_map( 'intval', $excluded_terms ) ) . ') )';
		}
	}
	...
	if ( is_user_logged_in() ) {
		...
		foreach ( (array) $private_states as $state ) {
			if ( current_user_can( $read_private_cap ) ) {
				$where .= $mndb->prepare( " OR p.post_status = %s", $state );
			} else {
				$where .= $mndb->prepare( " OR (p.post_author = %d AND p.post_status = %s)", $user_id, $state );
			}
		}
		$where .= " )";
	} else {
	 ...
	 * @param MN_Post $post           MN_Post object.
	 */
	$where = apply_filters( "get_{$adjacent}_post_where", $mndb->prepare( "WHERE p.post_date $op %s AND p.post_type = %s $where", $current_post_date, $post->post_type ), $in_same_term, $excluded_terms, $taxonomy, $post );
	...
	$query = "SELECT p.ID FROM $mndb->posts AS p $join $where $sort";
	$query_key = 'adjacent_post_' . md5( $query );
	$result = mn_cache_get( $query_key, 'counts' );
	if ( false !== $result ) {
		if ( $result )
	...
	$result = $mndb->get_var( $query );
	if ( null === $result )
		$result = '';

	mn_cache_set( $query_key, $result, 'counts' );
 ...
 *
 * This is meant to be attached to actions like 'mn_head'. Do not call this directly in plugins
 * or theme templates.
 *
 * @since 3.0.0
 *
 ...
 */
function adjacent_posts_rel_link_mn_head() {
...
function get_adjacent_post_link( $format, $link, $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	 ...
	 * @param string  $format   Link anchor format.
	 * @param string  $link     Link permalink format.
	 * @param MN_Post $post     The adjacent post.
...
function get_pagenum_link($pagenum = 1, $escape = true ) {
	...
	if ( !$mn_rewrite->using_permalinks() || is_admin() ) {
		...
		$request = preg_replace( "|$mn_rewrite->pagination_base/\d+/?$|", '', $request);
		$request = preg_replace( '|^' . preg_quote( $mn_rewrite->index, '|' ) . '|i', '', $request);
		$request = ltrim($request, '/');

		$base = trailingslashit( get_bloginfo( 'url' ) );

		if ( $mn_rewrite->using_index_permalinks() && ( $pagenum > 1 || '' != $request ) )
			$base .= $mn_rewrite->index . '/';

		if ( $pagenum > 1 ) {
			$request = ( ( !empty( $request ) ) ? trailingslashit( $request ) : $request ) . user_trailingslashit( $mn_rewrite->pagination_base . "/" . $pagenum, 'paged' );
		}
...
function get_next_posts_link( $label = null, $max_page = 0 ) {
	global $paged, $mn_query;

	if ( !$max_page )
		$max_page = $mn_query->max_num_pages;
...
function get_posts_nav_link( $args = array() ) {
	...
	if ( !is_singular() ) {
		...
		$max_num_pages = $mn_query->max_num_pages;
 ...
 * @since 4.1.0
 *
 * @global MN_Query $mn_query Mtaandao Query object.
 *
 * @param array $args {
	...
	if ( 'newest' == get_option('default_comments_page') ) {
		if ( $pagenum != $max_page ) {
			if ( $mn_rewrite->using_permalinks() )
				$result = user_trailingslashit( trailingslashit($result) . $mn_rewrite->comments_pagination_base . '-' . $pagenum, 'commentpaged');
			else
...
function get_next_comments_link( $label = '', $max_page = 0 ) {
	...
	if ( empty($max_page) )
		$max_page = $mn_query->max_num_comment_pages;
...
function paginate_comments_links( $args = array() ) {
	...
	if ( $mn_rewrite->using_permalinks() )
		$defaults['base'] = user_trailingslashit(trailingslashit(get_permalink()) . $mn_rewrite->comments_pagination_base . '-%#%', 'commentpaged');
...
function get_shortcut_link() {
	...
	if ( $is_IE ) {
		...
		if ( ! empty( $ua ) && preg_match( '/\bMSIE (\d)/', $ua, $matches ) && (int) $matches[1] <= 8 ) {
			$url = mn_json_encode( admin_url( 'press-this.php' ) );
...
/**
 * Retrieves the URL for the current site where Mtaandao application files
 * (e.g. main-header.php or the admin/ folder) are accessible.
 *
...
/**
 * Retrieves the URL for a given site where Mtaandao application files
 * (e.g. main-header.php or the admin/ folder) are accessible.
 *
 * Returns the 'site_url' option with the appropriate protocol, 'https' if
 * is_ssl() and 'http' otherwise. If `$scheme` is 'http' or 'https',
...
function plugins_url( $path = '', $plugin = '' ) {
	...
	$path = mn_normalize_path( $path );
	$plugin = mn_normalize_path( $plugin );
 ...
 *                      been published yet.
 */
function mn_get_canonical_url( $post = null ) {
	$post = get_post( $post );
 ...
 *                are not enabled.
 */
function mn_get_shortlink( $id = 0, $context = 'post', $allow_slugs = true ) {
	/**
 ...
 *
 * Attached to the {@see 'mn_head'} action.
 *
 * @since 3.0.0
 */
function mn_shortlink_mn_head() {
	$shortlink = mn_get_shortlink( 0, 'query' );
 ...
 *
 * Attached to the {@see 'mn'} action.
 *
 * @since 3.0.0
 */
function mn_shortlink_header() {
	if ( headers_sent() )
		return;
...
function the_shortlink( $text = '', $title = '', $before = '', $after = '' ) {
	...
	$shortlink = mn_get_shortlink( $post->ID );
...
function get_avatar_data( $id_or_email, $args = null ) {
	...
	if ( is_numeric( $id_or_email ) ) {
	...
	} elseif ( $id_or_email instanceof MN_User ) {
		// User Object
		$user = $id_or_email;
	} elseif ( $id_or_email instanceof MN_Post ) {
		// Post Object
		$user = get_user_by( 'id', (int) $id_or_email->post_author );
	} elseif ( $id_or_email instanceof MN_Comment ) {
		/**
		...
		if ( ( ! $user || is_mn_error( $user ) ) && ! empty( $id_or_email->comment_author_email ) ) {
