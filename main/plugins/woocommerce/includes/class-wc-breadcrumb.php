...
class WC_Breadcrumb {
	...
	private function add_crumbs_product_category() {
		$current_term = $GLOBALS['mn_query']->get_queried_object();
	...
	private function add_crumbs_category() {
		$this_category = get_category( $GLOBALS['mn_query']->get_queried_object() );
	...
	private function add_crumbs_tag() {
		$queried_object = $GLOBALS['mn_query']->get_queried_object();
	...
	private function add_crumbs_tax() {
		$this_term = $GLOBALS['mn_query']->get_queried_object();
	...
	private function term_ancestors( $term_id, $taxonomy ) {
		...
		foreach ( $ancestors as $ancestor ) {
			...
			if ( ! is_mn_error( $ancestor ) && $ancestor ) {
