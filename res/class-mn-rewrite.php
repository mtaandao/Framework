 ...
 *
 * The Mtaandao Rewrite class writes the rewrite module rules to the .htaccess
 * file. It also handles parsing the request to get the correct setup for the
 * Mtaandao Query class.
 *
 * The Rewrite along with MN class function as a front controller for Mtaandao.
...
class MN_Rewrite {
	 ...
	 * `MN_Rewrite::$index` with a trailing slash appended. Otherwise the root
	 * will be empty.
	 *
	 * @since 1.5.0
	 * @access public
	 ...
	 *
	 * @see MN_Rewrite::init()
	 * @see MN_Rewrite::using_index_permalinks()
	 */
	public $root = '';
	...
	/**
	 * Rules that don't redirect to Mtaandao' index.php.
	 *
	...
	/**
	 * Whether to write every mod_rewrite rule for Mtaandao into the .htaccess file.
	 *
	 ...
	 * @var bool
	 *
	 * @see MN_Rewrite::mod_rewrite_rules()
	 */
	 ...
	 * set to true. Prior to Mtaandao 3.3 this flag indicated that every page
	 * would have a set of rules added to the top of the rewrite rules array.
	 * Now it tells MN::parse_request() to check if a URL matching the page
	 ...
	 * `MN_Rewrite::$rewritereplace` and are rewritten to the query
	 * variables listed in MN_Rewrite::$queryreplace.
	 *
	 ...
	 * of rewrite tags, see MN_Rewrite::$rewritecode.
	 *
	 * @since 1.5.0
	 * @access private
	 * @var array
	...
	/**
	 * Query variables that rewrite tags map to, see MN_Rewrite::$rewritecode.
	 *
	 * @since 1.5.0
	 * @access private
	 * @var array
	...
	public function page_uri_index() {
		...
		$pages = $mndb->get_results("SELECT ID, post_name, post_parent FROM $mndb->posts WHERE post_type = 'page' AND post_status != 'auto-draft'");
		...
		foreach ( $posts as $id => $post ) {
			// URL => page name
			$uri = get_page_uri($id);
			$attachments = $mndb->get_results( $mndb->prepare( "SELECT ID, post_name, post_parent FROM $mndb->posts WHERE post_type = 'attachment' AND post_parent = %d", $id ));
	 ...
	 * @access public
	 *
	 * @see MN_Rewrite::$rewritecode
	 * @see MN_Rewrite::$rewritereplace
	 * @see MN_Rewrite::$queryreplace
	 *
	 ...
	 *
	 * The main MN_Rewrite function for building the rewrite rule list. The
	 ...
	 *
	 * Shorten version of MN_Rewrite::generate_rewrite_rules() that allows for shorter
	 ...
	 * @access public
	 *
	 * @see MN_Rewrite::generate_rewrite_rules() See for long description and rest of parameters.
	 *
	 ...
	 * reference to the current MN_Rewrite instance to further manipulate the
	...
	public function rewrite_rules() {
		...
		$deprecated_files = array(
			'.*mn-(atom|rdf|rss|rss2|feed|commentsrss2)\.php$' => $this->index . '?feed=old',
			'.*mn-app\.php(/.*)?$' => $this->index . '?error=403',
		);

		// Registration rules.
		$registration_pages = array();
		if ( is_multisite() && is_main_site() ) {
			$registration_pages['.*register.php$'] = $this->index . '?signup=true';
			$registration_pages['.*activate.php$'] = $this->index . '?activate=true';
		}

		// Deprecated.
		$registration_pages['.*mn-register.php$'] = $this->index . '?register=true';
		 ...
		 * @since 1.5.0
		 *
		 * @param MN_Rewrite $this Current MN_Rewrite instance, passed by reference.
		 */
	 ...
	 *
	 * The difference between this method and MN_Rewrite::rewrite_rules() is that
	 ...
	 */
	public function mn_rewrite_rules() {
	 ...
	 *
	 * Will add the non_mn_rules property rules to the .htaccess file before
	 * the Mtaandao rewrite rules one.
	 *
	 * @since 1.5.0
	 * @access public
	 *
	...
	public function mod_rewrite_rules() {
		...
		// Add in the rules that don't redirect to MN's index.php (and thus shouldn't be handled by MN at all).
		foreach ( (array) $this->non_mn_rules as $match => $query) {
	...
	<system.webServer>
		...
		$rules .= '
			<rule name="Mtaandao: ' . esc_attr( home_url() ) . '" patternSyntax="Wildcard">
				<match url="*" />
					<conditions>
	 ...
	 * query strings. The new permastruct is added to the MN_Rewrite::$extra_permastructs array.
	 *
	 * When the rewrite rules are built by MN_Rewrite::rewrite_rules(), all of these extra
	 * permastructs are passed to MN_Rewrite::generate_rewrite_rules() which transforms them
	 * into the regular expressions that many love to hate.
	 *
	 * The `$args` parameter gives you control over how MN_Rewrite::generate_rewrite_rules()
	 ...
	 *     Default empty array.
	 *
	 *     @type bool $with_front  Whether the structure should be prepended with `MN_Rewrite::$front`.
	 *                             Default true.
	 ...
	 *
	 * Calls MN_Rewrite::mn_rewrite_rules() after removing the 'rewrite_rules' option.
	...
	public function init() {
		$this->extra_rules = $this->non_mn_rules = $this->endpoints = array();
	 ...
	 * MN_Rewrite::init() after the option is updated.
	 *
	 ...
	 * the current category base and the parameter value. Calls MN_Rewrite::init()
	 ...
	 * current tag base and the parameter value. Calls MN_Rewrite::init() after
