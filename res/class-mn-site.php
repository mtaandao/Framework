...
final class MN_Site {
	 ...
	 *
	 * Handling for this does not exist throughout Mtaandao core, but custom
	 ...
	 * @return MN_Site|false The site's object if found. False if not.
	 */
	public static function get_instance( $site_id ) {
		...
		$_site = mn_cache_get( $site_id, 'sites' );

		if ( ! $_site ) {
			$_site = $mndb->get_row( $mndb->prepare( "SELECT * FROM {$mndb->blogs} WHERE blog_id = %d LIMIT 1", $site_id ) );

			if ( empty( $_site ) || is_mn_error( $_site ) ) {
				return false;
			}

			mn_cache_add( $site_id, $_site, 'sites' );
		}

		return new MN_Site( $_site );
	}

	/**
	 * Creates a new MN_Site object.
	 *
	 ...
	 * @access public
	 *
	 * @param MN_Site|object $site A site object.
	 */
	...
	private function get_details() {
		...
		if ( false === $details ) {
			...
			if ( $cache_details ) {
				mn_cache_set( $this->blog_id, $details, 'site-details' );
			}
		}
