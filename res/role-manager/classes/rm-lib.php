<?php
/*
 * Stuff specific for Role Manager Mtaandao plugin
 * Author: Mtaandao
 ...
 */
class RM_Lib extends RM_Base_Lib {
	...
	public    $rm_object = 'role';  // what to process, 'role' or 'user'  
 ...
 protected $bbpress = null; // reference to the RM_bbPress class instance
 ...
 // Role_Manager::allow_add_user_as_superadmin()
 ...
 public $debug = false;
    ...
    public function __construct($options_id) {
        ...
        $this->debug = defined('RM_DEBUG') && (RM_DEBUG==1 || RM_DEBUG==true);
 
        $this->bbpress = RM_bbPress::get_instance($this);
    ...
    public static function get_instance($options_id = '') {
        ...
        if (self::$instance === null) {
            if (empty($options_id)) {
                throw new Exception('RM_Lib::get_inctance() - Error: plugin options ID string is required');
            }
            // new static() will work too
            self::$instance = new RM_Lib($options_id);
        }
    ...
    protected function upgrade() {
        ...
        $rm_version = $this->get_option('rm_version', '0');
        if (version_compare( $rm_version, RM_VERSION, '<' ) ) {
            // for upgrade to 4.18 and higher from older versions
            $this->init_rm_caps();
            $this->put_option('rm_version', RM_VERSION, true);
        }
    ...
    public function get_rm_object() {
        
        return $this->rm_object;
    }
    // end of get_rm_object();
    
    
    protected function get_rm_caps() {
        
        $rm_caps = array(
            'rm_edit_roles' => 1,
            'rm_create_roles' => 1,
            'rm_delete_roles' => 1,
            'rm_create_capabilities' => 1,
            'rm_delete_capabilities' => 1,
            'rm_manage_options' => 1,
            'rm_reset_roles' => 1
        );        
    ...
    public function init_rm_caps() {
        ...
        // Do not turn on RM caps for local administrator by default under multisite, as there is a superadmin.
        $turn_on = !$this->multisite;   
        
        $old_use_db = $mn_roles->use_db;
        $mn_roles->use_db = true;
        $administrator = $mn_roles->role_objects['administrator'];
        $rm_caps = $this->get_rm_caps();
        foreach(array_keys($rm_caps) as $cap) {
    ...
    /**
     * get options for Role Manager plugin
     * Role Manager stores its options at the main blog/site only and applies them to the all network
     * 
     */
    protected function init_options($options_id) {
        ...
        if ($this->multisite) { 
            if ( ! function_exists( 'is_plugin_active_for_network' ) ) {    // Be srm the function is defined before trying to use it
                require_once( ABSPATH . '/admin/res/plugin.php' );                
            }
            $this->active_for_network = is_plugin_active_for_network(RM_PLUGIN_BASE_NAME);
        }
        $current_blog = $mndb->blogid;
        if ($this->multisite && $current_blog!=$this->main_blog_id) {   
            if ($this->active_for_network) {   // plugin is active for whole network, so get RM options from the main blog
    ...
    public function flush_options() {
        ...
        if ($this->multisite && $current_blog!==$this->main_blog_id) {
            if ($this->active_for_network) {   // plugin is active for whole network, so get RM options from the main blog
                switch_to_blog($this->main_blog_id);  // in order to save RM options to the main blog
            }
        }
    ...
    /**
     * return key capability to have access to Role Manager Plugin
     * 
     * @return string
     */
    public function get_key_capability() {
        ...
        if (!$this->multisite) {
            $key_capability = RM_KEY_CAPABILITY;
        } else {
    ...
    public function get_settings_capability() {
        ...
        if (!$this->multisite) {
                $settings_access = 'rm_manage_options';
        } else {
            $enable_simple_admin_for_multisite = $this->get_option('enable_simple_admin_for_multisite', 0);
            if ( (defined('RM_ENABLE_SIMPLE_ADMIN_FOR_MULTISITE') && RM_ENABLE_SIMPLE_ADMIN_FOR_MULTISITE == 1) || 
                 $enable_simple_admin_for_multisite) {
    ...
    public function editor() {
        ...
        if (!$this->editor_init0()) {
            $this->show_message(esc_html__('Error: wrong request', 'role-manager'));
            return false;
        }                
    ...
    protected function advertisement() {
        ...
        if (!$this->is_pro()) {
            $this->advert = new RM_Advertisement();
            $this->advert->display();
        }
    }
...
<script language="javascript" type="text/javascript">
  ...
  var rm_current_role = '<?php echo $this->current_role; ?>';
  var rm_current_role_name  = '<?php echo $this->current_role_name; ?>';

</script>

<!-- popup dialogs markup -->
<div id="rm_add_role_dialog" class="rm-modal-dialog" style="padding: 10px;">
  <form id="rm_add_role_form" name="rm_add_role_form" method="POST">    
    <div class="rm-label"><?php esc_html_e('Role name (ID): ', 'role-manager'); ?></div>
    <div class="rm-input"><input type="text" name="user_role_id" id="user_role_id" size="25"/></div>
    <div class="rm-label"><?php esc_html_e('Display Role Name: ', 'role-manager'); ?></div>
    <div class="rm-input"><input type="text" name="user_role_name" id="user_role_name" size="25"/></div>
    <div class="rm-label"><?php esc_html_e('Make copy of: ', 'role-manager'); ?></div>
    <div class="rm-input"><?php echo $this->role_to_copy_html; ?></div>        
  </form>
</div>

<div id="rm_rename_role_dialog" class="rm-modal-dialog" style="padding: 10px;">
  <form id="rm_rename_role_form" name="rm_rename_role_form" method="POST">
    <div class="rm-label"><?php esc_html_e('Role name (ID): ', 'role-manager'); ?></div>
    <div class="rm-input"><input type="text" name="ren_user_role_id" id="ren_user_role_id" size="25" disabled /></div>
    <div class="rm-label"><?php esc_html_e('Display Role Name: ', 'role-manager'); ?></div>
    <div class="rm-input"><input type="text" name="ren_user_role_name" id="ren_user_role_name" size="25"/></div>    
  </form>
</div>

<div id="rm_delete_role_dialog" class="rm-modal-dialog">
  <div style="padding:10px;">
    <div class="rm-label"><?php esc_html_e('Select Role:', 'role-manager');?></div>
    <div class="rm-input"><?php echo $this->role_delete_html; ?></div>
  </div>
</div>
...
?>
<div id="rm_default_role_dialog" class="rm-modal-dialog">
  <div style="padding:10px;">
...
<div id="rm_delete_capability_dialog" class="rm-modal-dialog">
  <div style="padding:10px;">
    <div class="rm-label"><?php esc_html_e('Delete:', 'role-manager');?></div>
    <div class="rm-input"><?php echo $this->capability_remove_html; ?></div>
  </div>  
</div>

<div id="rm_add_capability_dialog" class="rm-modal-dialog">
  <div style="padding:10px;">
    <div class="rm-label"><?php esc_html_e('Capability name (ID): ', 'role-manager'); ?></div>
    <div class="rm-input"><input type="text" name="capability_id" id="capability_id" size="25"/></div>
  </div>  
</div>     
...
?>
<div id="rm_confirmation_dialog" class="rm-modal-dialog">
    <div id="rm_cd_html" style="padding:10px;"></div>
</div>
<?php
    }
    ...
    $container_width = ($this->rm_object == 'user') ? 1400 : 1200;
    
    $this->show_message($this->notification);
?>
<div class="wrap">
		  <div id="rm-icon" class="icon32"><br/></div>
    <h1><?php _e('Role Manager', 'role-manager'); ?></h1>
    <div id="rm_container" style="min-width: <?php echo $container_width;?>px;">
...
?>            
        ...
        <div class="has-sidebar" >
            <form id="rm_form" method="post" action="<?php echo RM_MN_ADMIN_URL . RM_PARENT.'?page=users-'.RM_PLUGIN_FILE;?>" >			
                <div id="rm_form_controls">
<?php
                    mn_nonce_field('role-manager', 'rm_nonce');
                    if ($this->rm_object == 'user') {
                        require_once( ABSPATH . RES . '/role-manager/rm-user-edit.php');
                    } else {
                        $this->set_current_role();
                        $this->role_edit_prepare_html();
                        require_once( ABSPATH . RES . '/role-manager/rm-role-edit.php');
                    }
?>
...
<?php
	...
	// content of Mtaandao Role Manager advertisement slot - for direct call
	protected function advertise_pro_version() {
		if ($this->is_pro()) {
			return;
		}
?>		
			<div id="rm_pro_advertisement" style="clear:left;display:block; float: left;">
				<a href="https://www.mtaandao.co.ke?utm_source=UserRoleEditor&utm_medium=banner&utm_campaign=Plugins " target="_new" >
<?php 
	if ($this->hide_pro_banner) {
		echo 'Mtaandao Role Manager: extended functionality, no advertisement - from $29.</a>';
	} else {
?>
					<img src="<?php echo RES;?>images/mn-role-manager-728x90.jpg" alt="Mtaandao Role Manager" 
						 title="More functionality and premium support with Pro version of Role Manager."/>
				</a><br />
				<label for="rm_hide_pro_banner">
					<input type="checkbox" name="rm_hide_pro_banner" id="rm_hide_pro_banner" onclick="rm_hide_pro_banner();"/>&nbsp;Thanks, hide this banner.
				</label>
<?php 
	}
?>
...
<?php		
    ...
    protected function init_current_role_name() {
        ...
        if (!isset($this->roles[$_POST['user_role']])) {
            $mess = esc_html__('Error: ', 'role-manager') . esc_html__('Role', 'role-manager') . ' <em>' . esc_html($_POST['user_role']) . '</em> ' . 
                    esc_html__('does not exist', 'role-manager');
            $this->current_role = '';
    ...
    protected function prepare_capabilities_to_save() {
        ...
        foreach ($this->full_capabilities as $available_capability) {
            $cap_id = str_replace(' ', RM_SPACE_REPLACER, $available_capability['inner']);
    ...
    protected function permissions_object_update($mess) {
        ...
        if ($this->rm_object == 'role') {  // save role changes to database
            if ($this->update_roles()) {
                if ($mess) {
                    $mess .= '<br/>';
                }
                if (!$this->apply_to_all) {
                    $mess = esc_html__('User role updated', 'role-manager');
                } else {
                    $mess = esc_html__('Roles are updated for all network', 'role-manager');
                }
            } else {
                if ($mess) {
                    $mess .= '<br/>';
                }
                $mess = esc_html__('Error occrmd during role(s) update', 'role-manager');
            }
        } else {
            if ($this->update_user($this->user_to_edit)) {
                ...
                $mess = esc_html__('User capabilities are updated successfully', 'role-manager');
            } else {
                if ($mess) {
                    $mess .= '<br/>';
                }
                $mess = esc_html__('Error occrmd during user update', 'role-manager');
            }
        }
        return $mess;
    }
    ...
    protected function process_user_request() {
        ...
        if (isset($_POST['action'])) {
            ...
            if ($action == 'reset') {
                ...
                set_site_transient( 'rm_caps_readable', $this->caps_readable, 600 );
                ...
                set_site_transient( 'rm_show_deprecated_caps', $this->show_deprecated_caps, 600 );
            } else if ($action == 'hide-admn-banner') {
                $this->hide_pro_banner = 1;
                $this->put_option('rm_hide_pro_banner', 1);	
                $this->flush_options();				
                ...
                $this->notification = esc_html__('User Roles are restored to Mtaandao default values. ', 'role-manager');
                ...
                do_action('rm_process_user_request');
            } // if ($action
        }
    ...
    if (isset($_POST['rm_apply_to_all'])) {
        $this->apply_to_all = 1;
    } else {
        $this->apply_to_all = 0;
    }
    ...
    protected function editor_init0() {
        $this->caps_readable = get_site_transient('rm_caps_readable');
        if (false === $this->caps_readable) {
            $this->caps_readable = $this->get_option('rm_caps_readable');
            set_site_transient('rm_caps_readable', $this->caps_readable, 600);
        }
        $this->show_deprecated_caps = get_site_transient('rm_show_deprecated_caps');
        if (false === $this->show_deprecated_caps) {
            $this->show_deprecated_caps = $this->get_option('rm_show_deprecated_caps');
            set_site_transient('rm_caps_readable', $this->caps_readable, 600);
        }

        $this->hide_pro_banner = $this->get_option('rm_hide_pro_banner', 0);
        $this->get_default_role();

        // could be sent as by POST, as by GET
        if (isset($_REQUEST['object'])) {
            $this->rm_object = $_REQUEST['object'];
    ...
    public function editor_init1() {
        ...
        if (empty($this->role_additional_options)) {
            $this->role_additional_options = RM_Role_Additional_Options::get_instance($this);
        }
        
        if (!$this->is_pro()) {
            require_once(RM_PLUGIN_DIR . 'res/classes/advertisement.php');
        }
    ...
    public function has_administrator_role($user_id) {
        ...
        // cache checking result for the futrm use
    ...
    /**
     * Checks if user is allowed to use Role Manager
     * 
    ...
    public function user_is_admin($user_id = false) {
        ...
        $rm_key_capability = $this->get_key_capability();
        if (empty($user_id)) {                    
            $user_id = $current_user->ID;
        }
        $result = user_can($user_id, $rm_key_capability);
...
/*    
    ...
    protected function restore_user_roles() 
    {
        ...
        $error_message = 'Error! ' . __('Database operation error. Check log file.', 'role-manager');
...
?>    
            ...
            $mess = __('No backup data. It is created automatically before the first role data update.', 'role-manager');
        }
...
*/
    ...
    /**
     * return array of built-in MN capabilities (MN 3.1 admin/res/schema.php) 
     * 
     * @return array 
     */
    public function get_built_in_mn_caps() {
        ...
        $caps['update_designs'] = 1;
        $caps['install_designs'] = 1;
        $caps['update_core'] = 1;
        $caps['list_users'] = 1;
        $caps['remove_users'] = 1;
                
        if (version_compare($mn_version, '4.4', '<')) {
            $caps['add_users'] = 1;  // removed from MN v. 4.4.
        }
        
        $caps['promote_users'] = 1;
        $caps['edit_design_options'] = 1;
        $caps['delete_designs'] = 1;
        $caps['export'] = 1;
        $caps['delete_users'] = 1;
        $caps['create_users'] = 1;
        if ($this->multisite) {
            ...
            $caps['manage_network_designs'] = 1;
            $caps['manage_network_plugins'] = 1;
            $caps['manage_network_options'] = 1;
        }
                
        $caps = apply_filters('rm_built_in_mn_caps', $caps);
    ...
    protected function get_caps_to_remove() 
    {
        ...
        $rm_caps = $this->get_rm_caps();
        $caps_to_exclude = array_merge($caps_to_exclude, $rm_caps);
     ...
     * @param boolean $core - if true, then show Mtaandao core capabilities, else custom (plugins and designs created)
    ...
    protected function show_capabilities($core = true, $for_role = true, $edit_mode=true) {
        ...
        if (!( $this->multisite && is_super_admin() )) {  // do not limit SuperAdmin for multi-site
            if ($core && 'administrator' == $this->current_role) {
                $onclick_for_admin = 'onclick="rm_turn_it_back(this)"';
            }
        }

        if ($core) {
            $quant = count($this->get_built_in_mn_caps());
        ...
        foreach ($this->full_capabilities as $capability) {            
            if ($core) {
                ...
                if ($capability['mn_core']) { // show plugins and designs added capabilities
                    continue;
                }
            }
            ...
            $cap_id = str_replace(' ', RM_SPACE_REPLACER, $capability['inner']);
            echo '<div id="rm_div_cap_'. $cap_id.'" '. $hidden_class .'><input type="checkbox" name="' . $cap_id . '" id="' . 
    ...
    /**
     * output HTML code to create RM toolbar
     * 
...
?>	
        <div id="rm_toolbar" >
           <button id="rm_select_all" class="rm_toolbar_button">Select All</button>
<?php
        if ('administrator' != $this->current_role) {
?>   
               <button id="rm_unselect_all" class="rm_toolbar_button">Unselect All</button> 
               <button id="rm_reverse_selection" class="rm_toolbar_button">Reverse</button> 
<?php
        }
        if ($this->rm_object == 'role') {
?>              
               <hr />
               <div id="rm_update">
                <button id="rm_update_role" class="rm_toolbar_button button-primary" >Update</button> 
<?php
            do_action('rm_role_edit_toolbar_update');
?>                                   
               </div>
<?php
...
?>
               <hr />               
<?php 
                if (current_user_can('rm_create_roles')) {
?>
               <button id="rm_add_role" class="rm_toolbar_button">Add Role</button>
<?php
                }
?>
               <button id="rm_rename_role" class="rm_toolbar_button">Rename Role</button>   
<?php
            }   // restrict single site admin
            if (!$this->multisite || $super_admin || !$caps_access_restrict_for_simple_admin) { // restrict single site admin
                if (current_user_can('rm_create_capabilities')) {
?>
               <button id="rm_add_capability" class="rm_toolbar_button">Add Capability</button>
<?php
                }
            }   // restrict single site admin
            
            if (!$this->multisite || $super_admin || $add_del_role_for_simple_admin) { // restrict single site admin
                if (!empty($role_delete) && current_user_can('rm_delete_roles')) {
?>  
                   <button id="rm_delete_role" class="rm_toolbar_button">Delete Role</button>
<?php
                }
            } // restrict single site admin
            
            if (!$this->multisite || $super_admin || !$caps_access_restrict_for_simple_admin) { // restrict single site admin            
                if ($capability_remove && current_user_can('rm_delete_capabilities')) {
?>
                   <button id="rm_delete_capability" class="rm_toolbar_button">Delete Capability</button>
<?php
                }
                if ($this->multisite && !is_network_admin()) {  // Show for single site for MN multisite only
?>
               <hr />
               <button id="rm_default_role" class="rm_toolbar_button">Default Role</button>
               <hr />
<?php
                }
?>
               <div id="rm_service_tools">
<?php
                do_action('rm_role_edit_toolbar_service');
                if (!$this->multisite || 
                    (is_main_site( get_current_blog_id()) || (is_network_admin() && is_super_admin()))
                   ) {
                    if (current_user_can('rm_reset_roles')) {
?>                   
                  <button id="rm_reset_roles_button" class="rm_toolbar_button" style="color: red;" title="Reset Roles to its original state">Reset</button> 
<?php
                    }
                }
?>
            ...
            ?>
               ...
               <hr />
            	 <div id="rm_update_user">
                <button id="rm_update_role" class="rm_toolbar_button button-primary">Update</button> 
<?php
    do_action('rm_user_edit_toolbar_update');
?>                   
    ...
    protected function capability_help_link($capability) {
        ...
        switch ($capability) {
            case 'activate_plugins':
                $url = 'http://www.mtaandao.co.ke/activate_plugins-mtaandao-capability/';
                break;
            case 'add_users':
                $url = 'http://www.mtaandao.co.ke/add_users-mtaandao-user-capability/';
                break;
            case 'create_users':
                $url = 'http://www.mtaandao.co.ke/create_users-mtaandao-user-capability/';
                break;
            ...
            case 'delete_published_posts':
                $url = 'http://www.mtaandao.co.ke/delete-posts-and-pages-mtaandao-user-capabilities-set/';
                break;
            case 'delete_plugins':
                $url = 'http://www.mtaandao.co.ke/delete_plugins-mtaandao-user-capability/';
                break;
            case 'delete_designs':
                $url = 'http://www.mtaandao.co.ke/delete_designs-mtaandao-user-capability/';
                break;
            case 'delete_users':
                $url = 'http://www.mtaandao.co.ke/delete_users-mtaandao-user-capability/';
                break;
            case 'edit_dashboard':
                $url = 'http://www.mtaandao.co.ke/edit_dashboard-mtaandao-capability/';
                break;
            case 'edit_files':
                $url = 'http://www.mtaandao.co.ke/edit_files-mtaandao-user-capability/';
                break;
            case 'edit_plugins':
                $url = 'http://www.mtaandao.co.ke/edit_plugins-mtaandao-user-capability';
                break;
            case 'moderate_comments':
                $url = 'http://www.mtaandao.co.ke/moderate_comments-mtaandao-user-capability/';
                break;
            case 'read':
                $url = 'http://mtaandao.co.ke/mtaandao-read-capability/';
                break;
            case 'update_core':
                $url = 'http://www.mtaandao.co.ke/update_core-capability-for-mtaandao-user/';
                break;
            case 'rm_edit_roles':
                $url = 'https://www.mtaandao.co.ke/role-manager-4-18-new-permissions/';
                break;
            default:
                $url = '';
        }
        // end of switch
        if (!empty($url)) {
            $link = '<a href="' . $url . '" title="' . esc_html__('read about', 'role-manager') .' '. $capability .' '. 
                    esc_html__('user capability', 'role-manager') .'" target="new"><img src="' . 
                    RES . 'images/help.png" alt="' . esc_html__('Help', 'role-manager') . '" /></a>';
        } else {
            $link = '';
        }
    ...
    protected function add_capability_to_full_caps_list($cap_id) {
        if (!isset($this->full_capabilities[$cap_id])) {    // if capability was not added yet
            $cap = array();
            $cap['inner'] = $cap_id;
            $cap['human'] = esc_html__($this->convert_caps_to_readable($cap_id), 'role-manager');
    ...
    /**
     * Provide compatibility with plugins and designs which define their custom user capabilities using 
    ...
    public function _get_post_types() {
        $post_types = get_transient('rm_public_post_types');
        if (empty($post_types)) {
            ...
            set_transient('rm_public_post_types', $post_types, 30);
        }
    ...
    /**
     * Add capabilities for RM permissions system in case some were excluded from Administrator role
     * 
     */
    protected function add_rm_caps() {        
    ...
    protected function init_full_capabilities() {
        ...
        $this->full_capabilities = apply_filters('rm_full_capabilites', $this->full_capabilities);
    ...
    protected function mn_roles_reinit() {
        ...
        require_once(ABSPATH . '/admin/res/schema.php');
        populate_roles();
        $mn_roles->reinit();
    ...
    protected function reset_user_roles() {
        ...
        if (!current_user_can('rm_reset_roles')) {
            return esc_html__('Insufficient permissions to work with Role Manager','role-manager');
        }
    ...
    public function is_full_network_synch() {
        ...
        $result = defined('RM_MULTISITE_DIRECT_UPDATE') && RM_MULTISITE_DIRECT_UPDATE == 1;
    ...
    protected function log_event($message, $show_message = false) {
        ...
        $file_name = RM_PLUGIN_DIR . 'role-manager.log';
        ...
        if ($show_message) {
            $this->show_message('Error! ' . esc_html__('Error is occur. Please check the log file.', 'role-manager'));
        }
    }
    // end of log_event()
    ...
    protected function add_new_role() {
        ...
        if (isset($_POST['user_role_id']) && $_POST['user_role_id']) {
            ...
            if (!$valid_name || ($valid_name && ($match[0] != $user_role_id))) { // some non-alphanumeric charactes found!
                return esc_html__('Error: Role ID must contain latin characters, digits, hyphens or underscore only!', 'role-manager');
            }
            $numeric_name = preg_match('/[0-9]*/', $user_role_id, $match);
            if ($numeric_name && ($match[0] == $user_role_id)) { // numeric name discovered
                return esc_html__('Error: Mtaandao does not support numeric Role name (ID). Add latin characters to it.', 'role-manager');
            }
            ...
            if ($user_role_id) {
                ...
                if (isset($mn_roles->roles[$user_role_id])) {
                    return sprintf('Error! ' . esc_html__('Role %s exists already', 'role-manager'), $user_role_id);
                }
                ...
                if (!isset($result) || empty($result)) {
                    $mess = 'Error! ' . esc_html__('Error is encountered during new role create operation', 'role-manager');
                } else {
                    $mess = sprintf(esc_html__('Role %s is created successfully', 'role-manager'), $user_role_name);
                }
            }
        }
        return $mess;
    ...
    protected function rename_role() {
        ...
        if (empty($user_role_id)) {
            return esc_html__('Error: Role ID is empty!', 'role-manager');
        }
        ...
        if (!empty($new_role_name)) {
            ...
            return esc_html__('Error: Empty role display name is not allowed.', 'role-manager');
        }
        ...
        if (!isset($mn_roles->roles[$user_role_id])) {
            return sprintf('Error! ' . esc_html__('Role %s does not exists', 'role-manager'), $user_role_id);
        }                
        $this->current_role = $user_role_id;
        ...
        $mess = sprintf(esc_html__('Role %s is renamed to %s successfully', 'role-manager'), $old_role_name, $new_role_name);
    ...
    protected function delete_mn_roles($roles_to_del) {
        ...
        if (!current_user_can('rm_delete_roles')) {
            return esc_html__('Insufficient permissions to work with Role Manager','role-manager');
        }
        if (!isset($mn_roles)) {
            $mn_roles = new MN_Roles();
    ...
    protected function delete_role() {        
        ...
        if (isset($_POST['user_role_id']) && $_POST['user_role_id']) {
            ...
            if (empty($result)) {
                $mess = 'Error! ' . esc_html__('Error encountered during role delete operation', 'role-manager');
            } elseif ($role==-1) {
                $mess = sprintf(esc_html__('Unused roles are deleted successfully', 'role-manager'), $role);
            } else {
                $mess = sprintf(esc_html__('Role %s is deleted successfully', 'role-manager'), $role);
            }
    ...
    protected function change_default_role() {
        ...
        if (!empty($_POST['user_role_id'])) {
            ...
            if (isset($mn_roles->role_objects[$user_role_id]) && $user_role_id !== 'administrator') {
                ...
                if (empty($result)) {
                    $mess = 'Error! ' . esc_html__('Error encountered during default role change operation', 'role-manager');
                } else {
                    $this->get_default_role();
                    $mess = sprintf(esc_html__('Default role for new users is set to %s successfully', 'role-manager'), $mn_roles->role_names[$user_role_id]);
                }
            } elseif ($user_role_id === 'administrator') {
                $mess = 'Error! ' . esc_html__('Can not set Administrator role as a default one', 'role-manager');
            } else {
                $mess = 'Error! ' . esc_html__('This role does not exist - ', 'role-manager') . esc_html($user_role_id);            
            }
        }
...
// Standard Mtaandao roles
            __('Editor', 'role-manager');
            __('Author', 'role-manager');
            __('Contributor', 'role-manager');
            __('Subscriber', 'role-manager');
// Standard Mtaandao capabilities
            __('Switch designs', 'role-manager');
            __('Edit designs', 'role-manager');
            __('Activate plugins', 'role-manager');
            __('Edit plugins', 'role-manager');
            __('Edit users', 'role-manager');
            __('Edit files', 'role-manager');
            __('Manage options', 'role-manager');
            __('Moderate comments', 'role-manager');
            __('Manage categories', 'role-manager');
            __('Manage links', 'role-manager');
            __('Upload files', 'role-manager');
            __('Import', 'role-manager');
            __('Unfiltered html', 'role-manager');
            __('Edit posts', 'role-manager');
            __('Edit others posts', 'role-manager');
            __('Edit published posts', 'role-manager');
            __('Publish posts', 'role-manager');
            __('Edit pages', 'role-manager');
            __('Read', 'role-manager');
            __('Level 10', 'role-manager');
            __('Level 9', 'role-manager');
            __('Level 8', 'role-manager');
            __('Level 7', 'role-manager');
            __('Level 6', 'role-manager');
            __('Level 5', 'role-manager');
            __('Level 4', 'role-manager');
            __('Level 3', 'role-manager');
            __('Level 2', 'role-manager');
            __('Level 1', 'role-manager');
            __('Level 0', 'role-manager');
            __('Edit others pages', 'role-manager');
            __('Edit published pages', 'role-manager');
            __('Publish pages', 'role-manager');
            __('Delete pages', 'role-manager');
            __('Delete others pages', 'role-manager');
            __('Delete published pages', 'role-manager');
            __('Delete posts', 'role-manager');
            __('Delete others posts', 'role-manager');
            __('Delete published posts', 'role-manager');
            __('Delete private posts', 'role-manager');
            __('Edit private posts', 'role-manager');
            __('Read private posts', 'role-manager');
            __('Delete private pages', 'role-manager');
            __('Edit private pages', 'role-manager');
            __('Read private pages', 'role-manager');
            __('Delete users', 'role-manager');
            __('Create users', 'role-manager');
            __('Unfiltered upload', 'role-manager');
            __('Edit dashboard', 'role-manager');
            __('Update plugins', 'role-manager');
            __('Delete plugins', 'role-manager');
            __('Install plugins', 'role-manager');
            __('Update designs', 'role-manager');
            __('Install designs', 'role-manager');
            __('Update core', 'role-manager');
            __('List users', 'role-manager');
            __('Remove users', 'role-manager');
            __('Add users', 'role-manager');
            __('Promote users', 'role-manager');
            __('Edit design options', 'role-manager');
            __('Delete designs', 'role-manager');
            __('Export', 'role-manager');
        }
    }
    // end of rm_TranslationData()
    ...
    protected function add_new_capability() {
        ...
        if (!$valid_name || ($valid_name && ($match[0] != $user_capability))) { // some non-alphanumeric charactes found!    
            return esc_html__('Error: Capability name must contain latin characters and digits only!', 'role-manager');
        }
        ...
        if (!isset($this->full_capabilities[$user_capability])) {
            ...
            $mn_roles->use_db = true;
            $mn_roles->add_cap($admin_role, $user_capability);
            $mess = sprintf(esc_html__('Capability %s is added successfully', 'role-manager'), $user_capability);
        } else {
            $mess = sprintf(esc_html__('Capability %s exists already', 'role-manager'), $user_capability);
        }
    ...
    protected function delete_capability() {
        ...
        if (!current_user_can('rm_delete_capabilities')) {
            return esc_html__('Insufficient permissions to work with Role Manager','role-manager');
        }
        $mess = '';
        if (!empty($_POST['user_capability_id'])) {
            ...
            if (!is_array($caps_to_remove) || count($caps_to_remove) == 0 || !isset($caps_to_remove[$capability_id])) {
                return sprintf(esc_html__('Error! You do not have permission to delete this capability: %s!', 'role-manager'), $capability_id);
            }
            ...
            $mess = sprintf(esc_html__('Capability %s was removed successfully', 'role-manager'), $capability_id);
        }
...
?>		  
            ...
            <strong><?php esc_html_e('Version:', 'role-manager');?></strong> <?php echo RM_VERSION; ?><br/><br/>
            <a class="rm_rsb_link" style="background-image:url(<?php echo RES . 'images/vladimir.png'; ?>);" target="_blank" href="http://www.mtaandao.co.ke/"><?php _e("Author's website", 'role-manager'); ?></a><br/>
            <a class="rm_rsb_link" style="background-image:url(<?php echo RES . 'images/role-manager-icon.png'; ?>);" target="_blank" href="https://www.mtaandao.co.ke"><?php _e('Plugin webpage', 'role-manager'); ?></a><br/>
            <a class="rm_rsb_link" style="background-image:url(<?php echo RES . 'images/role-manager-icon.png'; ?>);" target="_blank" href="https://www.mtaandao.co.ke/download-plugin"><?php _e('Plugin download', 'role-manager'); ?></a><br/>
            <a class="rm_rsb_link" style="background-image:url(<?php echo RES . 'images/changelog-icon.png'; ?>);" target="_blank" href="https://www.mtaandao.co.ke/changelog"><?php _e('Changelog', 'role-manager'); ?></a><br/>
            <a class="rm_rsb_link" style="background-image:url(<?php echo RES . 'images/faq-icon.png'; ?>);" target="_blank" href="http://www.mtaandao.co.ke/role-manager-mtaandao-plugin/#faq"><?php _e('FAQ', 'role-manager'); ?></a><br/>
            <hr />
...
<?php         
    ...
    protected function show_admin_role_allowed() {
        ...
        $show_admin_role = ((defined('RM_SHOW_ADMIN_ROLE') && RM_SHOW_ADMIN_ROLE==1) || $show_admin_role==1) && $this->user_is_admin();
    ...
    public function role_default_prepare_html($select_width=200) {
        ...
        foreach ($this->roles as $key => $value) {
            $selected = $this->option_selected($key, $this->mn_default_role);
            ...
            if ($show_admin_role || $key != 'administrator') {
                $translated_name = esc_html__($value['name'], 'role-manager');  // get translation from RM language file, if exists
    ...
    private function role_delete_prepare_html() {
        ...
        if ($roles_can_delete && count($roles_can_delete) > 0) {
            ...
            foreach ($roles_can_delete as $key => $value) {
                $this->role_delete_html .= '<option value="' . $key . '">' . esc_html__($value, 'role-manager') . '</option>';
            }
            $this->role_delete_html .= '<option value="-1" style="color: red;">' . esc_html__('Delete All Unused Roles', 'role-manager') . '</option>';
    ...
    private function role_select_copy_prepare_html($select_width=200) {
        ...
        $this->role_to_copy_html = '<select id="user_role_copy_from" name="user_role_copy_from" style="width: '. $select_width .'px">
            <option value="none" selected="selected">' . esc_html__('None', 'role-manager') . '</option>';
        $this->role_select_html = '<select id="user_role" name="user_role" onchange="rm_role_change(this.value);">';
        foreach ($this->roles as $key => $value) {
...
<?php        
    ...
    public function get_assign_role() {
        $assign_role = new RM_Assign_Role($this);
