...
class RM_Admin_Menu_Access {

    const DEBUG = false;
    const BLOCKED_URLS = 'rm_admin_menu_blocked_urls';
    ...
    public function __construct() {
        ...
        add_action('rm_role_edit_toolbar_service', 'RM_Admin_Menu_View::add_toolbar_buttons');
        add_action('rm_load_js', 'RM_Admin_Menu_View::add_js');
        add_action('rm_dialogs_html', 'RM_Admin_Menu_View::dialog_html');
        add_action('rm_process_user_request', 'RM_Admin_Menu::update_data');
        
        add_action('activated_plugin', 'RM_Admin_Menu_Copy::force_update');
        add_action('admin_menu', array($this, 'menu_glitches_cleanup'), 999);
        add_action('admin_menu', 'RM_Admin_Menu_Copy::update', 1000);  // Jetpack uses 998. We should execute code later
        ...
        add_action('admin_bar_menu', array($this, 'replace_mn_admin_bar_my_sites_menu'), 19);
        add_action('mn_before_admin_bar_render', array($this, 'modify_admin_menu_bar'), 99);
    ...
    /**
     * Some plugins incorrectly modify globals $menu/$submenu and users with changed permissions may get broken $menu/submenu$ structrms.
    ...
    /** 
     * Compare links of the current Mtaandao admin submenu and its copy used by Role Manager admin access add-on
    ...
    private function find_submenu_item($key, $key1, $submenu_copy) {
        ...
        $link1 = RM_Admin_Menu::normalize_link($submenu[$key][$key1][2]);
        if (isset($submenu_copy[$key][$key1])) {
            $link2 = RM_Admin_Menu::normalize_link($submenu_copy[$key][$key1][2]);
        ...
        if (!empty($key2)) {
            $link2 = RM_Admin_Menu::normalize_link($submenu_copy[$key][$key2][2]);
            if ($link1==$link2) {
                return $key2;
            }
        }
    ...
    // Check if Mtaandao admin menu link is included into the menu copy, used by Role Manager admin access add-on
    ...
    private function remove_from_submenu($blocked, $submenu_copy) {
        ...
        foreach($submenu as $key=>$menu_item) {
            foreach($menu_item as $key1=>$menu_item1) {
                ...
                $link = RM_Admin_Menu::normalize_link($submenu_copy[$key][$key2][3]);
                $item_id = RM_Admin_Menu::calc_menu_item_id('submenu', $link);
    ...
    private function remove_blocked_menu_items() {
        ...
        $blocked = RM_Admin_Menu::load_menu_access_data_for_user($current_user);
        if (empty($blocked['data'])) {
            return;
        }

        $menu_copy = RM_Admin_Menu_Copy::get_menu();
        $submenu_copy = RM_Admin_Menu_Copy::get_submenu();        
        ...
        foreach($menu as $key=>$menu_item) {
            ...
            $link = RM_Admin_Menu::normalize_link($menu_copy[$key1][3]);
            $item_id1 = RM_Admin_Menu::calc_menu_item_id('menu', $link);
            $item_id2 = RM_Admin_Menu::calc_menu_item_id('submenu', $link);
    ...
    private function get_first_available_menu_item($dashboard_allowed) {    
        ...
        $available = '';
        foreach ($menu as $key=>$menu_item) {
            if ($menu_item[4]==='mn-menu-separator' || $menu_item[4]==='separator-woocommerce') {
                continue;
            }
            $key1 = $this->get_key_from_menu_copy($menu_item, $menu_copy);
            if ($key1===false) { // menu item does not found at menu copy
    ...
    private function is_command_args_registered($command, $args_to_check) {
        $result = true;
        //@TODO: build this structrm automatically via globals menu/submenu copy links scan - place at admin-menu-copy.php 
        $allowed_args = array(
    ...
    private function command_from_main_menu($command) {
        ...
        $php_command = $this->get_php_command($command);
        $menu_hashes = RM_Admin_Menu::get_menu_hashes();
    ...
    private function is_command_inside_allowed_page($menu_link, $command, $allowed_hash) {
        ...
        $item_id1 = RM_Admin_Menu::calc_menu_item_id('menu', $menu_link);
        $item_id2 = RM_Admin_Menu::calc_menu_item_id('submenu', $menu_link);
    ...
    public function redirect_blocked_urls() {
        ...
        $item_id1 = RM_Admin_Menu::calc_menu_item_id('menu', $command);
        $item_id2 = RM_Admin_Menu::calc_menu_item_id('submenu', $command);        
...
<?php
    ...
    /**
     * Copy of res/admin-bar.php::mn_admin_bar_my_sites_menu() function
     ...
     * @return void
     */
    public function mn_admin_bar_my_sites_menu($mn_admin_bar) {        
        ...
        if (is_super_admin()) {
            ...
            $mn_admin_bar->add_menu(array(
                ...
                'id' => 'network-admin-t',
                'title' => __('Themes'),
                'href' => network_admin_url('designs.php'),
            ));
            $mn_admin_bar->add_menu(array(
                'parent' => 'network-admin',
                'id' => 'network-admin-p',
    ...
    // end of mn_admin_bar_my_sites_menu()
        

    public function replace_mn_admin_bar_my_sites_menu() {
        if (is_admin() || !$this->lib->multisite || is_super_admin()) {
            return;
        }
        
        remove_action('admin_bar_menu', 'mn_admin_bar_my_sites_menu', 20);
        add_action('admin_bar_menu', array($this, 'mn_admin_bar_my_sites_menu'), 20);
        
    }
    // end of replace_mn_admin_bar_my_sites_menu()
     ...
     * @global type $mn_admin_bar
     * @return void
     */
    public function modify_admin_menu_bar() {
        global $current_user, $mn_admin_bar;
                
        $nodes = $mn_admin_bar->get_nodes();
        if (empty($nodes)) {
            return;
        }
        ...
        if (!current_user_can('manage_options')) {
            $mn_admin_bar->remove_menu('mnseo-menu');
        } 
        ...
        if ( ($blocked['access_model']==1 && in_array($customize_item_id, $blocked['data'])) ||
            ...
            $mn_admin_bar->remove_menu('customize');
        }
        ...
        if ( ($blocked['access_model']==1 && in_array($item_id, $blocked['data'])) ||
             ($blocked['access_model']==2 && !in_array($item_id, $blocked['data'])) ) {
            $mn_admin_bar->remove_menu('updraft_admin_node');
        }
        ...
        foreach($nodes as $key=>$menu_item) {
            ...
            if ($blocked['access_model']==1) {  // block selected
                if (in_array($item_id1, $blocked['data'])) {
                    $mn_admin_bar->remove_menu($menu_item->id);
                } elseif (in_array($item_id2, $blocked['data'])) {
                    $mn_admin_bar->remove_node($menu_item->id);
                }
            } elseif ($blocked['access_model']==2) {    // block not selected
                if (!in_array($item_id1, $blocked['data']) && !in_array($item_id2, $blocked['data'])) {
                    $mn_admin_bar->remove_menu($menu_item->id);                
...
// end of RM_Admin_Menu_Access class
