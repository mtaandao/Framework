...
if ( ! current_user_can('install_themes') )
	mn_die( __( 'Sorry, you are not allowed to install themes on this site.' ) );

if ( is_multisite() && ! is_network_admin() ) {
	mn_redirect( network_admin_url( 'theme-install.php' ) );
	exit();
}
...
mn_localize_script( 'theme', '_mnThemeSettings', array(
	'themes'   => false,
	'settings' => array(
		'isInstall'     => true,
	...
	'l10n' => array(
		...
		'back'   => __( 'Back' ),
		'error'  => __( 'An unexpected error occurred. Something may be wrong with mtaandao.co.ke or this server&#8217;s configuration. If you continue to have problems, please try the <a href="https://mtaandao.co.ke/support/">support forums</a>.' ),
...
$help_overview =
	'<p>' . sprintf(
			...
			__( 'You can find additional themes for your site by using the Theme Browser/Installer on this screen, which will display themes from the <a href="%s" target="_blank">Mtaandao Theme Directory</a>. These themes are designed and developed by third parties, are available free of charge, and are compatible with the license Mtaandao uses.' ),
			__( 'https://mtaandao.co.ke/themes/' )
		) . '</p>' .
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="https://mtaandao.co.ke/docs/Using_Themes#Adding_New_Themes" target="_blank">Documentation on Adding New Themes</a>') . '</p>' .
...
<div class="wrap">
	...
	<div class="mn-filter hide-if-no-js">
		...
		<div class="favorites-form">
			...
			if ( isset( $_GET['_mnnonce'] ) && mn_verify_nonce( mn_unslash( $_GET['_mnnonce'] ), $action ) ) {
				$user = isset( $_GET['user'] ) ? mn_unslash( $_GET['user'] ) : get_user_option( 'mnorg_favorites' );
				update_user_meta( get_current_user_id(), 'mnorg_favorites', $user );
			} else {
				$user = get_user_option( 'mnorg_favorites' );
			}
			?>
			<p class="install-help"><?php _e( 'If you have marked themes as favorites on mtaandao.co.ke, you can browse them here.' ); ?></p>

			<p>
				<label for="mnorg-username-input"><?php _e( 'Your mtaandao.co.ke username:' ); ?></label>
				<input type="hidden" id="mnorg-username-nonce" name="_mnnonce" value="<?php echo esc_attr( mn_create_nonce( $action ) ); ?>" />
				<input type="search" id="mnorg-username-input" value="<?php echo esc_attr( $user ); ?>" />
	...
	<div class="theme-install-overlay mn-full-overlay expanded"></div>
