 ...
 * @see MN_Upgrader
 */
class Theme_Upgrader extends MN_Upgrader {
	 ...
	 * }
	 *
	 * @return bool|MN_Error True if the install was successful, false or a MN_Error object otherwise.
	 */
	public function install( $package, $args = array() ) {
		...
		if ( ! $this->result || is_mn_error($this->result) )
			return $this->result;
	 ...
	 *                                    Default true.
	 * }
	 * @return bool|MN_Error True if the upgrade was successful, false or a MN_Error object otherwise.
	 */
	 ...
	 * @return string|MN_Error The source or a MN_Error.
	 */
	public function check_package( $source ) {
		...
		if ( ! file_exists( $working_directory . 'style.css' ) ) {
			return new MN_Error( 'incompatible_archive_theme_no_style', $this->strings['incompatible_archive'],
		...
		if ( empty( $info['Name'] ) ) {
			return new MN_Error( 'incompatible_archive_theme_no_name', $this->strings['incompatible_archive'],
				/* translators: %s: style.css */
		...
		if ( empty( $info['Template'] ) && ! file_exists( $working_directory . 'index.php' ) ) {
			return new MN_Error( 'incompatible_archive_theme_no_index', $this->strings['incompatible_archive'],
	 ...
	 * @access public
	 *
	 * @param bool|MN_Error  $return
	 * @param array          $theme
	 * @return bool|MN_Error
	 */
	public function current_before($return, $theme) {
		if ( is_mn_error($return) )
			return $return;
	...
	public function delete_old_theme( $removed, $local_destination, $remote_destination, $theme ) {
		...
		$theme = $theme['theme'];
		$themes_dir = trailingslashit( $mn_filesystem->mn_themes_dir( $theme ) );
		if ( $mn_filesystem->exists( $themes_dir . $theme ) ) {
			if ( ! $mn_filesystem->delete( $themes_dir . $theme, true ) )
				return false;
		}
	...
	/**
	 * Get the MN_Theme object for a theme.
	 *
	 * @since 2.8.0
	 ...
	 * @return MN_Theme|false The theme's info object, or false `$theme` is not supplied
