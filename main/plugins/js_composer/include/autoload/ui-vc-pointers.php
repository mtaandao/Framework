...
function vc_pointer_load() {
	...
	mn_localize_script( 'mn-pointer', 'vcPointer', $vc_pointers );
}
...
function vc_pointer_reset() {
	...
	$prev_meta_value = get_user_meta( get_current_user_id(), 'dismissed_mn_pointers', true );
	...
	if ( count( $dismissed ) > 0 && count( $pointers ) ) {
		...
		update_user_meta( get_current_user_id(), 'dismissed_mn_pointers', $meta_value, $prev_meta_value );
	}

	mn_send_json( array( 'success' => true ) );
}
...
add_action( 'mn_ajax_vc_pointer_reset', 'vc_pointer_reset' );
