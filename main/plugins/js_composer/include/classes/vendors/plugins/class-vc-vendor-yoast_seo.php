...
class Vc_Vendor_YoastSeo implements Vc_Vendor_Interface {
	...
	/**
	 * Created to improve yoast multiply calling mnseo_pre_analysis_post_content filter.
	 * @since 4.5.3
	...
	public function load() {
		if ( class_exists( 'MNSEO_Metabox' ) && ( 'admin_page' === vc_mode() || 'admin_frontend_editor' === vc_mode() ) ) {
			add_filter( 'mnseo_pre_analysis_post_content', array(
				&$this,
				'filterResults',
			) );
		}
	...
	public function filterResults( $content ) {
		if ( empty( $this->parsedContent ) ) {
			global $post, $mn_the_query;
			$mn_the_query->post = $post; // since 4.5.3 to avoid the_post replaces
			/**
			 * @since 4.4.3
	...
	public function enqueueJs() {
		mn_enqueue_script( 'vc_vendor_yoast_js', vc_asset_url( 'js/vendors/yoast.js' ), array( 'yoast-seo-post-scraper' ), WPB_VC_VERSION, true );
	}

	public function frontendEditorBuild() {
		$vc_yoast_meta_box = $GLOBALS['mnseo_metabox'];
		remove_action( 'admin_init', array(
			$GLOBALS['mnseo_meta_columns'],
			'setup_hooks',
		) );
		apply_filters( 'mnseo_use_page_analysis', false );
		...
		remove_action( 'mn_insert_post', array(
			$vc_yoast_meta_box,
			'save_postdata',
		) );
