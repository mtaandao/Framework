...
class WC_Admin_Report {
	...
	public function get_order_report_data( $args = array() ) {
		...
		$query['from']   = "FROM {$mndb->posts} AS posts";
		...
		foreach ( ( $data + $where ) as $key => $value ) {
			...
			switch ( $type ) {
				case 'meta' :
					$joins["meta_{$key}"] = "{$join_type} JOIN {$mndb->postmeta} AS meta_{$key} ON ( posts.ID = meta_{$key}.post_id AND meta_{$key}.meta_key = '{$key}' )";
					break;
				case 'parent_meta' :
					$joins["parent_meta_{$key}"] = "{$join_type} JOIN {$mndb->postmeta} AS parent_meta_{$key} ON (posts.post_parent = parent_meta_{$key}.post_id) AND (parent_meta_{$key}.meta_key = '{$key}')";
					break;
				case 'order_item_meta' :
					$joins["order_items"] = "{$join_type} JOIN {$mndb->prefix}woocommerce_order_items AS order_items ON (posts.ID = order_items.order_id)";

					if ( ! empty( $value['order_item_type'] ) ) {
						$joins["order_items"] .= " AND (order_items.order_item_type = '{$value['order_item_type']}')";
					}

					$joins["order_item_meta_{$key}"]  = "{$join_type} JOIN {$mndb->prefix}woocommerce_order_itemmeta AS order_item_meta_{$key} ON " .
		...
		if ( ! empty( $where_meta ) ) {
			foreach ( $where_meta as $value ) {
				...
				if ( 'order_item_meta' === $type ) {
					...
					$joins["order_items"] = "{$join_type} JOIN {$mndb->prefix}woocommerce_order_items AS order_items ON posts.ID = order_items.order_id";
					$joins["order_item_meta_{$key}"] = "{$join_type} JOIN {$mndb->prefix}woocommerce_order_itemmeta AS order_item_meta_{$key} ON order_items.order_item_id = order_item_meta_{$key}.order_item_id";

				} else {
					// If we have a where clause for meta, join the postmeta table
					$joins["meta_{$key}"] = "{$join_type} JOIN {$mndb->postmeta} AS meta_{$key} ON posts.ID = meta_{$key}.post_id";
				}
			}
		}

		if ( ! empty( $parent_order_status ) ) {
			$joins["parent"] = "LEFT JOIN {$mndb->posts} AS parent ON posts.post_parent = parent.ID";
		}
		...
		if ( $debug || $nocache || false === $cached_results || ! isset( $cached_results[ $query_hash ] ) ) {
			...
			$cached_results[ $query_hash ] = apply_filters( 'woocommerce_reports_get_order_report_data', $mndb->$query_type( $query ), $data );
