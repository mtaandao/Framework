...
final class MN_Comment {
	...
	/**
	 * Retrieves a MN_Comment instance.
	 *
	 * @since 4.4.0
	 * @access public
	 * @static
	 ...
	 * @return MN_Comment|false Comment object, otherwise false.
	 */
	public static function get_instance( $id ) {
		...
		if ( ! $_comment ) {
			$_comment = $mndb->get_row( $mndb->prepare( "SELECT * FROM $mndb->comments WHERE comment_ID = %d LIMIT 1", $comment_id ) );

			if ( ! $_comment ) {
				return false;
			}

			mn_cache_add( $_comment->comment_ID, $_comment, 'comment' );
		}

		return new MN_Comment( $_comment );
	}
	 ...
	 *                                 stored in a `children` property on the `MN_Comment` object.
	 ...
	 * }
	 * @return array Array of `MN_Comment` objects.
	 */
	public function get_children( $args = array() ) {
		...
		$_args = mn_parse_args( $args, $defaults );
	 ...
	 *
	 * Used by `MN_Comment_Query` when bulk-filling descendants.
	 *
	 * @since 4.4.0
	 * @access public
	 *
	 * @param MN_Comment $child Child comment.
	 */
	public function add_child( MN_Comment $child ) {
	 ...
	 * @return MN_Comment|bool Returns the comment object if found, otherwise false.
	 */
