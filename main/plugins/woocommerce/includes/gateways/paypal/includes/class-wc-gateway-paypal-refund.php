...
class WC_Gateway_Paypal_Refund {
	 ...
	 * @param  string   $reason
	 * @param  bool     $sandbox
	 * @return array|mn_error The parsed response from paypal, or a MN_Error object
	 */
	public static function refund_order( $order, $amount = null, $reason = '', $sandbox = false ) {
		$response = mn_safe_remote_post(
		...
		if ( empty( $response['body'] ) ) {
			return new MN_Error( 'paypal-refunds', 'Empty Response' );
		}
