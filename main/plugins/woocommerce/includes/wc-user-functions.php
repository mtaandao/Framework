...
if ( ! function_exists( 'wc_create_new_customer' ) ) {
	 ...
	 * @return int|MN_Error Returns MN_Error on failure, Int (user ID) on success.
	 */
	function wc_create_new_customer( $email, $username = '', $password = '' ) {

		// Check the email address.
		if ( empty( $email ) || ! is_email( $email ) ) {
			return new MN_Error( 'registration-error-invalid-email', __( 'Please provide a valid email address.', 'woocommerce' ) );
		}

		if ( email_exists( $email ) ) {
			return new MN_Error( 'registration-error-email-exists', __( 'An account is already registered with your email address. Please login.', 'woocommerce' ) );
		}
		...
		if ( 'no' === get_option( 'woocommerce_registration_generate_username' ) || ! empty( $username ) ) {
			...
			if ( empty( $username ) || ! validate_username( $username ) ) {
				return new MN_Error( 'registration-error-invalid-username', __( 'Please enter a valid account username.', 'woocommerce' ) );
			}

			if ( username_exists( $username ) ) {
				return new MN_Error( 'registration-error-username-exists', __( 'An account is already registered with that username. Please choose another.', 'woocommerce' ) );
			}
		} else {
		...
		if ( 'yes' === get_option( 'woocommerce_registration_generate_password' ) && empty( $password ) ) {
			$password           = mn_generate_password();
			$password_generated = true;
		} elseif ( empty( $password ) ) {
			return new MN_Error( 'registration-error-missing-password', __( 'Please enter an account password.', 'woocommerce' ) );
		} else {
			$password_generated = false;
		}

		// Use MN_Error to handle registration errors.
		$errors = new MN_Error();
		...
		$customer_id = mn_insert_user( $new_customer_data );

		if ( is_mn_error( $customer_id ) ) {
			return new MN_Error( 'registration-error', '<strong>' . __( 'ERROR', 'woocommerce' ) . '</strong>: ' . __( 'Couldn&#8217;t register you&hellip; please contact us if you continue to have problems.', 'woocommerce' ) );
		}
...
function wc_set_customer_auth_cookie( $customer_id ) {
	...
	mn_set_auth_cookie( $customer_id, true );
}
...
function wc_customer_bought_product( $customer_email, $user_id, $product_id ) {
	...
	if ( false === ( $result = get_transient( $transient_name ) ) ) {
		...
		$result = $mndb->get_col( "
			SELECT im.meta_value FROM {$mndb->posts} AS p
			INNER JOIN {$mndb->postmeta} AS pm ON p.ID = pm.post_id
			INNER JOIN {$mndb->prefix}woocommerce_order_items AS i ON p.ID = i.order_id
			INNER JOIN {$mndb->prefix}woocommerce_order_itemmeta AS im ON i.order_item_id = im.order_item_id
...
function wc_get_customer_download_permissions( $customer_id ) {
	...
	return apply_filters( 'woocommerce_permission_list', $mndb->get_results(
		$mndb->prepare( "
			SELECT * FROM {$mndb->prefix}woocommerce_downloadable_product_permissions as permissions
			WHERE user_id = %d
...
function wc_get_customer_total_spent( $user_id ) {
	...
	if ( '' === $spent ) {
		...
		$spent = $mndb->get_var( "SELECT SUM(meta2.meta_value)
			FROM $mndb->posts as posts

			LEFT JOIN {$mndb->postmeta} AS meta ON posts.ID = meta.post_id
			LEFT JOIN {$mndb->postmeta} AS meta2 ON posts.ID = meta2.post_id
...
function wc_get_customer_order_count( $user_id ) {
	...
	if ( '' === $count ) {
		...
		$count = $mndb->get_var( "SELECT COUNT(*)
			FROM $mndb->posts as posts
...
function wc_reset_order_customer_id_on_deleted_user( $user_id ) {
	...
	$mndb->update( $mndb->postmeta, array( 'meta_value' => 0 ), array( 'meta_key' => '_customer_user', 'meta_value' => $user_id ) );
}
...
function wc_disable_author_archives_for_customers() {
	global $mn_query, $author;

	if ( is_author() ) {
		$user = get_user_by( 'id', $author );

		if ( isset( $user->roles[0] ) && 'customer' === $user->roles[0] ) {
			mn_redirect( wc_get_page_permalink( 'shop' ) );
		}
	}
}
 ...
 *
 * Kudos to https://github.com/MN-API/MN-API for offering a better solution.
 *
 * @since 2.6.0
