<?php
/**
 * Deprecated functions from Mtaandao MU and the multisite feature. You shouldn't
 ...
 *
 * @since MU
 * @deprecated 3.0.0 Use mn_generate_password()
 * @see mn_generate_password()
 *
 * @param int $len Optional. The length of password to generate. Default 8.
 */
function generate_random_password( $len = 8 ) {
	_deprecated_function( __FUNCTION__, '3.0.0', 'mn_generate_password()' );
	return mn_generate_password( $len );
}
 ...
 *
 * @since MU
 * @deprecated 3.0.0 Use mn_die()
 * @see mn_die()
 */
function graceful_fail( $message ) {
	_deprecated_function( __FUNCTION__, '3.0.0', 'mn_die()' );
 ...
 *
 * @since MU
 * @deprecated 3.0.0 Use mn_get_sites()
 * @see mn_get_sites()
 *
...
function get_blog_list( $start = 0, $num = 10, $deprecated = '' ) {
	_deprecated_function( __FUNCTION__, '3.0.0', 'mn_get_sites()' );

	global $mndb;
	$blogs = $mndb->get_results( $mndb->prepare("SELECT blog_id, domain, path FROM $mndb->blogs WHERE site_id = %d AND public = '1' AND archived = '0' AND mature = '0' AND spam = '0' AND deleted = '0' ORDER BY registered DESC", $mndb->siteid), ARRAY_A );

	$blog_list = array();
	foreach ( (array) $blogs as $details ) {
		$blog_list[ $details['blog_id'] ] = $details;
		$blog_list[ $details['blog_id'] ]['postcount'] = $mndb->get_var( "SELECT COUNT(ID) FROM " . $mndb->get_blog_prefix( $details['blog_id'] ). "posts WHERE post_status='publish' AND post_type='post'" );
	}
 ...
 *
 * @since MU
 * @deprecated 3.3.0 Use mn_redirect()
 * @see mn_redirect()
 *
 * @param string $url Optional. Redirect URL. Default empty.
 */
function mnmu_admin_do_redirect( $url = '' ) {
	...
	if ( $ref ) {
		$ref = mnmu_admin_redirect_add_updated_param( $ref );
		mn_redirect( $ref );
		exit();
	}
	if ( ! empty( $_SERVER['HTTP_REFERER'] ) ) {
		mn_redirect( $_SERVER['HTTP_REFERER'] );
		exit();
	}

	$url = mnmu_admin_redirect_add_updated_param( $url );
	if ( isset( $_GET['redirect'] ) ) {
		if ( substr( $_GET['redirect'], 0, 2 ) == 's_' )
			$url .= '&action=blogs&s='. esc_html( substr( $_GET['redirect'], 2 ) );
	} elseif ( isset( $_POST['redirect'] ) ) {
		$url = mnmu_admin_redirect_add_updated_param( $_POST['redirect'] );
	}
	mn_redirect( $url );
	exit();
}
 ...
 * @return string
 */
function mnmu_admin_redirect_add_updated_param( $url = '' ) {
	_deprecated_function( __FUNCTION__, '3.3.0' );
...
function create_empty_blog( $domain, $path, $weblog_title, $site_id = 1 ) {
	...
	// Need to back up mndb table names, and create a new mn_blogs entry for new blog.
	// Need to get blog_id from mn_blogs, and create new table names.
...
function get_admin_users_for_domain( $sitedomain = '', $path = '' ) {
	...
	if ( ! $sitedomain )
		...
		$site_id = $mndb->get_var( $mndb->prepare( "SELECT id FROM $mndb->site WHERE domain = %s AND path = %s", $sitedomain, $path ) );

	if ( $site_id )
		return $mndb->get_results( $mndb->prepare( "SELECT u.ID, u.user_login, u.user_pass FROM $mndb->users AS u, $mndb->sitemeta AS sm WHERE sm.meta_key = 'admin_user_id' AND u.ID = sm.meta_value AND sm.site_id = %d", $site_id ), ARRAY_A );
 ...
 * }
 * @return array An empty array if the install is considered "large" via mn_is_large_network(). Otherwise,
