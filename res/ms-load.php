 ...
 */
function mn_get_active_network_plugins() {
 ...
 * @since 3.9.0
 *
 * @internal In 4.4.0, converted to a wrapper for MN_Network::get_by_path()
 *
 * @param string   $domain   Domain to check.
 * @param string   $path     Path to check.
 * @param int|null $segments Path segments to use. Defaults to null, or the full path.
 * @return MN_Network|false Network object if successful. False when no network is found.
 */
function get_network_by_path( $domain, $path, $segments = null ) {
	return MN_Network::get_by_path( $domain, $path, $segments );
}
...
function get_site_by_path( $domain, $path, $segments = null ) {
	 ...
	 * @since 3.9.0
	 *
	 * @param int|null $segments The number of path segments to consider. Mtaandao by default looks at
 ...
 * @global MN_Site    $current_blog The current site.
 *
 * @param string $domain    The requested domain.
...
function ms_load_current_site_and_network( $domain, $path, $subdomain = false ) {
	...
	if ( defined( 'DOMAIN_CURRENT_SITE' ) && defined( 'PATH_CURRENT_SITE' ) ) {
		 ...
		 */
		if ( ! $current_site = mn_cache_get( 'current_network', 'site-options' ) ) {
			// Are there even two networks installed?
			$one_network = $mndb->get_row( "SELECT * FROM $mndb->site LIMIT 2" ); // [sic]
			if ( 1 === $mndb->num_rows ) {
				$current_site = new MN_Network( $one_network );
				mn_cache_add( 'current_network', $current_site, 'site-options' );
			} elseif ( 0 === $mndb->num_rows ) {
		...
		if ( empty( $current_site ) ) {
			$current_site = MN_Network::get_by_path( $domain, $path, 1 );
		}
		...
		$current_blog = get_site_by_path( $domain, $path, 1 );
		if ( $current_blog ) {
			$current_site = MN_Network::get_instance( $current_blog->site_id ? $current_blog->site_id : 1 );
		} else {
	...
	if ( $current_blog && $current_blog->site_id != $current_site->id ) {
		$current_site = MN_Network::get_instance( $current_blog->site_id );
	}

	// No network has been found, bail.
	if ( empty( $current_site ) ) {
		/** This action is documented in res/ms-settings.php */
		do_action( 'ms_network_not_found', $domain, $path );
	...
	if ( empty( $current_blog ) && mn_installing() ) {
	...
	if ( empty( $current_blog ) ) {
		...
		if ( $subdomain && ! defined( 'NOBLOGREDIRECT' ) ) {
			...
			$destination .= 'register.php?new=' . str_replace( '.' . $current_site->domain, '', $domain );
		} elseif ( $subdomain ) {
	...
	if ( empty( $current_site->blog_id ) ) {
		...
		} elseif ( ! $current_site->blog_id = mn_cache_get( 'network:' . $current_site->id . ':main_site', 'site-options' ) ) {
			$current_site->blog_id = $mndb->get_var( $mndb->prepare( "SELECT blog_id FROM $mndb->blogs WHERE domain = %s AND path = %s",
				$current_site->domain, $current_site->path ) );
			mn_cache_add( 'network:' . $current_site->id . ':main_site', $current_site->blog_id, 'site-options' );
		}
	}
 ...
 *
 * Used when a blog's tables do not exist. Checks for a missing $mndb->site table as well.
 *
 * @access private
 * @since 3.0.0
...
function ms_not_installed( $domain, $path ) {
	...
	$query = $mndb->prepare( "SHOW TABLES LIKE %s", $mndb->esc_like( $mndb->site ) );
	if ( ! $mndb->get_var( $query ) ) {
		$msg .= '<p>' . sprintf(
			/* translators: %s: table name */
			__( '<strong>Database tables are missing.</strong> This means that MySQL is not running, Mtaandao was not installed properly, or someone deleted %s. You really should look at your database now.' ),
			'<code>' . $mndb->site . '</code>'
		) . '</p>';
	} else {
		$msg .= '<p>' . sprintf(
	...
	mn_die( $msg, $title, array( 'response' => 500 ) );
}
