...
<table class="wc_status_table widefat" cellspacing="0" id="status">
	<thead>
		<tr>
			<th colspan="3" data-export-label="Mtaandao Environment"><h2><?php _e( 'Mtaandao Environment', 'woocommerce' ); ?></h2></th>
		</tr>
	</thead>
	<tbody>
		<tr>
		...
		<tr>
			<td data-export-label="MN Version"><?php _e( 'MN Version', 'woocommerce' ); ?>:</td>
			<td class="help"><?php echo wc_help_tip( __( 'The version of Mtaandao installed on your site.', 'woocommerce' ) ); ?></td>
			<td><?php bloginfo('version'); ?></td>
		</tr>
		<tr>
			<td data-export-label="MN Multisite"><?php _e( 'MN Multisite', 'woocommerce' ); ?>:</td>
			<td class="help"><?php echo wc_help_tip( __( 'Whether or not you have Mtaandao Multisite enabled.', 'woocommerce' ) ); ?></td>
			<td><?php if ( is_multisite() ) echo '<span class="dashicons dashicons-yes"></span>'; else echo '&ndash;'; ?></td>
		</tr>
		<tr>
			<td data-export-label="MN Memory Limit"><?php _e( 'MN Memory Limit', 'woocommerce' ); ?>:</td>
			<td class="help"><?php echo wc_help_tip( __( 'The maximum amount of memory (RAM) that your site can use at one time.', 'woocommerce' ) ); ?></td>
			<td><?php
				$memory = wc_let_to_num( MN_MEMORY_LIMIT );
				...
				if ( $memory < 67108864 ) {
					echo '<mark class="error"><span class="dashicons dashicons-warning"></span> ' . sprintf( __( '%s - We recommend setting memory to at least 64MB. See: %s', 'woocommerce' ), size_format( $memory ), '<a href="https://codex.wordpress.org/Editing_mn-config.php#Increasing_memory_allocated_to_PHP" target="_blank">' . __( 'Increasing memory allocated to PHP', 'woocommerce' ) . '</a>' ) . '</mark>';
				} else {
		...
		<tr>
			<td data-export-label="MN Debug Mode"><?php _e( 'MN Debug Mode', 'woocommerce' ); ?>:</td>
			<td class="help"><?php echo wc_help_tip( __( 'Displays whether or not Mtaandao is in Debug Mode.', 'woocommerce' ) ); ?></td>
			<td>
				<?php if ( defined( 'MN_DEBUG' ) && MN_DEBUG ) : ?>
		...
		<tr>
			<td data-export-label="MN Cron"><?php _e( 'MN Cron', 'woocommerce' ); ?>:</td>
			<td class="help"><?php echo wc_help_tip( __( 'Displays whether or not MN Cron Jobs are enabled.', 'woocommerce' ) ); ?></td>
			<td>
				<?php if ( defined( 'DISABLE_MN_CRON' ) && DISABLE_MN_CRON ) : ?>
					<mark class="no">&ndash;</mark>
				<?php else : ?>
					<mark class="yes"><span class="dashicons dashicons-yes"></span></mark>
				<?php endif; ?>
		...
		<tr>
			...
			<td class="help"><?php echo wc_help_tip( __( 'The current language used by Mtaandao. Default = English', 'woocommerce' ) ); ?></td>
...
<table class="wc_status_table widefat" cellspacing="0">
	...
	<tbody>
		...
		if ( $mndb->use_mysqli ) {
			$ver = mysqli_get_server_info( $mndb->dbh );
		} else {
			$ver = mysql_get_server_info();
		}

		if ( ! empty( $mndb->is_mysql ) && ! stristr( $ver, 'MariaDB' ) ) : ?>
			<tr>
				...
				<td>
					...
					if ( version_compare( $mysql_version, '5.6', '<' ) ) {
						echo '<mark class="error"><span class="dashicons dashicons-warning"></span> ' . sprintf( __( '%s - We recommend a minimum MySQL version of 5.6. See: %s', 'woocommerce' ), esc_html( $mysql_version ), '<a href="https://mtaandao.co.ke/about/requirements/" target="_blank">' . __( 'Mtaandao Requirements', 'woocommerce' ) . '</a>' ) . '</mark>';
					} else {
		...
		<tr>
			...
			<td class="help"><?php echo wc_help_tip( __( 'The largest filesize that can be uploaded to your Mtaandao installation.', 'woocommerce' ) ); ?></td>
			<td><?php echo size_format( mn_max_upload_size() ); ?></td>
		</tr>
		<tr>
		...
		<?php
			...
			// MN Remote Post Check.
			$posting['mn_remote_post']['name'] = __( 'Remote Post', 'woocommerce');
			$posting['mn_remote_post']['help'] = wc_help_tip( __( 'PayPal uses this method of communicating when sending back transaction information.', 'woocommerce' ) );

			$response = mn_safe_remote_post( 'https://www.paypal.com/cgi-bin/webscr', array(
				'timeout'     => 60,
			...
			if ( ! is_mn_error( $response ) && $response['response']['code'] >= 200 && $response['response']['code'] < 300 ) {
				$posting['mn_remote_post']['success'] = true;
			} else {
				$posting['mn_remote_post']['note']    = __( 'mn_remote_post() failed. PayPal IPN won\'t work with your server. Contact your hosting provider.', 'woocommerce' );
				if ( is_mn_error( $response ) ) {
					$posting['mn_remote_post']['note'] .= ' ' . sprintf( __( 'Error: %s', 'woocommerce' ), wc_clean( $response->get_error_message() ) );
				} else {
					$posting['mn_remote_post']['note'] .= ' ' . sprintf( __( 'Status code: %s', 'woocommerce' ), wc_clean( $response['response']['code'] ) );
				}
				$posting['mn_remote_post']['success'] = false;
			}

			// MN Remote Get Check.
			$posting['mn_remote_get']['name'] = __( 'Remote Get', 'woocommerce');
			$posting['mn_remote_get']['help'] = wc_help_tip( __( 'WooCommerce plugins may use this method of communication when checking for plugin updates.', 'woocommerce' ) );

			$response = mn_safe_remote_get( 'https://woocommerce.com/wc-api/product-key-api?request=ping&network=' . ( is_multisite() ? '1' : '0' ) );

			if ( ! is_mn_error( $response ) && $response['response']['code'] >= 200 && $response['response']['code'] < 300 ) {
				$posting['mn_remote_get']['success'] = true;
			} else {
				$posting['mn_remote_get']['note']    = __( 'mn_remote_get() failed. The WooCommerce plugin updater won\'t work with your server. Contact your hosting provider.', 'woocommerce' );
				if ( is_mn_error( $response ) ) {
					$posting['mn_remote_get']['note'] .= ' ' . sprintf( __( 'Error: %s', 'woocommerce' ), wc_clean( $response->get_error_message() ) );
				} else {
					$posting['mn_remote_get']['note'] .= ' ' . sprintf( __( 'Status code: %s', 'woocommerce' ), wc_clean( $response['response']['code'] ) );
				}
				$posting['mn_remote_get']['success'] = false;
			}
				...
				<tr>
					...
					<td>
						<mark class="<?php echo $mark; ?>">
							<?php echo ! empty( $post['success'] ) ? '<span class="dashicons dashicons-yes"></span>' : '<span class="dashicons dashicons-no-alt"></span>'; ?> <?php echo ! empty( $post['note'] ) ? mn_kses_data( $post['note'] ) : ''; ?>
						</mark>
					</td>
				</tr>
				<?php
		...
		<tr>
			...
			foreach ( $tables as $table ) {
				...
				<tr>
					...
					<td class="help">&nbsp;</td>
					<td><?php echo $mndb->get_var( $mndb->prepare( "SHOW TABLES LIKE %s;", $mndb->prefix . $table ) ) !== $mndb->prefix . $table ? '<mark class="error"><span class="dashicons dashicons-warning"></span> ' . __( 'Table does not exist', 'woocommerce' ) . '</mark>' : '<mark class="yes"><span class="dashicons dashicons-yes"></span></mark>'; ?></td>
				</tr>
				<?php
			}
			...
			if ( ! empty( $plugin_data['Name'] ) ) {
				...
				if ( strstr( $dirname, 'woocommerce-' ) && strstr( $plugin_data['PluginURI'], 'woothemes.com' ) ) {
					...
					if ( false === ( $version_data = get_transient( md5( $plugin ) . '_version_data' ) ) ) {
						$changelog = mn_safe_remote_get( 'http://dzv365zjfbd8v.cloudfront.net/changelogs/' . $dirname . '/changelog.txt' );
						$cl_lines  = explode( "\n", mn_remote_retrieve_body( $changelog ) );
						if ( ! empty( $cl_lines ) ) {
...
<table class="wc_status_table widefat" cellspacing="0">
	...
	<tbody>
		...
		<?php
		if( is_child_theme() ) :
			$parent_theme         = mn_get_theme( $active_theme->Template );
				...
				foreach ( $scanned_files as $file ) {
					...
					if ( ! empty( $theme_file ) ) {
						...
						if ( $core_version && ( empty( $theme_version ) || version_compare( $theme_version, $core_version, '<' ) ) ) {
							...
							$found_files[ $plugin_name ][] = sprintf( __( '<code>%s</code> version <strong style="color:red">%s</strong> is out of date. The core version is %s', 'woocommerce' ), str_replace( MAIN . '/themes/', '', $theme_file ), $theme_version ? $theme_version : '-', $core_version );
						} else {
							$found_files[ $plugin_name ][] = sprintf( '<code>%s</code>', str_replace( MAIN . '/themes/', '', $theme_file ) );
						}
					}
				}
			}
