...
if ( ! current_user_can('read') )
	mn_die( __( 'Sorry, you are not allowed to access this page.' ) );
...
if ( 'updateblogsettings' == $action && isset( $_POST['primary_blog'] ) ) {
	...
	if ( $blog && isset( $blog->domain ) ) {
		...
		mn_die( __( 'The primary site you chose does not exist.' ) );
	}
}
...
get_current_screen()->add_help_tab( array(
	...
	'content' =>
		...
		'<p>' . __('Up until Mtaandao version 3.0, what is now called a Multisite Network had to be installed separately as Mtaandao MU (multi-user).') . '</p>'
) );

get_current_screen()->set_help_sidebar(
	'<p><strong>' . __('For more information:') . '</strong></p>' .
	'<p>' . __('<a href="https://mtaandao.co.ke/docs/Dashboard_My_Sites_Screen" target="_blank">Documentation on My Sites</a>') . '</p>' .
...
if ( in_array( get_site_option( 'registration' ), array( 'all', 'blog' ) ) ) {
	...
	$sign_up_url = apply_filters( 'mn_signup_location', network_site_url( 'register.php' ) );
...
<form id="myblogs" method="post">
	...
	if ( count( $blogs ) > 1 || has_action( 'myblogs_allblogs_options' ) || has_filter( 'myblogs_options' ) ) {
		...
		mn_nonce_field( 'update-my-sites' );
		submit_button();
	}
	?>
	</form>
