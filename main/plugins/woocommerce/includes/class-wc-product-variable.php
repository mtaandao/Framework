...
class WC_Product_Variable extends WC_Product {
	...
	public function get_variation_prices( $display = false ) {
		...
		foreach ( $filter_names as $filter_name ) {
			if ( ! empty( $mn_filter[ $filter_name ] ) ) {
				$price_hash[ $filter_name ] = array();

				foreach ( $mn_filter[ $filter_name ] as $priority => $callbacks ) {
					$price_hash[ $filter_name ][] = array_values( mn_list_pluck( $callbacks, 'function' ) );
				}
			}
		}
	...
	public function get_variation_attributes() {
		...
		if ( ! empty( $child_ids ) ) {
			foreach ( $attributes as $attribute ) {
				...
				$values = array_unique( $mndb->get_col( $mndb->prepare(
					"SELECT meta_value FROM {$mndb->postmeta} WHERE meta_key = %s AND post_id IN (" . implode( ',', array_map( 'esc_sql', $child_ids ) ) . ")",
					wc_variation_attribute_name( $attribute['name'] )
				) ) );

				// empty value indicates that all options for given attribute are available
				if ( in_array( '', $values ) ) {
					$values = $attribute['is_taxonomy'] ? mn_get_post_terms( $this->id, $attribute['name'], array( 'fields' => 'slugs' ) ) : wc_get_text_attributes( $attribute['value'] );
	...
	/**
	 * Match a variation to a given set of attributes using a MN_Query.
	 * @since  2.4.0
	...
	public function get_matching_variation( $match_attributes = array() ) {
		...
		$mndb->query( 'SET SESSION SQL_BIG_SELECTS=1' );

		$matches = get_posts( $query_args );

		if ( $matches && ! is_mn_error( $matches ) ) {
	...
	public function get_available_variation( $variation ) {
		...
		if ( has_post_thumbnail( $variation->get_variation_id() ) ) {
			...
			$attachment        = mn_get_attachment_image_src( $attachment_id, 'shop_single' );
			$full_attachment   = mn_get_attachment_image_src( $attachment_id, 'full' );
			$attachment_object = get_post( $attachment_id );
			$image             = $attachment ? current( $attachment ) : '';
			$image_link        = $full_attachment ? current( $full_attachment ) : '';
			$image_title       = get_the_title( $attachment_id );
			$image_alt         = trim( strip_tags( get_post_meta( $attachment_id, '_mn_attachment_image_alt', true ) ) );
			$image_caption     = $attachment_object->post_excerpt;
			$image_srcset      = function_exists( 'mn_get_attachment_image_srcset' ) ? mn_get_attachment_image_srcset( $attachment_id, 'shop_single' ) : false;
			$image_sizes       = function_exists( 'mn_get_attachment_image_sizes' ) ? mn_get_attachment_image_sizes( $attachment_id, 'shop_single' ) : false;
		...
		$availability_html = empty( $availability['availability'] ) ? '' : '<p class="stock ' . esc_attr( $availability['class'] ) . '">' . mn_kses_post( $availability['availability'] ) . '</p>';
