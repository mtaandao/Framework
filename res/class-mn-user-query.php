 ...
 * @since 3.1.0
 *
 * @see MN_User_Query::prepare_query() for information on accepted arguments.
 */
class MN_User_Query {
	 ...
	 * @access public
	 *
	 * @param array $args Query vars, as passed to `MN_User_Query`.
	...
	public static function fill_query_vars( $args ) {
		...
		return mn_parse_args( $args, $defaults );
	}
	...
	public function prepare_query( $query = array() ) {
		...
		/**
		 * Fires before the MN_User_Query has been parsed.
		 *
		 * The passed MN_User_Query object contains the query variables, not
		 * yet passed into SQL.
		 *
		 * @since 4.0.0
		 *
		 * @param MN_User_Query $this The current MN_User_Query instance,
		 *                            passed by reference.
		 */
		...
		if ( is_array( $qv['fields'] ) ) {
			...
			foreach ( $qv['fields'] as $field ) {
				...
				$this->query_fields[] = "$mndb->users.$field";
			}
			$this->query_fields = implode( ',', $this->query_fields );
		} elseif ( 'all' == $qv['fields'] ) {
			$this->query_fields = "$mndb->users.*";
		} else {
			$this->query_fields = "$mndb->users.ID";
		}
		...
		if ( ! empty( $qv['include'] ) ) {
			$include = mn_parse_id_list( $qv['include'] );
		} else {
			$include = false;
		}
		...
		if ( $qv['has_published_posts'] && $blog_id ) {
			...
			foreach ( $post_types as &$post_type ) {
				$post_type = $mndb->prepare( '%s', $post_type );
			}

			$posts_table = $mndb->get_blog_prefix( $blog_id ) . 'posts';
			$this->query_where .= " AND $mndb->users.ID IN ( SELECT DISTINCT $posts_table.post_author FROM $posts_table WHERE $posts_table.post_status = 'publish' AND $posts_table.post_type IN ( " . join( ", ", $post_types ) . " ) )";
		}
		...
		if ( isset( $qv['who'] ) && 'authors' == $qv['who'] && $blog_id ) {
			$who_query = array(
				'key' => $mndb->get_blog_prefix( $blog_id ) . 'user_level',
				'value' => 0,
				'compare' => '!=',
			);
		...
		if ( $blog_id && ( ! empty( $roles ) || ! empty( $role__in ) || ! empty( $role__not_in ) || is_multisite() ) ) {
			...
			if ( ! empty( $roles ) ) {
				foreach ( $roles as $role ) {
					$roles_clauses[] = array(
						'key'     => $mndb->get_blog_prefix( $blog_id ) . 'capabilities',
						'value'   => '"' . $role . '"',
						'compare' => 'LIKE',
					);
				}
		...
		if ( ! empty( $this->meta_query->queries ) ) {
			$clauses = $this->meta_query->get_sql( 'user', $mndb->users, 'ID', $this );
		...
		if ( isset( $qv['number'] ) && $qv['number'] > 0 ) {
			if ( $qv['offset'] ) {
				$this->query_limit = $mndb->prepare("LIMIT %d, %d", $qv['offset'], $qv['number']);
			} else {
				$this->query_limit = $mndb->prepare( "LIMIT %d, %d", $qv['number'] * ( $qv['paged'] - 1 ), $qv['number'] );
			}
		}
		...
		if ( $search ) {
			...
			if ( ! $search_columns ) {
				...
				elseif ( preg_match('|^https?://|', $search) && ! ( is_multisite() && mn_is_large_network( 'users' ) ) )
			...
			/**
			 * Filters the columns to search in a MN_User_Query search.
			 *
		...
		if ( ! empty( $include ) ) {
			// Sanitized earlier.
			$ids = implode( ',', $include );
			$this->query_where .= " AND $mndb->users.ID IN ($ids)";
		} elseif ( ! empty( $qv['exclude'] ) ) {
			$ids = implode( ',', mn_parse_id_list( $qv['exclude'] ) );
			$this->query_where .= " AND $mndb->users.ID NOT IN ($ids)";
		}

		// Date queries are allowed for the user_registered field.
		if ( ! empty( $qv['date_query'] ) && is_array( $qv['date_query'] ) ) {
			$date_query = new MN_Date_Query( $qv['date_query'], 'user_registered' );
			$this->query_where .= $date_query->get_sql();
		}

		/**
		 * Fires after the MN_User_Query has been parsed, and before
		 * the query is executed.
		 *
		 * The passed MN_User_Query object contains SQL parts formed
	...
	public function query() {
		...
		if ( is_array( $qv['fields'] ) || 'all' == $qv['fields'] ) {
			$this->results = $mndb->get_results( $this->request );
		} else {
			$this->results = $mndb->get_col( $this->request );
		}

		/**
		 * Filters SELECT FOUND_ROWS() query for the current MN_User_Query instance.
		 *
		 * @since 3.2.0
		 *
		 * @global mndb $mndb Mtaandao database abstraction object.
		 *
		 * @param string $sql The SELECT FOUND_ROWS() query for the current MN_User_Query.
		 */
		if ( isset( $qv['count_total'] ) && $qv['count_total'] )
			$this->total_users = $mndb->get_var( apply_filters( 'found_users_query', 'SELECT FOUND_ROWS()' ) );
		...
		if ( 'all_with_meta' == $qv['fields'] ) {
			...
			foreach ( $this->results as $userid )
				$r[ $userid ] = new MN_User( $userid, '', $qv['blog_id'] );

			$this->results = $r;
		} elseif ( 'all' == $qv['fields'] ) {
			foreach ( $this->results as $key => $user ) {
				$this->results[ $key ] = new MN_User( $user, '', $qv['blog_id'] );
			}
		}
	}
	...
	protected function get_search_sql( $string, $cols, $wild = false ) {
		...
		$like = $leading_wild . $mndb->esc_like( $string ) . $trailing_wild;

		foreach ( $cols as $col ) {
			if ( 'ID' == $col ) {
				$searches[] = $mndb->prepare( "$col = %s", $string );
			} else {
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( in_array( $orderby, array( 'login', 'nicename', 'email', 'url', 'registered' ) ) ) {
			...
			) p ON ({$mndb->users}.ID = p.post_author)
			";
			$_orderby = 'post_count';
		} elseif ( 'ID' == $orderby || 'id' == $orderby ) {
			$_orderby = 'ID';
		} elseif ( 'meta_value' == $orderby || $this->get( 'meta_key' ) == $orderby ) {
			$_orderby = "$mndb->usermeta.meta_value";
		} elseif ( 'meta_value_num' == $orderby ) {
			$_orderby = "$mndb->usermeta.meta_value+0";
		} elseif ( 'include' === $orderby && ! empty( $this->query_vars['include'] ) ) {
			$include = mn_parse_id_list( $this->query_vars['include'] );
			$include_sql = implode( ',', $include );
			$_orderby = "FIELD( $mndb->users.ID, $include_sql )";
