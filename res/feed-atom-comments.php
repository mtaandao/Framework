...
<feed
	...
	<?php
		/** This action is documented in res/feed-atom.php */
		do_action( 'atom_ns' );
...
>
	<title type="text"><?php
		...
		elseif ( is_search() )
			...
			printf( ent2ncr( __( 'Comments for %s' ) ), get_mn_title_rss() );
	?></title>
...
<?php else : // This comment is in reply to another comment
	...
	// The rel attribute below and the id tag above should be GUIDs, but MN doesn't create them for comments (unlike posts). Either way, it's more important that they both use the same system
?>
