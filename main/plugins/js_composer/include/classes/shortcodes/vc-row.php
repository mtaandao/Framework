...
class WPBakeryShortCode_VC_Row extends WPBakeryShortCode {
	...
	protected function shortcodeScripts() {
		mn_register_script( 'vc_jquery_skrollr_js', vc_asset_url( 'lib/bower/skrollr/dist/skrollr.min.js' ), array( 'jquery' ), WPB_VC_VERSION, true );
		mn_register_script( 'vc_youtube_iframe_api_js', '//www.youtube.com/iframe_api', array(), WPB_VC_VERSION, true );
	}
	...
	public function buildStyle( $bg_image = '', $bg_color = '', $bg_image_repeat = '', $font_color = '', $padding = '', $margin_bottom = '' ) {
		...
		$has_image = false;
		$style = '';
		if ( (int) $bg_image > 0 && false !== ( $image_url = mn_get_attachment_url( $bg_image ) ) ) {
			$has_image = true;
