...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="https://mtaandao.co.ke/docs/Network_Admin_Settings_Screen" target="_blank">Documentation on Network Settings</a>') . '</p>' .
...
if ( $_POST ) {
	...
	$options = array(
		...
		'illegal_names', 'limited_email_domains', 'banned_email_domains', 'MNLANG', 'admin_email',
		'first_comment_email',
	);

	// Handle translation install.
	if ( ! empty( $_POST['MNLANG'] ) && mn_can_install_language_pack() ) {  // @todo: Skip if already installed
	...
	foreach ( $options as $option_name ) {
		if ( ! isset($_POST[$option_name]) )
			continue;
		$value = mn_unslash( $_POST[$option_name] );
	 ...
	 * @since MU
	 */
	do_action( 'update_mnmu_options' );

	mn_redirect( add_query_arg( 'updated', 'true', network_admin_url( 'settings.php' ) ) );
	exit();
}
...
<div class="wrap">
	...
	<form method="post" action="settings.php" novalidate="novalidate">
		<?php mn_nonce_field( 'siteoptions' ); ?>
			...
			<tr>
				...
				<td>
					...
					<?php if ( is_subdomain_install() ) {
						...
						/* translators: 1: NOBLOGREDIRECT 2: configuration.php */
...
<?php echo esc_textarea( get_site_option( 'first_comment' ) ) ?></textarea>
		...
		if ( ! empty( $languages ) || ! empty( $translations ) ) {
			...
			<table class="form-table">
				<tr>
					<th><label for="MNLANG"><?php _e( 'Default Language' ); ?></label></th>
					<td>
						<?php
