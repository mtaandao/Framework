...
class WC_Admin_API_Keys_Table_List extends MN_List_Table {
	...
	public function column_description( $key ) {
		...
		$actions = array(
			...
			'trash' => '<a class="submitdelete" title="' . esc_attr__( 'Revoke API Key', 'woocommerce' ) . '" href="' . esc_url( mn_nonce_url( add_query_arg( array( 'revoke-key' => $key['key_id'] ), admin_url( 'admin.php?page=wc-settings&tab=api&section=keys' ) ), 'revoke' ) ) . '">' . __( 'Revoke', 'woocommerce' ) . '</a>'
		);
	...
	public function prepare_items() {
		...
		if ( ! empty( $_REQUEST['s'] ) ) {
			$search = "AND description LIKE '%" . esc_sql( $mndb->esc_like( wc_clean( $_REQUEST['s'] ) ) ) . "%' ";
		}

		// Get the API keys
		$keys = $mndb->get_results(
			"SELECT key_id, user_id, description, permissions, truncated_key, last_access FROM {$mndb->prefix}woocommerce_api_keys WHERE 1 = 1 {$search}" .
			$mndb->prepare( "ORDER BY key_id DESC LIMIT %d OFFSET %d;", $per_page, $offset ), ARRAY_A
		);

		$count = $mndb->get_var( "SELECT COUNT(key_id) FROM {$mndb->prefix}woocommerce_api_keys WHERE 1 = 1 {$search};" );
