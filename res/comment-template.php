 ...
 *
 * These functions are meant to live inside of the Mtaandao loop.
 *
 * @package Mtaandao
 * @subpackage Template
 */
 ...
 *
 * @since 1.5.0
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MN_Comment object.
 *
 * @param int|MN_Comment $comment_ID Optional. MN_Comment or the ID of the comment for which to retrieve the author.
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MN_Comment object.
 *
 * @param int|MN_Comment $comment_ID Optional. MN_Comment or the ID of the comment for which to print the author.
 *									 Default current comment.
 */
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MN_Comment object.
 *
 * @param int|MN_Comment $comment_ID Optional. MN_Comment or the ID of the comment for which to get the author's email.
 *									 Default current comment.
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MN_Comment object.
 *
 * @param int|MN_Comment $comment_ID Optional. MN_Comment or the ID of the comment for which to print the author's email.
 *									 Default current comment.
 */
 ...
 *
 * @since 0.71
 * @since 16.10.0 Added the `$comment` parameter.
 *
 ...
 * @param int|MN_Comment $comment  Optional. Comment ID or MN_Comment object. Default is the current comment.
 */
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MN_Comment object.
 *
 * @param int|MN_Comment $comment_ID Optional. MN_Comment or the ID of the comment for which to get the author's link.
 *									 Default current comment.
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MN_Comment object.
 *
 * @param int|MN_Comment $comment_ID Optional. MN_Comment or the ID of the comment for which to print the author's link.
 *									 Default current comment.
 */
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MN_Comment object.
 *
 * @param int|MN_Comment $comment_ID Optional. MN_Comment or the ID of the comment for which to get the author's IP address.
 *									 Default current comment.
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MN_Comment object.
 *
 * @param int|MN_Comment $comment_ID Optional. MN_Comment or the ID of the comment for which to print the author's IP address.
 *									 Default current comment.
 */
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MN_Comment object.
 *
 * @param int|MN_Comment $comment_ID Optional. MN_Comment or the ID of the comment for which to get the author's URL.
 *									 Default current comment.
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MN_Comment object.
 *
 * @param int|MN_Comment $comment_ID Optional. MN_Comment or the ID of the comment for which to print the author's URL.
 *									 Default current comment.
 */
 ...
 *                                 Default empty.
 * @param int|MN_Comment $comment  Optional. Comment ID or MN_Comment object.
 ...
 *
 * @since 2.7.0
 * @since 4.4.0 Added the ability for `$comment` to also accept a MN_Comment object.
 *
 * @param string|array   $class    Optional. One or more classes to add to the class list.
 *                                 Default empty.
 * @param int|MN_Comment $comment  Comment ID or MN_Comment object. Default current comment.
 * @param int|MN_Post    $post_id  Post ID or MN_Post object. Default current post.
 ...
 * @param int|MN_Comment $comment_id Comment ID or MN_Comment object. Default current comment.
 * @param int|MN_Post    $post_id    Post ID or MN_Post object. Default current post.
...
function get_comment_class( $class = '', $comment_id = null, $post_id = null ) {
	 ...
	 * @param int         $comment_id The comment id.
	 * @param MN_Comment  $comment    The comment object.
	 * @param int|MN_Post $post_id    The post ID or MN_Post object.
	 */
 ...
 * @param int|MN_Comment  $comment_ID MN_Comment or ID of the comment for which to get the date.
 *                                    Default current comment.
 ...
 * @param int|MN_Comment $comment_ID MN_Comment or ID of the comment for which to print the date.
 *                                   Default current comment.
 */
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MN_Comment object.
 *
 * @param int|MN_Comment $comment_ID  MN_Comment or ID of the comment for which to get the excerpt.
 *                                    Default current comment.
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MN_Comment object.
 *
 * @param int|MN_Comment $comment_ID  MN_Comment or ID of the comment for which to print the excerpt.
 *                                    Default current comment.
 */
 ...
 *
 * @since 1.5.0
 * @since 4.4.0 Added the ability for `$comment` to also accept a MN_Comment object. Added `$cpage` argument.
 *
 ...
 *
 * @param MN_Comment|int|null $comment Comment to retrieve. Default current comment.
 * @param array               $args {
...
function get_comment_link( $comment = null, $args = array() ) {
	global $mn_rewrite, $in_comment_loop;
	...
	if ( $cpage && get_option( 'page_comments' ) ) {
		if ( $mn_rewrite->using_permalinks() ) {
			if ( $cpage ) {
				$link = trailingslashit( $link ) . $mn_rewrite->comments_pagination_base . '-' . $cpage;
			}
	 ...
	 * @param MN_Comment $comment The current comment object.
...
function get_comments_link( $post_id = 0 ) {
	 ...
	 * @param int|MN_Post $post_id       Post ID or MN_Post object.
	 */
 ...
 *
 * @param int|MN_Comment  $comment_ID MN_Comment or ID of the comment for which to get the text.
 *                                    Default current comment.
 ...
 * @see Walker_Comment::comment()
 *
 * @param int|MN_Comment  $comment_ID MN_Comment or ID of the comment for which to print the text.
 *                                    Default current comment.
 ...
 * @since 4.4.0 Added the ability for `$comment_ID` to also accept a MN_Comment object.
 *
 * @param int|MN_Comment $comment_ID Optional. MN_Comment or ID of the comment for which to get the type.
 *                                   Default current comment.
...
function trackback_rdf( $deprecated = '' ) {
	...
	the_permalink();
	echo '"'."\n";
	echo '    dc:title="'.str_replace('--', '&#x2d;&#x2d;', mntexturize(strip_tags(get_the_title()))).'"'."\n";
 ...
 * @since 2.1.3
 */
function mn_comment_form_unfiltered_html_nonce() {
	$post = get_post();
	$post_id = $post ? $post->ID : 0;

	if ( current_user_can( 'unfiltered_html' ) ) {
		mn_nonce_field( 'unfiltered-html-comment_' . $post_id, '_mn_unfiltered_html_comment_disabled', false );
		echo "<script>(function(){if(window===window.parent){document.getElementById('_mn_unfiltered_html_comment_disabled').name='_mn_unfiltered_html_comment';}})();</script>\n";
	}
}
 ...
 *
 * Uses the Mtaandao database object to query for the comments. The comments
 ...
 * default theme. If either does not exist, then the Mtaandao process will be
...
function comments_template( $file = '/comments.php', $separate_comments = false ) {
	global $mn_query, $withcomments, $post, $mndb, $id, $comment, $user_login, $user_ID, $user_identity, $overridden_cpage;
	...
	if ( get_option( 'page_comments' ) ) {
		...
		if ( $page ) {
			...
			$top_level_query = new MN_Comment_Query();
	 ...
	 * @since 4.5.0
	 *
	 * @see MN_Comment_Query::__construct()
	 *
	 * @param array $comment_args {
	 *     Array of MN_Comment_Query arguments.
	 *
	...
	$comment_query = new MN_Comment_Query( $comment_args );
	 ...
	 * @param int   $post_ID  Post ID.
	 */
	$mn_query->comments = apply_filters( 'comments_array', $comments_flat, $post->ID );

	$comments = &$mn_query->comments;
	$mn_query->comment_count = count($mn_query->comments);
	$mn_query->max_num_comment_pages = $comment_query->max_num_pages;

	if ( $separate_comments ) {
		$mn_query->comments_by_type = separate_comments($comments);
		$comments_by_type = &$mn_query->comments_by_type;
	} else {
		$mn_query->comments_by_type = array();
	}

	$overridden_cpage = false;
	if ( '' == get_query_var( 'cpage' ) && $mn_query->max_num_comment_pages > 1 ) {
 ...
 * }
 * @param int|MN_Comment $comment Comment being replied to. Default current comment.
 * @param int|MN_Post    $post    Post ID or MN_Post object the comment is going to be displayed on.
...
function get_comment_reply_link( $args = array(), $comment = null, $post = null ) {
	 ...
	 * @param MN_Comment $comment The object of the comment being replied to.
	 * @param MN_Post    $post    The MN_Post object.
	 */
	$args = apply_filters( 'comment_reply_link_args', $args, $comment, $post );

	if ( get_option( 'comment_registration' ) && ! is_user_logged_in() ) {
		$link = sprintf( '<a rel="nofollow" class="comment-reply-login" href="%s">%s</a>',
			esc_url( mn_login_url( get_permalink() ) ),
			$args['login_text']
		);
	} else {
 ...
 * }
 * @param int|MN_Post $post    Optional. Post ID or MN_Post object the comment is going to be displayed on.
 *                             Default current post.
...
function get_post_reply_link($args = array(), $post = null) {
	 ...
	 * @param int|MN_Post $post      The post ID or MN_Post object.
	 */
 ...
 *           comment. See https://core.trac.mtaandao.co.ke/changeset/36512.
 *
 * @since 2.7.0
 *
 ...
 * }
 * @param array $comments Optional. Array of MN_Comment objects.
 */
function mn_list_comments( $args = array(), $comments = null ) {
	global $mn_query, $comment_alt, $comment_depth, $comment_thread_alt, $overridden_cpage, $in_comment_loop;
	 ...
	 * @since 4.0.0
	 *
	 * @see mn_list_comments()
	 *
	 * @param array $r An array of arguments for displaying comments.
	 */
	$r = apply_filters( 'mn_list_comments_args', $r );
	...
	if ( null !== $comments ) {
		...
		/*
		 * If 'page' or 'per_page' has been passed, and does not match what's in $mn_query,
		 * perform a separate comment query and allow Walker_Comment to paginate.
		 */
		if ( $r['page'] || $r['per_page'] ) {
			$current_cpage = get_query_var( 'cpage' );
			if ( ! $current_cpage ) {
				$current_cpage = 'newest' === get_option( 'default_comments_page' ) ? 1 : $mn_query->max_num_comment_pages;
			}
		...
		// Otherwise, fall back on the comments from `$mn_query->comments`.
		} else {
			if ( empty($mn_query->comments) )
				return;
			if ( 'all' != $r['type'] ) {
				if ( empty($mn_query->comments_by_type) )
					$mn_query->comments_by_type = separate_comments($mn_query->comments);
				if ( empty($mn_query->comments_by_type[$r['type']]) )
					return;
				$_comments = $mn_query->comments_by_type[$r['type']];
			} else {
				$_comments = $mn_query->comments;
			}

			if ( $mn_query->max_num_comment_pages ) {
	...
	mn_queue_comments_for_comment_meta_lazyload( $_comments );
 ...
 * @since 16.10.0 Introduced the 'action' argument.
 *
 * @param array       $args {
 ...
 *     @type string $action               The comment form element action attribute. Default '/article-comments.php'.
 ...
 * }
 * @param int|MN_Post $post_id Post ID or MN_Post object to generate the form for. Default current post.
 */
function comment_form( $args = array(), $post_id = null ) {
	...
	$defaults = array(
		...
		/** This filter is documented in res/link-template.php */
		'must_log_in'          => '<p class="must-log-in">' . sprintf(
		                              /* translators: %s: login URL */
		                              __( 'You must be <a href="%s">logged in</a> to post a comment.' ),
		                              mn_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) )
		                          ) . '</p>',
		/** This filter is documented in res/link-template.php */
		'logged_in_as'         => '<p class="logged-in-as">' . sprintf(
		                              ...
		                              $user_identity,
		                              mn_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) )
		                          ) . '</p>',
		'comment_notes_before' => '<p class="comment-notes"><span id="email-notes">' . __( 'Your email address will not be published.' ) . '</span>'. ( $req ? $required_text : '' ) . '</p>',
		'comment_notes_after'  => '',
		'action'               => site_url( '/article-comments.php' ),
		'id_form'              => 'commentform',
		'id_submit'            => 'submit',
	 ...
	 */
	$args = mn_parse_args( $args, apply_filters( 'comment_form_defaults', $defaults ) );
