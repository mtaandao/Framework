...
class Custom_Image_Header {
	...
	public function __construct($admin_header_callback, $admin_image_div_callback = '') {
		...
		add_action( 'mn_ajax_custom-header-crop',   array( $this, 'ajax_header_crop'        ) );
		add_action( 'mn_ajax_custom-header-add',    array( $this, 'ajax_header_add'         ) );
		add_action( 'mn_ajax_custom-header-remove', array( $this, 'ajax_header_remove'      ) );
	}
	...
	public function help() {
		...
		get_current_screen()->add_help_tab( array(
			...
			'content' =>
				...
				'<p>' . __( 'If your theme has more than one default header image, or you have uploaded more than one custom header image, you have the option of having Mtaandao display a randomly different image on each page of your site. Click the &#8220;Random&#8221; radio button next to the Uploaded Images or Default Images section to enable this feature.') . '</p>' .
		...
		get_current_screen()->set_help_sidebar(
			...
			'<p>' . __( '<a href="https://mtaandao.co.ke/docs/Appearance_Header_Screen" target="_blank">Documentation on Custom Header</a>' ) . '</p>' .
	...
	public function step() {
		...
		$step = (int) $_GET['step'];
		if ( $step < 1 || 3 < $step ||
			( 2 == $step && ! mn_verify_nonce( $_REQUEST['_mnnonce-custom-header-upload'], 'custom-header-upload' ) ) ||
			( 3 == $step && ! mn_verify_nonce( $_REQUEST['_mnnonce'], 'custom-header-crop-image' ) )
		)
			return 1;
	...
	public function js_includes() {
		...
		if ( ( 1 == $step || 3 == $step ) ) {
			mn_enqueue_media();
			mn_enqueue_script( 'custom-header' );
			if ( current_theme_supports( 'custom-header', 'header-text' ) )
				mn_enqueue_script( 'mn-color-picker' );
		} elseif ( 2 == $step ) {
			mn_enqueue_script('imgareaselect');
		}
	}
	...
	public function take_action() {
		...
		if ( isset( $_POST['resetheader'] ) ) {
			check_admin_referer( 'custom-header-options', '_mnnonce-custom-header-options' );
	...
	public function process_default_headers() {
		...
		$this->default_headers = $_mn_default_headers;
...
(function($){
	...
	$(document).ready(function() {
		...
		text_color.mnColorPicker({
			change: function( event, ui ) {
				pickColor( text_color.mnColorPicker('color') );
			},
			clear: function() {
				pickColor( '' );
			}
...
<td>
	...
	?></p>
	<form enctype="multipart/form-data" id="upload-form" class="mn-upload-form" method="post" action="<?php echo esc_url( add_query_arg( 'step', 2 ) ) ?>">
	<p>
		<label for="upload"><?php _e( 'Choose an image from your computer:' ); ?></label><br />
		<input type="file" id="upload" name="import" />
		<input type="hidden" name="action" value="save" />
		<?php mn_nonce_field( 'custom-header-upload', '_mnnonce-custom-header-upload' ); ?>
	...
	<?php
		$modal_update_href = esc_url( add_query_arg( array(
			'page' => 'custom-header',
			'step' => 2,
			'_mnnonce-custom-header-upload' => mn_create_nonce('custom-header-upload'),
...
mn_nonce_field( 'custom-header-options', '_mnnonce-custom-header-options' ); ?>
...
<?php }
	...
	public function step_2() {
		check_admin_referer('custom-header-upload', '_mnnonce-custom-header-upload');
		...
		if ( empty( $_POST ) && isset( $_GET['file'] ) ) {
			...
			$url = mn_get_attachment_image_src( $attachment_id, 'full' );
			$url = $url[0];
		} elseif ( isset( $_POST ) ) {
		...
		if ( file_exists( $file ) ) {
			...
			$data = mn_get_attachment_metadata( $attachment_id );
			...
			&& $width == get_theme_support( 'custom-header', 'width' ) && $height == get_theme_support( 'custom-header', 'height' ) )
		{
			 ...
			 */
			do_action( 'mn_create_file_in_uploads', $file, $attachment_id ); // For replication

			return $this->finished();
		} elseif ( $width > $max_width ) {
			$oitar = $width / $max_width;
			$image = mn_crop_image($attachment_id, 0, 0, $width, $height, $max_width, $height / $oitar, false, str_replace(basename($file), 'midsize-'.basename($file), $file));
			if ( ! $image || is_mn_error( $image ) )
				mn_die( __( 'Image could not be processed. Please go back and try again.' ), __( 'Image Processing Error' ) );

			/** This filter is documented in admin/custom-header.php */
			$image = apply_filters( 'mn_create_file_in_uploads', $image, $attachment_id ); // For replication
...
<form method="post" action="<?php echo esc_url(add_query_arg('step', 3)); ?>">
	...
	<?php } ?>
	<?php mn_nonce_field( 'custom-header-crop-image' ) ?>
	...
	public function step_2_manage_upload() {
		...
		$mn_filetype = mn_check_filetype_and_ext( $uploaded_file['tmp_name'], $uploaded_file['name'] );
		if ( ! mn_match_mime_types( 'image', $mn_filetype['type'] ) )
			mn_die( __( 'The uploaded file is not a valid image. Please try again.' ) );

		$file = mn_handle_upload($uploaded_file, $overrides);

		if ( isset($file['error']) )
			mn_die( $file['error'],  __( 'Image Upload Error' ) );
		...
		// Save the data
		$attachment_id = mn_insert_attachment( $object, $file );
	 ...
	 *
	 * @since 2.1.0
	 * @since 4.4.0 Switched to using mn_get_attachment_url() instead of the guid
	...
	public function step_3() {
		...
		if ( empty( $_POST['skip-cropping'] ) )
			$cropped = mn_crop_image( $attachment_id, (int) $_POST['x1'], (int) $_POST['y1'], (int) $_POST['width'], (int) $_POST['height'], $width, $height );
		...
		$url = mn_get_attachment_url( $attachment_id );
		$this->set_header_image( compact( 'url', 'attachment_id', 'width', 'height' ) );

		// Cleanup.
		$medium = str_replace( basename( $original ), 'midsize-' . basename( $original ), $original );
		if ( file_exists( $medium ) ) {
			mn_delete_file( $medium );
		}

		if ( empty( $_POST['create-new-attachment'] ) && empty( $_POST['skip-cropping'] ) ) {
			mn_delete_file( $original );
		}
	...
	public function admin_page() {
		if ( ! current_user_can('edit_theme_options') )
			mn_die(__('Sorry, you are not allowed to customize headers.'));
		$step = $this->step();
		if ( 2 == $step )
			$this->step_2();
		elseif ( 3 == $step )
	...
	final public function set_header_image( $choice ) {
		if ( is_array( $choice ) || is_object( $choice ) ) {
			...
			update_post_meta( $choice['attachment_id'], '_mn_attachment_is_custom_header', get_stylesheet() );
	...
	final public function create_attachment_object( $cropped, $parent_attachment_id ) {
		...
		$parent_url = mn_get_attachment_url( $parent->ID );
	...
	final public function insert_attachment( $object, $cropped ) {
		 ...
		 */
		$metadata = apply_filters( 'mn_header_image_attachment_metadata', $metadata );
	...
	public function ajax_header_crop() {
		...
		if ( ! $cropped || is_mn_error( $cropped ) ) {
			mn_send_json_error( array( 'message' => __( 'Image could not be processed. Please go back and try again.' ) ) );
		}
		...
		$object['url']           = mn_get_attachment_url( $new_attachment_id );;
		$object['width']         = $dimensions['dst_width'];
		$object['height']        = $dimensions['dst_height'];

		mn_send_json_success( $object );
	}
	...
	public function ajax_header_add() {
		...
		$key = '_mn_attachment_custom_header_last_used_' . get_stylesheet();
		update_post_meta( $attachment_id, $key, time() );
		update_post_meta( $attachment_id, '_mn_attachment_is_custom_header', get_stylesheet() );
	...
	public function ajax_header_remove() {
		...
		delete_post_meta( $attachment_id, '_mn_attachment_is_custom_header', get_stylesheet() );
	...
	/**
	 *
	 * @param MN_Customize_Manager $mn_customize
	 */
	public function customize_set_last_used( $mn_customize ) {
		$data = $mn_customize->get_setting( 'header_image_data' )->post_value();
	...
	public function get_uploaded_header_images() {
		...
		$timestamp_key = '_mn_attachment_custom_header_last_used_' . get_stylesheet();
		$alt_text_key = '_mn_attachment_image_alt';
