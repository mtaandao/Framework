...
class WC_Product {
	...
	public function get_gallery_attachment_ids() {
		return apply_filters( 'woocommerce_product_gallery_attachment_ids', array_filter( array_filter( (array) explode( ',', $this->product_image_gallery ) ), 'mn_attachment_is_image' ), $this );
	}
	...
	public function set_stock( $amount = null, $mode = 'set' ) {
		...
		if ( ! is_null( $amount ) && $this->managing_stock() ) {
			...
			switch ( $mode ) {
				case 'add' :
					$mndb->query( $mndb->prepare( "UPDATE {$mndb->postmeta} SET meta_value = meta_value + %f WHERE post_id = %d AND meta_key='_stock'", $amount, $this->id ) );
				break;
				case 'subtract' :
					$mndb->query( $mndb->prepare( "UPDATE {$mndb->postmeta} SET meta_value = meta_value - %f WHERE post_id = %d AND meta_key='_stock'", $amount, $this->id ) );
				break;
				default :
					$mndb->query( $mndb->prepare( "UPDATE {$mndb->postmeta} SET meta_value = %f WHERE post_id = %d AND meta_key='_stock'", $amount, $this->id ) );
				break;
			}

			// Clear caches
			mn_cache_delete( $this->id, 'post_meta' );
	...
	public static function sync_average_rating( $post_id ) {
		...
		if ( $count ) {
			...
			$ratings = $mndb->get_var( $mndb->prepare("
				SELECT SUM(meta_value) FROM $mndb->commentmeta
				LEFT JOIN $mndb->comments ON $mndb->commentmeta.comment_id = $mndb->comments.comment_ID
	...
	public static function sync_rating_count( $post_id ) {
		...
		$counts     = array();
		$raw_counts = $mndb->get_results( $mndb->prepare( "
			SELECT meta_value, COUNT( * ) as meta_value_count FROM $mndb->commentmeta
			LEFT JOIN $mndb->comments ON $mndb->commentmeta.comment_id = $mndb->comments.comment_ID
			WHERE meta_key = 'rating'
			AND comment_post_ID = %d
			AND comment_approved = '1'
	...
	public function get_review_count() {
		...
		if ( ! metadata_exists( 'post', $this->id, '_wc_review_count' ) ) {
			$count = $mndb->get_var( $mndb->prepare("
				SELECT COUNT(*) FROM $mndb->comments
	...
	public function get_related( $limit = 5 ) {
		...
		if ( false === $related_posts || sizeof( $related_posts ) < $limit ) {
			...
			if ( 1 === sizeof( $cats_array ) && 1 === sizeof( $tags_array )) {
				...
				// Get the posts
				$related_posts = $mndb->get_col( implode( ' ', $query ) );
			}
	...
	public function get_attributes() {
		...
		$taxonomies = mn_list_pluck( wc_get_attribute_taxonomies(), 'attribute_name' );
	...
	public function get_image_id() {
		...
		} elseif ( ( $parent_id = mn_get_post_parent_id( $this->id ) ) && has_post_thumbnail( $parent_id ) ) {
	...
	protected function get_related_terms( $term ) {
		...
		$terms = apply_filters( 'woocommerce_get_related_' . $term . '_terms', mn_get_post_terms( $this->id, $term ), $this->id );
	...
	protected function build_related_query( $cats_array, $tags_array, $exclude_ids, $limit ) {
		...
		$query           = array();
		$query['fields'] = "SELECT DISTINCT ID FROM {$mndb->posts} p";
		$query['join']   = " INNER JOIN {$mndb->postmeta} pm ON ( pm.post_id = p.ID AND pm.meta_key='_visibility' )";
		$query['join']  .= " INNER JOIN {$mndb->term_relationships} tr ON (p.ID = tr.object_id)";
		$query['join']  .= " INNER JOIN {$mndb->term_taxonomy} tt ON (tr.term_taxonomy_id = tt.term_taxonomy_id)";
		$query['join']  .= " INNER JOIN {$mndb->terms} t ON (t.term_id = tt.term_id)";

		if ( get_option( 'woocommerce_hide_out_of_stock_items' ) === 'yes' ) {
			$query['join'] .= " INNER JOIN {$mndb->postmeta} pm2 ON ( pm2.post_id = p.ID AND pm2.meta_key='_stock_status' )";
		}
