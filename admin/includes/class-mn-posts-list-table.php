<?php
/**
 * List Table API: MN_Posts_List_Table class
 *
 * @package Mtaandao
 ...
 * @see MN_List_Table
 */
class MN_Posts_List_Table extends MN_List_Table {
	...
	public function __construct( $args = array() ) {
		...
		$this->user_posts_count = intval( $mndb->get_var( $mndb->prepare( "
			SELECT COUNT( 1 )
			FROM $mndb->posts
			WHERE post_type = %s
		...
		if ( 'post' === $post_type && $sticky_posts = get_option( 'sticky_posts' ) ) {
			...
			$this->sticky_posts_count = $mndb->get_var( $mndb->prepare( "SELECT COUNT( 1 ) FROM $mndb->posts WHERE post_type = %s AND post_status NOT IN ('trash', 'auto-draft') AND ID IN ($sticky_posts)", $post_type ) );
		}
	}
	...
	public function prepare_items() {
		global $avail_post_stati, $mn_query, $per_page, $mode;

		// is going to call mn()
		$avail_post_stati = mn_edit_posts_query();

		$this->set_hierarchical_display( is_post_type_hierarchical( $this->screen->post_type ) && 'menu_order title' === $mn_query->query['orderby'] );
		...
		if ( $this->hierarchical_display ) {
			$total_items = $mn_query->post_count;
		} elseif ( $mn_query->found_posts || $this->get_pagenum() === 1 ) {
			$total_items = $mn_query->found_posts;
		} else {
			$post_counts = (array) mn_count_posts( $post_type, 'readable' );
	...
	protected function get_views() {
		...
		$status_links = array();
		$num_posts = mn_count_posts( $post_type, 'readable' );
	...
	protected function categories_dropdown( $post_type ) {
		...
		if ( is_object_in_taxonomy( $post_type, 'category' ) ) {
			...
			mn_dropdown_categories( $dropdown_options );
		}
	}
...
<?php
	...
	public function get_columns() {
		...
		$taxonomies = mn_filter_object_list( $taxonomies, array( 'show_admin_column' => true ), 'and', 'name' );
	...
	public function display_rows( $posts = array(), $level = 0 ) {
		global $mn_query, $per_page;

		if ( empty( $posts ) )
			$posts = $mn_query->posts;
		...
		if ( empty( $_REQUEST['s'] ) ) {
			...
			foreach ( $pages as $page ) {
				...
				if ( $page->post_parent == $page->ID ) {
					$page->post_parent = 0;
					$mndb->update( $mndb->posts, array( 'post_parent' => 0 ), array( 'ID' => $page->ID ) );
	...
	public function column_title( $post ) {
		...
		if ( $can_edit_post && $post->post_status != 'trash' ) {
			$lock_holder = mn_check_post_lock( $post->ID );
		...
		if ( current_user_can( 'delete_post', $post->ID ) ) {
			if ( 'trash' === $post->post_status ) {
				$actions['untrash'] = sprintf(
					'<a href="%s" aria-label="%s">%s</a>',
					mn_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $post->ID ) ), 'untrash-post_' . $post->ID ),
					/* translators: %s: post title */
	...
	<?php endif; // $bulk
		...
		if ( post_type_supports( $screen->post_type, 'author' ) ) :
			...
			if ( is_super_admin() || current_user_can( $post_type_object->cap->edit_others_posts ) ) :
				...
				if ( $authors = mn_dropdown_users( $users_opt ) ) :
	...
	?>
			...
			<div class="inline-edit-group mn-clearfix">
	...
	<?php foreach ( $hierarchical_taxonomies as $taxonomy ) : ?>
			...
			<ul class="cat-checklist <?php echo esc_attr( $taxonomy->name )?>-checklist">
				<?php mn_terms_checklist( null, array( 'taxonomy' => $taxonomy->name ) ) ?>
			</ul>
	...
	<?php
		...
		mn_dropdown_pages( $dropdown_args );
	?>
			</label>
	...
	<?php
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			if ( $bulk ) {
				 ...
				 * @param MN_Post $post_type   The post type slug.
				 */
	...
	?>
		<p class="submit inline-edit-save">
			...
			<?php if ( ! $bulk ) {
				mn_nonce_field( 'inlineeditnonce', '_inline_edit', false );
				?>
