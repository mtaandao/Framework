 ...
 * @author vladimir
 */
class RM_Posts_Edit_Access_Bulk_Action {
    ...
    public function add_css() {
        ...
        if ( !(current_user_can('edit_users') && current_user_can('rm_edit_posts_access')) ) {
            return;
        }
...
?>
        <div id="rm_bulk_post_edit_access_dialog" class="rm-dialog">
            <div id="rm_bulk_post_edit_access_content" style="padding: 10px;">
                <span class="bold">What to do:</span>&nbsp;<input type="radio" name="rm_what_todo" id="rm_what_todo1" value="1" checked >
                <label for="rm_what_todo1">Add to existing data</label>
                <input type="radio" name="rm_what_todo" id="rm_what_todo2" value="2"  >
                <label for="rm_what_todo2">Replace existing data</label>
                <hr/>
                <input type="radio" name="rm_posts_restriction_type" id="rm_posts_restriction_type1" value="1" checked >
                <label for="rm_posts_restriction_type1">Allow</label>
                <input type="radio" name="rm_posts_restriction_type" id="rm_posts_restriction_type2" value="2"  >
                <label for="rm_posts_restriction_type2">Prohibit</label><br>
                edit these Posts (comma separated list of IDs):<br>
                <textarea name="rm_posts" id="rm_posts" rows="2" cols="50"></textarea><br/>
                for these Users (comma separated list of IDs):<br/>
                <textarea name="rm_users" id="rm_users" rows="2" cols="50"></textarea>
            </div>                
        </div>
<?php
        
        mn_enqueue_script('jquery-ui-dialog', false, array('jquery-ui-core','jquery-ui-button', 'jquery') );
        mn_register_script( 'rm-bulk-edit-access', admin_url( '/js/admn/rm-bulk-edit-access.js', RM_PLUGIN_FULL_PATH ) );
        mn_enqueue_script ( 'rm-bulk-edit-access' );      
        mn_localize_script( 'rm-bulk-edit-access', 'rm_bulk_edit_access_data', array(
            'mn_nonce' => mn_create_nonce('role-manager'),
            'action_title' => esc_html__('Edit Access', 'role-manager'),
            'dialog_title' => esc_html(__('Editor Restrictions Helper', 'role-manager')),
            'apply' => esc_html(__('Apply', 'role-manager')),
            'provide_user_ids' => esc_html(__('Provide list of users ID', 'role-manager'))
              ));
    ...
    private function bulk_update_prepare() {
        ...
        if (!current_user_can('rm_edit_posts_access')) {
            $answer = array('result'=>'failrm', 'message'=>html_esc__('You do not have enough permissions for this action.','role-manager'));
            return $answer;
        }
        ...
        if (empty($user_ids_str)) {
            $answer = array('result'=>'failrm', 'message'=>html_esc__('Provide users ID list.','role-manager'));
            return $answer;
        }
        
        $posts_ids_arr = explode(',',$post_ids_str);
        $posts_list_str = RM_Utils::filter_int_array_to_str($posts_ids_arr);
        
        $users_ids_arr = explode(',',$user_ids_str);
        $users_ids_str1 = RM_Utils::filter_int_array_to_str($users_ids_arr);
        if (empty($users_ids_str1)) {
            $answer = array('result'=>'failrm', 'message'=>html_esc__('Provide valid users ID list (integers separated by commas).', 'role-manager'));
            return $answer;
        }        
...
// end of class RM_Posts_Edit_Access_Bulk_Action
