...
class WC_Admin_Webhooks {
	...
	private function update_name( $webhook_id ) {
		...
		$mndb->update( $mndb->posts, array( 'post_title' => $name ), array( 'ID' => $webhook_id ) );
	}
	...
	private function update_secret( $webhook ) {
		$secret = ! empty( $_POST['webhook_secret'] ) ? $_POST['webhook_secret'] : mn_generate_password( 50, true, true );
	...
	private function save() {
		if ( empty( $_REQUEST['_mnnonce'] ) || ! mn_verify_nonce( $_REQUEST['_mnnonce'], 'woocommerce-settings' ) ) {
			mn_die( __( 'Action failed. Please refresh the page and retry.', 'woocommerce' ) );
		}
		...
		// Update date.
		mn_update_post( array( 'ID' => $webhook->id, 'post_modified' => current_time( 'mysql' ) ) );
		...
		if ( isset( $_POST['webhook_status'] ) && 'active' === $_POST['webhook_status'] && $pending_delivery ) {
			...
			if ( is_mn_error( $result ) ) {
				...
				mn_safe_redirect( admin_url( 'admin.php?page=wc-settings&tab=api&section=webhooks&edit-webhook=' . $webhook->id . '&error=' . urlencode( $result->get_error_message() ) ) );
				exit();
			}
		}

		// Redirect to webhook edit page to avoid settings save actions
		mn_safe_redirect( admin_url( 'admin.php?page=wc-settings&tab=api&section=webhooks&edit-webhook=' . $webhook->id . '&updated=1' ) );
		exit();
	}
	...
	private function create() {
		if ( empty( $_REQUEST['_mnnonce'] ) || ! mn_verify_nonce( $_REQUEST['_mnnonce'], 'create-webhook' ) ) {
			mn_die( __( 'Action failed. Please refresh the page and retry.', 'woocommerce' ) );
		}

		if ( ! current_user_can( 'publish_shop_webhooks' ) ) {
			mn_die( __( 'You don\'t have permissions to create Webhooks!', 'woocommerce' ) );
		}

		$webhook_id = mn_insert_post( array(
		...
		if ( is_mn_error( $webhook_id ) ) {
			mn_die( $webhook_id->get_error_messages() );
		}
		...
		mn_redirect( admin_url( 'admin.php?page=wc-settings&tab=api&section=webhooks&edit-webhook=' . $webhook_id . '&created=1' ) );
		exit();
	}
	...
	private function bulk_trash( $webhooks, $delete = false ) {
		foreach ( $webhooks as $webhook_id ) {
			if ( $delete ) {
				mn_delete_post( $webhook_id, true );
			} else {
				mn_trash_post( $webhook_id );
			}
		}
		...
		mn_redirect( admin_url( 'admin.php?page=wc-settings&tab=api&section=webhooks' . $status . '&' . $type . '=' . $qty ) );
		exit();
	}
	...
	private function bulk_untrash( $webhooks ) {
		...
		// Redirect to webhooks page
		mn_redirect( admin_url( 'admin.php?page=wc-settings&tab=api&section=webhooks&status=trash&untrashed=' . $qty ) );
		exit();
	}
	...
	private function bulk_actions() {
		...
		if ( ! current_user_can( 'edit_shop_webhooks' ) ) {
			mn_die( __( 'You don\'t have permissions to edit Webhooks!', 'woocommerce' ) );
		}
	...
	private function empty_trash() {
		if ( empty( $_REQUEST['_mnnonce'] ) || ! mn_verify_nonce( $_REQUEST['_mnnonce'], 'empty_trash' ) ) {
			mn_die( __( 'Action failed. Please refresh the page and retry.', 'woocommerce' ) );
		}

		if ( ! current_user_can( 'delete_shop_webhooks' ) ) {
			mn_die( __( 'You don\'t have permissions to delete Webhooks!', 'woocommerce' ) );
		}
		...
		// Redirect to webhooks page
		mn_redirect( admin_url( 'admin.php?page=wc-settings&tab=api&section=webhooks&deleted=' . $qty ) );
		exit();
	}
	...
	private static function table_list_output() {
		echo '<h2>' . __( 'Webhooks', 'woocommerce' ) . ' <a href="' . esc_url( mn_nonce_url( admin_url( 'admin.php?page=wc-settings&tab=api&section=webhooks&create-webhook=1' ), 'create-webhook' ) ) . '" class="add-new-h2">' . __( 'Add Webhook', 'woocommerce' ) . '</a></h2>';
