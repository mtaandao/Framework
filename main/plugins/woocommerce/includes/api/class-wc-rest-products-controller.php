...
class WC_REST_Products_Controller extends WC_REST_Posts_Controller {
	...
	protected function get_taxonomy_terms( $product, $taxonomy = 'cat' ) {
		...
		foreach ( mn_get_post_terms( $product->id, 'product_' . $taxonomy ) as $term ) {
			$terms[] = array(
				'id'   => $term->term_id,
				'name' => $term->name,
				'slug' => $term->slug,
	 ...
	 *
	 * @param MN_REST_Request $request Request object.
	 * @return MN_Error|stdClass $data Post object.
	 */
	protected function prepare_item_for_database( $request ) {
		...
		if ( isset( $request['name'] ) ) {
			$data->post_title = mn_filter_post_kses( $request['name'] );
		}

		// Post content.
		if ( isset( $request['description'] ) ) {
			$data->post_content = mn_filter_post_kses( $request['description'] );
		}

		// Post excerpt.
		if ( isset( $request['short_description'] ) ) {
			$data->post_excerpt = mn_filter_post_kses( $request['short_description'] );
		}
	...
	protected function save_product_images( $product_id, $images ) {
		if ( is_array( $images ) ) {
			...
			foreach ( $images as $image ) {
				...
				if ( ! empty( $image['name'] ) ) {
					mn_update_post( array( 'ID' => $attachment_id, 'post_title' => $image['name'] ) );
				}
			}
	...
	private function save_product_shipping_data( $product_id, $data ) {
		...
		if ( isset( $data['shipping_class'] ) ) {
			mn_set_object_terms( $product_id, wc_clean( $data['shipping_class'] ), 'product_shipping_class' );
		}
	}
	...
	protected function save_taxonomy_terms( $product_id, $terms, $taxonomy = 'cat' ) {
		$term_ids = mn_list_pluck( $terms, 'id' );
		$term_ids = array_unique( array_map( 'intval', $term_ids ) );

		mn_set_object_terms( $product_id, $term_ids, 'product_' . $taxonomy );
	...
	protected function save_product_meta( $product, $request ) {
		...
		if ( isset( $request['type'] ) ) {
			...
			mn_set_object_terms( $product->id, $product_type, 'product_type' );
		} else {
			...
			foreach ( $request['attributes'] as $attribute ) {
				...
				if ( $attribute_id ) {
					...
					if ( taxonomy_exists( $attribute_name ) ) {
						mn_set_object_terms( $product->id, $values, $attribute_name );
					}
		...
		if ( isset( $request['parent_id'] ) ) {
			$parent_id = mn_update_post( array( 'ID' => $product->id, 'post_parent' => absint( $request['parent_id'] ) ) );
		}
