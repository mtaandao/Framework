...
class Vc_WXR_Parser_SimpleXML {
	function parse( $file ) {
		...
		if ( ! $success || isset( $dom->doctype ) ) {
			return new MN_Error( 'SimpleXML_parse_error', __( 'There was an error when reading this WXR file', 'wordpress-importer' ), libxml_get_errors() );
		}
		...
		if ( ! $wxr_version ) {
			return new MN_Error( 'WXR_parse_error', __( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'wordpress-importer' ) );
		}
		...
		if ( ! isset( $namespaces['mn'] ) ) {
			$namespaces['mn'] = 'http://wordpress.org/export/1.1/';
		}
...
class Vc_WXR_Parser_XML {
	var $mn_tags = array(
		...
		'mn:is_sticky',
		'mn:term_id',
		'mn:category_nicename',
		'mn:category_parent',
		'mn:cat_name',
		'mn:category_description',
		'mn:tag_slug',
		'mn:tag_name',
		'mn:tag_description',
		'mn:term_taxonomy',
		...
		'mn:author_login',
		'mn:author_email',
		'mn:author_display_name',
		'mn:author_first_name',
		'mn:author_last_name',
	);
	var $mn_sub_tags = array(
		'mn:comment_id',
		'mn:comment_author',
		'mn:comment_author_email',
		'mn:comment_author_url',
		'mn:comment_author_IP',
		'mn:comment_date',
		'mn:comment_date_gmt',
		'mn:comment_content',
		'mn:comment_approved',
	...
	function parse( $file ) {
		...
		if ( ! xml_parse( $xml, file_get_contents( $file ), true ) ) {
			...
			return new MN_Error( 'XML_parse_error', 'There was an error when reading this WXR file', array(
				$current_line,
				$current_column,
				$error_string,
			) );
