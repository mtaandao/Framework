...
function vc_grid_item_editor_init() {
	...
	add_action( 'mn_ajax_vc_grid_item_editor_load_template_preview', array(
		&$vc_grid_item_editor,
...
function vc_grid_item_get_post_type() {
	...
	if ( vc_request_param( 'post_type' ) ) {
		...
		$post_type = $post instanceof MN_Post && $post->post_type ? $post->post_type : null;
	}
...
function vc_grid_item_editor_shortcodes() {
	...
	if ( ( 'true' === vc_request_param( 'vc_grid_item_editor' ) || ( is_admin() && vc_grid_item_get_post_type() === Vc_Grid_Item_Editor::postType() ) && vc_user_access()
			->mnAny( 'edit_posts', 'edit_pages' )->part( 'grid_builder' )->can()->get() )
	) {
 ...
 */
add_action( 'mn_ajax_vc_gitem_preview', 'vc_grid_item_render_preview', 5 );

/**
 * Add MN ui pointers in grid element editor.
 */
if ( is_admin() ) {
...
function vc_gitem_set_mapper_check_access() {
	if ( vc_user_access()->checkAdminNonce()->mnAny( 'edit_posts', 'edit_pages' )->part( 'grid_builder' )->can()->get() && 'true' === vc_post_param( 'vc_grid_item_editor' ) ) {
		vc_mapper()->setCheckForAccess( false );
	}
}

add_action( 'mn_ajax_vc_edit_form', 'vc_gitem_set_mapper_check_access' );
