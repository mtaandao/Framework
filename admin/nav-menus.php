<?php
/**
 * Mtaandao Administration for Navigation Menus
 * Interface functions
 *
 * @version 2.0.0
 *
...
if ( ! current_theme_supports( 'menus' ) && ! current_theme_supports( 'widgets' ) )
	mn_die( __( 'Your theme does not support navigation menus or widgets.' ) );
...
switch ( $action ) {
	case 'add-menu-item':
		...
		elseif ( isset( $_REQUEST['menu-item'] ) )
			mn_save_nav_menu_items( $nav_menu_selected_id, $_REQUEST['menu-item'] );
		break;
	case 'move-down-menu-item' :
		...
		if ( is_nav_menu_item( $menu_item_id ) ) {
			$menus = isset( $_REQUEST['menu'] ) ? array( (int) $_REQUEST['menu'] ) : mn_get_object_terms( $menu_item_id, 'nav_menu', array( 'fields' => 'ids' ) );
			if ( ! is_mn_error( $menus ) && ! empty( $menus[0] ) ) {
				$menu_id = (int) $menus[0];
				$ordered_menu_items = mn_get_nav_menu_items( $menu_id );
				$menu_item_data = (array) mn_setup_nav_menu_item( get_post( $menu_item_id ) );
				...
				if (
					...
					$next_item_data = (array) mn_setup_nav_menu_item( get_post( $next_item_id ) );
					...
					if (
						...
						$parent_object = mn_setup_nav_menu_item( get_post( $parent_db_id ) );

						if ( ! is_mn_error( $parent_object ) ) {
					...
					// Make menu item a child of its next sibling.
						...
						mn_update_post($menu_item_data);
						mn_update_post($next_item_data);
					}
	...
	case 'delete-menu-item':
		...
		if ( is_nav_menu_item( $menu_item_id ) && mn_delete_post( $menu_item_id, true ) )
	...
	case 'delete':
		...
		if ( is_nav_menu( $nav_menu_selected_id ) ) {
			$deletion = mn_delete_nav_menu( $nav_menu_selected_id );
		} else {
	...
	case 'delete_menus':
		...
		foreach ( $_REQUEST['delete_menus'] as $menu_id_to_delete ) {
			...
			$deletion = mn_delete_nav_menu( $menu_id_to_delete );
			if ( is_mn_error( $deletion ) ) {
				$messages[] = '<div id="message" class="error notice is-dismissible"><p>' . $deletion->get_error_message() . '</p></div>';
				$deletion_error = true;
			}
	...
	case 'update':
		...
		if ( 0 == $nav_menu_selected_id ) {
			...
			if ( $new_menu_title ) {
				$_nav_menu_selected_id = mn_update_nav_menu_object( 0, array('menu-name' => $new_menu_title) );

				if ( is_mn_error( $_nav_menu_selected_id ) ) {
					$messages[] = '<div id="message" class="error notice is-dismissible"><p>' . $_nav_menu_selected_id->get_error_message() . '</p></div>';
				} else {
					$_menu_object = mn_get_nav_menu_object( $_nav_menu_selected_id );
					$nav_menu_selected_id = $_nav_menu_selected_id;
					$nav_menu_selected_title = $_menu_object->name;
					if ( isset( $_REQUEST['menu-item'] ) )
						mn_save_nav_menu_items( $nav_menu_selected_id, absint( $_REQUEST['menu-item'] ) );
					if ( isset( $_REQUEST['zero-menu-state'] ) ) {
						// If there are menu items, add them
						mn_nav_menu_update_menu_items( $nav_menu_selected_id, $nav_menu_selected_title );
					...
					mn_redirect( admin_url( 'nav-menus.php?menu=' . $_nav_menu_selected_id ) );
					exit();
				}
			} else {
		...
		// Update existing menu.
			...
			$_menu_object = mn_get_nav_menu_object( $nav_menu_selected_id );
			...
			if ( ! is_mn_error( $_menu_object ) ) {
				$_nav_menu_selected_id = mn_update_nav_menu_object( $nav_menu_selected_id, array( 'menu-name' => $menu_title ) );
				if ( is_mn_error( $_nav_menu_selected_id ) ) {
			...
			// Update menu items.
			if ( ! is_mn_error( $_menu_object ) ) {
				$messages = array_merge( $messages, mn_nav_menu_update_menu_items( $_nav_menu_selected_id, $nav_menu_selected_title ) );

				// If the menu ID changed, redirect to the new URL.
				if ( $nav_menu_selected_id != $_nav_menu_selected_id ) {
					mn_redirect( admin_url( 'nav-menus.php?menu=' . intval( $_nav_menu_selected_id ) ) );
					exit();
				}
			}
		}
	...
	case 'locations':
		if ( ! $num_locations ) {
			mn_redirect( admin_url( 'nav-menus.php' ) );
			exit();
		}
 ...
 */
$page_count = mn_count_posts( 'page' );
...
if ( 0 == $menu_count && ! $add_new_screen && ! $one_theme_location_no_menus )
	mn_redirect( admin_url( 'nav-menus.php?action=edit&menu=0' ) );
...
if ( ! $nav_menu_selected_title && is_nav_menu( $nav_menu_selected_id ) ) {
	$_menu_object = mn_get_nav_menu_object( $nav_menu_selected_id );
	$nav_menu_selected_title = ! is_mn_error( $_menu_object ) ? $_menu_object->name : '';
}

// Generate truncated menu names.
foreach ( (array) $nav_menus as $key => $_nav_menu ) {
	$nav_menus[$key]->truncated_name = mn_html_excerpt( $_nav_menu->name, 40, '&hellip;' );
}
 ...
 *
 * @global int $_mn_nav_menu_max_depth
 */
global $_mn_nav_menu_max_depth;
$_mn_nav_menu_max_depth = 0;

// Calling mn_get_nav_menu_to_edit generates $_mn_nav_menu_max_depth.
if ( is_nav_menu( $nav_menu_selected_id ) ) {
	$menu_items = mn_get_nav_menu_items( $nav_menu_selected_id, array( 'post_status' => 'any' ) );
	$edit_markup = mn_get_nav_menu_to_edit( $nav_menu_selected_id );
}
 ...
 * @return string
 */
function mn_nav_menu_max_depth( $classes ) {
	global $_mn_nav_menu_max_depth;
	return "$classes menu-max-depth-$_mn_nav_menu_max_depth";
}

add_filter('admin_body_class', 'mn_nav_menu_max_depth');

mn_nav_menu_setup();
mn_initial_nav_menu_meta_boxes();
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __('<a href="https://mtaandao.co.ke/docs/Appearance_Menus_Screen" target="_blank">Documentation on Menus</a>') . '</p>' .
...
<div class="wrap">
		...
		if ( current_user_can( 'customize' ) ) :
			...
			printf(
				...
				esc_url( add_query_arg( array(
					...
					'return' => urlencode( mn_unslash( $_SERVER['REQUEST_URI'] ) ),
	...
	<div id="menu-locations-wrap">
		<form method="post" action="<?php echo esc_url( add_query_arg( array( 'action' => 'locations' ), admin_url( 'nav-menus.php' ) ) ); ?>">
							...
							<select name="menu-locations[<?php echo $_location; ?>]" id="locations-<?php echo $_location; ?>">
								...
								<?php foreach ( $nav_menus as $menu ) : ?>
									...
									<option <?php if ( $selected ) echo 'data-orig="true"'; ?> <?php selected( $selected ); ?> value="<?php echo $menu->term_id; ?>">
										<?php echo mn_html_excerpt( $menu->name, 40, '&hellip;' ); ?>
									</option>
								<?php endforeach; ?>
							</select>
			...
			<p class="button-controls mn-clearfix"><?php submit_button( __( 'Save Changes' ), 'primary left', 'nav-menu-locations', false ); ?></p>
			<?php mn_nonce_field( 'save-menu-locations' ); ?>
				...
				<?php foreach ( (array) $nav_menus as $_nav_menu ) : ?>
					<option value="<?php echo esc_attr( $_nav_menu->term_id ); ?>" <?php selected( $_nav_menu->term_id, $nav_menu_selected_id ); ?>>
						...
						if ( ! empty( $menu_locations ) && in_array( $_nav_menu->term_id, $menu_locations ) ) {
							 ...
							 */
							$assigned_locations = array_slice( $locations_assigned_to_this_menu, 0, absint( apply_filters( 'mn_nav_locations_listed_per_menu', 3 ) ) );
	...
	<div id="nav-menus-frame" class="mn-clearfix">
	<div id="menu-settings-column" class="metabox-holder<?php if ( isset( $_GET['menu'] ) && '0' == $_GET['menu'] ) { echo ' metabox-holder-disabled'; } ?>">
		...
		<form id="nav-menu-meta" class="nav-menu-meta" method="post" enctype="multipart/form-data">
			...
			<?php mn_nonce_field( 'add-menu_item', 'menu-settings-column-nonce' ); ?>
		...
		<div id="menu-management">
			<form id="update-nav-menu" method="post" enctype="multipart/form-data">
				<div class="menu-edit <?php if ( $add_new_screen ) echo 'blank-slate'; ?>">
					...
					mn_nonce_field( 'update-nav_menu', 'update-nav-menu-nonce' );
					...
					<div id="nav-menu-header">
						<div class="major-publishing-actions mn-clearfix">
					...
					<div id="post-body">
						<div id="post-body-content" class="mn-clearfix">
							<?php if ( ! $add_new_screen ) : ?>
							...
							<?php
							if ( isset( $edit_markup ) && ! is_mn_error( $edit_markup ) ) {
								echo $edit_markup;
							} else {
							?>
											...
											<?php if ( ! empty( $menu_locations[ $location ] ) && $menu_locations[ $location ] != $nav_menu_selected_id ) : ?>
												<span class="theme-location-set"><?php
													...
													printf( _x( '(Currently set to: %s)', 'menu location' ),
														mn_get_nav_menu_object( $menu_locations[ $location ] )->name
													);
												?></span>
											<?php endif; ?>
										</dd>
					...
					<div id="nav-menu-footer">
						<div class="major-publishing-actions mn-clearfix">
							...
							<span class="delete-action">
								<a class="submitdelete deletion menu-delete" href="<?php echo esc_url( mn_nonce_url( add_query_arg( array( 'action' => 'delete', 'menu' => $nav_menu_selected_id ), admin_url( 'nav-menus.php' ) ), 'delete-nav_menu-' . $nav_menu_selected_id) ); ?>"><?php _e('Delete Menu'); ?></a>
