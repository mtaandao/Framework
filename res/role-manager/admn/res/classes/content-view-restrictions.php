...
class RM_Content_View_Restrictions {
    
    const view_posts_access_cap = 'rm_view_posts_access';
    const content_for_roles = 'rm_content_for_roles';
    const prohibit_allow_flag = 'rm_prohibit_allow_flag';
    const post_access_error_action = 'rm_post_access_error_action';
    const post_access_error_message = 'rm_post_access_error_message';
    ...
    public function __construct() {        
        ...
        $this->posts_view_access = new RM_Posts_View_Access();
        ...
        new RM_Content_View_Restrictions_Posts_List();
        ...
        add_filter('rm_post_access_error_message', 'mntexturize');
        add_filter('rm_post_access_error_message', 'convert_smilies');
        add_filter('rm_post_access_error_message', 'convert_chars');
        add_filter('rm_post_access_error_message', 'mnautop');
        add_filter('rm_post_access_error_message', 'shortcode_unautop');
        add_filter('rm_post_access_error_message', 'do_shortcode');
    ...
    // block access to RM's post meta (custom) fields, if user does not have enough permissions
    ...
    public function add_post_meta_box() {
        ...
        foreach ($post_types as $post_type) {
            add_meta_box(
                    'rm_content_view_restrictions_meta_box', 
                    esc_html__('Content View Restrictions', 'role-manager'), 
    ...
    function render_post_meta_box($post) {
        ...
        $roles_list = '<input type="checkbox" id="rm_roles_auto_select" name="rm_roles_selector" value="1"><hr/>';
        foreach($mn_roles->roles as $role_id=>$role_data) {
            ...
            $roles_list .= '<input type="checkbox" id="'. $role_id .'" name="'. $role_id .'" class="rm_role_cb" value="1" '. $role_selected .'>&nbsp'.
                           '<label for="'. $role_id .'">' .$role_data['name'] .' ('. $role_id .')</label><br>'."\n";
        }
        $roles_list .= '<input type="checkbox" id="no_role" name="no_role" class="rm_role_cb" value="1" '. (in_array('no_role', $selected_roles) ? 'checked' : '') . '>&nbsp'.
        ...
        if (empty($post_access_error_action)) {
            ...
            $post_access_error_action = apply_filters('rm_default_post_access_error_action', 0);
        }
        ...
        mn_nonce_field('rm_content_view_restrictions_meta_box', 'rm_content_view_restrictions_meta_box_nonce');        
    ?>
<strong><?php esc_html_e('Type:','role-manager');?></strong>&nbsp;&nbsp;
<input type="radio" id="rm_prohibit_flag" name="rm_prohibit_allow_flag" value="1"  <?php echo $selected1;?> > <label for="rm_prohibit_flag"><?php echo esc_html_e('Prohibit Access', 'role-manager');?></label>&nbsp;
<input type="radio" id="rm_allow_flag" name="rm_prohibit_allow_flag" value="2"  <?php echo $selected2;?> > <label for="rm_allow_flag"><?php echo esc_html_e('Allow Access', 'role-manager');?></label><br>
<strong><?php esc_html_e('for Roles:','role-manager');?></strong>&nbsp;&nbsp;<button id="edit_content_for_roles"><?php echo esc_html_e('Edit Roles List', 'role-manager');?></button><br>
<textarea id="rm_content_for_roles" name="rm_content_for_roles" rows="3" style="width: 100%;" readonly="readonly"><?php echo $content_for_roles;?></textarea>
<strong><?php esc_html_e('Action:','role-manager');?>&nbsp;&nbsp;
<input type="radio" id="rm_return_http_error_404" name="rm_post_access_error_action" value="1"  <?php echo $selected3;?> > <label for="rm_return_http_error_404"><?php esc_html_e('Return HTTP 404 error', 'role-manager');?></label>&nbsp;
<input type="radio" id="rm_show_post_access_error_message" name="rm_post_access_error_action" value="2"  <?php echo $selected4;?> > <label for="rm_show_post_access_error_message"><?php esc_html_e('Show access error message', 'role-manager');?></label><br>
<div id="rm_post_access_error_message_container" style="display: none;">
    <?php esc_html_e('Error message:', 'role-manager');?><br/>
    <textarea id="rm_post_access_error_message" name="rm_post_access_error_message" rows="5" style="width: 100%;"><?php echo $post_access_error_message;?></textarea> 
</div>    
<div style="text-align: right; color: #cccccc; font-size: 0.8em;"><?php esc_html_e('Mtaandao Role Manager', 'role-manager');?></div>
...
<div id="edit_roles_list_dialog" style="display: none;">
   ...
   /**
    ...
    public function admin_load_js($hook_suffix) {
        ...
        mn_register_script('rm-admn-content-view-restrictions', admin_url('/js/admn/rm-admn-content-view-restrictions.js', RM_PLUGIN_FULL_PATH));
        mn_enqueue_script('rm-admn-content-view-restrictions');
        mn_localize_script('rm-admn-content-view-restrictions', 'rm_data_pro', array(
            'mn_nonce' => mn_create_nonce('role-manager'),
            'edit_content_for_roles' => esc_html__('Edit Roles List', 'role-manager'),
            'edit_content_for_roles_title' => esc_html__('Roles List restrict/allow content view', 'role-manager'),
            'save_roles_list' => esc_html__('Save', 'role-manager'),
            'close' => esc_html__('Close', 'role-manager')
        ));
    ...
    protected function check_security($post_id) {
        ...
        $nonce = filter_input(INPUT_POST, 'rm_content_view_restrictions_meta_box_nonce', FILTER_SANITIZE_STRING);                
        if (empty($nonce) || !mn_verify_nonce($nonce, 'rm_content_view_restrictions_meta_box')) {
            return false;
        }
    ...
    public function save_meta_data($post_id) {
        ...
        $rm_prohibit_allow_flag = $this->lib->get_request_var('rm_prohibit_allow_flag', 'post', 'int');
        // Update the meta field.
        update_post_meta($post_id, self::prohibit_allow_flag, $rm_prohibit_allow_flag);

        $rm_content_for_roles0 = $this->lib->get_request_var('rm_content_for_roles', 'post');
        // Update the meta field.
        $roles_to_check = explode(',', $rm_content_for_roles0);
        $roles_to_save = array();
        ...
        $rm_content_for_roles1 = implode(', ', $roles_to_save);
        update_post_meta($post_id, self::content_for_roles, $rm_content_for_roles1);
        
        $rm_post_access_error_action = $this->lib->get_request_var('rm_post_access_error_action', 'post', 'int');
        update_post_meta($post_id, self::post_access_error_action, $rm_post_access_error_action);
        
        $rm_post_access_error_message = $_POST['rm_post_access_error_message'];
        update_post_meta($post_id, self::post_access_error_message, $rm_post_access_error_message);
    }
    ...
    private function get_post_access_error_message($post_id) {        
        ...
        $post_access_error_message = apply_filters('rm_post_access_error_message', $post_access_error_message);
    ...
    private function check_post_level_permissions($content) {
        ...
        $rm_prohibit_allow_flag = get_post_meta($post->ID, self::prohibit_allow_flag, true);
        $rm_content_for_roles = get_post_meta($post->ID, self::content_for_roles, true);
        if (empty($rm_content_for_roles)) {
            return $data;
        }
        $roles = explode(', ', $rm_content_for_roles);
        if (count($roles)==0) {
            return $data;
        }
    ...
    private function check_roles_level_permissions($content) {
        ...
        $blocked = RM_Content_View_Restrictions_Controller::load_access_data_for_user($current_user);
        if (empty($blocked)) {
            return $content;
        }
    ...
    private function get_post_from_last_query() {
        ...
        $post_id = 0;
        foreach($keys as $key) {
            $post_id = RM_Utils::get_int_after_key($key, $mndb->last_query);
            if ($post_id>0) {
                break;
            }            
        }
    ...
    public static function get_post_view_access_users($post_id) {
        ...
        $rm_content_for_roles = get_post_meta($post_id, RM_Content_View_Restrictions::content_for_roles, true);
        if (empty($rm_content_for_roles)) {
            return false;
        }
        $restricted_roles = explode(', ', $rm_content_for_roles);
        if (count($restricted_roles)==0) {
            return false;
        }
        
        $rm_prohibit_allow_flag = get_post_meta($post_id, self::prohibit_allow_flag, true);
        $restriction = ($rm_prohibit_allow_flag==1) ? 'prohibited' : 'allowed';
...
// end of RM_Content_View_Restrictions class
