...
class WC_Cart {
	...
	public function __construct() {
		add_action( 'mn_loaded', array( $this, 'init' ) ); // Get cart after MN and plugins are loaded.
		add_action( 'mn', array( $this, 'maybe_set_cart_cookies' ), 99 ); // Set cookies
	...
	/**
	 * Loads the cart data from the PHP session during Mtaandao init and hooks in other methods.
	 */
	...
	/**
	 * Will set cart cookies if needed, once, during MN hook.
	 */
	public function maybe_set_cart_cookies() {
		if ( ! headers_sent() && did_action( 'mn_loaded' ) ) {
			if ( ! $this->is_empty() ) {
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function get_cart_from_session() {
			...
			if ( is_array( $cart ) ) {
				...
				update_meta_cache( 'post', mn_list_pluck( $cart, 'product_id' ) );
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function get_cart_contents_count() {
			return apply_filters( 'woocommerce_cart_contents_count', array_sum( mn_list_pluck( $this->get_cart(), 'quantity' ) ) );
		}
		...
		*/
		...
		public function check_cart_item_validity() {
			...
			foreach ( $this->get_cart() as $cart_item_key => $values ) {
				...
				if ( ! $_product || ! $_product->exists() || 'trash' === $_product->post->post_status ) {
					...
					$return = new MN_Error( 'invalid', __( 'An item which is no longer available was removed from your cart.', 'woocommerce' ) );
				}
			}
		...
		public function check_cart_item_stock() {
			...
			foreach ( $this->get_cart() as $cart_item_key => $values ) {
				...
				if ( get_option( 'woocommerce_hold_stock_minutes' ) > 0 && ! $_product->backorders_allowed() ) {
					...
					$held_stock = $mndb->get_var(
						$mndb->prepare( "
							SELECT SUM( order_item_meta.meta_value ) AS held_qty
							FROM {$mndb->posts} AS posts
							LEFT JOIN {$mndb->prefix}woocommerce_order_items as order_items ON posts.ID = order_items.order_id
							LEFT JOIN {$mndb->prefix}woocommerce_order_itemmeta as order_item_meta ON order_items.order_item_id = order_item_meta.order_item_id
							LEFT JOIN {$mndb->prefix}woocommerce_order_itemmeta as order_item_meta2 ON order_items.order_item_id = order_item_meta2.order_item_id
			...
			if ( ! empty( $cart_item['data']->variation_id ) && is_array( $cart_item['variation'] ) ) {
				...
				foreach ( $cart_item['variation'] as $name => $value ) {
					...
					if ( taxonomy_exists( $taxonomy ) ) {
						...
						if ( ! is_mn_error( $term ) && $term && $term->name ) {
							$value = $term->name;
						}
			...
			if ( sizeof( $item_data ) > 0 ) {
				...
				if ( $flat ) {
					foreach ( $item_data as $data ) {
						echo esc_html( $data['key'] ) . ': ' . mn_kses_post( $data['display'] ) . "\n";
					}
				} else {
		...
		public function get_remove_url( $cart_item_key ) {
			...
			return apply_filters( 'woocommerce_get_remove_url', $cart_page_url ? mn_nonce_url( add_query_arg( 'remove_item', $cart_item_key, $cart_page_url ), 'woocommerce-cart' ) : '' );
		}
		...
		public function get_undo_url( $cart_item_key ) {
			...
			return apply_filters( 'woocommerce_get_undo_url', $cart_page_url ? mn_nonce_url( add_query_arg( $query_args, $cart_page_url ), 'woocommerce-cart' ) : '', $cart_item_key );
		}
		...
		public function get_cart() {
			if ( ! did_action( 'mn_loaded' ) ) {
				_doing_it_wrong( __FUNCTION__, __( 'Get cart should not be called before the mn_loaded action.', 'woocommerce' ), '2.3' );
			}
		...
		public function add_to_cart( $product_id = 0, $quantity = 1, $variation_id = 0, $variation = array(), $cart_item_data = array() ) {
			...
			try {
				...
				if ( 'product_variation' == get_post_type( $product_id ) ) {
					...
					$product_id   = mn_get_post_parent_id( $variation_id );
				}
