...
class MN_Site_Query {
	 ...
	 *
	 * @param string|array $query Array or string of MN_Site_Query arguments. See MN_Site_Query::__construct().
	 */
	...
	public function get_sites() {
		...
		if ( false === $cache_value ) {
			...
			mn_cache_add( $cache_key, $cache_value, 'sites' );
		} else {
	...
	protected function get_site_ids() {
		...
		if ( ! empty( $site_id ) ) {
			$this->sql_clauses['where']['ID'] = $mndb->prepare( 'blog_id = %d', $site_id );
		}

		// Parse site IDs for an IN clause.
		if ( ! empty( $this->query_vars['site__in'] ) ) {
			$this->sql_clauses['where']['site__in'] = "blog_id IN ( " . implode( ',', mn_parse_id_list( $this->query_vars['site__in'] ) ) . ' )';
		}

		// Parse site IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['site__not_in'] ) ) {
			$this->sql_clauses['where']['site__not_in'] = "blog_id NOT IN ( " . implode( ',', mn_parse_id_list( $this->query_vars['site__not_in'] ) ) . ' )';
		}

		$network_id = absint( $this->query_vars['network_id'] );

		if ( ! empty( $network_id ) ) {
			$this->sql_clauses['where']['network_id'] = $mndb->prepare( 'site_id = %d', $network_id );
		}

		// Parse site network IDs for an IN clause.
		if ( ! empty( $this->query_vars['network__in'] ) ) {
			$this->sql_clauses['where']['network__in'] = 'site_id IN ( ' . implode( ',', mn_parse_id_list( $this->query_vars['network__in'] ) ) . ' )';
		}

		// Parse site network IDs for a NOT IN clause.
		if ( ! empty( $this->query_vars['network__not_in'] ) ) {
			$this->sql_clauses['where']['network__not_in'] = 'site_id NOT IN ( ' . implode( ',', mn_parse_id_list( $this->query_vars['network__not_in'] ) ) . ' )';
		}

		if ( ! empty( $this->query_vars['domain'] ) ) {
			$this->sql_clauses['where']['domain'] = $mndb->prepare( 'domain = %s', $this->query_vars['domain'] );
		}

		// Parse site domain for an IN clause.
		if ( is_array( $this->query_vars['domain__in'] ) ) {
			$this->sql_clauses['where']['domain__in'] = "domain IN ( '" . implode( "', '", $mndb->_escape( $this->query_vars['domain__in'] ) ) . "' )";
		}

		// Parse site domain for a NOT IN clause.
		if ( is_array( $this->query_vars['domain__not_in'] ) ) {
			$this->sql_clauses['where']['domain__not_in'] = "domain NOT IN ( '" . implode( "', '", $mndb->_escape( $this->query_vars['domain__not_in'] ) ) . "' )";
		}

		if ( ! empty( $this->query_vars['path'] ) ) {
			$this->sql_clauses['where']['path'] = $mndb->prepare( 'path = %s', $this->query_vars['path'] );
		}

		// Parse site path for an IN clause.
		if ( is_array( $this->query_vars['path__in'] ) ) {
			$this->sql_clauses['where']['path__in'] = "path IN ( '" . implode( "', '", $mndb->_escape( $this->query_vars['path__in'] ) ) . "' )";
		}

		// Parse site path for a NOT IN clause.
		if ( is_array( $this->query_vars['path__not_in'] ) ) {
			$this->sql_clauses['where']['path__not_in'] = "path NOT IN ( '" . implode( "', '", $mndb->_escape( $this->query_vars['path__not_in'] ) ) . "' )";
		}

		if ( is_numeric( $this->query_vars['archived'] ) ) {
			$archived = absint( $this->query_vars['archived'] );
			$this->sql_clauses['where']['archived'] = $mndb->prepare( "archived = %d ", $archived );
		}

		if ( is_numeric( $this->query_vars['mature'] ) ) {
			$mature = absint( $this->query_vars['mature'] );
			$this->sql_clauses['where']['mature'] = $mndb->prepare( "mature = %d ", $mature );
		}

		if ( is_numeric( $this->query_vars['spam'] ) ) {
			$spam = absint( $this->query_vars['spam'] );
			$this->sql_clauses['where']['spam'] = $mndb->prepare( "spam = %d ", $spam );
		}

		if ( is_numeric( $this->query_vars['deleted'] ) ) {
			$deleted = absint( $this->query_vars['deleted'] );
			$this->sql_clauses['where']['deleted'] = $mndb->prepare( "deleted = %d ", $deleted );
		}

		if ( is_numeric( $this->query_vars['public'] ) ) {
			$public = absint( $this->query_vars['public'] );
			$this->sql_clauses['where']['public'] = $mndb->prepare( "public = %d ", $public );
		}
		...
		if ( strlen( $this->query_vars['search'] ) ) {
			...
			/**
			 * Filters the columns to search in a MN_Site_Query search.
			 *
			 ...
			 * @param MN_Site_Query $this           The current MN_Site_Query instance.
			 */
		...
		if ( ! empty( $date_query ) && is_array( $date_query ) ) {
			$this->date_query = new MN_Date_Query( $date_query, 'registered' );
		...
		$this->sql_clauses['from']    = "FROM $mndb->blogs $join";
		...
		$site_ids = $mndb->get_col( $this->request );
	...
	private function set_found_sites() {
		...
		if ( $this->query_vars['number'] && ! $this->query_vars['no_found_rows'] ) {
			...
			$this->found_sites = (int) $mndb->get_var( $found_sites_query );
		}
	}
	...
	protected function get_search_sql( $string, $columns ) {
		...
		if ( false !== strpos( $string, '*' ) ) {
			$like = '%' . implode( '%', array_map( array( $mndb, 'esc_like' ), explode( '*', $string ) ) ) . '%';
		} else {
		...
		foreach ( $columns as $column ) {
			$searches[] = $mndb->prepare( "$column LIKE %s", $like );
		}
	...
	protected function parse_orderby( $orderby ) {
		...
		switch ( $orderby ) {
			case 'site__in':
				...
				$parsed = "FIELD( {$mndb->blogs}.blog_id, $site__in )";
				break;
			case 'network__in':
				$network__in = implode( ',', array_map( 'absint', $this->query_vars['network__in'] ) );
				$parsed = "FIELD( {$mndb->blogs}.site_id, $network__in )";
				break;
			case 'domain':
			case 'last_updated':
			case 'path':
