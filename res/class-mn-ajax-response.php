...
class MN_Ajax_Response {
	...
	/**
	 * Constructor - Passes args to MN_Ajax_Response::add().
	 *
	 * @since 2.1.0
	 * @see MN_Ajax_Response::add()
	 *
	 ...
	 *     @type int|MN_Error    $id           The response ID, used as the response type `id` attribute. Also
	 *                                         accepts a `MN_Error` object if the ID does not exist. Default 0.
	 ...
	 *     @type string|MN_Error $data         The response content/message. Also accepts a MN_Error object if the
	...
	public function add( $args = '' ) {
		...
		if ( is_mn_error( $data ) ) {
			foreach ( (array) $data->get_error_codes() as $code ) {
				$response .= "<mn_error code='$code'><![CDATA[" . $data->get_error_message( $code ) . "]]></mn_error>";
				...
				$response .= "<mn_error_data code='$code'$class>";
	...
	public function send() {
		...
		echo "<?xml version='1.0' encoding='" . get_option( 'blog_charset' ) . "' standalone='yes'?><mn_ajax>";
