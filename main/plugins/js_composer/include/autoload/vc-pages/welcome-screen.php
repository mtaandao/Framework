...
function vc_welcome_menu_hooks() {
	$settings_tab_enabled = vc_user_access()->mnAny( 'manage_options' )->part( 'settings' )->can( 'vc-general-tab' )->get();
...
function vc_page_welcome_redirect() {
	...
	$redirect && mn_redirect( admin_url( 'admin.php?page=' . rawurlencode( vc_page_welcome_slug() ) ) );
}
