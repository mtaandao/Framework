...
class WC_CLI_Product extends WC_CLI_Command {
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc product create --title="Product Name"
	 *
	 *     mn wc product create --title="Product Name" --
	 *
	 * @since 2.5.0
	...
	public function create( $args, $assoc_args ) {
		...
		try {
			...
			if ( $porcelain ) {
				...
				MN_CLI::success( "Created product $id." );
			}
		} catch ( WC_CLI_Exception $e ) {
			// Remove the product when fails
			$this->clear_product( $id );

			MN_CLI::error( $e->getMessage() );
		}
	}
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc product delete 123
	 *
	 *     mn wc product delete $(mn wc product list --format=ids)
	 *
	 * @since 2.5.0
	 */
	public function delete( $args, $assoc_args ) {
		...
		foreach ( $args as $id ) {
			...
			$r = mn_delete_post( $id, true );

			if ( $r ) {
				MN_CLI::success( "Deleted product $id." );
			} else {
				$exit_code += 1;
				MN_CLI::warning( "Failed deleting product $id." );
			}
		}
		exit( $exit_code ? 1 : 0 );
	}
	 ...
	 * ## AVAILABLE FIELDS
	 *
	 * For more fields, see: mn wc product list --help
	 *
	 * ## EXAMPLES
	 *
	 *     mn wc product get 123 --fields=id,title,sku
	 *
	 * @since 2.5.0
	 */
	 ...
	 *     mn wc product list
	 *
	 *     mn wc product list --field=id
	 *
	 *     mn wc product list --fields=id,title,type --format=json
	 *
	 * @subcommand list
	 * @since      2.5.0
	 */
	public function list_( $args, $assoc_args ) {
		$query_args = $this->merge_mn_query_args( $this->get_list_query_args( $assoc_args ), $assoc_args );
		$formatter  = $this->get_formatter( $assoc_args );

		if ( 'ids' === $formatter->format ) {
			$query_args['fields'] = 'ids';
			$query = new MN_Query( $query_args );
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc product reviews 123
	 *
	 *     mn wc product reviews 123 --fields=id,rating,reviewer_email
	 *
	 * @since 2.5.0
	 */
	...
	public function types( $__, $___ ) {
		...
		foreach ( $product_types as $type => $label ) {
			MN_CLI::line( sprintf( '%s: %s', $label, $type ) );
		}
	}
	 ...
	 * ## AVAILABLE_FIELDS
	 *
	 * For more fields, see: mn wc product create --help
	 *
	 * ## EXAMPLES
	 *
	 *     mn wc product update 123 --title="New Product Title" --description="New description"
	 *
	 * @since 2.5.0
	 */
	public function update( $args, $assoc_args ) {
		try {
			...
			MN_CLI::success( "Updated product $id." );
		} catch ( WC_CLI_Exception $e ) {
			MN_CLI::error( $e->getMessage() );
		}
	}
	...
	/**
	 * Format posts from MN_Query result to items in which each item contain
	...
	private function get_product_data( $product ) {
		...
		$product_data = array(
			...
			'categories'         => implode( ', ', mn_get_post_terms( $product->id, 'product_cat', array( 'fields' => 'names' ) ) ),
			'tags'               => implode( ', ', mn_get_post_terms( $product->id, 'product_tag', array( 'fields' => 'names' ) ) ),
			'images'             => $this->get_images( $product ),
			'featured_src'       => mn_get_attachment_url( get_post_thumbnail_id( $product->is_type( 'variation' ) ? $product->variation_id : $product->id ) ),
	...
	private function upload_product_image( $image_url ) {
		...
		$mn_filetype = mn_check_filetype( $file_name, $this->allowed_image_mime_types() );
