jQuery( document ).ready( function( $ ) {
	...
	$( 'body' ).addClass( 'custom-branding-admin' );
	...
	// Admin Branding
	$( '#toplevel_page_custom_branding_admin_logo, #toplevel_page_custom_branding_admin_logo_folded' ).on( 'click', 'a', function( e ) {
		e.preventDefault();
	} );
	...
	$( '.pageSection' ).on( 'click', '.imageSelect', function( e ) {
		...
		file_frame = mn.media.frames.file_frame = mn.media( {
	...
	// Colorpicker
	if ( $( 'body' ).is( '.toplevel_page_custom_branding_color_schemes' ) ) {
	...
	if ( $( '.colorCustom input[name="custom_branding_settings[colorScheme]"]' ).is( ':checked' ) ) {
	...
	$( 'body.plugins-php #custom-branding-admin .deactivate' ).on( 'click', 'a', function( e ) {

		var response = confirm('Are you sure you want to disable Custom Branding? Once disabled, all your settings will be lost. \n\nIf you want to save your settings, make sure to export them on the Custom Branding Import/Export page first. \n\nClick "OK" to DISABLE Custom Branding and lose your settings.');
		if (response === true) {
			//$(this ).click();
		} else {
			e.preventDefault();
