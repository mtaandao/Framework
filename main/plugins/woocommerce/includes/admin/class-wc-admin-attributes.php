...
class WC_Admin_Attributes {
	 ...
	 * @return bool|MN_error result
	 */
	private static function valid_attribute_name( $attribute_name ) {
		if ( strlen( $attribute_name ) >= 28 ) {
			return new MN_Error( 'error', sprintf( __( 'Slug "%s" is too long (28 characters max). Shorten it, please.', 'woocommerce' ), sanitize_title( $attribute_name ) ) );
		} elseif ( wc_check_if_attribute_name_is_reserved( $attribute_name ) ) {
			return new MN_Error( 'error', sprintf( __( 'Slug "%s" is not allowed because it is a reserved term. Change it, please.', 'woocommerce' ), sanitize_title( $attribute_name ) ) );
		}
	...
	private static function process_add_attribute() {
		...
		if ( empty( $attribute['attribute_name'] ) || empty( $attribute['attribute_label'] ) ) {
			return new MN_Error( 'error', __( 'Please, provide an attribute name and slug.', 'woocommerce' ) );
		} elseif ( ( $valid_attribute_name = self::valid_attribute_name( $attribute['attribute_name'] ) ) && is_mn_error( $valid_attribute_name ) ) {
			return $valid_attribute_name;
		} elseif ( taxonomy_exists( wc_attribute_taxonomy_name( $attribute['attribute_name'] ) ) ) {
			return new MN_Error( 'error', sprintf( __( 'Slug "%s" is already in use. Change it, please.', 'woocommerce' ), sanitize_title( $attribute['attribute_name'] ) ) );
		}

		$mndb->insert( $mndb->prefix . 'woocommerce_attribute_taxonomies', $attribute );

		do_action( 'woocommerce_attribute_added', $mndb->insert_id, $attribute );
	...
	private static function process_edit_attribute() {
		...
		$old_attribute_name = $mndb->get_var( "SELECT attribute_name FROM {$mndb->prefix}woocommerce_attribute_taxonomies WHERE attribute_id = $attribute_id" );
		...
		$mndb->update( $mndb->prefix . 'woocommerce_attribute_taxonomies', $attribute, array( 'attribute_id' => $attribute_id ) );
		...
		if ( $old_attribute_name != $attribute['attribute_name'] && ! empty( $old_attribute_name ) ) {
			// Update taxonomies in the mn term taxonomy table
			$mndb->update(
				$mndb->term_taxonomy,
			...
			$mndb->query( $mndb->prepare( "UPDATE {$mndb->postmeta} SET meta_value = REPLACE( meta_value, %s, %s ) WHERE meta_key = '_product_attributes'",
	...
	private static function process_delete_attribute() {
		...
		$attribute_name = $mndb->get_var( "SELECT attribute_name FROM {$mndb->prefix}woocommerce_attribute_taxonomies WHERE attribute_id = $attribute_id" );
		$taxonomy       = wc_attribute_taxonomy_name( $attribute_name );

		do_action( 'woocommerce_before_attribute_delete', $attribute_id, $attribute_name, $taxonomy );

		if ( $attribute_name && $mndb->query( "DELETE FROM {$mndb->prefix}woocommerce_attribute_taxonomies WHERE attribute_id = $attribute_id" ) ) {
	...
	public static function edit_attribute() {
		...
		$attribute_to_edit = $mndb->get_row( "SELECT * FROM " . $mndb->prefix . "woocommerce_attribute_taxonomies WHERE attribute_id = '$edit'" );
		...
		<div class="wrap woocommerce">
			...
			<?php
				...
				<form action="edit.php?post_type=product&amp;page=product_attributes&amp;edit=<?php echo absint( $edit ); ?>" method="post">
					...
					<?php mn_nonce_field( 'woocommerce-save-attribute_' . $edit ); ?>
				</form>
			<?php } ?>
		</div>
		<?php
	...
	public static function add_attribute() {
		...
		<div class="wrap woocommerce">
			...
			<div id="col-container">
				<div id="col-right">
					<div class="col-wrap">
						<table class="widefat attributes-table mn-list-table ui-sortable" style="width:100%">
							<thead>
								<tr>
											...
											?><tr>
												...
												<td class="attribute-terms"><?php
													...
													if ( taxonomy_exists( $taxonomy ) ) {
														...
														$terms_string = implode( ', ', mn_list_pluck( $terms, 'name' ) );
														if ( $terms_string ) {
															echo $terms_string;
														} else {
					...
					<div class="col-wrap">
						<div class="form-wrap">
							...
							<form action="edit.php?post_type=product&amp;page=product_attributes" method="post">
								...
								<?php mn_nonce_field( 'woocommerce-add-new_attribute' ); ?>
							</form>
						</div>
					</div>
				</div>
