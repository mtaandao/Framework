...
(function($) {
	...
	function bindToggleButton() {
		$toggleButton = $pass1Row.find('.mn-hide-pw');
	...
	function bindPasswordForm() {
		...
		$passwordWrapper = $pass1Row.find( '.mn-pwd' );
		$generateButton  = $pass1Row.find( 'button.mn-generate-pw' );
		...
		$cancelButton = $pass1Row.find( 'button.mn-cancel-pw' );
		$cancelButton.on( 'click', function () {
			...
			mn.ajax.post( 'generate-password' )
				.done( function( data ) {
					$pass1.data( 'pw', data );
				} );
	...
	function check_pass_strength() {
		...
		strength = mn.passwordStrength.meter( pass1, mn.passwordStrength.userInputBlacklist(), pass1 );
	...
	$(document).ready( function() {
		...
		$colorpicker.on( 'click.colorpicker', '.color-option', function() {
			...
			if ( user_id === current_user_id ) {
				...
				// repaint icons
				if ( typeof mn !== 'undefined' && mn.svgPainter ) {
					try {
						colors = $.parseJSON( $this.children( '.icon_colors' ).val() );
					} catch ( error ) {}

					if ( colors ) {
						mn.svgPainter.setColors( colors );
						mn.svgPainter.paint();
					}
				}
