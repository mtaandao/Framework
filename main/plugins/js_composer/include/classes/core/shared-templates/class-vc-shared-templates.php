...
class Vc_Shared_Templates {
	 ...
	 * @var string
	 */
	protected $download_link_url = 'http://support.mtaandao.co.ke/plugins/page-builder/templates/download-link';
	...
	public function init() {
		...
		*/
		...
		add_filter( 'mn_ajax_vc_shared_templates_download', array(
			$this,
	...
	private function downloadTemplate( $requestUrl ) {
		$downloadUrlRequest = mn_remote_get( $requestUrl );
	...
	private function parseRequest( $request ) {
		...
		if ( isset( $body['status'], $body['url'] ) && 1 === $body['status'] ) {
			...
			if ( is_mn_error( $downloadedTemplateFile ) || ! $downloadedTemplateFile ) {
				return false;
			}
