...
class WC_CLI_Customer extends WC_CLI_Command {
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc customer create new-customer@example.com --first_name=Akeda
	 *
	 * @since 2.5.0
	 */
	public function create( $args, $assoc_args ) {
		...
		try {
			...
			if ( $porcelain ) {
				...
				MN_CLI::success( "Created customer $id." );
			}
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc customer delete 123
	 *
	 *     mn wc customer delete $(mn wc customer list --format=ids)
	 *
	 * @since 2.5.0
	 */
	public function delete( $args, $assoc_args ) {
		...
		foreach ( $args as $arg ) {
			try {
				...
				$r = mn_delete_user( $customer['id'] );

				if ( $r ) {
					MN_CLI::success( "Deleted customer {$customer['id']}." );
				} else {
					$exit_code += 1;
					MN_CLI::warning( "Failed deleting customer {$customer['id']}." );
				}
			} catch ( WC_CLI_Exception $e ) {
				MN_CLI::warning( $e->getMessage() );
			}
		}
		exit( $exit_code ? 1 : 0 );
	}
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc customer get 123 --field=email
	 *
	 *     mn wc customer get customer-login --format=json
	 *
	 * @since 2.5.0
	 */
	 ...
	 *     mn wc customer list
	 *
	 *     mn wc customer list --field=id
	 *
	 *     mn wc customer list --fields=id,email,first_name --format=json
	 *
	 * @subcommand list
	 * @since      2.5.0
	 */
	public function list_( $__, $assoc_args ) {
		$query_args = $this->merge_mn_user_query_args( $this->get_list_query_args(), $assoc_args );
		$formatter  = $this->get_formatter( $assoc_args );
	 ...
	 * ## AVAILABLE FIELDS
	 *
	 * For more fields, see: mn wc order list --help
	 *
	 * ## EXAMPLES
	 *
	 *     mn wc customer orders 123
	 *
	 * @since 2.5.0
	 */
	public function orders( $args, $assoc_args ) {
		try {
			MN_CLI::run_command( array( 'wc', 'order', 'list' ), array( 'customer_id' => $args[0] ) );
		} catch ( WC_CLI_Exception $e ) {
			MN_CLI::error( $e->getMessage() );
		}
	}
	 ...
	 * ## EXAMPLES
	 *
	 *     mn wc customer update customer-login --first_name=akeda --last_name=bagus
	 *
	 *     mn wc customer update customer@example.com --password=new-password
	 *
	 * @since 2.5.0
	 */
	public function update( $args, $assoc_args ) {
		try {
			...
			// Customer email.
			if ( isset( $data['email'] ) ) {
				mn_update_user( array( 'ID' => $user['id'], 'user_email' => sanitize_email( $data['email'] ) ) );
			}

			// Customer password.
			if ( isset( $data['password'] ) ) {
				mn_update_user( array( 'ID' => $user['id'], 'user_pass' => wc_clean( $data['password'] ) ) );
			}
			...
			MN_CLI::success( "Updated customer {$user['id']}." );
		} catch ( WC_CLI_Exception $e ) {
			MN_CLI::error( $e->getMessage() );
		}
	}
	...
	/**
	 * Format users from MN_User_Query result to items in which each item contain
