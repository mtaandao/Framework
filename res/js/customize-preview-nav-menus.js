/* global _mnCustomizePreviewNavMenusExports */
mn.customize.navMenusPreview = mn.customize.MenusCustomizerPreview = ( function( $, _, mn, api ) {
	'use strict';
	...
	if ( 'undefined' !== typeof _mnCustomizePreviewNavMenusExports ) {
		_.extend( self.data, _mnCustomizePreviewNavMenusExports );
	}
	...
	self.init = function() {
		...
		if ( api.selectiveRefresh ) {
			...
			/*
			 * Ensure that mn_nav_menu() instances nested inside of other partials
	...
	if ( api.selectiveRefresh ) {
		...
		/**
		 * Partial representing an invocation of mn_nav_menu().
		 *
		 * @class
		 * @augments mn.customize.selectiveRefresh.Partial
		 * @since 4.5.0
		 */
		self.NavMenuInstancePartial = api.selectiveRefresh.Partial.extend({
			 ...
			 *
			 * @since 4.5.0
			 * @param {mn.customize.Value|string} setting  - Object or ID.
			 ...
			 *
			 * @inheritdoc
			 * @param {mn.customize.selectiveRefresh.Placement} placement
			 */
			renderContent: function( placement ) {
				...
				if ( api.selectiveRefresh.Partial.prototype.renderContent.call( partial, placement ) ) {
					...
					$( document ).trigger( 'customize-preview-menu-refreshed', [ {
						...
						mnNavArgs: placement.context, // @deprecated
						mnNavMenuArgs: placement.context,
		 ...
		 * @since 4.5.0
		 *
		 * @param {mn.customize.Value} setting
		 * @param {object}             [options]
		...
		self.onChangeNavMenuLocationsSetting = function() {
			...
			// If there are no mn_nav_menu() instances that refer to the theme location, do full refresh.
