...
get_current_screen()->add_help_tab( array(
	...
	'title'   => __('Overview'),
	'content' =>
		'<p>' . __('Only use this screen once you have updated to a new version of Mtaandao through Updates/Available Updates (via the Network Administration navigation menu or the Toolbar). Clicking the Upgrade Network button will step through each site in the network, five at a time, and make sure any database updates are applied.') . '</p>' .
...
switch ( $action ) {
	case "upgrade":
		...
		if ( $n < 5 ) {
			/**
			 * @global string $mn_db_version
			 */
			global $mn_db_version;
		...
		foreach ( (array) $blogs as $details ) {
			...
			$response = mn_remote_get( $upgrade_url, array(
				'timeout'     => 120,
				'httpversion' => '1.1',
				'sslverify'   => false,
			) );
			 ...
			 * @since MU
			 *
			 * @param array|MN_Error $response The upgrade response array or MN_Error on failure.
			 */
	...
	case 'show':
	default:
		if ( get_site_option( 'mnmu_upgrade_site' ) != $GLOBALS['mn_db_version'] ) :
		?>
		<h2><?php _e( 'Database Update Required' ); ?></h2>
		<p><?php _e( 'Mtaandao has been updated! Before we send you on your way, we need to individually upgrade the sites in your network.' ); ?></p>
		<?php endif; ?>
