...
## 3.3.3.8 
* Fixed:      Issues with file writing. Basically many users don't install Mtaandao with all the permissions correct. 
...
## 3.2.9.16 
* Fixed:      Redux uploads dir should NOT be ~/main/uploads, but just main. 
              As per Otto.
...
## 3.1.9.18 
* Fixed:      SMALL issue with Mtaandao 3.9. Now it works.  ;)
...
## 3.1.9.6 
* Modified:   removed add_submenu_page when creating a submenu for us in the mn admin area.  mn
              approved API is used in it's place to being Redux up to mn.org theme check standards.
...
* Removed:  mn_get_current_user check.
...
* Fixed issue #132 (See #134, thanks @andreilupu): Could not have multiple Mtaandao Editors (mn_editor) as the
                    same ID was shared. Also fixed various styles to match Mtaandao for this field.
...
* More bugfixes
* Fixed spacing field
* Converted Redux to run as an auto-updating plugin. Getting ready to post to Mtaandao.org
...
* Added optional flag for ```mnautop``` on editors
* Added password field type
* Added ```checkbox_hide_all``` option
* Added mn3.5 media chooser
* Added Google webfonts previews
* Updated to mn3.5 color picker
* Minor style tweaks
