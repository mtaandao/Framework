 ...
 * @global MN_Post_Type $post_type_object
 * @global MN_Post      $post
 */
global $post_type, $post_type_object, $post;

mn_enqueue_script('post');
$_mn_editor_expand = $_content_editor_dfw = false;
 ...
 */
if ( post_type_supports( $post_type, 'editor' ) && ! mn_is_mobile() &&
	 ! ( $is_IE && preg_match( '/MSIE [5678]/', $_SERVER['HTTP_USER_AGENT'] ) ) &&
	 apply_filters( 'mn_editor_expand', true, $post_type ) ) {

	mn_enqueue_script('editor-expand');
	$_content_editor_dfw = true;
	$_mn_editor_expand = ( get_user_setting( 'editor_expand', 'on' ) === 'on' );
}
...
if ( $post_ID == get_option( 'page_for_posts' ) && empty( $post->post_content ) ) {
	add_action( 'edit_form_after_title', '_mn_posts_page_notice' );
...
$messages['post'] = array(
	...
	/* translators: %s: date and time of the revision */
	 5 => isset($_GET['revision']) ? sprintf( __( 'Post restored to revision from %s.' ), mn_post_revision_title( (int) $_GET['revision'], false ) ) : false,
...
$messages['page'] = array(
	 ...
	 4 => __( 'Page updated.' ),
	/* translators: %s: date and time of the revision */
	 5 => isset($_GET['revision']) ? sprintf( __( 'Page restored to revision from %s.' ), mn_post_revision_title( (int) $_GET['revision'], false ) ) : false,
...
if ( 'auto-draft' == $post->post_status ) {
	...
	$autosave = mn_get_post_autosave( $post_ID );
}
...
if ( $autosave && mysql2date( 'U', $autosave->post_modified_gmt, false ) > mysql2date( 'U', $post->post_modified_gmt, false ) ) {
	foreach ( _mn_post_revision_fields( $post ) as $autosave_field => $_autosave_field ) {
	...
	if ( ! $notice )
		mn_delete_post_revision( $autosave->ID );
...
if ( 'post' == $post_type ) {
	...
	$discussion_settings  = '<p>' . __('<strong>Send Trackbacks</strong> &mdash; Trackbacks are a way to notify legacy blog systems that you&#8217;ve linked to them. Enter the URL(s) you want to send trackbacks. If you link to other Mtaandao sites they&#8217;ll be notified automatically using pingbacks, and this field is unnecessary.') . '</p>';
...
?>>
<?php mn_nonce_field($nonce_action); ?>
...
if ( 'draft' != get_post_status( $post ) )
	mn_original_referer_field(true, 'previous');
...
if ( has_filter( 'pre_get_shortlink' ) || has_filter( 'get_shortlink' ) ) {
	$shortlink = mn_get_shortlink($post->ID, 'post');
...
<?php
mn_nonce_field( 'samplepermalink', 'samplepermalinknonce', false );
?>
</div><!-- /titlediv -->
<?php
}
...
?>
<div id="postdivrich" class="postarea<?php if ( $_mn_editor_expand ) { echo ' mn-editor-expand'; } ?>">

<?php mn_editor( $post->post_content, 'content', array(
	...
	'tinymce' => array(
		'resize' => false,
		'mn_autoresize_on' => $_mn_editor_expand,
		'add_unload_trigger' => false,
	),
) ); ?>
<table id="post-status-info"><tbody><tr>
	<td id="mn-word-count" class="hide-if-no-js"><?php printf( __( 'Word count: %s' ), '<span class="word-count">0</span>' ); ?></td>
...
<?php if ( ! mn_is_mobile() && post_type_supports( $post_type, 'title' ) && '' === $post->post_title ) : ?>
