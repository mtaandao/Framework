 ...
 */
function mn_embed_register_handler( $id, $regex, $callback, $priority = 10 ) {
	global $mn_embed;
	$mn_embed->register_handler( $id, $regex, $callback, $priority );
}
 ...
 */
function mn_embed_unregister_handler( $id, $priority = 10 ) {
	global $mn_embed;
	$mn_embed->unregister_handler( $id, $priority );
}
 ...
 */
function mn_embed_defaults( $url = '' ) {
...
function mn_oembed_add_provider( $format, $provider, $regex = false ) {
	...
	if ( did_action( 'plugins_loaded' ) ) {
		...
		MN_oEmbed::_add_provider_early( $format, $provider, $regex );
	}
}
...
function mn_oembed_remove_provider( $format ) {
	...
	if ( did_action( 'plugins_loaded' ) ) {
		...
		MN_oEmbed::_remove_provider_early( $format );
	}
 ...
 * @since 2.9.0
 *
 * @see mn_embed_register_handler()
 */
function mn_maybe_load_embeds() {
	/**
	...
	mn_embed_register_handler( 'youtube_embed_url', '#https?://(www.)?youtube\.com/(?:v|embed)/([^/]+)#i', 'mn_embed_handler_youtube' );
	 ...
	 */
	mn_embed_register_handler( 'audio', '#^https?://.+?\.(' . join( '|', mn_get_audio_extensions() ) . ')$#i', apply_filters( 'mn_audio_embed_handler', 'mn_embed_handler_audio' ), 9999 );
	 ...
	 */
	mn_embed_register_handler( 'video', '#^https?://.+?\.(' . join( '|', mn_get_video_extensions() ) . ')$#i', apply_filters( 'mn_video_embed_handler', 'mn_embed_handler_video' ), 9999 );
}
 ...
 *                        mn_embed_register_handler().
 * @param array  $attr    Embed attributes.
 * @param string $url     The original URL that was matched by the regex.
 * @param array  $rawattr The original unmodified attributes.
 * @return string The embed HTML.
 */
function mn_embed_handler_youtube( $matches, $attr, $url, $rawattr ) {
	global $mn_embed;
	$embed = $mn_embed->autoembed( "https://youtube.com/watch?v={$matches[2]}" );
	 ...
	 * @param array  $rawattr The original unmodified attributes.
	 */
	return apply_filters( 'mn_embed_handler_youtube', $embed, $attr, $url, $rawattr );
}
 ...
 * @since 3.6.0
 *
 * @param array  $matches The RegEx matches from the provided regex when calling mn_embed_register_handler().
 * @param array  $attr Embed attributes.
 * @param string $url The original URL that was matched by the regex.
 * @param array  $rawattr The original unmodified attributes.
 * @return string The embed HTML.
 */
function mn_embed_handler_audio( $matches, $attr, $url, $rawattr ) {
	 ...
	 * @param array  $rawattr The original unmodified attributes.
	 */
	return apply_filters( 'mn_embed_handler_audio', $audio, $attr, $url, $rawattr );
}
 ...
 * @return string The embed HTML.
 */
function mn_embed_handler_video( $matches, $attr, $url, $rawattr ) {
	$dimensions = '';
	 ...
	 * @param array  $rawattr The original unmodified attributes.
	 */
	return apply_filters( 'mn_embed_handler_video', $video, $attr, $url, $rawattr );
}
 ...
 * @since 4.4.0
 */
function mn_oembed_register_route() {
	$controller = new MN_oEmbed_Controller();
 ...
 * @since 4.4.0
 */
function mn_oembed_add_discovery_links() {
	$output = '';
 ...
 * @since 4.4.0
 */
function mn_oembed_add_host_js() {
	mn_enqueue_script( 'mn-embed' );
}
 ...
 * @since 4.4.0
 *
 * @param int|MN_Post $post Optional. Post ID or object. Defaults to the current post.
...
function get_post_embed_url( $post = null ) {
	 ...
	 * @param MN_Post $post      The corresponding post object.
	 */
...
function get_post_embed_html( $width, $height, $post = null ) {
	...
	$output = '<blockquote class="mn-embedded-content"><a href="' . esc_url( get_permalink( $post ) ) . '">' . get_the_title( $post ) . "</a></blockquote>\n";

	$output .= "<script type='text/javascript'>\n";
	$output .= "<!--//--><![CDATA[//><!--\n";
	if ( SCRIPT_DEBUG ) {
		$output .= file_get_contents( ABSPATH . RES . '/js/mn-embed.js' );
	} else {
		/*
		 * If you're looking at a src version of this file, you'll see an "include"
		 * statement below. This is used by the `grunt build` process to directly
		 * include a minified version of mn-embed.js, instead of using the
...
JS;
	...
	$output .= sprintf(
		'<iframe sandbox="allow-scripts" security="restricted" src="%1$s" width="%2$d" height="%3$d" title="%4$s" frameborder="0" marginwidth="0" marginheight="0" scrolling="no" class="mn-embedded-content"></iframe>',
		esc_url( $embed_url ),
		absint( $width ),
		absint( $height ),
		esc_attr(
 ...
 * @since 4.4.0
 *
 * @param MN_Post|int $post  Post object or ID.
...
function get_oembed_response_data_rich( $data, $post, $width, $height ) {
	...
	if ( 'attachment' === get_post_type( $post ) ) {
		if ( mn_attachment_is_image( $post ) ) {
			$thumbnail_id = $post->ID;
		} else if ( mn_attachment_is( 'video', $post ) ) {
	...
	if ( $thumbnail_id ) {
		list( $thumbnail_url, $thumbnail_width, $thumbnail_height ) = mn_get_attachment_image_src( $thumbnail_id, array( $width, 99999 ) );
 ...
 */
function mn_oembed_ensure_format( $format ) {
 ...
 * @param MN_HTTP_ResponseInterface $result  Result to send to the client. Usually a MN_REST_Response.
 * @param MN_REST_Request           $request Request used to generate the response.
 * @param MN_REST_Server            $server  Server instance.
 * @return true
 */
 ...
 */
function mn_filter_oembed_result( $result, $data, $url ) {
	...
	if ( false !== $mn_oembed->get_provider( $url, array( 'discover' => false ) ) ) {
		return $result;
	}
	...
	$html = mn_kses( $result, $allowed_html );
	...
	if ( ! empty( $content[1] ) ) {
		...
		$html = str_replace( '<blockquote', '<blockquote class="mn-embedded-content"', $html );
	}

	$html = str_replace( '<iframe', '<iframe class="mn-embedded-content" sandbox="allow-scripts" security="restricted"', $html );

	preg_match( '/ src=[\'"]([^\'"]*)[\'"]/', $html, $results );

	if ( ! empty( $results ) ) {
		$secret = mn_generate_password( 10, false );
 ...
 */
function mn_embed_excerpt_more( $more_string ) {
	if ( ! is_embed() ) {
		return $more_string;
	}

	$link = sprintf( '<a href="%1$s" class="mn-embed-more" target="_top">%2$s</a>',
 ...
 */
function mn_embed_excerpt_attachment( $content ) {
	if ( is_attachment() ) {
 ...
 *
 * Allows plugins to queue scripts for the embed iframe end using mn_enqueue_script().
 * Runs first in oembed_head().
 *
 * @since 4.4.0
 */
function enqueue_embed_scripts() {
	mn_enqueue_style( 'mn-embed-template-ie' );
...
function print_embed_styles() {
	...
	<?php
		if ( SCRIPT_DEBUG ) {
			readfile( ABSPATH . RES . "/css/mn-embed-template.css" );
		} else {
			/*
			 * If you're looking at a src version of this file, you'll see an "include"
			 * statement below. This is used by the `grunt build` process to directly
			 * include a minified version of mn-oembed-embed.css, instead of using the
...
function print_embed_scripts() {
	...
	<?php
		if ( SCRIPT_DEBUG ) {
			readfile( ABSPATH . RES . "/js/mn-embed-template.js" );
		} else {
			/*
			 * If you're looking at a src version of this file, you'll see an "include"
			 * statement below. This is used by the `grunt build` process to directly
			 * include a minified version of mn-embed-template.js, instead of using the
			 * readfile() method from above.
			 *
			 * If you're looking at a build version of this file, you'll see a string of
			 * minified JavaScript. If you need to debug it, please turn on SCRIPT_DEBUG
			 * and edit mn-embed-template.js directly.
			 */
			?>
			!function(a,b){"use strict";function c(b,c){a.parent.postMessage({message:b,value:c,secret:g},"*")}function d(){function d(){l.className=l.className.replace("hidden",""),b.querySelector('.mn-embed-share-tab-button [aria-selected="true"]').focus()}function e(){l.className+=" hidden",b.querySelector(".mn-embed-share-dialog-open").focus()}function f(a){var c=b.querySelector('.mn-embed-share-tab-button [aria-selected="true"]');c.setAttribute("aria-selected","false"),b.querySelector("#"+c.getAttribute("aria-controls")).setAttribute("aria-hidden","true"),a.target.setAttribute("aria-selected","true"),b.querySelector("#"+a.target.getAttribute("aria-controls")).setAttribute("aria-hidden","false")}function g(a){var c,d,e=a.target,f=e.parentElement.previousElementSibling,g=e.parentElement.nextElementSibling;if(37===a.keyCode)c=f;else{if(39!==a.keyCode)return!1;c=g}"rtl"===b.documentElement.getAttribute("dir")&&(c=c===f?g:f),c&&(d=c.firstElementChild,e.setAttribute("tabindex","-1"),e.setAttribute("aria-selected",!1),b.querySelector("#"+e.getAttribute("aria-controls")).setAttribute("aria-hidden","true"),d.setAttribute("tabindex","0"),d.setAttribute("aria-selected","true"),d.focus(),b.querySelector("#"+d.getAttribute("aria-controls")).setAttribute("aria-hidden","false"))}function h(a){var c=b.querySelector('.mn-embed-share-tab-button [aria-selected="true"]');n!==a.target||a.shiftKey?c===a.target&&a.shiftKey&&(n.focus(),a.preventDefault()):(c.focus(),a.preventDefault())}function i(a){var b,d=a.target;b=d.hasAttribute("href")?d.getAttribute("href"):d.parentElement.getAttribute("href"),b&&(c("link",b),a.preventDefault())}if(!k){k=!0;var j,l=b.querySelector(".mn-embed-share-dialog"),m=b.querySelector(".mn-embed-share-dialog-open"),n=b.querySelector(".mn-embed-share-dialog-close"),o=b.querySelectorAll(".mn-embed-share-input"),p=b.querySelectorAll(".mn-embed-share-tab-button button"),q=b.querySelector(".mn-embed-featured-image img");if(o)for(j=0;j<o.length;j++)o[j].addEventListener("click",function(a){a.target.select()});if(m&&m.addEventListener("click",function(){d()}),n&&n.addEventListener("click",function(){e()}),p)for(j=0;j<p.length;j++)p[j].addEventListener("click",f),p[j].addEventListener("keydown",g);b.addEventListener("keydown",function(a){27===a.keyCode&&-1===l.className.indexOf("hidden")?e():9===a.keyCode&&h(a)},!1),a.self!==a.top&&(c("height",Math.ceil(b.body.getBoundingClientRect().height)),q&&q.addEventListener("load",function(){c("height",Math.ceil(b.body.getBoundingClientRect().height))}),b.addEventListener("click",i))}}function e(){a.self!==a.top&&(clearTimeout(i),i=setTimeout(function(){c("height",Math.ceil(b.body.getBoundingClientRect().height))},100))}function f(){a.self===a.top||g||(g=a.location.hash.replace(/.*secret=([\d\w]{10}).*/,"$1"),clearTimeout(h),h=setTimeout(function(){f()},100))}var g,h,i,j=b.querySelector&&a.addEventListener,k=!1;j&&(f(),b.documentElement.className=b.documentElement.className.replace(/\bno-js\b/,"")+" js",b.addEventListener("DOMContentLoaded",d,!1),a.addEventListener("load",d,!1),a.addEventListener("resize",e,!1))}(window,document);
			<?php
		}
	?>
	</script>
...
function _oembed_filter_feed_content( $content ) {
	return str_replace( '<iframe class="mn-embedded-content" sandbox="allow-scripts" security="restricted" style="position: absolute; clip: rect(1px, 1px, 1px, 1px);"', '<iframe class="mn-embedded-content" sandbox="allow-scripts" security="restricted"', $content );
}
...
function print_embed_sharing_button() {
	...
	?>
	<div class="mn-embed-share">
		<button type="button" class="mn-embed-share-dialog-open" aria-label="<?php esc_attr_e( 'Open sharing dialog' ); ?>">
...
function print_embed_sharing_dialog() {
	...
	?>
	<div class="mn-embed-share-dialog hidden" role="dialog" aria-label="<?php esc_attr_e( 'Sharing options' ); ?>">
		<div class="mn-embed-share-dialog-content">
			<div class="mn-embed-share-dialog-text">
				...
				<div id="mn-embed-share-tab-html" class="mn-embed-share-tab" role="tabpanel" aria-hidden="true">
					<textarea class="mn-embed-share-input" aria-describedby="mn-embed-share-description-html" tabindex="0" readonly><?php echo esc_textarea( get_post_embed_html( 600, 400 ) ); ?></textarea>

					<p class="mn-embed-share-description" id="mn-embed-share-description-html">
						<?php _e( 'Copy and paste this code into your site to embed' ); ?>
					</p>
				</div>
			</div>

			<button type="button" class="mn-embed-share-dialog-close" aria-label="<?php esc_attr_e( 'Close sharing dialog' ); ?>">
...
function the_embed_site_title() {
	$site_title = sprintf(
		'<a href="%s" target="_top"><img src="%s" srcset="%s 2x" width="32" height="32" alt="" class="mn-embed-site-icon"/><span>%s</span></a>',
		esc_url( home_url() ),
		esc_url( get_site_icon_url( 32, admin_url( 'images/m-logo-g.png' ) ) ),
		esc_url( get_site_icon_url( 64, admin_url( 'images/m-logo-g.png' ) ) ),
		esc_html( get_bloginfo( 'name' ) )
	);

	$site_title = '<div class="mn-embed-site-title">' . $site_title . '</div>';
 ...
 */
function mn_filter_pre_oembed_result( $result, $url, $args ) {
	$post_id = url_to_postid( $url );

	/** This filter is documented in res/class-mn-oembed-controller.php */
	...
	$data = _mn_oembed_get_object()->data2html( (object) $data, $url );
