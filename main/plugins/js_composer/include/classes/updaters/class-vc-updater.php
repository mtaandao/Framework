...
class Vc_Updater {
	 ...
	 * @var string
	 */
	protected $version_url = 'http://updates.mtaandao.co.ke/plugins/page-builder/';
	 ...
	 * @var string
	 */
	protected $download_link_url = 'http://support.mtaandao.co.ke/plugins/page-builder/updates/download-link';
	 ...
	 * @param $reply
	 * @param $package
	 * @param $updater MN_Upgrader
	 *
	 * @return mixed|string|MN_Error
	 */
	public function preUpgradeFilter( $reply, $package, $updater ) {
		...
		$res = $updater->fs_connect( array( MN_CONTENT_DIR ) );
		if ( ! $res ) {
			return new MN_Error( 'no_credentials', __( "Error! Can't connect to filesystem", 'js_composer' ) );
		}
		...
		if ( ! vc_license()->isActivated() ) {
			...
			return new MN_Error( 'no_credentials', __( 'To receive automatic updates license activation is required. Please visit <a href="' . $url . '' . '" target="_blank">Settings</a> to activate your Visual Composer.', 'js_composer' ) . ' ' . sprintf( ' <a href="http://go.mtaandao.co.ke/plugins/page-builder/faq-update-in-theme" target="_blank">%s</a>', __( 'Got Visual Composer in theme?', 'js_composer' ) ) );
		}
		...
		if ( ! $response ) {
			return new MN_Error( 'no_credentials', __( 'Download link could not be retrieved', 'js_composer' ) );
		}

		if ( ! $response['status'] ) {
			return new MN_Error( 'no_credentials', $response['error'] );
		}
		...
		if ( is_mn_error( $downloaded_archive ) ) {
			return $downloaded_archive;
		}

		$plugin_directory_name = dirname( vc_plugin_name() );

		// MN will use same name for plugin directory as archive name, so we have to rename it
	...
	public function upgradeFilterFromEnvato( $reply, $package, $updater ) {
		...
		/** @var \MN_Filesystem_Base $mn_filesystem */
		...
		if ( ( isset( $updater->skin->plugin ) && vc_plugin_name() === $updater->skin->plugin ) || ( isset( $updater->skin->plugin_info ) && $updater->skin->plugin_info['Name'] === $this->title ) ) {
			...
			if ( ! vc_license()->isActivated() || empty( $username ) || empty( $api_key ) || empty( $purchase_code ) ) {
				return new MN_Error( 'no_credentials', __( 'To receive automatic updates license activation is required. Please visit <a href="' . admin_url( 'admin.php?page=vc-updater' ) . '' . '" target="_blank">Settings</a> to activate your Visual Composer.', 'js_composer' ) );
			}
			$json = mn_remote_get( $this->envatoDownloadPurchaseUrl( $username, $api_key, $purchase_code ) );
			$result = json_decode( $json['body'], true );
			if ( ! isset( $result['download-purchase']['download_url'] ) ) {
				return new MN_Error( 'no_credentials', __( 'Error! Envato API error', 'js_composer' ) . ( isset( $result['error'] ) ? ': ' . $result['error'] : '.' ) );
			}
			...
			$upgrade_folder = $mn_filesystem->mn_content_dir() . 'uploads/js_composer_envato_package';
			if ( is_dir( $upgrade_folder ) ) {
				$mn_filesystem->delete( $upgrade_folder );
			}
			...
			return new MN_Error( 'no_credentials', __( 'Error on unzipping package', 'js_composer' ) );
		}
	...
	public function removeTemporaryDir() {
		...
		global $mn_filesystem;
		/** @var \MN_Filesystem_Base $mn_filesystem */
		if ( is_dir( $mn_filesystem->mn_content_dir() . 'uploads/js_composer_envato_package' ) ) {
			$mn_filesystem->delete( $mn_filesystem->mn_content_dir() . 'uploads/js_composer_envato_package', true );
		}
	}
