...
/**
 * Wrapper for mn_get_post_terms which supports ordering by parent.
 *
 * NOTE: At this point in time, ordering by menu_order for example isn't possible with this function. mn_get_post_terms has no.
...
function wc_get_product_terms( $product_id, $taxonomy, $args = array() ) {
	...
	if ( ! empty( $args['orderby'] ) && in_array( $args['orderby'], array( 'name_num', 'parent' ) ) ) {
		...
		// Unset for mn_get_post_terms
		unset( $args['orderby'] );
		unset( $args['fields'] );

		$terms = mn_get_post_terms( $product_id, $taxonomy, $args );
		...
		switch ( $fields ) {
			case 'names' :
				$terms = mn_list_pluck( $terms, 'name' );
				break;
			case 'ids' :
				$terms = mn_list_pluck( $terms, 'term_id' );
				break;
			case 'slugs' :
				$terms = mn_list_pluck( $terms, 'slug' );
				break;
		}
	} elseif ( ! empty( $args['orderby'] ) && $args['orderby'] === 'menu_order' ) {
		// mn_get_post_terms doesn't let us use custom sort order
		$args['include'] = mn_get_post_terms( $product_id, $taxonomy, array( 'fields' => 'ids' ) );
 ...
 * @param  MN_POST object $a
 * @param  MN_POST object $b
 * @return int
 */
 ...
 * We use a custom walker, just like Mtaandao does.
 *
...
function wc_product_dropdown_categories( $args = array(), $deprecated_hierarchical = 1, $deprecated_show_uncategorized = 1, $deprecated_orderby = '' ) {
	...
	$current_product_cat = isset( $mn_query->query_vars['product_cat'] ) ? $mn_query->query_vars['product_cat'] : '';
	$defaults            = array(
		'pad_counts'         => 1,
		'show_count'         => 1,
		'hierarchical'       => 1,
...
function wc_taxonomy_metadata_update_content_for_split_terms( $old_term_id, $new_term_id, $term_taxonomy_id, $taxonomy ) {
	...
	if ( get_option( 'db_version' ) < 34370 ) {
		if ( 'product_cat' === $taxonomy || taxonomy_is_product_attribute( $taxonomy ) ) {
			$old_meta_data = $mndb->get_results( $mndb->prepare( "SELECT * FROM {$mndb->prefix}woocommerce_termmeta WHERE woocommerce_term_id = %d;", $old_term_id ) );

			// Copy across to split term
			if ( $old_meta_data ) {
				foreach ( $old_meta_data as $meta_data ) {
					$mndb->insert(
						"{$mndb->prefix}woocommerce_termmeta",
						array(
...
/**
 * Migrate data from WC term meta to MN term meta
 *
 ...
 *
 * @param  string $mn_db_version The new $mn_db_version.
 * @param  string $mn_current_db_version The old (current) $mn_db_version.
 */
function wc_taxonomy_metadata_migrate_data( $mn_db_version, $mn_current_db_version ) {
	if ( $mn_db_version >= 34370 && $mn_current_db_version < 34370 ) {
		global $mndb;
...
add_action( 'mn_upgrade', 'wc_taxonomy_metadata_migrate_data', 10, 2 );

/**
 * WooCommerce Term Meta API
 *
 * WC tables for storing term meta are @deprecated from Mtaandao 4.4 since 4.4 has its own table.
 * This function serves as a wrapper, using the new table if present, or falling back to the WC table.
 *
 * @todo These functions should be deprecated with notices in a future WC version, allowing users a chance to upgrade Mtaandao.
 *
 * @param mixed $term_id
...
function wc_terms_clauses( $clauses, $taxonomies, $args ) {
	...
	// Query join.
	if ( get_option( 'db_version' ) < 34370 ) {
		$clauses['join'] .= " LEFT JOIN {$mndb->woocommerce_termmeta} AS tm ON (t.term_id = tm.woocommerce_term_id AND tm.meta_key = '" . esc_sql( $meta_name ) . "') ";
	} else {
		$clauses['join'] .= " LEFT JOIN {$mndb->termmeta} AS tm ON (t.term_id = tm.term_id AND tm.meta_key = '" . esc_sql( $meta_name ) . "') ";
	}
...
function _wc_term_recount( $terms, $taxonomy, $callback = true, $terms_are_term_taxonomy_ids = true ) {
	...
	if ( get_option( 'woocommerce_hide_out_of_stock_items' ) == 'yes' ) {
		$stock_join  = "LEFT JOIN {$mndb->postmeta} AS meta_stock ON posts.ID = meta_stock.post_id";
		$stock_query = "
	...
	// Main query
	$count_query = "
		SELECT COUNT( DISTINCT posts.ID ) FROM {$mndb->posts} as posts
		LEFT JOIN {$mndb->postmeta} AS meta_visibility ON posts.ID = meta_visibility.post_id
		...
		LEFT JOIN {$mndb->postmeta} AS postmeta ON posts.ID = postmeta.post_id
		$stock_join
	...
	foreach ( $terms as $term_id ) {
		...
		if ( is_taxonomy_hierarchical( $taxonomy->name ) ) {
			...
			if ( ( $children = get_term_children( $term_id, $taxonomy->name ) ) && ! is_mn_error( $children ) ) {
		...
		// Get the count
		$count = $mndb->get_var( $count_query . $term_query );
