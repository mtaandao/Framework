...
jQuery( function( $ ) {
	...
	// Scroll to first checked category - https://github.com/scribu/mn-category-checklist-tree/blob/d1c3c1f449e1144542efa17dde84a9f52ade1739/category-checklist-tree.php
	$( function() {
	...
	$( document.body ).on( 'click', '.upload_file_button', function( event ) {
		...
		downloadable_file_frame = mn.media.frames.downloadable_file = mn.media({
	...
	$( '.add_product_images' ).on( 'click', 'a', function( event ) {
		...
		// Create the media frame.
		product_gallery_frame = mn.media.frames.product_gallery = mn.media({
			// Set the title of the modal.
			title: $el.data( 'choose' ),
			button: {
				text: $el.data( 'update' )
