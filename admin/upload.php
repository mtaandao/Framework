...
if ( !current_user_can('upload_files') )
	mn_die( __( 'Sorry, you are not allowed to upload files.' ) );
...
if ( 'grid' === $mode ) {
	...
	// let JS handle this
	unset( $q['s'] );
	$vars = mn_edit_attachments_query_vars( $q );
	...
	mn_localize_script( 'media-grid', '_mnMediaGridSettings', array(
	...
	?>
	<div class="wrap" id="mn-media-grid" data-search="<?php _admin_search_query() ?>">
		<h1>
		<?php
		echo esc_html( $title );
		if ( current_user_can( 'upload_files' ) ) { ?>
...
$mn_list_table = _get_list_table('MN_Media_List_Table');
...
if ( $doaction ) {
	...
	if ( 'delete_all' == $doaction ) {
		$post_ids = $mndb->get_col( "SELECT ID FROM $mndb->posts WHERE post_type='attachment' AND post_status = 'trash'" );
		$doaction = 'delete';
	...
	switch ( $doaction ) {
		case 'detach':
			mn_media_attach_action( $_REQUEST['parent_post_id'], 'detach' );
			break;

		case 'attach':
			mn_media_attach_action( $_REQUEST['found_post_id'] );
			break;
...
if ( ! empty( $_GET['trashed'] ) && $trashed = absint( $_GET['trashed'] ) ) {
	...
	$message .= ' <a href="' . esc_url( mn_nonce_url( 'upload.php?doaction=undo&action=untrash&ids='.(isset($_GET['ids']) ? $_GET['ids'] : ''), "bulk-media" ) ) . '">' . __('Undo') . '</a>';
...
$messages[4] = __( 'Media file moved to the trash.' ) . ' <a href="' . esc_url( mn_nonce_url( 'upload.php?doaction=undo&action=untrash&ids='.(isset($_GET['ids']) ? $_GET['ids'] : ''), "bulk-media" ) ) . '">' . __( 'Undo' ) . '</a>';
