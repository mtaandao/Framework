...
$referer = remove_query_arg( 'update', mn_get_referer() );
...
if ( $action ) {
	...
	switch ( $action ) {
		case 'newuser':
			check_admin_referer( 'add-user', '_mnnonce_add-new-user' );
			$user = $_POST['user'];
			if ( ! is_array( $_POST['user'] ) || empty( $user['username'] ) || empty( $user['email'] ) ) {
				$update = 'err_new';
			} else {
				$password = mn_generate_password( 12, false);
				$user_id = mnmu_create_user( esc_html( strtolower( $user['username'] ) ), $password, esc_html( $user['email'] ) );
	...
	mn_safe_redirect( add_query_arg( 'update', $update, $referer ) );
	exit();
}
 ...
 */
if ( ! mn_is_large_network( 'users' ) && apply_filters( 'show_network_site_users_add_existing_form', true ) )
...
<form action="site-users.php?action=adduser" id="adduser" method="post">
	...
	<table class="form-table">
		<tr>
			...
			<td><input type="text" class="regular-text mn-suggest-user" name="newuser" id="newuser" /></td>
		</tr>
		<tr>
...
<form action="<?php echo network_admin_url('site-users.php?action=newuser'); ?>" id="newuser" method="post">
	...
	<?php mn_nonce_field( 'add-user', '_mnnonce_add-new-user' ) ?>
