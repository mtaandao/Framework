...
class RWMB_Core {
	...
	public function __construct() {
		...
		add_action( 'init', array( $this, 'register_mnml_hooks' ) );
	 ...
	 * @since 4.3.10
	 */
	public function fix_page_template( MN_Post $post ) {
		$template       = get_post_meta( $post->ID, '_mn_page_template', true );
		$page_templates = mn_get_theme()->get_page_templates();

		// If the template doesn't exists, remove the data to allow WordPress to save
		if ( ! isset( $page_templates[ $template ] ) ) {
			delete_post_meta( $post->ID, '_mn_page_template' );
		}
	}

	/**
	 * Register mnml compatibility hooks
	 */
	public function register_mnml_hooks() {
