 ...
 *
 * These functions must be used within the Mtaandao Loop.
 *
 * @link https://mtaandao.co.ke/docs/Author_Templates
 *
 * @package Mtaandao
 * @subpackage Template
 */
 ...
 * @since 0.71
 * @see get_the_author()
 * @link https://mtaandao.co.ke/docs/Template_Tags/the_author
 *
 * @param string $deprecated Deprecated.
 ...
 * @link https://mtaandao.co.ke/docs/Template_Tags/the_author_meta
 * @since 2.8.0
 *
 * @global object $authordata The current author's DB object.
 *
 ...
 * author's name.
 *
 * @link https://mtaandao.co.ke/docs/Template_Tags/the_author_link
 *
 * @since 2.1.0
 */
 ...
 *
 * @link https://mtaandao.co.ke/docs/Template_Tags/the_author_posts
 * @since 0.71
 */
...
function get_author_posts_url( $author_id, $author_nicename = '' ) {
	...
	$link = $mn_rewrite->get_author_permastruct();
 ...
 *
 * @link https://mtaandao.co.ke/docs/Template_Tags/mn_list_authors
 *
 * @since 1.2.0
 *
...
function mn_list_authors( $args = '' ) {
	...
	$query_args = mn_array_slice_assoc( $args, array( 'orderby', 'order', 'number', 'exclude', 'include' ) );
	$query_args['fields'] = 'ids';
	$authors = get_users( $query_args );

	$author_count = array();
	foreach ( (array) $mndb->get_results( "SELECT DISTINCT post_author, COUNT(ID) AS count FROM $mndb->posts WHERE " . get_private_posts_cap_sql( 'post' ) . " GROUP BY post_author" ) as $row ) {
...
function is_multi_author() {
	...
	if ( false === ( $is_multi_author = get_transient( 'is_multi_author' ) ) ) {
		$rows = (array) $mndb->get_col("SELECT DISTINCT post_author FROM $mndb->posts WHERE post_type = 'post' AND post_status = 'publish' LIMIT 2");
