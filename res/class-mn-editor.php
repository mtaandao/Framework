<?php
/**
 * Facilitates adding of the Mtaandao editor as used on the Write and Edit screens.
 *
 * @package Mtaandao
 * @since 3.3.0
 *
 * Private, not included by default. See mn_editor() in res/general-template.php.
 */

final class _MN_Editors {
	 ...
	 *
	 *     @type bool       $mnautop           Whether to use mnautop(). Default true.
	 ...
	 *     @type bool       $dfw               Deprecated in 4.1. Since 4.3 used only to enqueue mn-fullscreen-stub.js
	...
	public static function parse_settings( $editor_id, $settings ) {
		...
		/**
		 * Filters the mn_editor() settings.
		 *
		 * @since 4.0.0
		 *
		 * @see _MN_Editors()::parse_settings()
		 *
		 * @param array  $settings  Array of editor arguments.
		 * @param string $editor_id ID for the current editor instance.
		 */
		$settings = apply_filters( 'mn_editor_settings', $settings, $editor_id );

		$set = mn_parse_args( $settings, array(
			'mnautop'             => true,
			'media_buttons'       => true,
			'default_editor'      => '',
		...
		if ( self::$this_tinymce ) {
			if ( false !== strpos( $editor_id, '[' ) ) {
				...
				_deprecated_argument( 'mn_editor()', '3.9.0', 'TinyMCE editor IDs cannot have brackets.' );
			}
		}
		...
		if ( 'content' === $editor_id && empty( $set['tinymce']['mn_autoresize_on'] ) ) {
	...
	public static function editor( $content, $editor_id, $settings = array() ) {
		...
		$editor_class = ' class="' . trim( esc_attr( $set['editor_class'] ) . ' mn-editor-area' ) . '"';
		...
		if ( self::$this_tinymce ) {
			...
			if ( self::$this_quicktags ) {
				$default_editor = $set['default_editor'] ? $set['default_editor'] : mn_default_editor();
				// 'html' is used for the "Text" editor tab.
				if ( 'html' !== $default_editor ) {
					$default_editor = 'tinymce';
				}

				$buttons .= '<button type="button" id="' . $editor_id_attr . '-tmce" class="mn-switch-editor switch-tmce"' .
					' data-mn-editor-id="' . $editor_id_attr . '">' . __('Visual') . "</button>\n";
				$buttons .= '<button type="button" id="' . $editor_id_attr . '-html" class="mn-switch-editor switch-html"' .
					' data-mn-editor-id="' . $editor_id_attr . '">' . _x( 'Text', 'Name for the Text editor tab (formerly HTML)' ) . "</button>\n";
			} else {
				$default_editor = 'tinymce';
			}
		}
		...
		$wrap_class = 'mn-core-ui mn-editor-wrap ' . $switch_class;

		if ( $set['_content_editor_dfw'] ) {
			$wrap_class .= ' has-dfw';
		}

		echo '<div id="mn-' . $editor_id_attr . '-wrap" class="' . $wrap_class . '">';

		if ( self::$editor_buttons_css ) {
			mn_print_styles( 'editor-buttons' );
		...
		if ( ! empty( $buttons ) || $set['media_buttons'] ) {
			echo '<div id="mn-' . $editor_id_attr . '-editor-tools" class="mn-editor-tools hide-if-no-js">';
			...
			if ( $set['media_buttons'] ) {
				...
				echo '<div id="mn-' . $editor_id_attr . '-media-buttons" class="mn-media-buttons">';
			...
			echo '<div class="mn-editor-tabs">' . $buttons . "</div>\n";
			echo "</div>\n";
		}
		 ...
		 */
		$the_editor = apply_filters( 'the_editor', '<div id="mn-' . $editor_id_attr . '-editor-container" class="mn-editor-container">' .
			$quicktags_toolbar .
	...
	public static function editor_settings($editor_id, $set) {
		global $mn_version, $tinymce_version;
		...
		if ( empty(self::$first_init) ) {
			if ( is_admin() ) {
				...
				add_action( 'mn_print_footer_scripts', array( __CLASS__, 'editor_js' ), 50 );
				add_action( 'mn_print_footer_scripts', array( __CLASS__, 'enqueue_scripts' ), 1 );
			}
		}
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty( self::$first_init ) ) {
				...
				if ( $set['teeny'] ) {
					 ...
					 * @param string $editor_id Unique editor identifier, e.g. 'content'.
					 */
					self::$plugins = $plugins = apply_filters( 'teeny_mce_plugins', array( 'colorpicker', 'lists', 'fullscreen', 'image', 'Mtaandao', 'mneditimage', 'mnlink' ), $editor_id );
				} else {
					 ...
					 * in Mtaandao should be added to the TinyMCE instance.
					 *
					 * @since 3.3.0
					 *
					...
					if ( ! empty( $mce_external_plugins ) ) {
						 ...
						 *
						 * The language file should follow the same format as mn_mce_translation(),
				...
				self::$first_init = array(
					...
					'formats' => '{' .
						'alignleft: [' .
							...
							'{selector: "img,table,dl.mn-caption", classes: "alignleft"}' .
						'],' .
						'aligncenter: [' .
							'{selector: "p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li", styles: {textAlign:"center"}},' .
							'{selector: "img,table,dl.mn-caption", classes: "aligncenter"}' .
						'],' .
						'alignright: [' .
							'{selector: "p,h1,h2,h3,h4,h5,h6,td,th,div,ul,ol,li", styles: {textAlign:"right"}},' .
							'{selector: "img,table,dl.mn-caption", classes: "alignright"}' .
						'],' .
					...
					'entity_encoding' => 'raw',
					'keep_styles' => false,
					'cache_suffix' => 'mn-mce-' . $tinymce_version,

					// Limit the preview styles in the menu/toolbar
					'preview_styles' => 'font-family font-size font-weight font-style text-decoration text-transform',

					'end_container_on_empty_block' => true,
					'mneditimage_disable_captions' => $no_captions,
					'mneditimage_html5_captions' => current_theme_supports( 'html5', 'caption' ),
					'plugins' => implode( ',', $plugins ),
					'mn_lang_attr' => get_bloginfo( 'language' )
				);

				if ( ! empty( $mce_external_plugins ) ) {
					self::$first_init['external_plugins'] = mn_json_encode( $mce_external_plugins );
				}
				...
				// Mtaandao default stylesheet and dashicons
				$mce_css = array(
					$dashicons,
					self::$baseurl . '/skins/mtaandao/main.css?' . $version
				);
			...
			if ( $set['teeny'] ) {
				...
				$mce_buttons = array( 'bold', 'italic', 'strikethrough', 'bullist', 'numlist', 'blockquote', 'hr', 'alignleft', 'aligncenter', 'alignright', 'link', 'unlink', 'mn_more', 'spellchecker' );
			...
			if ( $post = get_post() ) {
				...
				if ( post_type_supports( $post->post_type, 'post-formats' ) ) {
					...
					if ( $post_format && ! is_mn_error( $post_format ) )
	...
	public static function enqueue_scripts() {
		if ( self::$has_tinymce )
			mn_enqueue_script('editor');

		if ( self::$has_quicktags ) {
			mn_enqueue_script( 'quicktags' );
			mn_enqueue_style( 'buttons' );
		}

		if ( in_array('mnlink', self::$plugins, true) || in_array('link', self::$qt_buttons, true) ) {
			mn_enqueue_script('mnlink');
			mn_enqueue_script( 'jquery-ui-autocomplete' );
		}

		if ( self::$old_dfw_compat ) {
			mn_enqueue_script('mn-fullscreen-stub');
		}
		 ...
		 */
		do_action( 'mn_enqueue_editor', array(
	 ...
	 * Can be used directly (_MN_Editors::mn_mce_translation()) by passing the same locale as set in the TinyMCE init object.
	 *
	 * @static
	 ...
	 */
	public static function mn_mce_translation( $mce_locale = '', $json_only = false ) {
		 ...
		 */
		$mce_translation = apply_filters( 'mn_mce_translation', $mce_translation, $mce_locale );
		...
		if ( $json_only ) {
			return mn_json_encode( $mce_translation );
		}

		$baseurl = self::$baseurl ? self::$baseurl : includes_url( 'js/tinymce' );

		return "tinymce.addI18n( '$mce_locale', " . mn_json_encode( $mce_translation ) . ");\n" .
	...
	public static function editor_js() {
		global $mn_version, $tinymce_version, $concatenate_scripts, $compress_scripts;
		 ...
		 */
		do_action( 'before_mn_tiny_mce', self::$mce_settings );
		?>
		...
		// Load tinymce.js when running from /src, else load mn-tinymce.js.gz (production) or tinymce.min.js (SCRIPT_DEBUG)
		$mce_suffix = false !== strpos( $mn_version, '-src' ) ? '' : '.min';

		if ( $tmce_on ) {
			if ( $compressed ) {
				echo "<script type='text/javascript' src='{$baseurl}/mn-tinymce.php?c=1&amp;$version'></script>\n";
			} else {
			...
			echo "<script type='text/javascript'>\n" . self::mn_mce_translation() . "</script>\n";

			if ( self::$ext_plugins ) {
				// Load the old-format English strings to prevent unsightly labels in old style popups
				echo "<script type='text/javascript' src='{$baseurl}/langs/mn-langs-en.js?$version'></script>\n";
			}
		}
		 ...
		 * @param array $mce_settings TinyMCE settings array.
		 */
		do_action( 'mn_tiny_mce_init', self::$mce_settings );
		...
		( function() {
			...
			if ( typeof tinymce !== 'undefined' ) {
				...
				for ( id in tinyMCEPreInit.mceInit ) {
					...
					$wrap = tinymce.$( '#mn-' + id + '-wrap' );

					if ( ( $wrap.hasClass( 'tmce-active' ) || ! tinyMCEPreInit.qtInit.hasOwnProperty( id ) ) && ! init.mn_skip_init ) {
						tinymce.init( init );

						if ( ! window.mnActiveEditor ) {
							window.mnActiveEditor = id;
						}
					}
				}
			}
		 ...
		 * @param array $mce_settings TinyMCE settings array.
		 */
		do_action( 'after_mn_tiny_mce', self::$mce_settings );
	}
	 ...
	 */
	public static function mn_fullscreen_html() {
	 ...
	 */
	public static function mn_link_query( $args = array() ) {
		 ...
		 *
		 * @see MN_Query for a full list of arguments
		 *
		 * @since 3.7.0
		 *
		 * @param array $query An array of MN_Query arguments.
		 */
		$query = apply_filters( 'mn_link_query_args', $query );
		 ...
		 * @param array $query  An array of MN_Query arguments.
		 */
		return apply_filters( 'mn_link_query', $results, $query );
	}
	 ...
	 * @static
	 */
	public static function mn_link_dialog() {
		// display: none is required here, see #MN27605
		?>
		<div id="mn-link-backdrop" style="display: none"></div>
		<div id="mn-link-wrap" class="mn-core-ui" style="display: none" role="dialog" aria-labelledby="link-modal-title">
		<form id="mn-link" tabindex="-1">
		<?php mn_nonce_field( 'internal-linking', '_ajax_linking_nonce', false ); ?>
		<h1 id="link-modal-title"><?php _e( 'Insert/edit link' ) ?></h1>
		<button type="button" id="mn-link-close"><span class="screen-reader-text"><?php _e( 'Close' ); ?></span></button>
		<div id="link-selector">
			<div id="link-options">
				<p class="howto" id="mnlink-enter-url"><?php _e( 'Enter the destination URL' ); ?></p>
				<div>
					<label><span><?php _e( 'URL' ); ?></span>
					<input id="mn-link-url" type="text" aria-describedby="mnlink-enter-url" /></label>
				</div>
				<div class="mn-link-text-field">
					<label><span><?php _e( 'Link Text' ); ?></span>
					<input id="mn-link-text" type="text" /></label>
				</div>
				<div class="link-target">
					<label><span></span>
					<input type="checkbox" id="mn-link-target" /> <?php _e( 'Open link in a new tab' ); ?></label>
				</div>
			</div>
			<p class="howto" id="mnlink-link-existing-content"><?php _e( 'Or link to existing content' ); ?></p>
			<div id="search-panel">
				<div class="link-search-wrapper">
					<label>
						<span class="search-label"><?php _e( 'Search' ); ?></span>
						<input type="search" id="mn-link-search" class="link-search-field" autocomplete="off" aria-describedby="mnlink-link-existing-content" />
		...
		<div class="submitbox">
			<div id="mn-link-cancel">
				<button type="button" class="button"><?php _e( 'Cancel' ); ?></button>
			</div>
			<div id="mn-link-update">
				<input type="submit" value="<?php esc_attr_e( 'Add Link' ); ?>" class="button button-primary" id="mn-link-submit" name="mn-link-submit">
			</div>
		</div>
		</form>
		</div>
