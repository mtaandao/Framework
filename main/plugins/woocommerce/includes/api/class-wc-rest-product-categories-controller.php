...
class WC_REST_Product_Categories_Controller extends WC_REST_Terms_Controller {
	 ...
	 *
	 * @param MN_Term $item Term object.
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( $image_id = get_woocommerce_term_meta( $item->term_id, 'thumbnail_id' ) ) {
			...
			$data['image'] = array(
				...
				'src'           => mn_get_attachment_url( $image_id ),
				'title'         => get_the_title( $attachment ),
				'alt'           => get_post_meta( $image_id, '_mn_attachment_image_alt', true ),
			);
		}
		...
		if ( ! empty( $request['image'] ) ) {
			...
			if ( $image_id && mn_attachment_is_image( $image_id ) ) {
				...
				if ( ! empty( $request['image']['alt'] ) ) {
					update_post_meta( $image_id, '_mn_attachment_image_alt', wc_clean( $request['image']['alt'] ) );
				}

				// Set the image title.
				if ( ! empty( $request['image']['title'] ) ) {
					mn_update_post( array( 'ID' => $image_id, 'post_title' => wc_clean( $request['image']['title'] ) ) );
				}
			}
		}
		...
		$schema = array(
			...
			'properties'           => array(
				...
				'description' => array(
					...
					'context'     => array( 'view', 'edit' ),
					'arg_options' => array(
						'sanitize_callback' => 'mn_filter_post_kses',
					),
				),
				'display' => array(
