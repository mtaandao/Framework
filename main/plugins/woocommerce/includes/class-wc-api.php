...
class WC_API extends WC_Legacy_API {
	...
	public function handle_api_requests() {
		...
		if ( ! empty( $_GET['wc-api'] ) ) {
			$mn->query_vars['wc-api'] = $_GET['wc-api'];
		}

		// wc-api endpoint requests.
		if ( ! empty( $mn->query_vars['wc-api'] ) ) {
			...
			$api_request = strtolower( wc_clean( $mn->query_vars['wc-api'] ) );
	...
	private function rest_api_init() {
		...
		// REST API was included starting Mtaandao 4.4.
	...
	private function rest_api_includes() {
		...
		// MN-API classes and functions.
		include_once( 'vendor/mn-rest-functions.php' );
		if ( ! class_exists( 'MN_REST_Controller' ) ) {
			include_once( 'vendor/class-mn-rest-controller.php' );
		}
