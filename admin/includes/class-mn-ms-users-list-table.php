<?php
/**
 * List Table API: MN_MS_Users_List_Table class
 *
 * @package Mtaandao
 ...
 * @see MN_List_Table
 */
class MN_MS_Users_List_Table extends MN_List_Table {
	/**
	 *
	 * @return bool
	 */
	...
	public function prepare_items() {
		global $usersearch, $role, $mndb, $mode;
		...
		if ( $role === 'super' ) {
			...
			$args['include'] = $mndb->get_col( "SELECT ID FROM $mndb->users WHERE user_login IN ('$logins')" );
		}
		 ...
		 */
		if ( !$usersearch && mn_is_large_network( 'users' ) ) {
		...
		/** This filter is documented in admin/includes/class-mn-users-list-table.php */
	...
	public function get_columns() {
		 ...
		 */
		return apply_filters( 'mnmu_users_columns', $users_columns );
	}
	 ...
	 * @access public
	 *
	 * @param MN_User $user The current MN_User object.
	 */
	...
	public function column_username( $user ) {
		...
		$edit_link = esc_url( add_query_arg( 'mn_http_referer', urlencode( mn_unslash( $_SERVER['REQUEST_URI'] ) ), get_edit_user_link( $user->ID ) ) );
	...
	protected function handle_row_actions( $user, $column_name, $primary ) {
		...
		if ( current_user_can( 'delete_user', $user->ID ) && ! in_array( $user->user_login, $super_admins ) ) {
			$actions['delete'] = '<a href="' . $delete = esc_url( network_admin_url( add_query_arg( '_mn_http_referer', urlencode( mn_unslash( $_SERVER['REQUEST_URI'] ) ), mn_nonce_url( 'users.php', 'deleteuser' ) . '&amp;action=deleteuser&amp;id=' . $user->ID ) ) ) . '" class="delete">' . __( 'Delete' ) . '</a>';
		}
