 ...
 * @see mn_ajax_nopriv_heartbeat(), mn_ajax_heartbeat()
 *
...
( function( $, window, undefined ) {
	var Heartbeat = function() {
		...
		function initialize() {
			...
			if ( hidden ) {
				...
				$document.on( visibilitychange + '.mn-heartbeat', function() {
			...
			$(window).on( 'unload.mn-heartbeat', function() {
		...
		function userIsActive() {
			...
			$document.off( '.mn-heartbeat-active' );

			$('iframe').each( function( i, frame ) {
				if ( isLocalFrame( frame ) ) {
					$( frame.contentWindow ).off( '.mn-heartbeat-active' );
				}
			});
		...
		function checkUserActivity() {
			...
			if ( ! settings.userActivityEvents ) {
				$document.on( 'mouseover.mn-heartbeat-active keyup.mn-heartbeat-active touchend.mn-heartbeat-active', function() {
					userIsActive();
				});

				$('iframe').each( function( i, frame ) {
					if ( isLocalFrame( frame ) ) {
						$( frame.contentWindow ).on( 'mouseover.mn-heartbeat-active keyup.mn-heartbeat-active touchend.mn-heartbeat-active', function() {
							userIsActive();
						});
					}
				});
		 ...
		 * Use mn.heartbeat.isQueued('handle') to see if any data is already queued for that handle.
		 *
	...
	// Ensure the global `mn` object exists.
	window.mn = window.mn || {};
	window.mn.heartbeat = new Heartbeat();
