...
/**
 * Loads the MN image-editing interface.
 *
 * @param int         $post_id Post ID.
 * @param bool|object $msg     Optional. Message to display for image editor updates or errors.
 *                             Default false.
 */
function mn_image_editor($post_id, $msg = false) {
	$nonce = mn_create_nonce("image_editor-$post_id");
	...
	$backup_sizes = get_post_meta( $post_id, '_mn_attachment_backup_sizes', true );
	$can_restore = false;
	...
	?>
	<div class="imgedit-wrap mn-clearfix">
...
<?php } ?>
	...
	<?php if ( $thumb && $sub_sizes ) {
		$thumb_img = mn_constrain_dimensions( $thumb['width'], $thumb['height'], 160, 120 );
	?>
	...
	<div class="imgedit-panel-content mn-clearfix">
		<?php echo $note; ?>
		<div class="imgedit-menu mn-clearfix">
		...
		if ( mn_image_editor_supports( array( 'mime_type' => get_post_mime_type( $post_id ), 'methods' => array( 'rotate' ) ) ) ) {
			$note_no_rotate = '';
	?>
...
/**
 * Streams image in MN_Image_Editor to browser.
 * Provided for backcompat reasons
 *
 * @param MN_Image_Editor $image
 * @param string $mime_type
 * @param int $post_id
 * @return bool
 */
function mn_stream_image( $image, $mime_type, $post_id ) {
	if ( $image instanceof MN_Image_Editor ) {

		/**
		 * Filters the MN_Image_Editor instance for the image to be streamed to the browser.
		 *
		 * @since 3.5.0
		 *
		 * @param MN_Image_Editor $image   MN_Image_Editor instance.
		 * @param int             $post_id Post ID.
		 */
		$image = apply_filters( 'image_editor_save_pre', $image, $post_id );

		if ( is_mn_error( $image->stream( $mime_type ) ) )
			return false;

		return true;
	} else {
		_deprecated_argument( __FUNCTION__, '3.5.0', __( '$image needs to be an MN_Image_Editor object' ) );
 ...
 * @return bool
 */
function mn_save_image_file( $filename, $image, $mime_type, $post_id ) {
	if ( $image instanceof MN_Image_Editor ) {
		 ...
		 * @param int             $post_id   Post ID.
		 */
		$saved = apply_filters( 'mn_save_image_editor_file', null, $filename, $image, $mime_type, $post_id );
		 ...
		 *
		 * @since 2.9.0
		 * @deprecated 3.5.0 Use mn_save_image_editor_file instead.
		 *
		 * @param mixed           $override  Value to return instead of saving. Default null.
		 * @param string          $filename  Name of the file to be saved.
		 * @param MN_Image_Editor $image     MN_Image_Editor instance.
		 ...
		 * @param int             $post_id   Post ID.
		 */
		$saved = apply_filters( 'mn_save_image_file', null, $filename, $image, $mime_type, $post_id );
		...
		switch ( $mime_type ) {
			case 'image/jpeg':
				...
				/** This filter is documented in res/class-mn-image-editor.php */
...
function _rotate_image_resource($img, $angle) {
	_deprecated_function( __FUNCTION__, '3.5.0', __( 'Use MN_Image_Editor::rotate' ) );
...
function _flip_image_resource($img, $horz, $vert) {
	_deprecated_function( __FUNCTION__, '3.5.0', __( 'Use MN_Image_Editor::flip' ) );
	$w = imagesx($img);
	$h = imagesy($img);
	$dst = mn_imagecreatetruecolor($w, $h);
	if ( is_resource($dst) ) {
		$sx = $vert ? ($w - 1) : 0;
		$sy = $horz ? ($h - 1) : 0;
		$sw = $vert ? -$w : $w;
 ...
 * @return MN_Image_Editor MN_Image_Editor instance with changes applied.
 */
function image_edit_apply_changes( $image, $changes ) {
	...
	if ( $image instanceof MN_Image_Editor ) {
		...
		/**
		 * Filters the MN_Image_Editor instance before applying changes to the image.
		 *
		 * @since 3.5.0
		 *
		 * @param MN_Image_Editor $image   MN_Image_Editor instance.
 		 * @param array           $changes Array of change operations.
		 */
		$image = apply_filters( 'mn_image_editor_before_change', $image, $changes );
		 ...
		 *
		 * @since 2.9.0
		 * @deprecated 3.5.0 Use mn_image_editor_before_change instead.
		 *
...
function stream_preview_image( $post_id ) {
	...
	$img = mn_get_image_editor( _load_image_to_edit_path( $post_id ) );

	if ( is_mn_error( $img ) ) {
		return false;
	}

	$changes = !empty($_REQUEST['history']) ? json_decode( mn_unslash($_REQUEST['history']) ) : null;
	if ( $changes )
	...
	if ( is_mn_error( $img->resize( $w2, $h2 ) ) )
		return false;

	return mn_stream_image( $img, $post->post_mime_type, $post_id );
}
 ...
 */
function mn_restore_image($post_id) {
	$meta = mn_get_attachment_metadata($post_id);
	$file = get_attached_file($post_id);
	$backup_sizes = $old_backup_sizes = get_post_meta( $post_id, '_mn_attachment_backup_sizes', true );
	$restored = false;
	$msg = new stdClass;
	...
	if ( isset($backup_sizes['full-orig']) && is_array($backup_sizes['full-orig']) ) {
		...
		$meta['file'] = _mn_relative_upload_path( $restored_file );
	...
	if ( ! mn_update_attachment_metadata( $post_id, $meta ) ||
		( $old_backup_sizes !== $backup_sizes && ! update_post_meta( $post_id, '_mn_attachment_backup_sizes', $backup_sizes ) ) ) {
 ...
 * @return \stdClass
 */
function mn_save_image( $post_id ) {
	...
	$img = mn_get_image_editor( _load_image_to_edit_path( $post_id, 'full' ) );
	if ( is_mn_error( $img ) ) {
	...
	if ( $scale && $fwidth > 0 && $fheight > 0 ) {
		...
		$changes = json_decode( mn_unslash($_REQUEST['history']) );
		if ( $changes )
			$img = image_edit_apply_changes($img, $changes);
	} else {
	...
	if ( !mn_save_image_file($new_path, $img, $post->post_mime_type, $post_id) ) {
	...
	if ( 'nothumb' == $target || 'all' == $target || 'full' == $target || $scaled ) {
		...
		$meta['file'] = _mn_relative_upload_path( $new_path );
	 ...
	 * https://core.trac.mtaandao.co.ke/ticket/32171
	 */
	...
	if ( isset( $sizes ) ) {
		...
		foreach ( $sizes as $size ) {
			...
			if ( isset( $_mn_additional_image_sizes[ $size ] ) ) {
				$width  = intval( $_mn_additional_image_sizes[ $size ]['width'] );
				$height = intval( $_mn_additional_image_sizes[ $size ]['height'] );
				$crop   = ( $nocrop ) ? false : $_mn_additional_image_sizes[ $size ]['crop'];
			} else {
	...
	if ( $success ) {
		mn_update_attachment_metadata( $post_id, $meta );
		update_post_meta( $post_id, '_mn_attachment_backup_sizes', $backup_sizes);

		if ( $target == 'thumbnail' || $target == 'all' || $target == 'full' ) {
			// Check if it's an image edit from attachment edit screen
			if ( ! empty( $_REQUEST['context'] ) && 'edit-attachment' == $_REQUEST['context'] ) {
				$thumb_url = mn_get_attachment_image_src( $post_id, array( 900, 600 ), true );
