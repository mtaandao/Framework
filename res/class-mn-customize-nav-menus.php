<?php
/**
 * Mtaandao Customize Nav Menus classes
 *
 * @package Mtaandao
 ...
 * @since 4.3.0
 *
 * @see MN_Customize_Manager
 */
final class MN_Customize_Nav_Menus {

	/**
	 * MN_Customize_Manager instance.
	 *
	 * @since 4.3.0
	 * @access public
	 * @var MN_Customize_Manager
	 */
	public $manager;
	 ...
	 * @access public
	 *
	 * @param object $manager An instance of the MN_Customize_Manager class.
	 */
	public function __construct( $manager ) {
		...
		add_action( 'mn_ajax_load-available-menu-items-customizer', array( $this, 'ajax_load_available_items' ) );
		add_action( 'mn_ajax_search-available-menu-items-customizer', array( $this, 'ajax_search_available_items' ) );
	...
	public function filter_nonces( $nonces ) {
		$nonces['customize-menus'] = mn_create_nonce( 'customize-menus' );
		return $nonces;
	}
	...
	public function ajax_load_available_items() {
		...
		if ( empty( $_POST['type'] ) || empty( $_POST['object'] ) ) {
			mn_send_json_error( 'nav_menus_missing_type_or_object_parameter' );
		}
		...
		if ( is_mn_error( $items ) ) {
			mn_send_json_error( $items->get_error_code() );
		} else {
			mn_send_json_success( array( 'items' => $items ) );
		}
	}
	 ...
	 * @return MN_Error|array Returns either a MN_Error object or an array of menu items.
	 */
	public function load_available_items_query( $type = 'post_type', $object = 'page', $page = 0 ) {
		...
		if ( 'post_type' === $type ) {
			...
			if ( ! $post_type ) {
				return new MN_Error( 'nav_menus_invalid_post_type' );
			}
	...
	public function ajax_search_available_items() {
		...
		if ( empty( $_POST['search'] ) ) {
			mn_send_json_error( 'nav_menus_missing_search_parameter' );
		}
		...
		$s = sanitize_text_field( mn_unslash( $_POST['search'] ) );
		$items = $this->search_available_items_query( array( 'pagenum' => $p, 's' => $s ) );

		if ( empty( $items ) ) {
			mn_send_json_error( array( 'message' => __( 'No results found.' ) ) );
		} else {
			mn_send_json_success( array( 'items' => $items ) );
		}
	}
	 ...
	 *
	 * Based on MN_Editor::mn_link_query().
	 *
	 * @since 4.3.0
	 * @access public
	 *
	...
	public function enqueue_scripts() {
		mn_enqueue_style( 'customize-nav-menus' );
		mn_enqueue_script( 'customize-nav-menus' );

		$temp_nav_menu_setting      = new MN_Customize_Nav_Menu_Setting( $this->manager, 'nav_menu[-1]' );
		$temp_nav_menu_item_setting = new MN_Customize_Nav_Menu_Item_Setting( $this->manager, 'nav_menu_item[-1]' );

		// Pass data to JS.
		$settings = array(
			'allMenus'             => mn_get_nav_menus(),
		...
		$data = sprintf( 'var _mnCustomizeNavMenusSettings = %s;', mn_json_encode( $settings ) );
		mn_scripts()->add_data( 'customize-nav-menus', 'data', $data );
		...
		mn_localize_script( 'nav-menu', 'menus', $nav_menus_l10n );
	}
	...
	public function filter_dynamic_setting_args( $setting_args, $setting_id ) {
		if ( preg_match( MN_Customize_Nav_Menu_Setting::ID_PATTERN, $setting_id ) ) {
			$setting_args = array(
				'type'      => MN_Customize_Nav_Menu_Setting::TYPE,
				'transport' => 'postMessage',
			);
		} elseif ( preg_match( MN_Customize_Nav_Menu_Item_Setting::ID_PATTERN, $setting_id ) ) {
			$setting_args = array(
				'type'      => MN_Customize_Nav_Menu_Item_Setting::TYPE,
				'transport' => 'postMessage',
			);
		}
		return $setting_args;
	...
	public function filter_dynamic_setting_class( $setting_class, $setting_id, $setting_args ) {
		...
		if ( ! empty( $setting_args['type'] ) && MN_Customize_Nav_Menu_Setting::TYPE === $setting_args['type'] ) {
			$setting_class = 'MN_Customize_Nav_Menu_Setting';
		} elseif ( ! empty( $setting_args['type'] ) && MN_Customize_Nav_Menu_Item_Setting::TYPE === $setting_args['type'] ) {
			$setting_class = 'MN_Customize_Nav_Menu_Item_Setting';
		}
	...
	public function customize_register() {
		...
		$this->manager->register_panel_type( 'MN_Customize_Nav_Menus_Panel' );
		$this->manager->register_control_type( 'MN_Customize_Nav_Menu_Control' );
		$this->manager->register_control_type( 'MN_Customize_Nav_Menu_Name_Control' );
		$this->manager->register_control_type( 'MN_Customize_Nav_Menu_Auto_Add_Control' );
		$this->manager->register_control_type( 'MN_Customize_Nav_Menu_Item_Control' );
		...
		if ( current_theme_supports( 'widgets' ) ) {
			...
			$description .= '<p>' . sprintf( __( 'Menus can be displayed in locations defined by your theme or in <a href="%s">widget areas</a> by adding a &#8220;Custom Menu&#8221; widget.' ), "javascript:mn.customize.panel( 'widgets' ).focus();" ) . '</p>';
		} else {
			$description .= '<p>' . __( 'Menus can be displayed in locations defined by your theme.' ) . '</p>';
		}
		$this->manager->add_panel( new MN_Customize_Nav_Menus_Panel( $this->manager, 'nav_menus', array(
			'title'       => __( 'Menus' ),
			'description' => $description,
			'priority'    => 100,
			// 'theme_supports' => 'menus|widgets', @todo allow multiple theme supports
		) ) );
		$menus = mn_get_nav_menus();
		...
		if ( current_theme_supports( 'widgets' ) ) {
			/* translators: URL to the widgets panel of the customizer */
			$description .= '<p>' . sprintf( __( 'You can also place menus in <a href="%s">widget areas</a> with the &#8220;Custom Menu&#8221; widget.' ), "javascript:mn.customize.panel( 'widgets' ).focus();" ) . '</p>';
		}
		...
		foreach ( $menus as $menu ) {
			$choices[ $menu->term_id ] = mn_html_excerpt( $menu->name, 40, '&hellip;' );
		}
		...
		foreach ( $locations as $location => $description ) {
			...
			$this->manager->add_control( new MN_Customize_Nav_Menu_Location_Control( $this->manager, $setting_id, array(
				'label'       => $description,
		...
		foreach ( $menus as $menu ) {
			...
			$this->manager->add_section( new MN_Customize_Nav_Menu_Section( $this->manager, $section_id, array(
			...
			$this->manager->add_setting( new MN_Customize_Nav_Menu_Setting( $this->manager, $nav_menu_setting_id, array(
				'transport' => 'postMessage',
			) ) );

			// Add the menu contents.
			$menu_items = (array) mn_get_nav_menu_items( $menu_id );
			...
			foreach ( array_values( $menu_items ) as $i => $item ) {
				...
				$this->manager->add_setting( new MN_Customize_Nav_Menu_Item_Setting( $this->manager, $menu_item_setting_id, array(
					'value'     => $value,
					'transport' => 'postMessage',
				) ) );

				// Create a control for each menu item.
				$this->manager->add_control( new MN_Customize_Nav_Menu_Item_Control( $this->manager, $menu_item_setting_id, array(
					'label'    => $item->title,
					'section'  => $section_id,
					'priority' => 10 + $i,
				) ) );
		...
		$this->manager->add_section( new MN_Customize_New_Menu_Section( $this->manager, 'add_menu', array(
			'title'    => __( 'Add a Menu' ),
			'panel'    => 'nav_menus',
			'priority' => 999,
		) ) );
		...
		$this->manager->add_control( new MN_Customize_New_Menu_Control( $this->manager, 'create_new_menu', array(
			'section'  => 'add_menu',
			'settings' => array(),
		) ) );
	}
	 ...
	 *
	 * Templates are imported into the JS use mn.template.
	 *
	 * @since 4.3.0
	 * @access public
	 */
				...
				<div class="menu-item-bar">
					<div class="menu-item-handle">
						...
						<span class="item-title" aria-hidden="true">
							<span class="menu-item-title<# if ( ! data.title ) { #> no-title<# } #>">{{ data.title || mn.customize.Menus.data.l10n.untitled }}</span>
						</span>
						<button type="button" class="button-link item-add">
							<span class="screen-reader-text"><?php
								/* translators: 1: Title of a menu item, 2: Type of a menu item */
								printf( __( 'Add to menu: %1$s (%2$s)' ), '{{ data.title || mn.customize.Menus.data.l10n.untitled }}', '{{ data.type_label }}' );
							?></span>
						</button>
					</div>
				</div>
		...
		<div id="available-menu-items" class="accordion-container">
			...
			<div id="new-custom-menu-item" class="accordion-section">
				...
				<div class="accordion-section-content customlinkdiv">
					...
					<p id="menu-item-url-wrap" class="mn-clearfix">
					...
					<p id="menu-item-name-wrap" class="mn-clearfix">
	...
	public function customize_preview_init() {
		add_action( 'mn_enqueue_scripts', array( $this, 'customize_preview_enqueue_deps' ) );
		add_filter( 'mn_nav_menu_args', array( $this, 'filter_mn_nav_menu_args' ), 1000 );
		add_filter( 'mn_nav_menu', array( $this, 'filter_mn_nav_menu' ), 10, 2 );
		add_filter( 'mn_footer', array( $this, 'export_preview_data' ), 1 );
		add_filter( 'customize_render_partials_response', array( $this, 'export_partial_rendered_nav_menu_instances' ) );
	}

	/**
	 * Keep track of the arguments that are being passed to mn_nav_menu().
	 *
	 * @since 4.3.0
	 * @access public
	 * @see mn_nav_menu()
	 * @see MN_Customize_Widgets_Partial_Refresh::filter_dynamic_sidebar_params()
	 *
	 * @param array $args An array containing mn_nav_menu() arguments.
	 * @return array Arguments.
	 */
	public function filter_mn_nav_menu_args( $args ) {
		/*
		 * The following conditions determine whether or not this instance of
		 * mn_nav_menu() can use selective refreshed. A mn_nav_menu() can be
		 * selective refreshed if...
		 */
		$can_partial_refresh = (
			// ...if mn_nav_menu() is directly echoing out the menu (and thus isn't manipulating the string after generated),
			! empty( $args['echo'] )
			&&
	...
	/**
	 * Prepares mn_nav_menu() calls for partial refresh.
	 *
	 ...
	 * @param object $args             An object containing mn_nav_menu() arguments.
	 * @return null
	 */
	public function filter_mn_nav_menu( $nav_menu_content, $args ) {
		if ( isset( $args->customize_preview_nav_menus_args['can_partial_refresh'] ) && $args->customize_preview_nav_menus_args['can_partial_refresh'] ) {
			...
			$attributes .= sprintf( ' data-customize-partial-placement-context="%s"', esc_attr( mn_json_encode( $args->customize_preview_nav_menus_args ) ) );
	...
	public function customize_preview_enqueue_deps() {
		mn_enqueue_script( 'customize-preview-nav-menus' ); // Note that we have overridden this.
		mn_enqueue_style( 'customize-preview' );
	}
	...
	public function export_preview_data() {
		...
		// Why not mn_localize_script? Because we're not localizing, and it forces values into strings.
		$exports = array(
			'navMenuInstanceArgs' => $this->preview_nav_menu_instance_args,
		);
		printf( '<script>var _mnCustomizePreviewNavMenusExports = %s;</script>', mn_json_encode( $exports ) );
	}

	/**
	 * Export any mn_nav_menu() calls during the rendering of any partials.
	 *
	 * @since 4.5.0
	 * @access public
	 *
	...
	/**
	 * Render a specific menu via mn_nav_menu() using the supplied arguments.
	 *
	 * @since 4.3.0
	 * @access public
	 *
	 * @see mn_nav_menu()
	 *
	 * @param MN_Customize_Partial $partial       Partial.
