...
/** Sets up the Mtaandao Environment. */
...
if ( is_array( get_site_option( 'illegal_names' )) && isset( $_GET[ 'new' ] ) && in_array( $_GET[ 'new' ], get_site_option( 'illegal_names' ) ) ) {
	mn_redirect( network_home_url() );
	die();
}

/**
 * Prints signup_header via mn_head
 *
 * @since MU
 */
...
add_action( 'mn_head', 'do_signup_header' );

if ( !is_multisite() ) {
	mn_redirect( mn_registration_url() );
	die();
}

if ( !is_main_site() ) {
	mn_redirect( network_site_url( 'register.php' ) );
	die();
}

// Fix for page title
$mn_query->is_404 = false;
 ...
 * @since MU
 */
function mnmu_signup_stylesheet() {
	?>
	<style type="text/css">
...
add_action( 'mn_head', 'mnmu_signup_stylesheet' );
get_header( 'mn-signup' );
...
<div class="mu_register mn-signup-container">
<?php
/**
 ...
 * @param MN_Error|string $errors     A MN_Error object containing existing errors. Defaults to empty string.
 */
function show_blog_form( $blogname = '', $blog_title = '', $errors = '' ) {
	...
	if ( ! empty( $languages ) ) :
		...
		<p>
			...
			<?php
			// Network default.
			$lang = get_site_option( 'MNLANG' );

			if ( isset( $_POST['MNLANG'] ) ) {
				$lang = $_POST['MNLANG'];
			}
	 ...
	 * @since 3.0.0
	 *
	 * @param MN_Error $errors A MN_Error object possibly containing 'blogname' or 'blog_title' errors.
	 */
...
function validate_blog_form() {
	...
	return mnmu_validate_blog_signup($_POST['blogname'], $_POST['blog_title'], $user);
}
...
function show_user_form($user_name = '', $user_email = '', $errors = '') {
	 ...
	 * @since 3.0.0
	 *
	 * @param MN_Error $errors A MN_Error object containing containing 'user_name' or 'user_email' errors.
	 */
...
function validate_user_form() {
	return mnmu_validate_user_signup($_POST['user_name'], $_POST['user_email']);
}
...
function signup_another_blog( $blogname = '', $blog_title = '', $errors = '' ) {
	 ...
	 *     @type MN_Error $errors     A MN_Error object possibly containing 'blogname' or 'blog_title' errors.
	 * }
	 */
	...
	<form id="setupform" method="post" action="register.php">
...
function validate_another_blog_signup() {
	global $mndb, $blogname, $blog_title, $errors, $domain, $path;
	$current_user = mn_get_current_user();
	if ( ! is_user_logged_in() ) {
		die();
	}
	...
	if ( ! empty( $_POST['MNLANG'] ) ) {

		$languages = signup_get_available_languages();

		if ( in_array( $_POST['MNLANG'], $languages ) ) {
			$language = mn_unslash( sanitize_text_field( $_POST['MNLANG'] ) );

			if ( $language ) {
				$blog_meta_defaults['MNLANG'] = $language;
			}
		}
	...
	$blog_id = mnmu_create_blog( $domain, $path, $blog_title, $current_user->ID, $meta, $mndb->siteid );
...
function signup_user( $user_name = '', $user_email = '', $errors = '' ) {
	 ...
	 *     @type MN_Error $errors     A MN_Error object with possible errors relevant to the sign-up user.
	 * }
	 */
	...
	?></h2>
	<form id="setupform" method="post" action="register.php" novalidate="novalidate">
		<input type="hidden" name="stage" value="validate-user-signup" />
		<?php
		/** This action is documented in register.php */
...
function validate_user_signup() {
	...
	mnmu_signup_user( $user_name, $user_email, apply_filters( 'add_signup_meta', array() ) );
...
function signup_blog($user_name = '', $user_email = '', $blogname = '', $blog_title = '', $errors = '') {
	 ...
	 *     @type MN_Error $errors     A MN_Error object with possible errors relevant to new site creation variables.
	 * }
	 */
...
function validate_blog_signup() {
	...
	$user_result = mnmu_validate_user_signup( $_POST['user_name'], $_POST['user_email'] );
	...
	$result = mnmu_validate_blog_signup( $_POST['blogname'], $_POST['blog_title'] );
	$domain = $result['domain'];
	$path = $result['path'];
	$blogname = $result['blogname'];
	$blog_title = $result['blog_title'];
	...
	mnmu_signup_blog($domain, $path, $blog_title, $user_name, $user_email, $meta);
 ...
 */
$active_signup = apply_filters( 'mnmu_active_signup', $active_signup );
...
if ( $active_signup == 'none' ) {
	...
	$login_url = mn_login_url( network_site_url( 'register.php' ) );
...
<?php
/**
 * Fires after the sign-up forms, before mn_footer.
 *
 * @since 3.0.0
 */
do_action( 'after_signup_form' ); ?>

<?php get_footer( 'mn-signup' );
