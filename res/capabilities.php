	...
	case 'delete_page':
		...
		if ( $post->post_author && $user_id == $post->post_author ) {
			...
			if ( in_array( $post->post_status, array( 'publish', 'future' ), true ) ) {
				...
				$status = get_post_meta( $post->ID, '_mn_trash_meta_status', true );
 ...
 *                           "Meta" capabilities, e.g. 'edit_post', 'edit_user', etc., are capabilities used
 *                           by map_meta_cap() to map to other "primitive" capabilities, e.g. 'edit_posts',
 *                           'edit_others_posts', etc. Accessed via func_get_args() and passed to MN_User::has_cap(),
 *                           then map_meta_cap().
...
/**
 * Retrieves the global MN_Roles instance and instantiates it if necessary.
 *
 * @since 4.3.0
 *
 * @global MN_Roles $mn_roles MN_Roles global instance.
 *
 * @return MN_Roles MN_Roles global instance if not already instantiated.
 */
function mn_roles() {
	global $mn_roles;
 ...
 * @return MN_Role|null MN_Role object if found, null if the role does not exist.
 */
function get_role( $role ) {
	return mn_roles()->get_role( $role );
}
 ...
 * @return MN_Role|null MN_Role object if role is added, null if already exists.
 */
function add_role( $role, $display_name, $capabilities = array() ) {
	if ( empty( $role ) ) {
		return;
	}
	return mn_roles()->add_role( $role, $display_name, $capabilities );
}
