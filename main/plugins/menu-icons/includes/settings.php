...
final class Menu_Icons_Settings {
	...
	public static function init() {
		...
		add_action( 'mn_ajax_menu_icons_update_settings', array( __CLASS__, '_ajax_menu_icons_update_settings' ) );
	}


	/**
	 * Prepare admin/nav-menus.php page
	 *
	 * @since   0.3.0
	...
	public static function _maybe_update_settings() {
		if ( ! empty( $_POST['menu-icons']['settings'] ) ) {
			...
			mn_redirect( self::_reset_settings() );
		}
	}
	 ...
	 *
	 * @since   0.7.0
	 * @mn_hook action mn_ajax_menu_icons_update_settings
	 */
	public static function _ajax_menu_icons_update_settings() {
		...
		$redirect_url = self::_update_settings( $_POST['menu-icons']['settings'] ); // Input var okay.
		mn_send_json_success( array( 'redirectUrl' => $redirect_url ) );
	}
	...
	public static function _admin_notices() {
		...
		if ( ! empty( $message_type ) && ! empty( $messages[ $message_type ] ) ) {
			printf(
				...
				mn_kses( $messages[ $message_type ], array( 'strong' => true ) )
			);
		}
	...
	public static function get_fields() {
		$menu_id    = self::get_current_menu_id();
		$icon_types = mn_list_pluck( Menu_Icons::get( 'types' ), 'name' );
		...
		?>
			<div class="taxonomydiv">
				...
				<div id="menu-icons-settings-extensions" class="tabs-panel _extensions">
					...
					<ul>
						<li><a target="_blank" href="http://mtaandao.co.ke/plugins/menu-icons-icomoon/">IcoMoon</a></li>
					</ul>
				</div>
			</div>
			<p class="submitbox button-controls">
				<?php mn_nonce_field( self::UPDATE_KEY, self::UPDATE_KEY ) ?>
	 ...
	 *
	 * @since   0.3.0
	 * @mn_hook action admin_enqueue_scripts
	 */
	public static function _enqueue_assets() {
			...
			array(
				'text'           => array(
					...
					'settingsInfo' => sprintf(
						...
						'<a target="_blank" href="http://mtaandao.co.ke/plugins/simple-custom-css/">Simple Custom CSS</a>'
					),
				),
		...
		mn_localize_script( 'menu-icons', 'menuIcons', $js_data );
	}
}
