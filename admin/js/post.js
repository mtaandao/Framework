/* global postL10n, ajaxurl, mnAjax, setPostThumbnailL10n, postboxes, pagenow, tinymce, alert, deleteUserSetting */
/* global theList:true, theExtraList:true, getUserSetting, setUserSetting, commentReply */

var commentsBox, MNSetThumbnailHTML, MNSetThumbnailID, MNRemoveThumbnail, mntitlehint, makeSlugeditClickable, editPermalink;
	...
	get : function(total, num) {
		...
		$.post(ajaxurl, data,
			function(r) {
				...
				$('#the-comment-list').append('<tr><td colspan="2">'+mnAjax.broken+'</td></tr>');
			}
		);
...
MNSetThumbnailHTML = function(html){
	$('.inside', '#postimagediv').html(html);
};

MNSetThumbnailID = function(id){
...
MNRemoveThumbnail = function(nonce){
	$.post(ajaxurl, {
...
(function($) {
	...
	$(document).on( 'heartbeat-send.mn-refresh-nonces', function( e, data ) {
		var post_id,
			$authCheck = $('#mn-auth-check-wrap');

		if ( check || ( $authCheck.length && ! $authCheck.hasClass( 'hidden' ) ) ) {
			if ( ( post_id = $('#post_ID').val() ) && $('#_mnnonce').val() ) {
				data['mn-refresh-post-nonces'] = {
					post_id: post_id
				};
			}
		}
	}).on( 'heartbeat-tick.mn-refresh-nonces', function( e, data ) {
		var nonces = data['mn-refresh-post-nonces'];
...
jQuery(document).ready( function($) {
	...
	$('#post-lock-dialog .notification-dialog').on( 'keydown', function(e) {
		...
		if ( target.hasClass('mn-tab-first') && e.shiftKey ) {
			$(this).find('.mn-tab-last').focus();
			e.preventDefault();
		} else if ( target.hasClass('mn-tab-last') && ! e.shiftKey ) {
			$(this).find('.mn-tab-first').focus();
			e.preventDefault();
		}
	}).filter(':visible').find('.mn-tab-first').focus();

	// Set the heartbeat interval to 15 sec. if post lock dialogs are enabled
	if ( mn.heartbeat && $('#post-lock-dialog').length ) {
		mn.heartbeat.interval( 15 );
	}
	...
	$('#post-preview').on( 'click.post-preview', function( event ) {
		var $this = $(this),
			$form = $('form#post'),
			$previewField = $('input#mn-preview'),
			target = $this.attr('target') || 'mn-preview',
		...
		if ( mn.autosave ) {
			mn.autosave.server.tempBlockSave();
		}
	...
	$document.on( 'autosave-disable-buttons.edit-post', function() {
		...
		if ( ! mn.heartbeat || ! mn.heartbeat.hasConnectionError() ) {
	...
	$(window).on( 'beforeunload.edit-post', function() {
		...
		if ( ( editor && ! editor.isHidden() && editor.isDirty() ) ||
			( mn.autosave && mn.autosave.server.postChanged() ) ) {
		...
		$.ajax({
			...
			async: false,
			data: {
				action: 'mn-remove-post-lock',
				_mnnonce: $('#_mnnonce').val(),
				post_ID: $('#post_ID').val(),
	...
	$('.categorydiv').each( function(){
		...
		// TODO: move to jQuery 1.3+, support for multiple hierarchical taxonomies, see mn-lists.js
		...
		$('#' + taxonomy + '-add-toggle').click( function( e ) {
			e.preventDefault();
			$('#' + taxonomy + '-adder').toggleClass( 'mn-hidden-children' );
	...
	if ( $('#postcustom').length ) {
		$( '#the-list' ).mnList( { addAfter: function() {
	...
	function editPermalink() {
		...
		buttons.children( '.save' ).click( function() {
			...
			$.post(ajaxurl, {
				...
				$( '.edit-slug' ).focus();
				mn.a11y.speak( postL10n.permalinkSaved );
			});
		});
	...
	( function() {
		...
		function endDrag() {
			...
			if ( mce ) {
				editor.focus();
				toolbarHeight = parseInt( $( '#mn-content-editor-container .mce-toolbar-grp' ).height(), 10 );
		...
		$handle.on( 'mousedown.mn-editor-resize', function( event ) {
			...
			$document.on( 'mousemove.mn-editor-resize', dragging )
				.on( 'mouseup.mn-editor-resize mouseleave.mn-editor-resize', endDrag );

			event.preventDefault();
		}).on( 'mouseup.mn-editor-resize', endDrag );
	})();
	...
	$textarea.on( 'keydown.mn-autosave', function( event ) {
		if ( event.which === 83 ) {
			if ( event.shiftKey || event.altKey || ( isMac && ( ! event.metaKey || event.ctrlKey ) ) || ( ! isMac && ! event.ctrlKey ) ) {
				return;
			}

			mn.autosave && mn.autosave.server.triggerSave();
			event.preventDefault();
		}
	});
	...
	if ( $( '#original_post_status' ).val() === 'auto-draft' && window.history.replaceState ) {
		...
		$( '#publish' ).on( 'click', function() {
			...
			location += 'mn-post-new-reload=true';
...
( function( $, counter ) {
	$( function() {
		var $content = $( '#content' ),
			$count = $( '#mn-word-count' ).find( '.word-count' ),
			prevCount = 0,
			contentEditor;
...
} )( jQuery, new mn.utils.WordCounter() );
