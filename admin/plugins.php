...
if ( ! current_user_can('activate_plugins') )
	mn_die( __( 'Sorry, you are not allowed to manage plugins for this site.' ) );
...
$s = isset($_REQUEST['s']) ? urlencode( mn_unslash( $_REQUEST['s'] ) ) : '';
...
if ( $action ) {
	...
	switch ( $action ) {
		case 'activate':
			if ( ! current_user_can('activate_plugins') )
				mn_die(__('Sorry, you are not allowed to activate plugins for this site.'));

			if ( is_multisite() && ! is_network_admin() && is_network_only_plugin( $plugin ) ) {
				mn_redirect( self_admin_url("plugins.php?plugin_status=$status&paged=$page&s=$s") );
				exit;
			}
			...
			if ( is_mn_error( $result ) ) {
				if ( 'unexpected_output' == $result->get_error_code() ) {
					...
					mn_redirect(add_query_arg('_error_nonce', mn_create_nonce('plugin-activation-error_' . $plugin), $redirect));
					exit;
				} else {
					mn_die($result);
				}
			...
			if ( isset($_GET['from']) && 'import' == $_GET['from'] ) {
				mn_redirect( self_admin_url("import.php?import=" . str_replace('-importer', '', dirname($plugin))) ); // overrides the ?error=true one above and redirects to the Imports page, stripping the -importer suffix
			} else {
				mn_redirect( self_admin_url("plugins.php?activate=true&plugin_status=$status&paged=$page&s=$s") ); // overrides the ?error=true one above
			}
			exit;
		...
		case 'activate-selected':
			...
			mn_redirect( self_admin_url("plugins.php?activate-multi=true&plugin_status=$status&paged=$page&s=$s") );
			exit;
		...
		case 'deactivate':
			if ( ! current_user_can('activate_plugins') )
				mn_die(__('Sorry, you are not allowed to deactivate plugins for this site.'));
			...
			if ( headers_sent() )
				...
				mn_redirect( self_admin_url("plugins.php?deactivate=true&plugin_status=$status&paged=$page&s=$s") );
			exit;
		...
		case 'deactivate-selected':
			...
			mn_redirect( self_admin_url("plugins.php?deactivate-multi=true&plugin_status=$status&paged=$page&s=$s") );
			exit;

		case 'delete-selected':
			if ( ! current_user_can('delete_plugins') ) {
				mn_die(__('Sorry, you are not allowed to delete plugins for this site.'));
			}
			...
			if ( empty( $plugins ) ) {
				mn_redirect( self_admin_url( "plugins.php?error=true&main=true&plugin_status=$status&paged=$page&s=$s" ) );
				exit;
			}
			...
			mn_redirect( self_admin_url("plugins.php?deleted=$plugins_to_delete&plugin_status=$status&paged=$page&s=$s") );
			exit;
...
'title'		=> __('Overview'),
'content'	=>
	'<p>' . __('Plugins extend and expand the functionality of Mtaandao. Once a plugin is installed, you may activate it or deactivate it here.') . '</p>' .
...
<?php if ( isset($_GET['error']) ) :
	...
	<?php
		if ( ! isset( $_GET['main'] ) && ! isset( $_GET['charsout'] ) && mn_verify_nonce( $_GET['_error_nonce'], 'plugin-activation-error_' . $plugin ) ) {
			$iframe_url = add_query_arg( array(
				'action'   => 'error_scrape',
				'plugin'   => urlencode( $plugin ),
				'_mnnonce' => urlencode( $_GET['_error_nonce'] ),
...
<?php $mn_list_table->search_box( __( 'Search Installed Plugins' ), 'plugin' ); ?>
</form>
