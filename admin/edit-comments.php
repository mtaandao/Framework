...
if ( $doaction ) {
	...
	if ( 'delete_all' == $doaction && !empty( $_REQUEST['pagegen_timestamp'] ) ) {
		$comment_status = mn_unslash( $_REQUEST['comment_status'] );
		$delete_time = mn_unslash( $_REQUEST['pagegen_timestamp'] );
		$comment_ids = $mndb->get_col( $mndb->prepare( "SELECT comment_ID FROM $mndb->comments WHERE comment_approved = %s AND %s > comment_date_gmt", $comment_status, $delete_time ) );
		$doaction = 'delete';
	...
	$redirect_to = remove_query_arg( array( 'trashed', 'untrashed', 'deleted', 'spammed', 'unspammed', 'approved', 'unapproved', 'ids' ), mn_get_referer() );
	...
	foreach ( $comment_ids as $comment_id ) { // Check the permissions on each
		...
		switch ( $doaction ) {
			case 'approve' :
				mn_set_comment_status( $comment_id, 'approve' );
				$approved++;
				break;
			case 'unapprove' :
				mn_set_comment_status( $comment_id, 'hold' );
				$unapproved++;
				break;
			case 'spam' :
				mn_spam_comment( $comment_id );
				$spammed++;
				break;
			case 'unspam' :
				mn_unspam_comment( $comment_id );
				$unspammed++;
				break;
			case 'trash' :
				mn_trash_comment( $comment_id );
				$trashed++;
				break;
			case 'untrash' :
				mn_untrash_comment( $comment_id );
				$untrashed++;
				break;
			case 'delete' :
				mn_delete_comment( $comment_id );
				$deleted++;
				break;
		}
	}
...
if ( $post_id ) {
	$comments_count = mn_count_comments( $post_id );
	$draft_or_post_title = mn_html_excerpt( _draft_or_post_title( $post_id ), 50, '&hellip;' );
	...
	$comments_count = mn_count_comments();
	if ( $comments_count->moderated > 0 ) {
...
get_current_screen()->set_help_sidebar(
	...
	'<p>' . __( '<a href="https://mtaandao.co.ke/docs/Administration_Screens#Comments" target="_blank">Documentation on Comments</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://mtaandao.co.ke/docs/Comment_Spam" target="_blank">Documentation on Comment Spam</a>' ) . '</p>' .
	'<p>' . __( '<a href="https://mtaandao.co.ke/docs/Keyboard_Shortcuts" target="_blank">Documentation on Keyboard Shortcuts</a>' ) . '</p>' .
...
if ( $post_id ) {
	...
	printf( __( 'Comments on &#8220;%s&#8221;' ),
		sprintf( '<a href="%1$s">%2$s</a>',
			...
			mn_html_excerpt( _draft_or_post_title( $post_id ), 50, '&hellip;' )
		)
	);
} else {
	_e( 'Comments' );
...
if ( isset($_REQUEST['s']) && strlen( $_REQUEST['s'] ) ) {
	...
	printf( __( 'Search results for &#8220;%s&#8221;' ),
		mn_html_excerpt( esc_html( mn_unslash( $_REQUEST['s'] ) ), 50, '&hellip;' )
	);
	echo '</span>';
}
?></h1>
...
if ( isset($_REQUEST['approved']) || isset($_REQUEST['deleted']) || isset($_REQUEST['trashed']) || isset($_REQUEST['untrashed']) || isset($_REQUEST['spammed']) || isset($_REQUEST['unspammed']) || isset($_REQUEST['same']) ) {
	...
	if ( $approved > 0 || $deleted > 0 || $trashed > 0 || $untrashed > 0 || $spammed > 0 || $unspammed > 0 || $same > 0 ) {
		...
		if ( $spammed > 0 ) {
			...
			$messages[] = sprintf( _n( '%s comment marked as spam.', '%s comments marked as spam.', $spammed ), $spammed ) . ' <a href="' . esc_url( mn_nonce_url( "edit-comments.php?doaction=undo&action=unspam&ids=$ids", "bulk-comments" ) ) . '">' . __('Undo') . '</a><br />';
		}
		...
		if ( $trashed > 0 ) {
			...
			$messages[] = sprintf( _n( '%s comment moved to the Trash.', '%s comments moved to the Trash.', $trashed ), $trashed ) . ' <a href="' . esc_url( mn_nonce_url( "edit-comments.php?doaction=undo&action=untrash&ids=$ids", "bulk-comments" ) ) . '">' . __('Undo') . '</a><br />';
		}
...
<?php $mn_list_table->search_box( __( 'Search Comments' ), 'comment' ); ?>
...
<input type="hidden" name="_total" value="<?php echo esc_attr( $mn_list_table->get_pagination_arg('total_items') ); ?>" />
<input type="hidden" name="_per_page" value="<?php echo esc_attr( $mn_list_table->get_pagination_arg('per_page') ); ?>" />
<input type="hidden" name="_page" value="<?php echo esc_attr( $mn_list_table->get_pagination_arg('page') ); ?>" />
...
<?php
mn_comment_reply('-1', true, 'detail');
