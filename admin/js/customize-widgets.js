/* global _mnCustomizeWidgetsSettings */
(function( mn, $ ){
	...
	// Link settings
	api.Widgets.data = _mnCustomizeWidgetsSettings || {};
	l10n = api.Widgets.data.l10n;
	delete api.Widgets.data.l10n;

	/**
	 * mn.customize.Widgets.WidgetModel
	 *
	...
	/**
	 * mn.customize.Widgets.WidgetCollection
	 *
	...
	/**
	 * mn.customize.Widgets.SidebarModel
	 *
	...
	/**
	 * mn.customize.Widgets.SidebarCollection
	 *
	...
	/**
	 * mn.customize.Widgets.AvailableWidgetsPanelView
	 *
	 ...
	 */
	api.Widgets.AvailableWidgetsPanelView = mn.Backbone.View.extend({
	...
	/**
	 * mn.customize.Widgets.WidgetControl
	 *
	 * Customizer control for widgets.
	 * Note that 'widget_form' must match the MN_Widget_Form_Customize_Control::$type
	 *
	 * @constructor
	...
	api.Widgets.WidgetControl = api.Control.extend({
		...
		_setupWideWidget: function() {
			...
			$customizeSidebar = $( '.mn-full-overlay-sidebar-content:first' );
		...
		_setupReorderUI: function() {
			...
			$reorderNav.find( '.move-widget, .move-widget-down, .move-widget-up' ).each( function() {
				...
				if ( $( this ).is( '.move-widget' ) ) {
					...
					if ( isMoveUp ) {
						self.moveUp();
						mn.a11y.speak( l10n.widgetMovedUp );
					} else {
						self.moveDown();
						mn.a11y.speak( l10n.widgetMovedDown );
					}
		...
		_setupRemoveUI: function() {
			...
			replaceDeleteWithRemove = function() {
				$removeBtn.text( l10n.removeBtnLabel ); // mn_widget_control() outputs the link as "Delete"
		...
		/**
		 * @return {mn.customize.controlConstructor.sidebar_widgets[]}
		 */
		...
		updateWidget: function( args ) {
			...
			jqxhr = $.post( mn.ajax.settings.url, data );
	...
	/**
	 * mn.customize.Widgets.WidgetsPanel
	 *
	...
	/**
	 * mn.customize.Widgets.SidebarSection
	 *
	...
	/**
	 * mn.customize.Widgets.SidebarControl
	 *
	 * Customizer control for widgets.
	 * Note that 'sidebar_widgets' must match the MN_Widget_Area_Customize_Control::$type
	 *
	 * @since 3.9.0
	 *
	 * @constructor
	...
	api.Widgets.SidebarControl = api.Control.extend({
				...
				_( removedWidgetIds ).each( function( removedWidgetId ) {
					...
					setTimeout( function() {
						...
						api.each( function( otherSetting ) {
							if ( otherSetting.id === self.setting.id || 0 !== otherSetting.id.indexOf( 'sidebars_widgets[' ) || otherSetting.id === 'sidebars_widgets[mn_inactive_widgets]' ) {
								return;
							}
						...
						if ( api.Widgets.savedWidgetIds[removedWidgetId] ) {
							inactiveWidgets = api.value( 'sidebars_widgets[mn_inactive_widgets]' )().slice();
							inactiveWidgets.push( removedWidgetId );
							api.value( 'sidebars_widgets[mn_inactive_widgets]' )( _( inactiveWidgets ).unique() );
						}
		...
		toggleReordering: function( showOrHide ) {
			...
			if ( showOrHide ) {
				...
				mn.a11y.speak( l10n.reorderModeOn );
				...
				mn.a11y.speak( l10n.reorderModeOff );
		 ...
		 *
		 * @since 3.9.0
		 * @return {mn.customize.controlConstructor.widget_form[]}
		 */
	...
	api.Widgets.getSidebarWidgetControlContainingWidget = function( widgetId ) {
		...
		// @todo this can use widgetIdToSettingId(), then pass into mn.customize.control( x ).getSidebarWidgetsControl()
	 ...
	 *
	 * @param {mn.customize.Section|mn.customize.Panel|mn.customize.Control} focusConstruct - The object to initially focus.
	 * @param {mn.customize.Section|mn.customize.Panel|mn.customize.Control} returnConstruct - The object to return focus.
	 */
