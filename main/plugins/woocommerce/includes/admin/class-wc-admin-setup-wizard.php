...
class WC_Admin_Setup_Wizard {
	...
	private $tweets = array(
		'Someone give me woo-t, I just set up a new store with #Mtaandao and @WooCommerce!',
		'Someone give me high five, I just set up a new store with #Mtaandao and @WooCommerce!'
	);
	...
	public function setup_wizard() {
		...
		mn_register_script( 'select2', WC()->plugin_url() . '/assets/js/select2/select2' . $suffix . '.js', array( 'jquery' ), '3.5.2' );
		...
		mn_enqueue_style( 'woocommerce_admin_styles', WC()->plugin_url() . '/assets/css/admin.css', array(), WC_VERSION );
		mn_enqueue_style( 'wc-setup', WC()->plugin_url() . '/assets/css/wc-setup.css', array( 'dashicons', 'install' ), WC_VERSION );

		mn_register_script( 'wc-setup', WC()->plugin_url() . '/assets/js/admin/wc-setup.min.js', array( 'jquery', 'wc-enhanced-select', 'jquery-blockui' ), WC_VERSION );
		mn_localize_script( 'wc-setup', 'wc_setup_params', array(
	...
	public function setup_wizard_header() {
		...
		<head>
			...
			<?php mn_print_scripts( 'wc-setup' ); ?>
			<?php do_action( 'admin_print_styles' ); ?>
			<?php do_action( 'admin_head' ); ?>
		</head>
		<body class="wc-setup mn-core-ui">
	...
	public function setup_wizard_footer() {
		?>
			<?php if ( 'next_steps' === $this->step ) : ?>
				<a class="wc-return-to-dashboard" href="<?php echo esc_url( admin_url() ); ?>"><?php _e( 'Return to the Mtaandao Dashboard', 'woocommerce' ); ?></a>
			<?php endif; ?>
			</body>
		</html>
		<?php
	...
	public function wc_setup_introduction() {
		...
		<p><?php _e( 'No time right now? If you don’t want to go through the wizard, you can skip and return to the Mtaandao dashboard. Come back anytime if you change your mind!', 'woocommerce' ); ?></p>
	...
	public function wc_setup_pages_save() {
		...
		mn_redirect( esc_url_raw( $this->get_next_step_link() ) );
		exit;
	}
	...
	protected function get_wizard_payment_gateways() {
		$gateways = array(
			'paypal-braintree' => array(
				...
				'description' => sprintf( __( 'Safe and secure payments using credit cards or your customer\'s PayPal account. %sLearn more about PayPal%s.', 'woocommerce' ), '<a href="https://mtaandao.co.ke/plugins/woocommerce-gateway-paypal-powered-by-braintree/" target="_blank">', '</a>' ),
			...
			'paypal-ec' => array(
				...
				'description' => sprintf( __( 'Safe and secure payments using credit cards or your customer\'s PayPal account. %sLearn more about PayPal%s.', 'woocommerce' ), '<a href="https://mtaandao.co.ke/plugins/woocommerce-gateway-paypal-express-checkout/" target="_blank">', '</a>' ),
				'class'       => 'featured featured-row-last',
			...
			'stripe' => array(
				...
				'description' => sprintf( __( 'A modern and robust way to accept credit card payments on your store. %sLearn more about Stripe%s.', 'woocommerce' ), '<a href="https://mtaandao.co.ke/plugins/woocommerce-gateway-stripe/" target="_blank">', '</a>' ),
				...
				<?php foreach ( $gateways as $gateway_id => $gateway ) : ?>
					<li class="wc-wizard-gateway wc-wizard-gateway-<?php echo esc_attr( $gateway_id ); ?> <?php echo esc_attr( $gateway['class'] ); ?>">
						...
						<div class="wc-wizard-gateway-description">
							<?php echo mn_kses_post( mnautop( $gateway['description'] ) ); ?>
						</div>
	...
	public function wc_setup_payments_save() {
		...
		foreach ( $gateways as $gateway_id => $gateway ) {
			...
			if ( ! empty( $gateway['repo-slug'] ) && ! empty( $_POST[ 'wc-wizard-gateway-' . $gateway_id . '-enabled' ] ) ) {
				mn_schedule_single_event( time() + 10, 'woocommerce_plugin_background_installer', array( $gateway_id, $gateway ) );
			}
	...
	private function wc_setup_ready_actions() {
		...
		if ( isset( $_GET['wc_tracker_optin'] ) && isset( $_GET['wc_tracker_nonce'] ) && mn_verify_nonce( $_GET['wc_tracker_nonce'], 'wc_tracker_optin' ) ) {
			update_option( 'woocommerce_allow_tracking', 'yes' );
			WC_Tracker::send_tracking_data( true );

		} elseif ( isset( $_GET['wc_tracker_optout'] ) && isset( $_GET['wc_tracker_nonce'] ) && mn_verify_nonce( $_GET['wc_tracker_nonce'], 'wc_tracker_optout' ) ) {
	...
	public function wc_setup_ready() {
		...
		<?php if ( 'unknown' === get_option( 'woocommerce_allow_tracking', 'unknown' ) ) : ?>
			<div class="woocommerce-message woocommerce-tracker">
				...
				<p class="submit">
					<a class="button-primary button button-large" href="<?php echo esc_url( mn_nonce_url( add_query_arg( 'wc_tracker_optin', 'true' ), 'wc_tracker_optin', 'wc_tracker_nonce' ) ); ?>"><?php _e( 'Allow', 'woocommerce' ); ?></a>
					<a class="button-secondary button button-large skip"  href="<?php echo esc_url( mn_nonce_url( add_query_arg( 'wc_tracker_optout', 'true' ), 'wc_tracker_optout', 'wc_tracker_nonce' ) ); ?>"><?php _e( 'No thanks', 'woocommerce' ); ?></a>
				</p>
			</div>
		<?php endif; ?>
