...
class WC_Admin_API_Keys {
	...
	private function revoke_key() {
		if ( empty( $_REQUEST['_mnnonce'] ) || ! mn_verify_nonce( $_REQUEST['_mnnonce'], 'revoke' ) ) {
		...
		mn_redirect( esc_url_raw( add_query_arg( array( 'revoked' => 1 ), admin_url( 'admin.php?page=wc-settings&tab=api&section=keys' ) ) ) );
		exit();
	}
	...
	private function remove_key( $key_id ) {
		...
		$delete = $mndb->delete( $mndb->prefix . 'woocommerce_api_keys', array( 'key_id' => $key_id ), array( '%d' ) );
