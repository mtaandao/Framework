...
class Vc_Base {
	...
	public function initAdmin() {
		...
		add_action( 'mn_ajax_wpb_single_image_src', array(
			$this,
			'singleImageSrc',
		) ); // @todo move it
		add_action( 'mn_ajax_wpb_gallery_html', array(
			$this,
			'galleryHTML',
		) ); // @todo move it
	...
	public function singleImageSrc() {
		...
		vc_user_access()->checkAdminNonce()->validateDie()->mnAny( 'edit_posts', 'edit_pages' )->validateDie();
		...
		switch ( $source ) {
			...
			case 'featured_image':
				...
				if ( $img_id ) {
					$img = mn_get_attachment_image_src( $img_id, $img_size );
					if ( $img ) {
						$img = $img[0];
					}
				}
	...
	/**
	 * Hooked class method by mn_head MN action to output post custom css.
	 *
	...
	/**
	 * Hooked class method by mn_footer MN action to output shortcodes css editor settings from page meta data.
	 *
	 ...
	 *
	 * Calls mn_register_style for required css libraries files.
	 *
	 * @since  3.1
	 * @access public
	 */
	public function frontCss() {
		mn_register_style( 'flexslider', vc_asset_url( 'lib/bower/flexslider/flexslider.min.css' ), array(), WPB_VC_VERSION );
		mn_register_style( 'nivo-slider-css', vc_asset_url( 'lib/bower/nivoslider/nivo-slider.min.css' ), array(), WPB_VC_VERSION );
		mn_register_style( 'nivo-slider-theme', vc_asset_url( 'lib/bower/nivoslider/themes/default/default.min.css' ), array( 'nivo-slider-css' ), WPB_VC_VERSION );
		mn_register_style( 'prettyphoto', vc_asset_url( 'lib/prettyphoto/css/prettyPhoto.min.css' ), array(), WPB_VC_VERSION );
		mn_register_style( 'isotope-css', vc_asset_url( 'css/lib/isotope.min.css' ), array(), WPB_VC_VERSION );
		mn_register_style( 'font-awesome', vc_asset_url( 'lib/bower/font-awesome/css/font-awesome.min.css' ), array(), WPB_VC_VERSION );
		mn_register_style( 'animate-css', vc_asset_url( 'lib/bower/animate-css/animate.min.css' ), array(), WPB_VC_VERSION, false );
		...
		mn_register_style( 'js_composer_front', $front_css_file, array(), WPB_VC_VERSION );
		...
		if ( is_file( $upload_dir['basedir'] . '/' . $vc_upload_dir . '/custom.css' ) && filesize( $custom_css_path ) > 0 ) {
			...
			mn_register_style( 'js_composer_custom_css', $custom_css_url, array(), WPB_VC_VERSION );
		}
		add_action( 'mn_enqueue_scripts', array(
			$this,
			'enqueueStyle',
		) );
	 ...
	 *
	 * Calls mn_register_script for required css libraries files.
	 *
	 * @since  3.1
	 * @access public
	 */
	public function frontJsRegister() {
		mn_register_script( 'prettyphoto', vc_asset_url( 'lib/prettyphoto/js/jquery.prettyPhoto.min.js' ), array( 'jquery' ), WPB_VC_VERSION, true );
		mn_register_script( 'waypoints', vc_asset_url( 'lib/waypoints/waypoints.min.js' ), array( 'jquery' ), WPB_VC_VERSION, true );

		// @deprecated used in old tabs
		mn_register_script( 'jquery_ui_tabs_rotate', vc_asset_url( 'lib/bower/jquery-ui-tabs-rotate/jquery-ui-tabs-rotate.min.js' ), array(
			'jquery',
			'jquery-ui-tabs',
		), WPB_VC_VERSION, true );

		// used in vc_gallery, old grid
		mn_register_script( 'isotope', vc_asset_url( 'lib/bower/isotope/dist/isotope.pkgd.min.js' ), array( 'jquery' ), WPB_VC_VERSION, true );

		mn_register_script( 'twbs-pagination', vc_asset_url( 'lib/bower/twbs-pagination/jquery.twbsPagination.min.js' ), array( 'jquery' ), WPB_VC_VERSION, true );
		mn_register_script( 'nivo-slider', vc_asset_url( 'lib/bower/nivoslider/jquery.nivo.slider.pack.js' ), array( 'jquery' ), WPB_VC_VERSION, true );
		mn_register_script( 'flexslider', vc_asset_url( 'lib/bower/flexslider/jquery.flexslider-min.js' ), array( 'jquery' ), WPB_VC_VERSION, true );
		mn_register_script( 'wpb_composer_front_js', vc_asset_url( 'js/dist/js_composer_front.min.js' ), array( 'jquery' ), WPB_VC_VERSION, true );
	 ...
	 *
	 * Calls mn_register_script for required css libraries files for Admin dashboard.
	 *
	 * @since  3.1
	 ...
	 *
	 * Calls mn_register_style for required css libraries files for admin dashboard.
	 *
	 * @since  3.1
	 * @access public
	 */
	...
	/**
	 * Hooked class method by mn_head MN action.
	 * @since  4.2
	 * @access public
	 */
	public function addMetaData() {
		echo '<meta name="generator" content="Powered by Visual Composer - drag and drop page builder for Mtaandao."/>' . "\n";
	}

	/**
	 * Also add fix for IE8 bootstrap styles from MNExplorer
	 * @since  4.9
	 * @access public
	 */
	 ...
	 *
	 * Hooked class method by body_class MN filter. Method adds custom css class to body tag of the page to help
	 ...
	 *
	 * Hooked class method by the_excerpt MN filter. When user creates content with VC all content is always wrapped by
