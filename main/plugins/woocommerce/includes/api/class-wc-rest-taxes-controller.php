...
class WC_REST_Taxes_Controller extends WC_REST_Controller {
	...
	public function get_items( $request ) {
		...
		/**
		 * Filter arguments, before passing to $mndb->get_results(), when querying taxes via the REST API.
		 *
		 * @param array           $prepared_args Array of arguments for $mndb->get_results().
		...
		// Query taxes.
		$results = $mndb->get_results( $query . $order_by . $pagination );
		...
		// Query only for ids.
		$mndb->get_results( str_replace( 'SELECT *', 'SELECT tax_rate_id', $query ) );

		// Calcule totals.
		$total_taxes = (int) $mndb->num_rows;
		$response->header( 'X-MN-Total', (int) $total_taxes );
	...
	public function create_item( $request ) {
		if ( ! empty( $request['id'] ) ) {
			return new MN_Error( 'woocommerce_rest_tax_exists', __( 'Cannot create existing resource.', 'woocommerce' ), array( 'status' => 400 ) );
		}
	...
	public function delete_item( $request ) {
		...
		if ( ! $force ) {
			return new MN_Error( 'woocommerce_rest_trash_not_supported', __( 'Taxes do not support trashing.', 'woocommerce' ), array( 'status' => 501 ) );
		}
	...
	public function prepare_item_for_response( $tax, $request ) {
		...
		$locales = $mndb->get_results( $mndb->prepare( "
			SELECT location_code, location_type
			FROM {$mndb->prefix}woocommerce_tax_rate_locations
			WHERE tax_rate_id = %d
		", $id ) );

		if ( ! is_mn_error( $tax ) && ! is_null( $tax ) ) {
