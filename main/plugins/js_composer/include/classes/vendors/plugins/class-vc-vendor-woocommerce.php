...
class Vc_Vendor_Woocommerce implements Vc_Vendor_Interface {
	...
	public function enqueueJsBackend() {
		mn_enqueue_script( 'vc_vendor_woocommerce_backend', vc_asset_url( 'js/vendors/woocommerce.js' ), array( 'vc-backend-min-js' ), '1.0', true );
	}
	...
	public function enqueueJsFrontend() {
		mn_enqueue_script( 'vc_vendor_woocommerce_frontend', vc_asset_url( 'js/vendors/woocommerce.js' ), array( 'vc-frontend-editor-min-js' ), '1.0', true );
	}
	...
	public function addShortcodeSettings( $tag ) {
		...
		switch ( $tag ) {
			...
			case 'recent_products':
				 ...
				 * To learn more about the default ‘orderby’ parameters please reference the Mtaandao Codex: http://codex.wordpress.org/Class_Reference/MN_Query
				 *
				...
				$settings = array(
					...
					'params' => array(
						...
						array(
							...
							'save_always' => true,
							'description' => sprintf( __( 'Select how to sort retrieved products. More at %s.', 'js_composer' ), '<a href="http://codex.wordpress.org/Class_Reference/MN_Query#Order_.26_Orderby_Parameters" target="_blank">Mtaandao codex page</a>' ),
						),
						array(
							'type' => 'dropdown',
							...
							'save_always' => true,
							'description' => sprintf( __( 'Designates the ascending or descending order. More at %s.', 'js_composer' ), '<a href="http://codex.wordpress.org/Class_Reference/MN_Query#Order_.26_Orderby_Parameters" target="_blank">Mtaandao codex page</a>' ),
						),
					),
				);
				break;
				...
				$settings = array(
					...
					'params' => array(
						...
						array(
							...
							'std' => 'title',
							'save_always' => true,
							'description' => sprintf( __( 'Select how to sort retrieved products. More at %s. Default by Title', 'js_composer' ), '<a href="http://codex.wordpress.org/Class_Reference/MN_Query#Order_.26_Orderby_Parameters" target="_blank">Mtaandao codex page</a>' ),
						),
						array(
							'type' => 'dropdown',
							'heading' => __( 'Sort order', 'js_composer' ),
							...
							'value' => $order_way_values,
							'save_always' => true,
							'description' => sprintf( __( 'Designates the ascending or descending order. More at %s. Default by ASC', 'js_composer' ), '<a href="http://codex.wordpress.org/Class_Reference/MN_Query#Order_.26_Orderby_Parameters" target="_blank">Mtaandao codex page</a>' ),
						),
						array(
							'type' => 'autocomplete',
	...
	public function mapShortcodes() {
		add_action( 'mn_ajax_vc_woocommerce_get_attribute_terms', array(
			&$this,
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		$post_meta_infos = $mndb->get_results( $mndb->prepare( "SELECT a.ID AS id, a.post_title AS title, b.meta_value AS sku
					FROM {$mndb->posts} AS a
					LEFT JOIN ( SELECT meta_value, post_id  FROM {$mndb->postmeta} WHERE `meta_key` = '_sku' ) AS b ON b.post_id = a.ID
	...
	public function productIdAutocompleteSuggesterExactSku( $query ) {
		global $mndb;
		$query = trim( $query );
		$product_id = $mndb->get_var( $mndb->prepare( "SELECT post_id FROM {$mndb->postmeta} WHERE meta_key='_sku' AND meta_value='%s' LIMIT 1", stripslashes( $query ) ) );
	...
	public function productCategoryCategoryAutocompleteSuggester( $query, $slug = false ) {
		...
		$cat_id = (int) $query;
		$query = trim( $query );
		$post_meta_infos = $mndb->get_results( $mndb->prepare( "SELECT a.term_id AS id, b.name as name, b.slug AS slug
						FROM {$mndb->term_taxonomy} AS a
						INNER JOIN {$mndb->terms} AS b ON b.term_id = a.term_id
