...
( function( $, window ) {
	var PressThis = function() {
		var editor, $mediaList, $mediaThumbWrap,
			...
			sidebarIsOpen         = false,
			settings              = window.mnPressThisConfig || {},
			data                  = window.mnPressThisData || {},
			smallestWidth         = 128,
			hasSetFocus           = false,
			catsCache             = [],
		...
		function insertSelectedMedia( $element ) {
			...
			src = checkUrl( $element.attr( 'data-mn-src' ) || '' );
		...
		function addImg( src, displaySrc, i ) {
			...
			$element.attr( 'data-mn-src', src ).css( 'background-image', 'url(' + displaySrc + ')' )
		...
		function renderDetectedMedia() {
			...
			if ( data._embeds ) {
				$.each( data._embeds, function ( i, src ) {
					...
					$element.attr( 'data-mn-src', src ).find( 'span' ).text( __( 'suggestedEmbedAlt' ).replace( '%d', i + 1 ) );
				...
				if ( $button.length ) {
					if ( $button.hasClass( 'draft-button' ) ) {
						...
						if ( window.history.replaceState ) {
							...
							location += 'mn-press-this-reload=true';
	...
	window.mn = window.mn || {};
	window.mn.pressThis = new PressThis();
