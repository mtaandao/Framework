...
class WC_Product_Variation extends WC_Product {
	...
	public function __construct( $variation, $args = array() ) {
		...
		$this->id = ! empty( $args['parent_id'] ) ? intval( $args['parent_id'] ) : mn_get_post_parent_id( $this->variation_id );
	...
	public function get_permalink( $item_object = null ) {
		if ( ! empty( $item_object['variation'] ) ) {
			...
			$data_keys    = array_map( 'wc_variation_attribute_name', mn_list_pluck( $item_object['item_meta_array'], 'key' ) );
			$data_values  = mn_list_pluck( $item_object['item_meta_array'], 'value' );
	...
	public function set_stock( $amount = null, $mode = 'set' ) {
		...
		if ( ! is_null( $amount ) && true === $this->managing_stock() ) {
			...
			switch ( $mode ) {
				case 'add' :
					$mndb->query( $mndb->prepare( "UPDATE {$mndb->postmeta} SET meta_value = meta_value + %f WHERE post_id = %d AND meta_key='_stock'", $amount, $this->variation_id ) );
				break;
				case 'subtract' :
					$mndb->query( $mndb->prepare( "UPDATE {$mndb->postmeta} SET meta_value = meta_value - %f WHERE post_id = %d AND meta_key='_stock'", $amount, $this->variation_id ) );
				break;
				default :
					$mndb->query( $mndb->prepare( "UPDATE {$mndb->postmeta} SET meta_value = %f WHERE post_id = %d AND meta_key='_stock'", $amount, $this->variation_id ) );
				break;
			}

			// Clear caches
			mn_cache_delete( $this->variation_id, 'post_meta' );
		...
		if ( is_array( $variation_data ) ) {
			...
			foreach ( $attributes as $attribute ) {
				...
				if ( $attribute[ 'is_taxonomy' ] ) {
					...
					$post_terms = mn_get_post_terms( $this->id, $attribute[ 'name' ] );
	...
	public function get_variation_description() {
		return mnautop( do_shortcode( mn_kses_post( get_post_meta( $this->variation_id, '_variation_description', true ) ) ) );
	}
}
