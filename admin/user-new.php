...
if ( is_multisite() ) {
	add_filter( 'mnmu_signup_user_notification_email', 'admin_created_user_email' );
}

if ( isset($_REQUEST['action']) && 'adduser' == $_REQUEST['action'] ) {
	check_admin_referer( 'add-user', '_mnnonce_add-user' );

	$user_details = null;
	$user_email = mn_unslash( $_REQUEST['email'] );
	if ( false !== strpos( $user_email, '@' ) ) {
		...
		if ( is_super_admin() ) {
			...
			mn_redirect( add_query_arg( array('update' => 'enter_email'), 'user-new.php' ) );
			die();
		}
	}

	if ( !$user_details ) {
		mn_redirect( add_query_arg( array('update' => 'does_not_exist'), 'user-new.php' ) );
		die();
	}
...
%4$s' );
			mn_mail( $new_user_email, sprintf( __( '[%s] Joining confirmation' ), mn_specialchars_decode( get_option( 'blogname' ) ) ), sprintf( $message, get_option( 'blogname' ), home_url(), mn_specialchars_decode( translate_user_role( $role['name'] ) ), home_url( "/newbloguser/$newuser_key/" ) ) );
	...
	check_admin_referer( 'create-user', '_mnnonce_create-user' );
	...
	if ( ! is_multisite() ) {
		...
		$new_user_email = mn_unslash( $_REQUEST['email'] );
		$user_details = mnmu_validate_user_signup( $_REQUEST['user_login'], $new_user_email );
		if ( is_mn_error( $user_details[ 'errors' ] ) && !empty( $user_details[ 'errors' ]->errors ) ) {
			 ...
			 */
			$new_user_login = apply_filters( 'pre_user_login', sanitize_user( mn_unslash( $_REQUEST['user_login'] ), true ) );
			if ( isset( $_POST[ 'noconfirmation' ] ) && current_user_can( 'manage_network_users' ) ) {
				add_filter( 'mnmu_signup_user_notification', '__return_false' ); // Disable confirmation email
				add_filter( 'mnmu_welcome_user_notification', '__return_false' ); // Disable welcome email
			}
			mnmu_signup_user( $new_user_login, $new_user_email, array( 'add_to_blog' => $mndb->blogid, 'new_role' => $_REQUEST['role'] ) );
			if ( isset( $_POST[ 'noconfirmation' ] ) && current_user_can( 'manage_network_users' ) ) {
				$key = $mndb->get_var( $mndb->prepare( "SELECT activation_key FROM {$mndb->signups} WHERE user_login = %s AND user_email = %s", $new_user_login, $new_user_email ) );
				$new_user = mnmu_activate_signup( $key );
				if ( is_mn_error( $new_user ) ) {
...
get_current_screen()->set_help_sidebar(
    ...
    '<p>' . __('<a href="https://mtaandao.co.ke/docs/Users_Add_New_Screen" target="_blank">Documentation on Adding New Users</a>') . '</p>' .
...
if ( is_multisite() && current_user_can( 'promote_users' ) && ! mn_is_large_network( 'users' )
	...
	mn_enqueue_script( 'user-suggest' );
}
...
if ( isset($_GET['update']) ) {
	...
	if ( is_multisite() ) {
		...
		if ( ( isset( $_GET['user_id'] ) ) ) {
			...
			if ( $user_id_new ) {
				$edit_link = esc_url( add_query_arg( 'mn_http_referer', urlencode( mn_unslash( $_SERVER['REQUEST_URI'] ) ), get_edit_user_link( $user_id_new ) ) );
			}
		}
...
<?php if ( isset($add_user_errors) && is_mn_error( $add_user_errors ) ) : ?>
	<div class="error">
		<?php
...
<?php mn_nonce_field( 'add-user', '_mnnonce_add-user' ) ?>

<table class="form-table">
	<tr class="form-field form-required">
		<th scope="row"><label for="adduser-email"><?php echo $label; ?></label></th>
		<td><input name="email" type="<?php echo $type; ?>" id="adduser-email" class="mn-suggest-user" value="" /></td>
	</tr>
	<tr class="form-field">
		<th scope="row"><label for="adduser-role"><?php _e('Role'); ?></label></th>
		<td><select name="role" id="adduser-role">
			<?php mn_dropdown_roles( get_option('default_role') ); ?>
			</select>
		</td>
	</tr>
...
<?php mn_nonce_field( 'create-user', '_mnnonce_create-user' ); ?>
<?php
// Load up the passed data, else set to a default.
$creating = isset( $_POST['createuser'] );

$new_user_login = $creating && isset( $_POST['user_login'] ) ? mn_unslash( $_POST['user_login'] ) : '';
$new_user_firstname = $creating && isset( $_POST['first_name'] ) ? mn_unslash( $_POST['first_name'] ) : '';
$new_user_lastname = $creating && isset( $_POST['last_name'] ) ? mn_unslash( $_POST['last_name'] ) : '';
$new_user_email = $creating && isset( $_POST['email'] ) ? mn_unslash( $_POST['email'] ) : '';
$new_user_uri = $creating && isset( $_POST['url'] ) ? mn_unslash( $_POST['url'] ) : '';
$new_user_role = $creating && isset( $_POST['role'] ) ? mn_unslash( $_POST['role'] ) : '';
$new_user_send_notification = $creating && ! isset( $_POST['send_user_notification'] ) ? false : true;
$new_user_ignore_pass = $creating && isset( $_POST['noconfirmation'] ) ? mn_unslash( $_POST['noconfirmation'] ) : '';
...
<?php if ( !is_multisite() ) { ?>
	...
	<tr class="form-field form-required user-pass1-wrap">
		...
		<td>
			...
			<button type="button" class="button button-secondary mn-generate-pw hide-if-no-js"><?php _e( 'Show password' ); ?></button>
			<div class="mn-pwd hide-if-js">
				<?php $initial_password = mn_generate_password( 24 ); ?>
...
<?php } // !is_multisite ?>
	<tr class="form-field">
		...
		<td><select name="role" id="role">
			...
			mn_dropdown_roles($new_user_role);
			?>
			</select>
		</td>
	</tr>
