...
class WC_API_Customers extends WC_API_Resource {
	...
	public function __construct( WC_API_Server $server ) {
		...
		// modify MN_User_Query to support created_at date filtering
	...
	public function get_customer( $id, $fields = null ) {
		...
		$customer = new MN_User( $id );

		// get info about user's last order
		$last_order = $mndb->get_row( "SELECT id, post_date_gmt
	...
	public function get_customers_count( $filter = array() ) {
		...
		if ( ! current_user_can( 'list_users' ) )
			return new MN_Error( 'woocommerce_api_user_cannot_read_customers_count', __( 'You do not have permission to read the customers count', 'woocommerce' ), array( 'status' => 401 ) );
	...
	public function create_customer( $data ) {
		...
		if ( ! current_user_can( 'create_users' ) )
			return new MN_Error( 'woocommerce_api_user_cannot_create_customer', __( 'You do not have permission to create this customer', 'woocommerce' ), array( 'status' => 401 ) );
	...
	public function get_customer_orders( $id, $fields = null ) {
		...
		$order_ids = $mndb->get_col( $mndb->prepare( "SELECT id
						FROM $mndb->posts AS posts
						LEFT JOIN {$mndb->postmeta} AS meta on posts.ID = meta.post_id
						WHERE meta.meta_key = '_customer_user'
	 ...
	 *
	 * Note that MN_User_Query does not have built-in pagination so limit & offset are used to provide limited
	 * pagination support
	 *
	 * @since 2.1
	 * @param array $args request arguments for filtering query
	 * @return MN_User_Query
	 */
	private function query_customers( $args = array() ) {
		...
		$query = new MN_User_Query( $query_args );
	...
	/**
	 * Modify the MN_User_Query to support filtering on the date the customer was created
	 *
	 * @since 2.1
	 ...
	 * 2) the ID returns a valid MN_User
	 ...
	 * @return int|MN_Error valid user ID or MN_Error if any of the checks fails
	 */
	protected function validate_request( $id, $type, $context ) {
		...
		// validate ID
		if ( empty( $id ) )
			return new MN_Error( 'woocommerce_api_invalid_customer_id', __( 'Invalid customer ID', 'woocommerce' ), array( 'status' => 404 ) );

		// non-existent IDs return a valid MN_User object with the user ID = 0
		$customer = new MN_User( $id );

		if ( 0 === $customer->ID )
			return new MN_Error( 'woocommerce_api_invalid_customer', __( 'Invalid customer', 'woocommerce' ), array( 'status' => 404 ) );
		...
		switch ( $context ) {
			...
			case 'read':
				if ( ! current_user_can( 'list_users' ) )
					return new MN_Error( 'woocommerce_api_user_cannot_read_customer', __( 'You do not have permission to read this customer', 'woocommerce' ), array( 'status' => 401 ) );
				break;

			case 'edit':
				if ( ! current_user_can( 'edit_users' ) )
					return new MN_Error( 'woocommerce_api_user_cannot_edit_customer', __( 'You do not have permission to edit this customer', 'woocommerce' ), array( 'status' => 401 ) );
				break;

			case 'delete':
				if ( ! current_user_can( 'delete_users' ) )
					return new MN_Error( 'woocommerce_api_user_cannot_delete_customer', __( 'You do not have permission to delete this customer', 'woocommerce' ), array( 'status' => 401 ) );
				break;
		}
