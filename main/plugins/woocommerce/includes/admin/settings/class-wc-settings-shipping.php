...
class WC_Settings_Shipping extends WC_Settings_Page {
	...
	protected function zone_methods_screen( $zone_id ) {
		...
		if ( ! $zone ) {
			mn_die( __( 'Zone does not exist!', 'woocommerce' ) );
		}

		mn_localize_script( 'wc-shipping-zone-methods', 'shippingZoneMethodsLocalizeScript', array(
			'methods'                 => $zone->get_shipping_methods(),
			'zone_id'                 => $zone->get_zone_id(),
			'wc_shipping_zones_nonce' => mn_create_nonce( 'wc_shipping_zones_nonce' ),
			'strings'                 => array(
		...
		mn_enqueue_script( 'wc-shipping-zone-methods' );
	...
	protected function zones_screen() {
		...
		mn_localize_script( 'wc-shipping-zones', 'shippingZonesLocalizeScript', array(
		...
		mn_enqueue_script( 'wc-shipping-zones' );
	...
	protected function instance_settings_screen( $instance_id ) {
		...
		if ( ! $shipping_method ) {
			mn_die( __( 'Invalid shipping method!', 'woocommerce' ) );
		}
		if ( ! $zone ) {
			mn_die( __( 'Zone does not exist!', 'woocommerce' ) );
		}
		if ( ! $shipping_method->has_settings() ) {
			mn_die( __( 'This shipping method does not have any settings to configure.', 'woocommerce' ) );
		}
	...
	protected function output_shipping_class_screen() {
		$wc_shipping = WC_Shipping::instance();
		mn_localize_script( 'wc-shipping-classes', 'shippingClassesLocalizeScript', array(
			...
			'wc_shipping_classes_nonce' => mn_create_nonce( 'wc_shipping_classes_nonce' ),
			'strings'       => array(
				'unload_confirmation_msg' => __( 'Your changed data will be lost if you leave this page without saving.', 'woocommerce' ),
				'save_failed'             => __( 'Your changes were not saved. Please retry.', 'woocommerce' )
			),
		) );
		mn_enqueue_script( 'wc-shipping-classes' );
