<?php
/**
 * Mtaandao Plugin Install Administration API
 *
 * @package Mtaandao
 ...
 *         @type bool $requires          Whether to return the required Mtaandao version. Default true.
 ...
 *         @type bool $added             Whether to return the date when the plugin was added to the mtaandao.co.ke
 *                                       repository. Default true.
 *         @type bool $tags              Whether to return the assigned tags. Default true.
 *         @type bool $compatibility     Whether to return the Mtaandao compatibility list. Default true.
 ...
 *         {@link https://developer.mtaandao.co.ke/reference/functions/plugins_api/ function reference article}
...
function plugins_api( $action, $args = array() ) {
	...
	if ( false === $res ) {
		$url = $http_url = 'http://api.mtaandao.co.ke/plugins/info/1.0/';
...
/**
 * Retrieve popular Mtaandao plugin tags.
 *
 * @since 2.7.0
 *
 * @param array $args
...
function install_dashboard() {
	...
	if ( is_mn_error($api_tags) ) {
		echo $api_tags->get_error_message();
	} else {
		//Set up the tags in a way which can be interpreted by mn_generate_tag_cloud()
		$tags = array();
		...
		echo mn_generate_tag_cloud($tags, array( 'single_text' => __('%s plugin'), 'multiple_text' => __('%s plugins') ) );
	}
...
function install_search_form( $deprecated = true ) {
	$type = isset( $_REQUEST['type'] ) ? mn_unslash( $_REQUEST['type'] ) : 'term';
	$term = isset( $_REQUEST['s'] ) ? mn_unslash( $_REQUEST['s'] ) : '';
...
<div class="upload-plugin">
	...
	<form method="post" enctype="multipart/form-data" class="mn-upload-form" action="<?php echo self_admin_url('update.php?action=upload-plugin'); ?>">
...
function display_plugins_table() {
	...
	switch ( current_filter() ) {
		case 'install_plugins_favorites' :
			if ( empty( $_GET['user'] ) && ! get_user_option( 'mnorg_favorites' ) ) {
				return;
			}
			break;
		...
		case 'install_plugins_beta' :
			printf(
				'<p>' . __( 'You are using a development version of Mtaandao. These feature plugins are also under development. <a href="%s">Learn more</a>.' ) . '</p>',
				'https://make.mtaandao.co.ke/core/handbook/about/release-cycle/features-as-plugins/'
			);
			break;
	}
...
function install_plugin_install_status($api, $loop = false) {
		...
		foreach ( (array)$update_plugins->response as $file => $plugin ) {
			if ( $plugin->slug === $api->slug ) {
				...
				if ( current_user_can('update_plugins') )
					$url = mn_nonce_url(self_admin_url('update.php?action=upgrade-plugin&plugin=' . $update_file), 'upgrade-plugin_' . $update_file);
				break;
			}
		}
	}
		...
		if ( is_dir( MN_PLUGIN_DIR . '/' . $api->slug ) ) {
			...
			if ( empty($installed_plugin) ) {
				if ( current_user_can('install_plugins') )
					$url = mn_nonce_url(self_admin_url('update.php?action=install-plugin&plugin=' . $api->slug), 'install-plugin_' . $api->slug);
			} else {
	...
	if ( isset($_GET['from']) )
		$url .= '&amp;from=' . urlencode( mn_unslash( $_GET['from'] ) );
...
function install_plugin_information() {
	...
	$api = plugins_api( 'plugin_information', array(
		'slug' => mn_unslash( $_REQUEST['plugin'] ),
		'is_ssl' => is_ssl(),
		'fields' => array(
			'banners' => true,
			'reviews' => true,
	...
	foreach ( (array) $api->sections as $section_name => $content ) {
		$api->sections[$section_name] = mn_kses( $content, $plugins_allowedtags );
	}

	foreach ( array( 'version', 'author', 'requires', 'tested', 'homepage', 'downloaded', 'slug' ) as $key ) {
		if ( isset( $api->$key ) ) {
			$api->$key = mn_kses( $api->$key, $plugins_allowedtags );
		}
	}

	$_tab = esc_attr( $tab );

	$section = isset( $_REQUEST['section'] ) ? mn_unslash( $_REQUEST['section'] ) : 'description'; // Default to the Description tab, Do not translate, API returns English.
...
<div id="<?php echo $_tab; ?>-content" class='<?php echo $_with_banner; ?>'>
	<div class="fyi">
		<ul>
			...
			<?php } if ( ! empty( $api->requires ) ) { ?>
				<li>
					<strong><?php _e( 'Requires Mtaandao Version:' ); ?></strong>
					<?php
		...
		<?php if ( ! empty( $api->rating ) ) { ?>
			...
			<?php mn_star_rating( array( 'rating' => $api->rating, 'type' => 'percent', 'number' => $api->num_ratings ) ); ?>
		...
		if ( ! empty( $api->ratings ) && array_sum( (array) $api->ratings ) > 0 ) { ?>
			...
			foreach ( $api->ratings as $key => $ratecount ) {
				...
				<div class="counter-container">
						<span class="counter-label"><a href="https://mtaandao.co.ke/support/view/plugin-reviews/<?php echo $api->slug; ?>?filter=<?php echo $key; ?>"
			...
			<ul class="contributors">
				...
				foreach ( (array) $api->contributors as $contrib_username => $contrib_profile ) {
					...
					if ( empty( $contrib_profile ) ) {
						echo "<li><img src='https://mtaandao.co.ke/grav-redirect.php?user={$contrib_username}&amp;s=36' width='18' height='18' alt='' />{$contrib_username}</li>";
					} else {
						echo "<li><a href='{$contrib_profile}' target='_blank'><img src='https://mtaandao.co.ke/grav-redirect.php?user={$contrib_username}&amp;s=36' width='18' height='18' alt='' />{$contrib_username}</a></li>";
					}
				}
				?>
			</ul>
	...
	foreach ( (array) $api->sections as $section_name => $content ) {
		$content = links_add_base_url( $content, 'https://mtaandao.co.ke/plugins/' . $api->slug . '/' );
