...
class WC_Comments {
	...
	public static function init() {
		...
		// Clear transients
		add_action( 'mn_update_comment_count', array( __CLASS__, 'clear_transients' ) );
		...
		// Count comments
		add_filter( 'mn_count_comments', array( __CLASS__, 'mn_count_comments' ), 10, 2 );

		// Delete comments count cache whenever there is a new comment or a comment status changes
		add_action( 'mn_insert_comment', array( __CLASS__, 'delete_comments_count_cache' ) );
		add_action( 'mn_set_comment_status', array( __CLASS__, 'delete_comments_count_cache' ) );
	...
	public static function exclude_order_comments( $clauses ) {
		...
		if ( ! stristr( $clauses['join'], "JOIN $mndb->posts ON" ) ) {
			$clauses['join'] .= " LEFT JOIN $mndb->posts ON comment_post_ID = $mndb->posts.ID ";
		}
		...
		$clauses['where'] .= " $mndb->posts.post_type NOT IN ('" . implode( "','", wc_get_order_types() ) . "') ";
	...
	public static function exclude_order_comments_from_feed_join( $join ) {
		...
		if ( ! stristr( $join, "JOIN $mndb->posts ON" ) ) {
			$join = " LEFT JOIN $mndb->posts ON $mndb->comments.comment_post_ID = $mndb->posts.ID ";
		}
	...
	public static function exclude_order_comments_from_feed_where( $where ) {
		...
		$where .= " $mndb->posts.post_type NOT IN ('" . implode( "','", wc_get_order_types() ) . "') ";
	...
	public static function exclude_webhook_comments( $clauses ) {
		...
		if ( ! strstr( $clauses['join'], "JOIN $mndb->posts" ) ) {
			$clauses['join'] .= " LEFT JOIN $mndb->posts ON comment_post_ID = $mndb->posts.ID ";
		}
		...
		$clauses['where'] .= " $mndb->posts.post_type <> 'shop_webhook' ";
	...
	public static function exclude_webhook_comments_from_feed_where( $where ) {
		...
		$where .= " $mndb->posts.post_type <> 'shop_webhook' ";
	...
	public static function check_comment_rating( $comment_data ) {
		...
		if (
			...
			mn_die( __( 'Please rate the product.', 'woocommerce' ) );
			exit;
		}
	 ...
	 * will be regenerated next time WC_Comments::mn_count_comments()
	 * is called.
	 *
	 * @return void
	 */
	...
	/**
	 * Remove order notes from mn_count_comments().
	 * @since  2.2
	 * @param  object $stats
	 * @param  int $post_id
	 * @return object
	 */
	public static function mn_count_comments( $stats, $post_id ) {
		global $mndb;
		...
		if ( 0 === $post_id ) {
			...
			if ( ! $stats ) {
				...
				$count = $mndb->get_results( "SELECT comment_approved, COUNT( * ) AS num_comments FROM {$mndb->comments} WHERE comment_type != 'order_note' GROUP BY comment_approved", ARRAY_A );
	...
	/**
	 * Make sure MN displays avatars for comments with the `review` type.
	 * @since  2.3
