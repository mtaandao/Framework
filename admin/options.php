...
mn_reset_vars(array('action', 'option_page'));
...
if ( is_multisite() ) {
	if ( ! empty($_GET[ 'adminhash' ] ) ) {
		...
		mn_redirect( admin_url( $redirect ) );
		exit;
	} elseif ( ! empty( $_GET['dismiss'] ) && 'new_admin_email' == $_GET['dismiss'] ) {
		check_admin_referer( 'dismiss-' . get_current_blog_id() . '-new_admin_email' );
		delete_option( 'adminhash' );
		delete_option( 'new_admin_email' );
		mn_redirect( admin_url( 'options-general.php?updated=true' ) );
		exit;
	}
}
...
$whitelist_options = array(
	'general' => array( 'blogname', 'blogdescription', 'gmt_offset', 'date_format', 'time_format', 'start_of_week', 'timezone_string', 'MNLANG' ),
...
if ( 'update' == $action ) {
	...
	if ( !isset( $whitelist_options[ $option_page ] ) )
		mn_die( __( '<strong>ERROR</strong>: options page not found.' ) );

	if ( 'options' == $option_page ) {
		if ( is_multisite() && ! is_super_admin() )
			mn_die( __( 'Sorry, you are not allowed to modify unregistered settings for this site.' ) );
		$options = explode( ',', mn_unslash( $_POST[ 'page_options' ] ) );
	} else {
	...
	if ( 'general' == $option_page ) {
		...
		if ( !empty($_POST['date_format']) && isset($_POST['date_format_custom']) && '\c\u\s\t\o\m' == mn_unslash( $_POST['date_format'] ) )
			$_POST['date_format'] = $_POST['date_format_custom'];
		if ( !empty($_POST['time_format']) && isset($_POST['time_format_custom']) && '\c\u\s\t\o\m' == mn_unslash( $_POST['time_format'] ) )
		...
		if ( ! empty( $_POST['MNLANG'] ) && ( ! is_multisite() || is_super_admin() ) ) { // @todo: Skip if already installed
			require_once( ABSPATH . 'admin/includes/translation-install.php' );

			if ( mn_can_install_language_pack() ) {
				$language = mn_download_language_pack( $_POST['MNLANG'] );
				if ( $language ) {
					$_POST['MNLANG'] = $language;
				}
			}
		}
	}
	...
	if ( $options ) {
			...
			if ( $unregistered ) {
				_deprecated_argument( 'options.php', '2.7.0',
					sprintf(
						...
						__( 'The %s setting is unregistered. Unregistered settings are deprecated. See https://mtaandao.co.ke/docs/Settings_API' ),
						'<code>' . $option . '</code>'
					)
				);
			}
		...
		// Switch translation in case MNLANG was changed.
		$language = get_option( 'MNLANG' );
		if ( $language ) {
	 ...
	 */
	$goback = add_query_arg( 'settings-updated', 'true',  mn_get_referer() );
	mn_redirect( $goback );
	exit;
}
...
<div class="wrap">
  ...
  <?php mn_nonce_field('options-options') ?>
  <input type="hidden" name="action" value="update" />
  <input type="hidden" name="option_page" value="options" />
  <table class="form-table">
<?php
$options = $mndb->get_results( "SELECT * FROM $mndb->options ORDER BY option_name" );
