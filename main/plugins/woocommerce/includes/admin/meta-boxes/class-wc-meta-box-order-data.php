...
class WC_Meta_Box_Order_Data {
						...
						<?php
							...
							echo '<div class="address">';
								...
								if ( $order->get_formatted_billing_address() ) {
									echo '<p><strong>' . __( 'Address', 'woocommerce' ) . ':</strong>' . mn_kses( $order->get_formatted_billing_address(), array( 'br' => array() ) ) . '</p>';
								} else {
							...
							foreach ( self::$billing_fields as $key => $field ) {
								...
								switch ( $field['type'] ) {
									case 'select' :
										woocommerce_mn_select( $field );
									break;
									default :
										woocommerce_mn_text_input( $field );
									break;
								}
							}
							?>
							...
							woocommerce_mn_text_input( array( 'id' => '_transaction_id', 'label' => __( 'Transaction ID', 'woocommerce' ) ) );
						...
						<?php
							...
							echo '<div class="address">';
								...
								if ( $order->get_formatted_shipping_address() ) {
									echo '<p><strong>' . __( 'Address', 'woocommerce' ) . ':</strong>' . mn_kses( $order->get_formatted_shipping_address(), array( 'br' => array() ) ) . '</p>';
								} else {
							...
							if ( apply_filters( 'woocommerce_enable_order_notes_field', 'yes' == get_option( 'woocommerce_enable_order_comments', 'yes' ) ) ) {
								...
								<textarea rows="1" cols="40" name="excerpt" tabindex="6" id="excerpt" placeholder="<?php esc_attr_e( 'Customer\'s notes about the order', 'woocommerce' ); ?>"><?php echo mn_kses_post( $post->post_excerpt ); ?></textarea></p>
								<?php
							}
	...
	public static function save( $post_id, $post ) {
		...
		$mndb->query( $mndb->prepare( "UPDATE $mndb->posts SET post_date = %s, post_date_gmt = %s WHERE ID = %s", $date, get_gmt_from_date( $date ), $post_id ) );

		clean_post_cache( $post_id );

		// If customer changed, update any downloadable permissions
		if ( $customer_changed ) {
			$mndb->update( $mndb->prefix . "woocommerce_downloadable_product_permissions",
				array(
