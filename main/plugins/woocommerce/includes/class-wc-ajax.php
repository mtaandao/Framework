...
class WC_AJAX {
	...
	public static function define_ajax() {
		if ( ! empty( $_GET['wc-ajax'] ) ) {
			...
			if ( ! MN_DEBUG || ( MN_DEBUG && ! MN_DEBUG_DISPLAY ) ) {
				@ini_set( 'display_errors', 0 );
			}
			$GLOBALS['mndb']->hide_errors();
		}
	}
	...
	public static function do_wc_ajax() {
		...
		if ( ! empty( $_GET['wc-ajax'] ) ) {
			$mn_query->set( 'wc-ajax', sanitize_text_field( $_GET['wc-ajax'] ) );
		}

		if ( $action = $mn_query->get( 'wc-ajax' ) ) {
	...
	/**
	 * Hook in methods - uses Mtaandao ajax handlers (admin-ajax).
	 */
	public static function add_ajax_events() {
		...
		foreach ( $ajax_events as $ajax_event => $nopriv ) {
			add_action( 'mn_ajax_woocommerce_' . $ajax_event, array( __CLASS__, $ajax_event ) );

			if ( $nopriv ) {
				add_action( 'mn_ajax_nopriv_woocommerce_' . $ajax_event, array( __CLASS__, $ajax_event ) );
	...
	public static function get_variation() {
		...
		$variation_id = $variable_product->get_matching_variation( mn_unslash( $_POST ) );
	...
	public static function feature_product() {
		...
		mn_safe_redirect( mn_get_referer() ? remove_query_arg( array( 'trashed', 'untrashed', 'deleted', 'ids' ), mn_get_referer() ) : admin_url( 'edit.php?post_type=product' ) );
		die();
	}
	...
	public static function mark_order_status() {
		...
		mn_safe_redirect( mn_get_referer() ? mn_get_referer() : admin_url( 'edit.php?post_type=shop_order' ) );
		die();
	}
	...
	public static function add_new_attribute() {
		...
		if ( taxonomy_exists( $taxonomy ) ) {
			...
			$result = mn_insert_term( $term, $taxonomy );
			...
			for ( $i = 0; $i <= $attribute_names_max_key; $i++ ) {
				...
				if ( $is_taxonomy ) {
					...
					if ( isset( $attribute_values[ $i ] ) ) {
						...
						// Text based attributes - Posted values are term names, mn_set_object_terms wants ids or slugs.
						} else {
							$values     = array();
							...
							foreach ( $raw_values as $value ) {
								...
								if ( ! $term ) {
									$term = mn_insert_term( $value, $attribute_names[ $i ] );
	...
	public static function add_variation() {
		...
		if ( $variation_id ) {
			...
			$variation_data['image']          = $variation_data['_thumbnail_id'] ? mn_get_attachment_thumb_url( $variation_data['_thumbnail_id'] ) : '';
			$variation_data['shipping_class'] = $shipping_classes && ! is_mn_error( $shipping_classes ) ? current( $shipping_classes )->term_id : '';
	...
	public static function link_all_variations() {
		...
		foreach ( $possible_variations as $variation ) {
			...
			$variation_id = mn_insert_post( $variation_post_data );
	...
	public static function revoke_access_to_download() {
		...
		$mndb->query( $mndb->prepare( "DELETE FROM {$mndb->prefix}woocommerce_downloadable_product_permissions WHERE permission_id = %d;", $permission_id ) );
			...
			if ( ! empty( $files ) ) {
				foreach ( $files as $download_id => $file ) {
					if ( $inserted_id = wc_downloadable_file_permission( $download_id, $product_id, $order ) ) {
						...
						$download = $mndb->get_row( $mndb->prepare( "SELECT * FROM {$mndb->prefix}woocommerce_downloadable_product_permissions WHERE permission_id = %d", $inserted_id ) );
	...
	public static function remove_order_item_meta() {
		...
		$mndb->delete( "{$mndb->prefix}woocommerce_order_itemmeta", array(
	...
	public static function add_order_note() {
		...
		$post_id   = absint( $_POST['post_id'] );
		$note      = mn_kses_post( trim( stripslashes( $_POST['note'] ) ) );
	...
	public static function json_search_products( $term = '', $post_types = array( 'product' ) ) {
		...
		$like_term = '%' . $mndb->esc_like( $term ) . '%';

		if ( is_numeric( $term ) ) {
			$query = $mndb->prepare( "
				SELECT ID FROM {$mndb->posts} posts LEFT JOIN {$mndb->postmeta} postmeta ON posts.ID = postmeta.post_id
		...
		$posts          = array_unique( $mndb->get_col( $query ) );
	...
	public static function json_search_customers() {
		...
		$customers_query = new MN_User_Query( apply_filters( 'woocommerce_json_search_customers_query', array(
			'fields'         => 'all',
		...
		mn_send_json( $found_customers );
	}

	/**
	 * When searching using the MN_User_Query, search names (user meta) too.
	 * @param  object $query
	 * @return object
	 */
	public static function json_search_customer_name( $query ) {
		...
		if ( method_exists( $mndb, 'esc_like' ) ) {
			$term = $mndb->esc_like( $term );
		} else {
			$term = like_escape( $term );
		}

		$query->query_from  .= " INNER JOIN {$mndb->usermeta} AS user_name ON {$mndb->users}.ID = user_name.user_id AND ( user_name.meta_key = 'first_name' OR user_name.meta_key = 'last_name' ) ";
		$query->query_where .= $mndb->prepare( " OR user_name.meta_value LIKE %s ", '%' . $term . '%' );
	}
	...
	public static function product_ordering() {
		...
		$siblings = $mndb->get_results( $mndb->prepare( "
			SELECT ID, menu_order FROM {$mndb->posts} AS posts
	...
	public static function refund_line_items() {
		...
		try {
			...
			mn_send_json_success( $response_data );
			...
			mn_send_json_error( array( 'error' => $e->getMessage() ) );
		}
	}
	...
	public static function delete_refund() {
		...
		foreach ( $refund_ids as $refund_id ) {
			if ( $refund_id && 'shop_order_refund' === get_post_type( $refund_id ) ) {
				$order_id = mn_get_post_parent_id( $refund_id );
				wc_delete_shop_order_transients( $order_id );
				mn_delete_post( $refund_id );
	...
	public static function update_api_key() {
		...
		try {
			...
			$description = sanitize_text_field( mn_unslash( $_POST['description'] ) );
			...
			if ( 0 < $key_id ) {
				...
				$mndb->update(
					$mndb->prefix . 'woocommerce_api_keys',
					$data,
					array( 'key_id' => $key_id ),
					array(
						'%d',
				...
				$data['revoke_url']      = '<a style="color: #a00; text-decoration: none;" href="' . esc_url( mn_nonce_url( add_query_arg( array( 'revoke-key' => $key_id ), admin_url( 'admin.php?page=wc-settings&tab=api&section=keys' ) ), 'revoke' ) ). '">' . __( 'Revoke Key', 'woocommerce' ) . '</a>';
			}

			mn_send_json_success( $data );
		} catch ( Exception $e ) {
			mn_send_json_error( array( 'message' => $e->getMessage() ) );
		}
	}
	...
	public static function get_customer_location() {
		...
		mn_send_json_success( array( 'hash' => $location_hash ) );
	}
	...
	public static function load_variations() {
		...
		// Get attributes
		$attributes        = array();
		$posted_attributes = mn_unslash( $_POST['attributes'] );
	...
	public static function save_variations() {
		...
		$product_type_terms = mn_get_object_terms( $product_id, 'product_type' );
	...
	private static function variation_bulk_action_toggle_enabled( $variations, $data ) {
		...
		foreach ( $variations as $variation_id ) {
			...
			$mndb->update( $mndb->posts, array( 'post_status' => $new_status ), array( 'ID' => $variation_id ) );
		}
	}
	...
	public static function tax_rates_save_changes() {
		if ( ! isset( $_POST['wc_tax_nonce'], $_POST['changes'] ) ) {
			mn_send_json_error( 'missing_fields' );
			exit;
		}

		$current_class = ! empty( $_POST['current_class'] ) ? $_POST['current_class'] : ''; // This is sanitized seven lines later.

		if ( ! mn_verify_nonce( $_POST['wc_tax_nonce'], 'wc_tax_nonce-class:' . $current_class ) ) {
			mn_send_json_error( 'bad_nonce' );
			exit;
		}
		...
		// Check User Caps
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			mn_send_json_error( 'missing_capabilities' );
			exit;
		}
	...
	public static function shipping_zones_save_changes() {
		...
		if ( ! mn_verify_nonce( $_POST['wc_shipping_zones_nonce'], 'wc_shipping_zones_nonce' ) ) {
			mn_send_json_error( 'bad_nonce' );
			exit;
		}
	...
	public static function shipping_zone_methods_save_changes() {
		...
		foreach ( $changes as $instance_id => $data ) {
			$method_id = $mndb->get_var( $mndb->prepare( "SELECT method_id FROM {$mndb->prefix}woocommerce_shipping_zone_methods WHERE instance_id = %d", $instance_id ) );

			if ( isset( $data['deleted'] ) ) {
				if ( $mndb->delete( "{$mndb->prefix}woocommerce_shipping_zone_methods", array( 'instance_id' => $instance_id ) ) ) {
			...
			if ( isset( $method_data['method_order'] ) ) {
				$mndb->update( "{$mndb->prefix}woocommerce_shipping_zone_methods", array( 'method_order' => absint( $method_data['method_order'] ) ), array( 'instance_id' => absint( $instance_id ) ) );
			}

			if ( isset( $method_data['enabled'] ) ) {
				$is_enabled = absint( 'yes' === $method_data['enabled'] );
				if ( $mndb->update( "{$mndb->prefix}woocommerce_shipping_zone_methods", array( 'is_enabled' => $is_enabled ), array( 'instance_id' => absint( $instance_id ) ) ) ) {
	...
	public static function shipping_classes_save_changes() {
		...
		if ( ! mn_verify_nonce( $_POST['wc_shipping_classes_nonce'], 'wc_shipping_classes_nonce' ) ) {
			mn_send_json_error( 'bad_nonce' );
			exit;
		}
		...
		foreach ( $changes as $term_id => $data ) {
			...
			if ( isset( $data['deleted'] ) ) {
				...
				mn_delete_term( $term_id, 'product_shipping_class' );
				continue;
			}
			...
			if ( isset( $data['newRow'] ) ) {
				...
				$term_id = mn_insert_term( $update_args['name'], 'product_shipping_class', $update_args );
			} else {
				mn_update_term( $term_id, 'product_shipping_class', $update_args );
			}
