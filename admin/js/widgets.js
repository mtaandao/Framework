...
mnWidgets = {
	...
	init : function() {
		...
		$('#widgets-right .sidebar-name').click( function() {
			...
			$document.triggerHandler( 'mn-pin-menu' );
		});
		...
		$(document.body).bind('click.widgets-toggle', function(e) {
			...
			if ( target.parents('.widget-top').length && ! target.parents('#available-widgets').length ) {
				...
				mnWidgets.save( target.closest('div.widget'), 0, 1, 0 );
				e.preventDefault();
			} else if ( target.hasClass('widget-control-remove') ) {
				mnWidgets.save( target.closest('div.widget'), 1, 1, 0 );
				e.preventDefault();
			} else if ( target.hasClass('widget-control-close') ) {
				widget = target.closest('div.widget');
				widget.removeClass( 'open' );
				mnWidgets.close( widget );
				e.preventDefault();
			} else if ( target.attr( 'id' ) === 'inactive-widgets-control-remove' ) {
				mnWidgets.removeInactiveWidgets();
				e.preventDefault();
			}
		});
		...
		$('#widget-list').children('.widget').draggable({
			...
			start: function( event, ui ) {
				...
				if ( chooser.length ) {
					...
					$( '#mnbody-content' ).append( chooser.hide() );
		...
		sidebars.droppable( {
			...
			over: function( event ) {
				...
				if ( mnWidgets.hoveredSidebar && ! $wrap.is( mnWidgets.hoveredSidebar ) ) {
					// Close the previous Sidebar as the Widget has been dragged onto another Sidebar.
					mnWidgets.closeSidebar( event );
				}

				if ( $wrap.hasClass( 'closed' ) ) {
					mnWidgets.hoveredSidebar = $wrap;
					$wrap.removeClass( 'closed' );
				}
		...
		sidebars.sortable({
			...
			stop: function( event, ui ) {
				...
				mnWidgets.hoveredSidebar = null;

				if ( $widget.hasClass('deleting') ) {
					mnWidgets.save( $widget, 1, 0, 1 ); // delete widget
					$widget.remove();
					return;
				}
				...
				if ( addNew ) {
					...
					mnWidgets.saveOrder( $sidebar.attr('id') );
				}
			},
	...
	saveOrder : function( sidebarId ) {
		var data = {
			...
			savewidgets: $('#_mnnonce_widgets').val(),
			sidebars: []
		};
		...
		$.post( ajaxurl, data, function() {
			$( '#inactive-widgets-control-remove' ).prop( 'disabled' , ! $( '#mn_inactive_widgets .widget' ).length );
		...
		$.post( ajaxurl, data, function(r) {
			...
			if ( del ) {
				...
				if ( animate ) {
					...
					if ( sidebarId === 'mn_inactive_widgets' ) {
						$( '#inactive-widgets-control-remove' ).prop( 'disabled' , ! $( '#mn_inactive_widgets .widget' ).length );
					}
				}
			} else {
	...
	removeInactiveWidgets : function() {
		...
		a = {
			...
			removeinactivewidgets : $( '#_mnnonce_remove_inactive_widgets' ).val()
		};

		data = $.param( a );

		$.post( ajaxurl, data, function() {
			$( '#mn_inactive_widgets .widget' ).remove();
...
$document.ready( function(){ mnWidgets.init(); } );
