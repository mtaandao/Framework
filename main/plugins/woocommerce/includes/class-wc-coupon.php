...
class WC_Coupon {
	 ...
	 * @since 2.5.0 woocommerce_coupon_code_query was removed in favour of woocommerce_get_coupon_id_from_code filter on the return. mn_cache was also implemented.
	 * @param  string $code
	 * @return int
	 */
	private function get_coupon_id_from_code( $code ) {
		...
		$coupon_id = mn_cache_get( WC_Cache_Helper::get_cache_prefix( 'coupons' ) . 'coupon_id_from_code_' . $code, 'coupons' );

		if ( false === $coupon_id ) {
			$sql = $mndb->prepare( "SELECT ID FROM $mndb->posts WHERE post_title = %s AND post_type = 'shop_coupon' AND post_status = 'publish' ORDER BY post_date DESC LIMIT 1;", $this->code );

			if ( $coupon_id = apply_filters( 'woocommerce_get_coupon_id_from_code', $mndb->get_var( $sql ), $this->code ) ) {
				mn_cache_set( WC_Cache_Helper::get_cache_prefix( 'coupons' ) . 'coupon_id_from_code_' . $code, $coupon_id, 'coupons' );
			}
		}
	...
	public function dcr_usage_count( $used_by = '' ) {
		if ( $this->id && $this->usage_count > 0 ) {
			...
			if ( $used_by ) {
				 ...
				 */
				$meta_id = $mndb->get_var( $mndb->prepare( "SELECT meta_id FROM $mndb->postmeta WHERE meta_key = '_used_by' AND meta_value = %s AND post_id = %d LIMIT 1;", $used_by, $this->id ) );
				if ( $meta_id ) {
	...
	private function validate_user_usage_limit( $user_id = 0 ) {
		...
		if ( $this->usage_limit_per_user > 0 && is_user_logged_in() && $this->id ) {
			global $mndb;
			$usage_count = $mndb->get_var( $mndb->prepare( "SELECT COUNT( meta_id ) FROM {$mndb->postmeta} WHERE post_id = %d AND meta_key = '_used_by' AND meta_value = %d;", $this->id, $user_id ) );
