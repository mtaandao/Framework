...
class WC_Gateway_Paypal extends WC_Payment_Gateway {
	 ...
	 * @param  float  $amount
	 * @param  string $reason
	 * @return bool True or false based on success, or a MN_Error object
	 */
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( ! $this->can_refund_order( $order ) ) {
			...
			return new MN_Error( 'error', __( 'Refund Failed: No transaction ID', 'woocommerce' ) );
		}
		...
		if ( is_mn_error( $result ) ) {
			...
			return new MN_Error( 'error', $result->get_error_message() );
		}
		...
		return isset( $result['L_LONGMESSAGE0'] ) ? new MN_Error( 'error', $result['L_LONGMESSAGE0'] ) : false;
	}
}
