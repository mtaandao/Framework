...
class WC_Tax {
	...
	private static function get_matched_tax_rates( $country, $state, $postcode, $city, $tax_class ) {
		...
		$criteria   = array();
		$criteria[] = $mndb->prepare( "tax_rate_country IN ( %s, '' )", strtoupper( $country ) );
		$criteria[] = $mndb->prepare( "tax_rate_state IN ( %s, '' )", strtoupper( $state ) );
		$criteria[] = $mndb->prepare( "tax_rate_class = %s", sanitize_title( $tax_class ) );

		// Pre-query postcode ranges for PHP based matching.
		$postcode_search = wc_get_wildcard_postcodes( $postcode, $country );
		$postcode_ranges = $mndb->get_results( "SELECT tax_rate_id, location_code FROM {$mndb->prefix}woocommerce_tax_rate_locations WHERE location_type = 'postcode' AND location_code LIKE '%...%';" );
		...
		$locations_criteria[] = "
			...
			AND (
				...
				OR NOT EXISTS (
					SELECT sub.tax_rate_id FROM {$mndb->prefix}woocommerce_tax_rate_locations as sub
		...
		$found_rates = $mndb->get_results( "
			SELECT tax_rates.*
			FROM {$mndb->prefix}woocommerce_tax_rates as tax_rates
			LEFT OUTER JOIN {$mndb->prefix}woocommerce_tax_rate_locations as locations ON tax_rates.tax_rate_id = locations.tax_rate_id
			LEFT OUTER JOIN {$mndb->prefix}woocommerce_tax_rate_locations as locations2 ON tax_rates.tax_rate_id = locations2.tax_rate_id
	...
	public static function is_compound( $key ) {
		global $mndb;
		return $mndb->get_var( $mndb->prepare( "SELECT tax_rate_compound FROM {$mndb->prefix}woocommerce_tax_rates WHERE tax_rate_id = %s", $key ) ) ? true : false;
	}
	...
	public static function get_rate_label( $key_or_rate ) {
		...
		if ( is_object( $key_or_rate ) ) {
			...
			$key       = $key_or_rate;
			$rate_name = $mndb->get_var( $mndb->prepare( "SELECT tax_rate_name FROM {$mndb->prefix}woocommerce_tax_rates WHERE tax_rate_id = %s", $key ) );
		}
	...
	public static function get_rate_percent( $key_or_rate ) {
		...
		if ( is_object( $key_or_rate ) ) {
			...
			$key      = $key_or_rate;
			$tax_rate = $mndb->get_var( $mndb->prepare( "SELECT tax_rate FROM {$mndb->prefix}woocommerce_tax_rates WHERE tax_rate_id = %s", $key ) );
		}
	...
	public static function get_rate_code( $key_or_rate ) {
		...
		if ( is_object( $key_or_rate ) ) {
			...
			$key  = $key_or_rate;
			$rate = $mndb->get_row( $mndb->prepare( "SELECT * FROM {$mndb->prefix}woocommerce_tax_rates WHERE tax_rate_id = %s", $key ) );
		}
	...
	public static function _insert_tax_rate( $tax_rate ) {
		...
		$mndb->insert( $mndb->prefix . 'woocommerce_tax_rates', self::prepare_tax_rate( $tax_rate ) );

		WC_Cache_Helper::incr_cache_prefix( 'taxes' );

		do_action( 'woocommerce_tax_rate_added', $mndb->insert_id, $tax_rate );

		return $mndb->insert_id;
	}
	...
	public static function _get_tax_rate( $tax_rate_id, $output_type = ARRAY_A ) {
		...
		return $mndb->get_row( $mndb->prepare( "
			SELECT *
			FROM {$mndb->prefix}woocommerce_tax_rates
			WHERE tax_rate_id = %d
	...
	public static function _delete_tax_rate( $tax_rate_id ) {
		...
		$mndb->query( $mndb->prepare( "DELETE FROM {$mndb->prefix}woocommerce_tax_rate_locations WHERE tax_rate_id = %d;", $tax_rate_id ) );
		$mndb->query( $mndb->prepare( "DELETE FROM {$mndb->prefix}woocommerce_tax_rates WHERE tax_rate_id = %d;", $tax_rate_id ) );
	...
	private static function _update_tax_rate_locations( $tax_rate_id, $values, $type ) {
		...
		$mndb->query(
			$mndb->prepare( "
				DELETE FROM {$mndb->prefix}woocommerce_tax_rate_locations WHERE tax_rate_id = %d AND location_type = %s;
				", $tax_rate_id, $type
			)
		);
		...
		if ( sizeof( $values ) > 0 ) {
			...
			$mndb->query( "
				INSERT INTO {$mndb->prefix}woocommerce_tax_rate_locations ( location_code, tax_rate_id, location_type ) VALUES $sql;
				" );
		}
	...
	public static function get_rates_for_tax_class( $tax_class ) {
		...
		$rates     = $mndb->get_results( $mndb->prepare( "SELECT * FROM `{$mndb->prefix}woocommerce_tax_rates` WHERE `tax_rate_class` = %s ORDER BY `tax_rate_order`;", sanitize_title( $tax_class ) ) );
		$locations = $mndb->get_results( "SELECT * FROM `{$mndb->prefix}woocommerce_tax_rate_locations`" );

		if ( ! empty( $rates ) ) {
			// Set the rates keys equal to their ids.
			$rates = array_combine( mn_list_pluck( $rates, 'tax_rate_id' ), $rates );
		}
