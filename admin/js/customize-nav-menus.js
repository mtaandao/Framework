/* global _mnCustomizeNavMenusSettings, mnNavMenu, console */
( function( api, mn, $ ) {
	'use strict';

	/**
	 * Set up mnNavMenu for drag and drop.
	 */
	mnNavMenu.originalInit = mnNavMenu.init;
	mnNavMenu.options.menuItemDepthPerLevel = 20;
	mnNavMenu.options.sortableItems         = '> .customize-control-nav_menu_item';
	mnNavMenu.options.targetTolerance       = 10;
	mnNavMenu.init = function() {
	...
	if ( 'undefined' !== typeof _mnCustomizeNavMenusSettings ) {
		$.extend( api.Menus.data, _mnCustomizeNavMenusSettings );
	}
	...
	/**
	 * mn.customize.Menus.AvailableItemModel
	 *
	 * A single available menu item model. See PHP's MN_Customize_Nav_Menu_Item_Setting class.
	 *
	 * @constructor
	...
	/**
	 * mn.customize.Menus.AvailableItemCollection
	 *
	...
	/**
	 * mn.customize.Menus.AvailableMenuItemsPanelView
	 *
	 ...
	 */
	api.Menus.AvailableMenuItemsPanelView = mn.Backbone.View.extend({
		...
		doSearch: function( page ) {
			var self = this, params,
				...
				itemTemplate = mn.template( 'available-menu-item' );
			...
			if ( page < 0 ) {
				...
				mn.a11y.speak( api.Menus.data.l10n.itemsLoadingMore );
			...
			self.currentRequest = mn.ajax.post( 'search-available-menu-items-customizer', params );
			...
			self.currentRequest.done(function( data ) {
				...
				if ( items && page > 1 ) {
					mn.a11y.speak( api.Menus.data.l10n.itemsFoundMore.replace( '%d', items.length ) );
				} else if ( items && page === 1 ) {
					mn.a11y.speak( api.Menus.data.l10n.itemsFound.replace( '%d', items.length ) );
				}
			});
		...
		loadItems: function( type, object ) {
			...
			request = mn.ajax.post( 'load-available-menu-items-customizer', params );
	...
	/**
	 * mn.customize.Menus.MenusPanel
	 *
	 * Customizer panel for menus. This is used only for screen options management.
	 * Note that 'menus' must match the MN_Customize_Menu_Panel::$type.
	 *
	 * @constructor
	 * @augments mn.customize.Panel
	 */
	api.Menus.MenusPanel = api.Panel.extend({
		...
		saveManageColumnsState: _.debounce( function() {
			...
			panel._updateHiddenColumnsRequest = mn.ajax.post( 'hidden-columns', {
				hidden: panel.hidden(),
	...
	/**
	 * mn.customize.Menus.MenuSection
	 *
	 * Customizer section for menus. This is used only for lazy-loading child controls.
	 * Note that 'nav_menu' must match the MN_Customize_Menu_Section::$type.
	 *
	 * @constructor
	 * @augments mn.customize.Section
	 */
	api.Menus.MenuSection = api.Section.extend({
		...
		onChangeExpanded: function( expanded, args ) {
			...
			if ( expanded ) {
				mnNavMenu.menuList = section.container.find( '.accordion-section-content:first' );
				mnNavMenu.targetList = mnNavMenu.menuList;

				// Add attributes needed by mnNavMenu
				$( '#menu-to-edit' ).removeAttr( 'id' );
				mnNavMenu.menuList.attr( 'id', 'menu-to-edit' ).addClass( 'menu' );
				...
				if ( 'resolved' !== section.deferred.initSortables.state() ) {
					mnNavMenu.initSortables(); // Depends on menu-to-edit ID being set above.
					section.deferred.initSortables.resolve( mnNavMenu.menuList ); // Now MenuControl can extend the sortable.

					// @todo Note that mn.customize.reflowPaneContents() is debounced, so this immediate change will show a slight flicker while priorities get updated.
	...
	/**
	 * mn.customize.Menus.NewMenuSection
	 *
	 * Customizer section for new menus.
	 * Note that 'new_menu' must match the MN_Customize_New_Menu_Section::$type.
	 *
	 * @constructor
	 * @augments mn.customize.Section
	 */
	api.Menus.NewMenuSection = api.Section.extend({
		...
		onChangeExpanded: function( expanded ) {
			var section = this,
				...
				customizer = section.container.closest( '.mn-full-overlay-sidebar-content' );
			if ( expanded ) {
	...
	/**
	 * mn.customize.Menus.MenuLocationControl
	 *
	 * Customizer control for menu locations (rendered as a <select>).
	 * Note that 'nav_menu_location' must match the MN_Customize_Nav_Menu_Location_Control::$type.
	 *
	 * @constructor
	 * @augments mn.customize.Control
	 */
	...
	/**
	 * mn.customize.Menus.MenuItemControl
	 *
	 * Customizer control for menu items.
	 * Note that 'menu_item' must match the MN_Customize_Menu_Item_Control::$type.
	 *
	 * @constructor
	 * @augments mn.customize.Control
	 */
	api.Menus.MenuItemControl = api.Control.extend({
		...
		_setupReorderUI: function() {
			...
			template = mn.template( 'menu-item-reorder-nav' );
		...
		_setupRemoveUI: function() {
			...
			$removeBtn.on( 'click', function() {
				...
				control.container.slideUp( function() {
					...
					mn.a11y.speak( api.Menus.data.l10n.itemDeleted );
		...
		/**
		 * @return {mn.customize.controlConstructor.nav_menu|null}
		 */
		...
		moveUp: function() {
			...
			mn.a11y.speak( api.Menus.data.l10n.movedUp );
		},
		...
		moveDown: function() {
			this._changePosition( 1 );
			mn.a11y.speak( api.Menus.data.l10n.movedDown );
		},
		/**
		...
		moveLeft: function() {
			this._changeDepth( -1 );
			mn.a11y.speak( api.Menus.data.l10n.movedLeft );
		},
		...
		moveRight: function() {
			this._changeDepth( 1 );
			mn.a11y.speak( api.Menus.data.l10n.movedRight );
		},
	...
	/**
	 * mn.customize.Menus.MenuNameControl
	 *
	...
	/**
	 * mn.customize.Menus.MenuAutoAddControl
	 *
	...
	/**
	 * mn.customize.Menus.MenuControl
	 *
	 * Customizer control for menus.
	 * Note that 'nav_menu' must match the MN_Menu_Customize_Control::$type
	 *
	 * @constructor
	 * @augments mn.customize.Control
	 */
	api.Menus.MenuControl = api.Control.extend({
		 ...
		 *
		 * Notice that the UI aspects here are handled by mnNavMenu.initSortables()
			...
			menuList.on( 'sortstop', function() {
				setTimeout( function() { // Next tick.
					...
					_.each( menuItemControls, function( menuItemControl ) {
						...
						// Note that mnNavMenu will be setting this .menu-item-data-parent-id input's value.
			...
			if ( section && section.expanded() ) {
				section.collapse({
					completeCallback: function() {
						removeSection();
						mn.a11y.speak( api.Menus.data.l10n.menuDeleted );
		...
		toggleReordering: function( showOrHide ) {
			...
			if ( this.isReordering ) {
				...
				mn.a11y.speak( api.Menus.data.l10n.reorderModeOn );
				...
				mn.a11y.speak( api.Menus.data.l10n.reorderModeOff );
		...
		/**
		 * @return {mn.customize.controlConstructor.nav_menu_item[]}
		 */
		 ...
		 * @returns {mn.customize.Menus.controlConstructor.nav_menu_item} The newly-created nav_menu_item control instance.
		 */
		addItemToMenu: function( item ) {
			...
			mn.a11y.speak( api.Menus.data.l10n.itemAdded );
	...
	/**
	 * mn.customize.Menus.NewMenuControl
	 *
	 * Customizer control for creating new menus and handling deletion of existing menus.
	 * Note that 'new_menu' must match the MN_Customize_New_Menu_Control::$type.
	 *
	 * @constructor
	 * @augments mn.customize.Control
	 */
	api.Menus.NewMenuControl = api.Control.extend({
		...
		submit: function() {
			...
			mn.a11y.speak( api.Menus.data.l10n.menuAdded );
	...
	/**
	 * Extends mn.customize.controlConstructor with control constructor for
	...
	/**
	 * Extends mn.customize.panelConstructor with section constructor for menus.
	 */
	...
	/**
	 * Extends mn.customize.sectionConstructor with section constructor for menu.
	 */
	...
	api.Menus.applySavedData = function( data ) {
		...
		_( data.nav_menu_updates ).each(function( update ) {
			...
			if ( 'inserted' === update.status ) {
				...
				mn.customize.control.each(function( control ){
					if ( /^nav_menu_locations\[/.test( control.id ) ) {
				...
				api.each( function( setting ) {
					...
					if ( /^nav_menu_locations\[/.test( setting.id ) && setting.get() === update.previous_term_id ) {
						...
						setting._dirty = false; // Not dirty because this is has also just been done on server in MN_Customize_Nav_Menu_Setting::update().
	 ...
	 *
	 * @param menuId
	 * @return {mn.customize.controlConstructor.menus[]}
	 */
