 ...
 *
 * Used internally by the MN_Embed class, but is designed to be generic.
 *
 * @link https://mtaandao.co.ke/docs/oEmbed oEmbed Codex Article
...
class MN_oEmbed {
	...
	public function __construct() {
		...
		$providers = array(
			...
			'#https?://Mtaandao.tv/.*#i'                         => array( 'http://Mtaandao.tv/oembed/',                               true  ),
			...
			'#https?://(www\.)?issuu\.com/.+/docs/.+#i'           => array( 'http://issuu.com/oembed_mn',                                true  ),
			...
			'#https?://videopress.com/v/.*#'                      => array( 'https://public-api.Mtaandao.com/oembed/1.0/?for=' . $host, true  ),
		 ...
		 *
		 * Since Mtaandao 4.4, oEmbed discovery is enabled for all users and allows embedding of sanitized
		 ...
		 * | Mtaandao.tv | Mtaandao.tv          |      Yes       | 2.9.0     |
		...
		// Fix any embeds that contain new lines in the middle of the HTML which breaks mnautop().
	 ...
	 *
	 * The provider is removed just-in-time when mn_oembed_add_provider() is called before
	 ...
	 *
	 * The provider is removed just-in-time when mn_oembed_remove_provider() is called before
	 * the {@see 'plugins_loaded'} hook.
	 *
	...
	public function discover( $url ) {
		...
		// Fetch URL content
		$request = mn_safe_remote_get( $url, $args );
		if ( $html = mn_remote_retrieve_body( $request ) ) {
	...
	public function fetch( $provider, $url, $args = '' ) {
		$args = mn_parse_args( $args, mn_embed_defaults( $url ) );
		...
		foreach ( array( 'json', 'xml' ) as $format ) {
			...
			if ( is_mn_error( $result ) && 'not-implemented' == $result->get_error_code() )
				continue;
			return ( $result && ! is_mn_error( $result ) ) ? $result : false;
		}
		return false;
	}
	 ...
	 * @return false|object|MN_Error False on failure, otherwise the result in the form of an object.
	 */
	private function _fetch_with_format( $provider_url_with_args, $format ) {
		$provider_url_with_args = add_query_arg( 'format', $format, $provider_url_with_args );

		/** This filter is documented in res/class-oembed.php */
		$args = apply_filters( 'oembed_remote_get_args', array(), $provider_url_with_args );

		$response = mn_safe_remote_get( $provider_url_with_args, $args );
		if ( 501 == mn_remote_retrieve_response_code( $response ) )
			return new MN_Error( 'not-implemented' );
		if ( ! $body = mn_remote_retrieve_body( $response ) )
			return false;
	...
	/**
	 * Converts a data object from MN_oEmbed::fetch() and returns the HTML.
	 *
	 * @since 2.9.0
	 * @access public
	 *
	 ...
	 * @param object $data Data object from MN_oEmbed::data2html()
