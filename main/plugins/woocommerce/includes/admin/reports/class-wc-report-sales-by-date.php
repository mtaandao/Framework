...
class WC_Report_Sales_By_Date extends WC_Admin_Report {
	...
	private function query_report_data() {
		...
		$this->report_data->total_tax          = wc_format_decimal( array_sum( mn_list_pluck( $this->report_data->orders, 'total_tax' ) ) - $this->report_data->total_tax_refunded, 2 );
		$this->report_data->total_shipping     = wc_format_decimal( array_sum( mn_list_pluck( $this->report_data->orders, 'total_shipping' ) ) - $this->report_data->total_shipping_refunded, 2 );
		$this->report_data->total_shipping_tax = wc_format_decimal( array_sum( mn_list_pluck( $this->report_data->orders, 'total_shipping_tax' ) ) - $this->report_data->total_shipping_tax_refunded, 2 );

		// Total the refunds and sales amounts. Sales subract refunds.
		$this->report_data->total_sales = wc_format_decimal( array_sum( mn_list_pluck( $this->report_data->orders, 'total_sales' ) ) - $this->report_data->total_refunds, 2 );
		...
		$this->report_data->total_orders          = absint( array_sum( mn_list_pluck( $this->report_data->order_counts, 'count' ) ) );
		$this->report_data->total_coupons         = number_format( array_sum( mn_list_pluck( $this->report_data->coupons, 'discount_amount' ) ), 2, '.', '' );
		$this->report_data->total_refunded_orders = absint( count( $this->report_data->full_refunds ) );

		// Item counts
		$this->report_data->total_items = absint( array_sum( mn_list_pluck( $this->report_data->order_items, 'order_item_count' ) ) ) - $this->report_data->refunded_order_items;
					...
					main_chart = jQuery.plot(
						...
						series,
						{
							...
							xaxes: [ {
								...
								monthNames: <?php echo json_encode( array_values( $mn_locale->month_abbrev ) ) ?>,
								tickLength: 1,
