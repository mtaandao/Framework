...
class WC_Checkout {
	...
	public function create_order() {
		...
		try {
			...
			return new MN_Error( 'checkout-error', $e->getMessage() );
		}
	...
	public function process_checkout() {
		try {
			if ( empty( $_POST['_mnnonce'] ) || ! mn_verify_nonce( $_POST['_mnnonce'], 'woocommerce-process_checkout' ) ) {
			...
			foreach ( $this->checkout_fields as $fieldset_key => $fieldset ) {
				...
				foreach ( $fieldset as $key => $field ) {
					...
					switch ( $field['type'] ) {
						...
						break;
						case "textarea" :
							$this->posted[ $key ] = isset( $_POST[ $key ] ) ? mn_strip_all_tags( mn_check_invalid_utf8( stripslashes( $_POST[ $key ] ) ) ) : '';
						break;
						default :
			...
			if ( ! isset( $_POST['woocommerce_checkout_update_totals'] ) && wc_notice_count( 'error' ) == 0 ) {
				...
				if ( ! is_user_logged_in() && ( $this->must_create_account || ! empty( $this->posted['createaccount'] ) ) ) {
					...
					if ( $this->posted['billing_first_name'] && apply_filters( 'woocommerce_checkout_update_customer_data', true, $this ) ) {
						...
						mn_update_user( apply_filters( 'woocommerce_checkout_customer_userdata', $userdata, $this ) );
					}
				}
