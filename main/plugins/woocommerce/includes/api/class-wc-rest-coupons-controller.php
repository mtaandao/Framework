...
class WC_REST_Coupons_Controller extends WC_REST_Posts_Controller {
	...
	public function query_args( $args, $request ) {
		...
		if ( ! empty( $request['code'] ) ) {
			$id = $mndb->get_var( $mndb->prepare( "SELECT id FROM $mndb->posts WHERE post_title = %s AND post_type = 'shop_coupon' AND post_status = 'publish'", $request['code'] ) );
	...
	public function prepare_item_for_response( $post, $request ) {
		...
		// Get the coupon code.
		$code = $mndb->get_var( $mndb->prepare( "SELECT post_title FROM $mndb->posts WHERE id = %s AND post_type = 'shop_coupon' AND post_status = 'publish'", $post->ID ) );
	...
	protected function prepare_item_for_database( $request ) {
		...
		if ( 'POST' === $request->get_method() && empty( $data->ID ) ) {
			if ( empty( $request['code'] ) ) {
				return new MN_Error( 'woocommerce_rest_empty_coupon_code', sprintf( __( 'The coupon code cannot be empty.', 'woocommerce' ), 'code' ), array( 'status' => 400 ) );
			}
		}
		...
		if ( ! empty( $schema['properties']['code'] ) && ! empty( $request['code'] ) ) {
			...
			if ( $coupon_found ) {
				return new MN_Error( 'woocommerce_rest_coupon_code_already_exists', __( 'The coupon code already exists', 'woocommerce' ), array( 'status' => 400 ) );
			}
		...
		if ( ! empty( $schema['properties']['excerpt'] ) && isset( $request['description'] ) ) {
			$data->post_excerpt = mn_filter_post_kses( $request['description'] );
		}
