...
class Vc_Templates_Panel_Editor {
	...
	public function init() {
		 ...
		 */
		add_action( 'mn_ajax_vc_save_template', array(
			&$this,
			'save',
		) );
		add_action( 'mn_ajax_vc_backend_load_template', array(
			&$this,
			'renderBackendTemplate',
		) );
		add_action( 'mn_ajax_vc_frontend_load_template', array(
			&$this,
			'renderFrontendTemplate',
		) );
		add_action( 'mn_ajax_vc_load_template_preview', array(
			&$this,
			'renderTemplatePreview',
		) );
		add_action( 'mn_ajax_vc_delete_template', array(
			&$this,
			'delete',
		) );
...
HTML;
	...
	function renderFrontendTemplate() {
		...
		add_action( 'mn_print_scripts', array(
			&$this,
	...
	public function save() {
		vc_user_access()->checkAdminNonce()->validateDie()->mnAny( 'edit_posts', 'edit_pages' )->validateDie()->part( 'templates' )->checkStateAny( true, null )->validateDie();
	...
	public function registerPreviewScripts() {
		...
		mn_register_script( 'vc_editors-templates-preview-js', vc_asset_url( 'js/editors/templates-preview.js' ), array(
			'vc-backend-min-js',
		), WPB_VC_VERSION, true );
	}
	...
	public function enqueuePreviewScripts() {
		...
		mn_enqueue_script( 'vc_editors-templates-preview-js' );
	}
	 ...
	 *
	 * Used by action 'mn_print_scripts'.
	 *
	...
	public function addScriptsToTemplatePreview() {
		// mn_enqueue_script( 'vc-template-preview-script', vc_asset_url( 'js/editors/vc_ui-panel-templates-preview-be.js' ), array( 'vc-backend-min-js' ), WPB_VC_VERSION, true );
	}
