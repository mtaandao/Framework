...
class WC_Admin_Settings {
	...
	public static function output() {
		...
		mn_enqueue_script( 'woocommerce_settings', WC()->plugin_url() . '/assets/js/admin/settings' . $suffix . '.js', array( 'jquery', 'jquery-ui-datepicker', 'jquery-ui-sortable', 'iris', 'select2' ), WC()->version, true );

		mn_localize_script( 'woocommerce_settings', 'woocommerce_settings_params', array(
		...
		foreach ( $options as $value ) {
			...
			switch ( $value['type'] ) {
				...
				case 'title':
					...
					if ( ! empty( $value['desc'] ) ) {
						echo mnautop( mntexturize( mn_kses_post( $value['desc'] ) ) );
					}
				...
				case 'single_select_page' :
					...
					if ( isset( $value['args'] ) ) {
						$args = mn_parse_args( $value['args'], $args );
					}

					?><tr valign="top" class="single_select_page">
						<th scope="row" class="titledesc"><?php echo esc_html( $value['title'] ) ?> <?php echo $tooltip_html; ?></th>
						<td class="forminp">
							<?php echo str_replace(' id=', " data-placeholder='" . esc_attr__( 'Select a page&hellip;', 'woocommerce' ) .  "' style='" . $value['css'] . "' class='" . $value['class'] . "' id=", mn_dropdown_pages( $args ) ); ?> <?php echo $description; ?>
						</td>
					</tr><?php
					break;
	...
	public static function get_field_description( $value ) {
		...
		if ( $description && in_array( $value['type'], array( 'textarea', 'radio' ) ) ) {
			$description = '<p style="margin-top:0">' . mn_kses_post( $description ) . '</p>';
		} elseif ( $description && in_array( $value['type'], array( 'checkbox' ) ) ) {
			$description = mn_kses_post( $description );
		} elseif ( $description ) {
			$description = '<span class="description">' . mn_kses_post( $description ) . '</span>';
		}
	...
	public static function save_fields( $options ) {
		...
		foreach ( $options as $option ) {
			...
			if ( strstr( $option['id'], '[' ) ) {
				...
				$raw_value    = isset( $_POST[ $option_name ][ $setting_name ] ) ? mn_unslash( $_POST[ $option_name ][ $setting_name ] ) : null;
			} else {
				$option_name  = $option['id'];
				$setting_name = '';
				$raw_value    = isset( $_POST[ $option['id'] ] ) ? mn_unslash( $_POST[ $option['id'] ] ) : null;
			}
			...
			switch ( $option['type'] ) {
				case 'checkbox' :
					...
					break;
				case 'textarea' :
					$value = mn_kses_post( trim( $raw_value ) );
					break;
				case 'multiselect' :
				case 'multi_select_countries' :
