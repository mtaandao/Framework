...
( function( tinymce ) {
// Set the minimum value for the modals z-index higher than #mnadminbar (100000)
tinymce.ui.FloatPanel.zIndex = 100100;

tinymce.PluginManager.add( 'Mtaandao', function( editor ) {
	var mnAdvButton, style,
		DOM = tinymce.DOM,
		each = tinymce.each,
		__ = editor.editorManager.i18n.translate,
		$ = window.jQuery,
		mn = window.mn,
		hasWpautop = ( mn && mn.editor && mn.editor.autop && editor.getParam( 'mnautop', true ) );
	...
	function toggleToolbars( state ) {
		...
		if ( pixels && ! initial ) {
			...
			if ( state === 'hide' ) {
				...
				mnAdvButton && mnAdvButton.active( false );
			} else {
				setUserSetting('hidetb', '1');
				mnAdvButton && mnAdvButton.active( true );
			}
		}

		editor.fire( 'mn-toolbar-toggle' );
	}

	// Add the kitchen sink button :)
	editor.addButton( 'mn_adv', {
		tooltip: 'Toolbar Toggle',
		cmd: 'MN_Adv',
		onPostRender: function() {
			mnAdvButton = this;
			mnAdvButton.active( getUserSetting( 'hidetb' ) === '1' ? true : false );
		}
	});

	// Hide the toolbars after loading
	editor.on( 'PostRender', function() {
		if ( editor.getParam( 'Mtaandao_adv_hidden', true ) && getUserSetting( 'hidetb', '0' ) === '0' ) {
			toggleToolbars( 'hide' );
		}
	});

	editor.addCommand( 'MN_Adv', function() {
		toggleToolbars();
	});

	editor.on( 'focus', function() {
        window.mnActiveEditor = editor.id;
    });
		...
		if ( event.content ) {
			if ( event.content.indexOf( '<!--more' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--more(.*?)-->/g, function( match, moretext ) {
					return '<img src="' + tinymce.Env.transparentSrc + '" data-mn-more="more" data-mn-more-text="' + moretext + '" ' +
						'class="mn-more-tag mce-mn-more" alt="" title="' + title + '" data-mce-resize="false" data-mce-placeholder="1" />';
				});
			}
			...
			if ( event.content.indexOf( '<!--nextpage-->' ) !== -1 ) {
				...
				event.content = event.content.replace( /<!--nextpage-->/g,
					'<img src="' + tinymce.Env.transparentSrc + '" data-mn-more="nextpage" class="mn-more-tag mce-mn-nextpage" ' +
						'alt="" title="' + title + '" data-mce-resize="false" data-mce-placeholder="1" />' );
			}

			if ( event.load && event.format !== 'raw' && hasWpautop ) {
				event.content = mn.editor.autop( event.content );
			}
			...
			if ( event.content.indexOf( '<script' ) !== -1 || event.content.indexOf( '<style' ) !== -1 ) {
				event.content = event.content.replace( /<(script|style)[^>]*>[\s\S]*?<\/\1>/g, function( match, tag ) {
					return '<img ' +
						...
						'data-mn-preserve="' + encodeURIComponent( match ) + '" ' +
	...
	editor.on( 'PostProcess', function( event ) {
		if ( event.get ) {
			event.content = event.content.replace(/<img[^>]+>/g, function( image ) {
				...
				if ( image.indexOf( 'data-mn-more="more"' ) !== -1 ) {
					if ( match = image.match( /data-mn-more-text="([^"]+)"/ ) ) {
						moretext = match[1];
					}

					string = '<!--more' + moretext + '-->';
				} else if ( image.indexOf( 'data-mn-more="nextpage"' ) !== -1 ) {
					string = '<!--nextpage-->';
				} else if ( image.indexOf( 'data-mn-preserve' ) !== -1 ) {
					if ( match = image.match( / data-mn-preserve="([^"]+)"/ ) ) {
	...
	editor.on( 'ResolveName', function( event ) {
		...
		if ( event.target.nodeName === 'IMG' && ( attr = editor.dom.getAttrib( event.target, 'data-mn-more' ) ) ) {
			event.name = attr;
		}
	});

	// Register commands
	editor.addCommand( 'MN_More', function( tag ) {
		var parent, html, title,
			classname = 'mn-more-tag',
			dom = editor.dom,
			node = editor.selection.getNode();

		tag = tag || 'more';
		classname += ' mce-mn-' + tag;
		title = tag === 'more' ? 'Read more...' : 'Next page';
		title = __( title );
		html = '<img src="' + tinymce.Env.transparentSrc + '" alt="" title="' + title + '" class="' + classname + '" ' +
			'data-mn-more="' + tag + '" data-mce-resize="false" data-mce-placeholder="1" />';
	...
	editor.addCommand( 'MN_Code', function() {
		editor.formatter.toggle('code');
	});

	editor.addCommand( 'MN_Page', function() {
		editor.execCommand( 'MN_More', 'nextpage' );
	});

	editor.addCommand( 'MN_Help', function() {
		...
		if ( editor.plugins.mntextpattern && ( ! tinymce.Env.ie || tinymce.Env.ie > 8 ) ) {
		...
		if ( dialog.$el ) {
			...
			$wrap = dialog.$el.find( '.mce-mn-help' );
	...
	editor.addCommand( 'MN_Medialib', function() {
		if ( mn && mn.media && mn.media.editor ) {
			mn.media.editor.open( editor.id );
		}
	});

	// Register buttons
	editor.addButton( 'mn_more', {
		tooltip: 'Insert Read More tag',
		onclick: function() {
			editor.execCommand( 'MN_More', 'more' );
		}
	});

	editor.addButton( 'mn_page', {
		tooltip: 'Page break',
		onclick: function() {
			editor.execCommand( 'MN_More', 'nextpage' );
		}
	...
	editor.addButton( 'mn_help', {
		tooltip: 'Keyboard Shortcuts',
		cmd: 'MN_Help'
	});

	editor.addButton( 'mn_code', {
		tooltip: 'Code',
		cmd: 'MN_Code',
		stateSelector: 'code'
	});
	...
	editor.on( 'init', function() {
		...
		dom.setAttrib( doc.documentElement, 'lang', editor.getParam( 'mn_lang_attr' ) );
		...
		if ( editor.getParam( 'mn_paste_filters', true ) ) {
	...
	editor.on( 'SaveContent', function( event ) {
		...
		if ( hasWpautop ) {
			event.content = mn.editor.removep( event.content );
		}
	});
	...
	editor.on( 'preInit', function() {
		...
		editor.addShortcut( 'meta+s', '', function() {
			if ( mn && mn.autosave ) {
				mn.autosave.server.triggerSave();
			}
		} );
	...
	editor.on( 'preinit', function() {
		var Factory = tinymce.ui.Factory,
			...
			mnAdminbar = document.getElementById( 'mnadminbar' ),
			...
			if ( editor.id === 'content' ) {
				mnStatusbar = document.getElementById( 'post-status-info' );
			}
		...
		function create( buttons, bottom ) {
			...
			function reposition() {
				...
				var scrollX = window.pageXOffset || document.documentElement.scrollLeft,
					...
					mnAdminbarBottom = mnAdminbar ? mnAdminbar.getBoundingClientRect().bottom : 0,
					...
					mnStatusbarTop = mnStatusbar ? windowHeight - mnStatusbar.getBoundingClientRect().top : 0,
					blockedTop = Math.max( 0, mnAdminbarBottom, mceToolbarBottom, iframeRect.top ),
					blockedBottom = Math.max( 0, mceStatusbarTop, mnStatusbarTop, windowHeight - iframeRect.bottom ),
		...
		editor.mn = editor.mn || {};
		editor.mn._createToolbar = create;
	}, true );
