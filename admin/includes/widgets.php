<?php
/**
 * Mtaandao Widgets Administration API
 *
 * @package Mtaandao
...
function mn_list_widgets() {
	...
	foreach ( $sort as $widget ) {
		...
		if ( isset($mn_registered_widget_controls[$widget['id']]['id_base']) && isset($widget['params'][0]['number']) ) {
		...
		$args = mn_list_widget_controls_dynamic_sidebar( array( 0 => $args, 1 => $widget['params'][0] ) );
 ...
 */
function mn_list_widget_controls( $sidebar, $sidebar_name = '' ) {
	add_filter( 'dynamic_sidebar_params', 'mn_list_widget_controls_dynamic_sidebar' );

	$description = mn_sidebar_description( $sidebar );
 ...
 * @return array
 */
function mn_list_widget_controls_dynamic_sidebar( $params ) {
	...
	if ( is_callable( $mn_registered_widgets[$widget_id]['callback'] ) ) {
		$mn_registered_widgets[$widget_id]['_callback'] = $mn_registered_widgets[$widget_id]['callback'];
		$mn_registered_widgets[$widget_id]['callback'] = 'mn_widget_control';
	}
 ...
 * @return array
 */
function mn_widget_control( $sidebar_args ) {
	global $mn_registered_widgets, $mn_registered_widget_controls, $sidebars_widgets;

	$widget_id = $sidebar_args['widget_id'];
	$sidebar_id = isset($sidebar_args['id']) ? $sidebar_args['id'] : false;
	$key = $sidebar_id ? array_search( $widget_id, $sidebars_widgets[$sidebar_id] ) : '-1'; // position of widget in sidebar
	$control = isset($mn_registered_widget_controls[$widget_id]) ? $mn_registered_widget_controls[$widget_id] : array();
	$widget = $mn_registered_widgets[$widget_id];
	...
	$mn_registered_widgets[$widget_id]['callback'] = $mn_registered_widgets[$widget_id]['_callback'];
	unset($mn_registered_widgets[$widget_id]['_callback']);
...
<?php echo ( $widget_description = mn_widget_description($widget_id) ) ? "$widget_description\n" : "$widget_title\n"; ?>
	</div>
<?php
 ...
 * @return string
 */
function mn_widgets_access_body_class($classes) {
