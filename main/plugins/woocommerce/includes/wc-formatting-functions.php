...
function wc_sanitize_tooltip( $var ) {
	return htmlspecialchars( mn_kses( html_entity_decode( $var ), array(
		'br'     => array(),
		'em'     => array(),
		'strong' => array(),
		'small'  => array(),
...
function wc_price( $price, $args = array() ) {
	extract( apply_filters( 'wc_price_args', mn_parse_args( $args, array(
		'ex_tax_label'       => false,
		'currency'           => '',
 ...
 * a MN core method exists (see https://core.trac.wordpress.org/ticket/24730).
 *
...
function wc_format_content( $raw_string ) {
	return apply_filters( 'woocommerce_format_content', do_shortcode( shortcode_unautop( mnautop( $raw_string ) ) ), $raw_string );
}
...
function wc_format_product_short_description( $content ) {
	...
	if ( class_exists( 'MNCom_Markdown' ) ) {
		$markdown = MNCom_Markdown::get_instance();

		return mnautop( $markdown->transform( $content, array( 'unslash' => false ) ) );
	}
...
function wc_format_option_hold_stock_minutes( $value, $option, $raw_value ) {
	...
	if ( '' !== $value ) {
		mn_schedule_single_event( time() + ( absint( $value ) * 60 ), 'woocommerce_cancel_unpaid_orders' );
	}
...
function wc_sanitize_term_text_based( $term ) {
	return trim( mn_unslash( strip_tags( $term ) ) );
}
