...
jQuery( function ( $ ) {
	...
	UploadButton = views.UploadButton = Backbone.View.extend( {
		...
		tagName: 'div',
		template: mn.template( 'rwmb-upload-area' ),
		render: function () {
		...
		initialize: function ( options ) {
			...
			if ( mn.Uploader.browser.supported ) {
				this.initUploader();
			}
		...
		initUploader: function () {
			...
			this.uploader.bind( 'FilesAdded', function ( up, files ) {
				_.each( files, function ( file ) {
					...
					attributes = _.extend( {
						...
						filename: file.name,
						menuOrder: 0,
						uploadedTo: mn.media.model.settings.post.id,
					...
					file.attachment = mn.media.model.Attachment.create( attributes );
					mn.Uploader.queue.add( file.attachment );
			...
			this.uploader.bind( 'FileUploaded', function ( up, file, response ) {
				...
				mn.media.model.Attachment.get( response.data.id, file.attachment );

				complete = mn.Uploader.queue.all( function ( attachment ) {
					return ! attachment.get( 'uploading' );
				} );

				if ( complete ) {
					mn.Uploader.queue.reset();
				}
			} );
