...
class WC_Cache_Helper {
	...
	/**
	 * Get prefix for use with mn_cache_set. Allows all cache in a group to be invalidated at once.
	 * @param  string $group
	 * @return string
	 */
	public static function get_cache_prefix( $group ) {
		// Get cache key - uses cache key wc_orders_cache_prefix to invalidate when needed
		$prefix = mn_cache_get( 'wc_' . $group . '_cache_prefix', $group );

		if ( false === $prefix ) {
			$prefix = 1;
			mn_cache_set( 'wc_' . $group . '_cache_prefix', $prefix, $group );
		}
	...
	public static function incr_cache_prefix( $group ) {
		mn_cache_incr( 'wc_' . $group . '_cache_prefix', 1, $group );
	}
	...
	public static function geolocation_ajax_redirect() {
		if ( 'geolocation_ajax' === get_option( 'woocommerce_default_customer_address' ) && ! is_checkout() && ! is_cart() && ! is_account_page() && ! is_ajax() && empty( $_POST ) ) {
			...
			if ( empty( $current_hash ) || $current_hash !== $location_hash ) {
				...
				$redirect_url = trailingslashit( home_url( $mn->request ) );
				...
				if ( ! get_option( 'permalink_structure' ) ) {
					$redirect_url = add_query_arg( $mn->query_string, '', $redirect_url );
				}

				$redirect_url = add_query_arg( 'v', $location_hash, remove_query_arg( 'v', $redirect_url ) );

				mn_safe_redirect( esc_url_raw( $redirect_url ), 307 );
				exit;
			}
		}
	}
	 ...
	 *
	 * When using default MN transients we're able to do this with a DB query to.
	...
	public static function delete_version_transients( $version = '' ) {
		if ( ! mn_using_ext_object_cache() && ! empty( $version ) ) {
			global $mndb;

			$limit    = apply_filters( 'woocommerce_delete_version_transients_limit', 1000 );
			$affected = $mndb->query( $mndb->prepare( "DELETE FROM {$mndb->options} WHERE option_name LIKE %s ORDER BY option_id LIMIT %d;", "\_transient\_%" . $version, $limit ) );

			// If affected rows is equal to limit, there are more rows to delete. Delete in 10 secs.
			if ( $affected === $limit ) {
				mn_schedule_single_event( time() + 10, 'delete_version_transients', array( $version ) );
			}
		}
	}
