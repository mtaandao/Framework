...
if ( ! function_exists( 'is_checkout_pay_page' ) ) {
	...
	function is_checkout_pay_page() {
		...
		return is_checkout() && ! empty( $mn->query_vars['order-pay'] );
	}
}
...
if ( ! function_exists( 'is_wc_endpoint_url' ) ) {
	...
	function is_wc_endpoint_url( $endpoint = false ) {
		...
		if ( $endpoint !== false ) {
			...
			return isset( $mn->query_vars[ $endpoint_var ] );
		} else {
...
if ( ! function_exists( 'is_view_order_page' ) ) {
	...
	*/
	function is_view_order_page() {
		...
		return ( is_page( wc_get_page_id( 'myaccount' ) ) && isset( $mn->query_vars['view-order'] ) );
	}
}
...
if ( ! function_exists( 'is_edit_account_page' ) ) {
	...
	*/
	function is_edit_account_page() {
		...
		return ( is_page( wc_get_page_id( 'myaccount' ) ) && isset( $mn->query_vars['edit-account'] ) );
	}
}
...
if ( ! function_exists( 'is_order_received_page' ) ) {
	...
	*/
	function is_order_received_page() {
		...
		return ( is_page( wc_get_page_id( 'checkout' ) ) && isset( $mn->query_vars['order-received'] ) );
	}
}
...
if ( ! function_exists( 'is_add_payment_method_page' ) ) {
	...
	*/
	function is_add_payment_method_page() {
		...
		return ( is_page( wc_get_page_id( 'myaccount' ) ) && isset( $mn->query_vars['add-payment-method'] ) );
	}
}
...
if ( ! function_exists( 'is_lost_password_page' ) ) {
	...
	*/
	function is_lost_password_page() {
		...
		return ( is_page( wc_get_page_id( 'myaccount' ) ) && isset( $mn->query_vars['lost-password'] ) );
	}
}
...
/**
 * Check if the checkout is configured for https. Look at options, MN HTTPS plugin, or the permalink itself.
 *
 * @since  2.5.0
 * @return bool
 */
function wc_checkout_is_https() {
	return wc_site_is_https() || 'yes' === get_option( 'woocommerce_force_ssl_checkout' ) || class_exists( 'MtaandaoHTTPS' ) || strstr( wc_get_page_permalink( 'checkout' ), 'https:' );
}
...
function wc_post_content_has_shortcode( $tag = '' ) {
	...
	return is_a( $post, 'MN_Post' ) && has_shortcode( $post->post_content, $tag );
}
