 ...
 *
 * This class is most useful in Mtaandao multi-network installations where the
...
class MN_Network {
	 ...
	 * @return MN_Network|bool The network's object if found. False if not.
	 */
	public static function get_instance( $network_id ) {
		...
		$_network = mn_cache_get( $network_id, 'networks' );

		if ( ! $_network ) {
			$_network = $mndb->get_row( $mndb->prepare( "SELECT * FROM {$mndb->site} WHERE id = %d LIMIT 1", $network_id ) );

			if ( empty( $_network ) || is_mn_error( $_network ) ) {
				return false;
			}

			mn_cache_add( $network_id, $_network, 'networks' );
		}

		return new MN_Network( $_network );
	}

	/**
	 * Create a new MN_Network object.
	 *
	 ...
	 * @access public
	 *
	 * @param MN_Network|object $network A network object.
	 */
	 ...
	 * @return MN_Network|bool Network object if successful. False when no network is found.
	 */
	public static function get_by_path( $domain = '', $path = '', $segments = null ) {
		...
		if ( mn_using_ext_object_cache() ) {
			$using_paths = mn_cache_get( 'networks_have_paths', 'site-options' );
			if ( false === $using_paths ) {
				$using_paths = (int) $mndb->get_var( "SELECT id FROM {$mndb->site} WHERE path <> '/' LIMIT 1" );
				mn_cache_add( 'networks_have_paths', $using_paths, 'site-options'  );
			}
		}
		...
		$search_domains = "'" . implode( "', '", $mndb->_escape( $domains ) ) . "'";

		if ( ! $using_paths ) {
			$network = $mndb->get_row( "
				SELECT * FROM {$mndb->site}
				WHERE domain IN ({$search_domains})
				ORDER BY CHAR_LENGTH(domain)
				DESC LIMIT 1
			" );

			if ( ! empty( $network ) && ! is_mn_error( $network ) ) {
				return new MN_Network( $network );
			}

			return false;

		} else {
			$search_paths = "'" . implode( "', '", $mndb->_escape( $paths ) ) . "'";
			$networks = $mndb->get_results( "
				SELECT * FROM {$mndb->site}
				WHERE domain IN ({$search_domains})
