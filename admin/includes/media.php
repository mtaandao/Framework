<?php
/**
 * Mtaandao Administration Media API.
 *
 * @package Mtaandao
...
function update_gallery_tab($tabs) {
	...
	if ( $post_id )
		$attachments = intval( $mndb->get_var( $mndb->prepare( "SELECT count(*) FROM $mndb->posts WHERE post_type = 'attachment' AND post_status != 'trash' AND post_parent = %d", $post_id ) ) );
...
function get_image_send_to_editor( $id, $caption, $title, $align, $url = '', $rel = false, $size = 'medium', $alt = '' ) {
	...
	if ( $rel ) {
		if ( is_string( $rel ) ) {
			...
			$rel = ' rel="attachment mn-att-' . intval( $id ) . '"';
		}
	} else {
		$rel = '';
	}
...
win.send_to_editor( <?php echo mn_json_encode( $html ); ?> );
</script>
<?php
	exit;
}
 ...
 * @param array  $overrides Override the mn_handle_upload() behavior. Optional.
 * @return int|MN_Error ID of the attachment or a MN_Error object on failure.
 */
function media_handle_upload($file_id, $post_id, $post_data = array(), $overrides = array( 'test_form' => false )) {
	...
	$file = mn_handle_upload($_FILES[$file_id], $overrides, $time);

	if ( isset($file['error']) )
		return new MN_Error( 'upload_error', $file['error'] );
	...
	if ( preg_match( '#^audio#', $type ) ) {
		$meta = mn_read_audio_metadata( $file );
	...
	} elseif ( 0 === strpos( $type, 'image/' ) && $image_meta = @mn_read_image_metadata( $file ) ) {
	...
	// Save the data
	$id = mn_insert_attachment($attachment, $file, $post_id);
	if ( !is_mn_error($id) ) {
		mn_update_attachment_metadata( $id, mn_generate_attachment_metadata( $id, $file ) );
	}
 ...
 * @return int|object The ID of the attachment or a MN_Error on failure.
 */
function media_handle_sideload( $file_array, $post_id, $desc = null, $post_data = array() ) {
	...
	$file = mn_handle_sideload( $file_array, $overrides, $time );
	if ( isset($file['error']) )
		return new MN_Error( 'upload_error', $file['error'] );
 ...
 */
function mn_iframe($content_func /* ... */) {
	_mn_admin_html_begin();
?>
<title><?php bloginfo('name') ?> &rsaquo; <?php _e('Uploads'); ?> &#8212; <?php _e('Mtaandao'); ?></title>
<?php
...
if ( ( is_array( $content_func ) && ! empty( $content_func[1] ) && 0 === strpos( (string) $content_func[1], 'media' ) )
	...
	mn_enqueue_style( 'deprecated-media' );
mn_enqueue_style( 'ie' );
?>
<script type="text/javascript">
...
<body<?php if ( isset($GLOBALS['body_id']) ) echo ' id="' . $GLOBALS['body_id'] . '"'; ?> class="mn-core-ui no-js">
<script type="text/javascript">
...
?>
<script type="text/javascript">if(typeof mnOnload=='function')mnOnload();</script>
</body>
</html>
<?php
}
...
function media_buttons($editor_id = 'content') {
	...
	mn_enqueue_media( array(
		'post' => $post
	) );

	$img = '<span class="mn-media-buttons-icon"></span> ';
	...
	if ( $legacy_filter ) {
		// #MN22559. Close <a> if a plugin started by closing <a> to open their own <a> tag.
 ...
 * @since 2.5.0
 *
 * @return mixed void|object MN_Error on failure
 */
function media_upload_form_handler() {
	...
	if ( !empty($_POST['attachments']) ) foreach ( $_POST['attachments'] as $attachment_id => $attachment ) {
		 ...
		 * @since 2.5.0
		 *
		 * @see mn_get_attachment_metadata()
		 *
		...
		if ( isset($attachment['image_alt']) ) {
			$image_alt = mn_unslash( $attachment['image_alt'] );
			if ( $image_alt != get_post_meta($attachment_id, '_mn_attachment_image_alt', true) ) {
				$image_alt = mn_strip_all_tags( $image_alt, true );

				// Update_meta expects slashed.
				update_post_meta( $attachment_id, '_mn_attachment_image_alt', mn_slash( $image_alt ) );
			}
		}
		...
		foreach ( get_attachment_taxonomies($post) as $t ) {
			if ( isset($attachment[$t]) )
				mn_set_object_terms($attachment_id, array_map('trim', preg_split('/,+/', $attachment[$t])), $t, false);
		}
	}
	...
	if ( isset($send_id) ) {
		$attachment = mn_unslash( $_POST['attachments'][$send_id] );

		$html = isset( $attachment['post_title'] ) ? $attachment['post_title'] : '';
		if ( !empty($attachment['url']) ) {
			$rel = '';
			if ( strpos($attachment['url'], 'attachment_id') || get_attachment_link($send_id) == $attachment['url'] )
				$rel = " rel='attachment mn-att-" . esc_attr($send_id) . "'";
 ...
 * @return null|string
 */
function mn_media_upload_handler() {
	$errors = array();
	$id = 0;
	...
	if ( !empty($_POST['insertonlybutton']) ) {
		...
		if ( isset( $_POST['media_type'] ) && 'image' != $_POST['media_type'] ) {
			$title = esc_html( mn_unslash( $_POST['title'] ) );
			if ( empty( $title ) )
			...
			$align = '';
			$alt = esc_attr( mn_unslash( $_POST['alt'] ) );
			if ( isset($_POST['align']) ) {
				$align = esc_attr( mn_unslash( $_POST['align'] ) );
	...
	if ( isset( $_POST['save'] ) ) {
		...
		mn_enqueue_script( 'admin-gallery' );
 		return mn_iframe( 'media_upload_gallery_form', $errors );
	...
	if ( isset($_GET['tab']) && $_GET['tab'] == 'type_url' ) {
		...
		if ( isset( $_GET['type'] ) && in_array( $_GET['type'], array( 'video', 'audio', 'file' ) ) )
			$type = $_GET['type'];
		return mn_iframe( 'media_upload_type_url_form', $type, $errors, $id );
	}

	return mn_iframe( 'media_upload_type_form', 'image', $errors, $id );
}
 ...
 * @return string|MN_Error Populated HTML img tag on success, MN_Error object otherwise.
 */
function media_sideload_image( $file, $post_id, $desc = null, $return = 'html' ) {
	if ( ! empty( $file ) ) {
		...
		if ( ! $matches ) {
			return new MN_Error( 'image_sideload_failed', __( 'Invalid image URL' ) );
		}
		...
		if ( is_mn_error( $file_array['tmp_name'] ) ) {
	...
	if ( ! empty( $src ) ) {
		...
		return new MN_Error( 'image_sideload_failed' );
	}
}
...
function media_upload_library() {
	...
	return mn_iframe( 'media_upload_library_form', $errors );
}
...
function image_link_input_fields($post, $url_type = '') {
	...
	$file = mn_get_attachment_url($post->ID);
 ...
 * @since 3.4.0
 *
 * @param MN_Post $edit_post Attachment MN_Post object.
 * @return string HTML markup for the textarea element.
 */
function mn_caption_input_textarea($edit_post) {
 ...
 * @param MN_Post $post        The MN_Post attachment object.
 ...
 * @since 2.5.0
 *
 * @param array $post       The MN_Post attachment object converted to an array.
 * @param array $attachment An array of attachment metadata.
...
function image_attachment_fields_to_save( $post, $attachment ) {
	if ( substr( $post['post_mime_type'], 0, 5 ) == 'image' ) {
		if ( strlen( trim( $post['post_title'] ) ) == 0 ) {
			...
			$post['post_title'] = preg_replace( '/\.\w+$/', '', mn_basename( $attachment_url ) );
...
function get_attachment_fields_to_edit($post, $errors = null) {
	if ( is_int($post) )
		$post = get_post($post);
	if ( is_array($post) )
		$post = new MN_Post( (object) $post );

	$image_url = mn_get_attachment_url($post->ID);
	...
	$form_fields = array(
		...
		'post_excerpt' => array(
			'label'      => __('Caption'),
			'input'      => 'html',
			'html'       => mn_caption_input_textarea($edit_post)
		),
		'post_content' => array(
		...
		'image_url'	=> array(
			...
			'value'      => mn_get_attachment_url($post->ID),
	...
	if ( substr($post->post_mime_type, 0, 5) == 'image' ) {
		$alt = get_post_meta($post->ID, '_mn_attachment_image_alt', true);
		if ( empty($alt) )
			$alt = '';
...
function get_media_items( $post_id, $errors ) {
	...
	if ( $post_id ) {
		...
		if ( is_array($GLOBALS['mn_the_query']->posts) )
			foreach ( $GLOBALS['mn_the_query']->posts as $attachment )
...
function get_media_item( $attachment_id, $args = null ) {
	...
	if ( ( $attachment_id = intval( $attachment_id ) ) && $thumb_url = mn_get_attachment_image_src( $attachment_id, 'thumbnail', true ) )
	...
	$args = mn_parse_args( $args, $default_args );
	...
	$filename = esc_html( mn_basename( $file ) );
	$title = esc_attr( $post->post_title );

	$post_mime_types = get_post_mime_types();
	$keys = array_keys( mn_match_mime_types( array_keys( $post_mime_types ), $post->post_mime_type ) );
	$type = reset( $keys );
	...
	$display_title = $r['show_title'] ? "<div class='filename new'><span class='title'>" . mn_html_excerpt( $display_title, 60, '&hellip;' ) . "</span></div>" : '';
	...
	$media_dims = '';
	$meta = mn_get_attachment_metadata( $post->ID );
	...
	$image_edit_button = '';
	if ( mn_attachment_is_image( $post->ID ) && mn_image_editor_supports( array( 'mime_type' => $post->post_mime_type ) ) ) {
		$nonce = mn_create_nonce( "image_editor-$post->ID" );
	...
	if ( $delete && current_user_can( 'delete_post', $attachment_id ) ) {
		if ( !EMPTY_TRASH_DAYS ) {
			$delete = "<a href='" . mn_nonce_url( "post.php?action=delete&amp;post=$attachment_id", 'delete-post_' . $attachment_id ) . "' id='del[$attachment_id]' class='delete-permanently'>" . __( 'Delete Permanently' ) . '</a>';
		} elseif ( !MEDIA_TRASH ) {
			...
			'<p>' . sprintf( __( 'You are about to delete %s.' ), '<strong>' . $filename . '</strong>' ) . "</p>
			 <a href='" . mn_nonce_url( "post.php?action=delete&amp;post=$attachment_id", 'delete-post_' . $attachment_id ) . "' id='del[$attachment_id]' class='button'>" . __( 'Continue' ) . "</a>
			 <a href='#' class='button' onclick=\"this.parentNode.style.display='none';return false;\">" . __( 'Cancel' ) . "</a>
			 </div>";
		} else {
			$delete = "<a href='" . mn_nonce_url( "post.php?action=trash&amp;post=$attachment_id", 'trash-post_' . $attachment_id ) . "' id='del[$attachment_id]' class='delete'>" . __( 'Move to Trash' ) . "</a>
			<a href='" . mn_nonce_url( "post.php?action=untrash&amp;post=$attachment_id", 'untrash-post_' . $attachment_id ) . "' id='undo[$attachment_id]' class='undo hidden'>" . __( 'Undo' ) . "</a>";
		}
	} else {
		$delete = '';
	}
	...
	if ( 'image' == $type && $calling_post_id && current_theme_supports( 'post-thumbnails', get_post_type( $calling_post_id ) )
		...
		$ajax_nonce = mn_create_nonce( "set_post_thumbnail-$calling_post_id" );
		$thumbnail = "<a class='mn-post-thumbnail' id='mn-post-thumbnail-" . $attachment_id . "' href='#' onclick='MNSetAsThumbnail(\"$attachment_id\", \"$ajax_nonce\");return false;'>" . esc_html( $calling_post_type_object->labels->use_featured_image ) . "</a>";
	}
...
function media_upload_form( $errors = null ) {
	...
	if ( ! _device_can_upload() ) {
		echo '<p>' . sprintf( __('The web browser on your device cannot be used to upload files. You may be able to use the <a href="%s">native app for your device</a> instead.'), 'https://apps.mtaandao.co.ke/' ) . '</p>';
		return;
	}
...
<div id="media-upload-error"><?php
	...
	if (isset($errors['upload_error']) && is_mn_error($errors['upload_error']))
...
$post_params = array(
	"post_id" => $post_id,
	"_mnnonce" => mn_create_nonce('media-form'),
	"type" => $_type,
	"tab" => $_tab,
	"short" => "1",
);
...
mnUploaderInit = <?php echo mn_json_encode( $plupload_init ); ?>;
</script>
 ...
 */
echo apply_filters( 'type_url_form_media', mn_media_insert_url_form( $type ) );
?>
</div>
</div>
</form>
...
<p class="ml-submit">
<input type="button" class="button" style="display:none;" onMouseDown="mngallery.update();" name="insert-gallery" id="insert-gallery" value="<?php esc_attr_e( 'Insert gallery' ); ?>" />
<input type="button" class="button" style="display:none;" onMouseDown="mngallery.update();" name="update-gallery" id="update-gallery" value="<?php esc_attr_e( 'Update gallery settings' ); ?>" />
</p>
</div>
</form>
<?php
...
function media_upload_library_form($errors) {
	global $mndb, $mn_query, $mn_locale, $type, $tab, $post_mime_types;
...
<?php
$type_links = array();
$_num_posts = (array) mn_count_attachments();
$matches = mn_match_mime_types(array_keys($post_mime_types), array_keys($_num_posts));
...
if ( empty($_GET['post_mime_type']) && !empty($num_posts[$type]) ) {
	...
	list($post_mime_types, $avail_post_mime_types) = mn_edit_attachments_query();
}
...
foreach ( $post_mime_types as $mime_type => $label ) {
	...
	if ( !mn_match_mime_types($mime_type, $avail_post_mime_types) )
		continue;

	if ( isset($_GET['post_mime_type']) && mn_match_mime_types($mime_type, $_GET['post_mime_type']) )
		$class = ' class="current"';
...
$page_links = paginate_links( array(
	...
	'prev_text' => __('&laquo;'),
	'next_text' => __('&raquo;'),
	'total' => ceil($mn_query->found_posts / 10),
	'current' => $q['paged'],
));
...
$arc_query = "SELECT DISTINCT YEAR(post_date) AS yyear, MONTH(post_date) AS mmonth FROM $mndb->posts WHERE post_type = 'attachment' ORDER BY post_date DESC";

$arc_result = $mndb->get_results( $arc_query );
...
foreach ($arc_result as $arc_row) {
	...
	echo esc_html( $mn_locale->get_month($arc_row->mmonth) . " $arc_row->yyear" );
	echo "</option>\n";
}
?>
</select>
 ...
 */
function mn_media_insert_url_form( $default_view = 'image' ) {
	/** This filter is documented in admin/includes/media.php */
	if ( ! apply_filters( 'disable_captions', '' ) ) {
		$caption = '
		<tr class="image-only">
...
function media_upload_html_bypass() {
	...
	<p class="upload-html-bypass hide-if-no-js">
	   <?php _e('You are using the browser&#8217;s built-in file uploader. The Mtaandao uploader includes multiple file selection and drag and drop capability. <a href="#">Switch to the multi-file uploader</a>.'); ?>
	</p>
	<?php
}
 ...
 * @since 3.5.0
 *
 * @param MN_Post $post A post object.
 */
function edit_form_image_editor( $post ) {
	...
	if ( $attachment_id = intval( $post->ID ) )
		$thumb_url = mn_get_attachment_image_src( $attachment_id, array( 900, 450 ), true );

	$alt_text = get_post_meta( $post->ID, '_mn_attachment_image_alt', true );

	$att_url = mn_get_attachment_url( $post->ID ); ?>
	<div class="mn_attachment_holder mn-clearfix">
	<?php
	if ( mn_attachment_is_image( $post->ID ) ) :
		$image_edit_button = '';
		if ( mn_image_editor_supports( array( 'mime_type' => $post->post_mime_type ) ) ) {
			$nonce = mn_create_nonce( "image_editor-$post->ID" );
			$image_edit_button = "<input type='button' id='imgedit-open-btn-$post->ID' onclick='imageEdit.open( $post->ID, \"$nonce\" )' class='button' value='" . esc_attr__( 'Edit Image' ) . "' /> <span class='spinner'></span>";
		}
	?>
		...
		<div<?php if ( $open ) echo ' style="display:none"'; ?> class="mn_attachment_image mn-clearfix" id="media-head-<?php echo $attachment_id; ?>">
		...
		<div<?php if ( ! $open ) echo ' style="display:none"'; ?> class="image-editor" id="image-editor-<?php echo $attachment_id; ?>">
			<?php if ( $open ) mn_image_editor( $attachment_id ); ?>
		</div>
	<?php
	elseif ( $attachment_id && mn_attachment_is( 'audio', $post ) ):

		mn_maybe_generate_attachment_metadata( $post );

		echo mn_audio_shortcode( array( 'src' => $att_url ) );

	elseif ( $attachment_id && mn_attachment_is( 'video', $post ) ):
		...
		if ( ! empty( $thumb_id ) ) {
			$attr['poster'] = mn_get_attachment_url( $thumb_id );
		}

		echo mn_video_shortcode( $attr );
		 ...
		 * @param MN_Post $post A post object.
		 */
		do_action( 'mn_edit_form_attachment_display', $post );

	endif; ?>
	</div>
	<div class="mn_attachment_details edit-form-section">
		<p>
	...
	<?php if ( 'image' === substr( $post->post_mime_type, 0, 5 ) ) : ?>
		<p>
			...
			<input type="text" class="widefat" name="_mn_attachment_image_alt" id="attachment_alt" value="<?php echo esc_attr( $alt_text ); ?>" />
		</p>
	<?php endif; ?>
	...
	<?php mn_editor( $post->post_content, 'attachment_content', $editor_args ); ?>
 ...
 */
function mn_add_id3_tag_data( &$metadata, $data ) {
 ...
 */
function mn_read_video_metadata( $file ) {
	if ( ! file_exists( $file ) ) {
		return false;
	}
	...
	mn_add_id3_tag_data( $metadata, $data );
 ...
 * @return array|bool Returns array of metadata, if found.
 */
function mn_read_audio_metadata( $file ) {
	if ( ! file_exists( $file ) ) {
		return false;
	}
	$metadata = array();
 ...
 *                          Default 'attach'.
 */
function mn_media_attach_action( $parent_id, $action = 'attach' ) {
	global $mndb;
	...
	if ( ! empty( $ids ) ) {
		...
		if ( 'attach' === $action ) {
			$result = $mndb->query( $mndb->prepare( "UPDATE $mndb->posts SET post_parent = %d WHERE post_type = 'attachment' AND ID IN ( $ids_string )", $parent_id ) );
		} else {
			$result = $mndb->query( "UPDATE $mndb->posts SET post_parent = 0 WHERE post_type = 'attachment' AND ID IN ( $ids_string )" );
		}
