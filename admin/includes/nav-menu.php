 ...
 */
function _mn_ajax_menu_quick_search( $request = array() ) {
	$args = array();
	...
	if ( 'get-post-item' == $type ) {
		if ( post_type_exists( $object_type ) ) {
			if ( isset( $request['ID'] ) ) {
				...
				if ( 'markup' == $response_format ) {
					echo walk_nav_menu_tree( array_map('mn_setup_nav_menu_item', array( get_post( $object_id ) ) ), 0, (object) $args );
			...
			if ( isset( $request['ID'] ) ) {
				$object_id = (int) $request['ID'];
				if ( 'markup' == $response_format ) {
					echo walk_nav_menu_tree( array_map('mn_setup_nav_menu_item', array( get_term( $object_id, $object_type ) ) ), 0, (object) $args );
				} elseif ( 'json' == $response_format ) {
		...
		if ( 'posttype' == $matches[1] && get_post_type_object( $matches[2] ) ) {
			$search_results_query = new MN_Query( array(
				'no_found_rows'          => true,
			...
			while ( $search_results_query->have_posts() ) {
				...
				if ( 'markup' == $response_format ) {
					$var_by_ref = $post->ID;
					echo walk_nav_menu_tree( array_map('mn_setup_nav_menu_item', array( get_post( $var_by_ref ) ) ), 0, (object) $args );
				} elseif ( 'json' == $response_format ) {
					echo mn_json_encode(
						array(
							'ID' => $post->ID,
			...
			if ( empty( $terms ) || is_mn_error( $terms ) )
				return;
			foreach ( (array) $terms as $term ) {
				if ( 'markup' == $response_format ) {
					echo walk_nav_menu_tree( array_map('mn_setup_nav_menu_item', array( $term ) ), 0, (object) $args );
				} elseif ( 'json' == $response_format ) {
					echo mn_json_encode(
						array(
							'ID' => $term->term_id,
 ...
 * @since 3.0.0
 **/
function mn_nav_menu_setup() {
	// Register meta boxes
	mn_nav_menu_post_type_meta_boxes();
	add_meta_box( 'add-custom-links', __( 'Custom Links' ), 'mn_nav_menu_item_link_meta_box', 'nav-menus', 'side', 'default' );
	mn_nav_menu_taxonomy_meta_boxes();
 ...
 **/
function mn_initial_nav_menu_meta_boxes() {
	global $mn_meta_boxes;

	if ( get_user_option( 'metaboxhidden_nav-menus' ) !== false || ! is_array($mn_meta_boxes) )
		return;

	$initial_meta_boxes = array( 'add-post-type-page', 'add-post-type-post', 'add-custom-links', 'add-category' );
	$hidden_meta_boxes = array();

	foreach ( array_keys($mn_meta_boxes['nav-menus']) as $context ) {
		foreach ( array_keys($mn_meta_boxes['nav-menus'][$context]) as $priority ) {
			foreach ( $mn_meta_boxes['nav-menus'][$context][$priority] as $box ) {
 ...
 * @since 3.0.0
 */
function mn_nav_menu_post_type_meta_boxes() {
	...
	foreach ( $post_types as $post_type ) {
		...
		if ( $post_type ) {
			...
			add_meta_box( "add-post-type-{$id}", $post_type->labels->name, 'mn_nav_menu_item_post_type_meta_box', 'nav-menus', 'side', $priority, $post_type );
		}
	}
}
 ...
 * @since 3.0.0
 */
function mn_nav_menu_taxonomy_meta_boxes() {
	...
	foreach ( $taxonomies as $tax ) {
		...
		if ( $tax ) {
			$id = $tax->name;
			add_meta_box( "add-{$id}", $tax->labels->name, 'mn_nav_menu_item_taxonomy_meta_box', 'nav-menus', 'side', 'default', $tax );
		}
	}
}
 ...
 */
function mn_nav_menu_disabled_check( $nav_menu_selected_id ) {
 ...
 */
function mn_nav_menu_item_link_meta_box() {
	...
	<div class="customlinkdiv" id="customlinkdiv">
		...
		<p class="button-controls mn-clearfix">
			<span class="add-to-menu">
				<input type="submit"<?php mn_nav_menu_disabled_check( $nav_menu_selected_id ); ?> class="button-secondary submit-add-to-menu right" value="<?php esc_attr_e('Add to Menu'); ?>" name="add-custom-menu-item" id="submit-customlinkdiv" />
 ...
 *     @type MN_Post_Type $args     Extra meta box arguments (the post type object for this meta box).
 * }
 */
function mn_nav_menu_item_post_type_meta_box( $object, $box ) {
	global $_nav_menu_placeholder, $nav_menu_selected_id;
	...
	<div id="posttype-<?php echo $post_type_name; ?>" class="posttypediv">
		...
		?>">
			<ul id="<?php echo $post_type_name; ?>checklist-most-recent" class="categorychecklist form-no-clear">
				 ...
				 * @param array $args        An array of MN_Query arguments.
				 * @param array $box         Arguments passed to mn_nav_menu_item_post_type_meta_box().
				 */
				$most_recent = apply_filters( "nav_menu_items_{$post_type_name}_recent", $most_recent, $args, $box );

				echo walk_nav_menu_tree( array_map('mn_setup_nav_menu_item', $most_recent), 0, (object) $args );
				?>
			</ul>
		</div><!-- /.tabs-panel -->
		...
		?>" id="tabs-panel-posttype-<?php echo $post_type_name; ?>-search">
			...
			<ul id="<?php echo $post_type_name; ?>-search-checklist" data-mn-lists="list:<?php echo $post_type_name?>" class="categorychecklist form-no-clear">
			<?php if ( ! empty( $search_results ) && ! is_mn_error( $search_results ) ) : ?>
				<?php
				$args['walker'] = $walker;
				echo walk_nav_menu_tree( array_map('mn_setup_nav_menu_item', $search_results), 0, (object) $args );
				?>
			<?php elseif ( is_mn_error( $search_results ) ) : ?>
		...
		?>">
			...
			<?php endif; ?>
			<ul id="<?php echo $post_type_name; ?>checklist" data-mn-lists="list:<?php echo $post_type_name?>" class="categorychecklist form-no-clear">
				<?php
				$args['walker'] = $walker;
				 ...
				 * @param array        $args      An array of MN_Query arguments.
				 * @param MN_Post_Type $post_type The current post type object for this menu item meta box.
				 */
				$posts = apply_filters( "nav_menu_items_{$post_type_name}", $posts, $args, $post_type );

				$checkbox_items = walk_nav_menu_tree( array_map('mn_setup_nav_menu_item', $posts), 0, (object) $args );
		...
		<p class="button-controls mn-clearfix">
			...
			<span class="add-to-menu">
				<input type="submit"<?php mn_nav_menu_disabled_check( $nav_menu_selected_id ); ?> class="button-secondary submit-add-to-menu right" value="<?php esc_attr_e( 'Add to Menu' ); ?>" name="add-post-type-menu-item" id="<?php echo esc_attr( 'submit-posttype-' . $post_type_name ); ?>" />
				<span class="spinner"></span>
			</span>
		</p>
 ...
 * }
 */
function mn_nav_menu_item_taxonomy_meta_box( $object, $box ) {
	...
	$num_pages = ceil( mn_count_terms( $taxonomy_name , array_merge( $args, array('number' => '', 'offset' => '') ) ) / $per_page );
	...
	<div id="taxonomy-<?php echo $taxonomy_name; ?>" class="taxonomydiv">
		...
		?>">
			<ul id="<?php echo $taxonomy_name; ?>checklist-pop" class="categorychecklist form-no-clear" >
				...
				$args['walker'] = $walker;
				echo walk_nav_menu_tree( array_map('mn_setup_nav_menu_item', $popular_terms), 0, (object) $args );
				?>
			</ul>
		</div><!-- /.tabs-panel -->
		...
		?>">
			...
			<?php endif; ?>
			<ul id="<?php echo $taxonomy_name; ?>checklist" data-mn-lists="list:<?php echo $taxonomy_name?>" class="categorychecklist form-no-clear">
				<?php
				$args['walker'] = $walker;
				echo walk_nav_menu_tree( array_map('mn_setup_nav_menu_item', $terms), 0, (object) $args );
				?>
			</ul>
			<?php if ( ! empty( $page_links ) ) : ?>
				<div class="add-menu-item-pagelinks">
		...
		?>" id="tabs-panel-search-taxonomy-<?php echo $taxonomy_name; ?>">
			...
			<ul id="<?php echo $taxonomy_name; ?>-search-checklist" data-mn-lists="list:<?php echo $taxonomy_name?>" class="categorychecklist form-no-clear">
			<?php if ( ! empty( $search_results ) && ! is_mn_error( $search_results ) ) : ?>
				<?php
				$args['walker'] = $walker;
				echo walk_nav_menu_tree( array_map('mn_setup_nav_menu_item', $search_results), 0, (object) $args );
		...
		<p class="button-controls mn-clearfix">
			...
			<span class="add-to-menu">
				<input type="submit"<?php mn_nav_menu_disabled_check( $nav_menu_selected_id ); ?> class="button-secondary submit-add-to-menu right" value="<?php esc_attr_e( 'Add to Menu' ); ?>" name="add-taxonomy-menu-item" id="<?php echo esc_attr( 'submit-taxonomy-' . $taxonomy_name ); ?>" />
				<span class="spinner"></span>
			</span>
		</p>
 ...
 */
function mn_save_nav_menu_items( $menu_id = 0, $menu_data = array() ) {
	$menu_id = (int) $menu_id;
	$items_saved = array();
	...
	if ( 0 == $menu_id || is_nav_menu( $menu_id ) ) {
		...
		foreach ( (array) $menu_data as $_possible_db_id => $_item_object_data ) {
			...
			$items_saved[] = mn_update_nav_menu_item( $menu_id, $_actual_db_id, $args );
 ...
 */
function _mn_nav_menu_meta_box_object( $object = null ) {
 ...
 * @return string|MN_Error $output The menu formatted to edit or error object on failure.
 */
function mn_get_nav_menu_to_edit( $menu_id = 0 ) {
	$menu = mn_get_nav_menu_object( $menu_id );

	// If the menu exists, get its items.
	if ( is_nav_menu( $menu ) ) {
		$menu_items = mn_get_nav_menu_items( $menu->term_id, array('post_status' => 'any') );
		 ...
		 */
		$walker_class_name = apply_filters( 'mn_edit_nav_menu_walker', 'Walker_Nav_Menu_Edit', $menu_id );

		if ( class_exists( $walker_class_name ) ) {
			$walker = new $walker_class_name;
		} else {
			return new MN_Error( 'menu_walker_not_exist',
		...
		$result .= walk_nav_menu_tree( array_map('mn_setup_nav_menu_item', $menu_items), 0, (object) array('walker' => $walker ) );
		$result .= ' </ul> ';
		return $result;
	} elseif ( is_mn_error( $menu ) ) {
		return $menu;
	}
 ...
 */
function mn_nav_menu_manage_columns() {
	return array(
 ...
 */
function _mn_delete_orphaned_draft_menu_items() {
	global $mndb;
	$delete_timestamp = time() - ( DAY_IN_SECONDS * EMPTY_TRASH_DAYS );

	// Delete orphaned draft menu items.
	$menu_items_to_delete = $mndb->get_col($mndb->prepare("SELECT ID FROM $mndb->posts AS p LEFT JOIN $mndb->postmeta AS m ON p.ID = m.post_id WHERE post_type = 'nav_menu_item' AND post_status = 'draft' AND meta_key = '_menu_item_orphaned' AND meta_value < '%d'", $delete_timestamp ) );

	foreach ( (array) $menu_items_to_delete as $menu_item_id )
		mn_delete_post( $menu_item_id, true );
}
 ...
 */
function mn_nav_menu_update_menu_items ( $nav_menu_selected_id, $nav_menu_selected_title ) {
	$unsorted_menu_items = mn_get_nav_menu_items( $nav_menu_selected_id, array( 'orderby' => 'ID', 'output' => ARRAY_A, 'output_key' => 'ID', 'post_status' => 'draft,publish' ) );
	$messages = array();
	$menu_items = array();
	...
	if ( ! empty( $_POST['menu-item-db-id'] ) ) {
		foreach ( (array) $_POST['menu-item-db-id'] as $_key => $k ) {
			...
			$menu_item_db_id = mn_update_nav_menu_item( $nav_menu_selected_id, ( $_POST['menu-item-db-id'][$_key] != $_key ? 0 : $_key ), $args );

			if ( is_mn_error( $menu_item_db_id ) ) {
	...
	$nav_menu_option['auto_add'] = array_intersect( $nav_menu_option['auto_add'], mn_get_nav_menus( array( 'fields' => 'ids' ) ) );
	update_option( 'nav_menu_options', $nav_menu_option );

	mn_defer_term_counting( false );

	/** This action is documented in res/nav-menu.php */
	do_action( 'mn_update_nav_menu', $nav_menu_selected_id );
 ...
 * @access private
 */
function _mn_expand_nav_menu_post_data() {
		...
		foreach ( $data as $post_input_data ) {
			...
			for ( $i = count( $array_bits ) - 1; $i >= 0; $i -- ) {
				if ( $i == count( $array_bits ) - 1 ) {
					$new_post_data[ $array_bits[ $i ] ] = mn_slash( $post_input_data->value );
				} else {
