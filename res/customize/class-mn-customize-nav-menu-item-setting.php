<?php
/**
 * Customize API: MN_Customize_Nav_Menu_Item_Setting class
 *
 * @package Mtaandao
 ...
 *
 * Subclass of MN_Customize_Setting to represent a nav_menu taxonomy term, and
 ...
 */
class MN_Customize_Nav_Menu_Item_Setting extends MN_Customize_Setting {
	 ...
	 * @var array
	 *
	 * @see mn_setup_nav_menu_item()
	 */
	public $default = array(
		// The $menu_item_data for mn_update_nav_menu_item().
		'object_id'        => 0,
		...
		'status'           => 'publish',
		'original_title'   => '',
		'nav_menu_term_id' => 0, // This will be supplied as the $menu_id arg for mn_update_nav_menu_item().
		'_invalid'         => false,
	);
	...
	/**
	 * Storage of pre-setup menu item to prevent wasted calls to mn_setup_nav_menu_item().
	 *
	 * @since 4.3.0
	 * @access protected
	 * @var array
	 ...
	 * @var int
	 *
	 * @see MN_Customize_Nav_Menu_Item_Setting::update()
	 * @see MN_Customize_Nav_Menu_Item_Setting::amend_customize_save_response()
	 */
	...
	/**
	 * Any error object returned by mn_update_nav_menu_item() when setting is updated.
	 *
	 * @since 4.3.0
	 * @access public
	 * @var MN_Error
	 ...
	 * @access public
	 *
	 * @param MN_Customize_Manager $manager Bootstrap Customizer instance.
	 ...
	 */
	public function __construct( MN_Customize_Manager $manager, $id, array $args = array() ) {
		if ( empty( $manager->nav_menus ) ) {
			throw new Exception( 'Expected MN_Customize_Manager::$nav_menus to be set.' );
		}
		...
		add_action( 'mn_update_nav_menu_item', array( $this, 'flush_cached_value' ), 10, 2 );
		...
		if ( $this->is_previewed && $this->_previewed_blog_id === get_current_blog_id() ) {
			...
			if ( $this->post_id > 0 ) {
				...
				if ( $post && self::POST_TYPE === $post->post_type ) {
					$value = (array) mn_setup_nav_menu_item( $post );
				}
			}
			...
			// Cache the value for future calls to avoid having to re-call mn_setup_nav_menu_item().
			$this->value = $value;
			$this->populate_value();
			$value = $this->value;
		}
	 ...
	 *
	 * Translates some properties added by mn_setup_nav_menu_item() and removes others.
	 *
	 * @since 4.3.0
	 * @access protected
	 *
	 * @see MN_Customize_Nav_Menu_Item_Setting::value()
	 */
	protected function populate_value() {
		...
		if ( ! isset( $this->value['nav_menu_term_id'] ) && $this->post_id > 0 ) {
			$menus = mn_get_post_terms( $this->post_id, MN_Customize_Nav_Menu_Setting::TAXONOMY, array(
				'fields' => 'ids',
			) );
			if ( ! empty( $menus ) ) {
	 ...
	 * @access public
	 *
	 * @see MN_Customize_Manager::post_value()
	 *
	...
	public function preview() {
		...
		add_filter( 'mn_get_nav_menu_items', array( $this, 'filter_mn_get_nav_menu_items' ), 10, 3 );

		$sort_callback = array( __CLASS__, 'sort_mn_get_nav_menu_items' );
		if ( ! has_filter( 'mn_get_nav_menu_items', $sort_callback ) ) {
			add_filter( 'mn_get_nav_menu_items', array( __CLASS__, 'sort_mn_get_nav_menu_items' ), 1000, 3 );
		}
	...
	/**
	 * Filters the mn_get_nav_menu_items() result to supply the previewed menu items.
	 *
	 * @since 4.3.0
	 * @access public
	 *
	 * @see mn_get_nav_menu_items()
	 *
	 ...
	 */
	public function filter_mn_get_nav_menu_items( $items, $menu, $args ) {
		...
		if ( $should_update ) {
			foreach ( $items as $item ) {
				if ( $item->db_id === $this->post_id ) {
					foreach ( get_object_vars( $this->value_as_mn_post_nav_menu_item() ) as $key => $value ) {
						$item->$key = $value;
					}
					$mutated = true;
				}
			...
			if ( ! $mutated ) {
				$items[] = $this->value_as_mn_post_nav_menu_item();
			}
		}
	...
	/**
	 * Re-apply the tail logic also applied on $items by mn_get_nav_menu_items().
	 *
	 * @since 4.3.0
	 * @access public
	 * @static
	 ...
	 * @return array Array of menu items,
	 */
	public static function sort_mn_get_nav_menu_items( $items, $menu, $args ) {
	...
	/**
	 * Get the value emulated into a MN_Post and set up as a nav_menu_item.
	 *
	 * @since 4.3.0
	 * @access public
	 *
	 * @return MN_Post With mn_setup_nav_menu_item() applied.
	 */
	public function value_as_mn_post_nav_menu_item() {
		...
		/** This filter is documented in res/nav-menu.php */
		$post->description = apply_filters( 'nav_menu_description', mn_trim_words( $post->description, 200 ) );

		/** This filter is documented in res/nav-menu.php */
		$post = apply_filters( 'mn_setup_nav_menu_item', $post );
	 ...
	 * Sanitize an input.
	 *
	 * Note that parent::sanitize() erroneously does mn_unslash() on $value, but
	...
	public function sanitize( $menu_item_value ) {
		...
		$menu_item_value = mn_array_slice_assoc( $menu_item_value, array_keys( $default ) );
		...
		// Apply the same filters as when calling mn_insert_post().
		$menu_item_value['title'] = mn_unslash( apply_filters( 'title_save_pre', mn_slash( $menu_item_value['title'] ) ) );
		$menu_item_value['attr_title'] = mn_unslash( apply_filters( 'excerpt_save_pre', mn_slash( $menu_item_value['attr_title'] ) ) );
		$menu_item_value['description'] = mn_unslash( apply_filters( 'content_save_pre', mn_slash( $menu_item_value['description'] ) ) );
		...
		/** This filter is documented in res/class-mn-customize-setting.php */
	 ...
	 *                           entirely. See MN_Customize_Nav_Menu_Item_Setting::$default for what the value
	 *                           should consist of.
	 * @return null|void
	 */
	protected function update( $value ) {
		...
		if ( $is_delete ) {
			...
			if ( $is_placeholder ) {
				...
				$r = mn_delete_post( $this->post_id, true );

				if ( false === $r ) {
					$this->update_error  = new MN_Error( 'delete_failure' );
			...
			if ( $value['nav_menu_term_id'] < 0 ) {
				...
				if ( ! $nav_menu_setting || ! ( $nav_menu_setting instanceof MN_Customize_Nav_Menu_Setting ) ) {
					$this->update_status = 'error';
					$this->update_error  = new MN_Error( 'unexpected_nav_menu_setting' );
					return;
				}

				if ( false === $nav_menu_setting->save() ) {
					$this->update_status = 'error';
					$this->update_error  = new MN_Error( 'nav_menu_setting_failure' );
					return;
				}

				if ( $nav_menu_setting->previous_term_id !== intval( $value['nav_menu_term_id'] ) ) {
					$this->update_status = 'error';
					$this->update_error  = new MN_Error( 'unexpected_previous_term_id' );
					return;
				}
			...
			if ( $value['menu_item_parent'] < 0 ) {
				...
				if ( ! $parent_nav_menu_item_setting || ! ( $parent_nav_menu_item_setting instanceof MN_Customize_Nav_Menu_Item_Setting ) ) {
					$this->update_status = 'error';
					$this->update_error  = new MN_Error( 'unexpected_nav_menu_item_setting' );
					return;
				}

				if ( false === $parent_nav_menu_item_setting->save() ) {
					$this->update_status = 'error';
					$this->update_error  = new MN_Error( 'nav_menu_item_setting_failure' );
					return;
				}

				if ( $parent_nav_menu_item_setting->previous_post_id !== intval( $value['menu_item_parent'] ) ) {
					$this->update_status = 'error';
					$this->update_error  = new MN_Error( 'unexpected_previous_post_id' );
					return;
				}
	 ...
	 * @see MN_Customize_Nav_Menu_Item_Setting::update()
	 *
	 * @param array $data Additional information passed back to the 'saved' event on `mn.customize`.
