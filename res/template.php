 ...
 *
 * Works by first retrieving the current tag name, for example 'tag-Mtaandao.php',
 ...
 *
 * By default the mtaandao-template is returned.
 *
 * The template path is filterable via the dynamic {@see '$type_template'} hook,
 * e.g. 'embed_template'.
 *
 ...
 *
 * Searches in the STYLESHEETPATH before TEMPLATEPATH and res/theme-compat
...
/**
 * Require the template file with Mtaandao environment.
 *
 * The globals are set up for the template file to ensure that the Mtaandao
...
function load_template( $_template_file, $require_once = true ) {
	global $posts, $post, $mn_did_header, $mn_query, $mn_rewrite, $mndb, $mn_version, $mn, $id, $comment, $user_ID;

	if ( is_array( $mn_query->query_vars ) ) {
		extract( $mn_query->query_vars, EXTR_SKIP );
	}
