<?php
/**
 * Import Mtaandao Administration Screen
 *
 * @package Mtaandao
...
get_current_screen()->add_help_tab( array(
	...
	'content' => '<p>' . __('This screen lists links to plugins to import data from blogging/content management platforms. Choose the platform you want to import from, and click Install Now when you are prompted in the popup window. If your platform is not listed, click the link to search the plugin directory for other importer plugins to see if there is one for your platform.') . '</p>' .
		'<p>' . __('In previous versions of Mtaandao, all importers were built-in. They have been turned into plugins since most people only use them once or infrequently.') . '</p>',
) );
...
if ( ! empty( $_GET['invalid'] ) && isset( $popular_importers[ $_GET['invalid'] ] ) ) {
	...
	if ( $importer_id != $_GET['invalid'] ) { // Prevent redirect loops.
		mn_redirect( admin_url( 'admin.php?import=' . $importer_id ) );
		exit;
	}
	unset( $importer_id );
}
...
<?php endif; ?>
<p><?php _e('If you have posts or comments in another system, Mtaandao can import those into this site. To get started, choose a system to import from below:'); ?></p>

<?php
// Registered (already installed) importers. They're stored in the global $mn_importers.
		...
		if ( isset( $data['install'] ) ) {
			...
			if ( file_exists( MN_PLUGIN_DIR . '/' . $plugin_slug ) ) {
				...
				if ( ! empty( $plugins ) ) {
					...
					$url = mn_nonce_url( add_query_arg( array(
						'action' => 'activate',
						'plugin' => $plugin_file,
						'from'   => 'import',
