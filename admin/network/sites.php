...
$mn_list_table = _get_list_table( 'MN_MS_Sites_List_Table' );
...
if ( isset( $_GET['action'] ) ) {
	...
	if ( 'confirm' === $_GET['action'] ) {
		...
		if ( ! array_key_exists( $site_action, $manage_actions ) ) {
			mn_die( __( 'The requested action is not valid.' ) );
		}
		...
		if ( $current_site->blog_id == $id ) {
			mn_die( __( 'Sorry, you are not allowed to change the current site.' ) );
		}
		...
		?>
			<div class="wrap">
				...
				<form action="sites.php?action=<?php echo esc_attr( $site_action ); ?>" method="post">
					...
					<input type="hidden" name="_mn_http_referer" value="<?php echo esc_attr( mn_get_referer() ); ?>" />
					<?php mn_nonce_field( $site_action . '_' . $id, '_mnnonce', false ); ?>
	...
	switch ( $_GET['action'] ) {
		...
		case 'deleteblog':
			if ( ! current_user_can( 'delete_sites' ) )
				mn_die( __( 'Sorry, you are not allowed to access this page.' ), '', array( 'response' => 403 ) );

			$updated_action = 'not_deleted';
			if ( $id != '0' && $id != $current_site->blog_id && current_user_can( 'delete_site', $id ) ) {
				mnmu_delete_blog( $id, true );
						...
						switch ( $doaction ) {
							case 'delete':
								if ( ! current_user_can( 'delete_site', $val ) )
									mn_die( __( 'Sorry, you are not allowed to delete the site.' ) );

								$updated_action = 'all_delete';
								mnmu_delete_blog( $val, true );
							break;
	...
	if ( ! empty( $updated_action ) ) {
		mn_safe_redirect( add_query_arg( array( 'updated' => $updated_action ), mn_get_referer() ) );
		exit();
	}
}
...
<?php $mn_list_table->search_box( __( 'Search Sites' ), 'site' ); ?>
