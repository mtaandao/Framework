...
class WC_Shortcodes {
	...
	private static function product_loop( $query_args, $atts, $loop_name ) {
		...
		$products                    = new MN_Query( apply_filters( 'woocommerce_shortcode_products_query', $query_args, $atts, $loop_name ) );
	...
	public static function product_categories( $atts ) {
		...
		// get terms and workaround MN bug with parents/pad counts
		$args = array(
		...
		if ( '' !== $atts['parent'] ) {
			$product_categories = mn_list_filter( $product_categories, array( 'parent' => $atts['parent'] ) );
		}
	...
	public static function product( $atts ) {
		...
		$products = new MN_Query( apply_filters( 'woocommerce_shortcode_products_query', $args, $atts ) );
	...
	public static function product_page( $atts ) {
		...
		$single_product = new MN_Query( $args );
		...
		while ( $single_product->have_posts() ) : $single_product->the_post(); mn_enqueue_script( 'wc-single-product' ); ?>
	...
	public static function order_by_rating_post_clauses( $args ) {
		...
		$args['where']   .= " AND $mndb->commentmeta.meta_key = 'rating' ";
		$args['join']    .= "LEFT JOIN $mndb->comments ON($mndb->posts.ID               = $mndb->comments.comment_post_ID) LEFT JOIN $mndb->commentmeta ON($mndb->comments.comment_ID = $mndb->commentmeta.comment_id)";
		$args['orderby'] = "$mndb->commentmeta.meta_value DESC";
