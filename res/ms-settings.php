<?php
/**
 * Used to set up and fix common variables and include
 * the Multisite procedural and class library.
 *
 * Allows for some configuration in configuration.php (see ms-default-constants.php)
 *
 * @package Mtaandao
 * @subpackage Multisite
 * @since 3.0.0
 */

/**
 * Objects representing the current network and current site.
 *
 * These may be populated through a custom `sunrise.php`. If not, then this
 * file will attempt to populate them based on the current request.
 *
 * @global MN_Network $current_site The current network.
 * @global object     $current_blog The current site.
 * @since 3.0.0
 */
global $current_site, $current_blog;

/** MN_Network class */
require_once( ABSPATH . RES . '/class-mn-network.php' );

/** MN_Site class */
require_once( ABSPATH . RES . '/class-mn-site.php' );

/** Multisite loader */
require_once( ABSPATH . RES . '/ms-load.php' );

/** Default Multisite constants */
require_once( ABSPATH . RES . '/ms-default-constants.php' );

if ( defined( 'SUNRISE' ) ) {
	include_once( MAIN . '/sunrise.php' );
}

/** Check for and define SUBDOMAIN_INSTALL and the deprecated VHOST constant. */
ms_subdomain_constants();

// This block will process a request if the current network or current site objects
// have not been populated in the global scope through something like `sunrise.php`.
if ( !isset( $current_site ) || !isset( $current_blog ) ) {

	$domain = strtolower( stripslashes( $_SERVER['HTTP_HOST'] ) );
	if ( substr( $domain, -3 ) == ':80' ) {
		$domain = substr( $domain, 0, -3 );
		$_SERVER['HTTP_HOST'] = substr( $_SERVER['HTTP_HOST'], 0, -3 );
	} elseif ( substr( $domain, -4 ) == ':443' ) {
		$domain = substr( $domain, 0, -4 );
		$_SERVER['HTTP_HOST'] = substr( $_SERVER['HTTP_HOST'], 0, -4 );
	}

	$path = stripslashes( $_SERVER['REQUEST_URI'] );
	if ( is_admin() ) {
		$path = preg_replace( '#(.*)/admin/.*#', '$1/', $path );
	}
	list( $path ) = explode( '?', $path );

	$bootstrap_result = ms_load_current_site_and_network( $domain, $path, is_subdomain_install() );

	if ( true === $bootstrap_result ) {
		// `$current_blog` and `$current_site are now populated.
	} elseif ( false === $bootstrap_result ) {
		ms_not_installed( $domain, $path );
	} else {
		header( 'Location: ' . $bootstrap_result );
		exit;
	}
	unset( $bootstrap_result );

	$blog_id = $current_blog->blog_id;
	$public  = $current_blog->public;

	if ( empty( $current_blog->site_id ) ) {
		// This dates to [MU134] and shouldn't be relevant anymore,
		// but it could be possible for arguments passed to insert_blog() etc.
		$current_blog->site_id = 1;
	}

	$site_id = $current_blog->site_id;
	mn_load_core_site_options( $site_id );
}

$mndb->set_prefix( $table_prefix, false ); // $table_prefix can be set in sunrise.php
$mndb->set_blog_id( $current_blog->blog_id, $current_blog->site_id );
$table_prefix = $mndb->get_blog_prefix();
$_mn_switched_stack = array();
$switched = false;

// need to init cache again after blog_id is set
mn_start_object_cache();

if ( ! $current_site instanceof MN_Network ) {
	$current_site = new MN_Network( $current_site );
}

if ( ! $current_blog instanceof MN_Site ) {
	$current_blog = new MN_Site( $current_blog );
}

// Define upload directory constants
ms_upload_constants();

/**
 * Fires after the current site and network have been detected and loaded
 * in multisite's bootstrap.
 *
 * @since 16.10.0
 */
do_action( 'ms_loaded' );
