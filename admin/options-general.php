...
<?php if ( !is_multisite() ) { ?>
<tr>
<th scope="row"><label for="siteurl"><?php _e('Mtaandao Address (URL)') ?></label></th>
<td><input name="siteurl" type="url" id="siteurl" value="<?php form_option( 'siteurl' ); ?>"<?php disabled( defined( 'MN_SITEURL' ) ); ?> class="regular-text code<?php if ( defined( 'MN_SITEURL' ) ) echo ' disabled' ?>" /></td>
</tr>
<tr>
...
<td>
<select name="default_role" id="default_role"><?php mn_dropdown_roles( get_option('default_role') ); ?></select>
</td>
</tr>
<?php } else { ?>
<tr>
...
<p><?php
	...
	printf(
		' <a href="%1$s">%2$s</a>',
		esc_url( mn_nonce_url( admin_url( 'options.php?dismiss=new_admin_email' ), 'dismiss-' . get_current_blog_id() . '-new_admin_email' ) ),
		__( 'Cancel' )
	);
?></p>
</div>
...
for ($day_index = 0; $day_index <= 6; $day_index++) :
	...
	echo "\n\t<option value='" . esc_attr($day_index) . "' $selected>" . $mn_locale->get_weekday($day_index) . '</option>';
endfor;
?>
</select></td>
</tr>
...
if ( ! is_multisite() && defined( 'MNLANG' ) && '' !== MNLANG && 'en_US' !== MNLANG && ! in_array( MNLANG, $languages ) ) {
	$languages[] = MNLANG;
}
if ( ! empty( $languages ) || ! empty( $translations ) ) {
	?>
	<tr>
		<th width="33%" scope="row"><label for="MNLANG"><?php _e( 'Site Language' ); ?></label></th>
		<td>
			<?php
			$locale = get_locale();
			...
			mn_dropdown_languages( array(
				...
				'show_available_translations' => ( ! is_multisite() || is_super_admin() ) && mn_can_install_language_pack(),
			) );

			// Add note about deprecated MNLANG constant.
			if ( defined( 'MNLANG' ) && ( '' !== MNLANG ) && $locale !== MNLANG ) {
				if ( is_super_admin() ) {
					?>
					<p class="description">
