...
if ( is_multisite() && ! is_network_admin() ) {
	mn_redirect( network_admin_url( 'plugin-editor.php' ) );
	exit();
}

if ( !current_user_can('edit_plugins') )
	mn_die( __('Sorry, you are not allowed to edit plugins for this site.') );

$title = __("Edit Plugins");
$parent_file = 'plugins.php';

mn_reset_vars( array( 'action', 'error', 'file', 'plugin' ) );
...
case 'update':
	...
	$newcontent = mn_unslash( $_POST['newcontent'] );
	if ( is_writeable($real_file) ) {
		...
		if ( is_plugin_active($file) || isset($_POST['phperror']) ) {
			...
			mn_redirect(add_query_arg('_mnnonce', mn_create_nonce('edit-plugin-test_' . $file), "plugin-editor.php?file=$file&liveupdate=1&scrollto=$scrollto&networkwide=" . $network_wide));
			exit;
		}
		mn_redirect( self_admin_url("plugin-editor.php?file=$file&a=te&scrollto=$scrollto") );
	} else {
		mn_redirect( self_admin_url("plugin-editor.php?file=$file&scrollto=$scrollto") );
	}
	exit;
...
default:
	...
	if ( ! is_file($real_file) ) {
		mn_die(sprintf('<p>%s</p>', __('No such file exists! Double check the name and try again.')));
	} else {
		...
		if ( preg_match('/\.([^.]+)$/', $real_file, $matches) ) {
			...
			if ( !in_array( $ext, $editable_extensions) )
				mn_die(sprintf('<p>%s</p>', __('Files of this type are not editable.')));
		}
	}
	...
	get_current_screen()->set_help_sidebar(
		...
		'<p>' . __('<a href="https://mtaandao.co.ke/docs/Plugins_Editor_Screen" target="_blank">Documentation on Editing Plugins</a>') . '</p>' .
		'<p>' . __('<a href="https://mtaandao.co.ke/docs/Writing_a_Plugin" target="_blank">Documentation on Writing Plugins</a>') . '</p>' .
		'<p>' . __('<a href="https://mtaandao.co.ke/support/" target="_blank">Support Forums</a>') . '</p>'
	);

	require_once(ABSPATH . 'admin/admin-header.php');

	update_recently_edited(MN_PLUGIN_DIR . '/' . $file);

	$content = file_get_contents( $real_file );

	if ( '.php' == substr( $real_file, strrpos( $real_file, '.' ) ) ) {
		$functions = mn_doc_link_parse( $content );
...
<?php elseif (isset($_GET['phperror'])) : ?>
 <div id="message" class="updated"><p><?php _e('This plugin has been deactivated because your changes resulted in a <strong>fatal error</strong>.') ?></p>
	<?php
		if ( mn_verify_nonce( $_GET['_error_nonce'], 'plugin-activation-error_' . $file ) ) {
...
<form name="template" id="template" action="plugin-editor.php" method="post">
	<?php mn_nonce_field('edit-plugin_' . $file) ?>
		...
		<div id="documentation" class="hide-if-no-js"><label for="docs-list"><?php _e('Documentation:') ?></label> <?php echo $docs_select ?> <input type="button" class="button" value="<?php esc_attr_e( 'Look Up' ) ?> " onclick="if ( '' != jQuery('#docs-list').val() ) { window.open( 'https://api.mtaandao.co.ke/core/handbook/1.0/?function=' + escape( jQuery( '#docs-list' ).val() ) + '&amp;locale=<?php echo urlencode( get_locale() ) ?>&amp;version=<?php echo urlencode( $mn_version ) ?>&amp;redirect=true'); }" /></div>
		<?php endif; ?>
