...
class RM_Posts_Edit_Access {
    
    private $lib = null;
    private $user = null;   // RM_Posts_Edit_Access_User class instance        
    private $screen;
    ...
    public function __construct() {
        ...
        $this->user = new RM_Posts_Edit_Access_User($this);
    ...
    public function set_final_hooks() {
        ...
        $wc_bookings_active = RM_Plugin_Presence::is_active('woocommerce-bookings');   // Woocommerce Bookings plugin 
        if ($wc_bookings_active) {
            RM_WC_Bookings::separate_user_transients();            
        }
        ...
        // add_filter('mn_count_posts', array($this, 'recount_mn_posts'));  // @TODO
    ...
    public function recount_mn_posts($views) {
    ...
    public function block_edit_post($caps, $cap='', $user_id=0, $args=array()) {
        ...
        $restrict_it = apply_filters('rm_restrict_edit_post_type', $post->post_type);
    ...
    public function restrict_posts_list($query) {                
        ...
        if (!empty($query->query['post_type'])) {
            $restrict_it = apply_filters('rm_restrict_edit_post_type', $query->query['post_type']);
            if (empty($restrict_it)) {
                return;
            }         
        }
        ...
        if ($query->query['post_type']=='attachment') { 
            $show_full_list = apply_filters('rm_attachments_show_full_list', false);
    ...
    public function restrict_pages_list($pages) {
        ...
        $restrict_it = apply_filters('rm_restrict_edit_post_type', 'page');
        if (empty($restrict_it)) {
            return;
        }
     ...
     * Modified by Mtaandao (role-manager.com)
     * 
     */
    protected function count_posts($type = 'post', $perm = '') {
        ...
        $count = mn_cache_get($cache_key, 'counts');
        if (false !== $count)
            return $count;

        $count = $mndb->get_results($mndb->prepare($query, $type), ARRAY_A);
        ...
        $stats = (object) $stats;
        mn_cache_set($cache_key, $stats, 'counts');
    ...
    /**
     * Modification to this MN function was done by Marcus Sykes (http://msyk.es)
     * His comments follow untouched:
     * Almost-exact copy of MN_Posts_List_Table::get_views(), but makes subtle changes for $this references and calls internal Admin_For_Authors::mn_count_posts() function instead
	    * Changes highlighted with comments starting //EDIT 
	    */
    public function get_views() {
        global $mndb, $locked_post_status, $avail_post_stati;
        ...
        if ( !current_user_can( $post_type_object->cap->edit_others_posts ) ) {            
            ...
            $query = "SELECT COUNT( 1 ) FROM $mndb->posts
            ...
            $user_posts_count = $mndb->get_var($mndb->prepare($query, $post_type, $current_user_id));
        ...
        $status_links['all'] = "<a href='edit.php?post_type=$post_type{$allposts}'$class>" .  
                    esc_html__('All', 'role-manager') . ' <span class="count">('. $total_posts .')</span></a>';
        ...
        //EDIT - START this whole if statement gets sticky posts stat, copied from MN_Posts_List_Table::_construct() but there's maybe a better way for this
        global $mndb;
        if ('post' == $post_type && $sticky_posts = get_option('sticky_posts')) {
            $sticky_posts = implode(', ', array_map('absint', (array) $sticky_posts));
            $sticky_posts_count = $mndb->get_var($mndb->prepare("SELECT COUNT( 1 ) FROM $mndb->posts WHERE post_type = %s AND post_status != 'trash' AND ID IN ($sticky_posts)", $post_type));
        }
        //EDIT - END
        ...
        if (!empty($sticky_posts_count)) {
            ...
            $sticky_link = array('sticky' => "<a href='edit.php?post_type=$post_type&amp;show_sticky=1'$class>" . 
                esc_html__('Sticky', 'role-manager') . ' <span class="count">('. $sticky_posts_count .')</span></a>');                
