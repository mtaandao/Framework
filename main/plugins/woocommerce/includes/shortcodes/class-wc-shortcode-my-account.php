...
class WC_Shortcode_My_Account {
	...
	public static function output( $atts ) {
		...
		if ( ! is_user_logged_in() ) {
			...
			if ( isset( $mn->query_vars['lost-password'] ) ) {
				self::lost_password();
			} else {
	...
	public static function edit_address( $load_address = 'billing' ) {
		...
		// Enqueue scripts
		mn_enqueue_script( 'wc-country-select' );
		mn_enqueue_script( 'wc-address-i18n' );
	...
	public static function lost_password() {
		 ...
		 */
			...
			if ( isset( $_COOKIE[ 'mn-resetpass-' . COOKIEHASH ] ) && 0 < strpos( $_COOKIE[ 'mn-resetpass-' . COOKIEHASH ], ':' ) ) {
				list( $rp_login, $rp_key ) = array_map( 'wc_clean', explode( ':', mn_unslash( $_COOKIE[ 'mn-resetpass-' . COOKIEHASH ] ), 2 ) );
	 ...
	 *
	 * Based on retrieve_password() in core login.php.
	 *
	 * @uses $mndb Mtaandao Database object
	...
	public static function retrieve_password() {
		...
		$hashed = $mn_hasher->HashPassword( $key );

		$mndb->update( $mndb->users, array( 'user_activation_key' => $hashed ), array( 'user_login' => $user_login ) );
	 ...
	 * @return MN_USER|bool User's database row on success, false for invalid keys
	 */
	public static function check_password_reset_key( $key, $login ) {
		...
		$user = $mndb->get_row( $mndb->prepare( "SELECT * FROM $mndb->users WHERE user_login = %s", $login ) );
		...
		if ( ! empty( $user ) ) {
			...
			$valid = $mn_hasher->CheckPassword( $key, $user->user_activation_key );
		}
	...
	public static function reset_password( $user, $new_pass ) {
		...
		mn_password_change_notification( $user );
	}
	...
	public static function set_reset_password_cookie( $value = '' ) {
		...
		$rp_path   = current( explode( '?', mn_unslash( $_SERVER['REQUEST_URI'] ) ) );
