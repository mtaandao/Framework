...
class RWMB_Image_Field extends RWMB_File_Field {
	...
	public static function admin_enqueue_scripts() {
		...
		mn_enqueue_style( 'rwmb-image', RWMB_CSS_URL . 'image.css', array(), RWMB_VER );
	}
	...
	public static function file_html( $image ) {
		list( $src ) = mn_get_attachment_image_src( $image, 'thumbnail' );
		return sprintf(
			'<li id="item_%s">
				<img src="%s">
	...
	public static function file_info( $file, $args = array() ) {
		...
		$image      = mn_get_attachment_image_src( $file, $args['size'] );
		...
		$info       = array(
			...
			'path'        => $path,
			'url'         => $image[0],
			'full_url'    => mn_get_attachment_url( $file ),
			'title'       => $attachment->post_title,
			'caption'     => $attachment->post_excerpt,
			'description' => $attachment->post_content,
			'alt'         => get_post_meta( $file, '_mn_attachment_image_alt', true ),
		);
		if ( function_exists( 'mn_get_attachment_image_srcset' ) ) {
			$info['srcset'] = mn_get_attachment_image_srcset( $file );
		}

		$info = mn_parse_args( $info, mn_get_attachment_metadata( $file ) );
