...
function wc_template_redirect() {
	...
	if ( ! empty( $_GET['page_id'] ) && '' === get_option( 'permalink_structure' ) && $_GET['page_id'] == wc_get_page_id( 'shop' ) ) {
		mn_safe_redirect( get_post_type_archive_link('product') );
		exit;
	}

	// When on the checkout with an empty cart, redirect to cart page
	elseif ( is_page( wc_get_page_id( 'checkout' ) ) && WC()->cart->is_empty() && empty( $mn->query_vars['order-pay'] ) && ! isset( $mn->query_vars['order-received'] ) ) {
	...
	// Logout
	elseif ( isset( $mn->query_vars['customer-logout'] ) ) {
		mn_redirect( str_replace( '&amp;', '&', mn_logout_url( wc_get_page_permalink( 'myaccount' ) ) ) );
		exit;
	}

	// Redirect to the product page if we have a single product
	elseif ( is_search() && is_post_type_archive( 'product' ) && apply_filters( 'woocommerce_redirect_single_search_result', true ) && 1 === absint( $mn_query->found_posts ) ) {
		$product = wc_get_product( $mn_query->post );

		if ( $product && $product->is_visible() ) {
			mn_safe_redirect( get_permalink( $product->id ), 302 );
			exit;
		}
	}
...
if ( ! function_exists( 'woocommerce_demo_store' ) ) {
	...
	function woocommerce_demo_store() {
		...
		echo apply_filters( 'woocommerce_demo_store', '<p class="demo_store">' . mn_kses_post( $notice ) . '</p>', $notice );
	}
}
...
if ( ! function_exists( 'woocommerce_template_loop_add_to_cart' ) ) {
	...
	function woocommerce_template_loop_add_to_cart( $args = array() ) {
		...
		if ( $product ) {
			...
			$args = apply_filters( 'woocommerce_loop_add_to_cart_args', mn_parse_args( $args, $defaults ), $product );
...
if ( ! function_exists( 'woocommerce_catalog_ordering' ) ) {
	...
	function woocommerce_catalog_ordering() {
		...
		if ( 1 === $mn_query->found_posts || ! woocommerce_products_will_display() ) {
			return;
		}
...
if ( ! function_exists( 'woocommerce_variable_add_to_cart' ) ) {
	...
	function woocommerce_variable_add_to_cart() {
		...
		mn_enqueue_script( 'wc-add-to-cart-variation' );
...
if ( ! function_exists( 'woocommerce_quantity_input' ) ) {
	...
	function woocommerce_quantity_input( $args = array(), $product = null, $echo = true ) {
		...
		$args = apply_filters( 'woocommerce_quantity_input_args', mn_parse_args( $args, $defaults ), $product );
...
if ( ! function_exists( 'woocommerce_review_display_gravatar' ) ) {
	 ...
	 *
	 * @param array $comment MN_Comment.
	 * @return void
	 */
...
if ( ! function_exists( 'woocommerce_breadcrumb' ) ) {
	...
	function woocommerce_breadcrumb( $args = array() ) {
		$args = mn_parse_args( $args, apply_filters( 'woocommerce_breadcrumb_defaults', array(
			'delimiter'   => '&nbsp;&#47;&nbsp;',
...
if ( ! function_exists( 'woocommerce_products_will_display' ) ) {
	...
	function woocommerce_products_will_display() {
		...
		if ( false === ( $products_will_display = get_transient( $transient_name ) ) ) {
			$has_children = $mndb->get_col( $mndb->prepare( "SELECT term_id FROM {$mndb->term_taxonomy} WHERE parent = %d AND taxonomy = %s", $term->term_id, $term->taxonomy ) );
...
if ( ! function_exists( 'woocommerce_product_subcategories' ) ) {
	...
	function woocommerce_product_subcategories( $args = array() ) {
		...
		// NOTE: using child_of instead of parent - this is not ideal but due to a MN bug ( https://core.trac.wordpress.org/ticket/15626 ) pad_counts won't work
		...
		if ( ! apply_filters( 'woocommerce_product_subcategories_hide_empty', false ) ) {
			$product_categories = mn_list_filter( $product_categories, array( 'count' => 0 ), 'NOT' );
		}
		...
		if ( $product_categories ) {
			...
			if ( is_product_category() ) {
				...
				switch ( $display_type ) {
					case 'subcategories' :
						$mn_query->post_count    = 0;
						$mn_query->max_num_pages = 0;
					break;
					case '' :
...
if ( ! function_exists( 'woocommerce_subcategory_thumbnail' ) ) {
	...
	function woocommerce_subcategory_thumbnail( $category ) {
		...
		if ( $thumbnail_id ) {
			$image = mn_get_attachment_image_src( $thumbnail_id, $small_thumbnail_size  );
			$image = $image[0];
		} else {
...
if ( ! function_exists( 'wc_dropdown_variation_attribute_options' ) ) {
	...
	function wc_dropdown_variation_attribute_options( $args = array() ) {
		$args = mn_parse_args( apply_filters( 'woocommerce_dropdown_variation_attribute_options_args', $args ), array(
			'options'          => false,
			'attribute'        => false,
			'product'          => false,
			'selected' 	       => false,
...
if ( ! function_exists( 'woocommerce_account_content' ) ) {
	...
	function woocommerce_account_content() {
		...
		foreach ( $mn->query_vars as $key => $value ) {
