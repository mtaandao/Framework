...
class WC_Shipping_Legacy_Flat_Rate extends WC_Shipping_Method {
	...
	public function process_admin_options() {
		...
		if ( 'no' === $this->settings[ 'enabled' ] ) {
			mn_redirect( admin_url( 'admin.php?page=wc-settings&tab=shipping&section=options' ) );
			exit;
		}
	}
	...
	public function calculate_shipping( $package = array() ) {
		...
		foreach ( $found_shipping_classes as $shipping_class => $products ) {
			...
			$class_cost = $this->evaluate_cost( $class_cost_string, array(
				'qty'  => array_sum( mn_list_pluck( $products, 'quantity' ) ),
				'cost' => array_sum( mn_list_pluck( $products, 'line_total' ) )
			) );
