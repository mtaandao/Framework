...
class WC_REST_Order_Notes_Controller extends WC_REST_Controller {
	...
	public function get_items( $request ) {
		...
		if ( empty( $order->post_type ) || $this->post_type !== $order->post_type ) {
			return new MN_Error( 'woocommerce_rest_{$this->post_type}_invalid_id', __( 'Invalid order id.', 'woocommerce' ), array( 'status' => 404 ) );
		}
	...
	public function create_item( $request ) {
		...
		if ( empty( $order->post_type ) || $this->post_type !== $order->post_type ) {
			return new MN_Error( 'woocommerce_rest_order_invalid_id', __( 'Invalid order id.', 'woocommerce' ), array( 'status' => 404 ) );
		}
		...
		if ( ! $note_id ) {
			return new MN_Error( 'woocommerce_api_cannot_create_order_note', __( 'Cannot create order note, please try again.', 'woocommerce' ), array( 'status' => 500 ) );
		}
		 ...
		 *
		 * @param MN_Comment      $note      New order note object.
	...
	public function delete_item( $request ) {
		...
		if ( ! $force ) {
			return new MN_Error( 'woocommerce_rest_trash_not_supported', __( 'Webhooks do not support trashing.', 'woocommerce' ), array( 'status' => 501 ) );
		}
		...
		$result = mn_delete_comment( $note->comment_ID, true );

		if ( ! $result ) {
			return new MN_Error( 'woocommerce_rest_cannot_delete', sprintf( __( 'The %s cannot be deleted.', 'woocommerce' ), 'order_note' ), array( 'status' => 500 ) );
		}

		/**
		 * Fires after a order note is deleted or trashed via the REST API.
		 *
		 * @param MN_Comment       $note     The deleted or trashed order note.
	 ...
	 *
	 * @param MN_Comment $note Order note object.
	 * @param MN_REST_Request $request Request object.
	...
	public function prepare_item_for_response( $note, $request ) {
		 ...
		 * @param MN_Comment       $note     Order note object used to create response.
		 * @param MN_REST_Request  $request  Request object.
		 */
	 ...
	 *
	 * @param MN_Comment $note Delivery order_note object.
