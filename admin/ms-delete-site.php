...
if ( ! current_user_can( 'delete_site' ) )
	mn_die(__( 'Sorry, you are not allowed to delete this site.'));

if ( isset( $_GET['h'] ) && $_GET['h'] != '' && get_option( 'delete_blog_hash' ) != false ) {
	if ( hash_equals( get_option( 'delete_blog_hash' ), $_GET['h'] ) ) {
		mnmu_delete_blog( $mndb->blogid );
		mn_die( sprintf( __( 'Thank you for using %s, your site has been deleted. Happy trails to you until we meet again.' ), $current_site->site_name ) );
	} else {
		mn_die( __( "I'm sorry, the link you clicked is stale. Please select another option." ) );
	}
}
...
if ( isset( $_POST['action'] ) && $_POST['action'] == 'deleteblog' && isset( $_POST['confirmdelete'] ) && $_POST['confirmdelete'] == '1' ) {
	...
	$hash = mn_generate_password( 20, false );
...
###SITE_NAME###" );
	...
	mn_mail( get_option( 'admin_email' ), "[ " . mn_specialchars_decode( get_option( 'blogname' ) ) . " ] ".__( 'Delete My Site' ), $content );
	?>
...
<?php } else {
	...
	<form method="post" name="deletedirect">
		<?php mn_nonce_field( 'delete-blog' ) ?>
