...
function wc_get_attribute_taxonomies() {
	if ( false === ( $attribute_taxonomies = get_transient( 'wc_attribute_taxonomies' ) ) ) {
		...
		$attribute_taxonomies = $mndb->get_results( "SELECT * FROM " . $mndb->prefix . "woocommerce_attribute_taxonomies order by attribute_name ASC;" );
...
function wc_attribute_taxonomy_name_by_id( $attribute_id ) {
	...
	if ( $attribute_name && ! is_mn_error( $attribute_name ) ) {
...
function wc_attribute_taxonomy_id_by_name( $name ) {
	...
	$taxonomies = mn_list_pluck( wc_get_attribute_taxonomies(), 'attribute_id', 'attribute_name' );
...
function wc_attribute_label( $name, $product = '' ) {
	...
	if ( taxonomy_is_product_attribute( $name ) ) {
		...
		$label = $mndb->get_var( $mndb->prepare( "SELECT attribute_label FROM {$mndb->prefix}woocommerce_attribute_taxonomies WHERE attribute_name = %s;", $name ) );
...
function wc_attribute_orderby( $name ) {
	global $wc_product_attributes, $mndb;
	...
	if ( isset( $wc_product_attributes[ 'pa_' . $name ] ) ) {
		...
		$orderby = $mndb->get_var( $mndb->prepare( "SELECT attribute_orderby FROM " . $mndb->prefix . "woocommerce_attribute_taxonomies WHERE attribute_name = %s;", $name ) );
	}
...
function wc_check_if_attribute_name_is_reserved( $attribute_name ) {
	...
	$reserved_terms = array(
		...
		'shomnosts', 'static', 'subpost', 'subpost_id', 'tag', 'tag__and', 'tag__in', 'tag__not_in', 'tag_id',
