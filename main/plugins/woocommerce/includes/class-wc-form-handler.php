...
class WC_Form_Handler {
	...
	public static function init() {
		...
		add_action( 'mn_loaded', array( __CLASS__, 'checkout_action' ), 20 );
		add_action( 'mn_loaded', array( __CLASS__, 'process_login' ), 20 );
		add_action( 'mn_loaded', array( __CLASS__, 'process_registration' ), 20 );
		add_action( 'mn_loaded', array( __CLASS__, 'process_lost_password' ), 20 );
		add_action( 'mn_loaded', array( __CLASS__, 'process_reset_password' ), 20 );
		add_action( 'mn_loaded', array( __CLASS__, 'cancel_order' ), 20 );
		add_action( 'mn_loaded', array( __CLASS__, 'order_again' ), 20 );
		add_action( 'mn_loaded', array( __CLASS__, 'update_cart_action' ), 20 );
		add_action( 'mn_loaded', array( __CLASS__, 'add_to_cart_action' ), 20 );

		// May need $mn global to access query vars.
		add_action( 'mn', array( __CLASS__, 'pay_action' ), 20 );
		add_action( 'mn', array( __CLASS__, 'add_payment_method_action' ), 20 );
		add_action( 'mn', array( __CLASS__, 'delete_payment_method_action' ), 20 );
		add_action( 'mn', array( __CLASS__, 'set_default_payment_method_action' ), 20 );
	}
	...
	public static function redirect_reset_password_link() {
		if ( is_account_page() && ! empty( $_GET['key'] ) && ! empty( $_GET['login'] ) ) {
			...
			mn_safe_redirect( add_query_arg( 'show-reset-form', 'true', wc_lostpassword_url() ) );
			exit;
		}
	}
	...
	public static function save_address() {
		...
		if ( empty( $_POST['action'] ) || 'edit_address' !== $_POST['action'] || empty( $_POST['_mnnonce'] ) || ! mn_verify_nonce( $_POST['_mnnonce'], 'woocommerce-edit_address' ) ) {
			return;
		}
		...
		$load_address = isset( $mn->query_vars['edit-address'] ) ? wc_edit_address_i18n( sanitize_title( $mn->query_vars['edit-address'] ), true ) : 'billing';
		...
		if ( wc_notice_count( 'error' ) == 0 ) {
			...
			mn_safe_redirect( wc_get_page_permalink( 'myaccount' ) );
			exit;
		}
	}
	...
	public static function save_account_details() {
		...
		if ( empty( $_POST[ 'action' ] ) || 'save_account_details' !== $_POST[ 'action' ] || empty( $_POST['_mnnonce'] ) || ! mn_verify_nonce( $_POST['_mnnonce'], 'save_account_details' ) ) {
			return;
		}
		...
		if ( ! empty( $pass1 ) && ! mn_check_password( $pass_cur, $current_user->user_pass, $current_user->ID ) ) {
	...
	public static function checkout_action() {
		if ( isset( $_POST['woocommerce_checkout_place_order'] ) || isset( $_POST['woocommerce_checkout_update_totals'] ) ) {
			...
			if ( WC()->cart->is_empty() ) {
				mn_redirect( wc_get_page_permalink( 'cart' ) );
				exit;
			}
	...
	public static function pay_action() {
		...
		if ( isset( $_POST['woocommerce_pay'] ) && isset( $_POST['_mnnonce'] ) && mn_verify_nonce( $_POST['_mnnonce'], 'woocommerce-pay' ) ) {

			ob_start();

			// Pay for existing order
			$order_key  = $_GET['key'];
			$order_id   = absint( $mn->query_vars['order-pay'] );
			...
			if ( $order->id == $order_id && $order->order_key == $order_key && $order->needs_payment() ) {
				...
				if ( $order->needs_payment() ) {
					...
					mn_safe_redirect( $order->get_checkout_order_received_url() );
					exit;
				}
	...
	public static function add_payment_method_action() {
		if ( isset( $_POST['woocommerce_add_payment_method'], $_POST['payment_method'], $_POST['_mnnonce'] ) && mn_verify_nonce( $_POST['_mnnonce'], 'woocommerce-add-payment-method' ) ) {
	...
	public static function delete_payment_method_action() {
		...
		if ( isset( $mn->query_vars['delete-payment-method'] ) ) {

			$token_id = absint( $mn->query_vars['delete-payment-method'] );
			...
			if ( false === mn_verify_nonce( $_REQUEST['_mnnonce'], 'delete-payment-method-' . $token_id ) ) {
				wc_add_notice( __( 'Invalid payment method', 'woocommerce' ), 'error' );
				$delete = false;
			}
			...
			mn_redirect( wc_get_account_endpoint_url( 'payment-methods' ) );
			exit();
		}
	...
	public static function set_default_payment_method_action() {
		...
		if ( isset( $mn->query_vars['set-default-payment-method'] ) ) {

			$token_id = absint( $mn->query_vars['set-default-payment-method'] );
			$token = WC_Payment_Tokens::get( $token_id );
			$delete = true;
			...
			if ( false === mn_verify_nonce( $_REQUEST['_mnnonce'], 'set-default-payment-method-' . $token_id ) ) {
				wc_add_notice( __( 'Invalid payment method', 'woocommerce' ), 'error' );
				$delete = false;
			}
	...
	public static function update_cart_action() {
		...
		// Remove from cart
		elseif ( ! empty( $_GET['remove_item'] ) && isset( $_GET['_mnnonce'] ) && mn_verify_nonce( $_GET['_mnnonce'], 'woocommerce-cart' ) ) {
			...
			$referer  = mn_get_referer() ? remove_query_arg( array( 'remove_item', 'add-to-cart', 'added-to-cart' ), add_query_arg( 'removed_item', '1', mn_get_referer() ) ) : wc_get_cart_url();
			mn_safe_redirect( $referer );
			exit;
		}

		// Undo Cart Item
		elseif ( ! empty( $_GET['undo_item'] ) && isset( $_GET['_mnnonce'] ) && mn_verify_nonce( $_GET['_mnnonce'], 'woocommerce-cart' ) ) {
			$cart_item_key = sanitize_text_field( $_GET['undo_item'] );

			WC()->cart->restore_cart_item( $cart_item_key );

			$referer  = mn_get_referer() ? remove_query_arg( array( 'undo_item', '_mnnonce' ), mn_get_referer() ) : wc_get_cart_url();
			mn_safe_redirect( $referer );
			exit;
		}

		// Update Cart - checks apply_coupon too because they are in the same form
		if ( ( ! empty( $_POST['apply_coupon'] ) || ! empty( $_POST['update_cart'] ) || ! empty( $_POST['proceed'] ) ) && isset( $_POST['_mnnonce'] ) && mn_verify_nonce( $_POST['_mnnonce'], 'woocommerce-cart' ) ) {
			...
			if ( ! empty( $_POST['proceed'] ) ) {
				mn_safe_redirect( wc_get_checkout_url() );
				exit;
			} elseif ( $cart_updated ) {
				wc_add_notice( __( 'Cart updated.', 'woocommerce' ) );
				$referer = remove_query_arg( 'remove_coupon', ( mn_get_referer() ? mn_get_referer() : wc_get_cart_url() ) );
				mn_safe_redirect( $referer );
				exit;
			}
		}
	...
	public static function order_again() {
		...
		// Nothing to do
		if ( ! isset( $_GET['order_again'] ) || ! is_user_logged_in() || ! isset( $_GET['_mnnonce'] ) || ! mn_verify_nonce( $_GET['_mnnonce'], 'woocommerce-order_again' ) ) {
			return;
		}
		...
		mn_safe_redirect( wc_get_cart_url() );
		exit;
	}
	...
	private static function add_to_cart_handler_variable( $product_id ) {
		...
		if ( empty( $variation_id ) ) {
			$variation_id = $adding_to_cart->get_matching_variation( mn_unslash( $_POST ) );
		}
	...
	public static function process_login() {
		$nonce_value = isset( $_POST['_mnnonce'] ) ? $_POST['_mnnonce'] : '';
		$nonce_value = isset( $_POST['woocommerce-login-nonce'] ) ? $_POST['woocommerce-login-nonce'] : $nonce_value;

		if ( ! empty( $_POST['login'] ) && mn_verify_nonce( $nonce_value, 'woocommerce-login' ) ) {

			try {
				$creds    = array();
				$username = trim( $_POST['username'] );

				$validation_error = new MN_Error();
				...
				$user                   = mn_signon( apply_filters( 'woocommerce_login_credentials', $creds ), $secure_cookie );
				...
				if ( is_mn_error( $user ) ) {
					...
					} elseif ( mn_get_referer() ) {
						$redirect = mn_get_referer();
					} else {
						$redirect = wc_get_page_permalink( 'myaccount' );
					}

					mn_redirect( apply_filters( 'woocommerce_login_redirect', $redirect, $user ) );
					exit;
				}
	...
	public static function process_lost_password() {
		if ( isset( $_POST['wc_reset_password'] ) && isset( $_POST['user_login'] ) && isset( $_POST['_mnnonce'] ) && mn_verify_nonce( $_POST['_mnnonce'], 'lost_password' ) ) {
			$success = WC_Shortcode_My_Account::retrieve_password();

			// If successful, redirect to my account with query arg set
			if ( $success ) {
				mn_redirect( add_query_arg( 'reset-link-sent', 'true', remove_query_arg( array( 'key', 'login', 'reset' ) ) ) );
				exit;
			}
		}
	}
	...
	public static function process_reset_password() {
		$posted_fields = array( 'wc_reset_password', 'password_1', 'password_2', 'reset_key', 'reset_login', '_mnnonce' );
		...
		if ( ! mn_verify_nonce( $posted_fields['_mnnonce'], 'reset_password' ) ) {
			return;
		}
		...
		if ( $user instanceof MN_User ) {
			...
			wc_add_mn_error_notices( $errors );
			...
			if ( 0 === wc_notice_count( 'error' ) ) {
				...
				mn_redirect( add_query_arg( 'reset', 'true', remove_query_arg( array( 'key', 'login', 'reset-link-sent' ) ) ) );
				exit;
			}
		}
	}
	...
	public static function process_registration() {
		...
		if ( ! empty( $_POST['register'] ) && mn_verify_nonce( $nonce_value, 'woocommerce-register' ) ) {
			...
			try {
				...
				mn_safe_redirect( apply_filters( 'woocommerce_registration_redirect', mn_get_referer() ? mn_get_referer() : wc_get_page_permalink( 'myaccount' ) ) );
				exit;
