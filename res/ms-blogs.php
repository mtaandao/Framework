 ...
 */
function mnmu_update_blogs_date() {
	global $mndb;

	update_blog_details( $mndb->blogid, array('last_updated' => current_time('mysql', true)) );
	/**
	 ...
	 */
	do_action( 'mnmu_blog_updated', $mndb->blogid );
}
 ...
 *                                  Default is true.
 * @return MN_Site|false Blog details on success. False on failure.
 */
function get_blog_details( $fields = null, $get_all = true ) {
	...
	if ( is_array($fields ) ) {
		if ( isset($fields['blog_id']) ) {
			...
			$blog = mn_cache_get($key, 'blog-lookup');
			if ( false !== $blog )
				return $blog;
			if ( substr( $fields['domain'], 0, 4 ) == 'www.' ) {
				$nowww = substr( $fields['domain'], 4 );
				$blog = $mndb->get_row( $mndb->prepare( "SELECT * FROM $mndb->blogs WHERE domain IN (%s,%s) AND path = %s ORDER BY CHAR_LENGTH(domain) DESC", $nowww, $fields['domain'], $fields['path'] ) );
			} else {
				$blog = $mndb->get_row( $mndb->prepare( "SELECT * FROM $mndb->blogs WHERE domain = %s AND path = %s", $fields['domain'], $fields['path'] ) );
			}
			if ( $blog ) {
				mn_cache_set($blog->blog_id . 'short', $blog, 'blog-details');
			...
			if ( substr( $fields['domain'], 0, 4 ) == 'www.' ) {
				$nowww = substr( $fields['domain'], 4 );
				$blog = $mndb->get_row( $mndb->prepare( "SELECT * FROM $mndb->blogs WHERE domain IN (%s,%s) ORDER BY CHAR_LENGTH(domain) DESC", $nowww, $fields['domain'] ) );
			} else {
				$blog = $mndb->get_row( $mndb->prepare( "SELECT * FROM $mndb->blogs WHERE domain = %s", $fields['domain'] ) );
			}
			if ( $blog ) {
				mn_cache_set($blog->blog_id . 'short', $blog, 'blog-details');
				$blog_id = $blog->blog_id;
	...
	$details = mn_cache_get( $blog_id . $all, 'blog-details' );
	...
	if ( $details ) {
		if ( ! is_object( $details ) ) {
			if ( $details == -1 ) {
				...
				mn_cache_delete( $blog_id . $all, 'blog-details' );
				unset($details);
			}
		} else {
			return $details;
	...
	// Try the other cache.
	if ( $get_all ) {
		$details = mn_cache_get( $blog_id . 'short', 'blog-details' );
	} else {
		$details = mn_cache_get( $blog_id, 'blog-details' );
		...
		if ( $details ) {
			if ( ! is_object( $details ) ) {
				if ( $details == -1 ) {
					...
					// Clear old pre-serialized objects. Cache clients do better with that.
					mn_cache_delete( $blog_id, 'blog-details' );
					unset($details);
				}
			} else {
				return $details;
	...
	if ( empty($details) ) {
		$details = MN_Site::get_instance( $blog_id );
		if ( ! $details ) {
			// Set the full cache.
			mn_cache_set( $blog_id, -1, 'blog-details' );
			return false;
		}
	}

	if ( ! $details instanceof MN_Site ) {
		$details = new MN_Site( $details );
	}
	...
	mn_cache_set( $key, $details, 'blog-lookup' );
...
function update_blog_details( $blog_id, $details = array() ) {
	...
	$result = $mndb->update( $mndb->blogs, $update_details, array('blog_id' => $blog_id) );
...
function clean_blog_cache( $blog ) {
	...
	mn_cache_delete( $blog_id, 'sites' );
	mn_cache_delete( $blog_id, 'site-details' );
	mn_cache_delete( $blog_id , 'blog-details' );
	mn_cache_delete( $blog_id . 'short' , 'blog-details' );
...
function get_site( $site = null ) {
	...
	if ( $site instanceof MN_Site ) {
		$_site = $site;
	} elseif ( is_object( $site ) ) {
		$_site = new MN_Site( $site );
	} else {
		$_site = MN_Site::get_instance( $site );
	}
...
function _prime_site_caches( $ids ) {
	...
	if ( ! empty( $non_cached_ids ) ) {
		$fresh_sites = $mndb->get_results( sprintf( "SELECT * FROM $mndb->blogs WHERE blog_id IN (%s)", join( ",", array_map( 'intval', $non_cached_ids ) ) ) );
...
function update_site_cache( $sites ) {
	...
	foreach ( $sites as $site ) {
		mn_cache_add( $site->blog_id, $site, 'sites' );
		mn_cache_add( $site->blog_id . 'short', $site, 'blog-details' );
	}
}
...
/**
 * Removes option by name for a given blog id. Prevents removal of protected Mtaandao options.
 *
 * @since MU
 *
 * @param int    $id     A blog ID. Can be null to refer to the current blog.
 ...
 * @global mndb            $mndb
 * @global int             $blog_id
 * @global array           $_mn_switched_stack
 * @global bool            $switched
 * @global string          $table_prefix
 * @global MN_Object_Cache $mn_object_cache
 *
...
function switch_to_blog( $new_blog, $deprecated = null ) {
	...
	if ( function_exists( 'mn_cache_switch_to_blog' ) ) {
		mn_cache_switch_to_blog( $new_blog );
	} else {
		...
		if ( function_exists( 'mn_cache_add_global_groups' ) ) {
			if ( is_array( $global_groups ) ) {
				mn_cache_add_global_groups( $global_groups );
			} else {
				mn_cache_add_global_groups( array( 'users', 'userlogins', 'usermeta', 'user_meta', 'useremail', 'userslugs', 'site-transient', 'site-options', 'site-lookup', 'blog-lookup', 'blog-details', 'rss', 'global-posts', 'blog-id-cache', 'networks', 'sites', 'site-details' ) );
			}
...
function ms_is_switched() {
	return ! empty( $GLOBALS['_mn_switched_stack'] );
}
...
function update_blog_status( $blog_id, $pref, $value, $deprecated = null ) {
	...
	$result = $mndb->update( $mndb->blogs, array($pref => $value, 'last_updated' => current_time('mysql', true)), array('blog_id' => $blog_id) );
...
function get_blog_status( $id, $pref ) {
	...
	return $mndb->get_var( $mndb->prepare("SELECT %s FROM {$mndb->blogs} WHERE blog_id = %d", $pref, $id) );
}
...
function get_last_updated( $deprecated = '', $start = 0, $quantity = 40 ) {
	...
	return $mndb->get_results( $mndb->prepare("SELECT blog_id, domain, path FROM $mndb->blogs WHERE site_id = %d AND public = '1' AND archived = '0' AND mature = '0' AND spam = '0' AND deleted = '0' AND last_updated != '0000-00-00 00:00:00' ORDER BY last_updated DESC limit %d, %d", $mndb->siteid, $start, $quantity ) , ARRAY_A );
}
 ...
 *
 * @param MN_Network|int|null $network Optional. Network to retrieve. Default is the current network.
 * @return MN_Network|null The network object or null if not found.
 */
function get_network( $network = null ) {
	...
	if ( $network instanceof MN_Network ) {
		$_network = $network;
	} elseif ( is_object( $network ) ) {
		$_network = new MN_Network( $network );
	} else {
		$_network = MN_Network::get_instance( $network );
	}
...
function clean_network_cache( $ids ) {
	...
	mn_cache_set( 'last_changed', microtime(), 'networks' );
}
...
function update_network_cache( $networks ) {
	foreach ( (array) $networks as $network ) {
		mn_cache_add( $network->id, $network, 'networks' );
	}
}
...
function _prime_network_caches( $network_ids ) {
	...
	if ( !empty( $non_cached_ids ) ) {
		$fresh_networks = $mndb->get_results( sprintf( "SELECT $mndb->site.* FROM $mndb->site WHERE id IN (%s)", join( ",", array_map( 'intval', $non_cached_ids ) ) ) );
