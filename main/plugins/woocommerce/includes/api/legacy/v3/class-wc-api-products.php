...
class WC_API_Products extends WC_API_Resource {
	...
	public function get_products_count( $type = null, $filter = array() ) {
		try {
			...
			return new MN_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function create_product( $data ) {
		...
		try {
			...
			$id = mn_insert_post( $new_product, true );
	...
	public function edit_product( $id, $data ) {
		try {
			...
			// Product title.
			if ( isset( $data['title'] ) ) {
				mn_update_post( array( 'ID' => $id, 'post_title' => wc_clean( $data['title'] ) ) );
			}

			// Product name (slug).
			if ( isset( $data['name'] ) ) {
				mn_update_post( array( 'ID' => $id, 'post_name' => sanitize_title( $data['name'] ) ) );
			}

			// Product status.
			if ( isset( $data['status'] ) ) {
				mn_update_post( array( 'ID' => $id, 'post_status' => wc_clean( $data['status'] ) ) );
			}
			...
			if ( isset( $data['short_description'] ) ) {
				...
				mn_update_post( array( 'ID' => $id, 'post_excerpt' => $post_excerpt ) );
			}
			...
			if ( isset( $data['description'] ) ) {
				...
				mn_update_post( array( 'ID' => $id, 'post_content' => $post_content ) );
			}

			// Menu order.
			if ( isset( $data['menu_order'] ) ) {
				mn_update_post( array( 'ID' => $id, 'menu_order' => intval( $data['menu_order'] ) ) );
			}
	...
	public function delete_product( $id, $force = false ) {
		...
		if ( $force ) {
			...
			$result = mn_delete_post( $id, true );
		} else {
			$result = mn_trash_post( $id );
		}

		if ( ! $result ) {
			return new MN_Error( 'woocommerce_api_cannot_delete_product', sprintf( __( 'This %s cannot be deleted', 'woocommerce' ), 'product' ), array( 'status' => 500 ) );
		}
	...
	public function get_product_orders( $id, $fields = null, $filter = array(), $status = null, $page = 1 ) {
		...
		$order_ids = $mndb->get_col( $mndb->prepare( "
			SELECT order_id
			FROM {$mndb->prefix}woocommerce_order_items
			WHERE order_item_id IN ( SELECT order_item_id FROM {$mndb->prefix}woocommerce_order_itemmeta WHERE meta_key = '_product_id' AND meta_value = %d )
	...
	public function get_product_category( $id, $fields = null ) {
		try {
			...
			if ( $image_id = get_woocommerce_term_meta( $term_id, 'thumbnail_id' ) ) {
				$image = mn_get_attachment_url( $image_id );
			}
	 ...
	 * @since  2.5.0
	 * @param  array          $data Posted data
	 * @return array|MN_Error       Product category if succeed, otherwise MN_Error
	 *                              will be returned
	 */
	public function create_product_category( $data ) {
		...
		try {
			...
			$data = mn_parse_args( $data['product_category'], $defaults );
			...
			$insert = mn_insert_term( $data['name'], 'product_cat', $data );
			if ( is_mn_error( $insert ) ) {
			...
			if ( $image_id && mn_attachment_is_image( $image_id ) ) {
	...
	public function edit_product_category( $id, $data ) {
		...
		try {
			...
			if ( isset( $data['image'] ) ) {
				...
				if ( ! mn_attachment_is_image( $image_id ) ) {
					$image_id = '';
				}
			}

			$update = mn_update_term( $id, 'product_cat', $data );
			if ( is_mn_error( $update ) ) {
	 ...
	 * @since  2.5.0
	 * @param  int            $id Product category term ID
	 * @return array|MN_Error     Success message if succeed, otherwise MN_Error
	 *                            will be returned
	 */
	public function delete_product_category( $id ) {
		...
		try {
			...
			$id      = absint( $id );
			$deleted = mn_delete_term( $id, 'product_cat' );
			if ( ! $deleted || is_mn_error( $deleted ) ) {
			...
			if ( get_option( 'db_version' ) < 34370 ) {
				$mndb->delete( $mndb->woocommerce_termmeta, array( 'woocommerce_term_id' => $id ), array( '%d' ) );
			}
	 ...
	 * @since  2.5.0
	 * @param  array          $data Posted data
	 * @return array|MN_Error       Product tag if succeed, otherwise MN_Error
	 *                              will be returned
	 */
	public function create_product_tag( $data ) {
		try {
			...
			$data = mn_parse_args( $data['product_tag'], $defaults );
			$data = apply_filters( 'woocommerce_api_create_product_tag_data', $data, $this );

			$insert = mn_insert_term( $data['name'], 'product_tag', $data );
			if ( is_mn_error( $insert ) ) {
	...
	public function edit_product_tag( $id, $data ) {
		try {
			...
			$update = mn_update_term( $id, 'product_tag', $data );
			if ( is_mn_error( $update ) ) {
	...
	public function delete_product_tag( $id ) {
		try {
			...
			$id      = absint( $id );
			$deleted = mn_delete_term( $id, 'product_tag' );
			if ( ! $deleted || is_mn_error( $deleted ) ) {
	...
	private function query_products( $args ) {
		...
		return new MN_Query( $query_args );
	}
	...
	private function get_product_data( $product ) {
		return array(
			...
			'description'        => mnautop( do_shortcode( $product->get_post_data()->post_content ) ),
			...
			'categories'         => mn_get_post_terms( $product->id, 'product_cat', array( 'fields' => 'names' ) ),
			'tags'               => mn_get_post_terms( $product->id, 'product_tag', array( 'fields' => 'names' ) ),
			'images'             => $this->get_images( $product ),
			'featured_src'       => (string) mn_get_attachment_url( get_post_thumbnail_id( $product->is_type( 'variation' ) ? $product->variation_id : $product->id ) ),
			...
			'purchase_note'      => mnautop( do_shortcode( mn_kses_post( $product->purchase_note ) ) ),
	...
	protected function save_product_meta( $product_id, $data ) {
		...
		if ( isset( $data['type'] ) ) {
			...
			mn_set_object_terms( $product_id, $product_type, 'product_type' );
		} else {
			...
			foreach ( $data['attributes'] as $attribute ) {
				...
				if ( $is_taxonomy ) {
					...
					if ( taxonomy_exists( $taxonomy ) ) {
						mn_set_object_terms( $product_id, $values, $taxonomy );
					}
		...
		if ( isset( $data['parent_id'] ) ) {
			mn_update_post( array( 'ID' => $product_id, 'post_parent' => absint( $data['parent_id'] ) ) );
		}
		...
		if ( isset( $data['categories'] ) && is_array( $data['categories'] ) ) {
			...
			mn_set_object_terms( $product_id, $term_ids, 'product_cat' );
		}

		// Product tags.
		if ( isset( $data['tags'] ) && is_array( $data['tags'] ) ) {
			$term_ids = array_unique( array_map( 'intval', $data['tags'] ) );
			mn_set_object_terms( $product_id, $term_ids, 'product_tag' );
		}
		...
		if ( isset( $data['reviews_allowed'] ) ) {
			...
			$mndb->update( $mndb->posts, array( 'comment_status' => $reviews_allowed ), array( 'ID' => $product_id ) );
		}
	...
	protected function save_variations( $id, $data ) {
		...
		foreach ( $variations as $menu_order => $variation ) {
			...
			if ( ! $variation_id ) {
				...
				$variation_id = mn_insert_post( $new_variation );
				...
				$mndb->update( $mndb->posts, $update_variation, array( 'ID' => $variation_id ) );

				do_action( 'woocommerce_update_product_variation', $variation_id );
			}

			// Stop with we don't have a variation ID
			if ( is_mn_error( $variation_id ) ) {
			...
			if ( isset( $variation['image'] ) && is_array( $variation['image'] ) ) {
				...
				if ( $image && is_array( $image ) ) {
					if ( isset( $image['position'] ) && $image['position'] == 0 ) {
						...
						if ( ! empty( $image['alt'] ) ) {
							update_post_meta( $attachment_id, '_mn_attachment_image_alt', wc_clean( $image['alt'] ) );
						}

						// Set the image title if present.
						if ( ! empty( $image['title'] ) ) {
							mn_update_post( array( 'ID' => $attachment_id, 'post_title' => $image['title'] ) );
						}
			...
			if ( isset( $variation['description'] ) ) {
				update_post_meta( $variation_id, '_variation_description', mn_kses_post( $variation['description'] ) );
			}
	...
	private function save_product_shipping_data( $id, $data ) {
		...
		if ( isset( $data['shipping_class'] ) ) {
			mn_set_object_terms( $id, wc_clean( $data['shipping_class'] ), 'product_shipping_class' );
		}
	}
	...
	private function get_images( $product ) {
		...
		foreach ( $attachment_ids as $position => $attachment_id ) {
			...
			$attachment = mn_get_attachment_image_src( $attachment_id, 'full' );
			...
			$images[] = array(
				...
				'alt'        => get_post_meta( $attachment_id, '_mn_attachment_image_alt', true ),
	 ...
	 * @return int|MN_Error attachment id
	 */
	...
	protected function upload_image_from_url( $image_url, $upload_for = 'product_image' ) {
		...
		// Get the file.
		$response = mn_safe_remote_get( $image_url, array(
			'timeout' => 10
		) );
		...
		} elseif ( 200 !== mn_remote_retrieve_response_code( $response ) ) {
		...
		$mn_filetype = mn_check_filetype( $file_name, wc_rest_allowed_image_mime_types() );
		...
		// Upload the file.
		$upload = mn_upload_bits( $file_name, '', mn_remote_retrieve_body( $response ) );
	 ...
	 *
	 * @since  2.5.0
	 * @param  integer $upload Upload information from mn_upload_bits
	 ...
	 *
	 * @since  2.5.0
	 * @param  array $upload Upload information from mn_upload_bits
	 * @param  int   $id     Post ID. Default to 0.
	 * @return int           Attachment ID
	 */
	protected function set_uploaded_image_as_attachment( $upload, $id = 0 ) {
		$info    = mn_check_filetype( $upload['file'] );
		$title   = '';
		$content = '';

		if ( $image_meta = @mn_read_image_metadata( $upload['file'] ) ) {
		...
		$attachment_id = mn_insert_attachment( $attachment, $upload['file'], $id );
		if ( ! is_mn_error( $attachment_id ) ) {
			mn_update_attachment_metadata( $attachment_id, mn_generate_attachment_metadata( $attachment_id, $upload['file'] ) );
		}
	...
	public function get_product_attribute( $id, $fields = null ) {
		...
		try {
			...
			$attribute = $mndb->get_row( $mndb->prepare( "
				SELECT *
				FROM {$mndb->prefix}woocommerce_attribute_taxonomies
				WHERE attribute_id = %d
			 ", $id ) );

			if ( is_mn_error( $attribute ) || is_null( $attribute ) ) {
	...
	public function create_product_attribute( $data ) {
		...
		try {
			...
			$insert = $mndb->insert(
				$mndb->prefix . 'woocommerce_attribute_taxonomies',
				array(
	...
	public function delete_product_attribute( $id ) {
		...
		try {
			...
			$attribute_name = $mndb->get_var( $mndb->prepare( "
			...
			if ( taxonomy_exists( $taxonomy ) ) {
				...
				foreach ( $terms as $term ) {
					mn_delete_term( $term->term_id, $taxonomy );
				}
			}
	...
	public function create_product_attribute_term( $attribute_id, $data ) {
		...
		try {
			...
			if ( isset( $data['slug'] ) ) {
				$args['slug'] = sanitize_title( mn_unslash( $data['slug'] ) );
			}

			$term = mn_insert_term( $data['name'], $taxonomy, $args );
	...
	public function edit_product_attribute_term( $attribute_id, $id, $data ) {
		...
		try {
			...
			// Update name.
			if ( isset( $data['name'] ) ) {
				$args['name'] = wc_clean( mn_unslash( $data['name'] ) );
			}
			...
			$term = mn_update_term( $id, $taxonomy, $args );
	...
	public function delete_product_attribute_term( $attribute_id, $id ) {
		...
		try {
			...
			$id   = absint( $id );
			$term = mn_delete_term( $id, $taxonomy );
	...
	protected function clear_product( $product_id ) {
		...
		foreach ( (array) $attachments as $attachment ) {
			mn_delete_attachment( $attachment->ID, true );
		}

		// Delete product
		mn_delete_post( $product_id, true );
	}
	 ...
	 * @since  2.5.0
	 * @param  string|null    $fields Fields to limit response to
	 * @return array|MN_Error         List of product shipping classes if succeed,
	 *                                otherwise MN_Error will be returned
	 */
	 ...
	 * @param  string|null    $fields Fields to limit response to
	 * @return array|MN_Error         Product shipping class if succeed, otherwise
	 *                                MN_Error will be returned
	 */
	...
	public function create_product_shipping_class( $data ) {
		...
		try {
			...
			$data = mn_parse_args( $data['product_shipping_class'], $defaults );
			...
			$insert = mn_insert_term( $data['name'], 'product_shipping_class', $data );
			if ( is_mn_error( $insert ) ) {
	...
	public function edit_product_shipping_class( $id, $data ) {
		...
		try {
			...
			if ( is_mn_error( $shipping_class ) ) {
				return $shipping_class;
			}

			$update = mn_update_term( $id, 'product_shipping_class', $data );
			if ( is_mn_error( $update ) ) {
	...
	public function delete_product_shipping_class( $id ) {
		...
		try {
			...
			$id      = absint( $id );
			$deleted = mn_delete_term( $id, 'product_shipping_class' );
			if ( ! $deleted || is_mn_error( $deleted ) ) {
