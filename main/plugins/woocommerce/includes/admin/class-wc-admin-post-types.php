...
class WC_Admin_Post_Types {
	...
	public function __construct() {
		...
		// MN List table columns. Defined here so they are always available for events such as inline editing.
		...
		add_action( 'mn_trash_post', array( $this, 'trash_post' ) );
	...
	public function post_updated_messages( $messages ) {
		...
		$messages['product'] = array(
			...
			5 => isset( $_GET['revision'] ) ? sprintf( __( 'Product restored to revision from %s', 'woocommerce' ), mn_post_revision_title( (int) $_GET['revision'], false ) ) : false,
		...
		$messages['shop_order'] = array(
			...
			5 => isset( $_GET['revision'] ) ? sprintf( __( 'Order restored to revision from %s', 'woocommerce' ), mn_post_revision_title( (int) $_GET['revision'], false ) ) : false,
		...
		$messages['shop_coupon'] = array(
			...
			5 => isset( $_GET['revision'] ) ? sprintf( __( 'Coupon restored to revision from %s', 'woocommerce' ), mn_post_revision_title( (int) $_GET['revision'], false ) ) : false,
	...
	public function render_product_columns( $column ) {
		...
		switch ( $column ) {
			...
			case 'product_tag' :
				...
				break;
			case 'featured' :
				$url = mn_nonce_url( admin_url( 'admin-ajax.php?action=woocommerce_feature_product&product_id=' . $post->ID ), 'woocommerce-feature-product' );
	...
	/**
	 * Render product row actions for old version of Mtaandao.
	 * Since Mtaandao 4.3 we don't have to build the row actions.
	 *
	 * @param MN_Post $post
	 * @param string  $title
	 */
	private function _render_product_row_actions( $post, $title ) {
		...
		if ( version_compare( $mn_version, '4.3-beta', '>=' ) ) {
			return;
		}
		...
		if ( current_user_can( $post_type_object->cap->delete_post, $post->ID ) ) {
			if ( 'trash' == $post->post_status ) {
				$actions['untrash'] = '<a title="' . esc_attr( __( 'Restore this item from the Trash', 'woocommerce' ) ) . '" href="' . mn_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $post->ID ) ), 'untrash-post_' . $post->ID ) . '">' . __( 'Restore', 'woocommerce' ) . '</a>';
	...
	public function render_shop_coupon_columns( $column ) {
		...
		switch ( $column ) {
			...
			break;
			case 'description' :
				echo mn_kses_post( $post->post_excerpt );
			break;
		}
	}

	/**
	 * Render shop_coupon row actions for old version of Mtaandao.
	 * Since Mtaandao 4.3 we don't have to build the row actions.
	 *
	 * @param MN_Post $post
	 * @param string  $title
					...
					<?php
						...
						if ( $the_order->has_status( array( 'pending', 'on-hold' ) ) ) {
							$actions['processing'] = array(
								'url'       => mn_nonce_url( admin_url( 'admin-ajax.php?action=woocommerce_mark_order_status&status=processing&order_id=' . $post->ID ), 'woocommerce-mark-order-status' ),
						...
						if ( $the_order->has_status( array( 'pending', 'on-hold', 'processing' ) ) ) {
							$actions['complete'] = array(
								'url'       => mn_nonce_url( admin_url( 'admin-ajax.php?action=woocommerce_mark_order_status&status=completed&order_id=' . $post->ID ), 'woocommerce-mark-order-status' ),
	...
	public function product_sortable_columns( $columns ) {
		...
		return mn_parse_args( $custom, $columns );
	}
	 ...
	 *
	 * Based on Simple Page Ordering by 10up (https://mtaandao.co.ke/extend/plugins/simple-page-ordering/).
	 *
	 * @param  array $views
	 * @return array
	 */
	public function product_sorting_link( $views ) {
		global $post_type, $mn_query;

		if ( ! current_user_can( 'edit_others_pages' ) ) {
			return $views;
		}

		$class            = ( isset( $mn_query->query['orderby'] ) && $mn_query->query['orderby'] == 'menu_order title' ) ? 'current' : '';
	...
	public function bulk_and_quick_edit_save_post( $post_id, $post ) {
		...
		if ( mn_is_post_revision( $post_id ) || mn_is_post_autosave( $post_id ) ) {
			return $post_id;
		}
		...
		if ( isset( $_REQUEST['woocommerce_quick_edit_nonce'] ) && ! mn_verify_nonce( $_REQUEST['woocommerce_quick_edit_nonce'], 'woocommerce_quick_edit_nonce' ) ) {
			return $post_id;
		}
		if ( isset( $_REQUEST['woocommerce_bulk_edit_nonce'] ) && ! mn_verify_nonce( $_REQUEST['woocommerce_bulk_edit_nonce'], 'woocommerce_bulk_edit_nonce' ) ) {
			return $post_id;
		}
	...
	private function quick_edit_save( $post_id, $product ) {
		...
		if ( ! empty( $_REQUEST['_shipping_class'] ) ) {
			...
			mn_set_object_terms( $post_id, $shipping_class, 'product_shipping_class' );
		}
	 ...
	 *
	 * Using Javascript until Mtaandao core fixes: https://core.trac.wordpress.org/ticket/16031.
	 */
	...
	public function bulk_action() {
		$mn_list_table = _get_list_table( 'MN_Posts_List_Table' );
		$action        = $mn_list_table->current_action();
		...
		mn_redirect( esc_url_raw( $sendback ) );
		exit();
	}
	 ...
	 * @param MN_Query $mn
	 */
	public function shop_order_search_custom_fields( $mn ) {
		global $pagenow;

		if ( 'edit.php' != $pagenow || empty( $mn->query_vars['s'] ) || $mn->query_vars['post_type'] != 'shop_order' ) {
			return;
		}
		...
		if ( ! empty( $post_ids ) ) {
			...
			unset( $mn->query_vars['s'] );

			// so we know we're doing this.
			$mn->query_vars['shop_order_search'] = true;

			// Search by found posts.
			$mn->query_vars['post__in'] = array_merge( $post_ids, array( 0 ) );
		}
	}
	...
	public function product_filters() {
		...
		foreach ( $terms as $term ) {
			...
			if ( isset( $mn_query->query['product_type'] ) ) {
				$output .= selected( $term->slug, $mn_query->query['product_type'], false );
			}
			...
			if ( 'simple' == $term->name ) {
				...
				if ( isset( $mn_query->query['product_type'] ) ) {
					$output .= selected( 'downloadable', $mn_query->query['product_type'], false );
				}
				...
				if ( isset( $mn_query->query['product_type'] ) ) {
					$output .= selected( 'virtual', $mn_query->query['product_type'], false );
				}
	...
	public function request_query( $vars ) {
		global $typenow, $mn_query, $mn_post_statuses;
		...
		if ( 'product' === $typenow ) {
			...
			if ( ! isset( $vars['post_status'] ) ) {
				...
				foreach ( $post_statuses as $status => $value ) {
					if ( isset( $mn_post_statuses[ $status ] ) && false === $mn_post_statuses[ $status ]->show_in_admin_all_list ) {
	...
	public function product_search( $where ) {
		global $pagenow, $mndb, $mn;

		if ( 'edit.php' != $pagenow || ! is_search() || ! isset( $mn->query_vars['s'] ) || 'product' != $mn->query_vars['post_type'] ) {
			return $where;
		}

		$search_ids = array();
		$terms      = explode( ',', $mn->query_vars['s'] );
		...
		foreach ( $terms as $term ) {
			...
			$sku_to_id = $mndb->get_results( $mndb->prepare( "SELECT ID, post_parent FROM {$mndb->posts} LEFT JOIN {$mndb->postmeta} ON {$mndb->posts}.ID = {$mndb->postmeta}.post_id WHERE meta_key='_sku' AND meta_value LIKE %s;", '%' . $mndb->esc_like( wc_clean( $term ) ) . '%' ) );
			$sku_to_id = array_merge( mn_list_pluck( $sku_to_id, 'ID' ), mn_list_pluck( $sku_to_id, 'post_parent' ) );
		...
		if ( sizeof( $search_ids ) > 0 ) {
			$where = str_replace( 'AND (((', "AND ( ({$mndb->posts}.ID IN (" . implode( ',', $search_ids ) . ")) OR ((", $where );
		}
				...
				case 'product' :
					...
					if ( ! empty( $child_products ) ) {
						foreach ( $child_products as $child ) {
							...
							mn_update_post( $child_post );
						}
					}

					if ( $parent_id = mn_get_post_parent_id( $id ) ) {
						wc_delete_product_transients( $parent_id );
					}

				break;
				case 'product_variation' :
					wc_delete_product_transients( mn_get_post_parent_id( $id ) );
				break;
				case 'shop_order' :
					$refunds = $mndb->get_results( $mndb->prepare( "SELECT ID FROM $mndb->posts WHERE post_type = 'shop_order_refund' AND post_parent = %d", $id ) );

					if ( ! is_null( $refunds ) ) {
						foreach ( $refunds as $refund ) {
							mn_delete_post( $refund->ID, true );
						}
					}
				break;
			}
	...
	public function trash_post( $id ) {
		...
		if ( $id > 0 ) {
			...
			if ( in_array( $post_type, wc_get_order_types( 'order-count' ) ) ) {
				...
				foreach ( $refunds as $refund ) {
					$mndb->update( $mndb->posts, array( 'post_status' => 'trash' ), array( 'ID' => $refund->ID ) );
				}
	...
	public function untrash_post( $id ) {
		...
		if ( $id > 0 ) {
			...
			if ( in_array( $post_type, wc_get_order_types( 'order-count' ) ) ) {
				...
				foreach ( $refunds as $refund ) {
					$mndb->update( $mndb->posts, array( 'post_status' => 'wc-completed' ), array( 'ID' => $refund->ID ) );
				}
	...
	public function delete_order_items( $postid ) {
		...
		if ( in_array( get_post_type( $postid ), wc_get_order_types() ) ) {
			...
			$mndb->query( "
				DELETE {$mndb->prefix}woocommerce_order_items, {$mndb->prefix}woocommerce_order_itemmeta
				FROM {$mndb->prefix}woocommerce_order_items
				JOIN {$mndb->prefix}woocommerce_order_itemmeta ON {$mndb->prefix}woocommerce_order_items.order_item_id = {$mndb->prefix}woocommerce_order_itemmeta.order_item_id
				WHERE {$mndb->prefix}woocommerce_order_items.order_id = '{$postid}';
				" );
	...
	public function delete_order_downloadable_permissions( $postid ) {
		...
		if ( in_array( get_post_type( $postid ), wc_get_order_types() ) ) {
			...
			$mndb->query( $mndb->prepare( "
				DELETE FROM {$mndb->prefix}woocommerce_downloadable_product_permissions
				WHERE order_id = %d
			", $postid ) );
	...
	public function process_product_file_download_paths( $product_id, $variation_id, $downloadable_files ) {
		...
		if ( ! empty( $new_download_ids ) || ! empty( $removed_download_ids ) ) {
			...
			$existing_permissions = $mndb->get_results( $mndb->prepare( "SELECT * from {$mndb->prefix}woocommerce_downloadable_product_permissions WHERE product_id = %d GROUP BY order_id", $product_id ) );
					...
					if ( ! empty( $removed_download_ids ) ) {
						foreach ( $removed_download_ids as $download_id ) {
							if ( apply_filters( 'woocommerce_process_product_file_download_paths_remove_access_to_old_file', true, $download_id, $product_id, $order ) ) {
								$mndb->query( $mndb->prepare( "DELETE FROM {$mndb->prefix}woocommerce_downloadable_product_permissions WHERE order_id = %d AND product_id = %d AND download_id = %s", $order->id, $product_id, $download_id ) );
							}
						}
					}
					// Add permissions
					if ( ! empty( $new_download_ids ) ) {
						...
						foreach ( $new_download_ids as $download_id ) {
							...
							if ( apply_filters( 'woocommerce_process_product_file_download_paths_grant_access_to_new_file', true, $download_id, $product_id, $order ) ) {
								...
								if ( ! $mndb->get_var( $mndb->prepare( "SELECT 1=1 FROM {$mndb->prefix}woocommerce_downloadable_product_permissions WHERE order_id = %d AND product_id = %d AND download_id = %s", $order->id, $product_id, $download_id ) ) ) {
	...
	public function disable_dfw_feature_pointer() {
		...
		if ( $screen && 'product' === $screen->id && 'post' === $screen->base ) {
			remove_action( 'admin_print_footer_scripts', array( 'MN_Internal_Pointers', 'pointer_mn410_dfw' ) );
		}
	}
	...
	public function maybe_render_blank_state( $which ) {
		...
		if ( in_array( $post_type, array( 'shop_order', 'product', 'shop_coupon' ) ) && 'bottom' === $which ) {
			$counts = (array) mn_count_posts( $post_type );
			...
			echo '<style type="text/css">#posts-filter .mn-list-table, #posts-filter .tablenav.top, .wrap .subsubsub  { display: none; } </style></div>';
		}
	}
}
