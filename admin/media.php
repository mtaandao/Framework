...
case 'editattachment' :
	...
	if ( !current_user_can('edit_post', $attachment_id) )
		mn_die ( __('Sorry, you are not allowed to edit this attachment.') );

	$errors = media_upload_form_handler();

	if ( empty($errors) ) {
		$location = 'media.php';
		if ( $referer = mn_get_original_referer() ) {
...
case 'edit' :
	...
	if ( empty( $_GET['attachment_id'] ) ) {
		mn_redirect( admin_url('upload.php') );
		exit();
	}
	...
	if ( empty($att->ID) ) mn_die( __('You attempted to edit an attachment that doesn&#8217;t exist. Perhaps it was deleted?') );
	if ( 'attachment' !== $att->post_type ) mn_die( __('You attempted to edit an item that isn&#8217;t an attachment. Please go back and try again.') );
	if ( $att->post_status == 'trash' ) mn_die( __('You can&#8217;t edit this attachment because it is in the Trash. Please move it out of the Trash and try again.') );
...
<?php mn_original_referer_field(true, 'previous'); ?>
