 ...
 * @since 3.4.0
 */
final class MN_Theme implements ArrayAccess {
	...
	private static $default_themes = array(
		'classic'        => 'Mtaandao Classic',
		'default'        => 'Mtaandao Default',
	 ...
	 *
	 * Default is false. Can be set with the {@see 'mn_cache_themes_persistently'} filter.
	 *
	 * @static
	 * @access private
	 * @var bool
	...
	/**
	 * Constructor for MN_Theme.
	 *
	 * @global array $mn_theme_directories
	 *
	 * @param string $theme_dir Directory of the theme within the theme_root.
	 * @param string $theme_root Theme root.
	 * @param MN_Error|void $_child If this theme is a parent theme, the child may be passed for validation purposes.
	 */
	public function __construct( $theme_dir, $theme_root, $_child = null ) {
		...
		if ( ! isset( self::$persistently_cache ) ) {
			...
			self::$persistently_cache = apply_filters( 'mn_cache_themes_persistently', false, 'MN_Theme' );
			if ( self::$persistently_cache ) {
				mn_cache_add_global_groups( 'themes' );
				if ( is_int( self::$persistently_cache ) )
					self::$cache_expiration = self::$persistently_cache;
			} else {
				mn_cache_add_non_persistent_groups( 'themes' );
			}
		}
		...
		if ( ! in_array( $theme_root, (array) $mn_theme_directories ) && in_array( dirname( $theme_root ), (array) $mn_theme_directories ) ) {
		...
		if ( is_array( $cache ) ) {
			...
			if ( ! file_exists( $this->theme_root . '/' . $this->stylesheet ) )
				$this->errors = new MN_Error( 'theme_not_found', sprintf( __( 'The theme directory "%s" does not exist.' ), esc_html( $this->stylesheet ) ) );
			else
				$this->errors = new MN_Error( 'theme_no_stylesheet', __( 'Stylesheet is missing.' ) );
			...
			$this->headers['Name'] = $this->stylesheet;
			$this->errors = new MN_Error( 'theme_stylesheet_not_readable', __( 'Stylesheet is not readable.' ) );
			$this->template = $this->stylesheet;
			$this->cache_add( 'theme', array( 'headers' => $this->headers, 'errors' => $this->errors, 'stylesheet' => $this->stylesheet, 'template' => $this->template ) );
			return;
		} else {
		...
		if ( ! $this->template && ! ( $this->template = $this->headers['Template'] ) ) {
			...
			if ( ! file_exists( $this->theme_root . '/' . $this->stylesheet . '/index.php' ) ) {
				...
				$this->errors = new MN_Error( 'theme_no_index', $error_message );
				$this->cache_add( 'theme', array( 'headers' => $this->headers, 'errors' => $this->errors, 'stylesheet' => $this->stylesheet, 'template' => $this->template ) );
				return;
			}
		}
		...
		if ( ! is_array( $cache ) && $this->template != $this->stylesheet && ! file_exists( $this->theme_root . '/' . $this->template . '/index.php' ) ) {
			...
			if ( '.' != $parent_dir && file_exists( $this->theme_root . '/' . $parent_dir . '/' . $this->template . '/index.php' ) ) {
				...
				$this->errors = new MN_Error( 'theme_no_parent', sprintf( __( 'The parent theme is missing. Please install the "%s" parent theme.' ), esc_html( $this->template ) ) );
				$this->cache_add( 'theme', array( 'headers' => $this->headers, 'errors' => $this->errors, 'stylesheet' => $this->stylesheet, 'template' => $this->template ) );
				$this->parent = new MN_Theme( $this->template, $this->theme_root, $this );
				return;
			}
		}
		...
		if ( $this->template != $this->stylesheet ) {
			...
			if ( $_child instanceof MN_Theme && $_child->template == $this->stylesheet ) {
				$_child->parent = null;
				$_child->errors = new MN_Error( 'theme_parent_invalid', sprintf( __( 'The "%s" theme is not a valid parent theme.' ), esc_html( $_child->template ) ) );
				$_child->cache_add( 'theme', array( 'headers' => $_child->headers, 'errors' => $_child->errors, 'stylesheet' => $_child->stylesheet, 'template' => $_child->template ) );
				// The two themes actually reference each other with the Template header.
				if ( $_child->stylesheet == $this->template ) {
					$this->errors = new MN_Error( 'theme_parent_invalid', sprintf( __( 'The "%s" theme is not a valid parent theme.' ), esc_html( $this->template ) ) );
					$this->cache_add( 'theme', array( 'headers' => $this->headers, 'errors' => $this->errors, 'stylesheet' => $this->stylesheet, 'template' => $this->template ) );
				}
				return;
			}
			// Set the parent. Pass the current instance so we can do the crazy checks above and assess errors.
			$this->parent = new MN_Theme( $this->template, isset( $theme_root_template ) ? $theme_root_template : $this->theme_root, $this );
		}
	 ...
	 * @access public
	 *
	 * @return MN_Error|false MN_Error if there are errors, or false.
	 */
	public function errors() {
		return is_mn_error( $this->errors ) ? $this->errors : false;
	}
	 ...
	 * @access public
	 *
	 * @return MN_Theme|false Parent theme, or false if the current theme is not a child theme.
	 */
	 ...
	 * @return bool Return value from mn_cache_add()
	 */
	private function cache_add( $key, $data ) {
		return mn_cache_add( $key . '-' . $this->cache_hash, $data, 'themes', self::$cache_expiration );
	}
	...
	private function cache_get( $key ) {
		return mn_cache_get( $key . '-' . $this->cache_hash, 'themes' );
	}
	...
	public function cache_delete() {
		foreach ( array( 'theme', 'screenshot', 'headers', 'page_templates' ) as $key )
			mn_cache_delete( $key . '-' . $this->cache_hash, 'themes' );
	 ...
	 * get_template() takes into account where Mtaandao actually located the theme and
	...
	private function sanitize_header( $header, $value ) {
		switch ( $header ) {
			...
			case 'Name' :
				...
				$value = mn_kses( $value, $header_tags );
				break;
			case 'Author' :
			...
			case 'Description' :
				...
				$value = mn_kses( $value, $header_tags_with_a );
				break;
			case 'ThemeURI' :
			case 'AuthorURI' :
	...
	private function translate_header( $header, $value ) {
		switch ( $header ) {
			...
			case 'Tags' :
				...
				if ( ! isset( $tags_list ) ) {
					$tags_list = array(
						// As of 16.10, deprecated tags which are only used to provide translation for older themes.
	 ...
	 * for all other URLs returned by MN_Theme, so we pass it to the public function
	...
	public function get_page_templates( $post = null ) {
		 ...
		 * @param MN_Theme     $this           The theme object.
		 * @param MN_Post|null $post           The post being edited, provided for context, or null.
		 */
	...
	/**
	 * Determines the latest Mtaandao default theme that is installed.
	 *
	 * This hits the filesystem.
	 *
	 * @return MN_Theme|false Object, or false if no theme is installed, which would be bad.
	 */
	public static function get_core_default_theme() {
		foreach ( array_reverse( self::$default_themes ) as $slug => $name ) {
			$theme = mn_get_theme( $slug );
			if ( $theme->exists() ) {
				return $theme;
			}
		}
	...
	public static function get_allowed_on_site( $blog_id = null ) {
		...
		/** This filter is documented in res/class-mn-theme.php */
		return (array) apply_filters( 'site_allowed_themes', $allowed_themes[ $blog_id ], $blog_id );
	}
	...
	public static function sort_by_name( &$themes ) {
		if ( 0 === strpos( get_locale(), 'en_' ) ) {
			uasort( $themes, array( 'MN_Theme', '_name_sort' ) );
		} else {
			uasort( $themes, array( 'MN_Theme', '_name_sort_i18n' ) );
		}
	}
