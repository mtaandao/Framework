<?php
/**
 * List Table API: MN_Media_List_Table class
 *
 * @package Mtaandao
 ...
 * @see MN_List_Table
 */
class MN_Media_List_Table extends MN_List_Table {
	/**
	...
	public function prepare_items() {
		global $mn_query, $post_mime_types, $avail_post_mime_types, $mode;

		list( $post_mime_types, $avail_post_mime_types ) = mn_edit_attachments_query( $_REQUEST );
		...
		foreach ( $post_mime_types as $mime_type => $label ) {
			...
			$selected = selected(
				$filter && 0 === strpos( $filter, 'post_mime_type:' ) &&
					mn_match_mime_types( $mime_type, str_replace( 'post_mime_type:', '', $filter ) ),
				true,
				false
			);
...
<?php
		if ( ! is_singular() ) {
			...
			/** This action is documented in admin/includes/class-mn-posts-list-table.php */
...
?>
	...
	public function column_title( $post ) {
		...
		$thumb = mn_get_attachment_image( $post->ID, array( 60, 60 ), true, array( 'alt' => '' ) );
		$link_start = $link_end = '';
		...
		<p class="filename">
			...
			echo esc_html( mn_basename( $file ) );
			?>
		</p>
		<?php
	}
...
?>
			...
			if ( $user_can_edit ):
				$detach_url = add_query_arg( array(
					...
					'media[]' => $post->ID,
					'_mnnonce' => mn_create_nonce( 'bulk-' . $this->_args['plural'] )
				), 'upload.php' );
				printf(
	...
	public function display_rows() {
		...
		$post_ids = mn_list_pluck( $mn_query->posts, 'ID' );
		reset( $mn_query->posts );
	...
	private function _get_row_actions( $post, $att_title ) {
			...
			if ( current_user_can( 'delete_post', $post->ID ) ) {
				if ( EMPTY_TRASH_DAYS && MEDIA_TRASH ) {
					$actions['trash'] = sprintf(
						...
						mn_nonce_url( "post.php?action=trash&amp;post=$post->ID", 'trash-post_' . $post->ID ),
						/* translators: %s: attachment title */
					...
					$actions['delete'] = sprintf(
						...
						mn_nonce_url( "post.php?action=delete&amp;post=$post->ID", 'delete-post_' . $post->ID ),
						$delete_ays,
						/* translators: %s: attachment title */
			...
			if ( current_user_can( 'delete_post', $post->ID ) ) {
				if ( $this->is_trash ) {
					$actions['untrash'] = sprintf(
						'<a href="%s" class="submitdelete aria-button-if-js" aria-label="%s">%s</a>',
						mn_nonce_url( "post.php?action=untrash&amp;post=$post->ID", 'untrash-post_' . $post->ID ),
						/* translators: %s: attachment title */
		 ...
		 * @param MN_Post $post     MN_Post object for the current attachment.
		 * @param bool    $detached Whether the list table contains media not attached
		 *                          to any posts. Default true.
		 */
