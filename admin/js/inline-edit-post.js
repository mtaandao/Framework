				...
				if (r) {
					if ( -1 !== r.indexOf( '<tr' ) ) {
						...
						$( inlineEditPost.what + id ).hide().fadeIn( 400, function() {
							...
							mn.a11y.speak( inlineEditL10n.saved );
						});
					} else {
						r = r.replace( /<.[^<>]*?>/g, '' );
						$errorSpan.html( r ).show();
						mn.a11y.speak( $errorSpan.text() );
					}
				} else {
					$errorSpan.html( inlineEditL10n.error ).show();
					mn.a11y.speak( inlineEditL10n.error );
				}
			},
		'html');
...
$( document ).on( 'heartbeat-tick.mn-check-locked-posts', function( e, data ) {
	var locked = data['mn-check-locked-posts'] || {};

	$('#the-list tr').each( function(i, el) {
		var key = el.id, row = $(el), lock_data, avatar;

		if ( locked.hasOwnProperty( key ) ) {
			if ( ! row.hasClass('mn-locked') ) {
				...
				row.addClass('mn-locked');
			}
		} else if ( row.hasClass('mn-locked') ) {
			// Make room for the CSS animation
			row.removeClass('mn-locked').delay(1000).find('.locked-info span').empty();
		}
	});
}).on( 'heartbeat-send.mn-check-locked-posts', function( e, data ) {
	var check = [];
	...
	if ( check.length ) {
		data['mn-check-locked-posts'] = check;
	}
}).ready( function() {
	// Set the heartbeat interval to 15 sec.
	if ( typeof mn !== 'undefined' && mn.heartbeat ) {
