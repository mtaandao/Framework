 ...
 * @since 3.9.0
 */
function mn_underscore_audio_template() {
	$audio_types = mn_get_audio_extensions();
?>
...
>
	...
	<source src="{{ data.model.src }}" type="{{ mn.media.view.settings.embedMimes[ data.model.src.split('.').pop() ] }}" />
	<# } #>

	<?php foreach ( $audio_types as $type ):
	?><# if ( ! _.isEmpty( data.model.<?php echo $type ?> ) ) { #>
	<source src="{{ data.model.<?php echo $type ?> }}" type="{{ mn.media.view.settings.embedMimes[ '<?php echo $type ?>' ] }}" />
	<# } #>
	<?php endforeach;
?></audio>
<?php
 ...
 * @since 3.9.0
 */
function mn_underscore_video_template() {
	$video_types = mn_get_video_extensions();
?>
<#  var w_rule = '', classes = [],
		w, h, settings = mn.media.view.settings,
...
#>
<div style="{{ w_rule }}" class="mn-video">
<video controls
	class="mn-video-shortcode {{ classes.join( ' ' ) }}"
	<# if ( w ) { #>width="{{ w }}"<# } #>
	<# if ( h ) { #>height="{{ h }}"<# } #>
	<?php
 ...
 * @global bool $is_IE
 */
function mn_print_media_templates() {
	global $is_IE;
	$class = 'media-modal mn-core-ui';
	...
	<script type="text/html" id="tmpl-uploader-inline">
		...
		<?php if ( ! _device_can_upload() ) : ?>
			<h2 class="upload-instructions"><?php printf( __( 'The web browser on your device cannot be used to upload files. You may be able to use the <a href="%s">native app for your device</a> instead.' ), 'https://apps.mtaandao.co.ke/' ); ?></h2>
	...
	<script type="text/html" id="tmpl-attachment-details-two-column">
		<div class="attachment-media-view {{ data.orientation }}">
			<div class="thumbnail thumbnail-{{ data.type }}">
				...
				<# if ( 'audio' === data.type ) { #>
				<div class="mn-media-wrapper">
					<audio style="visibility: hidden" controls class="mn-audio-shortcode" width="100%" preload="none">
				...
				<# } else if ( 'video' === data.type ) {
					...
					} else if ( mn.media.view.settings.contentWidth ) {
						w_rule = 'width: ' + mn.media.view.settings.contentWidth + 'px;';
					}
				#>
				<div style="{{ w_rule }}" class="mn-media-wrapper mn-video">
					<video controls="controls" class="mn-video-shortcode" preload="metadata"
	...
	<script type="text/html" id="tmpl-gallery-settings">
		...
		<label class="setting">
			...
			<select class="link-to"
				...
				<option value="post" <# if ( ! mn.media.galleryDefaults.link || 'post' == mn.media.galleryDefaults.link ) {
					#>selected="selected"<# }
				#>>
					<?php esc_attr_e('Attachment Page'); ?>
				</option>
				<option value="file" <# if ( 'file' == mn.media.galleryDefaults.link ) { #>selected="selected"<# } #>>
					<?php esc_attr_e('Media File'); ?>
				</option>
				<option value="none" <# if ( 'none' == mn.media.galleryDefaults.link ) { #>selected="selected"<# } #>>
					<?php esc_attr_e('None'); ?>
				</option>
			</select>
		</label>
			...
			<select class="columns" name="columns"
				...
				<?php for ( $i = 1; $i <= 9; $i++ ) : ?>
					<option value="<?php echo esc_attr( $i ); ?>" <#
						if ( <?php echo $i ?> == mn.media.galleryDefaults.columns ) { #>selected="selected"<# }
					#>>
						<?php echo esc_html( $i ); ?>
					</option>
				<?php endfor; ?>
	...
	<script type="text/html" id="tmpl-audio-details">
		<# var ext, html5types = {
			mp3: mn.media.view.settings.embedMimes.mp3,
			ogg: mn.media.view.settings.embedMimes.ogg
		}; #>

		<?php $audio_types = mn_get_audio_extensions(); ?>
		<div class="media-embed media-embed-details">
			<div class="embed-media-settings embed-audio-settings">
				<?php mn_underscore_audio_template() ?>
	...
	<script type="text/html" id="tmpl-video-details">
		<# var ext, html5types = {
			mp4: mn.media.view.settings.embedMimes.mp4,
			ogv: mn.media.view.settings.embedMimes.ogv,
			webm: mn.media.view.settings.embedMimes.webm
		}; #>

		<?php $video_types = mn_get_video_extensions(); ?>
		<div class="media-embed media-embed-details">
			<div class="embed-media-settings embed-video-settings">
				<div class="mn-video-holder">
				<#
				...
				<?php mn_underscore_video_template() ?>
				...
				<# _.each( data.attachments, function( attachment, index ) { #>
					<dl class="gallery-item">
						...
						<# if ( attachment.caption ) { #>
							<dd class="mn-caption-text gallery-caption">
