...
abstract class WC_Settings_API {
	...
	/**
	 * Return the name of the option in the MN DB.
	 * @since 2.6.0
	 * @return string
	 */
	...
	public function init_settings() {
		...
		if ( ! is_array( $this->settings ) ) {
			...
			$this->settings = array_merge( array_fill_keys( array_keys( $form_fields ), '' ), mn_list_pluck( $form_fields, 'default' ) );
		}
	}
	...
	public function get_description_html( $data ) {
		...
		return $description ? '<p class="description">' . mn_kses_post( $description ) . '</p>' . "\n" : '';
	}
	...
	public function generate_text_html( $key, $data ) {
		...
		$data = mn_parse_args( $data, $defaults );

		ob_start();
		?>
		<tr valign="top">
			<th scope="row" class="titledesc">
				<label for="<?php echo esc_attr( $field_key ); ?>"><?php echo mn_kses_post( $data['title'] ); ?></label>
				<?php echo $this->get_tooltip_html( $data ); ?>
			</th>
			<td class="forminp">
				<fieldset>
					<legend class="screen-reader-text"><span><?php echo mn_kses_post( $data['title'] ); ?></span></legend>
		...
		<tr valign="top">
			...
			<td class="forminp">
				<fieldset>
					...
					<input <?php disabled( $data['disabled'], true ); ?> class="<?php echo esc_attr( $data['class'] ); ?>" type="checkbox" name="<?php echo esc_attr( $field_key ); ?>" id="<?php echo esc_attr( $field_key ); ?>" style="<?php echo esc_attr( $data['css'] ); ?>" value="1" <?php checked( $this->get_option( $key ), 'yes' ); ?> <?php echo $this->get_custom_attribute_html( $data ); ?> /> <?php echo mn_kses_post( $data['label'] ); ?></label><br/>
					<?php echo $this->get_description_html( $data ); ?>
				</fieldset>
			</td>
		</tr>
	...
	public function generate_title_html( $key, $data ) {
		...
		?>
			...
			<h3 class="wc-settings-sub-title <?php echo esc_attr( $data['class'] ); ?>" id="<?php echo esc_attr( $field_key ); ?>"><?php echo mn_kses_post( $data['title'] ); ?></h3>
			<?php if ( ! empty( $data['description'] ) ) : ?>
				<p><?php echo mn_kses_post( $data['description'] ); ?></p>
			<?php endif; ?>
	...
	public function validate_text_field( $key, $value ) {
		...
		return mn_kses_post( trim( stripslashes( $value ) ) );
	}
	...
	public function validate_textarea_field( $key, $value ) {
		$value = is_null( $value ) ? '' : $value;
		return mn_kses( trim( stripslashes( $value ) ),
			array_merge(
				array(
					'iframe' => array( 'src' => true, 'style' => true, 'id' => true, 'class' => true )
				),
				mn_kses_allowed_html( 'post' )
			)
		);
	}
